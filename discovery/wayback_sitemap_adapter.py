...
async def _validate_and_prepare(
    domain_or_url: str, max_results: int, timeout_s: float
) -> tuple[DiscoveryBatchResult | None, str | None, float | None, float | None]:
...
async def _fetch_sitemapindex(
    domain: str, timeout_s: float
) -> tuple[list[str] | None, float, DiscoveryBatchResult | None]:
...
async def _fetch_sitemaps(
    sitemap_urls: list[str], per_sitemap_timeout: float, remaining_timeout: float
) -> tuple[list[tuple[str, list[str]]], float]:
...
def _aggregate_hits(
    ...
    for task_result in sitemap_results:
        ...
        for url in urls:
            ...
            hit = _build_hit(
                query=raw_input, url=snapshot_url, snippet=snippet, retrieved_ts=now_ts, rank=len(hits_list)
            )
...
async def async_search_wayback_sitemap(
    domain_or_url: str, max_results: int = 50, timeout_s: float = _DEFAULT_TIMEOUT_S
) -> DiscoveryBatchResult:
    """
    ...
    """
    error_result, domain, max_results, remaining_timeout = await _validate_and_prepare(
        domain_or_url, max_results, timeout_s
    )
    if error_result:
        return error_result
    ...
    sitemap_results, fetch_elapsed = await _fetch_sitemaps(
        sitemap_urls, per_sitemap_timeout, remaining_timeout - elapsed
    )
