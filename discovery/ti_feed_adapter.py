...
async def _generic_post_feed(
    ...
    *,
    ...
    try:
        s = await async_get_httpx_session()
        resp, err = await checked_httpx_post(
            s, url, json=payload or {}, timeout=httpx.Timeout(timeout), failure_kind=source_name
        )
        if err:
            logger.debug("[%s] %s", source_name, err)
            return []
...
async def fetch_threatfox(days: int = 1) -> list[dict]:
    ...
    try:
        s = await async_get_httpx_session()
        data, status, err = await checked_httpx_post(
            s,
            "https://threatfox-api.abuse.ch/api/v1/",
            json={"query": "get_iocs", "days": days},
            timeout=httpx.Timeout(float(os.environ.get("HLEDAC_FEED_TIMEOUT", "45")), connect=10.0),
...
async def fetch_feodo_c2() -> list[dict]:
    ...
    try:
        s = await async_get_httpx_session()
        data, status, err = await checked_httpx_get(
            s,
...
async def query_circl_pdns(domain: str, max_results: int = 50) -> list[dict]:
    ...
    try:
        s = await async_get_httpx_session()
        text, status, err = await checked_httpx_get(
            s, f"https://www.circl.lu/pdns/query/{domain}", timeout=httpx.Timeout(15), failure_kind="circl_pdns"
        )
        if err:
...
async def enrich_ip_internetdb(ip: str) -> dict:
    ...
    try:
        s = await async_get_httpx_session()
        data, status, err = await checked_httpx_get(
            s, f"https://internetdb.shodan.io/{ip}", timeout=httpx.Timeout(8), failure_kind="shodan_internetdb"
        )
        if err:
...
async def enrich_findings_greynoise_community(
    session: httpx.AsyncClient, findings: list[dict], max_lookups: int = 40
) -> list[dict]:
    """
...
def _parse_pastebin_urls(html_text: str, max_pastes: int = 10) -> list[str]:
    ...
    pattern = re.compile(
        r'<td[^>]*>\s*<a[^>]+href=["\'](/raw/[^"\']+)["\'][^>]*>[^<]*</a>\s*</td>', re.DOTALL | re.IGNORECASE
    )
    paste_urls = []
...
async def scrape_pastebin_for_keyword(keyword: str, max_pastes: int = 10) -> list[dict]:
    ...
    try:
        s = await async_get_httpx_session()
        text, status, err = await checked_httpx_get(
            s, "https://pastebin.com/archive", timeout=httpx.Timeout(10), failure_kind="pastebin_archive"
        )
        if err:
        ...
        for raw_url, fetch_result in zip(paste_urls, batch_results, strict=False):
            try:
    ...
    async def fetch_archives_for_url(self, url: str, limit: int | None = None) -> tuple[NormalizedEntry, ...]:
        ...
        try:
            ...
            async with asyncio.timeout(self.TIMEOUT_PER_SOURCE):
                results_dict: dict[str, list[ArchiveResult]] = (
                    await discovery.search_url(url, sources=["wayback", "archive_today"], limit_per_source=limit),
                )
...
async def _handle_domain_to_pdns(task, scheduler) -> None:
...
async def _handle_domain_to_ct(task, scheduler) -> None:
...
async def _handle_ct_live_monitor(task, scheduler) -> None:
...
async def _handle_multi_engine_search(task, scheduler) -> None:
...
async def _handle_github_dork(task, scheduler) -> None:
...
async def _handle_rdap_lookup(task, scheduler) -> None:
...
async def _handle_ahmia_search(task, scheduler) -> None:
...
async def _handle_paste_keyword_search(task, scheduler) -> None:
...
async def _handle_wayback_search(task, scheduler) -> None:
...
async def _handle_commoncrawl_search(task, scheduler) -> None:
...
async def fetch_i2p_eepsite(url: str, proxy_url: str = "http://127.0.0.1:4444") -> dict:
    ...
    try:
        ...
        resp = await session.get(
            url, timeout=httpx.Timeout(60), headers={"User-Agent": "Mozilla/5.0 (compatible; research)"}
        )
        content = resp.text
        return {
            "url": url,
...
async def _handle_i2p_eepsite_fetch(task, scheduler) -> None:
...
async def fetch_ipfs_cid(cid: str) -> dict:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return {
                "cid": cid,
                "source": "ipfs_local_daemon",
                "content": data[:100000].decode("utf-8", errors="replace"),
                "size": len(data),
                "error": None,
            }
    except Exception as e:
...
async def search_ipfs(query: str, max_results: int = 10) -> list[dict]:
    ...
    try:
        s = await async_get_httpx_session()
        resp = await s.get(
            "https://api.ipfs-search.com/v1/search", params={"q": query, "type": "any"}, timeout=httpx.Timeout(15)
        )
        if resp.status_code == 200:
            data = resp.json()
...
async def _handle_ipfs_fetch(task, scheduler) -> None:
    ...
    if m:
        ...
        if scheduler._duckdb_store is not None and content:
            ...
            finding = CanonicalFinding(
                ...
                payload_text=content[:2000] if content else None,
            )
            findings.append(finding)
    else:
            ...
            items.append(
                {
                    ...
                    "port": int(parts[3]) if len(parts) > 3 and parts[3].strip().isdigit() else 70,
                }
            )
    return items
...
async def _handle_gopher_fetch(task, scheduler) -> None:
...
async def query_ripe_stat_asn(ip: str) -> dict:
    ...
    try:
        s = await async_get_httpx_session()
        resp = await s.get(
            "https://stat.ripe.net/data/prefix-overview/data.json", params={"resource": ip}, timeout=httpx.Timeout(15)
        )
        if resp.status_code == 200:
...
async def query_team_cymru_asn(ip: str) -> dict:
    ...
    try:
        proc = await asyncio.create_subprocess_exec(
            "nslookup", "-type=TXT", query_name, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
        async with asyncio.timeout(5.0):
...
async def _handle_bgp_asn_lookup(task, scheduler) -> None:
...
async def _handle_bgp_routing_history(task, scheduler) -> None:
    ...
    def bgp_callback(timestamp: float, prefix: str, as_path: str, event_type: str) -> None:
        pass
...
async def _handle_malwarebazaar_search(task, scheduler) -> None:
...
async def _handle_cve_to_github(task, scheduler) -> None:
    """
