...
def _result_for_replay_hit(domain_norm: str, cached: dict, start: float) -> tuple[DiscoveryBatchResult, PDNSOutcome]:
    ...
    result = _make_pdns_result(
        tuple(cached_hits) if isinstance(cached_hits, list) else cached_hits, None, "replay_hit", elapsed
    )
    return (result, outcome)
...
def _handle_pdns_network_error(
    domain_norm: str, err: str, elapsed: float, cooldown_now: float
) -> tuple[DiscoveryBatchResult, PDNSOutcome]:
...
def _handle_pdns_success(
    domain_norm: str, text: str, domain: str, start: float, cooldown_now: float
) -> tuple[DiscoveryBatchResult, PDNSOutcome]:
...
def _check_early_exit(
    domain: str, domain_norm: str, cooldown_now: float, start: float
) -> tuple[DiscoveryBatchResult, PDNSOutcome] | None:
...
async def _execute_pdns_request(
    domain_norm: str, domain: str, timeout_s: float, cooldown_now: float, start: float
) -> tuple[DiscoveryBatchResult, PDNSOutcome]:
