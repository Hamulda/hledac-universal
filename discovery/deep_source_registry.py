...
class DeepSource(Struct, frozen=True):
    ...
    def __post_init__(self) -> None:
        if not MIN_RELIABILITY <= self.reliability <= MAX_RELIABILITY:
            raise ValueError(
                f"DeepSource.reliability must be in [{MIN_RELIABILITY}, {MAX_RELIABILITY}], got {self.reliability} for {self.source_id}"
            )
        ...
        if _onion_tor_required(self.base_url) and self.transport_required != "tor":
            raise ValueError(
                f"DeepSource {self.source_id}: .onion URL requires transport_required='tor', got {self.transport_required!r}"
            )
        if _i2p_required(self.base_url) and self.transport_required != "i2p":
            raise ValueError(
                f"DeepSource {self.source_id}: .i2p URL requires transport_required='i2p', got {self.transport_required!r}"
            )
...
_CURATED_SOURCES: tuple[tuple[str, str, SourceTier, TransportRequired, DataType, float], ...] = (
    ...
    ("Facebook CT", "https://ct.facebook.com/", "surface", "none", "ct_logs", 0.9),
    ("Google Pilot CT", "https://ct.googleapis.com/logs/eu1/xenon2025h1/", "surface", "none", "ct_logs", 0.9),
    ("Google Argon CT", "https://ct.googleapis.com/logs/eu1/argon2025h1/", "surface", "none", "ct_logs", 0.9),
    ...
    ("CIRCL pDNS", "https://www.circl.lu/services/passive-dns/", "surface", "none", "passive_dns", 0.9),
    ("Robtex pDNS", "https://passive.robtex.com/", "surface", "none", "passive_dns", 0.8),
    ...
    ("CommonCrawl Index", "https://index.commoncrawl.org/", "surface", "none", "repo", 0.9),
    ("archive.is", "https://archive.is/", "surface", "none", "repo", 0.85),
    ("CachedView", "https://cachedview.nl/", "surface", "none", "repo", 0.65),
    ("Mementos Web", "http://timetravel.mementoweb.org/", "surface", "none", "repo", 0.8),
    ("Pastebin (scraping)", "https://pastebin.com/", "surface", "none", "paste", 0.75),
    ("Ghostbin", "https://ghostbin.org/", "surface", "none", "paste", 0.6),
    ("Privatebin (known instance)", "https://privatebin.net/", "surface", "none", "paste", 0.65),
    ("Throwbin", "https://throwbin.io/", "surface", "none", "paste", 0.5),
    ("Rentry", "https://rentry.co/", "surface", "none", "paste", 0.7),
    ...
    ("OpenAlex", "https://api.openalex.org/", "academic", "none", "academic", 0.9),
    ("PubMed E-utilities", "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/", "academic", "none", "academic", 0.95),
    ("grep.app", "https://grep.app/api/search?", "surface", "none", "repo", 0.9),
    ("Sourcegraph public", "https://sourcegraph.com/.api/search/stream?", "surface", "none", "repo", 0.9),
    ("GitHub code search", "https://api.github.com/search/code?", "surface", "none", "repo", 0.95),
    ("GitLab search", "https://gitlab.com/api/v4/search?", "surface", "none", "repo", 0.8),
    ("Codeberg search", "https://codeberg.org/api/v1/repos/search?", "surface", "none", "repo", 0.7),
    ("searchcode", "https://searchcode.com/api/codesearch_I/", "surface", "none", "repo", 0.75),
    ("Have I Been Pwned", "https://haveibeenpwned.com/api/v3/", "surface", "none", "leak_db", 0.9),
    ("DeHashed (gated)", "https://dehashed.com/", "surface", "none", "leak_db", 0.85),
    ("LeakCheck", "https://leakcheck.io/", "surface", "none", "leak_db", 0.7),
    ("IntelligenceX", "https://intelx.io/", "surface", "none", "leak_db", 0.85),
    ("Leak-Lookup", "https://leak-lookup.com/", "surface", "none", "leak_db", 0.6),
    ("Ahmia.fi (Tor search mirror)", "https://ahmia.fi/", "surface", "none", "forum", 0.8),
    ("dark.fail", "https://dark.fail/", "surface", "none", "forum", 0.75),
    ("onion.live", "https://onion.live/", "surface", "none", "forum", 0.6),
    ...
    ("IPFS public gateway (ipfs.io)", "https://ipfs.io/", "p2p", "none", "repo", 0.9),
    ("IPFS gateway (dweb.link)", "https://dweb.link/", "p2p", "none", "repo", 0.85),
    ("IPFS gateway (cloudflare)", "https://cf-ipfs.com/", "p2p", "none", "repo", 0.8),
    ("IPFS gateway (nftstorage)", "https://nftstorage.link/", "p2p", "none", "repo", 0.7),
    ...
    ("I2P Pastebin", "http://paste.i2p/", "dark", "i2p", "paste", 0.6),
    ("I2P Bug Tracker", "http://trac.i2p2.i2p/", "dark", "i2p", "repo", 0.7),
...
class DeepSourceRegistry:
    ...
    __slots__ = ("_db", "_env", "_lmdb_path", "_semaphore", "_sources")
    ...
    def __init__(self) -> None:
        ...
        logger.debug(
            "DeepSourceRegistry: %d curated sources loaded (cap=%d)", len(self._sources), MAX_SOURCES_IN_REGISTRY
        )
