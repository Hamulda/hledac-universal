...
class COREAdapter:
    ...
    __slots__ = tuple(('_api_key', '_cache', '_cache_ttl', '_semaphore'))
    ...
    async def _fetch(self, endpoint: str, method: str='GET', data: dict | None=None) -> dict | None:
        """Fetch from CORE API."""
    ...
    async def fulltext_search(self, query: str, max_results: int=MAX_RESULTS, highlight: bool=True) -> COREResult:
        """
        ...
        try:
            payload = {'query': query, 'page': 1, 'pageSize': max_results, 'minFullTextRelevance': 0.1}
            if highlight:
            ...
            for item in data.get('results', [])[:max_results]:
                ...
                works.append(COREWork(id=item.get('id', 0), title=item.get('title', ''), authors=authors, year=item.get('year'), abstract=item.get('abstract'), doi=item.get('doi'), fulltext=fulltext, highlight=highlight_text, download_url=download_url, repositories=repos, topics=item.get('topics', []), oai_ids=item.get('oaiIds', [])))
                if highlight_text and highlight_text != item.get('abstract'):
                    page_results.append(COREPageResult(text=highlight_text[:300], score=item.get('fullText', {}).get('score', 0.0) or item.get('score', 0.0), source=f"CORE:{item.get('id', 0)}"))
    ...
    async def search_by_doi(self, doi: str) -> COREWork | None:
        ...
        try:
            ...
            for auth in data.get('authors', [])[:10]:
                ...
                if name:
                    authors.append(name)
            return COREWork(id=data.get('id', 0), title=data.get('title', ''), authors=authors, year=data.get('year'), abstract=data.get('abstract'), doi=data.get('doi'), fulltext=data.get('fullText', {}).get('text') if isinstance(data.get('fullText'), dict) else None, highlight=None, download_url=data.get('downloadUrl'), repositories=[r.get('name', '') for r in data.get('repositories', [])[:5]], topics=data.get('topics', []), oai_ids=data.get('oaiIds', []))
        except Exception as e:
            logger.debug(f'CORE DOI lookup error: {e}')
            return None

    def to_canonical_findings(self, works: list[COREWork], query: str) -> list[CanonicalFinding]:
        """Convert CORE works to CanonicalFinding."""
        findings = []
        for work in works:
            import hashlib
            fid = hashlib.sha256(f'{query}\x00{work.id}\x00core'.encode()).hexdigest()[:16]
            payload_parts = [f'title: {work.title}', f"authors: {', '.join(work.authors[:5])}{('...' if len(work.authors) > 5 else '')}", f"year: {work.year or 'N/A'}", f"doi: {work.doi or 'N/A'}", f"download: {work.download_url or 'N/A'}", f"repositories: {', '.join(work.repositories[:3])}", f"topics: {', '.join(work.topics[:5])}"]
            if work.highlight:
                payload_parts.append(f'fulltext_match: {work.highlight[:400]}')
            elif work.abstract:
                payload_parts.append(f'abstract: {work.abstract[:400]}')
            findings.append(CanonicalFinding(finding_id=fid, query=query, source_type='core_fulltext', confidence=0.85, ts=time.time(), provenance=('core', str(work.id), work.title[:50]), payload_text='\n'.join(payload_parts)))
        return findings
_adapter: COREAdapter | None = None

async def search_core_fulltext(query: str, max_results: int=MAX_RESULTS, highlight: bool=True) -> list[CanonicalFinding]:
    """
...
__all__ = ['COREAdapter', 'COREWork', 'COREPageResult', 'COREResult', 'search_core_fulltext', 'lookup_core_doi']