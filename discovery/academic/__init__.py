...
def __getattr__(name: str):
    ...
    if name.startswith("Arxiv"):
        ...
        name.startswith("Unpaywall")
        or name.startswith("OAPaper")
        or name == "resolve_doi"
        or (name == "resolve_multiple_dois")
        or (name == "find_free_pdf")
    ):
...
async def search_all_academic(query: str, max_results_per_source: int = 10, timeout_s: float = 10.0) -> dict[str, list]:
    ...
    try:
        ...
        tasks.append(
            run_adapter("s2orc", s2orc_mod.search_s2orc, max_results=max_results_per_source, include_citations=True)
        )
    except Exception:  # noqa: BLE001
        pass
    try:
    ...
    try:
        completed = await safe_wait_for(
            parallel_ok(*tasks, label="__init__:209"), timeout=timeout_s, label="academic_search_gather"
        )
    except TimeoutError:
        import logging
