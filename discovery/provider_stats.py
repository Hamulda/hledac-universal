...
class ProviderStats(msgspec.Struct):
    ...
    def record_success(self, latency_ms: float, hits: tuple[Any, ...], unique_hosts: int) -> None:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'name': self.name, 'success_count': self.success_count, 'fail_count': self.fail_count, 'timeout_count': self.timeout_count, 'total_latency_ms': self.total_latency_ms, 'total_hits': self.total_hits, 'total_unique_hits': self.total_unique_hits, 'last_error_type': self.last_error_type, 'last_error_time': self.last_error_time, 'reliability_ewma': self.reliability_ewma, 'novelty_ewma': self.novelty_ewma, 'cost_ewma': self.cost_ewma, 'call_count': self.call_count, 'last_call_time': self.last_call_time, 'recent_errors': self.recent_errors, '_version': self._version}
...
class ProviderStatsRegistry:
    ...
    """
    __slots__ = tuple(('_last_save_time', '_snapshot_path', '_stats'))

    def __init__(self) -> None:
        self._stats: dict[str, ProviderStats] = {}
        for name in PROVIDER_NAMES:
            self._stats[name] = ProviderStats(name=name, cost_ewma=PROVIDER_COST_ESTIMATE.get(name, 1000.0))
    ...
    def record_success(self, provider: str, latency_ms: float, hits: tuple[Any, ...]=(), unique_hosts: int=0) -> None:
