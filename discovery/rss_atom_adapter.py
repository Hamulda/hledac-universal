...
def _entry_dedup_key(
    entry_url: str, title: str, published_raw: str, guid_raw: str | None, is_permalink: bool | None
) -> str:
    """
...
def _parse_atom_entry(
    entry, feed_url: str, retrieved_ts: float, feed_title: str, feed_language: str, seen_keys: set[str], rank: int
) -> FeedEntryHit | None:
...
def _parse_feed_xml(
    ...
    for strategy_xml, parser_fn, mode in parse_strategies:
        entries, _ = _try_parse_with_mode(
            strategy_xml, feed_url, retrieved_ts, parser_fn, mode, _parse_mode_out, _feed_type_out
        )
            ...
            with asyncio.Runner() as runner:
                result = runner.run(
                    async_fetch_public_text(
                        feed_url, timeout_s=timeout_s, max_bytes=max_bytes, bypass_circuit_breaker=True
                    )
                )
        else:
...
async def async_fetch_feed_entries(
    feed_url: str, max_entries: int = 50, timeout_s: float = 35.0, max_bytes: int = 2000000
) -> FeedBatchResult:
    ...
    parsed, had_valid_feed_type, parse_error, src_err, raw_xml = _parse_and_validate_feed(
        result, feed_url, retrieved_ts
    )
    if parse_error:
    ...
    return FeedBatchResult(feed_url=feed_url, entries=tuple(entries), error=error_tag, raw_xml=raw_xml)
...
class _FeedLinkParser(HTMLParser):
    ...
    __slots__ = ("_base_href", "_error", "_hits")
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if not self._is_valid_feed_link(href, attr_dict.get("rel", "")):
            return
        self._hits.append(
            {"href": href, "type": attr_dict.get("type", "").lower(), "title": attr_dict.get("title", "") or ""}
        )
...
def discover_feed_urls_from_html(
    page_url: str, html_text: str, max_candidates: int = _MAX_CANDIDATES_DEFAULT
...
async def async_discover_feed_urls(
    page_url: str, timeout_s: float = 35.0, max_bytes: int = 2000000, max_candidates: int = _MAX_CANDIDATES_DEFAULT
) -> FeedDiscoveryBatchResult:
    """
    ...
    try:
        result = await async_fetch_public_text(
            page_url, timeout_s=timeout_s, max_bytes=max_bytes, bypass_circuit_breaker=True
        )
    except CancelledError:
        raise
    if result.error or result.text is None:
    ...
    batch: FeedDiscoveryBatchResult = await asyncio.to_thread(
        discover_feed_urls_from_html, page_url, result.text, max_candidates
    )
    return batch
...
def get_default_feed_seeds() -> tuple[FeedSeed, ...]:
    ...
    """
    return (
        FeedSeed(feed_url="https://www.cisa.gov/feeds/hns.xml", label="CISA HNS", source="curated_seed", priority=10),
        FeedSeed(
        ...
        FeedSeed(
            feed_url="https://krebsonsecurity.com/feed/", label="Krebs on Security", source="curated_seed", priority=7
        ),
        FeedSeed(feed_url="https://abuse.ch/feeds/urlhaus/", label="URLhaus", source="curated_seed", priority=10),
        FeedSeed(
            feed_url="https://www.welivesecurity.com/feed/", label="WeLiveSecurity", source="curated_seed", priority=3
        ),
        FeedSeed(
        ...
        FeedSeed(feed_url="https://isc.sans.edu/rssfeed.xml", label="SANS ISC", source="curated_seed", priority=6),
        FeedSeed(
...
def merge_feed_sources(
    discovered: tuple[FeedDiscoveryHit, ...], seeds: tuple[FeedSeed, ...]
    ...
    return tuple(
        MergedFeedSource(
            feed_url=item["feed_url"], label=item["label"], origin=item["origin"], priority=item["priority"]
        )
...
def _error_result(
    feed_url: str, error_tag: str, src_accessibility: str | None, result, raw_xml: str | None = None
) -> FeedBatchResult:
...
def _parse_xml_result(
    entries: list[FeedEntryHit] | None, mode: str, _parse_mode_out: list[str] | None
) -> list[FeedEntryHit]:
