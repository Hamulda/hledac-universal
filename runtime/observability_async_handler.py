...
class AsyncLogHandler:
    ...
    __slots__ = tuple(('_drop_oldest', '_queue', '_sl', '_started', '_stop_event', '_thread'))

    def __init__(self, drop_oldest: bool=True, queue_size: int=MAX_QUEUE_SIZE) -> None:
...
async def configure_async_logging() -> None:
    ...
    if sl is not None:
        try:
            sl.configure(processors=[sl.contextvars.merge_contextvars, sl.stdlib.filter_by_level, sl.stdlib.add_log_level, sl.stdlib.PositionalArgumentsFormatter(), _inject_trace_context_async, _json_renderer_async], wrapper_class=sl.stdlib.BoundLogger, context_class=dict, logger_factory=sl.stdlib.LoggerFactory(), cache_logger_on_first_use=True)
        except Exception:
            pass
...
def _json_renderer_async(logger: Any, method: str, event: dict[str, Any]) -> None:
    ...
    try:
        ...
        if ctx:
            event = {**ctx, **event}
        event = {'timestamp': datetime.now(timezone.utc).isoformat(), 'level': method.upper(), 'event': event.pop('event', ''), **event}
