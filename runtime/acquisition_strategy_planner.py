...
def _feed_budget_to_dict(fdb) -> dict:
    ...
    if hasattr(fdb_type, "__struct_fields__"):
        return {
            "max_feed_accepted_before_nonfeed_terminal": getattr(fdb, "max_feed_accepted_before_nonfeed_terminal", 0)
            or 0,
            "max_feed_per_source": getattr(fdb, "max_feed_per_source", 0) or 0,
            "max_feed_share_before_nonfeed_terminal": getattr(fdb, "max_feed_share_before_nonfeed_terminal", 0.0)
            or 0.0,
        }
    ...
    def cap_feeding(
        ...
        if self.is_active():
            if (
                ...
                return (
                    True,
                    f"feed_cap_active:global:{feed_accepted_so_far}>={self.max_feed_accepted_before_nonfeed_terminal}",
                )
...
_LANE_DISABLED_REASONS: dict[str, Callable[[AcquisitionContext], str]] = {
    ...
    AcquisitionLane.PUBLIC: lambda ctx: (
        ...
        else "hardware_critical"
        if ctx.hardware_critical
        else "query_not_domain"
    ),
    ...
    AcquisitionLane.ACADEMIC: lambda ctx: "hardware_critical" if ctx.hardware_critical else "non_academic_profile",
    AcquisitionLane.IPFS: lambda ctx: "no_cid_in_query" if not ctx.cid_present else "hardware_critical",
    AcquisitionLane.OPEN_SOURCE: lambda ctx: "hardware_critical" if ctx.hardware_critical else "non_academic_profile",
}
    ...
    return (
        ...
        _lane_rule(
            ...
            lambda ctx: (
                ...
                else ctx.is_nonfeed_diagnostic and ctx.has_domain and not ctx.transport_degraded
                if ctx.is_nonfeed_diagnostic
            ...
            lambda ctx: (
                ...
                if ctx.is_deep_osint_m1
                else "nonfeed_diagnostic_domain"
                if ctx.is_nonfeed_diagnostic and ctx.has_domain
                else "transport_degraded"
                if ctx.transport_degraded
                else "hardware_critical"
                if ctx.uma_state in ("critical", "emergency")
                else "query_eligible"
        ...
        _lane_rule(
            ...
            LaneSpecCT,
            lambda ctx: (
                (ctx.has_domain or ctx.aggressive_mode or ctx.is_nonfeed_diagnostic)
                and (not ctx.hardware_critical or ctx.aggressive_mode)
                and (not ctx.is_deep_osint_m1 or ctx.aggressive_mode)
            ),
        ...
        _lane_rule(
            ...
            LaneSpecDOH,
            lambda ctx: (
                (ctx.has_domain or ctx.is_nonfeed_diagnostic and ctx.has_domain)
                and (not ctx.hardware_critical or ctx.is_nonfeed_diagnostic or ctx.aggressive_mode)
                and not ctx.is_deep_osint_m1
            ),
        ...
        _lane_rule(
            ...
            LaneSpecWayback,
            lambda ctx: (
                (ctx.has_url or ctx.has_long_duration or ctx.is_nonfeed_diagnostic and ctx.has_domain)
                and (not ctx.hardware_critical or ctx.is_nonfeed_diagnostic or ctx.aggressive_mode)
                and not ctx.is_deep_osint_m1
            ),
...
def required_terminal_lanes(
    snapshot: AcquisitionStrategySnapshot, query: str, uma_state: str, swap_detected: bool
        ...
        if hasattr(outcome, "family"):
            sfo_dicts.append(
                ...
                if hasattr(outcome, "to_dict")
                    ...
                    "built_count": outcome.built_count,
...
def _build_nonfeed_lane_eligibility(
    query: str, acquisition_profile: str, plan: AcquisitionStrategySnapshot | None
) -> dict:
    """
...
def build_acquisition_report(
    ...
    source_family_outcomes: list[SourceFamilyOutcome] | None = None,
    ...
    if _effective_profile == "default":
        ...
        if _env_override is not None:
            logger.debug(
                "[build_acquisition_report] acquisition_profile='default' resolved to %r from HLEDAC_ACQUISITION_PROFILE env var. This is expected only when called directly without normalization.",
                _env_override,
            )
    ...
    return {
        ...
        "nonfeed_lane_eligibility": _build_nonfeed_lane_eligibility(
            query=query, acquisition_profile=_effective_profile, plan=plan
        ),
        ...
        "acquisition_plan_lanes_for_prelude": list(acquisition_plan_lanes_for_prelude)
        if acquisition_plan_lanes_for_prelude
        else [],
        "acquisition_plan_enabled_lanes_for_prelude": list(acquisition_plan_enabled_lanes_for_prelude)
        if acquisition_plan_enabled_lanes_for_prelude
        else [],
        ...
        "nonfeed_prelude_expected_lanes": list(nonfeed_prelude_expected_lanes)
        if nonfeed_prelude_expected_lanes
        else [],
        "nonfeed_prelude_attempted_lanes": list(nonfeed_prelude_attempted_lanes)
        if nonfeed_prelude_attempted_lanes
        else [],
        "nonfeed_prelude_terminal_lanes": list(nonfeed_prelude_terminal_lanes)
        if nonfeed_prelude_terminal_lanes
        else [],
...
def canonicalize_source_family_outcomes(outcomes: list[SourceFamilyOutcome]) -> list[SourceFamilyOutcome]:
    ...
    for fam_norm, group in _groups.items():
            ...
            if first.family != fam_norm:
                _result.append(
                    SourceFamilyOutcome(
                        ...
                        raw_count=first.raw_count,
                        built_count=first.built_count,
        ...
        next((l for l in lane_candidates if l), fam_norm.upper())
        _result.append(
            SourceFamilyOutcome(
                family=fam_norm,
                attempted=attempted,
...
class NonfeedMissionController:
    ...
    def _get_lane_outcome(
        family: str, acquisition_lane_outcomes: tuple, public_outcome: dict | None, ct_quarantine_count: int
    ) -> dict | None:
...
_MISSION_LANES: dict[str, tuple[tuple[str, ...], tuple[str, ...]]] = {
    ...
    MissionIntent.DOMAIN_RECON: (
        (AcquisitionLane.PUBLIC, AcquisitionLane.CT, AcquisitionLane.PIVOT_EXECUTOR),
        (AcquisitionLane.WAYBACK, AcquisitionLane.PASSIVE_DNS),
    ),
    MissionIntent.INFRA_RECON: (
        (AcquisitionLane.PUBLIC, AcquisitionLane.CT, AcquisitionLane.PIVOT_EXECUTOR),
...
def _build_plan_impl(
    ...
    ctx = _build_acquisition_context(
        query,
        duration_s,
        aggressive_mode,
        uma_state,
...
def _build_acquisition_context(
    ...
    has_domain = _resolve_domain_indicator(
...
def _parse_transport_and_stealth(
...
def _build_nonfeed_debug(
    ...
    enabled_nonfeed, disabled_nonfeed, disabled_reasons, scheduled_nonfeed, hardware_skipped = (
        _categorize_nonfeed_lanes(plans, _NONFEED_LANES, _hardware_blocked)
    )
...
def _wallet_to_findings(wallet_analysis, query: str) -> list:
    ...
    try:
        ...
        finding = CanonicalFinding(
            ...
            source_type=getattr(SourceType, "BLOCKCHAIN_FORENSICS", "blockchain_forensics")
            if SourceType
            else "blockchain_forensics",
            confidence=0.75,
            query=query[:128],
            ts=0.0,
