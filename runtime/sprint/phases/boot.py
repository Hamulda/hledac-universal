...
from ..cleanup import _cleanup_stale_locks
    ...
    try:
        ...
        if FeatureFlags.get(FeatureFlag.REMOTE_DEBUG_DISABLE):
            ...
            if os.environ.get("PYTHON_DISABLE_REMOTE_DEBUG") != "1":
                sys.exit(
                    "HLEDAC_REQUIRE_REMOTE_DEBUG_DISABLED=1 but PYTHON_DISABLE_REMOTE_DEBUG not set — OSINT runtime requires external debugger disabled"
                )
    except ImportError:
        pass
...
def _generate_seed_state(
    ...
    if replay_seed is not None:
        ...
        logger.info(
            "[ULTIMATE-001] Generated seed=%d, tot_iv=%s (use --seed %d for deterministic replay)",
            seed_state.prng_seed,
            seed_state.tot_iv[:8],
            seed_state.prng_seed,
        )
