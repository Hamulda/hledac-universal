...
_otel_instrumented = lazy_decorator('otel:instrumented', default=lazy_decorator('hledac.universal.otel._instrumentation:instrumented'))
_IntCounterLayout = optional('runtime.int_counter_layout', default=optional('hledac.universal.runtime.int_counter_layout'))
...
class _AdvisoryLogLRU(msgspec.Struct, gc=False):
    ...
    counts: dict[str, int]
    order: deque[str]
_advisory_log_lru_var: contextvars.ContextVar[_AdvisoryLogLRU] = contextvars.ContextVar('_advisory_log_lru', default=_AdvisoryLogLRU(counts={}, order=deque(maxlen=_ADVISORY_LOG_LRU_MAX)))
_advisory_log_suppressed_total_var: contextvars.ContextVar[int] = contextvars.ContextVar('_advisory_log_suppressed_total', default=0)
...
_sprint_run_ctx: contextvars.ContextVar[SprintRunContext | None] = contextvars.ContextVar('_sprint_run_ctx', default=None)
...
def _advisory_log_stats() -> dict:
    ...
    lru = _advisory_log_lru_var.get()
    return {'unique_keys': len(lru.counts), 'max_keys': _ADVISORY_LOG_LRU_MAX, 'suppressed_total': _advisory_log_suppressed_total_var.get()}
...
def _build_deep_security_config(sprint_mode: str) -> DeepSecurityConfig:
    ...
    if sprint_mode == 'aggressive':
        return DeepSecurityConfig(privacy_level='medium', enable_quantum_safe=False, enable_steganography=False, enable_obfuscation=True, enable_destruction=True, enable_audit=True, chaff_enabled=True, chaff_ratio=0.2, auto_cleanup=True)
    else:
        return DeepSecurityConfig(privacy_level='low', enable_quantum_safe=False, enable_steganography=False, enable_obfuscation=False, enable_destruction=True, enable_audit=True, chaff_enabled=False, chaff_ratio=0.0, auto_cleanup=True)

def resolve_nonfeed_expected_lanes(result_expected: tuple[str, ...], local_expected: list[str], plan_debug_expected: list[str], acquisition_profile: str) -> tuple[str, ...]:
    """
...
from hledac.universal.transport.circuit_breaker import _BREAKERS, MAX_TRACKED_DOMAINS, CBState, get_all_breaker_snapshots, get_all_breaker_states
...
from hledac.universal.knowledge.target_memory import MAX_MEMORY_ENTITIES, MAX_MEMORY_EXPOSURES, MAX_MEMORY_PIVOTS, TargetMemoryService, TargetMemoryUpdate
from hledac.universal.pipeline.pivot_lane_planner import plan_lanes_for_pivot_seeds
from hledac.universal.runtime.acquisition_strategy import AcquisitionLane, AcquisitionLaneOutcome, NonfeedMissionController, NonfeedPlanDebug, NonfeedSeedContext, _get_ct_adapter, build_lane_query, canonicalize_source_family_outcomes, get_lane_plan, infer_mission_intent, is_lane_enabled, normalize_source_family_outcome, required_terminal_lanes, run_enabled_acquisition_lanes_streaming, terminality_report
from hledac.universal.runtime.cti.db.duckdb_domain_mv import get_domain_mv
from hledac.universal.runtime.nonfeed_candidate_ledger import NonfeedCandidateLedger, extract_domain_candidates_from_text
from hledac.universal.runtime.pivot_planner import generate_pivot_candidates_from_query
from hledac.universal.runtime.shadow_inputs import collect_graph_summary, collect_lifecycle_snapshot, collect_model_control_facts, collect_provider_runtime_facts
...
from hledac.universal.runtime.source_finding_bridge import REJECTION_UNSUPPORTED_SHAPE, ct_results_to_findings, passive_dns_results_to_findings, record_ct_storage_results, wayback_results_to_findings
...
def _compute_public_stage(outcome: dict | None, public_result: Any | None=None) -> tuple[str, dict]:
    ...
    """
    if outcome is None:
        return (_PublicStage.NOT_SCHEDULED, {'discovered_urls': 0, 'fetch_attempted': 0, 'fetch_success': 0, 'fetch_timeout': 0, 'fetch_error': 0, 'parse_attempted': 0, 'parse_success': 0, 'quality_rejected': 0, 'storage_rejected': 0, 'accepted_findings': 0})
    ...
    if public_result is not None:
        ...
        stage_counters = {'discovered_urls': _discovered_count, 'raw_count_source': _raw_count_source, 'fetch_attempted': getattr(pr, 'public_fetch_attempted', 0) or 0, 'fetch_success': getattr(pr, 'public_fetch_success', 0) or 0, 'fetch_timeout': getattr(pr, 'public_skipped_timeout', 0) or 0, 'fetch_error': getattr(pr, 'public_skipped_fetch_error', 0) or 0, 'parse_attempted': getattr(pr, 'public_fetch_success', 0) or 0, 'parse_success': getattr(pr, 'public_fetch_success', 0) or 0, 'quality_rejected': getattr(pr, 'public_acceptance_rejected', 0) or 0, 'storage_rejected': getattr(pr, 'public_rejected_storage_rejected', 0) or 0, 'accepted_findings': getattr(pr, 'public_findings_accepted', 0) or 0}
        stage_counters['rejection_reasons'] = list((getattr(pr, 'public_acceptance_reject_reasons', None) or {}).items())[:5]
        ...
        if bootstrap_enabled and raw_count > 0:
            ...
            terminal_stage = _derive_terminal_stage(error=error, timeout=timeout, skip_reason=skip_reason, raw_count=raw_count, built_count=built_count, accepted_count=accepted_count, attempted=attempted, skipped=skipped, public_stage_failure=psf)
    else:
        stage_counters = {'discovered_urls': raw_count, 'fetch_attempted': 0, 'fetch_success': 0, 'fetch_timeout': 0, 'fetch_error': 0, 'parse_attempted': built_count, 'parse_success': built_count, 'quality_rejected': 0, 'storage_rejected': 0, 'accepted_findings': accepted_count}
        terminal_stage = _derive_terminal_stage(error=error, timeout=timeout, skip_reason=skip_reason, raw_count=raw_count, built_count=built_count, accepted_count=accepted_count, attempted=attempted, skipped=skipped, public_stage_failure=None)
    return (terminal_stage, stage_counters)

def _derive_terminal_stage(error: str, timeout: bool, skip_reason: str, raw_count: int, built_count: int, accepted_count: int, attempted: bool, skipped: bool, public_stage_failure: str | None) -> str:
...
class _LifecycleAdapter:
    ...
    """
    __slots__ = ('_lc', '_cached_phase_attr', '_cached_start_attr', '_cached_tick_attr', '_cached_remaining_time_attr', '_cached_is_terminal_attr', '_cached_should_enter_windup_attr', '_cached_should_enter_windup_attr2', '_cached_mark_warmup_done_attr', '_cached_recommended_tool_mode_attr', '_cached_request_abort_attr', '_cached_abort_requested_attr', '_cached_abort_reason_attr', '_cached_set_pre_loop_cost_attr', '_cached_set_windup_lead_attr', '_cached_set_first_cycle_ran_attr', '_cached_set_deadline_expired_attr', '_phase_transition_callback', '_prev_phase')
...
class LaneBudgetPool(msgspec.Struct, gc=False):
    ...
    def get_lane_stats(self) -> dict:
        return {n: {'allocated_s': a.allocated_s, 'consumed_s': a.consumed_s, 'released_s': a.released_s, 'timeout_count': a.timeout_count} for n, a in self._allocations.items()}
...
class FeedDominanceGuard(msgspec.Struct, gc=False):
    ...
    def compute(self, total_accepted: int, feed_accepted: int, nonfeed_accepted: int, eligible_nonfeed_lanes_terminal: bool=False, nonfeed_diagnostic_timed_out: bool=False) -> FeedDominanceGuardResult:
        if total_accepted == 0:
            return FeedDominanceGuardResult(feed_dominance_ratio=0.0, nonfeed_accepted_findings=0, feed_dominance_class='balanced', should_recommend_nonfeed_diagnostic=False, guard_triggered=False, block_early_exit=False, reason='no findings')
        ...
        return FeedDominanceGuardResult(feed_dominance_ratio=ratio, nonfeed_accepted_findings=nonfeed, feed_dominance_class=dom_class, should_recommend_nonfeed_diagnostic=should_recommend, guard_triggered=guard_triggered, block_early_exit=block_early_exit, reason=f'feed_dominance={dom_class}:{ratio:.3f}:feed={feed_accepted}:nonfeed={nonfeed}')
...
INT_COUNTER_LAYOUT_NAMES: tuple[str, ...] = ('cycles_started', 'cycles_completed', 'consecutive_empty_cycles', 'unique_entry_hashes_seen', 'duplicate_entry_hashes_skipped', 'hard_deadline_checked_count', 'windup_guard_call_count', 'windup_guard_callback_supplied_count', 'windup_guard_callback_executed_count', 'policy_quality_feedback_calls', 'policy_quality_feedback_errors', 'ipfs_cids_attempted', 'multimodal_enriched_findings', 'feed_suppression_count', 'forensics_enriched_ct_findings', 'acquisition_lanes_skipped')
...
def _import_exporters():
    from hledac.universal.export import render_cti_stix_bundle_to_path, render_diagnostic_markdown_to_path, render_jsonld_to_path, render_stix_bundle_to_path
    from hledac.universal.export.stix_exporter import collect_cti_export_inputs
    return (render_diagnostic_markdown_to_path, render_jsonld_to_path, render_stix_bundle_to_path, render_cti_stix_bundle_to_path, collect_cti_export_inputs)
...
class SprintScheduler:
    __slots__ = ('_config', '_result', '_stop_requested', '_lane_budget_pool', '_lifecycle', '_flags', '_governor', '_backpressure_monitor', '_fetch_coordinator', '_graph_accumulator', '_duckdb_store', '_duckdb_can_ingest', '_duckdb_read_con', '_duckdb_writer_task', '_duckdb_writer_shutdown', '_duckdb_write_queue', '_writer_wakeup', '_enrichment_services', '_evidence_log', '_memory_delta_tracker', '_lane_rejections', '_lane_rejections_dropped', '_lane_rejections_total_seen', '_feed_budget_triggered', '_public_outcome', '_public_pipeline_result', '_public_consecutive_timeouts', '_public_bootstrap_enabled_at_timeout', '_ct_consecutive_timeouts', '_ct_pdns_active_done', '_ct_log_client', '_lane_outcomes', '_source_economics', '_shadow_pd_summary', '_hypothesis_pack_cache', '_pivot_planner', '_pivot_ioc_graph', '_injected_ioc_graph', '_planner_seed_iocs', '_planner_lanes', '_acquisition_plan', '_hypothesis_depth', '_hypothesis_query_count', '_enqueue_pivot', '_policy_manager', '_stealth_layer', '_ghost_layer', '_prefetch_oracle', '_prefetch_pipeline', '_temporal_predictor', '_security_coordinator', '_privacy_layer', '_forensics_enricher', '_forensics_lmdb_env', '_multimodal_enricher', '_multimodal_lmdb_env', '_analyst_workbench', '_communication_layer', '_ioc_scorer', '_ioc_cooccurrence_miner', '_ioc_cooccurrence_miner', '_tor_transport', '_i2p_transport', '_nym_transport', '_dht_node', '_advisory_gate_snapshot', '_arrow_last_flush', '_branch_value_summary', '_correlation_cache', '_dedup_dirty', '_dedup_env', '_dedup_seen', '_dedup_loading_task', '_seen_hashes', '_recent_iocs', '_dedup_rust', '_dedup_python_fallback', '_dedup_mode', '_prewindup_barrier_delayed', '_barrier_retry_count', '_nonfeed_predispatch_done', '_nonfeed_ledger', '_synth_windup_task', '_bg_tasks', '_sidecar_tasks', '_sidecars_skipped', '_sidecar_orchestrator', '_memory_manager', '_layer_manager', '_target_memory_service', '_metrics_registry', '_metrics_initialized', '_hermes_engine', '_wall_clock_start', '_last_cycle_start', '_last_ooda', '_last_sources', '_last_partial_finding_count', '_last_speculative', '_cycle_time_ema', '_effective_max_cycles', '_hard_deadline_monotonic', '_prev_chain_hash', '_rel_discovery_engine', '_run_exit_path_override', '_runner', '_lc_adapter', '_run_started_at', '_hermes_prewarm_task', '_hermes_prewarm_exception', '_barrier_import_done', '_doh_adapter', '_fetch_semaphore', '_query', '_privacy_context_id', '_sprint_id', '_sprint_depth', '_entries_per_source', '_hits_per_source', '_cycle_timeout_count', '_source_weights', '_novelty_bonuses', '_source_quality_feedback', '_feed_accepted_per_source', '_pending_extractions', '_extraction_drain_count', '_extraction_drain_deadline_s', '_pivot_queue', '_pivot_stats', '_speculative_results', '_speculative_dns_cache', '_ooda_interval', '_fetch_latency_ema', '_fetch_latency_ema_order', '_MAX_FETCH_LATENCY_EMA', '_hard_deadline_checked_count', '_ARROW_FLUSH_N', '_ARROW_FLUSH_S', '_ARROW_BATCH_HARD_CAP', '_MAX_FINDINGS_PER_SPRINT', '_arrow_batch_dropped_after_flush_failure', '_arrow_last_flush_error', '_finding_count', '_synthesis_engine', '_synthesis_runner', '_pivot_rewards', '_ioc_graph', '_all_findings', '_lane_verdicts', '_feed_verdicts', '_public_verdicts', '_planned_pivots', '_analyst_brief', '_peak_rss_gib', '_timer', '_health_cache', 'sprint_id')
    ...
    async def aclose(self, timeout_s: float=10.0) -> None:
        ...
        for _attr, _stop_attr in (('_tor_transport', '_tor_transport'), ('_i2p_transport', '_i2p_transport'), ('_nym_transport', '_nym_transport'), ('_dht_node', '_dht_node')):
    ...
    def _init_fetch_coordinator(self) -> None:
        ...
        self._fetch_coordinator = _FC(max_concurrent=20, pivot_queue_provider=lambda: getattr(self, '_pivot_queue', None), pivot_stats_provider=lambda: getattr(self, '_pivot_stats', {}) or {}, hypothesis_query_count_provider=lambda: getattr(self, '_hypothesis_query_count', 0), hypothesis_query_count_setter=lambda v: setattr(self, '_hypothesis_query_count', v), hypothesis_depth_provider=lambda: getattr(self, '_hypothesis_depth', 0), hypothesis_depth_setter=lambda v: setattr(self, '_hypothesis_depth', v), sprint_config_provider=lambda: self._config, adaptive_priority_provider=lambda tt, base: self._get_adaptive_priority(tt, base), enqueue_pivot_provider=lambda **kw: self.enqueue_pivot(**kw), concurrency_provider=lambda _mon=getattr(self, '_backpressure_monitor', None): _mon.backpressure_provider() if _mon is not None else None)
        try:
    ...
    def _sort_work_items_by_economics(self, items: list[SourceWork], current_cycle: int) -> list[SourceWork]:
        ...
        def economics_sort_key(item: SourceWork) -> tuple:
            ...
            posture_score = {'hot': 0, 'warm': 1, 'lukewarm': 2, 'marginal': 3, 'cold': 4}.get(econ.recent_health_posture, 5)
            if in_cooldown:
    ...
    async def record_hypothesis_feedback(self, pivot_type: str, ioc_type: str, produced_count: int, accepted_count: int, signal_value: float) -> None:
        """
        ...
        try:
            ...
            record = HypothesisFeedbackRecord(id=str(uuid.uuid4()), target_id=getattr(self, 'sprint_id', '') or 'default', pivot_type=pivot_type, ioc_type=ioc_type, produced_count=produced_count, accepted_count=accepted_count, signal_value=signal_value, ts=_time.time())
    ...
    def _check_hard_deadline(self) -> bool:
        ...
        if elapsed >= 0.0:
            ...
            log.warning(f'[F212A] Hard deadline exceeded at cycle {self._result.cycles_started}. Elapsed={elapsed:.1f}s. Stopping new work.')
    ...
    async def _initialize_sprint_run(self, adapter: Any, lifecycle: Any, ct_log_client: Any | None, policy_manager: Any, duckdb_store: Any, now_monotonic: float | None, query: str='') -> tuple[float, bool, Any | None]:
        ...
        def _prewarm_mlx_sync() -> None:
            ...
            try:
                ...
                if mgr is not None and (not mgr._is_loaded):
                    with asyncio.Runner() as _runner:
                        result = try_op(lambda: _runner.get_loop().run_until_complete(mgr._load_model()), label='mlx_embed_prewarm')
                        if result.is_err():
        ...
        def _prewarm_hermes_sync() -> None:
            ...
            try:
                ...
                pass
        self._hermes_prewarm_task: asyncio.Future[Any] | asyncio.Task[Any] | None = safe_create_task(asyncio.to_thread(_prewarm_hermes_sync), name='hermes_prewarm_phase1')
        ...
        logger.info('[WINDUP] effective_windup_lead_s=%.1fs final_windup_lead_s=%.1fs active_window_budget=%.1fs sprint_duration=%.1fs', _effective_windup, _final_windup, _active_window_budget, self._config.sprint_duration_s)
        ...
        self._dedup_loading_task: asyncio.Task | None = safe_create_task(self._load_dedup(), name='sprint:dedup_lazy_load')
        try:
    ...
    async def run(self, lifecycle: Any, sources: Sequence[str], now_monotonic: float | None=None, query: str='', duckdb_store: Any=None, ct_log_client: Any=None, policy_manager: Any=None, progress_callback: Any | None=None) -> SprintSchedulerResult:
        """
        ...
        if self._sprint_depth > 3:
            ...
            _logging.getLogger(__name__).critical('RECURSION GUARD: SprintScheduler.run depth=%d > 3 — aborting to prevent infinite recursion', self._sprint_depth)
        ...
        try:
            ...
            return await self._run_internal(lifecycle, sources, now_monotonic=now_monotonic, query=query, duckdb_store=duckdb_store, ct_log_client=ct_log_client, policy_manager=policy_manager, progress_callback=progress_callback)
        finally:
            _sprint_run_ctx.reset(_token)
            self._sprint_depth -= 1

    async def _run_internal(self, lifecycle: Any, sources: Sequence[str], now_monotonic: float | None=None, query: str='', duckdb_store: Any=None, ct_log_client: Any=None, policy_manager: Any=None, progress_callback: Any | None=None) -> SprintSchedulerResult:
        """
        ...
        async def _prewarm_all_models() -> None:
            ...
            async def _prewarm_mlx_embed() -> None:
                ...
                try:
                    ...
                    pass
            _cf_future = _mlx_prewarm_worker.prewarm_all([_prewarm_hermes(), _prewarm_modernbert(), _prewarm_mlx_embed()], timeout_s=120.0)
        ...
        try:
            ...
            _dedup_elapsed, _trace_enabled, _trace_snap_before = await self._initialize_sprint_run(adapter, lifecycle, ct_log_client, policy_manager, duckdb_store, now_monotonic, query=query)
            ...
            if ENV.get_bool('HLEDAC_ENABLE_DSPY') and query:
                try:
                    ...
                    if expanded and expanded:
                        ...
                        log.debug("[HERMES3_WIRING] DSPy expanded %d queries for '%s...'", len(_expanded_capped), query[:30])
            ...
            _active_window_budget = max(30.0, self._config.sprint_duration_s - self._config.effective_windup_lead_s - _pre_loop_elapsed)
            self._hard_deadline_monotonic = self._wall_clock_start + _active_window_budget
            self._result.hard_deadline_monotonic = self._hard_deadline_monotonic
            ...
            if _active_window_budget > 0 and (not self._check_hard_deadline()):
                log.warning(f'[PRE-LOOP DEADLINE] Hard deadline exceeded before first cycle. pre_loop_elapsed={_pre_loop_elapsed:.1f}s, active_window_budget={_active_window_budget:.1f}s. Skipping to windup.')
                await self._ensure_nonfeed_predispatch_before_finalization(query, 'pre_loop_deadline_exceeded')
                self._capture_timing_fields()
                await self._finalize_result_truth('pre_loop_deadline_exceeded', 'hard deadline exceeded before first cycle due to slow pre-loop init', 'GATHER', query)
            ...
            try:
                ...
                self._result.next_seeds_provider_yield = bool(getattr(_next_seeds_diagnostics, 'provider_yield_active', False)) if _next_seeds_diagnostics else False
                self._result.next_seeds_pivot_deepening = bool(getattr(_next_seeds_diagnostics, 'pivot_deepening_active', False)) if _next_seeds_diagnostics else False
                self._result.next_seeds_query_suggestions = getattr(_next_seeds_diagnostics, 'query_suggestions', ()) if _next_seeds_diagnostics else ()
                self._result.next_seeds_consumed_count = len(_next_seeds_ioc_seeds)
                self._result.next_seeds_seed_source = f'predecessor:{self._config.predecessor_sprint_id}' if self._config.predecessor_sprint_id else 'none'
                self._timer.phase('acquisition_plan_build_start')
                _synthetic_domains: list[str] = []
                _plan_kwargs = {'query': query, 'duration_s': self._config.sprint_duration_s, 'aggressive_mode': self._config.aggressive_mode, 'uma_state': _uma_state, 'swap_detected': _swap_detected, 'accepted_findings_so_far': self._result.accepted_findings, 'branch_timeout_count': self._result.branch_timeout_count, 'acquisition_profile': self._config.acquisition_profile or '', 'source_quality_weights': self._policy_manager.get_src_quality_weights() if self._policy_manager is not None and self._policy_manager.enabled else None, 'rl_lane_combo': self._result.rl_lane_combo if self._result.rl_lane_combo else None, 'synthetic_domains': _synthetic_domains}
                ...
                if self._result.nonfeed_plan_debug is not None:
                    try:
                        ...
                        _pivot_candidates = generate_pivot_candidates_from_query(query, mission_intent=_nd.mission_intent, graph_stats=_graph_stats)
                ...
                if self._result.nonfeed_plan_debug is not None:
                    ...
                    self._result.nonfeed_profile_expected_lanes = tuple((canonical_lane_name(x) for x in getattr(_nd, 'nonfeed_profile_expected_lanes', ()) or ()))
                    self._result.nonfeed_expected_lanes = tuple((canonical_lane_name(x) for x in getattr(_nd, 'scheduled_nonfeed_lanes', ()) or ()))
            ...
            try:
                ...
                from hledac.universal.utils.async_helpers import safe_create_task
                prelude_task = safe_create_task(self._run_mandatory_acquisition_prelude(self._result, query, duckdb_store, self._ct_log_client), name='sprint:prelude', eager_start=True)
            finally:
                self._timer.phase('prelude_end')
            _t_work_items = _time.monotonic()
            self._build_work_items(ordered_sources)
            first_cycle_task = safe_create_task(self._run_one_cycle(lifecycle, ordered_sources, now_monotonic=None, query=query, duckdb_store=duckdb_store), name='sprint:first_cycle', eager_start=True)
            _t_branch_start = _time.monotonic()
            _ok_results, _gathered_errors = await gather_taskgroup([prelude_task, first_cycle_task], concurrency=2, ctx='prelude_first_cycle')
            ...
            logger.info('[P4.3][pre-loop-cost] init=%.1fs dspy=%.1fs src_prioritize=%.1fs pre_loop_capture=%.1fs plan_parallel=%.1fs plan_build=%.1fs plan_total=%.1fs hermes_wait=%.1fs work_items=%.1fs branch=%.1fs prelude_dispatch=%.1fs total=%.1fs', _phase1_dur, _phase3_dur, _phase4_dur, _t_phase4 - _t_phase3, _plan_parallel_dur, _plan_build_dur, _plan_total_dur, _hermes_wait_dur, _work_items_dur, _branch_dur, _prelude_dispatch_dur, _pre_loop_total)
            ...
            if not self._check_hard_deadline():
                ...
                self._capture_timing_fields()
                await self._finalize_result_truth('hard_deadline_exceeded', 'hard deadline exceeded after first_cycle_gather', 'GATHER', query)
                return self._result
            try:
                while not self._runner.is_terminal():
                    ...
                    if not self._check_hard_deadline():
                        await self._ensure_nonfeed_predispatch_before_finalization(query, 'hard_deadline_exceeded')
                        self._capture_timing_fields()
                        await self._finalize_result_truth('hard_deadline_exceeded', f'hard deadline exceeded at cycle {self._result.cycles_started}', 'GATHER', query)
                        lifecycle.request_windup()
                        break
                    ...
                    _barrier_result = await self._ensure_pre_windup_lane_terminal_states(query, self._acquisition_plan, 'ok')
                    ...
                    if _barrier_required and (not _barrier_satisfied):
                        ...
                        if _barrier_retry_count > _barrier_max_retries:
                            log.warning('[F223-D] prewindup barrier timeout after %d retries -- forcing windup', _barrier_retry_count)
                            _barrier_satisfied = True
                        elif now_monotonic - self._wall_clock_start > _barrier_hard_timeout_s:
                            log.warning('[F223-D] prewindup barrier hard timeout %.1fs -- forcing windup', _barrier_hard_timeout_s)
                            _barrier_satisfied = True
                        elif not _barrier_delayed:
                            self._prewindup_barrier_delayed = True
                            self._result.prewindup_barrier_delayed_cycle = True
                            log.debug('[F207S-B] Prewindup barrier not satisfied (required=%s) -- delaying cycle once', _barrier_required)
                            continue
                    ...
                    _guard_result = self._runner.windup_guard(now_monotonic, pre_windup_barrier=lambda: self._check_prewindup_barrier_sync(query, duckdb_store))
                    ...
                    if _guard_result:
                        ...
                        safe_create_task(self._run_ioc_cooccurrence_sidecar(query, duckdb_store), name='sprint:ioc_cooccurrence')
                        self._synth_windup_task = safe_create_task(self._run_synthesis_sidecar(query, duckdb_store, lifecycle), name='sprint:synthesis_windup')
                    ...
                    if self._result.cycles_started == 0:
                        ...
                        logger.info('[prelude] completed in %.1fs (budget=%.0fs)', _elapsed, self._config.effective_windup_lead_s)
                    ...
                    if elapsed_wall > self._config.sprint_duration_s + self._config.cycle_sleep_s:
                        log.warning(f'[8BK] Duration budget exceeded: {elapsed_wall:.1f}s > {self._config.sprint_duration_s + self._config.cycle_sleep_s:.1f}s (grace={self._config.cycle_sleep_s:.1f}s). Forcing windup.')
                    ...
                    if _remaining_active < _min_active_window_s and self._result.consecutive_empty_cycles >= _min_empty_for_early_exit:
                        log.warning('[P1-1] Early windup: remaining=%.1fs < %.0fs and %d empty cycles', _remaining_active, _min_active_window_s, self._result.consecutive_empty_cycles)
                        try:
                            await check_zero_findings_alert(elapsed_s=_elapsed_wall, consecutive_empty_cycles=self._result.consecutive_empty_cycles, total_findings=self._result.accepted_findings)
                        except Exception:
                            pass
                        await self._ensure_nonfeed_predispatch_before_finalization(query, 'early_windup_empty_cycles')
                        self._capture_timing_fields()
                        await self._finalize_result_truth('early_windup_empty_cycles', f'early windup: {self._result.consecutive_empty_cycles} empty cycles, remaining={_remaining_active:.0f}s < {_min_active_window_s:.0f}s', 'GATHER', query)
                        break
                    try:
                        async with asyncio.timeout(_cycle_budget_s):
                            cycle_ok = await self._run_one_cycle(lifecycle, ordered_sources, now_monotonic, query, duckdb_store)
                    except TimeoutError:
                        self._cycle_timeout_count += 1
                        log.warning('[F-A3] cycle exceeded %.1fs budget (count=%d) -- counting as empty', _cycle_budget_s, self._cycle_timeout_count)
                    ...
                    if self._result.consecutive_empty_cycles >= _empty_cycle_limit:
                        log.warning('[F228G] %d consecutive empty cycles >= limit %d -- forcing windup', self._result.consecutive_empty_cycles, _empty_cycle_limit)
                        await self._ensure_nonfeed_predispatch_before_finalization(query, 'empty_cycle_break')
                        self._capture_timing_fields()
                        await self._finalize_result_truth('empty_cycle_break', f'empty cycles {self._result.consecutive_empty_cycles} >= limit {_empty_cycle_limit}', 'GATHER', query)
                        break
                    self._result.cycles_completed += 1
                    self._tick_metrics_on_cycle_end()
                    ...
                    if self._config.stop_on_first_accepted and self._result.accepted_findings > 0:
                        ...
                        if await self._ensure_mandatory_nonfeed_before_return(query, duckdb_store, 'stop_on_first_accepted'):
                            await self._ensure_nonfeed_predispatch_before_finalization(query, 'stop_on_first_accepted_break')
                            self._capture_timing_fields()
                            await self._finalize_result_truth('stop_on_first_accepted_break', 'first accepted finding, stop', 'GATHER', query)
                            break
                        continue
                    await self._runner.sleep_or_abort(self._config.effective_cycle_sleep_s, progress_callback, self._result)
                    ...
                    if _fd and self._acquisition_plan is not None:
                        ...
                        _nonfeed_disabled = not any((_plan_dict.get(lane_name.lower()) is not None for lane_name in ('ct', 'doh', 'wayback')))
                            ...
                            try:
                                ...
                                if self._governor is not None:
                                    try:
                                        ...
                                        pass
                                self._acquisition_plan = build_acquisition_plan(query=query, duration_s=self._config.sprint_duration_s, aggressive_mode=self._config.aggressive_mode, uma_state=_uma_state, swap_detected=False, accepted_findings_so_far=self._result.accepted_findings, branch_timeout_count=self._result.branch_timeout_count, acquisition_profile=self._config.acquisition_profile or '', source_quality_weights=None, rl_lane_combo=None, feed_domain_seeds=_fd, synthetic_domains=())
                                log.debug('[P5] Mid-sprint re-plan: %d feed_domain_seeds enabled CT/DOH/WAYBACK lanes', len(_fd))
                            except Exception:
                                pass
                    if self._runner.post_sleep_gate(now_monotonic):
                        ...
                        if await self._ensure_mandatory_nonfeed_before_return(query, duckdb_store, 'post_sleep_windup') and _lanes_with_evidence >= _MIN_LANES_FOR_EARLY_WINDUP:
                            await self._ensure_nonfeed_predispatch_before_finalization(query, 'post_sleep_windup_break')
                        _rescue_triggered = False
                        if self._result.accepted_findings >= 1000 and self._result.lane_ct_accepted_findings == 0 and (self._result.lane_wayback_accepted_findings == 0) and (self._result.lane_pdns_accepted_findings == 0) and (self._result.lane_blockchain_accepted_findings == 0) and (self._result.lane_ipfs_accepted_findings == 0) and (self._result.lane_doh_accepted_findings == 0) and (self._result.requested_duration_s >= 180):
                            ...
                            if _rescue_elapsed is not None and _rescue_elapsed > 0:
                                ...
                                log.info(f'[F220D] Feed dominance rescue window completed in {_rescue_elapsed:.1f}s, nonfeed findings: ct={self._result.lane_ct_accepted_findings} public={self._result.public_accepted_findings}')
                            else:
                                log.debug('[F220D] Feed dominance rescue window returned no candidates')
                        self._capture_timing_fields()
                        if _rescue_triggered:
                            await self._finalize_result_truth('post_sleep_windup_break', 'feed dominant nonfeed rescue attempted', 'WINDUP', query)
                            break
                        else:
            ...
            if self._evidence_log is not None and _layout is not None:
                try:
                    ...
                    self._evidence_log.create_event('decision', {'phase': 'CHAIN_SNAPSHOT', 'blake3_hex': _blake3_hex, 'sha256_hex': _sha256_hex, 'counter_snapshot': _snap, 'sprint_id': getattr(self, '_sprint_id', '')}, confidence=1.0)
                    self._prev_chain_hash = _blake3_hex
                except Exception as _e:
                    if self._evidence_log is not None:
                        try:
                            self._evidence_log.create_event('error', {'phase': 'CHAIN_SNAPSHOT_FAILED', 'error': str(_e)}, confidence=0.0)
                        except Exception:
                            pass
            ...
            _sidecar_task.add_done_callback(lambda t: log.debug('[F290] advisory_runner done: %s', t.exception() if t.done() and t.exception() else 'ok'))
                ...
                try:
                    ...
                    if _decisions:
                        try:
                            ...
                            self._result.policy_quality_feedback_sources += len({d.get('source_family') for d in _decisions})
                        except asyncio.CancelledError:
                            raise
                        except Exception as _e:
            ...
            if not self._result.early_exit_class:
                await self._finalize_result_truth(exit_path='run_complete', exit_reason='run() finished normally', exit_phase='TEARDOWN', query=query)
            try:
    ...
    async def _record_scheduler_exit(self, path: str, reason: str, phase: str | None=None) -> None:
        ...
        if not getattr(self._result, 'return_guard_checked', False):
            try:
                await self._ensure_mandatory_nonfeed_before_return(getattr(self, '_query', '') or '', None, f'scheduler_exit_capture({path})')
            except Exception:
                pass
    ...
    def _capture_timing_fields(self) -> None:
        ...
        self._result.elapsed_pct = self._result.actual_duration_s / self._result.requested_duration_s if self._result.requested_duration_s > 0 else 0.0
        ...
        if _windup_eff > 0.4:
            logger.warning('[F289-WINDUP] windup_efficiency=%.2f (>0.40 critical) — windup=%.0fs, active=%.0fs, total=%.0fs. Consider reducing windup lead.', _windup_eff, self._config.effective_windup_lead_s, self._result.active_window_budget_s, _total_window)
    ...
    def _compute_early_exit_class(self, exit_path: str) -> tuple[str, str]:
        ...
        try:
            ...
            _is_early_exit = exit_path in ('early_complete_return_guard_satisfied', 'early_complete_feed_only', 'early_complete_prelude_complete', 'early_complete_no_work_remaining') or r.elapsed_pct < 0.5
            if _is_early_exit:
                try:
                    from metrics_registry import get_metrics_registry
            ...
            if r.aborted and r.abort_reason == 'domain_detected_no_seeds':
                return (EarlyExitClass.EARLY_COMPLETE_NO_WORK_REMAINING, 'domain_detected_no_seeds: no seeds for CT/DOH/WAYBACK lanes')
            ...
            if hasattr(r, 'peak_rss_gib') and r.peak_rss_gib > 0:
                if r.elapsed_pct < 0.5 and r.budget_violations > 0:
                    return (EarlyExitClass.ABORTED_BY_MEMORY, f'memory pressure ({r.budget_violations} budget violations)')
            if r.elapsed_pct >= 0.9:
                return (EarlyExitClass.COMPLETED_FULL_DURATION, f'ran to planned duration ({r.actual_duration_s:.1f}s / {r.requested_duration_s:.1f}s)')
            nonfeed_accepted = r.lane_ct_accepted_findings + r.lane_wayback_accepted_findings + r.lane_pdns_accepted_findings + r.lane_blockchain_accepted_findings + r.lane_ipfs_accepted_findings + r.lane_doh_accepted_findings
            feed_accepted = r.accepted_findings - nonfeed_accepted
            _guard = FeedDominanceGuard(strict=self._config.require_nonfeed_corrob_for_early_exit).compute(total_accepted=r.accepted_findings, feed_accepted=feed_accepted, nonfeed_accepted=nonfeed_accepted)
            ...
            if r.accepted_findings > 0 and nonfeed_accepted == 0:
                ...
                if _guard.guard_triggered:
                    _base_reason += f' | feed-dominant:ratio={_guard.feed_dominance_ratio:.3f}:recommend nonfeed_diagnostic'
                return (EarlyExitClass.EARLY_COMPLETE_FEED_ONLY, _base_reason)
            if r.return_guard_satisfied and exit_path in ('windup_barrier_passed', 'windup_barrier_break', 'post_sleep_windup_break'):
                return (EarlyExitClass.EARLY_COMPLETE_RETURN_GUARD_SATISFIED, f'return guard satisfied, early windup ({exit_path})')
            if exit_path in ('stop_requested_break', 'stop_on_first_accepted_break', 'cycles_limit_reached', 'max_cycles_break'):
            ...
            if exit_path == 'prelude_complete' and r.elapsed_pct < 0.9:
                ...
                return (EarlyExitClass.EARLY_COMPLETE_PRELUDE_COMPLETE, f'prelude early exit ({pct:.0f}% of planned duration, exit_path={exit_path})')
            pct = r.elapsed_pct * 100
            return (EarlyExitClass.EARLY_COMPLETE_NO_WORK_REMAINING, f'early exit ({pct:.0f}% of planned duration, exit_path={exit_path})')
        except Exception as _exc:
    ...
    async def _finalize_result_truth(self, exit_path: str, exit_reason: str, exit_phase: str, query: str='') -> None:
        ...
        try:
            ...
            try:
                ...
                _total_findings = sum([self._result.lane_ct_accepted_findings, self._result.lane_public_accepted_findings, self._result.lane_wayback_accepted_findings, self._result.lane_pdns_accepted_findings, self._result.lane_blockchain_accepted_findings, self._result.lane_ipfs_accepted_findings, self._result.lane_doh_accepted_findings])
                ...
                pass
            _mlt_required = required_terminal_lanes(snapshot=self._acquisition_plan, query=query, uma_state=uma_state, swap_detected=swap_detected)
                        ...
                        if self._acquisition_plan is not None:
                            for _plan in self._acquisition_plan.plans or ():
                                if hasattr(_plan, 'lane') and _plan.lane == AcquisitionLane.PUBLIC:
                                    ...
                                    break
                        _observed_outcomes.append({'lane': 'PUBLIC', 'family': 'PUBLIC', 'attempted': False, 'skipped': True, 'terminal_state': 'skipped', 'skip_reason': _skip_reason, 'raw_count': 0, 'built_count': 0, 'accepted_count': 0, 'error': None, 'timeout': False, 'duration_s': None})
                        _seen_outcome_lanes.add('PUBLIC')
                        break
                        ...
                        if self._acquisition_plan is not None:
                            for _plan in self._acquisition_plan.plans or ():
                                if hasattr(_plan, 'lane') and _plan.lane == AcquisitionLane.CT:
                                    ...
                                    break
                        _observed_outcomes.append({'lane': 'CT', 'family': 'CT', 'attempted': False, 'skipped': True, 'terminal_state': 'skipped', 'skip_reason': _skip_reason, 'raw_count': 0, 'accepted_count': 0, 'error': None, 'timeout': False})
                        _seen_outcome_lanes.add('CT')
                        break
        ...
        if self._public_pipeline_result is not None:
            self._result.public_discovery_empty_reason = getattr(self._public_pipeline_result, 'public_discovery_empty_reason', '') or ''
        _acq_profile = getattr(self._config, 'acquisition_profile', 'default')
        if NonfeedMissionController.is_mission_profile(_acq_profile):
            try:
                _mission_snapshot = NonfeedMissionController.build_snapshot(acquisition_profile=_acq_profile, acquisition_lane_outcomes=self._result.acquisition_lane_outcomes or (), public_outcome=self._public_outcome, ct_quarantine_count=self._result.ct_quarantine_count, quality_rejection_ledger=self._result.quality_rejection_ledger or ())
        ...
        try:
            ...
            self._result.transport_efficiency = {'http3_enabled': _h3_stats().get('enabled', 0), 'http3_altsvc_hits': _h3_stats().get('altsvc_hits', 0), 'http3_altsvc_misses': _h3_stats().get('altsvc_misses', 0), 'http3_altsvc_records': _h3_stats().get('altsvc_records', 0), 'http3_cache_size': _h3_stats().get('cache_size', 0), 'http3_cache_hits': _h3_stats().get('cache_hit', 0), 'conditional_cache_hits': _cc_stats().get('lookup_hits', 0), 'conditional_cache_misses': _cc_stats().get('lookup_misses', 0), 'conditional_cache_errors': _cc_stats().get('lookup_errors', 0), 'conditional_cache_304s': _cc_stats().get('conditional_304s', 0), 'conditional_sends': _cc_stats().get('conditional_sends', 0), 'prewarm_sessions_created': _pw_stats().get('sessions_created', 0), 'prewarm_hit_rate': _pw_stats().get('round_robin_hits', 0)}
        except Exception as _exc:
        ...
        for _o in self._result.acquisition_lane_outcomes or ():
            ...
            if _lane_name == 'CT':
                ...
                if _error is not None:
                    ...
                    _ts = 'success'
                _ct_outcome = {'lane': 'CT', 'family': 'CT', 'attempted': True, 'terminal_state': _ts, 'raw_count': _raw, 'accepted_count': _accepted, 'error': _error, 'timeout': _timeout, 'skipped': _skipped}
                break
        ...
        if _ct_outcome is None:
            ...
            if _disc > 0 or _acc > 0 or _ct_adapter_called:
                ...
                if _raw > 0 and _acc == 0:
                    ...
                    _ts = 'success'
                _ct_outcome = {'lane': 'CT', 'family': 'CT', 'attempted': True, 'terminal_state': _ts, 'raw_count': _raw, 'accepted_count': _acc, 'error': None, 'timeout': False, 'skipped': False}
        return _ct_outcome
    ...
    def _final_source_family_outcomes_for_terminality(self) -> tuple[dict, ...]:
        ...
        if _ct_outcome is not None:
            ...
            _seen.add('CT')
        for _fam, _lane in [('ct', AcquisitionLane.CT), ('wayback', AcquisitionLane.WAYBACK), ('passive_dns', AcquisitionLane.PASSIVE_DNS), ('blockchain', AcquisitionLane.BLOCKCHAIN), ('feed', 'FEED'), ('public', AcquisitionLane.PUBLIC)]:
            if _lane == 'PUBLIC':
                continue
            ...
            if _lane == 'FEED':
                ...
                if _raw is not None:
                    _feed_accepted = (self._result.accepted_findings or 0) - (self._result.public_accepted_findings or 0) - (self._result.ct_log_accepted_findings or 0)
                    _feed_accepted = max(0, _feed_accepted)
                    if isinstance(_raw, list) and _raw:
                        _first = _raw[0]
                        if isinstance(_first, tuple):
                            _raw = {'verdict': _first, 'accepted_count': _feed_accepted, 'raw_count': _first[1] if len(_first) > 1 else 0, 'attempted': True}
    ...
    async def _maybe_dispatch_nonfeed_probe_lanes(self, query: str, duckdb_store: Any) -> None:
        ...
        if not is_lane_enabled(self._acquisition_plan, AcquisitionLane.CT):
            self._nonfeed_predispatch_done = True
            return
        _ct_already_run = self._result.ct_log_discovered > 0 or self._result.lane_ct_accepted_findings > 0 or getattr(self._ct_log_client, '_called', False)
        if _ct_already_run:
            self._nonfeed_predispatch_done = True
            return
        ...
        async def _run_ct_predispatch() -> AcquisitionLaneOutcome:
            ...
            try:
                async with asyncio.timeout(15.0):
                    ...
                    candidates, rejections, _ct_telemetry = ct_results_to_findings(result, ct_outcome, query, sprint_id=f'predispatch-{int(_time.time())}')
                    ...
                    if _candidate_findings and duckdb_store is not None:
                        if self._duckdb_can_ingest:
                            try:
                                ingest_results = await self._gate_then_ingest_and_accumulate(duckdb_store, list(_candidate_findings))
                                if self._evidence_log is not None:
                                    try:
                                        self._evidence_log.create_event('decision', {'gate': 'ingest', 'findings_count': len(_candidate_findings), 'accepted': True}, source_ids=[], confidence=1.0)
                                    except Exception:
                                        pass
                                accepted = sum((1 for r in ingest_results if isinstance(r, dict) and r.get('accepted')))
                            except Exception as _exc:
                                log.warning('sprint %s: quality gate ingest failed -- %s: %s', getattr(self._result, 'sprint_id', '?'), type(_exc).__name__, _exc)
                                ingest_results = []
                            else:
                    ...
                    if _bridge_raw > 0 and hasattr(result, 'hits') and result.hits:
                        ...
                        _raw_keys = tuple(sorted({k for hit in _sample_hits for k in (getattr(hit, 'url', None), getattr(hit, 'ct_name_value', None)) if k}))
                    _storage_attempted = bool(_candidate_findings) and duckdb_store is not None and self._duckdb_can_ingest
                    ...
                    if _ct_cache_used:
                        ...
                        self._result.ct_bridge_invoked = False
                    elif _bridge_raw == 0 and _ct_results_raw > 0 and (_bridge_candidates == 0) and (REJECTION_UNSUPPORTED_SHAPE in _rejection_reasons):
                    ...
                    self._emit_source_family_event(family='CT', event='terminal', reason='bridge_invoked', terminal_state='success')
                    ...
                    for _entry in _ct_quarantine_entries:
                        try:
                            self._nonfeed_ledger.add_ct_quarantine(domain=_entry.get('raw_value', ''), reject_reason=_entry.get('reject_reason', 'unknown'), source_url=_entry.get('source_url', ''), query=_entry.get('normalized_query', ''))
                        except Exception:
                            pass
                    ...
                    return AcquisitionLaneOutcome(lane=AcquisitionLane.CT, enabled=True, attempted=True, accepted_findings=accepted, produced_items=_ct_results_raw, duration_s=_time.monotonic() - _start, source_family='ct', ct_query=_shaped, ct_results_raw=_ct_results_raw, error=_ct_error, candidate_findings=_candidate_findings, rejection_reasons=_rejection_reasons, rejected_count=_rejected_count, sample_rejections=_sample_rejections)
            except TimeoutError:
                return AcquisitionLaneOutcome(lane=AcquisitionLane.CT, enabled=True, attempted=True, timeout=True, duration_s=_time.monotonic() - _start, error='predispatch_timeout', source_family='ct', ct_query=str(_shaped) if '_shaped' in dir() else '', ct_results_raw=_ct_results_raw, candidate_findings=_candidate_findings, rejection_reasons=_rejection_reasons, rejected_count=_rejected_count, sample_rejections=_sample_rejections)
            except Exception as exc:
                return AcquisitionLaneOutcome(lane=AcquisitionLane.CT, enabled=True, attempted=True, error=f'predispatch_error:{type(exc).__name__}:{exc}', duration_s=_time.monotonic() - _start, source_family='ct', ct_query=str(_shaped) if '_shaped' in dir() else '', ct_results_raw=_ct_results_raw, candidate_findings=_candidate_findings, rejection_reasons=_rejection_reasons, rejected_count=_rejected_count, sample_rejections=_sample_rejections)
        try:
            ...
            if _wayback_shaped and (not isinstance(_wayback_shaped, dict)):
                try:
                    ...
                    try:
                        ...
                        await miner.close()
                    candidates, rejections, wayback_telemetry = wayback_results_to_findings(result, query, sprint_id=f'predispatch-wb-{int(_time.time())}')
        ...
        if _memory_ok:
            ...
            if _pdns_shaped and (not isinstance(_pdns_shaped, dict)):
                try:
                    ...
                    pdns_findings, pdns_rejections, pdns_telemetry = passive_dns_results_to_findings(ips, pdns_outcome, query, sprint_id=f'predispatch-pdns-{int(_time.time())}')
                    if pdns_telemetry:
        ...
        self._nonfeed_predispatch_done = True
        log.debug('[F207M-A] Nonfeed pre-dispatch done: lanes=%s, skipped=%s, dur=%.2fs', _attempted_lanes, _skipped, _duration)
    ...
    async def _required_pre_windup_lanes(self, query: str, acquisition_plan: Any, memory_state: str) -> tuple[str, ...]:
        ...
        if self._governor is not None:
            try:
                ...
                pass
        mlt_tuples = required_terminal_lanes(snapshot=acquisition_plan, query=query, uma_state=uma_state, swap_detected=swap_detected)
        return tuple((mlt.lane.lower() for mlt in mlt_tuples))

    async def _ensure_pre_windup_lane_terminal_states(self, query: str, acquisition_plan: Any, memory_state: str) -> PreWindupBarrierResult:
        """
        ...
        errors: dict[str, str] = {}
        _ct_done = self._result.ct_log_discovered > 0 or self._result.lane_ct_accepted_findings > 0 or self._result.ct_request_timeout or (self._result.ct_terminal_stage in ('error', 'skipped', 'request_timeout', 'no_candidates', 'provider_cooldown', 'provider_unavailable'))
        ...
        for lane in required:
            ...
            if lane == 'ct' and _ct_done:
                ...
                continue
            if lane == 'public':
                _tasks['public'] = safe_create_task(self._attempt_public_prewindup_barrier(query), name='prewindup:public')
            elif lane == 'ct':
        ...
        satisfied = (len(attempted) >= len(_handled) or all((r in skipped or r in attempted for r in _handled))) and all((r in skipped or r in attempted for r in _handled))
        self._result.prewindup_barrier_checked = True
        self._result.prewindup_barrier_required_lanes = required
        ...
        return PreWindupBarrierResult(required_lanes=required, satisfied=satisfied, attempted_lanes=tuple(attempted), skipped_lanes=tuple(skipped.keys()), error_lanes=tuple(errors.keys()), duration_s=duration)
        ...
        try:
            ...
            try:
                async with asyncio.timeout(10.0):
                    result = await async_run_live_public_pipeline(query=shaped, store=None, max_results=3, fetch_timeout_s=10.0, fetch_concurrency=2, hermes_engine=None, memory_manager=None, enqueue_hypothesis_pivot=None)
            ...
            try:
                ...
                if not _findings:
                    ...
                    if not _texts:
                        ...
                        _extraction = extract_pivot_seeds_from_texts(_texts, source_family='feed', max_texts=1000, max_seeds=256)
                        ...
                        if _seed_count == 0:
                            ...
                            self._result.planned_pivot_lanes = tuple(sorted({item.lane.upper() for item in _plan.items}))
                            self._result.pivot_integration_reason = 'pivot_planned' if _plan.items else 'pivot_planner_empty'
            except Exception as _exc:
    ...
    async def _run_mandatory_acquisition_prelude(self, result: SprintSchedulerResult, query: str, duckdb_store: Any, ct_log_client: Any) -> None:
        """
        ...
        if self._acquisition_plan is not None:
            try:
                ...
                _required: tuple[str, ...] = cast(tuple[str, ...], tuple((mlt.lane.value if hasattr(mlt.lane, 'value') else str(mlt.lane).upper() for mlt in _mlt_tuples if getattr(mlt, 'required', False))))
            except Exception as _exc:
            ...
            try:
                from hledac.universal.runtime.acquisition_strategy import build_acquisition_plan
                from hledac.universal.runtime.acquisition_strategy import required_terminal_lanes as _rtl
                _minimal = build_acquisition_plan(query=query, duration_s=60.0, aggressive_mode=self._config.aggressive_mode, uma_state=_uma, swap_detected=False, acquisition_profile=self._config.acquisition_profile if self._config.acquisition_profile is not None else 'default', source_quality_weights=self._policy_manager.get_src_quality_weights() if self._policy_manager is not None and self._policy_manager.enabled else None)
        ...
        if _coros_for_gather:
            _ok_results, _gathered_errors = await gather_taskgroup(_coros_for_gather, concurrency=2, ctx='prelude_gather')
        else:
        ...
        if _is_nonfeed_diagnostic and (not _nonfeed_prelude_done):
            ...
            if self._pivot_planner is not None:
                try:
                    ...
                    if _pivot_seeds:
                        _seed_dicts = [{'value': p.ioc_value, 'seed_type': p.ioc_type} for p in _pivot_seeds if p.ioc_value and p.ioc_type]
                        if _seed_dicts:
            ...
            try:
                import re as _re
                _speculative_domain_re = _re.compile('\\b(?:[a-zA-Z0-9](?:[a-zA-Z0-9\\-]{0,61}[a-zA-Z0-9])?\\.)+(?:com|org|net|io|onion|xyz|app|dev|info|me|cc|biz|co|tv|ai|cy|su|ua|ro|hr|si|click|link|top|icu|buzz|stream|live|news|film|pub|club|social|blog|vip|pro|ru|cn|uk|de|fr|nl|br|in|au|ca|eu|org|net|edu|gov|mil|arpa|adsl|bbs)\\b', _re.IGNORECASE)
                _speculative_noise = frozenset({'example', 'test', 'localhost', 'invalid', 'sample', 'foo', 'bar', 'baz', 'qux', 'demo', 'placeholder', 'domain'})
            ...
            if not _cleaned_domains and duckdb_store is not None:
                try:
                    if hasattr(duckdb_store, '_conn'):
                        _kw_clause = ' OR '.join((f"LOWER(query_text) LIKE '%{kw}%'" for kw in [w for w in query.lower().split() if len(w) >= 4][:5]))
                        if _kw_clause:
                    ...
                    pass
            from hledac.universal.runtime.nonfeed_seed_runtime import run_runtime_pivot_prelude as _run_runtime_pivot_prelude
            _pivot_profile_active = _is_nonfeed_diagnostic or _is_deep_osint_m1
            _speculative_had_seeds = getattr(self._result, 'seed_context_available', False) and getattr(self._result, 'seed_context_source', '') == 'speculative_query_extract'
            if not _speculative_had_seeds:
                try:
                    _pivot_result = await _run_runtime_pivot_prelude(query=query, duckdb_store=duckdb_store, nonfeed_diagnostic_active=_pivot_profile_active, existing_findings=None, acquisition_profile=self._config.acquisition_profile or 'default')
                    ...
                    self._result.lanes_unlocked_by_seed_context = _pivot_result.get('lanes_unlocked_by_seed_context', [])
            ...
            if not self._result.seed_context_available:
                import re as _re
                _is_domain_query = bool(_re.search('\\b(?:[a-zA-Z0-9](?:[a-zA-Z0-9\\-]{0,61}[a-zA-Z0-9])?\\.)+[a-zA-Z]{2,}\\b', query))
                if _is_domain_query and (not self._result.seed_context_available):
                    from hledac.universal.runtime.nonfeed_seed_runtime import _should_allow_low_quality_seed_for_profile, classify_seed_quality
                    ...
                    if _is_low_quality:
                        ...
                        _fake_seed = NonfeedSeed(value=_domain, kind='domain', source='query', confidence=1.0, reason='domain_query_fallback')
                        _quality = classify_seed_quality(_fake_seed, query=query, context='')
                        if _quality.decision == 'drop':
                            log.debug('[F241B] domain fallback blocked by quality gate: domain=%s reason=%s', _domain, _quality.reason)
                            self._result.seed_quality_drop_count = getattr(self._result, 'seed_quality_drop_count', 0) + 1
                            ...
                            self._result.seed_quality_keep_count = getattr(self._result, 'seed_quality_keep_count', 0) + 1
                    ...
                    try:
                        import re as _re
                        _noise = {'the', 'a', 'an', 'of', 'for', 'in', 'on', 'at', 'to', 'and', 'or', 'is', 'are', 'was', 'were', 'be', 'been', 'being', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'must', 'can', 'with', 'by', 'from', 'that', 'this'}
            ...
            if _planner_seed_iocs and self._result.planner_action_skip_reason in ('', 'no_iocs_extracted', None):
                ...
                if _pa_doms or _pa_ips or _pa_urls:
                    ...
                    self._result.seed_context_available = True
                    self._result.seed_context_propagated = True
                    log.debug('[F237B] planner_action seeds merged: domains=%s, ips=%s, urls=%s', _pa_doms, _pa_ips, _pa_urls)
            ...
            if _unlocked and self._result.seed_context_available:
                ...
                _seed_lane_defaults: dict[str, tuple[float, int]] = {'DOH': (12.0, 8), 'CT': (18.0, 10), 'WAYBACK': (18.0, 8), 'PASSIVE_DNS': (12.0, 8)}
            ...
            if query and isinstance(query, str) and query.strip():
                _candidates = extract_domain_candidates_from_text(query, source_url=None, source_family='query', min_confidence=0.3)
            ...
            _pivot_has_seeds = self._result.pivot_seed_domains or self._result.pivot_seed_ips or self._result.pivot_seed_urls
            _next_seeds_has_iocs = self._result.next_seeds_ioc_domains or self._result.next_seeds_ioc_ips or self._result.next_seeds_ioc_urls or self._result.next_seeds_ioc_hashes
            if _pivot_has_seeds or _next_seeds_has_iocs:
                _duckpgq_seeds: tuple[dict, ...] = ()
                try:
                    _csm = get_cross_sprint_memory()
                    if _csm.is_available():
                        _all_seed_values = list((self._result.pivot_seed_domains or ()) + (self._result.pivot_seed_ips or ()) + (self._result.next_seeds_ioc_domains or ()) + (self._result.next_seeds_ioc_ips or ()))
                        if _all_seed_values:
                    ...
                    pass
                _seed_ctx = NonfeedSeedContext(domains=tuple((_synthetic_domains or ()) + tuple(_query_domain_candidates or []) + (self._result.pivot_seed_domains or ()) + (self._result.next_seeds_ioc_domains or ())), ips=tuple((self._result.pivot_seed_ips or ()) + (self._result.next_seeds_ioc_ips or ())), urls=tuple((self._result.pivot_seed_urls or ()) + (self._result.next_seeds_ioc_urls or ())), hashes=tuple((self._result.pivot_seed_hashes or ()) + (self._result.next_seeds_ioc_hashes or ())), cves=tuple((self._result.pivot_seed_cves or ()) + (self._result.next_seeds_ioc_cves or ())), duckpgq_entities=_duckpgq_seeds)
            await self._run_nonfeed_prelude_gather(query=query, duckdb_store=duckdb_store, ct_log_client=ct_log_client, nonfeed_lanes_to_run=_nonfeed_lanes_to_run, hardware_critical=_hardware_critical, t0=_t0, time_module=_time, prelude_budget_s=_prelude_budget_s, nonfeed_prelude_attempted=_nonfeed_prelude_attempted, nonfeed_prelude_terminal=_nonfeed_prelude_terminal, nonfeed_prelude_skipped=_nonfeed_prelude_skipped, nonfeed_prelude_accepted=_nonfeed_prelude_accepted, nonfeed_prelude_errors=_nonfeed_prelude_errors, pivot_lanes=_pivot_lanes, seed_context=_seed_ctx)
            _nonfeed_prelude_done = True
            if _has_domain and (not self._result.seed_context_available) and _is_nonfeed_diagnostic:
                log.critical('[F300S-P1] domain_detected=True but seed_context_available=False (acquisition_profile=%s, query=%r) -- skipping windup, entering early-exit', self._config.acquisition_profile or 'default', query[:80])
        ...
        if _is_nonfeed_diagnostic:
            ...
            self._result.nonfeed_prelude_missing_lanes = tuple((k for k in _nonfeed_prelude_expected if k not in _nonfeed_prelude_terminal and k not in _nonfeed_prelude_skipped))
        ...
        if query and isinstance(query, str) and query.strip():
            try:
                ...
                _h3_prewarm_candidates = extract_domain_candidates_from_text(query, source_url=None, source_family='query', min_confidence=0.3)
                ...
                pass
        log.debug('[F209A] Acquisition prelude done: required=%s, terminal=%s, missing=%s, skipped=%s, errors=%s, dur=%.2fs', _required, _terminal_lanes, _missing, _skipped, _errors, self._result.acquisition_prelude_duration_s)

    async def _run_nonfeed_prelude_gather(self, query: str, duckdb_store: Any, ct_log_client: Any, nonfeed_lanes_to_run: list[tuple[str, bool, float, int]], hardware_critical: bool, t0: float, time_module: Any, prelude_budget_s: float, nonfeed_prelude_attempted: list[str], nonfeed_prelude_terminal: list[str], nonfeed_prelude_skipped: dict[str, str], nonfeed_prelude_accepted: dict[str, int], nonfeed_prelude_errors: dict[str, str], pivot_lanes: Sequence[Any] | None=None, seed_context: NonfeedSeedContext | None=None) -> None:
        """
                ...
                try:
                    async with asyncio.timeout(min(_lane_timeout, 20.0)):
                        if _lane_name == 'WAYBACK':
                            return await self._run_wayback_prelude_lane(query, duckdb_store, time_module, nonfeed_prelude_attempted, nonfeed_prelude_terminal, nonfeed_prelude_accepted, nonfeed_prelude_skipped, seed_context=seed_context)
                        elif _lane_name == 'PASSIVE_DNS':
                            return await self._run_pdns_prelude_lane(query, duckdb_store, time_module, nonfeed_prelude_attempted, nonfeed_prelude_terminal, nonfeed_prelude_accepted, seed_context=seed_context)
                        elif _lane_name == 'DOH':
                            return await self._run_doh_prelude_lane(query, duckdb_store, time_module, nonfeed_prelude_attempted, nonfeed_prelude_terminal, nonfeed_prelude_accepted, pivot_doh_items=pivot_lanes, seed_context=seed_context)
                        elif _lane_name == 'CT':
                            from hledac.universal.runtime.acquisition_strategy import AcquisitionLane, build_lane_query
                            ...
                            try:
                                async with asyncio.timeout(min(_lane_timeout, 20.0)):
                                    _ct_result, _ct_outcome = await _ct_adapter(query=_ct_shaped, max_results=_lane_max_items, timeout_s=min(_lane_timeout, 15.0))
                    ...
                    return (_lane_name, 0)
        _tasks = [_run_lane(_name, _timeout, _max_items) for _name, _enabled, _timeout, _max_items in nonfeed_lanes_to_run]
        if _tasks:
    ...
    async def _run_public_prelude_lane(self, query: str) -> dict:
        ...
        try:
            ...
            if isinstance(_shaped, dict) or not _shaped:
                return {'lane': 'PUBLIC', 'attempted': False, 'skipped': True, 'skip_reason': 'empty_public_query', 'raw_count': 0, 'built_count': 0, 'accepted_count': 0, 'error': None, 'timeout': False, 'duration_s': None}
            async with asyncio.timeout(10.0):
            ...
            return {'lane': 'PUBLIC', 'attempted': True, 'skipped': False, 'skip_reason': None, 'raw_count': getattr(_pipeline_result, 'discovered', 0) or 0, 'built_count': getattr(_pipeline_result, 'fetched', 0) or 0, 'accepted_count': getattr(_pipeline_result, 'accepted_findings', 0) or 0, 'error': getattr(_pipeline_result, 'error', None), 'timeout': getattr(_pipeline_result, 'timed_out', False), 'duration_s': getattr(_pipeline_result, 'elapsed_s', None)}
        except TimeoutError:
            return {'lane': 'PUBLIC', 'attempted': True, 'skipped': False, 'timeout': True, 'error': None, 'duration_s': 10.0}
        except Exception as exc:
            return {'lane': 'PUBLIC', 'attempted': True, 'skipped': False, 'error': f'{type(exc).__name__}:{exc}', 'timeout': False, 'duration_s': None}
    ...
    async def _run_ct_prelude_lane(self, query: str, seed_ctx: Any) -> tuple:
        ...
        import asyncio
        import time as _time
        from hledac.universal.runtime.acquisition_strategy import AcquisitionLane, AcquisitionLaneOutcome, build_lane_query
        ...
        try:
            ...
            _candidates_prelude, _rejections_prelude, _ct_telemetry_prelude = ct_results_to_findings(_ct_result, _ct_outcome_obj, query, sprint_id=f'prelude-{int(_time.time())}')
            _accepted = 0
            _ct_outcome_prelude = AcquisitionLaneOutcome(lane=AcquisitionLane.CT, enabled=True, attempted=True, accepted_findings=_accepted, produced_items=_ct_results_raw, duration_s=0.0, source_family='ct', ct_query=str(_shaped), ct_results_raw=_ct_results_raw, error=_ct_error, candidate_findings=tuple(_candidates_prelude), rejection_reasons=tuple(_rejections_prelude), rejected_count=len(_rejections_prelude), sample_rejections=tuple(_rejections_prelude[:3]))
        except TimeoutError:
            _ct_outcome_prelude = AcquisitionLaneOutcome(lane=AcquisitionLane.CT, enabled=True, attempted=True, timeout=True, duration_s=15.0, error='prelude_timeout', source_family='ct', ct_query=str(_shaped) if '_shaped' in dir() else '', ct_results_raw=0)
        except Exception:
        ...
        if _ct_results_raw > 0 and hasattr(_ct_result, 'hits') and _ct_result.hits:
            _prelude_keys = tuple(sorted({k for hit in list(_ct_result.hits)[:3] for k in (getattr(hit, 'url', None), getattr(hit, 'ct_name_value', None)) if k}))
        ...
        _telemetry = {'loss_stage': _loss_stage, 'bridge_invoked': _bridge_invoked, 'prelude_keys': _prelude_keys, 'candidates_count': _prelude_candidates, 'ct_quarantine_count': _ct_quarantine_count, 'ct_quarantine_entries': _ct_quarantine_entries}
        if isinstance(_ct_telemetry_prelude, dict):
            _telemetry.update({'ct_bridge_candidate_count': _ct_telemetry_prelude.get('ct_bridge_candidate_count', 0), 'ct_bridge_valid_domain_count': _ct_telemetry_prelude.get('ct_bridge_valid_domain_count', 0), 'ct_bridge_build_success_count': _ct_telemetry_prelude.get('ct_bridge_build_success_count', 0), 'ct_bridge_quality_rejected_count': _ct_telemetry_prelude.get('ct_bridge_quality_rejected_count', 0)})
        return (_ct_outcome_prelude, _ct_result, _telemetry)

    async def _run_wayback_prelude_lane(self, query: str, duckdb_store: Any, time_module: Any, nonfeed_prelude_attempted: list[str], nonfeed_prelude_terminal: list[str], nonfeed_prelude_accepted: dict[str, int], nonfeed_prelude_skipped: dict[str, str], seed_context: NonfeedSeedContext | None=None) -> tuple[str, int]:
            ...
            if _wb_cands and duckdb_store and hasattr(duckdb_store, 'async_ingest_findings_batch'):
                ...
                try:
                    if not self._enqueue_duckdb_write(duckdb_store, list(_wb_cands), self.sprint_id or ''):
                        _t = safe_create_task(self._gate_then_ingest_and_accumulate(duckdb_store, list(_wb_cands), sprint_id=self.sprint_id or ''))
                        self._bg_tasks.add(_t)
                        _t.add_done_callback(self._bg_tasks.discard)
                    _wb_acc = sum((1 for r in _ing if isinstance(r, dict) and r.get('accepted'))) if _ing else 0
                except Exception as _exc:
                    log.warning('sprint %s: wayback ledger write failed -- %s: %s', getattr(self._result, 'sprint_id', '?'), type(_exc).__name__, _exc)
                    _wb_acc = 0
    ...
    async def _run_pdns_prelude_lane(self, query: str, duckdb_store: Any, time_module: Any, nonfeed_prelude_attempted: list[str], nonfeed_prelude_terminal: list[str], nonfeed_prelude_accepted: dict[str, int], seed_context: NonfeedSeedContext | None=None) -> tuple[str, int]:
            ...
            if _pdns_cands and duckdb_store and hasattr(duckdb_store, 'async_ingest_findings_batch'):
                try:
                    if not self._enqueue_duckdb_write(duckdb_store, list(_pdns_cands), self.sprint_id or ''):
                        _t = safe_create_task(self._gate_then_ingest_and_accumulate(duckdb_store, list(_pdns_cands), sprint_id=self.sprint_id or ''))
                        self._bg_tasks.add(_t)
                        _t.add_done_callback(self._bg_tasks.discard)
                except Exception:
                    pass
    ...
    async def _run_doh_prelude_lane(self, query: str, duckdb_store: Any, time_module: Any, nonfeed_prelude_attempted: list[str], nonfeed_prelude_terminal: list[str], nonfeed_prelude_accepted: dict[str, int], pivot_doh_items: Sequence[Any] | None=None, seed_context: NonfeedSeedContext | None=None) -> tuple[str, int]:
                ...
                if _doh_cands and duckdb_store and hasattr(duckdb_store, 'async_ingest_findings_batch'):
                    try:
                        if not self._enqueue_duckdb_write(duckdb_store, list(_doh_cands), self.sprint_id or ''):
                            _t = safe_create_task(self._gate_then_ingest_and_accumulate(duckdb_store, list(_doh_cands), sprint_id=self.sprint_id or ''))
                            self._bg_tasks.add(_t)
                            _t.add_done_callback(self._bg_tasks.discard)
                    except Exception:
                        pass
    ...
    async def _ensure_mandatory_nonfeed_before_return(self, query: str, duckdb_store: Any, reason: str) -> bool:
        ...
        if not _is_domain:
            ...
            if _pub_discovered > 0 and self._result.accepted_findings > 0:
                ...
                logger.warning('[RETURN_GUARD] Non-domain query, no accepted findings (%d) — holding sprint open', self._result.accepted_findings)
        ...
        if not _unsatisfied:
            if self._result.accepted_findings > 0:
                ...
                logger.warning('[RETURN_GUARD] Domain query, all lanes terminal but zero accepted findings (%d) — holding sprint open', self._result.accepted_findings)
                self._result.return_guard_satisfied = False
    ...
    def _check_prewindup_barrier_sync(self, query: str, duckdb_store: Any) -> bool:
        ...
        if required_lanes and (not satisfied):
            ...
            log.debug('[P1-B] Windup blocked: required=%s satisfied=%s attempted=%s skipped=%s errors=%s', required_lanes, satisfied, attempted_lanes, skipped_lanes, barrier_errors)
            return False
        return True

    async def _run_one_cycle(self, lifecycle, sources: Sequence[str], now_monotonic: float | None=None, query: str='', duckdb_store: Any=None) -> bool:
        ...
        if not work_items:
            ...
            if self._result.consecutive_empty_cycles > self._result.max_consecutive_empty_cycles:
                self._result.max_consecutive_empty_cycles = self._result.consecutive_empty_cycles
            log.debug('[P1-1] Empty work_items cycle %d (consecutive=%d)', current_cycle + 1, self._result.consecutive_empty_cycles)
            try:
                _elapsed = _time.monotonic() - self._wall_clock_start
                await check_zero_findings_alert(elapsed_s=_elapsed, consecutive_empty_cycles=self._result.consecutive_empty_cycles, total_findings=self._result.accepted_findings)
            except Exception:
                pass
            return True
        self._result.cycles_started += 1
    ...
    async def _run_one_cycle_stable(self, lifecycle, work_items: list, query: str, duckdb_store: Any) -> bool:
        ...
        logger.debug('[PUBLIC_BRANCH_ENTRY] wall_elapsed=%.2f sprint_duration=%.2f windup_lead=%.2f remaining_s=%.2f', _wall_elapsed, self._config.sprint_duration_s, self._config.effective_windup_lead_s, lifecycle.remaining_time())
        remaining_s = lifecycle.remaining_time()
        _nonfeed_terminal = bool(self._result.lane_ct_accepted_findings > 0 or self._result.lane_wayback_accepted_findings > 0 or self._result.lane_pdns_accepted_findings > 0 or (self._result.lane_blockchain_accepted_findings > 0) or (self._result.lane_ipfs_accepted_findings > 0) or (self._result.lane_doh_accepted_findings > 0))
        ...
        async def fetch_one(work) -> tuple[str, FeedPipelineRunResult]:
            async with semaphore:
                ...
                if not should_fetch:
                    log.debug('[F216E] Feed source skipped by budget cap: url=%s reason=%s', work.feed_url, budget_reason)
                    return (work.feed_url, FeedPipelineRunResult(feed_url=work.feed_url, fetched_entries=0, accepted_findings=0, stored_findings=0, patterns_configured=0, matched_patterns=0, pages=(), error='feed_budget_cap_suppressed'))
                try:
                    async with asyncio.timeout(30.0):
                        from hledac.universal.pipeline.live_feed_pipeline import FeedIngestContext
                        result = await async_run_live_feed(feed_url=work.feed_url, max_entries=work.max_entries, sprint_id=self.sprint_id or '', store=duckdb_store, ingest_ctx=FeedIngestContext(privacy_layer=self._privacy_layer, evidence_log=self._evidence_log, graph_accumulator=self._graph_accumulator, temporal_predictor=self._temporal_predictor, layer_manager=getattr(self, '_layer_manager', None)) if duckdb_store is not None else None)
                    return (work.feed_url, result)
                except TimeoutError:
                    return (work.feed_url, FeedPipelineRunResult(feed_url=work.feed_url, fetched_entries=0, accepted_findings=0, stored_findings=0, patterns_configured=0, matched_patterns=0, pages=(), error='timeout'))
                except Exception as exc:
                    return (work.feed_url, FeedPipelineRunResult(feed_url=work.feed_url, fetched_entries=0, accepted_findings=0, stored_findings=0, patterns_configured=0, matched_patterns=0, pages=(), error=f'exception:{type(exc).__name__}:{exc}'))
        ...
        async def _run_public_branch() -> None:
            ...
            if branch_timeout <= 0:
                ...
                self._public_outcome = {'lane': 'PUBLIC', 'attempted': True, 'skipped': True, 'skip_reason': 'terminal:remaining_too_low', 'raw_count': 0, 'built_count': 0, 'accepted_count': 0, 'error': 'terminal:remaining_too_low', 'timeout': True, 'duration_s': None}
                return
            try:
                _seed_ctx = None
                async with asyncio.timeout(branch_timeout):
                    self._public_bootstrap_enabled_at_timeout = _bootstrap_enabled
                    await self._run_public_discovery_in_cycle(query=query, duckdb_store=duckdb_store, hermes_engine=self._hermes_engine, memory_manager=self._memory_manager, public_bootstrap_enabled=_bootstrap_enabled, seed_context=_initial_seed_ctx)
            except TimeoutError:
                ...
                self._public_outcome = {'lane': 'PUBLIC', 'attempted': True, 'skipped': False, 'skip_reason': None, 'raw_count': self._result.public_discovered, 'built_count': 0, 'accepted_count': 0, 'error': 'terminal:timeout', 'timeout': True, 'duration_s': None}
            except Exception as exc:
                log.debug('[stable] PUBLIC branch error: %s', exc)
                self._result.public_error = f'{type(exc).__name__}:{exc}'
                self._public_outcome = {'lane': 'PUBLIC', 'attempted': True, 'skipped': False, 'skip_reason': None, 'raw_count': self._result.public_discovered, 'built_count': 0, 'accepted_count': 0, 'error': str(exc), 'timeout': False, 'duration_s': None}
        ...
        async def _run_advisory_branch() -> None:
            ...
            try:
                async with asyncio.timeout(lanes_timeout):
                    ...
                    _next_seeds_has_iocs = self._result.next_seeds_ioc_domains or self._result.next_seeds_ioc_ips or self._result.next_seeds_ioc_urls or self._result.next_seeds_ioc_hashes
                    if _pivot_has_seeds or _next_seeds_has_iocs:
                        _seed_ctx = NonfeedSeedContext(domains=tuple(_query_domain_candidates + (self._result.pivot_seed_domains or ()) + (self._result.next_seeds_ioc_domains or ())), ips=tuple((self._result.pivot_seed_ips or ()) + (self._result.next_seeds_ioc_ips or ())), urls=tuple((self._result.pivot_seed_urls or ()) + (self._result.next_seeds_ioc_urls or ())), hashes=tuple((self._result.pivot_seed_hashes or ()) + (self._result.next_seeds_ioc_hashes or ())), cves=tuple((self._result.pivot_seed_cves or ()) + (self._result.next_seeds_ioc_cves or ())))
                    ...
                    async for _outcome_batch in run_enabled_acquisition_lanes_streaming(snapshot=self._acquisition_plan if self._acquisition_plan is not None else None, query=query, store=duckdb_store, uma_state=_uma, clearnet_max=_clearnet_max, seed_context=_seed_ctx, graph_accumulator=self._graph_accumulator, min_finished=0, on_lane_complete=None):
                        _streaming_outcomes = list(_outcome_batch)
                        _total_accepted = sum((getattr(_o, 'accepted_findings', 0) or 0 for _o in _streaming_outcomes if getattr(_o, 'attempted', False)))
                    ...
                    if _outcomes:
                        ...
                        if ct_findings:
                            await self._run_ct_to_passivedns_active_pivot(ct_findings=ct_findings, duckdb_store=duckdb_store, remaining_s=remaining_s)
                        for _rec in self._result.quality_rejection_ledger or ():
                            try:
                                self._nonfeed_ledger.add_quality_rejection(source_family=_rec.source_family or 'unknown', reason=_rec.reason or 'unknown', sample_url=getattr(_rec, 'url_sample', '') or '', sample_value=getattr(_rec, 'finding_id', '')[:16])
                            except Exception:
                                pass
                ...
                _log_advisory_dedup(log, f'stable_advisory_lane_fail:{type(_exc).__name__}', '[stable] ADVISORY lane runner exception: %s: %s', type(_exc).__name__, _exc)
        try:
    ...
    async def _drain_pending_pattern_extractions(self, remaining_s: float) -> None:
        ...
        if completed or timed_out:
            log.debug('[F273C] pattern-extraction drain: completed=%d timed_out=%d elapsed=%.2fs adaptive_deadline=%.2fs registry_pre=%d registry_post=%d', completed, timed_out, _elapsed, _adaptive_deadline_s, pre['registry_size'], post['registry_size'])
    ...
    def _branch_timeout_s(self, branch_name: str, remaining_s: float) -> float:
        ...
        try:
            ...
            pass
        logger.debug('[BRANCH_TIMEOUT] branch=%s remaining_s=%.2f floor=%.2f uma_state=%s uma_gib=%.2f', branch_name, remaining_s, floor, _uma_state, _uma_gib)
        if remaining_s < floor:
            logger.debug('[BRANCH_TIMEOUT] branch=%s SKIPPED remaining_s=%.2f < floor=%.2f (uma_state=%s)', branch_name, remaining_s, floor, _uma_state)
            return 0.0
        base = self._config.branch_timeout_budget_s if self._config.branch_timeout_budget_s > 0 else self._config.aggressive_branch_timeout_s
        try:
            from hledac.universal.core.resource_governor import sample_uma_status as _sample_uma
            uma = _sample_uma()
            ...
            if uma_state == 'emergency':
                base = min(base, 20.0)
                log.debug('[F273G+F265H-EXT] %s branch timeout clamped to %.1fs (uma_state=%s system_used_gib=%.2f concurrency=1)', branch_name, base, uma_state, system_used_gib)
            elif uma_state == 'critical':
                if system_used_gib >= 6.85:
                    base = min(base, 15.0)
                    log.debug('[F273G+F265H-EXT] %s branch timeout clamped to %.1fs (uma_state=%s system_used_gib=%.2f concurrency=2)', branch_name, base, uma_state, system_used_gib)
                else:
                    base = min(base, 20.0)
                    log.debug('[F273G+F265H-EXT] %s branch timeout clamped to %.1fs (uma_state=%s system_used_gib=%.2f concurrency=3)', branch_name, base, uma_state, system_used_gib)
            elif uma_state == 'warn':
                try:
                    from hledac.universal.core.protocols import get_governor
                    gov = get_governor()
        ...
        log.debug('[F212-B] %s branch timeout: base=%.1fs remaining=%.1fs capped=%.1fs', branch_name, base, remaining_s, bounded)
        return bounded
    ...
    async def _run_one_cycle_aggressive(self, lifecycle, work_items: list, query: str, duckdb_store: Any) -> bool:
        ...
        _wall_elapsed = _time.monotonic() - self._wall_clock_start
        logger.debug('[PUBLIC_BRANCH_ENTRY:AGGRESSIVE] wall_elapsed=%.2f sprint_duration=%.2f windup_lead=%.2f remaining_s=%.2f', _wall_elapsed, self._config.sprint_duration_s, self._config.effective_windup_lead_s, lifecycle.remaining_time())
        remaining_s = lifecycle.remaining_time()
        floor = self._min_branch_remaining_s(remaining_s)
        if remaining_s < floor:
            ...
            self._emit_source_family_event(family='PUBLIC', event='timeout', reason='terminal:remaining_too_low', terminal_state='terminal')
            self._emit_source_family_event(family='CT', event='timeout', reason='terminal:remaining_too_low', terminal_state='terminal')
            self._notify_governor_branch_timeout()
            self._notify_governor_branch_timeout()
            return True
        _nonfeed_terminal = bool(self._result.lane_ct_accepted_findings > 0 or self._result.lane_wayback_accepted_findings > 0 or self._result.lane_pdns_accepted_findings > 0 or (self._result.lane_blockchain_accepted_findings > 0) or (self._result.lane_ipfs_accepted_findings > 0) or (self._result.lane_doh_accepted_findings > 0))
        ...
        async def _run_feed_branch() -> None:
            ...
            async def fetch_one(work) -> tuple[str, FeedPipelineRunResult]:
                ...
                async with semaphore:
                    try:
                        ...
                        return (work.feed_url, FeedPipelineRunResult(feed_url=work.feed_url, fetched_entries=0, accepted_findings=0, stored_findings=0, patterns_configured=0, matched_patterns=0, pages=(), error=f'exception:{type(exc).__name__}:{exc}'))
            tasks = [fetch_one(w) for w in work_items]
            results: list[tuple[str, FeedPipelineRunResult]] = await safe_gather_ok(*tasks, label='sprint_scheduler:14339')
            for feed_url, result in results:
                self._process_result(feed_url, result)
            if work_items:
                domains = []
            ...
            if _accepted:
                _log = get_logger(__name__)
                _log.debug('[F205C] Aggressive feed accepted findings not in scope for sidecar dispatch. accepted=%d', _accepted)
        ...
        async def _run_public_branch() -> None:
            ...
            if branch_timeout <= 0:
                ...
                self._result.branch_skipped_remaining_too_low += 1
                self._result.public_error = 'terminal:remaining_too_low'
                self._public_outcome = {'lane': 'PUBLIC', 'attempted': True, 'skipped': True, 'skip_reason': 'terminal:remaining_too_low', 'raw_count': 0, 'built_count': 0, 'accepted_count': 0, 'error': 'terminal:remaining_too_low', 'timeout': False, 'duration_s': None}
                return
            if self._public_consecutive_timeouts >= 3:
                ...
                self._public_outcome = {'lane': 'PUBLIC', 'attempted': True, 'skipped': True, 'skip_reason': 'terminal:circuit_breaker', 'raw_count': 0, 'built_count': 0, 'accepted_count': 0, 'error': 'terminal:circuit_breaker', 'timeout': False, 'duration_s': None}
                return
            try:
                _seed_ctx = None
                async with asyncio.timeout(branch_timeout):
                    ...
                    await self._run_public_discovery_in_cycle(query=query, duckdb_store=duckdb_store, hermes_engine=self._hermes_engine, memory_manager=self._memory_manager, public_bootstrap_enabled=_bootstrap_en, seed_context=_seed_ctx)
            except TimeoutError:
                ...
                self._emit_source_family_event(family='PUBLIC', event='timeout', reason=_precise_stage, terminal_state='terminal')
                self._public_consecutive_timeouts += 1
                self._public_outcome = {'lane': 'PUBLIC', 'attempted': True, 'skipped': False, 'skip_reason': None, 'raw_count': _existing_raw, 'built_count': 0, 'accepted_count': _existing_accepted, 'error': _precise_stage, 'timeout': True, 'duration_s': branch_timeout}
            except asyncio.CancelledError:
                ...
                self._result.public_error = f'{type(exc).__name__}:{exc}'
                self._public_outcome = {'lane': 'PUBLIC', 'attempted': True, 'skipped': False, 'skip_reason': None, 'raw_count': self._result.public_discovered, 'built_count': 0, 'accepted_count': self._result.public_accepted_findings, 'error': f'{type(exc).__name__}:{exc}', 'timeout': False, 'duration_s': branch_timeout}
            else:
        ...
        if outer_timeout > 0:
            try:
                ...
                self._emit_source_family_event(family='PUBLIC', event='timeout', reason='terminal:envelope_timeout', terminal_state='terminal')
                self._emit_source_family_event(family='CT', event='timeout', reason='terminal:envelope_timeout', terminal_state='terminal')
                self._notify_governor_branch_timeout()
                self._notify_governor_branch_timeout()
                self._public_outcome = {'lane': 'PUBLIC', 'attempted': True, 'skipped': False, 'skip_reason': None, 'raw_count': self._result.public_discovered, 'built_count': 0, 'accepted_count': self._result.public_accepted_findings, 'error': 'terminal:envelope_timeout', 'timeout': True, 'duration_s': outer_timeout}
            except BaseExceptionGroup as eg:
        ...
        try:
            ...
            pass
        if not self._check_hard_deadline():
            log.debug('[D6] Hard deadline exceeded after aggressive branch envelope timeout -- skipping advisory lanes. outer_timeout=%.1fs remaining_s=%.1fs', outer_timeout, remaining_s)
            return True
        else:
            log.debug('[F212-B] Aggressive gather skipped: outer_timeout=%.1fs (remaining=%.1fs)', outer_timeout, remaining_s)
            self._result.public_branch_timed_out = True
            self._result.ct_branch_timed_out = True
            self._result.branch_timeout_count += 2
            self._result.branch_skipped_remaining_too_low += 2
        ...
        if lanes_timeout > 0:
            try:
                async with asyncio.timeout(lanes_timeout):
                    ...
                    if self._governor is not None:
                        try:
                            ...
                            pass
                    _streaming_outcomes: list = []
                    async for _outcome_batch in run_enabled_acquisition_lanes_streaming(snapshot=self._acquisition_plan if self._acquisition_plan is not None else None, query=query, store=duckdb_store, uma_state=_uma, clearnet_max=_clearnet_max_adv, seed_context=None, graph_accumulator=self._graph_accumulator, min_finished=0, on_lane_complete=None):
                        _streaming_outcomes = list(_outcome_batch)
                        _total_accepted = sum((getattr(_o, 'accepted_findings', 0) or 0 for _o in _streaming_outcomes if getattr(_o, 'attempted', False)))
                        if _total_accepted >= 3:
                            break
                ...
                _log_advisory_dedup(log, f'aggressive_advisory_lane_fail:{type(_exc).__name__}', '[aggressive] ADVISORY lane runner exception: %s: %s', type(_exc).__name__, _exc)
        else:
            log.debug('[F212-B] ADVISORY lanes skipped in aggressive: remaining=%.1fs', remaining_s)
        return True

    async def _run_public_discovery_in_cycle(self, query: str='', duckdb_store: Any=None, hermes_engine: Any | None=None, memory_manager: Any | None=None, public_bootstrap_enabled: bool=False, seed_context: Any | None=None) -> None:
        """
        ...
        try:
            ...
            self._public_outcome = {'lane': 'PUBLIC', 'attempted': True, 'skipped': False, 'skip_reason': None, 'raw_count': self._result.public_discovered, 'built_count': 0, 'accepted_count': self._result.public_accepted_findings, 'error': f'import:{type(exc).__name__}', 'timeout': False, 'duration_s': None}
        ...
        try:
            public_result = await async_run_public(query=query_hint, store=duckdb_store, max_results=5, fetch_timeout_s=35.0, fetch_concurrency=3, hermes_engine=hermes_engine, memory_manager=memory_manager, enqueue_hypothesis_pivot=self.enqueue_hypothesis_pivot, public_bootstrap_enabled=public_bootstrap_enabled, seed_context=seed_context)
        except asyncio.CancelledError:
            raise
            ...
            self._public_outcome = {'lane': 'PUBLIC', 'attempted': True, 'skipped': False, 'skip_reason': None, 'raw_count': self._result.public_discovered, 'built_count': 0, 'accepted_count': self._result.public_accepted_findings, 'error': f'TaskGroup: {type(eg).__name__}', 'timeout': False, 'duration_s': None}
            try:
                _psd = {'candidate_providers': [], 'selected_provider': None, 'rejected_providers': [], 'rejection_reasons': {}, 'provider_errors': [{'provider': 'TaskGroup', 'error': _sanitize_debug_text(eg, max_chars=500), 'error_type': type(eg).__name__}], 'missing_dependencies': [], 'policy_flags': {}, 'bootstrap_enabled': False, 'bootstrap_disabled_reason': 'ExceptionGroup', 'exception_group_summary': _sanitize_debug_text(eg, max_chars=500)}
        ...
        self._public_outcome = {'lane': 'PUBLIC', 'attempted': True, 'skipped': False, 'skip_reason': None, 'raw_count': getattr(public_result, 'discovered', 0) or 0, 'built_count': getattr(public_result, 'fetched', 0) or 0, 'accepted_count': getattr(public_result, 'accepted_findings', 0) or 0, 'error': getattr(public_result, 'error', None), 'timeout': getattr(public_result, 'timed_out', False), 'duration_s': getattr(public_result, 'elapsed_s', None), 'public_bootstrap_enabled': _pub_bootstrap_en, 'public_bootstrap_order': _pub_bootstrap_ord, 'keyword_seed_fallback_triggered': _keyword_seed_fallback, 'public_bootstrap_prevented_discovery_timeout': _pub_bootstrap_prevented, 'public_bootstrap_first_fetch_attempted': _pub_bootstrap_fetch_att, 'public_discovered': _pub_discovered, 'public_fetch_candidate_count': _pub_fetch_candidate, 'public_fetch_attempted': _pub_fetch_attempted, 'public_fetch_success': _pub_fetch_success, 'public_fetch_failed': _pub_fetch_failed, 'public_acceptance_attempted': _pub_acceptance_attempted, 'public_acceptance_accepted': _pub_acceptance_accepted, 'public_acceptance_rejected': _pub_acceptance_rejected, 'public_acceptance_reject_reasons': _pub_acceptance_reject_reasons, 'public_terminal_classified_count': _pub_terminal_classified, 'public_unclassified_count': _pub_unclassified, 'public_terminal_reason_counts': _pub_terminal_reason_counts, 'public_skipped_duplicate': _pub_skipped_duplicate, 'public_skipped_unsupported_scheme': _pub_skipped_scheme, 'public_skipped_memory_gate': _pub_skipped_mem, 'public_skipped_quality_gate': _pub_skipped_quality, 'security_rejected_count': _security_rejected, 'pii_redacted_count': _pii_redacted, 'public_skipped_browser_unavailable': _pub_skipped_browser, 'public_skipped_xml_or_feed': _pub_skipped_xml, 'public_skipped_timeout': _pub_skipped_timeout, 'public_skipped_fetch_error': _pub_skipped_fetch_err, 'public_skipped_url_sample': _pub_skipped_url_sample, 'public_rejected_no_pattern_match': _pub_rejected_no_pattern, 'public_rejected_low_information': _pub_rejected_low_info, 'public_rejected_duplicate': _pub_rejected_duplicate, 'public_rejected_storage_rejected': _pub_rejected_storage, 'public_rejected_url_samples': _pub_rejected_url_sample, 'public_accepted_url_sample': _pub_accepted_url_sample}
        self._result.lane_public_accepted_findings = getattr(public_result, 'accepted_findings', 0) or 0
        _psd = {'candidate_providers': list(getattr(public_result, 'public_provider_selected', []) or []), 'selected_provider': list(getattr(public_result, 'public_provider_selected', []) or [])[0] if getattr(public_result, 'public_provider_selected', None) else None, 'rejected_providers': [p.get('provider', '?') for p in getattr(public_result, 'public_provider_skipped', []) or []], 'rejection_reasons': {p.get('provider', '?'): _sanitize_debug_text(p.get('reason', '')) for p in getattr(public_result, 'public_provider_skipped', []) or []}, 'provider_errors': [{'provider': _sanitize_debug_text(e.get('provider', '?') if isinstance(e, dict) else str(e)), 'error': _sanitize_debug_text(e.get('error', '') if isinstance(e, dict) else str(e)), 'error_type': e.get('error_type', '?') if isinstance(e, dict) else '?'} for e in getattr(public_result, 'public_provider_errors', []) or []], 'missing_dependencies': [], 'policy_flags': {'bootstrap_enabled': _pub_bootstrap_en, 'bootstrap_order': _pub_bootstrap_ord, 'bootstrap_prevented_discovery_timeout': _pub_bootstrap_prevented, 'bootstrap_first_fetch_attempted': _pub_bootstrap_fetch_att, 'public_bootstrap_enabled_at_result': _pub_bootstrap_en}, 'bootstrap_enabled': _pub_bootstrap_en, 'bootstrap_disabled_reason': 'nonfeed_diagnostic_profile_required' if not _pub_bootstrap_en and _pub_bootstrap_ord == 'disabled' else '', 'seed_context_available': _seed_ctx_has_any_items(seed_context), 'bootstrap_eligible': _pub_bootstrap_en and _pub_bootstrap_ord != 'disabled' and _seed_ctx_has_any_items(seed_context), 'bootstrap_used': getattr(public_result, 'public_bootstrap_candidates_count', 0) > 0, 'bootstrap_candidate_count': getattr(public_result, 'public_bootstrap_candidates_count', 0) or 0, 'discovery_error_type': getattr(public_result, 'discovery_error_type', '') or '', 'discovery_elapsed_s': getattr(public_result, 'discovery_elapsed_s', None)}
        self._result.public_provider_selection_debug = _psd
        log.debug(f'[8XE] Public discovery: discovered={public_result.discovered} matched={public_result.matched_patterns} accepted={public_result.accepted_findings}')
        if public_result.accepted_findings > 0 or public_result.stored_findings > 0:
            log.debug('[F205C] Public accepted findings not in scheduler scope for sidecar dispatch (pipeline-internal storage). accepted=%d stored=%d', public_result.accepted_findings, public_result.stored_findings)
        if ENV.get_bool('HLEDAC_ENABLE_QUERY_ROUTER'):
            try:
                ...
                if _qr_results:
                    _ = await self._gate_then_ingest_and_accumulate(duckdb_store, _qr_results, sprint_id=self.sprint_id or '')
                    log.debug('[F11E] QueryRouter ingested %d findings', len(_qr_results))
            except Exception as _qr_err:
                log.debug('[F11E] QueryRouter failed (non-fatal): %s', _qr_err)

    async def _dispatch_accepted_findings_sidecars(self, source_branch: str, findings: list, store: Any, query: str) -> None:
        """
        ...
        if self._sidecar_orchestrator is None:
            return
        await self._sidecar_orchestrator.dispatch_findings(source_branch=source_branch, findings=findings, store=store, query=query, sprint_id=self.sprint_id or '')
    ...
    async def _run_ct_log_discovery_in_cycle(self, query: str, store: Any) -> None:
        ...
        try:
            ...
            if findings:
                if self._enrichment_services:
                    await safe_gather_fire_and_forget(self._enrichment_services.enrich_ct_findings(findings, self._result), self._enrichment_services.enrich_findings_multimodal(findings, self._result), label='enrichment_ct_multimodal_parallel', logger_instance=log)
                try:
                ...
                if stored > 0:
                    ...
                    if hasattr(store, 'get_quality_rejection_ledger'):
                        ...
                        _ct_reasons = tuple((r.reason for r in _ledger if getattr(r, 'source_family', '') == 'ct_log'))[-10:]
                        if _ct_reasons:
                            self._result.ct_storage_rejection_reasons = _ct_reasons
                            log.warning('ct_storage_rejected: candidates=%d stored=%d top_reason=%s reasons=%s', len(findings), stored, _ct_reasons[0] if _ct_reasons else 'unknown', _ct_reasons)
                self._result.ct_log_accepted_findings = stored
                accepted_findings = [f for f, r in zip(findings, results, strict=False) if isinstance(r, dict) and r.get('accepted')]
                await self._dispatch_accepted_findings_sidecars(source_branch='ct', findings=accepted_findings, store=store, query=query)
                    ...
                    if security is not None and hasattr(security, 'validate_finding'):
                        ...
                        for f in accepted_findings:
                            try:
                                ...
                                if not _ok:
                                    _sec_rejected += 1
                                    logger.debug('[security_gate] rejected finding %s: %s', getattr(f, 'finding_id', ''), _reason)
                                    continue
                        ...
                        self._result.security_rejected_count = getattr(self._result, 'security_rejected_count', 0) + _sec_rejected
                        self._result.pii_redacted_count = getattr(self._result, 'pii_redacted_count', 0) + _sec_pii_redacted
                    if ENV.get_bool('HLEDAC_ENABLE_PRIVACY_LAYER'):
                        try:
                            _privacy = self._privacy_layer or getattr(self._layer_manager, 'privacy', None)
                            if _privacy and accepted_findings:
                                accepted_findings, _pii_count = await self._run_privacy_gate_async(accepted_findings, _privacy)
                                if _pii_count > 0:
                                    self._result.pii_findings_anonymized = getattr(self._result, 'pii_findings_anonymized', 0) + _pii_count
                        except Exception as _e:
                ...
                if ENV.get_bool('HLEDAC_ENABLE_LAYERS') and accepted_findings:
                    ...
                    for f in accepted_findings:
                        if ghost is not None:
                            try:
                                await ghost.execute_action(ActionType.SCAN, {'finding_id': getattr(f, 'finding_id', '') or '', 'query': query}, store_in_vault=True)
                        ...
                        if security is not None:
                            try:
                                ...
                                if audit is not None and hasattr(audit, 'log_action'):
                                    audit.log_action('finding_accepted', b'', {'finding_id': getattr(f, 'finding_id', '') or '', 'query': query})
                            except Exception as _e:
    ...
    async def _run_onion_discovery_sidecar(self) -> None:
        ...
        try:
            ...
            return
        try:
            ahmia_query = ' '.join([word for word in query.split() if len(word) > 2 and (not word.startswith('http'))][:5])
            if ahmia_query:
        ...
        async def crawl_seed(seed: str) -> list:
            ...
            inner_findings: list = []
            try:
                from hledac.universal.intelligence.dark_web_intelligence import DarkWebCrawler, darkweb_content_to_canonical
        ...
        try:
            ...
            _log_advisory_dedup(log, f'onion_ingest_fail:{type(e).__name__}', '[F251] Onion findings ingest failed: %s', e)
        ...
        async def fetch_i2p_address(addr: str) -> list:
            ...
            try:
                ...
                if content:
                    ...
                    from hledac.universal.knowledge.duckdb_store import CanonicalFinding
                    finding = CanonicalFinding(finding_id=f'i2p-{addr}-{int(datetime.now(UTC).timestamp() * 1000)}', query='i2p_discovery', source_type=SourceType.I2P_DISCOVERY, confidence=0.5, ts=datetime.now(UTC).isoformat(), provenance=['i2p_transport'], payload_text=content[:4096] if len(content) > 4096 else content)
                    if self._evidence_log is not None:
                        try:
                            self._evidence_log.create_event('observation', finding.model_dump() if hasattr(finding, 'model_dump') else vars(finding), source_ids=[finding.source_id] if hasattr(finding, 'source_id') else [], confidence=getattr(finding, 'confidence', 0.5))
                        except Exception:
                            pass
        ...
        try:
            ...
            try:
                if duckdb is not None:
                    ...
                    async with duckdb_pool.connection() as conn:
                        rows = await conn.execute(f"SELECT DISTINCT provenance FROM findings WHERE source_type = '{SourceType.CT_LOG}' AND provenance LIKE '%info_hash%' LIMIT 20").fetch()
                    for row in rows:
            ...
            if node is None:
                ...
                try:
                    ...
                    for r in results:
                        if r.get('info_hash'):
                            findings.append(dht_content_to_canonical(DHTFinding(info_hash=r['info_hash'], name=r.get('name', ''), files=r.get('files', []), size_bytes=r.get('size_bytes', 0), peers=r.get('peers', 0), source='dht_fallback'), query=self._query[:128]))
                except Exception as e:
            ...
            for batch in results:
                if isinstance(batch, list):
                    for item in batch:
                        try:
                            dht_finding = DHTFinding(info_hash=item.get('info_hash', ''), name=item.get('name', ''), files=item.get('files', []), size_bytes=item.get('size_bytes', 0), peers=item.get('peers', 0), source='dht')
                ...
                try:
                    ...
                    if response.items:
                        for item in response.items[:50]:
                            finding_dict = gopher.item_to_finding(item, query=query, sprint_id=getattr(self._result, 'sprint_id', None) or '')
    ...
    async def _run_digital_ghost_sidecar(self, file_findings: list) -> list:
        ...
        try:
            ...
            for r in results:
                ...
                try:
                    if hasattr(r, 'ghost_signals') and r.ghost_signals:
                        from hledac.universal.knowledge.duckdb_store import CanonicalFinding
                        finding = CanonicalFinding(source_type=SourceType.DIGITAL_GHOST_DETECTION, ioc_type='file', ioc_value=getattr(r, 'file_path', ''), confidence=getattr(r, 'overall_confidence', 0.5), payload={'ghost_signals_count': len(getattr(r, 'ghost_signals', [])), 'ghost_signals': [gs.__dict__ if hasattr(gs, '__dict__') else str(gs) for gs in getattr(r, 'ghost_signals', [])], 'deletion_indicators': getattr(r, 'deletion_indicators', False), 'recovered_content_count': len(getattr(r, 'recovered_content', []))}, tags=['digital_ghost', 'forensics'])
            ...
            _log_advisory_dedup(log, f'dghost_sidecar_fail:{type(e).__name__}', '[F3FORENSICS] Digital ghost sidecar failed: %s', e)
        return findings
    ...
    async def _run_steganography_sidecar(self, image_findings: list) -> list:
        ...
        try:
            ...
            for r in results:
                ...
                try:
                    ...
                    if suspicious and confidence > 0.3:
                        from hledac.universal.knowledge.duckdb_store import CanonicalFinding
                        finding = CanonicalFinding(source_type=SourceType.STEGANOGRAPHY_DETECTION, ioc_type='file', ioc_value=getattr(r, 'file_path', ''), confidence=confidence)
                        if self._evidence_log is not None:
                            try:
                                self._evidence_log.create_event('observation', finding.model_dump() if hasattr(finding, 'model_dump') else vars(finding), source_ids=[finding.source_id] if hasattr(finding, 'source_id') else [], confidence=getattr(finding, 'confidence', 0.5), payload={'chi_square_score': getattr(r, 'chi_square_score', 0.0), 'entropy_score': getattr(r, 'entropy_score', 0.0), 'lsb_suspicious': getattr(r, 'lsb_suspicious', False), 'stegdetect_available': getattr(r, 'stegdetect_available', False), 'stegdetect_result': getattr(r, 'stegdetect_result', None)}, tags=['steganography', 'forensics'])
                            except Exception:
                                pass
                        findings.append(finding)
                except Exception:
            ...
            _log_advisory_dedup(log, f'steg_sidecar_fail:{type(e).__name__}', '[F3FORENSICS] Steganography sidecar failed: %s', e)
        return findings
    ...
    async def _run_bgp_enrichment_sidecar(self) -> list[CanonicalFinding]:
        ...
        async def _query_one(ip_or_asn: str) -> list[CanonicalFinding]:
            try:
                async with sem:
                    return await safe_wait_for(bgp_enrich_to_canonical(ip_or_asn, query_context='sprint_enrichment'), timeout=30.0, label='bgp_enrich')
            except Exception:
                return []
        try:
    ...
    async def _run_banner_grab_sidecar(self) -> list[CanonicalFinding]:
        ...
        async def _grab_one(ip: str) -> list[CanonicalFinding]:
            try:
                return await safe_wait_for(banner_grab_to_canonical(ip, ports=ports, query_context='sprint_enrichment'), timeout=60.0, label='banner_grab')
            except Exception:
                return []
        try:
    ...
    def _accumulate_findings_to_graph(self, findings: list, sprint_id: str='') -> int:
        ...
        if ioc_graph is not None and (not getattr(ioc_graph, '_lock_acquired', True)):
            logger.warning('[GRAPH] IOC graph is READ-ONLY — IOC accumulation disabled (another sprint is holding the graph lock). Findings are still stored in DuckDB.')
            return 0
        ...
        for f in findings:
            try:
                if isinstance(f, dict):
                    text_fields = {'content': f.get('content') or '', 'raw_content': f.get('raw_content') or '', 'payload_text': f.get('payload_text') or '', 'title': f.get('title') or '', 'summary': f.get('summary') or ''}
                else:
                    text_fields = {'content': getattr(f, 'content', None) or '', 'raw_content': getattr(f, 'raw_content', None) or '', 'payload_text': getattr(f, 'payload_text', None) or '', 'title': getattr(f, 'title', None) or '', 'summary': getattr(f, 'summary', None) or ''}
                has_pii = False
                for field_name, field_value in text_fields.items():
                    if not field_value:
                        continue
                    pii_result = privacy_layer.detect_pii(field_value)
                    field_has_pii = bool(pii_result) if isinstance(pii_result, bool) else any((v for v in pii_result.values() if v))
                    if field_has_pii:
                        has_pii = True
    ...
    async def _gate_then_ingest_and_accumulate(self, store: Any, findings: list, sprint_id: str='') -> Any:
        ...
        if self._evidence_log is not None:
            try:
                self._evidence_log.create_event('observation', {'phase': 'CREATED', 'findings_count': len(findings), 'sprint_id': sprint_id or '', 'source': store.__class__.__name__ if hasattr(store, '__class__') else str(type(store))}, source_ids=[], confidence=1.0)
            except Exception:
                pass
        if self._evidence_log is not None:
            try:
                _finding_ids = [getattr(f, 'finding_id', None) or getattr(f, 'source_id', None) or str(hash(str(f))) for f in findings]
                self._evidence_log.create_event('observation', {'phase': 'CANDIDATE', 'findings_count': len(findings), 'finding_ids': _finding_ids[:20]}, source_ids=_finding_ids[:20], confidence=1.0)
            except Exception:
                pass
        ...
        if self._evidence_log is not None and _ingest_result is not None:
            try:
                if isinstance(_ingest_result, list):
                    ...
                    if _accepted > 0:
                        self._evidence_log.create_event('observation', {'phase': 'ACCEPTED', 'accepted_count': _accepted, 'total': len(_ingest_result)}, source_ids=[], confidence=1.0)
                    if _rejected > 0:
                        self._evidence_log.create_event('observation', {'phase': 'REJECTED', 'rejected_count': _rejected, 'total': len(_ingest_result)}, source_ids=[], confidence=1.0)
            except Exception:
                pass
        if len(findings) <= _MAX_CHUNK_SIZE and findings and (_ingest_result is not None):
            try:
                if isinstance(_ingest_result, list) and len(_ingest_result) == len(findings):
                    _accepted_findings = [f for f, r in zip(findings, _ingest_result) if isinstance(r, dict) and r.get('accepted')]
                else:
                ...
                if len(findings) > 0:
                    ...
                    logger.debug('[F320] graph_accum: %d/%d accepted (rejected=%d) for sprint_id=%s', _accepted, _total, _rejected, sprint_id or '')
    ...
    async def _process_chunk_parallel(self, chunk: list, sem: asyncio.Semaphore, sprint_id: str) -> tuple[list, list | None]:
                ...
                if self._graph_accumulator and duck_results is not None:
                    try:
                        if isinstance(duck_results, list) and len(duck_results) == len(chunk):
                            _accepted_chunk = [f for f, r in zip(chunk, duck_results) if isinstance(r, dict) and r.get('accepted')]
                        else:
        ...
        try:
            ...
            for src, dst, data in nx_graph.edges(data=True):
                ...
                if key not in seen_rels:
                    _DEFAULT_GRAPH_SERVICE.upsert_relation(src, dst, 'latent_related', weight=data.get('weight', 1.0), evidence='latent_networkx')
                    seen_rels.add(key)
                    added += 1
            if added:
    ...
    async def _run_ct_to_passivedns_active_pivot(self, ct_findings: list, duckdb_store: Any | None, remaining_s: float) -> None:
        """
        ...
        async def _run_pdns_for_domain(domain: str) -> tuple[str, list[str], PassiveDNSOutcome | None]:
            try:
                ...
                return (domain, [], None)
        try:
            gather_results = await safe_gather_ok(*[_run_pdns_for_domain(d) for d in pivot_domains], label='sprint_scheduler:18182')
        ...
        if pdns_results and store is not None:
            for res in pdns_results:
                ...
                if not ips or outcome is None:
                    continue
                pdns_findings, pdns_rejections, pdns_telemetry = passive_dns_results_to_findings(ips, outcome, domain, sprint_id=self.sprint_id or '')
                if pdns_findings:
                    try:
            ...
            for res in pdns_results:
                try:
                    ...
                    if outcome and hasattr(outcome, 'result_count'):
                        ...
                        if count > 0:
                            ledger.add_pivot_discovered(pivot_type='ct_to_passivedns', ioc_value=domain, source_hint=f'ct_domain:{domain}', reason=f'pdns_results={count}')
        ...
        if pdns_results:
            ...
            for res in pdns_results:
                ...
                if outcome is None:
                    continue
                _sfos.append({'family': 'passive_dns', 'lane': 'PASSIVE_DNS', 'attempted': getattr(outcome, 'attempted', True), 'accepted': getattr(outcome, 'result_count', 0) or 0, 'terminal_state': 'pivot_ct_domain', 'raw_count': 0, 'accepted_count': getattr(outcome, 'result_count', 0) or 0, 'error': getattr(outcome, 'error', None), 'timeout': getattr(outcome, 'timeout', False), 'skipped': False, 'pivot_source': 'ct', 'pivot_phase': 'active', 'pivot_domains': pivot_domains})
    ...
    def _accumulate_lane_findings(self, outcomes: tuple, query: str) -> None:
        ...
        """
        _LANE_SOURCE_MAP = {AcquisitionLane.CT: 'ct', AcquisitionLane.WAYBACK: 'wayback_archive', AcquisitionLane.PASSIVE_DNS: 'passive_dns', AcquisitionLane.BLOCKCHAIN: 'blockchain', AcquisitionLane.IPFS: 'ipfs', AcquisitionLane.PUBLIC: 'public'}
        for outcome in outcomes:
            ...
            if accepted > 0:
                ...
                if candidate_findings and remaining > 0:
                    ...
                    for cf in _cf_slice:
                        try:
                            _new_entries.append({'type': f"lane_{getattr(cf, 'source_type', verdict_tag) or verdict_tag}", 'source': getattr(cf, 'source_type', verdict_tag) or verdict_tag, 'matched_patterns': produced, 'accepted_findings': accepted, 'severity': 'medium', 'confidence': getattr(cf, 'confidence', 0.5) or 0.5, 'description': str(getattr(cf, 'payload_text', 'bridge finding') or 'bridge finding')[:200], 'ts': getattr(cf, 'ts', 0.0) or 0.0})
                        except asyncio.CancelledError:
                            raise
                        except Exception:
                            _errored += 1
                ...
                _already_graphed_in_lane = {AcquisitionLane.CT, AcquisitionLane.WAYBACK, AcquisitionLane.PASSIVE_DNS, AcquisitionLane.DOH, AcquisitionLane.BLOCKCHAIN, AcquisitionLane.IPFS, AcquisitionLane.ACADEMIC}
                if candidate_findings and lane_name not in _already_graphed_in_lane:
                    ...
                    if len(self._all_findings) < self._MAX_FINDINGS_PER_SPRINT:
                        self._all_findings.append({'type': f'lane_{verdict_tag}', 'source': verdict_tag, 'matched_patterns': produced, 'accepted_findings': accepted, 'severity': 'medium', 'confidence': quality * 0.8, 'description': f'{accepted} accepted findings from {verdict_tag} lane in {duration:.1f}s'})
            if rejected_count > 0 and source_family != 'unknown':
                ...
                for reason, count in reason_counts.items():
                    ...
                    self._lane_rejections.append({'source_family': source_family, 'rejection_reason': reason, 'rejected_count': count, 'sample': list(sample_rejections[:3]), 'verdict_tag': verdict_tag, 'lane_name': str(lane_name) if lane_name else 'unknown'})
    ...
    def _ingest_feed_public_candidates_to_ledger(self) -> None:
        ...
        """
        try:
            from hledac.universal.runtime.nonfeed_candidate_ledger import FAMILY_FEED, FAMILY_PUBLIC, MAX_DOMAIN_CANDIDATES_FOR_LANES, MAX_FEED_CANDIDATES, compute_lane_eligibility, extract_domain_candidates_from_text, filter_source_host_only, rank_candidates
        except Exception:
            return
        try:
            ...
            for outcome in outcomes:
                ...
                for c in candidates:
                    ...
                    if payload_text:
                        text_candidates = extract_domain_candidates_from_text(payload_text, source_url=source_url if source_url else None, source_family=source_family)
                        for tc in text_candidates[:MAX_FEED_CANDIDATES]:
                            all_candidates.append(tc)
                    if query:
                        query_candidates = extract_domain_candidates_from_text(query, source_url=source_url if source_url else None, source_family=source_family)
            ...
            if source_url_by_family:
                for _sf, surl in source_url_by_family.items():
                    if surl:
                        ...
                        break
            ranked = rank_candidates(all_candidates, max_total=MAX_DOMAIN_CANDIDATES_FOR_LANES, source_host_domains=source_host_domains)
            mv = get_domain_mv()
            for tc in ranked:
                try:
                    self._nonfeed_ledger.add_feed_candidate(domain=tc.domain, source_field=tc.source_field, confidence=tc.confidence, reason=f'{tc.reason} (seen={tc.seen_count})', sample_context=tc.sample_context[:200] if tc.sample_context else '')
                    try:
                        mv.upsert_candidate(domain=tc.domain, source_family=getattr(tc, 'source_field', '') or 'feed', ioc_type='domain', confidence=tc.confidence, rank_score=getattr(tc, 'rank_score', None), nonfeed_eligible_ct=eligibility.get('ct', {}).get('eligible', False) if isinstance(eligibility, dict) else False, nonfeed_eligible_doh=eligibility.get('doh', {}).get('eligible', False) if isinstance(eligibility, dict) else False, nonfeed_eligible_wayback=eligibility.get('wayback', {}).get('eligible', False) if isinstance(eligibility, dict) else False, nonfeed_eligible_pdns=eligibility.get('passive_dns', {}).get('eligible', False) if isinstance(eligibility, dict) else False)
                    except Exception:
                        pass
                except Exception:
                    pass
            ...
            if ranked:
                ...
                pdns_candidates = [tc.domain for tc in ranked if not tc.domain[0].isdigit() or tc.domain[0].isdigit()][:10]
        ...
        for outcome in outcomes:
            ...
            for rej in rejection_reasons[:5]:
                try:
                    ...
                    self._nonfeed_ledger.add_ct_quarantine(domain=domain_str, reject_reason=reason_str, source_url='', query=getattr(outcome, 'ct_query', '') or '')
                except Exception:
                    pass
            try:
                ...
                for i, result in enumerate(storage_results):
                    is_accepted = getattr(result, 'lmdb_success', False) if hasattr(result, 'lmdb_success') else result.get('lmdb_success', False) if isinstance(result, dict) else False
                    ...
                    if is_accepted:
                        accepted_count += 1
                        try:
                            self._nonfeed_ledger.add_public_event(stage='stored', candidate_id=candidate_id, reason='ct_stored', accepted=True, sample_url='', sample_value=domain[:64] if domain else candidate_id[:16])
                        except Exception:
                            pass
                    else:
                        try:
                            self._nonfeed_ledger.add_public_event(stage='quarantine', candidate_id=candidate_id, reason='ct_quarantine', accepted=False, sample_url='', sample_value=domain[:64] if domain else candidate_id[:16])
                        except Exception:
                            pass
                if accepted_count > 0:
                    self._result.lane_ct_accepted_findings = getattr(self._result, 'lane_ct_accepted_findings', 0) + accepted_count
            except asyncio.CancelledError:
                raise
            except Exception:
                pass
    ...
    async def _run_target_memory_update(self, findings: list[Any], store: Any, query: str) -> None:
        ...
        for finding in findings:
            ...
            if hasattr(finding, 'source_type') and getattr(finding, 'source_type', None) == 'rir_correlation':
                ...
                if asn:
                    rir_asns[asn] = {'org': org, 'netblock': netblock, 'country': country, 'ioc_type': ioc_type, 'ioc_value': ioc_value_from_payload}
                exposure_facets[target_id]['count'] += 1
        ...
        for target_id in set(entity_facets.keys()) | set(exposure_facets.keys()) | set(pivot_facets.keys()):
            update = TargetMemoryUpdate(target_id=target_id, sprint_id=self.sprint_id or '', finding_count=len(findings), entity_facets=entity_facets.get(target_id, {}), exposure_facets=exposure_facets.get(target_id, {}), pivot_facets=pivot_facets.get(target_id, {}), observed_ts=now)
    ...
    async def _run_ane_semantic_dedup_advisory(self) -> None:
        ...
        try:
            ...
            if deduped is not None and len(deduped) < original_count:
                ...
                logger.debug('[ANE-DEDUP] removed %d near-duplicates, %d → %d findings', removed, original_count, len(deduped))
    ...
    async def _run_ct_to_passivedns_pivot_advisory(self) -> None:
        ...
        async def _run_pdns_for_domain(domain: str) -> tuple[str, list[str], PassiveDNSOutcome]:
            ...
            try:
                ...
                return (domain, [], None)
        try:
            gather_results = await safe_gather_ok(*[_run_pdns_for_domain(d) for d in pivot_domains], label='sprint_scheduler:19776')
        ...
        if pdns_results:
            ...
            for res in pdns_results:
                ...
                if outcome is None:
                    continue
                _sfos.append({'family': 'passive_dns', 'lane': 'PASSIVE_DNS', 'attempted': getattr(outcome, 'attempted', True), 'accepted': getattr(outcome, 'result_count', 0) or 0, 'terminal_state': 'pivot_ct_domain', 'raw_count': 0, 'accepted_count': getattr(outcome, 'result_count', 0) or 0, 'error': getattr(outcome, 'error', None), 'timeout': getattr(outcome, 'timeout', False), 'skipped': False, 'pivot_source': 'ct', 'pivot_domains': pivot_domains})
            self._result.source_family_outcomes_list = _sfos
            ...
            try:
                ...
                if store is not None:
                    ...
                    for r in bgp_results:
                        finding = CanonicalFinding(finding_id=f'bgp-{r.prefix or r.asn}', source_type=SourceType.BGP_INTELLIGENCE, confidence=0.75, query=self._query[:128], ts=_time.time(), payload_text=f'ASN={r.asn} org={r.asn_name} prefix={r.prefix} country={r.country_code}', provenance=(f'asn:{r.asn}', f'source:{r.source}'))
                        findings.append(finding)
                    if findings:
                ...
                _sfos = list(getattr(self._result, 'source_family_outcomes_list', []) or [])
                _sfos.append({'family': 'bgp_advisory', 'lane': 'BGP_ADVISORY', 'attempted': True, 'accepted': len(bgp_results), 'terminal_state': 'advisory_bgp', 'raw_count': len(bgp_results), 'accepted_count': len(bgp_results), 'error': None, 'timeout': False, 'skipped': False})
                self._result.source_family_outcomes_list = _sfos
            finally:
                await adapter.close()
    ...
    async def _run_ti_feed_sidecar(self) -> None:
        ...
        try:
            ...
            if accepted_count > 0:
                self._emit_source_family_event('ti_feed_advisory', 'findings', count=accepted_count, reason=f'ti_feed_advisory: NVD+CISA KE collected {accepted_count} entries')
        except Exception:
            pass
    ...
    async def _run_wayback_cdx_deep_sidecar(self) -> None:
        ...
        try:
            ...
            try:
                ...
                if not unique_domains:
                    return
                result = await searcher.search(domains_or_urls=unique_domains, match_type='domain', limit_per_domain=100, concurrency=2)
                ...
                if store is not None:
                    ...
                    stored = 0
                _sfos = list(getattr(self._result, 'source_family_outcomes_list', []) or [])
                _sfos.append({'family': 'wayback_cdx_advisory', 'lane': 'WAYBACK_CDX_ADVISORY', 'attempted': True, 'accepted': stored, 'terminal_state': 'advisory_wayback_cdx', 'raw_count': len(result.results), 'accepted_count': stored, 'error': None, 'timeout': False, 'skipped': False})
                self._result.source_family_outcomes_list = _sfos
            finally:
    ...
    def _get_source_health_summary(self) -> dict:
        ...
        try:
            if not self._source_economics:
                return {}
            sorted_sources = sorted(self._source_economics.values(), key=lambda e: (self._POSTURE_ORDER.get(e.recent_health_posture, 5), -e.last_signal_cycle))
            entries = []
            for econ in sorted_sources[:self.MAX_SOURCE_HEALTH_ENTRIES]:
                entries.append({'source': econ.source, 'posture': econ.recent_health_posture, 'last_signal_cycle': econ.last_signal_cycle, 'silent_streak': econ.silent_streak, 'in_cooldown': econ.cooldown_until_cycle is not None})
    ...
    def _get_graph_signal(self) -> dict:
        ...
        try:
            ...
            if stats:
                return {'graph_nodes': stats.get('nodes', 0), 'graph_edges': stats.get('edges', 0), 'graph_pgq_available': stats.get('pgq_available', False)}
        except Exception:
            pass
        return {}
        ...
        try:
            ...
            if not ENV.get_bool('HLEDAC_ENABLE_GRAPH_ANALYSIS'):
                ...
                try:
                    ...
                    pass
            return {'nodes': nodes, 'edges': edges, 'domains': domains, 'connected_iocs': set(), 'node_degrees': node_degrees, 'confidence_by_node': confidence_by_node, 'source_count_by_node': source_count_by_node}
        except Exception:
            return {}
    ...
    def _get_windup_scorecard(self) -> dict:
        ...
        try:
            ...
            try:
                ...
                log.warning('sprint %s: scorecard cb_tracked_count failed -- %s: %s', getattr(self._result, 'sprint_id', '?'), type(_exc).__name__, _exc)
            ...
            if self._result.entered_active_at_monotonic is not None and self._result.first_cycle_started_at_monotonic is not None:
                active_dur = round(self._result.first_cycle_started_at_monotonic - self._result.entered_active_at_monotonic, 2)
            ...
            if self._result.accepted_findings > 0 and self._result.sprint_elapsed_s > 0:
                scorecard['findings_per_minute'] = round(self._result.accepted_findings / (self._result.sprint_elapsed_s / 60), 2)
            ...
            if len(scorecard) > self.MAX_WINDUP_SCORECARD_KEYS:
                priority_keys = ['cb_open_domains', 'phase_durations', 'graph_nodes', 'graph_edges', 'peak_rss_mb', 'accepted_findings', 'sidecar_findings', 'branch_timeouts', 'budget_violations', 'graph_pgq_available', 'cb_tracked_count']
                pruned: dict = {}
    ...
    def _get_circuit_breaker_summary(self) -> dict:
        ...
        try:
            ...
            for snap in snapshots[:self.MAX_BREAKER_DOMAINS]:
                entries.append({'domain': snap.domain, 'state': snap.state, 'failure_count': snap.failure_count, 'last_failure_kind': snap.last_failure_kind, 'recovery_timeout_s': round(snap.recovery_timeout_s, 1)})
            return {'total_tracked': len(snapshots), 'open_count': open_count, 'half_open_count': half_open_count, 'entries': entries, 'max_entries': self.MAX_BREAKER_DOMAINS}
        except Exception:
            return {}
    ...
    def _build_work_items(self, sources: Sequence[str]) -> list[SourceWork]:
        ...
        for url in sources:
            ...
            items.append(SourceWork(feed_url=url, source=url, tier=tier, max_entries=self._config.max_entries_per_cycle))
    ...
    def _feed_dominance_should_fetch(self, work: _FeedWorkItem, nonfeed_terminal: bool) -> tuple[bool, str]:
        ...
        if mission_intent is not None and nonfeed_unresolved:
            mission_cap_reason = budget.cap_feeding(feed_accepted_so_far=self._result.accepted_findings, nonfeed_accepted_so_far=self._result.lane_ct_accepted_findings + self._result.lane_wayback_accepted_findings + self._result.lane_pdns_accepted_findings + self._result.lane_blockchain_accepted_findings, feed_per_source=get_sprint_ctx().feed_accepted_per_source, mission_intent=mission_intent, nonfeed_unresolved=nonfeed_unresolved, acquisition_profile=acquisition_profile)
        ...
        if acquisition_profile == AcquisitionProfile.NONFEED_DIAGNOSTIC and nonfeed_unresolved:
            profile_cap_reason = budget.cap_feeding(feed_accepted_so_far=self._result.accepted_findings, nonfeed_accepted_so_far=self._result.lane_ct_accepted_findings + self._result.lane_wayback_accepted_findings + self._result.lane_pdns_accepted_findings + self._result.lane_blockchain_accepted_findings, feed_per_source=self._feed_accepted_per_source, mission_intent=mission_intent if mission_intent else infer_mission_intent(self._acquisition_plan.query) if self._acquisition_plan and self._acquisition_plan.query else 'unknown', nonfeed_unresolved=nonfeed_unresolved, acquisition_profile=acquisition_profile)
        ...
        budget_reason = budget.cap_feeding(feed_accepted_so_far=self._result.accepted_findings, nonfeed_accepted_so_far=self._result.lane_ct_accepted_findings + self._result.lane_wayback_accepted_findings + self._result.lane_pdns_accepted_findings + self._result.lane_blockchain_accepted_findings, feed_per_source=get_sprint_ctx().feed_accepted_per_source, nonfeed_unresolved=nonfeed_unresolved, acquisition_profile=acquisition_profile)
        if budget_reason[0]:
    ...
    def _process_result(self, feed_url: str, result) -> None:
        ...
        if hasattr(result, 'matched_patterns') and result.matched_patterns > 0:
            finding_entry = {'type': 'pattern_hit', 'source': feed_url, 'matched_patterns': result.matched_patterns, 'accepted_findings': result.accepted_findings, 'severity': 'medium', 'confidence': 0.6, 'description': f'{result.matched_patterns} pattern hits from {feed_url}'}
            if len(self._all_findings) < self._MAX_FINDINGS_PER_SPRINT:
        ...
        if self._prefetch_oracle is not None:
            try:
                self._prefetch_oracle.record_outcome(feed_url=feed_url, fetched=getattr(result, 'fetched_entries', 0), accepted=getattr(result, 'accepted_findings', 0), cycle=self._result.cycles_started, seen_new_urls=getattr(result, 'matched_patterns', 0))
            except Exception:
                pass
        self._feed_dominance_record_result(feed_url=feed_url, accepted_count=result.accepted_findings, suppressed=getattr(result, 'error', '') == 'feed_budget_cap_suppressed', reason=getattr(result, 'feed_budget_cap_reason', ''))
    ...
    async def _flush_dedup(self) -> None:
        ...
        try:
            ...
            written = putmulti_bounded(self._dedup_env, [(k.encode(), ts_bytes) for k in self._dedup_seen], overwrite=True)
    ...
    async def _run_synthesis_sidecar(self, query: str, duckdb_store: Any, lifecycle: Any) -> None:
        ...
        try:
            ...
            if report is not None:
                try:
                    self._result.synthesis_text = _msgspec_encode({'query': query, 'ioc_entities': [{'type': e.ioc_type, 'value': e.value} for e in getattr(report, 'ioc_entities', None) or []], 'threat_summary': getattr(report, 'threat_summary', ''), 'threat_actors': list(getattr(report, 'threat_actors', None) or []), 'confidence': getattr(report, 'confidence', 0.0), 'sources_count': getattr(report, 'sources_count', 0), 'timestamp': getattr(report, 'timestamp', 0.0)}).decode('utf-8')
                except Exception:
                    self._result.synthesis_text = str(report)[:4096]
                log.info('[F259] Synthesis complete: success=%s, findings=%d', self._result.synthesis_success, self._result.synthesis_findings_count)
                try:
    ...
    async def _run_social_identity_surface_sidecar(self, query: str, duckdb_store: Any) -> None:
        ...
        try:
            ...
            log.info('[F204I] Social Identity scan complete: scanned=%d, facets=%d, elapsed_ms=%.1f', result.scanned_count, len(result.facets), result.elapsed_ms)
        except Exception as e:
    ...
    async def _run_epistemic_gap_advisory(self, query: str, duckdb_store: Any) -> None:
        ...
        if not findings:
            ...
            return
        try:
            from hledac.universal.brain.dspy_programs import MAX_EPISTEMIC_FINDINGS, ContradictionResolverProgram, EpistemicGapProgram
        ...
        try:
            ...
            if memory is not None and identified_gaps:
                try:
                    await memory.record_sprint_outcome(sprint_id=getattr(self._result, 'sprint_id', 'unknown'), query=query, findings=findings, gaps=identified_gaps)
        ...
        try:
            ...
            for f in findings[:20]:
                ...
                if evidence:
                    ...
                    if conflict > 0.3:
                        contradictory_findings.append({'finding': f.get('payload_text', '') or str(f)[:150], 'conflict_mass': conflict, 'source': f.get('source_type', 'unknown')})
    ...
    async def _run_enhanced_research(self) -> list:
        ...
        try:
            ...
            pass
        seed_iocs: list[str] = []
        for src_field in ('pivot_seed_domains', 'pivot_seed_ips', 'pivot_seed_urls', 'pivot_seed_hashes', 'pivot_seed_cves', 'next_seeds_ioc_domains', 'next_seeds_ioc_ips', 'next_seeds_ioc_urls', 'next_seeds_ioc_hashes', 'next_seeds_ioc_cves'):
        ...
        engine_config = UnifiedResearchConfig(depth=depth, max_concurrent_tools=2, enable_temporal_analysis=True, enable_data_leak_check=False, enable_archive_search=True, enable_stealth_crawling=False, cache_results=True, cache_ttl_seconds=3600)
        ...
        for f in response.findings[:100]:
            try:
                ...
                canonical = CanonicalFinding(finding_id=f"er_{getattr(f, 'id', 'unknown')}", query=f"deep_research:{getattr(self._result, 'sprint_id', 'unknown')}", source_type=getattr(f, 'source_type', src) or src, confidence=confidence, ts=ts_float, provenance=getattr(f, 'url', '') or '', payload_text=f"{getattr(f, 'title', '')}\n{getattr(f, 'content', '')[:2000]}")
    ...
    async def _run_dark_surface_pivot_advisory(self) -> None:
        ...
        try:
            ...
            dark_queries = await hyp_eng.generate_dark_surface_queries(findings=findings_for_dark, hermes_engine=self._hermes_engine if self._hermes_engine is not None else None, tor_available=tor_available, i2p_available=i2p_available)
        except Exception as _e:
            ...
            try:
                ...
                for _dq in dark_queries:
                    if isinstance(_dq, DarkQuery):
                        _hyps.append(type('Hypothesis', (), {'hypothesis_id': str(uuid4()), 'statement': f'Dark query: {_dq.query} (type={_dq.query_type.value}, priority={_dq.priority:.2f})', 'status': ResearchContextStatus.PENDING, 'confidence': max(0.3, min(0.9, _dq.priority)), 'supporting_evidence': list(_dq.source_iocs) if _dq.source_iocs else [], 'contradicting_evidence': []})())
                from hledac.universal.research_context import ResearchContext
                _rc = ResearchContext(query=self._query or 'dark_surface_pivot', research_id=getattr(self, 'sprint_id', '') or str(uuid4()))
        ...
        try:
            ...
            items: list = []
            seen_pairs: set = set()
            _plan_dark_surface_pivot(dark_queries, items, seen_pairs, tor_available=tor_available, i2p_available=i2p_available)
            planned = len(items)
    ...
    async def _init_metrics_registry(self) -> None:
        ...
        try:
            ...
            correlation = {'run_id': self.sprint_id or 'default', 'branch_id': None, 'provider_id': None, 'action_id': None}
            self._metrics_registry = MetricsRegistry(run_dir=run_dir, run_id=self.sprint_id or 'default', correlation=correlation)
    ...
    def _get_metrics_summary(self) -> dict | None:
        ...
        try:
            ...
            return {'counter_count': summary.get('counter_count', 0), 'gauge_count': summary.get('gauge_count', 0), 'last_rss_mb': summary.get('gauges', {}).get('memory_rss_mb', 0.0), 'persist_available': summary.get('persist_available', False), 'closed': summary.get('closed', False)}
        except Exception:
            return None
    ...
    async def _prewarm_hermes_for_sprint(self) -> None:
        ...
        if self._config.aggressive_mode:
            ...
            if rss_before > RSS_PREWARM_HEADROOM_GB:
                log.debug(f'[P12] Skipping Hermes prewarm -- RSS {rss_before:.2f}GB > {RSS_PREWARM_HEADROOM_GB}GB headroom threshold')
                self._hermes_engine = None
        ...
        if _hermes_force and (not hermes_synthesis_enabled):
            log.debug('[F273D] Hermes force-load requested via flags.hermes_force -- overriding HLEDAC_ENABLE_HERMES_SYNTHESIS gate')
        ...
        if not hermes_synthesis_enabled:
            ...
            self._result.hermes_model_loaded = False
            log.debug(f"[F206AE] Hermes load skipped -- HLEDAC_ENABLE_HERMES_SYNTHESIS != '1', reason={hermes_load_skipped_reason}")
        else:
    ...
    async def _load_hermes_for_sprint(self) -> None:
        ...
        async def _load_in_thread() -> None:
            ...
            if _MLX_PREWARM_ENABLED:
                try:
                    from hledac.universal.brain.deephermes3_engine import _MLX_PREWARM_LAST_UNLOAD_TIME, _MLX_PREWARM_SKIP_THRESHOLD_S
                    if _MLX_PREWARM_LAST_UNLOAD_TIME is not None:
                        ...
                        if gap < _MLX_PREWARM_SKIP_THRESHOLD_S:
                            log.debug(f'[F267] MLX prewarm: last unload {gap:.1f}s ago < {_MLX_PREWARM_SKIP_THRESHOLD_S}s threshold -- verifying Metal cache')
            ...
            try:
                _loop = asyncio.get_running_loop()
                self._hermes_engine = await _loop.run_in_executor(None, lambda: asyncio.run(get_model_manager().load_model('hermes')))
    ...
    def _get_prewindup_barrier_report(self) -> dict | None:
        ...
        if not getattr(self._result, 'prewindup_barrier_checked', False):
            return None
        return {'required_lanes': list(getattr(self._result, 'prewindup_barrier_required_lanes', ())), 'satisfied': getattr(self._result, 'prewindup_barrier_satisfied', False), 'attempted_lanes': list(getattr(self._result, 'prewindup_barrier_attempted_lanes', ())), 'skipped_lanes': dict(getattr(self._result, 'prewindup_barrier_skipped_lanes', {})), 'errors': dict(getattr(self._result, 'prewindup_barrier_errors', {})), 'duration_s': round(getattr(self._result, 'prewindup_barrier_duration_s', 0.0), 3), 'windup_delayed': getattr(self._result, 'windup_delayed_for_nonfeed', False)}
    ...
    async def _maybe_export_partial(self, lifecycle) -> None:
        ...
        try:
            ...
            runtime_truth = {'is_meaningful': self._finding_count > 0, 'accepted_findings': self._finding_count, 'cycles_completed': self._result.cycles_completed, 'cycles_started': self._result.cycles_started, 'aggressive_mode': True}
            scorecard = {'cycles_started': self._result.cycles_started, 'cycles_completed': self._result.cycles_completed, 'total_pattern_hits': self._result.total_pattern_hits}
            handoff_dict = {'sprint_id': self.sprint_id or 'unknown', 'runtime_truth': runtime_truth, 'scorecard': scorecard}
            await export_partial_sprint(store=self._duckdb_store, handoff=handoff_dict, sprint_id=self.sprint_id or 'unknown', finding_count=self._finding_count)
    ...
    async def _run_export(self, lifecycle) -> None:
        ...
        _render_tasks = [asyncio.create_task(asyncio.to_thread(_render_sync, _fn, _suffix), name=f'export:render:{_suffix}') for _fn, _suffix in [(rend_md, 'md'), (rend_jsonld, 'jsonld'), (rend_stix, 'stix.json')]]
        _async_tasks = [safe_create_task(self._run_cti_export(rend_cti_stix, collect_cti_inputs, report, export_dir), name='export:cti'), safe_create_task(self._run_hypothesis_export(report, export_dir), name='export:hypothesis')]
    ...
    async def _run_hypothesis_export(self, report: dict[str, Any], export_dir: str | None) -> None:
        ...
        try:
            ...
            logger.info(f'Hypothesis export: {result.hypotheses_generated} hypotheses, {result.hidden_bridges} hidden bridges, {result.anomalies_detected} anomalies in {result.execution_time_s:.2f}s')
        except Exception as exc:
            logger.warning(f'Hypothesis export failed (non-critical): {exc}')
            self._result.export_paths.append(f'EXPORT_ERROR:hypothesis:{exc}')

    async def _run_cti_export(self, render_cti_stix_to_path: Any, collect_cti_inputs: Any, report: dict[str, Any], export_dir: str | None) -> None:
        """
        ...
        try:
            if len(cti_inputs.findings) > 1000:
                path = await asyncio.to_thread(render_cti_stix_to_path, findings=list(cti_inputs.findings), identity_candidates=list(cti_inputs.identity_candidates), attribution_scores=cti_inputs.attribution_scores, killchain_tags=cti_inputs.killchain_tags, evidence_chains=list(cti_inputs.evidence_chains), path=export_dir)
            else:
                path = render_cti_stix_to_path(findings=list(cti_inputs.findings), identity_candidates=list(cti_inputs.identity_candidates), attribution_scores=cti_inputs.attribution_scores, killchain_tags=cti_inputs.killchain_tags, evidence_chains=list(cti_inputs.evidence_chains), path=export_dir)
    ...
    def _build_public_stage_counters(self) -> dict:
        ...
        return {'public_discovered': getattr(pr, 'public_discovered', 0) or 0, 'public_fetch_candidate_count': getattr(pr, 'public_fetch_candidate_count', 0) or 0, 'public_fetch_attempted': getattr(pr, 'public_fetch_attempted', 0) or 0, 'public_fetch_success': getattr(pr, 'public_fetch_success', 0) or 0, 'public_fetch_failed': getattr(pr, 'public_fetch_failed', 0) or 0, 'public_acceptance_attempted': getattr(pr, 'public_acceptance_attempted', 0) or 0, 'public_acceptance_accepted': getattr(pr, 'public_acceptance_accepted', 0) or 0, 'public_acceptance_rejected': getattr(pr, 'public_acceptance_rejected', 0) or 0, 'public_acceptance_reject_reasons': getattr(pr, 'public_acceptance_reject_reasons', {}) or {}, 'public_terminal_classified_count': getattr(pr, 'public_terminal_classified_count', 0) or 0, 'public_unclassified_count': getattr(pr, 'public_unclassified_count', 0) or 0, 'public_terminal_reason_counts': getattr(pr, 'public_terminal_reason_counts', {}) or {}, 'public_skipped_duplicate': getattr(pr, 'public_skipped_duplicate', 0) or 0, 'public_skipped_unsupported_scheme': getattr(pr, 'public_skipped_unsupported_scheme', 0) or 0, 'public_skipped_memory_gate': getattr(pr, 'public_skipped_memory_gate', 0) or 0, 'public_skipped_quality_gate': getattr(pr, 'public_skipped_quality_gate', 0) or 0, 'security_rejected_count': getattr(pr, 'security_rejected_count', 0) or 0, 'pii_redacted_count': getattr(pr, 'pii_redacted_count', 0) or 0, 'public_skipped_browser_unavailable': getattr(pr, 'public_skipped_browser_unavailable', 0) or 0, 'public_skipped_xml_or_feed': getattr(pr, 'public_skipped_xml_or_feed', 0) or 0, 'public_skipped_timeout': getattr(pr, 'public_skipped_timeout', 0) or 0, 'public_skipped_fetch_error': getattr(pr, 'public_skipped_fetch_error', 0) or 0, 'public_skipped_url_sample': getattr(pr, 'public_skipped_url_sample', ()) or (), 'public_rejected_no_pattern_match': getattr(pr, 'public_rejected_no_pattern_match', 0) or 0, 'public_rejected_low_information': getattr(pr, 'public_rejected_low_information', 0) or 0, 'public_rejected_duplicate': getattr(pr, 'public_rejected_duplicate', 0) or 0, 'public_rejected_storage_rejected': getattr(pr, 'public_rejected_storage_rejected', 0) or 0, 'public_rejected_url_samples': getattr(pr, 'public_rejected_url_samples', ()) or (), 'public_accepted_url_sample': getattr(pr, 'public_accepted_url_sample', ()) or ()}
    ...
    async def _build_diagnostic_report(self, lifecycle) -> dict:
        ...
        report = {'run_id': run_id, 'phase': lifecycle.current_phase.name, 'cycles_started': r.cycles_started, 'cycles_completed': r.cycles_completed, 'unique_entry_hashes': r.unique_entry_hashes_seen, 'duplicates_skipped': r.duplicate_entry_hashes_skipped, 'pattern_hits': r.total_pattern_hits, 'accepted_findings': r.accepted_findings, 'aborted': r.aborted, 'abort_reason': r.abort_reason, 'stop_requested': r.stop_requested, 'lifecycle_snapshot': lifecycle.snapshot(), 'entries_per_source': dict(get_sprint_ctx().entries_per_source), 'hits_per_source': dict(get_sprint_ctx().hits_per_source)}
        ...
        if self._acquisition_plan is not None:
            _lanes_list = [{'lane': p.lane, 'enabled': p.enabled, 'reason': p.reason, 'max_items': p.max_items, 'timeout_s': p.timeout_s, 'concurrency': p.concurrency, 'risk_level': p.risk_level} for p in self._acquisition_plan.plans]
            ...
            report['acquisition_strategy'] = {'uma_state': self._acquisition_plan.uma_state, 'swap_detected': self._acquisition_plan.swap_detected, 'aggressive_mode': self._acquisition_plan.aggressive_mode, 'stealth_ready': self._acquisition_plan.stealth_ready, 'transport_degraded': self._acquisition_plan.transport_degraded, 'enforced': True, 'skipped_lanes': _skipped, 'executed_lanes': _executed, 'lanes': _lanes_list, 'nonfeed_plan_debug': {'domain_detected': self._acquisition_plan.nonfeed_plan_debug.domain_detected, 'wallet_detected': self._acquisition_plan.nonfeed_plan_debug.wallet_detected, 'enabled_nonfeed_lanes': list(self._acquisition_plan.nonfeed_plan_debug.enabled_nonfeed_lanes), 'disabled_nonfeed_lanes': list(self._acquisition_plan.nonfeed_plan_debug.disabled_nonfeed_lanes), 'disabled_reasons': list(self._acquisition_plan.nonfeed_plan_debug.disabled_reasons), 'scheduled_nonfeed_lanes': list(self._acquisition_plan.nonfeed_plan_debug.scheduled_nonfeed_lanes), 'hardware_skipped_lanes': list(self._acquisition_plan.nonfeed_plan_debug.hardware_skipped_lanes), 'nonfeed_execution_scheduled': self._acquisition_plan.nonfeed_plan_debug.nonfeed_execution_scheduled, 'nonfeed_execution_skip_reason': self._acquisition_plan.nonfeed_plan_debug.nonfeed_execution_skip_reason} if self._acquisition_plan.nonfeed_plan_debug is not None else None, 'prewindup_barrier': self._get_prewindup_barrier_report() if hasattr(self, '_get_prewindup_barrier_report') else None, 'prewindup_barrier_checked': getattr(self._result, 'prewindup_barrier_checked', False)}
        ...
        report['windup_guard_observation'] = {'call_count': _wg_call, 'callback_supplied_count': _wg_supplied, 'callback_executed_count': _wg_executed, 'last_reason': _wg_last_reason, 'last_phase': _wg_last_phase, 'last_allowed': _wg_last_allowed, 'callback_not_executed_reason': _wg_not_exec_reason}
        report['return_guard'] = {'checked': _rg_checked, 'required_lanes': _rg_attempted, 'satisfied': _rg_satisfied, 'delayed_for_nonfeed': _rg_delayed, 'block_reason': _rg_block, 'attempted_lanes': _rg_attempted, 'skipped_lanes': _rg_skipped, 'errors': _rg_errors}
        report['scheduler_exit'] = {'exit_path': _se_path, 'exit_reason': _se_reason, 'exit_phase': _se_phase, 'exit_cycle': _se_cycle, 'elapsed_s': _se_elapsed, 'guard_checked': _se_guard_checked, 'guard_required': _se_guard_required, 'guard_satisfied': _se_guard_satisfied}
        report['feed_dominance_budget'] = {'active': _feed_active, 'reason': _feed_reason, 'feed_accepted_before_cap': _feed_accepted_before, 'feed_suppressed_by_budget': _feed_suppressed, 'top_feed_source_counts': _feed_top, 'max_per_source_applied': _feed_max_per}
        if 'acquisition_strategy' in report:
            ...
            report['acquisition_strategy'] = {'terminality': _at_report, 'acquisition_terminality_checked': _at_checked, 'acquisition_terminality_satisfied': _at_satisfied, 'acquisition_terminality_missing_lanes': _at_missing}
        if self._lane_outcomes:
            ...
            report['acquisition_lanes'] = {'planned': _planned, 'attempted': _attempted, 'skipped': _skipped_lanes, 'outcomes': _outcomes_list, 'total_optional_findings': sum((o.get('accepted_findings', 0) for o in _outcomes_list)), 'lane_errors': _lane_errors}
        _sfo: dict[str, dict] = {}
        for _fam, _lane in [('ct', AcquisitionLane.CT), ('wayback', AcquisitionLane.WAYBACK), ('passive_dns', AcquisitionLane.PASSIVE_DNS), ('blockchain', AcquisitionLane.BLOCKCHAIN), ('feed', 'FEED'), ('public', AcquisitionLane.PUBLIC), ('doh', AcquisitionLane.DOH)]:
            _raw: dict | list | None = None
            if _lane == 'FEED':
                _raw = getattr(self, '_feed_verdicts', []) or None
                if _raw is not None:
        ...
        if hasattr(self._result, 'rdap_enrichment_attempted') and self._result.rdap_enrichment_attempted:
            _rdap_raw = {'attempted': True, 'accepted_count': getattr(self._result, 'rdap_enrichment_findings_stored', 0) or 0, 'raw_count': getattr(self._result, 'rdap_enrichment_findings_built', 0) or 0, 'error': getattr(self._result, 'rdap_enrichment_error', None), 'terminal_state': 'ATTEMPTED_ERROR' if getattr(self._result, 'rdap_enrichment_error', None) else 'ATTEMPTED_ACCEPTED' if getattr(self._result, 'rdap_enrichment_findings_stored', 0) else 'ATTEMPTED_NO_RESULTS'}
        ...
        if hasattr(self._result, 'rir_correlation_produced') and self._result.rir_correlation_produced > 0:
            _rir_raw = {'attempted': True, 'accepted_count': self._result.rir_correlation_produced or 0, 'raw_count': self._result.rir_correlation_produced or 0, 'error': None, 'terminal_state': 'ATTEMPTED_ACCEPTED'}
        ...
        try:
            ...
            if self._governor is not None:
                try:
                    ...
                    pass
            _mlt = required_terminal_lanes(snapshot=self._acquisition_plan, query=self._query, uma_state=_term_uma, swap_detected=_term_swap)
            ...
            if 'acquisition_strategy' in report:
                ...
                report['acquisition_strategy']['acquisition_terminality_satisfied'] = len(_fresh_term.get('missing_lanes', [])) == 0
                report['acquisition_strategy']['acquisition_terminality_missing_lanes'] = list(_fresh_term.get('missing_lanes', []))
        ...
        try:
            ...
            report['runtime_loop_telemetry'] = {'events': [], 'phase_totals_s': {}, 'slowest_phases': [], 'lane_timings': {}, 'timer_event_count': 0}
        try:
            from hledac.universal.runtime.acquisition_strategy import build_acquisition_report
            _wg_obs = {'call_count': _wg_call, 'callback_supplied_count': _wg_supplied, 'callback_executed_count': _wg_executed, 'last_reason': _wg_last_reason, 'last_phase': _wg_last_phase, 'last_allowed': _wg_last_allowed, 'callback_not_executed_reason': _wg_not_exec_reason}
            _rg_dict = {'checked': _rg_checked, 'required_lanes': _rg_attempted, 'satisfied': _rg_satisfied, 'delayed_for_nonfeed': _rg_delayed, 'block_reason': _rg_block, 'attempted_lanes': _rg_attempted, 'skipped_lanes': _rg_skipped, 'errors': _rg_errors}
            _pwb = None
            ...
            _FAM_MAP = {'CT': 'ct', 'WAYBACK': 'wayback', 'PASSIVE_DNS': 'passive_dns', 'BLOCKCHAIN': 'blockchain', 'PUBLIC': 'public', 'PIVOT_EXECUTOR': 'pivot_executor', 'DOH': 'doh'}
            _surf_ct = _sfo_by_family.get('ct', {})
            _surf_public = _sfo_by_family.get('public', {})
            _surfaced_lower = {_FAM_MAP.get(fam.upper(), fam.lower()) for fam, entry in _sfo_by_family.items() if entry.get('attempted') or entry.get('skipped')}
            ...
            report['acquisition_report'] = build_acquisition_report(query=self._query, plan=self._acquisition_plan, terminality=_at_report, nonfeed_plan_debug=_nd, source_family_outcomes=_sfo_list, return_guard=_rg_dict, prewindup_barrier=_pwb, scheduler_exit=_se_dict, windup_guard_observation=_wg_obs, acquisition_profile=self._config.acquisition_profile if self._config.acquisition_profile is not None else 'default', nonfeed_expected_lanes=_expected, nonfeed_missing_expected_lanes=_missing, wayback_terminal_state=_surf_wayback.get('terminal_state', ''), passive_dns_terminal_state=_surf_pdns.get('terminal_state', ''), nonfeed_surface_complete=_surf_complete, public_bootstrap_order=_pub_bootstrap_order, keyword_seed_fallback_triggered=_keyword_seed_fallback, public_bootstrap_prevented_discovery_timeout=_pub_bootstrap_prevented, public_bootstrap_first_fetch_attempted=_pub_bootstrap_fetch_att, public_stage_counters=self._build_public_stage_counters(), ct_bridge_rejections_count=getattr(self._result, 'ct_bridge_rejections_count', 0), ct_storage_rejected=getattr(self._result, 'ct_storage_rejected', 0), arrow_last_flush_error=getattr(self._result, 'arrow_last_flush_error', '') or '', arrow_batch_dropped=getattr(self._result, 'arrow_batch_dropped_after_flush_failure', 0), prewindup_barrier_errors=getattr(self._result, 'prewindup_barrier_errors', None) or {}, return_guard_errors=getattr(self._result, 'return_guard_errors', None) or {}, wayback_unchanged_rejected=getattr(self._result, 'wayback_unchanged_rejected', 0), nonfeed_provider_failures=getattr(self._result, 'nonfeed_provider_failures', None) or [], quality_rejection_summary_by_family=getattr(self._result, 'quality_rejection_summary_by_family', None) or {}, duplicate_rejection_summary_by_family=getattr(self._result, 'duplicate_rejection_summary_by_family', None) or {}, low_information_by_family=getattr(self._result, 'low_information_by_family', None) or {}, nonfeed_candidate_ledger_summary=self._nonfeed_ledger.summary() if hasattr(self, '_nonfeed_ledger') and self._nonfeed_ledger is not None and hasattr(self._nonfeed_ledger, 'summary') else {}, acquisition_plan_build_failed=getattr(self._result, 'acquisition_plan_build_failed', False), acquisition_plan_build_error_type=getattr(self._result, 'acquisition_plan_build_error_type', ''), acquisition_plan_build_error=getattr(self._result, 'acquisition_plan_build_error', ''), seed_context_available=getattr(self._result, 'seed_context_available', False), seed_context_propagated=getattr(self._result, 'seed_context_propagated', False), seed_context_skip_reason=getattr(self._result, 'seed_context_skip_reason', ''), seed_context_source=getattr(self._result, 'seed_context_source', ''), lanes_unlocked_by_seed_context=getattr(self._result, 'lanes_unlocked_by_seed_context', []) or [], public_provider_selection_debug=getattr(self._result, 'public_provider_selection_debug', None) or {})
        except Exception:
            pass
        return report
    ...
    @staticmethod
    def _adapt_source_weights_from_feedback_python(source_quality_feedback: dict[str, dict[str, int]], config: Any, source_weights: dict[str, float], log: logging.Logger) -> None:
        """
        ...
        for feed_url, fb in source_quality_feedback.items():
            ...
            log.debug(f'[F199A][Python] Source weight adaptation: {source_type} ({accepted}/{total}={ratio:.2%}) {current:.3f} -> {new_weight:.3f}')
    ...
    def _adapt_source_weights_from_feedback(self) -> None:
        ...
        for feed_url, fb in self._source_quality_feedback.items():
            ...
            stats_list.append({'fetched': float(total), 'accepted': float(accepted), 'current_weight': current, 'novelty': False})
        if batch_compute_scores is not None and stats_list:
            try:
                ...
                for i, (source_type, feed_url) in enumerate(source_type_map):
                    ...
                    log.debug(f"[F199A][NEON] Source weight adaptation: {source_type} ({int(fb['accepted'])}/{int(fb['fetched'])}) {stats_list[i]['current_weight']:.3f} -> {new_weight:.3f}")
            except Exception as e:
                log.debug(f'[F199A][NEON] batch_compute_scores failed: {e} -- falling back to Python')
                SprintScheduler._adapt_source_weights_from_feedback_python(self._source_quality_feedback, self._config, get_sprint_ctx().source_weights, log)
        elif stats_list:
            SprintScheduler._adapt_source_weights_from_feedback_python(self._source_quality_feedback, self._config, get_sprint_ctx().source_weights, log)
    ...
    async def log_source_hit(self, store: Any, sprint_id: str, source_type: str, findings_count: int, ioc_count: int) -> None:
        """Record a source hit for hit-rate tracking."""
        hit_rate = findings_count / max(1, findings_count + 1)
        try:
            await store.async_record_source_hit(sprint_id, _time.time(), source_type, findings_count, ioc_count, hit_rate)
        except Exception as e:
    ...
    async def health_check(self) -> HealthReport:
        ...
        try:
            ...
            fetch_coordinator_ok = status.get('tor_session_present', False) and (not status.get('tor_session_closed', True)) or (status.get('i2p_session_present', False) and (not status.get('i2p_session_closed', True)))
        ...
        report = HealthReport(duckdb_ok=duckdb_ok, hermes_ok=hermes_ok, fetch_coordinator_ok=fetch_coordinator_ok, graph_service_ok=graph_service_ok, nym_circuit_open=nym_circuit_open, evidence_log_ok=evidence_log_ok, memory_pressure_ok=memory_pressure_ok, overall_ok=overall_ok, blocking_ok=blocking_ok, errors=errors)
        if blocking_ok:
            if overall_ok:
    ...
    def enqueue_pivot(self, ioc_value: str, ioc_type: str, confidence: float, degree: float=1.0, task_type: str | None=None) -> None:
        ...
        """
        return self._fetch_coordinator.enqueue_pivot(ioc_value=ioc_value, ioc_type=ioc_type, confidence=confidence, degree=degree, task_type=task_type)
    ...
    def enqueue_hypothesis_pivot(self, ioc_value: str, ioc_type: str='hypothesis', confidence: float=0.7, depth: int=1) -> bool:
        ...
        """
        return self._fetch_coordinator.enqueue_hypothesis_pivot(ioc_value=ioc_value, ioc_type=ioc_type, confidence=confidence, depth=depth)
            ...
            if hyp_found > 0:
                try:
                    for ioc_entry in get_sprint_ctx().recent_iocs[-hyp_found:]:
                        ...
                        if ioc_val:
                            _DEFAULT_GRAPH_SERVICE.upsert_relation(task.ioc_value[:100], ioc_val, rel_type='confirmed_by', weight=0.8, evidence='hypothesis_probe')
                except Exception:
                    pass
    ...
    async def _run_ooda_cycle(self, ioc_graph) -> None:
        ...
        if node_count < 3 or edge_count == 0:
            ...
            if _query_text:
                try:
                    _cands = extract_domain_candidates_from_text(_query_text, source_family='PUBLIC', min_confidence=0.2)
                    for c in _cands[:5]:
    ...
    async def _maybe_flush_to_parquet(self) -> None:
        ...
        import time as _time
        now = _time.monotonic()
        if len(get_sprint_ctx().arrow_batch) < self._arrow_flush_n and now - self._arrow_last_flush < self._ARROW_FLUSH_S:
            return
        ...
        schema = pa.schema([('url', pa.string()), ('title', pa.string()), ('snippet', pa.string()), ('source', pa.string()), ('ioc', pa.string()), ('ioc_type', pa.string()), ('confidence', pa.float32()), ('timestamp', pa.timestamp('ms', tz='UTC')), ('sprint_id', pa.string())])
        ...
        try:
            ...
            if batch_len > hard_cap:
                ...
                log.warning(f'[8VD-PARQUET] flush failed ({exc}), dropped {drop_count} oldest entries to enforce HARD_CAP={hard_cap}, {len(get_sprint_ctx().arrow_batch)} remain')
            else:
    ...
    def buffer_finding(self, finding: dict) -> None:
        ...
        try:
            ...
            pass
        _text = ' '.join(filter(None, [finding.get('snippet', ''), finding.get('content', ''), finding.get('title', '')])).strip()
        if len(_text) > 10:
            try:
    ...
    def deduplicate_and_rank_findings(self, sprint_id: str | None=None) -> str:
        ...
        try:
            import polars as pl
            pl.scan_parquet(glob).filter(pl.col('url').is_not_null() | pl.col('ioc').is_not_null()).with_columns([pl.col('confidence').fill_null(0.5), pl.col('source').cast(pl.Categorical)]).group_by(['url', 'ioc']).agg([pl.col('title').first(), pl.col('source').first(), pl.col('confidence').max(), pl.len().alias('hit_count')]).sort('hit_count', descending=True).collect(engine='streaming').write_parquet(out, compression='snappy')
            return out
        except Exception:
            pass
        try:
            ...
            rows = con.execute(f"\n                SELECT\n                    FIRST(title)     AS title,\n                    FIRST(source)    AS source,\n                    url,\n                    ioc,\n                    MAX(confidence) AS confidence,\n                    COUNT(*)         AS hit_count\n                FROM read_parquet('{glob}')\n                WHERE url IS NOT NULL OR ioc IS NOT NULL\n                GROUP BY url, ioc\n                ORDER BY hit_count DESC\n            ").fetchall()
            if rows:
                schema = pa.schema([('title', pa.string()), ('source', pa.string()), ('url', pa.string()), ('ioc', pa.string()), ('confidence', pa.float64()), ('hit_count', pa.int64())])
    ...
    async def _init_dht_node_background(self) -> None:
        ...
        try:
            ...
            node = KademliaNode(node_id=f'hledac-sprint-{uuid.uuid4().hex[:8]}', governor=gov, bootstrap_nodes=list(DHT_BOOTSTRAP_PEERS))
            await node.start()
            self._dht_node = node
            ...
            try:
                ...
                if self._backpressure_monitor is not None:
                    try:
                        ...
                        log.debug(f'[BACKPRESSURE] evaluate: clearnet_max={bp.clearnet_max}, stealth_max={bp.stealth_max}, uma_state={bp.uma_state}')
                ...
                try:
                    from hledac.universal.monitoring.alert_manager import check_lock_contention_alert, check_memory_delta_alert, get_lock_contention_tracker
    ...
    def consume_shadow_pre_decision(self) -> Any:
        ...
        try:
            ...
            if self._fetch_latency_ema:
                ...
                if max_ema > 20.0:
                    ...
                    thermal = 'fair'
            lifecycle_bundle = collect_lifecycle_snapshot(lc, now_mono, thermal, windup_synthesis_mode='synthesis', windup_error=False, windup_engine=self._synthesis_engine or 'unknown')
        except Exception:
            return None
        try:
        ...
        try:
            mc_bundle = collect_model_control_facts(analyzer_result=None, raw_profile={'tools': [], 'sources': list(self._config.source_tier_map.keys()), 'privacy_level': 'STANDARD', 'use_tor': False, 'depth': 'STANDARD', 'use_tot': False, 'tot_mode': 'standard', 'models_needed': []})
        except Exception:
            from hledac.universal.runtime.shadow_inputs import ModelControlFactsBundle
            mc_bundle = ModelControlFactsBundle()
        export_facts = {'sprint_id': self.sprint_id or 'unknown', 'synthesis_engine': self._synthesis_engine or 'unknown', 'gnn_predictions': 0, 'top_nodes_count': 0, 'ranked_parquet_present': False, 'phase_durations': {}}
        try:
            parity = run_shadow_parity(lifecycle_bundle=lifecycle_bundle, graph_bundle=graph_bundle, model_control_bundle=mc_bundle, export_handoff_facts=export_facts, branch_decision=None, provider_recommend=None, correlation=None, runtime_mode=RuntimeMode.get_current())
        except Exception:
            return None
        try:
        ...
        try:
            ...
            has_network_tools = any((s in source_types for s in ['cisa_kev', 'threatfox_ioc', 'urlhaus_recent', 'feodo_ip', 'openphish_feed']))
            has_high_memory_tools = False
            pd_summary._tool_readiness_preview = {'tool_count': estimated_tool_count, 'tool_names': [], 'has_network_tools': has_network_tools, 'has_high_memory_tools': has_high_memory_tools, 'tool_cards_sample': [], '_deferred_registry': True}
        except Exception:
            pass
        try:
            from hledac.universal.runtime.shadow_pre_decision import preview_dispatch_parity
            task_candidates = ['cve_to_github', 'cve_to_academic', 'ip_to_ct', 'ip_to_greynoise', 'shodan_enrich', 'domain_to_dns', 'domain_to_wayback', 'domain_to_pdns', 'domain_to_ct', 'ahmia_search', 'rdap_lookup', 'hash_to_mb', 'wayback_search', 'commoncrawl_search', 'paste_keyword_search', 'github_dork', 'multi_engine_search', 'hypothesis_probe']
            ...
            dispatch_preview = preview_dispatch_parity(task_candidates=task_candidates, available_capabilities=available_caps, control_mode=ctrl_mode, registry_tools=registry_tools)
            try:
                ...
                execution_context = build_execution_context_readiness(dispatch_preview=dispatch_preview, correlation_context=correlation_context, execlogger_available=execlogger_available)
    ...
    def _build_shadow_readiness_preview(self) -> dict[str, Any]:
        ...
        if pd is None:
            return {}
        result: dict[str, Any] = {'runtime_mode': pd.runtime_mode, 'parity_timestamp_monotonic': pd.parity_timestamp_monotonic, 'lifecycle_readiness': {'phase': pd.lifecycle.workflow_phase, 'is_active': pd.lifecycle.is_active, 'is_windup': pd.lifecycle.is_windup, 'can_accept_work': pd.lifecycle.can_accept_work, 'should_prune': pd.lifecycle.should_prune, 'phase_conflict': pd.lifecycle.phase_conflict}, 'graph_readiness': {'backend': pd.graph.backend, 'readiness': pd.graph.readiness, 'nodes': pd.graph.nodes, 'edges': pd.graph.edges}, 'export_readiness': {'readiness': pd.export_readiness.readiness, 'synthesis_engine': pd.export_readiness.synthesis_engine}, 'model_control_readiness': {'readiness': pd.model_control.readiness, 'tools_count': pd.model_control.tools_count}, 'diff_taxonomy': [d.name for d in pd.diff_taxonomy], 'blockers': pd.blockers, 'unknowns': pd.unknowns, 'compat_seams': pd.compat_seams}
        if pd.decision_gate is not None:
            result['decision_gate'] = {'gate_status': pd.decision_gate.gate_status, 'blocker_count': pd.decision_gate.blocker_count, 'unknown_count': pd.decision_gate.unknown_count, 'compat_seam_count': pd.decision_gate.compat_seam_count, 'is_proceed_allowed': pd.decision_gate.is_proceed_allowed, 'defer_to_provider': pd.decision_gate.defer_to_provider, 'blocker_categories': pd.decision_gate.blocker_categories, 'unknown_categories': pd.decision_gate.unknown_categories}
        if pd.tool_readiness is not None:
            result['tool_readiness'] = {'readiness': pd.tool_readiness.readiness, 'tool_count': pd.tool_readiness.tool_count, 'has_network_tools': pd.tool_readiness.has_network_tools, 'has_high_memory_tools': pd.tool_readiness.has_high_memory_tools, 'control_mode': pd.tool_readiness.control_mode, 'pruned_tool_count': pd.tool_readiness.pruned_tool_count, 'resource_constraint': pd.tool_readiness.resource_constraint, 'can_execute': pd.tool_readiness.can_execute, 'defer_reason': pd.tool_readiness.defer_reason}
        if pd.windup_readiness is not None:
            result['windup_readiness'] = {'readiness': pd.windup_readiness.readiness, 'is_windup_phase': pd.windup_readiness.is_windup_phase, 'synthesis_mode': pd.windup_readiness.synthesis_mode, 'synthesis_engine': pd.windup_readiness.synthesis_engine, 'has_export_data': pd.windup_readiness.has_export_data, 'export_data_quality': pd.windup_readiness.export_data_quality, 'defer_reason': pd.windup_readiness.defer_reason}
        if pd.provider_note is not None:
            result['provider_activation_note'] = {'status': pd.provider_note.status, 'deferral_reason': pd.provider_note.deferral_reason, 'has_recommendation': pd.provider_note.has_recommendation, 'recommendation': pd.provider_note.recommendation, 'next_phase_hint': pd.provider_note.next_phase_hint}
        ...
        if self._advisory_gate_snapshot is not None:
            ...
            result['advisory_gate'] = {'gate_outcome': ag.gate_outcome, 'gate_status': ag.gate_status, 'blocker_count': ag.blocker_count, 'unknown_count': ag.unknown_count, 'compat_seam_count': ag.compat_seam_count, 'blocker_reasons': ag.blocker_reasons, 'unknown_reasons': ag.unknown_reasons, 'compat_seam_reasons': ag.compat_seam_reasons, 'defer_to_provider': ag.defer_to_provider, 'gate_evaluated_at_monotonic': ag.gate_evaluated_at_monotonic, 'gate_evaluated_at_wall': ag.gate_evaluated_at_wall}
        if pd.dispatch_parity is not None:
            result['dispatch_parity'] = {'readiness': pd.dispatch_parity.readiness, 'dispatch_path': pd.dispatch_parity.dispatch_path, 'canonical_count': pd.dispatch_parity.canonical_count, 'runtime_only_count': pd.dispatch_parity.runtime_only_count, 'satisfied_count': pd.dispatch_parity.satisfied_count, 'blocked_count': pd.dispatch_parity.blocked_count, 'runtime_only_handlers': pd.dispatch_parity.runtime_only_handlers, 'blockers': pd.dispatch_parity.blockers, 'pruned_tools': pd.dispatch_parity.pruned_tools, 'will_be_pruned': pd.dispatch_parity.will_be_pruned, 'control_mode': pd.dispatch_parity.control_mode}
            if pd.dispatch_parity.execution_context is not None:
                ec = pd.dispatch_parity.execution_context
                result['execution_context'] = {'capability_ready': ec.capability_ready, 'capability_missing': ec.capability_missing, 'correlation_ready': ec.correlation_ready, 'run_id_present': ec.run_id_present, 'branch_id_present': ec.branch_id_present, 'provider_id_present': ec.provider_id_present, 'action_id_present': ec.action_id_present, 'correlation_note': ec.correlation_note, 'audit_ready': ec.audit_ready, 'execlogger_note': ec.execlogger_note, 'canonical_tool_dispatch': ec.canonical_tool_dispatch, 'runtime_only_compat_dispatch': ec.runtime_only_compat_dispatch, 'blocker_matrix': ec.blocker_matrix}
        if pd.provider_readiness is not None:
            result['provider_readiness'] = {'readiness': pd.provider_readiness.readiness, 'has_recommendation': pd.provider_readiness.has_recommendation, 'recommendation': pd.provider_readiness.recommendation, 'lifecycle_ready': pd.provider_readiness.lifecycle_ready, 'control_ready': pd.provider_readiness.control_ready, 'thermal_safe': pd.provider_readiness.thermal_safe, 'has_facts': pd.provider_readiness.has_facts, 'blockers': pd.provider_readiness.blockers, 'unknowns': pd.provider_readiness.unknowns, 'next_phase_hint': pd.provider_readiness.next_phase_hint, 'deferred_reasons': pd.provider_readiness.deferred_reasons}
    ...
    def compute_sprint_intelligence(self) -> dict[str, Any]:
        ...
        result: dict[str, Any] = {'correlation': None, 'hypothesis_pack': None, 'branch_value': None, 'signal_path': None, 'feed_verdict': None, 'public_verdict': None}
        try:
            ...
            if lane_vlist:
                ...
                result['lane_verdict'] = {'dominant_tag': dominant_tag, 'ct_storage_rejection_reasons': list(getattr(self._result, 'ct_storage_rejection_reasons', ())), 'cycle_count': len(lane_vlist), 'total_signal_strength': total_signal, 'tag_distribution': verdict_tags, 'avg_quality': avg_quality, 'ct_findings': self._result.lane_ct_accepted_findings, 'wayback_findings': self._result.lane_wayback_accepted_findings, 'pdns_findings': self._result.lane_pdns_accepted_findings, 'blockchain_findings': self._result.lane_blockchain_accepted_findings, 'ipfs_findings': self._result.lane_ipfs_accepted_findings, 'doh_findings': self._result.lane_doh_accepted_findings, 'public_findings': self._result.lane_public_accepted_findings, 'ct_loss_stage': getattr(self._result, 'ct_loss_stage', 'no_loss'), 'ct_bridge_invoked': getattr(self._result, 'ct_bridge_invoked', False), 'ct_raw_count': getattr(self._result, 'ct_raw_count', 0), 'ct_raw_sample_count': getattr(self._result, 'ct_raw_sample_count', 0), 'ct_candidates_built': getattr(self._result, 'ct_candidates_built', 0), 'ct_bridge_rejections_count': getattr(self._result, 'ct_bridge_rejections_count', 0), 'ct_candidates_accumulated': getattr(self._result, 'ct_candidates_accumulated', 0), 'ct_candidates_stored': getattr(self._result, 'ct_candidates_stored', 0), 'ct_storage_rejected': getattr(self._result, 'ct_storage_rejected', 0), 'ct_bridge_candidate_count': getattr(self._result, 'ct_candidate_count', 0), 'ct_bridge_valid_domain_count': getattr(self._result, 'ct_valid_domain_count', 0), 'ct_bridge_build_success_count': getattr(self._result, 'ct_bridge_build_success_count', 0), 'ct_bridge_quality_rejected_count': getattr(self._result, 'ct_bridge_quality_rejected_count', 0), 'ct_expansion_clues_count': getattr(self._result, 'ct_expansion_clues_count', 0), 'ct_valid_public_domains': getattr(self._result, 'ct_valid_public_domains', 0), 'ct_wildcard_domains': getattr(self._result, 'ct_wildcard_domains', 0), 'ct_private_reserved_domains': getattr(self._result, 'ct_private_reserved_domains', 0), 'ct_duplicate_candidates': getattr(self._result, 'ct_duplicate_candidates', 0), 'quality_rejection_summary_by_family': getattr(self._result, 'quality_rejection_summary_by_family', {}), 'duplicate_rejection_summary_by_family': getattr(self._result, 'duplicate_rejection_summary_by_family', {}), 'low_information_by_family': getattr(self._result, 'low_information_by_family', {}), 'quality_rejection_ledger_size': len(getattr(self._result, 'quality_rejection_ledger', ())), 'wayback_advisory_clues_count': getattr(self._result, 'wayback_advisory_clues_count', 0), 'wayback_changed_url_count': getattr(self._result, 'wayback_changed_url_count', 0), 'wayback_added_url_count': getattr(self._result, 'wayback_added_url_count', 0), 'wayback_digest_changed_count': getattr(self._result, 'wayback_digest_changed_count', 0), 'wayback_unchanged_rejected': getattr(self._result, 'wayback_unchanged_rejected', 0), 'passive_dns_advisory_clues_count': getattr(self._result, 'passive_dns_advisory_clues_count', 0), 'passive_dns_private_ip_rejected': getattr(self._result, 'passive_dns_private_ip_rejected', 0), 'passive_dns_empty_ip_rejected': getattr(self._result, 'passive_dns_empty_ip_rejected', 0), 'arrow_last_flush_error': getattr(self._result, 'arrow_last_flush_error', '') or '', 'arrow_batch_dropped': getattr(self._result, 'arrow_batch_dropped_after_flush_failure', 0), 'prewindup_barrier_errors': getattr(self._result, 'prewindup_barrier_errors', 0) or 0, 'return_guard_errors': getattr(self._result, 'return_guard_errors', 0) or 0}
        except Exception:
        ...
        try:
            ...
            result['correlation'] = {'risk_score': round(corr.risk_score, 3), 'verdict': corr.verdict, 'anomaly_count': corr.anomaly_count, 'top_themes': list(corr.top_themes[:5]), 'theme_count': len(corr.themes), 'signal_quality': getattr(corr, 'signal_quality', 'weak'), 'cross_source_confidence': round(getattr(corr, 'cross_source_confidence', 0.0), 3), 'campaign_confidence': round(getattr(corr, 'campaign_confidence', 0.0), 3), 'dominant_cluster': getattr(corr, 'dominant_cluster', None), 'so_what': getattr(corr, 'so_what', ''), 'what_matters_first': getattr(corr, 'what_matters_first', ''), 'operator_shortlist': [{'action': item.get('action', ''), 'target': item.get('target', '')[:80], 'rationale': item.get('rationale', '')} for item in (getattr(corr, 'operator_shortlist', None) or [])[:3] if isinstance(item, dict)], 'confidence_note': getattr(corr, 'confidence_note', ''), 'corroborated_iocs_count': len(getattr(corr, 'corroborated_iocs', []) or []), 'top_priority_pivots_count': len(getattr(corr, 'top_priority_pivots', []) or [])}
        except Exception:
        ...
        try:
            ...
            if finding_texts:
                ...
                result['hypothesis_pack'] = {'hypothesis_count': len(pack.hypotheses), 'query_count': len(pack.suggested_queries), 'ioc_follow_ups': len(pack.ioc_follow_ups), 'source_hints_count': len(pack.source_hints), 'provenance': pack.provenance, 'signal_quality': getattr(pack, 'signal_quality', 'weak'), 'what_matters_first': getattr(pack, 'what_matters_first', ''), 'confidence_note': getattr(pack, 'confidence_note', ''), 'top_queries': [{'query': q.get('query', ''), 'rationale': q.get('rationale', '')[:80]} for q in (pack.suggested_queries or [])[:5] if isinstance(q, dict)], 'operator_shortlist': [{'action': item.get('action', ''), 'target': item.get('target', '')[:80], 'rationale': item.get('rationale', '')} for item in (getattr(pack, 'operator_shortlist', None) or [])[:3] if isinstance(item, dict)], 'discarded_as_redundant': [{'action_type': item.get('action_type', ''), 'query': item.get('query', '')[:120], 'reason_discarded': item.get('reason_discarded', ''), 'pivot_type': item.get('pivot_type', ''), 'priority': item.get('priority', 0.0)} for item in (getattr(pack, 'discarded_as_redundant', lambda max_items=3: [])(max_items=3) or [])[:3] if isinstance(item, dict)]}
        except Exception:
        ...
        try:
            ...
            if feed_vlist:
                ...
                result['feed_verdict'] = {'dominant_tag': dominant_tag, 'cycle_count': len(feed_vlist), 'total_signal_strength': total_signal, 'total_fallback_waste': total_fallback_waste, 'avg_quality': avg_quality, 'tag_distribution': verdict_tags}
        except Exception:
        ...
        try:
            ...
            if pub_vlist:
                ...
                result['public_verdict'] = {'cycle_count': len(pub_vlist), 'avg_waste_ratio': round(sum(waste_ratios) / len(waste_ratios), 3) if waste_ratios else 0.0, 'avg_value_ratio': round(sum(value_ratios) / len(value_ratios), 3) if value_ratios else 0.0, 'avg_corroboration_vs_burn': round(sum(corroborations) / len(corroborations), 3) if corroborations else 0.0, 'avg_discovery_squandered': round(sum(squandered_hits) / len(squandered_hits), 2) if squandered_hits else 0.0, 'total_discovery_squandered': sum(squandered_hits), 'avg_noise_fetch_ratio': round(sum(noise_ratios) / len(noise_ratios), 3) if noise_ratios else 0.0, 'dominant_next_action': dominant_action, 'dominant_confidence_note': dominant_conf, 'action_distribution': {a: next_actions.count(a) for a in set(next_actions)}}
        except Exception:
        ...
        try:
            ...
            if lane_vlist:
                ...
                dominant_tag = max(verdict_tags, key=verdict_tags.get) if verdict_tags else 'none'
                avg_quality = total_quality / len(lane_vlist) if lane_vlist else 0.0
                result['lane_verdict'] = {'dominant_tag': dominant_tag, 'ct_storage_rejection_reasons': list(getattr(self._result, 'ct_storage_rejection_reasons', ())), 'cycle_count': len(lane_vlist), 'total_signal_strength': total_signal, 'tag_distribution': verdict_tags, 'avg_quality': avg_quality, 'ct_findings': self._result.lane_ct_accepted_findings, 'wayback_findings': self._result.lane_wayback_accepted_findings, 'pdns_findings': self._result.lane_pdns_accepted_findings, 'blockchain_findings': self._result.lane_blockchain_accepted_findings, 'ipfs_findings': self._result.lane_ipfs_accepted_findings, 'doh_findings': self._result.lane_doh_accepted_findings, 'ct_loss_stage': getattr(self._result, 'ct_loss_stage', 'no_loss'), 'ct_bridge_invoked': getattr(self._result, 'ct_bridge_invoked', False), 'ct_raw_count': getattr(self._result, 'ct_raw_count', 0), 'ct_raw_sample_count': getattr(self._result, 'ct_raw_sample_count', 0), 'ct_candidates_built': getattr(self._result, 'ct_candidates_built', 0), 'ct_bridge_rejections_count': getattr(self._result, 'ct_bridge_rejections_count', 0), 'ct_candidates_accumulated': getattr(self._result, 'ct_candidates_accumulated', 0), 'ct_candidates_stored': getattr(self._result, 'ct_candidates_stored', 0), 'ct_storage_rejected': getattr(self._result, 'ct_storage_rejected', 0), 'ct_bridge_candidate_count': getattr(self._result, 'ct_candidate_count', 0), 'ct_bridge_valid_domain_count': getattr(self._result, 'ct_valid_domain_count', 0), 'ct_bridge_build_success_count': getattr(self._result, 'ct_bridge_build_success_count', 0), 'ct_bridge_quality_rejected_count': getattr(self._result, 'ct_bridge_quality_rejected_count', 0), 'ct_expansion_clues_count': getattr(self._result, 'ct_expansion_clues_count', 0), 'ct_valid_public_domains': getattr(self._result, 'ct_valid_public_domains', 0), 'ct_wildcard_domains': getattr(self._result, 'ct_wildcard_domains', 0), 'ct_private_reserved_domains': getattr(self._result, 'ct_private_reserved_domains', 0), 'ct_duplicate_candidates': getattr(self._result, 'ct_duplicate_candidates', 0), 'wayback_advisory_clues_count': getattr(self._result, 'wayback_advisory_clues_count', 0), 'wayback_changed_url_count': getattr(self._result, 'wayback_changed_url_count', 0), 'wayback_added_url_count': getattr(self._result, 'wayback_added_url_count', 0), 'wayback_digest_changed_count': getattr(self._result, 'wayback_digest_changed_count', 0), 'wayback_unchanged_rejected': getattr(self._result, 'wayback_unchanged_rejected', 0), 'passive_dns_advisory_clues_count': getattr(self._result, 'passive_dns_advisory_clues_count', 0), 'passive_dns_private_ip_rejected': getattr(self._result, 'passive_dns_private_ip_rejected', 0), 'passive_dns_empty_ip_rejected': getattr(self._result, 'passive_dns_empty_ip_rejected', 0), 'arrow_last_flush_error': getattr(self._result, 'arrow_last_flush_error', '') or '', 'arrow_batch_dropped': getattr(self._result, 'arrow_batch_dropped_after_flush_failure', 0), 'prewindup_barrier_errors': getattr(self._result, 'prewindup_barrier_errors', 0) or 0, 'return_guard_errors': getattr(self._result, 'return_guard_errors', 0) or 0}
        except Exception:
            result['lane_verdict'] = None
        try:
            ...
            lane_f = self._result.lane_ct_accepted_findings + self._result.lane_wayback_accepted_findings + self._result.lane_pdns_accepted_findings + self._result.lane_blockchain_accepted_findings + self._result.lane_ipfs_accepted_findings + self._result.lane_doh_accepted_findings
            ...
            feed_dominance_override = self._result.feed_dominance_ratio is not None and self._result.feed_dominance_ratio >= 0.95 and ((self._result.public_accepted_findings or 0) == 0) and (self._result.lane_ct_accepted_findings + self._result.lane_wayback_accepted_findings + self._result.lane_pdns_accepted_findings + self._result.lane_blockchain_accepted_findings + self._result.lane_ipfs_accepted_findings + self._result.lane_doh_accepted_findings == 0)
            if feed_dominance_override:
                dominant_path = 'feed'
                is_corroborated_flag = False
            else:
                is_corroborated_flag = cross_conf > 0.4
            result['signal_path'] = {'dominant_signal_path': dominant_path, 'next_pivot_recommendation': next_pivot, 'corroboration_score': corroboration_score, 'branch_mix_health': branch_mix_health, 'is_noisy': sig_quality == 'weak' and cross_conf < 0.2, 'is_corroborated': is_corroborated_flag, 'campaign_signal': camp_conf > 0.3}
        except Exception:
        ...
        try:
            ...
            result['branch_value'] = {'feed_findings': feed_f, 'public_findings': pub_f, 'feed_pattern_hits': feed_h, 'public_pattern_hits': pub_h, 'feed_pct': feed_pct, 'public_pct': pub_pct, 'branch_verdict': branch_verdict, 'recommendation': recommendation}
        except Exception:
        ...
        try:
            ...
            if total_findings == 0:
                posture = 'depleted'
            elif self._result.feed_dominance_ratio is not None and self._result.feed_dominance_ratio >= 0.95 and ((br_val.get('public_findings', 0) or 0) == 0) and ((br_val.get('feed_findings', 0) or 0) > 0):
                posture = 'noisy'
            ...
            avg_noise = pub_v.get('avg_noise_fetch_ratio', 0.0)
            branch_conversion_health = round((1.0 if is_corroborated else 0.0) * corroboration_score * (1.0 - avg_noise), 3)
            ...
            result['sprint_verdict'] = {'posture': posture, 'dominant_signal': dominant_signal, 'branch_verdict': branch_verdict, 'branch_mix': branch_mix, 'corroboration_score': corroboration_score, 'is_corroborated': is_corroborated, 'campaign_signal': campaign_signal, 'next_pivot': next_pivot, 'dominant_action': dominant_action, 'first_action': first_action, 'backup_action': backup_action, 'intel_what_matters': what_matters, 'confidence': dominant_conf, 'feed_tag': feed_tag, 'feed_avg_quality': feed_avg_qual, 'risk_score': risk_score, 'hypothesis_count': hyp_count, 'total_findings': total_findings, 'export_ready': export_ready, 'proof_grade': proof_grade, 'operator_ready': operator_ready, 'decision_pressure': decision_pressure, 'branch_conversion_health': branch_conversion_health, 'discovery_efficiency': discovery_efficiency}
        except Exception:
            pass
        return result
    ...
    def _emit_source_family_event(self, family: str, event: str, count: int=0, reason: str='', terminal_state: str='') -> None:
        """
        ...
        """
        evt = {'family': family, 'event': event, 'count': count, 'reason': reason, 'terminal_state': terminal_state, 'ts_monotonic': _time.monotonic()}
        ...
        try:
            ...
            for idx, insight in enumerate(insights[:20]):
                ...
                if not content:
                    continue
                if CanonicalFinding:
                    finding = CanonicalFinding(finding_id=f'graph_rag_ctx_{int(ts * 1000)}_{idx}', query=query, source_type=SourceType.CONTEXT_SEED)
                    if self._evidence_log is not None:
                        try:
                            self._evidence_log.create_event('observation', finding.model_dump() if hasattr(finding, 'model_dump') else vars(finding), source_ids=[finding.source_id] if hasattr(finding, 'source_id') else [], confidence=insight.get('similarity', 0.5), ts=ts, provenance=('graph_rag',), payload_text=content[:2048])
                        except Exception:
                            pass
                    findings.append(finding)
                else:
                    findings.append({'finding_id': f'graph_rag_ctx_{int(ts * 1000)}_{idx}', 'query': query, 'source_type': 'context_seed', 'confidence': insight.get('similarity', 0.5), 'ts': ts, 'provenance': ('graph_rag',), 'payload_text': content[:2048]})
            logger.debug('[graph_rag] found %d context insights', len(findings))
        except Exception as _e:
            logger.debug('[graph_rag] sidecar failed: %s', _e)
        return findings

async def async_run_tiered_feed_sprint_once(sources: Sequence[str], config: SprintSchedulerConfig | None=None, lifecycle: object | None=None, now_monotonic: float | None=None, query: str='', duckdb_store: Any=None) -> SprintSchedulerResult:
    """
    ...
    if lifecycle is None:
        ...
        lifecycle = SprintLifecycleManager(sprint_duration_s=config.sprint_duration_s, windup_lead_s=config.effective_windup_lead_s)
    scheduler = SprintScheduler(config)
    return await scheduler.run(lifecycle, sources, now_monotonic, query, duckdb_store)
SPRINT_TIERS: dict = {'quick': {'min_duration': 60, 'hermes': False, 'windup_lead_s': 0}, 'standard': {'min_duration': 180, 'hermes': True, 'windup_lead_s': 30}, 'deep': {'min_duration': 300, 'hermes': True, 'windup_lead_s': 30}, 'thorough': {'min_duration': 600, 'hermes': True, 'windup_lead_s': 30}}
