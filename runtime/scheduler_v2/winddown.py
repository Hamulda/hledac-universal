...
class WinddownOrchestrator:
    ...
    async def run(self, ctx: Any, lifecycle: Any, query: str) -> WinddownPhaseResult:
        ...
        try:
            ...
            _logger = __import__("logging").getLogger(__name__)
            _logger.info(
                "[PHYSICS-06] BlitzGC teardown: gc_reenabled=%s, gen2_collected=%d, gen0_ticks=%d, freeze=%s",
    ...
    async def _run_cti_export(
        self, ctx: Any, rend_cti_stix: Any, collect_cti_inputs: Any, report: dict[str, Any], export_dir: str | None
    ) -> None:
        """Run CTI STIX export."""
        try:
    ...
    async def _run_delta_sync(self, ctx: Any) -> None:
        ...
        try:
            ...
            _store = ctx.duckdb_store
            _sprint_id = (
                getattr(ctx, "sprint_id", None)
                or getattr(ctx, "_sprint_id", "")
                or str(getattr(ctx.result, "sprint_id", ""))
            )
            ...
            _logger.info(
                "[META-002] DeltaSyncEngine: sync complete for sprint=%s — synced=%d, errors=%d, observations=%d",
                _sprint_id,
...
def _import_exporters() -> tuple:
    ...
    from hledac.universal.export.report_render import collect_cti_inputs
    from hledac.universal.export.report_render import render_cti_stix as rend_cti_stix
    from hledac.universal.export.report_render import render_jsonld as rend_jsonld
    from hledac.universal.export.report_render import render_markdown as rend_md
    from hledac.universal.export.report_render import render_stix as rend_stix
