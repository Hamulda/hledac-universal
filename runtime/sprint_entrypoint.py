...
from hledac.universal._core import memory_cycle as _memory_cycle
...
def _fail_safe(level: str='debug', label: str=''):
    ...
    try:
        ...
        _log_level = {'debug': logger.debug, 'info': logger.info, 'warning': logger.warning, 'error': logger.error}.get(level, logger.debug)
        if label:
...
from hledac.universal._core.resource_governor import CLEAN_SWAP_MAX_GIB, HARD_BLOCK_SWAP_GIB, sample_uma_status
...
from hledac.universal.runtime.acquisition_strategy import ACQUISITION_REPORT_SCHEMA_VERSION, SourceFamilyOutcome, build_acquisition_report, canonicalize_source_family_outcomes, complete_source_family_outcomes_from_lane_details, normalize_source_family_outcome, reconcile_lane_detail_fields
...
_build_runtime = None
_run_runtime = None
from hledac.universal.utils.asyncx import first_completed, parallel, parallel_ok, safe_create_task, safe_wait_for, _check_gathered
...
_REPORT_SERIALIZE_OPTIONS: int = orjson.OPT_INDENT_2 if os.environ.get('HLEDAC_REPORT_PRETTY_PRINT', '0') == '1' else orjson.OPT_APPEND_NEWLINE
...
class SprintFlags(Struct, frozen=True):
    """
...
def _is_meaningful_run(actual_duration_s: float, cycles_completed: int, cycles_started: int, accepted_findings: int, total_pattern_hits: int, swap_detected: bool=False, uma_state: str='ok') -> tuple[bool, str]:
    """
...
class AcqReportPayload(Struct, frozen=True, eq=False):
    """
...
def _get_timing_fields(r: AcqReportPayload) -> dict[str, Any]:
    ...
    """
    return {'entered_active_at_monotonic': r.entered_active_at_monotonic, 'pre_loop_elapsed_s': r.pre_loop_elapsed_s, 'first_cycle_started_at_monotonic': r.first_cycle_started_at_monotonic, 'actual_duration_s': r.actual_duration_s, 'elapsed_pct': r.elapsed_pct, 'active_window_elapsed_s': r.active_window_elapsed_s, 'requested_duration_s': r.requested_duration_s, 'active_window_budget_s': r.active_window_budget_s, 'pre_loop_blocker_reason': r.pre_loop_blocker_reason, 'pre_active_starved': r.pre_active_starved, 'dedup_preload_count': r.dedup_preload_count, 'dedup_preload_elapsed_s': r.dedup_preload_elapsed_s, 'windup_guard_call_count': r.windup_guard_call_count, 'windup_guard_required_lanes': r.windup_guard_required_lanes, 'windup_guard_not_applicable': r.windup_guard_not_applicable, 'windup_guard_last_reason': r.windup_guard_last_reason, 'windup_guard_last_phase': r.windup_guard_last_phase, 'windup_guard_last_allowed': r.windup_guard_last_allowed, 'windup_guard_last_callback_not_executed_reason': r.windup_guard_last_callback_not_executed_reason, 'windup_guard_callback_supplied_count': r.windup_guard_callback_supplied_count, 'windup_guard_callback_executed_count': r.windup_guard_callback_executed_count, 'windup_efficiency': r.windup_efficiency, 'effective_windup_lead_used_s': r.effective_windup_lead_used_s, 'windup_lead_adaptive_factor': r.windup_lead_adaptive_factor, 'prewindup_barrier_checked': r.prewindup_barrier_checked, 'prewindup_barrier_satisfied': r.prewindup_barrier_satisfied, 'prewindup_barrier_duration_s': r.prewindup_barrier_duration_s, 'windup_delayed_for_nonfeed': r.windup_delayed_for_nonfeed, 'transport_efficiency': r.transport_efficiency}
...
def _get_memory_fields(r: AcqReportPayload) -> dict[str, Any]:
    ...
    """
    return {'peak_rss_gib': r.peak_rss_gib, 'governor_uma_state': r.governor_uma_state, 'governor_system_used_gib': r.governor_system_used_gib, 'governor_swap_detected': r.governor_swap_detected, 'governor_io_only': r.governor_io_only, 'malloc_pressure_relief_count': r.malloc_pressure_relief_count, 'malloc_pressure_relief_last_rc': r.malloc_pressure_relief_last_rc, 'malloc_pressure_relief_last_at_s': r.malloc_pressure_relief_last_at_s, 'pressure_violations': r.pressure_violations, 'budget_violations': r.budget_violations}
...
def _normalize_seed_context(report: dict[str, Any], r: AcqReportPayload) -> dict[str, Any]:
    """
...
def _build_sfo_list(r: AcqReportPayload) -> list[SourceFamilyOutcome]:
    ...
    if r.accepted_findings > 0 or r.total_pattern_hits > 0:
        sfo_list.append(normalize_source_family_outcome('FEED', {'family': 'FEED', 'attempted': True, 'skipped': False, 'skip_reason': None, 'raw_count': r.total_pattern_hits, 'built_count': 0, 'accepted_count': r.accepted_findings, 'error': None, 'timeout': False, 'duration_s': None}))
    ...
    pub_has_outcome = bool(r.public_discovered > 0 or r.public_accepted_findings > 0 or (pub_pts and pub_pts != 'NOT_SCHEDULED') or r.public_error or pub_fetch_attempted)
    if pub_has_outcome:
        sfo_list.append(normalize_source_family_outcome('PUBLIC', {'family': 'PUBLIC', 'attempted': True, 'skipped': False, 'skip_reason': None, 'raw_count': r.public_discovered, 'built_count': 0, 'accepted_count': r.public_accepted_findings, 'error': r.public_error or r.public_terminal_stage or None, 'timeout': r.public_terminal_stage == 'DISCOVERY_TIMEOUT', 'duration_s': None}))
    ct_has_outcome = bool(r.ct_log_discovered > 0 or r.ct_log_accepted_findings > 0 or r.ct_terminal_stage or r.ct_log_error or r.ct_planned or r.ct_scheduled or r.ct_request_attempted or r.ct_provider_status)
    if ct_has_outcome:
        sfo_list.append(normalize_source_family_outcome('CT', {'family': 'CT', 'attempted': bool(r.ct_request_attempted or r.ct_scheduled or r.ct_planned), 'skipped': False, 'skip_reason': None, 'raw_count': r.ct_log_discovered, 'built_count': 0, 'accepted_count': r.ct_log_accepted_findings, 'error': r.ct_log_error or r.ct_terminal_stage or None, 'timeout': r.ct_terminal_stage == 'request_timeout', 'duration_s': None}))
    ...
    for _o in r.acquisition_lane_outcomes or ():
        ...
        sfo_list.append(normalize_source_family_outcome(getattr(_o, 'source_family', _lane.upper()), {'family': getattr(_o, 'source_family', _lane.upper()), 'attempted': getattr(_o, 'attempted', False), 'skipped': not getattr(_o, 'attempted', False), 'skip_reason': None if getattr(_o, 'attempted', False) else 'lane_not_attempted', 'raw_count': getattr(_o, 'ct_results_raw', 0), 'built_count': getattr(_o, 'ct_candidates_built', 0), 'accepted_count': getattr(_o, 'accepted_findings', 0), 'error': getattr(_o, 'error', None), 'timeout': getattr(_o, 'timeout', False), 'duration_s': getattr(_o, 'duration_s', None)}))
...
def acq_payload_to_dict(result: Any, scheduler: Any, query: str, _duration_s: float) -> dict[str, Any]:
    ...
    se_dict = {'exit_path': r.scheduler_exit_path, 'exit_reason': r.scheduler_exit_reason, 'exit_phase': r.scheduler_exit_phase, 'exit_cycle': r.scheduler_exit_cycle, 'exit_elapsed_s': r.scheduler_exit_elapsed_s, 'exit_guard_checked': r.scheduler_exit_guard_checked, 'exit_guard_satisfied': r.scheduler_exit_guard_satisfied}
    rg_dict = {'return_guard_checked': r.return_guard_checked, 'return_guard_satisfied': r.return_guard_satisfied, 'return_guard_block_reason': r.return_guard_block_reason, 'return_guard_attempted_lanes': r.return_guard_attempted_lanes, 'return_guard_skipped_lanes': r.return_guard_skipped_lanes, 'return_guard_errors': r.return_guard_errors, 'return_guard_delayed_for_nonfeed': r.return_guard_delayed_for_nonfeed}
    wg_dict = {'windup_guard_call_count': r.windup_guard_call_count, 'windup_guard_callback_supplied_count': r.windup_guard_callback_supplied_count, 'windup_guard_callback_executed_count': r.windup_guard_callback_executed_count, 'windup_guard_required_lanes': r.windup_guard_required_lanes, 'windup_guard_not_applicable': r.windup_guard_not_applicable, 'windup_guard_last_reason': r.windup_guard_last_reason, 'windup_guard_last_allowed': r.windup_guard_last_allowed, 'windup_guard_callback_not_executed_reason': r.windup_guard_last_callback_not_executed_reason}
    pwb = {'checked': r.prewindup_barrier_checked, 'satisfied': r.prewindup_barrier_satisfied, 'required_lanes': r.prewindup_barrier_required_lanes, 'attempted_lanes': r.prewindup_barrier_attempted_lanes, 'skipped_lanes': r.prewindup_barrier_skipped_lanes, 'errors': r.prewindup_barrier_errors, 'duration_s': r.prewindup_barrier_duration_s, 'windup_delayed': r.windup_delayed_for_nonfeed, 'nonfeed_scheduler_gap_resolved': getattr(result, 'nonfeed_scheduler_gap_resolved', False)}
    ...
    try:
        _acq_report = build_acquisition_report(plan=plan, terminality=term_rep, nonfeed_plan_debug=nd, source_family_outcomes=sfo_list, return_guard=rg_dict, prewindup_barrier=pwb, scheduler_exit=se_dict, windup_guard_observation=wg_dict, query=query, acquisition_profile=_acq_profile, feed_cap_reason=_feed_cap_reason, nonfeed_priority_enabled=_nonfeed_priority_enabled, nonfeed_profile_expected_lanes=_nonfeed_profile_expected_lanes, public_terminal_stage=r.public_terminal_stage, public_stage_counters=r.public_stage_counters, public_discovery_empty_reason=r.public_discovery_empty_reason, public_discovery_debug_reason=r.public_discovery_debug_reason, public_provider_selection_debug=r.public_provider_selection_debug or {}, ct_provider_status=r.ct_provider_status, ct_cache_used=r.ct_cache_used, ct_cache_stale=r.ct_cache_stale, ct_cache_age_s=r.ct_cache_age_s, ct_quarantine_count=r.ct_quarantine_count, ct_quarantine_samples=r.ct_quarantine_samples, ct_planned=r.ct_planned, ct_scheduled=r.ct_scheduled, ct_provider_selected=r.ct_provider_selected, ct_request_attempted=r.ct_request_attempted, ct_request_timeout=r.ct_request_timeout, ct_raw_count=r.ct_raw_count, ct_bridge_invoked=r.ct_bridge_invoked, ct_candidates_built=r.ct_candidates_built, ct_storage_attempted=r.ct_storage_attempted, ct_storage_accepted=r.ct_storage_accepted, ct_terminal_stage=r.ct_terminal_stage, ct_prelude_missing_but_final_attempted=r.ct_prelude_missing_but_final_attempted, quality_rejection_summary_by_family=r.quality_rejection_summary_by_family, duplicate_rejection_summary_by_family=r.duplicate_rejection_summary_by_family, low_information_by_family=r.low_information_by_family, nonfeed_candidate_ledger_summary=r.nonfeed_candidate_ledger_summary, feed_dominance_budget=getattr(plan, 'feed_dominance_budget', None) if plan else None, doh_planned=r.doh_planned, doh_scheduled=r.doh_scheduled, doh_request_attempted=r.doh_request_attempted, doh_domains_attempted=r.doh_domains_attempted, doh_raw_count=r.doh_raw_count, doh_accepted_findings=r.doh_accepted_findings, doh_terminal_stage=r.doh_terminal_stage, doh_provider_errors=r.doh_provider_errors, doh_cache_used=r.doh_cache_used, nonfeed_expected_lanes=r.nonfeed_expected_lanes, nonfeed_missing_expected_lanes=r.nonfeed_missing_expected_lanes, wayback_terminal_state=r.wayback_terminal_state, passive_dns_terminal_state=r.passive_dns_terminal_state, nonfeed_surface_complete=getattr(result, 'nonfeed_surface_complete', False), pivot_seed_domains=r.pivot_seed_domains, pivot_seed_ips=r.pivot_seed_ips, pivot_seed_urls=r.pivot_seed_urls, pivot_seed_hashes=r.pivot_seed_hashes, pivot_seed_cves=r.pivot_seed_cves, seed_context_available=bool(r.pivot_seed_domains or r.pivot_seed_ips or r.pivot_seed_urls or r.pivot_seed_hashes or r.pivot_seed_cves), seed_context_propagated=r.seed_context_propagated, lanes_unlocked_by_seed_context=r.lanes_unlocked_by_seed_context, acquisition_plan_build_failed=r.acquisition_plan_build_failed, acquisition_plan_build_error_type=r.acquisition_plan_build_error_type, acquisition_plan_build_error=r.acquisition_plan_build_error, acquisition_plan_present_for_prelude=r.acquisition_plan_present_for_prelude, acquisition_plan_lanes_for_prelude=r.acquisition_plan_lanes_for_prelude, acquisition_plan_enabled_lanes_for_prelude=r.acquisition_plan_enabled_lanes_for_prelude, acquisition_plan_profile_for_prelude=r.acquisition_plan_profile_for_prelude, acquisition_plan_build_error_for_prelude=r.acquisition_plan_build_error_for_prelude, nonfeed_prelude_enabled=r.nonfeed_prelude_enabled, nonfeed_prelude_expected_lanes=r.nonfeed_prelude_expected_lanes, nonfeed_prelude_attempted_lanes=r.nonfeed_prelude_attempted_lanes, nonfeed_prelude_terminal_lanes=r.nonfeed_prelude_terminal_lanes, nonfeed_prelude_missing_lanes=r.nonfeed_prelude_missing_lanes, nonfeed_prelude_error_by_lane=r.nonfeed_prelude_error_by_lane, nonfeed_prelude_accepted_by_lane=r.nonfeed_prelude_accepted_by_lane, nonfeed_prelude_duration_s=r.nonfeed_prelude_duration_s, nonfeed_prelude_feed_blocked_until_complete=r.nonfeed_prelude_feed_blocked_until_complete)
        ...
        _acq_report.update(schema_version=f'{ACQUISITION_REPORT_SCHEMA_VERSION}-fallback', fallback_reason=f'canonical_build_failed: {_exc}', acquisition_report_fallback_used=True, terminality=term_rep, source_family_outcomes=sfo_list, return_guard=rg_dict, prewindup_barrier=pwb, scheduler_exit=se_dict, windup_guard_observation=wg_dict, nonfeed_plan_debug=nd, plan=getattr(plan, 'plans', None) if plan else None, prelude_plan=getattr(plan, 'plans', []) if plan else [], required_lane_plan=term_rep.get('required_lanes', []) if term_rep else [], runtime_attempted_lanes=[o.family for o in sfo_list if o.attempted and o.family], effective_acquisition_plan=list(set(term_rep.get('required_lanes', []) if term_rep else []) | {o.family for o in sfo_list if o.attempted and o.family}), plan_semantics='effective_runtime' if any((o.attempted for o in sfo_list)) else 'prelude_only')
        _acq_report['acquisition_profile_input'] = None
        _acq_report['acquisition_profile_effective'] = acq_effective
        _acq_report['acquisition_profile_normalized'] = False
        _acq_report['budget_violations'] = r.budget_violations
        ...
        _acq_report = complete_source_family_outcomes_from_prelude(_acq_report)
        _acq_report = _normalize_seed_context(_acq_report, r)
    return {'acquisition_report': _acq_report, 'acquisition_terminality_checked': r.acquisition_terminality_checked, 'acquisition_terminality_satisfied': r.acquisition_terminality_satisfied, 'acquisition_terminality_missing_lanes': r.acquisition_terminality_missing_lanes, 'acquisition_terminality_report': term_rep, 'source_family_outcomes': sfo_list, 'scheduler_exit': se_dict, 'return_guard': rg_dict, 'windup_guard_observation': wg_dict, 'prewindup_barrier': pwb, 'acquisition_prelude_checked': r.acquisition_prelude_checked, 'acquisition_prelude_ran': r.acquisition_prelude_ran, 'acquisition_prelude_required_lanes': r.acquisition_prelude_required_lanes, 'acquisition_prelude_terminal_lanes': r.acquisition_prelude_terminal_lanes, 'acquisition_prelude_missing_lanes': r.acquisition_prelude_missing_lanes, 'acquisition_prelude_skipped_lanes': r.acquisition_prelude_skipped_lanes, 'acquisition_prelude_errors': r.acquisition_prelude_errors, 'acquisition_prelude_duration_s': r.acquisition_prelude_duration_s, 'acquisition_prelude_reason': r.acquisition_prelude_reason, 'early_exit_class': r.early_exit_class, 'early_exit_reason': r.early_exit_reason, 'requested_duration_s': r.requested_duration_s, 'actual_duration_s': r.actual_duration_s, 'elapsed_pct': r.elapsed_pct, 'active_window_budget_s': r.active_window_budget_s, 'active_window_elapsed_s': r.active_window_elapsed_s}

def _scheduler_result_acquisition_payload(result: Any, scheduler: Any, query: str, duration_s: float) -> dict[str, Any]:
    """
...
def _runtime_truth(inp: RuntimeTruthInput) -> dict:
    ...
    is_meaningful, evidence_note = _is_meaningful_run(inp.actual_duration_s, inp.cycles_completed, inp.cycles_started, inp.accepted_findings, inp.total_pattern_hits, swap_detected=inp.swap_detected, uma_state=inp.uma_state)
    branch_mix = {'feed_findings': inp.feed_findings, 'public_findings': inp.public_accepted_findings, 'ct_findings': inp.ct_findings}
    if inp.ct_findings > 0 and inp.feed_findings == 0 and (inp.public_accepted_findings == 0):
        ...
        feed_dominance_ratio = inp.feed_findings / (inp.feed_findings + total_nonfeed) if inp.feed_findings + total_nonfeed > 0 else 1.0
        ...
        primary = 'none'
    return {'is_meaningful': is_meaningful, 'evidence_note': evidence_note, 'command_params': {'query': inp.query, 'requested_duration_s': inp.duration_s}, 'actual_duration_s': round(inp.actual_duration_s, 2), 'cycles_completed': inp.cycles_completed, 'cycles_started': inp.cycles_started, 'branch_mix': branch_mix, 'primary_signal_source': primary, 'total_pattern_hits': inp.total_pattern_hits, 'accepted_findings': inp.accepted_findings, 'pre_sprint_swap_detected': inp.swap_detected, 'pre_sprint_uma_state': inp.uma_state, 'branch_timeout_count': inp.branch_timeout_count, 'public_branch_timed_out': inp.public_branch_timed_out, 'ct_branch_timed_out': inp.ct_branch_timed_out}
...
def _run_sprint_preflight_guards(logger: logging.Logger, duration_s: float, windup_lead_s: float | None, flags: 'SprintFlags | None', force: bool, early_config: 'SprintSchedulerConfig') -> tuple[float, 'UMAStatus']:
    """
    ...
    if _active_window_s < 0.0:
        logger.error('[F271C-INVARIANT] computed negative active_window_s=%s (duration_s=%s, effective_windup_s=%s). F221 guard is broken.', _active_window_s, duration_s, _effective_windup_s)
        sys.exit(2)
    if _effective_windup_s >= _active_window_s * 0.8:
        ...
        if _force_override:
            logger.warning('[F289-FORCED] Windup %.0fs would consume %.0f%% of active window %.0fs. Proceeding due to --force.', _effective_windup_s, _pct, _active_window_s)
        else:
            logger.error('[F289-ABORT] Windup %.0fs would consume %.0f%% of active window %.0fs. Reduce windup (--windup-lead) or increase duration.', _effective_windup_s, _pct, _active_window_s)
            sys.exit(2)
    if _active_window_s < float(MIN_ACTIVE_WINDOW_S):
        _required_duration_s = int(_effective_windup_s + float(MIN_ACTIVE_WINDOW_S))
        if _force_override:
            logger.warning('[F221-FORCED] duration=%ds gives only %ds active window (windup_lead_effective=%ds). Proceeding due to --force.', int(duration_s), max(0, int(_active_window_s)), int(_effective_windup_s))
        else:
            logger.error('[F221-ABORT] Sprint duration %ds gives only %ds active window (windup_lead_effective=%ds). Minimum recommended: --duration %d. Use --force to override.', int(duration_s), max(0, int(_active_window_s)), int(_effective_windup_s), _required_duration_s)
            sys.exit(2)
    if windup_lead_s is not None:
        _windup_fraction = float(windup_lead_s) / float(duration_s)
        if _windup_fraction > 0.9:
            logger.warning('[F289-WINDUP-FRACTION] windup_lead_s=%.0fs is %.0f%% of duration=%ds. This may cause the sprint to enter WINDUP immediately, leaving almost no time for active acquisition. Consider reducing windup_lead_s or increasing sprint duration.', int(windup_lead_s), _windup_fraction * 100, int(duration_s))
...
def _configure_gc_for_sprint() -> dict:
    ...
    try:
        ...
        _gc_configured = True
        return {'delegated_to': 'BlitzGCStrategy', 'blitz_active': _result.get('blitz_active', False), 'freeze_method': _result.get('freeze_method', 'none'), 'blitz_thresholds': _result.get('blitz_thresholds'), 'startup_snapshot_count': _result.get('startup_snapshot_count', 0)}
    except Exception as _exc:
...
def run_pre_sprint_checks() -> bool:
    ...
    if not mlx_cache.MLX_AVAILABLE:
        ...
        try:
            ...
            logger.info(f"[BOOT] MLX buffers: cache={_format_mib(status['cache_limit_bytes'])} wired={_format_mib(status['wired_limit_bytes'])} configured={status['configured']}")
    ...
    if s.swap_used_gib > HARD_BLOCK_SWAP_GIB:
        logger.error('[BOOT] SWAP %.1fGB > %.1fGB — HARD_BLOCK (restart required). Exit 2.', s.swap_used_gib, HARD_BLOCK_SWAP_GIB)
        sys.exit(2)
    elif s.swap_used_gib > CLEAN_SWAP_MAX_GIB:
        logger.warning(f'[BOOT] SWAP {s.swap_used_gib:.1f}GB > {CLEAN_SWAP_MAX_GIB:.1f}GB (diagnostic tier) — doporučuji restart před long run')
    try:
        ...
        if flag_errors:
            logger.error('[SWARM-010] Flag validation failed (%d error(s)). Fix flags above or set --force to bypass.', len(flag_errors))
            sys.exit(2)
    except ImportError:
...
def _extract_nonfeed_debug_fields(nd_raw: Any | None) -> dict[str, Any] | None:
    ...
    if nd_raw is None:
        return None
    return {'domain_detected': getattr(nd_raw, 'domain_detected', False), 'wallet_detected': getattr(nd_raw, 'wallet_detected', False), 'enabled_nonfeed_lanes': getattr(nd_raw, 'enabled_nonfeed_lanes', ()) or (), 'disabled_nonfeed_lanes': getattr(nd_raw, 'disabled_nonfeed_lanes', ()) or (), 'disabled_reasons': getattr(nd_raw, 'disabled_reasons', ()) or (), 'scheduled_nonfeed_lanes': getattr(nd_raw, 'scheduled_nonfeed_lanes', ()) or (), 'hardware_skipped_lanes': getattr(nd_raw, 'hardware_skipped_lanes', ()) or (), 'nonfeed_execution_scheduled': getattr(nd_raw, 'nonfeed_execution_scheduled', False), 'nonfeed_execution_skip_reason': getattr(nd_raw, 'nonfeed_execution_skip_reason', None), 'acquisition_profile': getattr(nd_raw, 'acquisition_profile', 'default'), 'feed_cap_reason': getattr(nd_raw, 'feed_cap_reason', None), 'nonfeed_priority_enabled': getattr(nd_raw, 'nonfeed_priority_enabled', False), 'nonfeed_profile_expected_lanes': getattr(nd_raw, 'nonfeed_profile_expected_lanes', ()) or ()}

async def write_sprint_delta(store: 'DuckDBShadowStore', sprint_id: str, query: str, new_findings: int, dedup_hits: int, ioc_nodes: int, uma_baseline_gib: float, uma_peak_gib: float, synthesis_success: bool, duration_s: float, hits_per_source: dict[str, int], seed_state: Any=None) -> None:
    ...
    try:
        ...
        row = {'sprint_id': sprint_id, 'ts': time.time(), 'query': query, 'duration_s': duration_s, 'new_findings': new_findings, 'dedup_hits': dedup_hits, 'ioc_nodes': ioc_nodes, 'ioc_new_this_sprint': new_findings, 'uma_peak_gib': uma_peak_gib - uma_baseline_gib, 'synthesis_success': synthesis_success, 'findings_per_minute': findings_per_min, 'top_source_type': top_source, 'synthesis_confidence': 1.0 if synthesis_success else 0.0}
        if seed_state is not None:
        ...
        logger.info(f'[TEARDOWN] sprint_delta written: {new_findings} findings, {dedup_hits} dedup hits, UMA delta: {uma_peak_gib - uma_baseline_gib:+.2f}GiB, top_source: {top_source!r}, findings_per_min: {findings_per_min:.2f}')
    except Exception as exc:
...
async def _probe_dns(target_host: str) -> tuple[dict | None, list[str], str]:
    ...
    try:
        await safe_wait_for(asyncio.to_thread(socket.gethostbyname, target_host), timeout=5.0, label='dns_resolve')
...
async def _check_source_availability() -> tuple[dict[str, bool], list[str], str]:
    ...
    src_checks = [('crt.sh', 'https://crt.sh/?q=%.example.com'), ('circl_pdns', 'https://cirolve.circl.lu/api/pdns?q=example.com')]
    try:
        ...
        result = await parallel([check_source(name, url) for name, url in src_checks], policy='collect', ctx='source_availability')
...
def _build_timing_plan(duration_s: float, effective_windup: float, active_budget: float) -> dict:
    ...
    return {'duration': duration_s, 'windup_lead': effective_windup, 'active_budget': active_budget, 'phases': [{'phase': 'WINDUP', 't_start': 0.0, 't_end': effective_windup, 'description': 'seed, bootstrap'}, {'phase': 'ACTIVE', 't_start': effective_windup, 't_end': effective_windup + active_budget, 'description': f'{active_budget:.0f}s available for fetch'}, {'phase': 'SYNTHESIS', 't_start': effective_windup + active_budget, 't_end': duration_s, 'description': 'synthesis + export budget'}]}
...
async def dry_run_sprint(query: str, duration_s: float=300.0) -> None:
    ...
    """
    report: dict[str, Any] = {'target': query, 'duration': duration_s, 'windup_lead': 0.0, 'active_budget': 0.0, 'hermes_available': False, 'uma_available_gib': 0.0, 'sources_online': {}, 'issues': [], 'verdict': 'OK', 'sprint_timing_plan': None}
    all_issues: list[str] = []
    verdict = 'OK'
...
_VERDICT_TABLE: list[tuple[tuple, str]] = [(('aborted', True, 'accepted_findings', lambda v: v > 0), 'ABORTED_PARTIAL'), (('aborted', True), 'ABORTED_HARD'), (('hardware_limited', True), 'HARDWARE_LIMITED'), (('public_backend_degraded', True), 'DEGRADED'), (('accepted_findings', 0, 'public_discovered', lambda v: v > 0), 'NOVELTY'), (('accepted_findings', 0, 'total_pattern_hits', 0), 'DEPLETED'), (('accepted_findings', 0), 'SILENT'), (('dup_rate', lambda v: v > 85), 'NOISE_HEAVY'), (('public_pct', lambda v: v > 60), 'PUBLIC_LED'), (('public_pct', lambda v: v > 25), 'MIXED'), (('feed_fnd', lambda v: v > 0), 'FEED_LED')]
_VERDICT_TEMPLATES: dict[str, str] = {'ABORTED_PARTIAL': '⚠️  ABORTED (partial) — {base}', 'ABORTED_HARD': '⚠️  ABORTED: hard stop, no signal collected', 'HARDWARE_LIMITED': '💾  HARDWARE-LIMITED: swap/memory pressure blocked entry', 'DEGRADED': '🌐  DEGRADED: public backend/network error — check TOR/proxy/config', 'NOVELTY': '🔍  NOVELTY: public found hits, feed accepted nothing', 'DEPLETED': '🗿  DEPLETED: no pattern hits anywhere', 'SILENT': '🤷  SILENT: pattern hits but no accepted findings', 'NOISE_HEAVY': '📦  NOISE-HEAVY: duplicated heavily', 'PUBLIC_LED': '🌐  PUBLIC-LED: public discovery dominated', 'MIXED': '⚖️  MIXED: public contributed meaningfully', 'FEED_LED': '✅  FEED-LED: feed sources strong', 'SIGNAL': '✅  SIGNAL: good feed performance'}
_HINT_TABLE: list[tuple[tuple, str]] = [(('hardware_limited', True), 'hardware memory pressure — free RAM or restart before next run'), (('accepted_findings', 0, 'total_pattern_hits', 0), 'query may be too narrow — broaden terms or switch seed'), (('dup_rate', lambda v: v > 80), 'high dup rate — consider narrowing query scope'), (('public_pct', lambda v: v > 60), 'public discovery effective — let it run longer next time'), (('public_pct', lambda v: v < 10, 'feed_fnd', 0), 'feed yield low — check if sources still alive (urlhaus, threatfox)'), (('public_pct', lambda v: v < 10, 'feed_fnd', lambda v: v > 0), 'feed performing — rely on feed-first, use public as supplemental'), (('public_discovered', lambda v: v > 0, 'public_fetched', 0), 'public discovered but not fetched — check network/TOR'), (('stop_requested', True), 'early stop triggered — lower threshold or widen query')]
...
class VerdictHintInput:
    ...
    def to_dict(self) -> dict:
        return {'aborted': self.aborted, 'accepted_findings': self.accepted_findings, 'dup_rate': self.dup_rate, 'public_pct': self.public_pct, 'feed_fnd': self.feed_fnd, 'hardware_limited': self.hardware_limited, 'public_backend_degraded': self.public_backend_degraded, 'public_discovered': self.public_discovered, 'total_pattern_hits': self.total_pattern_hits, 'public_fetched': self.public_fetched, 'stop_requested': self.stop_requested}
...
class _CheckpointPriority:
    ...
    DEPLETED = 14
_CHECKPOINT_PRIORITY_MAP: dict[int, tuple[str, str]] = {_CheckpointPriority.SIGNAL_REACHES_FINDINGS: ('signal_reaches_findings', 'static'), _CheckpointPriority.PRE_ACTIVE_MEMORY_STARVATION: ('pre_active_memory_starvation', 'static'), _CheckpointPriority.SURVIVAL_ACTIVE_MINIMAL: ('survival_active_minimal', 'evidence_note'), _CheckpointPriority.HARDWARE_LIMITED_SMOKE: ('hardware_limited_smoke', 'evidence_note'), _CheckpointPriority.PUBLIC_BACKEND_DEGRADED: ('public_backend_degraded', 'public_error_degraded'), _CheckpointPriority.DEGRADED_PUBLIC_BLOCKER: ('degraded_public_blocker', 'public_error_blocked'), _CheckpointPriority.MEANINGFUL_EMPTY_RUN: ('meaningful_empty_run', 'static'), _CheckpointPriority.FEED_INGRESS_BLOCKER: ('feed_ingress_blocker', 'public_discovered'), _CheckpointPriority.FEED_SOURCE_INACCESSIBLE: ('feed_source_inaccessible', 'static'), _CheckpointPriority.SHORT_SIGNAL: ('short_signal', 'static'), _CheckpointPriority.TRUE_DEPLETED_QUERY: ('true_depleted_query', 'static'), _CheckpointPriority.CROSS_BRANCH_SOURCE_INACCESSIBLE: ('cross_branch_source_inaccessible', 'static'), _CheckpointPriority.WINDUP_EXPORT_FAIL_SOFT: ('windup_export_fail_soft', 'evidence_note'), _CheckpointPriority.DEPLETED: ('depleted', 'static')}
_CHECKPOINT_REASON_TEMPLATES: dict[int, str] = {_CheckpointPriority.SIGNAL_REACHES_FINDINGS: 'signal_reaches_findings', _CheckpointPriority.PRE_ACTIVE_MEMORY_STARVATION: 'pre_active_memory_starvation', _CheckpointPriority.MEANINGFUL_EMPTY_RUN: 'meaningful_empty_run', _CheckpointPriority.FEED_SOURCE_INACCESSIBLE: 'feed_source_inaccessible', _CheckpointPriority.SHORT_SIGNAL: 'short_signal_no_findings', _CheckpointPriority.TRUE_DEPLETED_QUERY: 'true_depleted_query:hits_without_acceptance', _CheckpointPriority.CROSS_BRANCH_SOURCE_INACCESSIBLE: 'cross_branch_source_inaccessible', _CheckpointPriority.DEPLETED: 'depleted_no_pattern_hits'}
...
def _compute_export_finish_status(final_phase: str, accepted_findings: int, aborted: bool) -> str:
    """
...
def _extract_result_fields(result: Any, export_finish_status: str | None) -> dict[str, Any]:
    ...
    """
    return {'cycles_started': result.cycles_started, 'cycles_completed': result.cycles_completed, 'pre_loop_elapsed_s': result.pre_loop_elapsed_s, 'pre_loop_blocker_reason': result.pre_loop_blocker_reason, 'pre_active_starvation': result.pre_active_starved, 'export_finish_layer_status': export_finish_status, 'public_error': result.public_error, 'ct_log_discovered': result.ct_log_discovered, 'ct_log_stored': result.ct_log_stored, 'ct_log_accepted_findings': result.ct_log_accepted_findings, 'cc_archive_injected': result.cc_archive_injected, 'academic_findings_count': result.academic_findings_count}
...
async def _run_sprint_boot(ctx: SprintRunContext, query: str, duration_s: float, windup_lead_s: float | None, flags: SprintFlags | None, force: bool, aggressive_mode: bool, resume: bool, prng_seed: int | None, replay_seed: int | None) -> None:
    """
    ...
    _effective_windup_s, _uma_pre_sprint = _run_sprint_preflight_guards(logger=logger, duration_s=duration_s, windup_lead_s=windup_lead_s, flags=flags, force=force, early_config=None)
    ...
    if replay_seed is not None:
        ...
        logger.info('[ULTIMATE-001] Replay mode: seed=%d, tot_iv=%s', ctx.seed_state.prng_seed, ctx.seed_state.tot_iv[:8])
    elif prng_seed is not None:
        ctx.seed_state = SprintSeedState.generate(query=query, explicit_seed=prng_seed)
        logger.info('[ULTIMATE-001] Using explicit seed=%d, tot_iv=%s', ctx.seed_state.prng_seed, ctx.seed_state.tot_iv[:8])
    else:
        ctx.seed_state = SprintSeedState.generate(query=query)
        logger.info('[ULTIMATE-001] Generated seed=%d, tot_iv=%s (use --seed %d for deterministic replay)', ctx.seed_state.prng_seed, ctx.seed_state.tot_iv[:8], ctx.seed_state.prng_seed)
    global _current_sprint_seed_state
    ...
    with contextlib.suppress(asyncio.CancelledError):
        _init_results = await parallel_ok(_duckdb_init_coro(ctx.store, logger), _cb_reset_coro, label='pre_init')
    if _init_results:
    ...
    if ctx.duckdb_init_ok and resume:
        ctx.resume_from, ctx.resume_step = await _attempt_tot_recovery(ctx.store, ctx.query_hash, logger)

async def _attempt_tot_recovery(store: Any, query_hash: str, logger: logging.Logger) -> tuple[dict | None, int]:
    """
    ...
    try:
        ...
        for _nid, _ndata in _raw_nodes.items():
            try:
                _nodes[_nid] = ThoughtNode(node_id=_ndata.get('node_id', _nid), thought=_ndata.get('thought', ''), value_estimate=_ndata.get('value_estimate', 0.0), parent=_ndata.get('parent'), children=_ndata.get('children', []), visited=_ndata.get('visited', False), expanded=_ndata.get('expanded', False), depth=_ndata.get('depth', 0), cost=_ndata.get('cost', 0.0), uncertainty=_ndata.get('uncertainty', 0.0))
            except Exception:
                pass
        if _nodes:
            logger.warning('[UNIFIED-006] 🔄 RESUMING ToT from checkpoint: orphan_sprint=%s step=%d nodes=%d', _orphan_sprint_id[:12], _orphan_step, len(_nodes))
...
async def _run_sprint_execute(ctx: SprintRunContext, query: str, duration_s: float, aggressive_mode: bool, deep_research: bool, extreme_mode: bool, acquisition_profile: str | None, flags: SprintFlags | None, rl_train_mode: bool, ui_mode: bool, export_dir: str) -> None:
    """
    ...
    """
    await _execute_scheduler_setup(ctx=ctx, query=query, duration_s=duration_s, aggressive_mode=aggressive_mode, deep_research=deep_research, extreme_mode=extreme_mode, acquisition_profile=acquisition_profile, flags=flags, rl_train_mode=rl_train_mode, export_dir=export_dir)
...
async def _execute_scheduler_setup(ctx: SprintRunContext, query: str, duration_s: float, aggressive_mode: bool, deep_research: bool, extreme_mode: bool, acquisition_profile: str | None, flags: SprintFlags | None, rl_train_mode: bool, export_dir: str) -> None:
    ...
    config = SprintSchedulerConfig(sprint_duration_s=float(duration_s), windup_lead_s=ctx.effective_windup_s, export_enabled=True, export_dir=export_dir, aggressive_mode=aggressive_mode, branch_timeout_budget_s=8.0 if aggressive_mode else 0.0, acquisition_profile=acquisition_profile, deep_research_enabled=deep_research, extreme_mode=extreme_mode)
    ...
    await _init.run(query, _wall_clock_start, ctx=None, cancel_event=ctx.cancel_event, flags=flags, sprint_id=ctx.sprint_id, sprint_duration_s=float(duration_s), windup_lead_s=ctx.effective_windup_s, duckdb_store=ctx.store, rl_train_mode=rl_train_mode, logger=logger, resume_from=ctx.resume_from, resume_step=ctx.resume_step, query_hash=ctx.query_hash)
...
async def _run_sprint_windup(ctx: SprintRunContext, query: str, duration_s: float, export_dir: str, deep_probe_enabled: bool) -> None:
    """
    ...
    report_dict = _build_report_dict(inp=ReportBuildInput(query=query, duration_s=duration_s, actual_duration=actual_duration, feed_fnd=metrics['feed_fnd'], dup_rate=metrics['dup_rate'], findings_per_min=metrics['findings_per_min'], public_pct=metrics['public_pct'], src_mix_str=metrics['src_mix_str'], verdict=classifications['verdict'], next_hint=classifications['next_hint'], phase_durations=_phase_durations, runtime_truth=classifications['runtime_truth'], timing_truth=classifications['timing_truth'], runtime_truth_level=classifications['runtime_truth_level'], observed_run_tuple=classifications['observed_run_tuple'], ckpt_category=classifications['ckpt_category'], checkpoint_zero_reason=classifications['checkpoint_zero_reason'], export_finish_status=classifications['export_finish_status'], uma_peak_gib=classifications['uma_peak_gib'], ctx=ctx, result=result, acq_payload_filtered=_acq_payload_filtered))
    ctx.report_path.write_bytes(_serialize_report(report_dict))
    logger.info(f'[REPORT] {ctx.report_path}')
    await _windup_phase5_export(ctx, result, _phase_durations, _acq_payload, metrics['src_mix_str'], classifications, deep_probe_enabled, query, actual_duration)
...
async def _windup_phase1_early(ctx: SprintRunContext, query: str, actual_duration: float) -> None:
    ...
    if ctx.evidence_log is not None:
        with _fail_safe_async('debug', 'evidence_log.windup_event'):
            ctx.evidence_log.create_event(event_type='observation', payload={'phase': 'WINDUP', 'sprint_id': ctx.sprint_id, 'query': query}, confidence=1.0)
    ...
    await write_sprint_delta(store=ctx.store, sprint_id=ctx.sprint_id, query=query, new_findings=result.accepted_findings, dedup_hits=result.duplicate_entry_hashes_skipped, ioc_nodes=result.unique_entry_hashes_seen, uma_baseline_gib=ctx.uma_baseline_gib, uma_peak_gib=uma_peak_gib, synthesis_success=result.accepted_findings > 0, duration_s=actual_duration, hits_per_source=result.hits_per_source, seed_state=ctx.seed_state)
    if ctx.evidence_log is not None:
    ...
    if ctx.evidence_log is not None:
        with _fail_safe_async('debug', 'evidence_log.teardown_event'):
            ctx.evidence_log.create_event(event_type='observation', payload={'phase': 'TEARDOWN', 'sprint_id': ctx.sprint_id, 'actual_duration_s': round(actual_duration, 2)}, confidence=1.0)
...
def _windup_phase3_compute_metrics(ctx: SprintRunContext, result: Any, actual_duration: float) -> dict:
    ...
    return {'findings_per_min': findings_per_min, 'dup_rate': dup_rate, 'feed_fnd': feed_fnd, 'public_pct': public_pct, 'src_mix_str': src_mix_str}
...
def _windup_phase3_compute_classifications(ctx: SprintRunContext, result: Any, metrics: dict, query: str, duration_s: float) -> dict:
    ...
    timing_truth = {'requested_duration_s': duration_s, 'windup_lead_s': ctx.scheduler._config.windup_lead_s, 'time_to_windup_s': round(time_to_windup_s, 2), 'time_to_teardown_s': round(ctx.phase_times['TEARDOWN'] - ctx.phase_times['BOOT'], 2), 'active_window_budget_s': round(duration_s - ctx.effective_windup_s, 2), 'windup_lead_observed_s': round(windup_lead_observed_s, 2), 'pre_scheduler_boot_s': round(pre_scheduler_boot_s, 2), 'scheduler_wall_s': round(scheduler_wall_s, 2), 'scheduler_returned_phase': 'ACTIVE' if result.entered_active_at_monotonic else 'entry_only', 'entered_active_truth': result.entered_active_at_monotonic is not None, 'first_cycle_truth': result.first_cycle_started_at_monotonic is not None, 'pre_active_starvation': result.pre_active_starved, 'pre_active_blocker': result.pre_loop_blocker_reason or None}
    _inline_hardware_limited = result.accepted_findings == 0 and result.total_pattern_hits == 0 and (result.cycles_started == 0) and (ctx.swap_detected_pre or ctx.uma_state_pre in ('critical', 'emergency'))
    _is_hardware_limited = not metrics.get('is_meaningful', False) and result.cycles_started == 0 and (ctx.swap_detected_pre or ctx.uma_state_pre in ('critical', 'emergency'))
    _is_pre_active_mem_starved = not metrics.get('is_meaningful', False) and result.cycles_started == 0 and (result.entered_active_at_monotonic is not None) and (ctx.swap_detected_pre or ctx.uma_state_pre in ('critical', 'emergency', 'warn'))
    verdict, next_hint = _compute_verdict_and_hint(inp=VerdictHintInput(aborted=result.aborted, accepted_findings=result.accepted_findings, dup_rate=metrics['dup_rate'], public_pct=metrics['public_pct'], feed_fnd=metrics['feed_fnd'], hardware_limited=_inline_hardware_limited, public_backend_degraded=result.public_backend_degraded, public_discovered=result.public_discovered, total_pattern_hits=result.total_pattern_hits, public_fetched=result.public_fetched, stop_requested=result.stop_requested))
    _total_ct = (result.lane_ct_accepted_findings or 0) + (result.ct_log_stored or 0)
    runtime_truth = _runtime_truth(inp=RuntimeTruthInput(actual_duration_s=actual_duration, query=query, duration_s=duration_s, cycles_completed=result.cycles_completed, cycles_started=result.cycles_started, accepted_findings=result.accepted_findings, total_pattern_hits=result.total_pattern_hits, public_accepted_findings=result.public_accepted_findings, feed_findings=metrics['feed_fnd'], ct_findings=_total_ct, swap_detected=ctx.swap_detected_pre, uma_state=ctx.uma_state_pre, branch_timeout_count=result.branch_timeout_count, public_branch_timed_out=result.public_branch_timed_out, ct_branch_timed_out=result.ct_branch_timed_out))
    ...
    runtime_truth_level = 'active' if is_meaningful and result.accepted_findings > 0 else 'pre_active_memory_starvation' if _is_pre_active_mem_starved else 'survival_active_minimal' if is_meaningful and ctx.uma_state_pre in ('warn', 'critical', 'emergency') else 'hardware_limited_smoke' if _is_hardware_limited else 'short_signal' if is_meaningful and result.total_pattern_hits > 0 else 'meaningful_empty' if is_meaningful else 'smoke'
    observed_run_tuple = (query[:40] if len(query) > 40 else query, round(actual_duration, 1), result.cycles_completed, metrics['src_mix_str'], runtime_truth_level)
    _public_backend = result.public_backend_degraded
    _feed_zero_check = result.accepted_findings == 0 and metrics['feed_fnd'] == 0
    _cross_branch_fail_check = result.accepted_findings == 0 and result.total_pattern_hits > 0 and (not _public_backend) and (not result.public_error)
    _ckpt_category, _checkpoint_zero_reason = _compute_checkpoint_category(inp=CheckpointInput(accepted_findings=result.accepted_findings, total_pattern_hits=result.total_pattern_hits, public_error=result.public_error, public_discovered=result.public_discovered, public_backend=_public_backend, feed_zero_check=_feed_zero_check, cross_branch_fail_check=_cross_branch_fail_check, is_pre_active_mem_starved=_is_pre_active_mem_starved, is_hardware_limited=_is_hardware_limited, is_meaningful=is_meaningful, uma_state_pre=ctx.uma_state_pre, feed_fnd=metrics['feed_fnd'], phase_times=ctx.phase_times), evidence_note=evidence_note)
    _export_finish_status = _compute_export_finish_status(final_phase=result.final_phase, accepted_findings=result.accepted_findings, aborted=result.aborted)
    uma_peak_gib = sample_uma_status().system_used_gib
    return {'verdict': verdict, 'next_hint': next_hint, 'runtime_truth': runtime_truth, 'timing_truth': timing_truth, 'runtime_truth_level': runtime_truth_level, 'observed_run_tuple': observed_run_tuple, 'ckpt_category': _ckpt_category, 'checkpoint_zero_reason': _checkpoint_zero_reason, 'export_finish_status': _export_finish_status, 'uma_peak_gib': uma_peak_gib, 'is_meaningful': is_meaningful, 'evidence_note': evidence_note}
...
async def _windup_phase5_export(ctx: SprintRunContext, result: Any, phase_durations: dict, acq_payload: dict, src_mix_str: str, classifications: dict, deep_probe_enabled: bool, query: str, actual_duration: float) -> None:
    ...
    with _fail_safe_async('warning', 'sprint_exporter'):
        ...
        handoff = _build_export_handoff(inp=ExportHandoffInput(query=query, duration_s=ctx.duration_s, actual_duration=actual_duration, runtime_truth=runtime_truth, timing_truth=timing_truth, runtime_truth_level=runtime_truth_level, observed_run_tuple=observed_run_tuple, src_mix_str=src_mix_str, ckpt_category=ckpt_category, checkpoint_zero_reason=checkpoint_zero_reason, export_finish_status=export_finish_status, phase_durations=phase_durations, ctx=ctx, result=result, top_seed_nodes=top_seed_nodes, live_feed_urls=ctx.live_feed_urls, acq_payload=acq_payload))
        _elog_instance = ctx.scheduler._evidence_log.value if ctx.scheduler._evidence_log else None
        export_result = await export_sprint(store=ctx.store, handoff=handoff, sprint_id=ctx.sprint_id, evidence_log=_elog_instance)
        ...
        if deep_probe_enabled:
            with _fail_safe_async('warning', 'deep_probe'):
                ...
                probe_result = await run_deep_probe_if_enabled(query=query, store=ctx.store, deep_probe_enabled=True)
                if probe_result:
...
def _build_report_dict(inp: ReportBuildInput) -> dict:
    ...
    ctx = inp.ctx
    result = inp.result
    result_dict = {'sprint_id': ctx.sprint_id, 'query': inp.query, 'duration_s': inp.duration_s, 'actual_duration_s': inp.actual_duration, 'accepted_findings': result.accepted_findings, 'feed_findings': inp.feed_fnd, 'public_accepted_findings': result.public_accepted_findings, 'public_discovered': result.public_discovered, 'public_fetched': result.public_fetched, 'public_matched_patterns': result.public_matched_patterns, 'public_stored_findings': result.public_stored_findings, 'public_error': result.public_error, 'ct_log_discovered': result.ct_log_discovered, 'ct_log_stored': result.ct_log_stored, 'ct_log_accepted_findings': result.ct_log_accepted_findings, 'ct_log_error': result.ct_log_error, 'cycles_completed': result.cycles_completed, 'cycles_started': result.cycles_started, 'unique_entry_hashes_seen': result.unique_entry_hashes_seen, 'duplicate_entry_hashes_skipped': result.duplicate_entry_hashes_skipped, 'total_pattern_hits': result.total_pattern_hits, 'dup_rate_pct': round(inp.dup_rate, 2), 'findings_per_min': round(inp.findings_per_min, 2), 'final_phase': result.final_phase, 'aborted': result.aborted, 'abort_reason': result.abort_reason, 'stop_requested': result.stop_requested, 'entries_per_source': result.entries_per_source, 'hits_per_source': result.hits_per_source, 'export_paths': result.export_paths, 'uma_peak_gib': inp.uma_peak_gib - ctx.uma_baseline_gib, 'synthesis_success': result.accepted_findings > 0, 'verdict': inp.verdict, 'next_hint': inp.next_hint, 'phase_timing': inp.phase_durations, 'runtime_truth': inp.runtime_truth, 'correlation_summary': ctx.intel.get('correlation'), 'hypothesis_pack_summary': ctx.intel.get('hypothesis_pack'), 'signal_path': ctx.intel.get('signal_path'), 'feed_verdict': ctx.intel.get('feed_verdict'), 'public_verdict': ctx.intel.get('public_verdict'), 'branch_value': ctx.intel.get('branch_value'), 'sprint_verdict': ctx.intel.get('sprint_verdict'), 'execution_context': {'query': inp.query, 'requested_duration_s': inp.duration_s, 'actual_duration_s': round(inp.actual_duration, 2), 'source_count': len(ctx.live_feed_urls), 'sources': ctx.live_feed_urls, 'platform': _PLATFORM_INFO.copy(), 'report_path': str(ctx.report_path), 'git_snapshot': 'unknown', 'export_dir': str(ctx.report_path.parent)}}
    intel = ctx.intel
    crs = {'meaningful': inp.runtime_truth['is_meaningful'], 'primary_signal': inp.runtime_truth['primary_signal_source'], 'posture': (intel.get('sprint_verdict') or {}).get('posture', 'unknown'), 'dominant_signal_path': (intel.get('signal_path') or {}).get('dominant_signal_path', 'unknown'), 'corroborated': (intel.get('signal_path') or {}).get('is_corroborated', False), 'is_noisy': (intel.get('signal_path') or {}).get('is_noisy', False), 'next_pivot': (intel.get('signal_path') or {}).get('next_pivot_recommendation', 'unknown'), 'branch_verdict': (intel.get('branch_value') or {}).get('branch_verdict', 'unknown'), 'risk_score': (intel.get('correlation') or {}).get('risk_score', 0.0), 'hypothesis_count': (intel.get('hypothesis_pack') or {}).get('hypothesis_count', 0), 'first_action': (intel.get('sprint_verdict') or {}).get('first_action', ''), 'confidence': (intel.get('sprint_verdict') or {}).get('confidence', ''), 'runtime_truth_level': inp.runtime_truth_level, 'checkpoint_zero_category': inp.ckpt_category, 'checkpoint_zero_reason': inp.checkpoint_zero_reason, 'observed_run_tuple': inp.observed_run_tuple, 'canonical_sprint_owner': 'runtime.sprint_entrypoint.run_sprint', 'canonical_path_used': 'run_sprint', 'effective_source_mix': inp.src_mix_str, 'effective_parallelism': len(ctx.live_feed_urls), 'effective_timeouts': {}, 'active_iteration_count': result.cycles_completed, **_extract_result_fields(result, inp.export_finish_status), 'timing_truth': inp.timing_truth, 'early_exit_class': getattr(result, 'early_exit_class', ''), 'early_exit_reason': getattr(result, 'early_exit_reason', ''), 'requested_duration_s': inp.duration_s, 'actual_duration_s': round(inp.actual_duration, 2), 'elapsed_pct': round(inp.actual_duration / inp.duration_s * 100, 1) if inp.duration_s > 0 else 0.0, 'active_window_budget_s': inp.timing_truth['active_window_budget_s'], 'active_window_elapsed_s': inp.timing_truth['time_to_windup_s'], 'governor_uma_state': getattr(result, 'governor_uma_state', ''), 'governor_system_used_gib': getattr(result, 'governor_system_used_gib', 0.0), 'governor_swap_detected': getattr(result, 'governor_swap_detected', False), 'governor_io_only': getattr(result, 'governor_io_only', False)}
    ...
    try:
        ...
        result_dict['rust_extensions'] = {}
    if ctx.evidence_log is not None:
        result_dict['evidence_manifest'] = {'total_count': ctx.evidence_log.size, 'ram_size': ctx.evidence_log.ram_size, 'persist_path': str(ctx.evidence_log.persist_path) if ctx.evidence_log.persist_path else None}
    else:
...
def _build_export_handoff(inp: ExportHandoffInput) -> Any:
    ...
    result = inp.result
    intel = ctx.intel
    scorecard = {'synthesis_engine_used': 'hermes3', 'gnn_predicted_links': 0, 'top_graph_nodes': inp.top_seed_nodes, 'phase_duration_seconds': inp.phase_durations, 'runtime_accepted_findings': (result.accepted_findings or 0) + (result.public_accepted_findings or 0), 'findings_per_minute': round(((result.accepted_findings or 0) + (result.public_accepted_findings or 0)) / (inp.actual_duration / 60.0), 2) if inp.actual_duration > 0 else 0.0, 'identity_candidates_found': result.identity_candidates_found, 'identity_findings_produced': result.identity_findings_produced, **inp.acq_payload}
    canonical_run_summary = {'meaningful': inp.runtime_truth['is_meaningful'], 'primary_signal': inp.runtime_truth['primary_signal_source'], 'posture': (intel.get('sprint_verdict') or {}).get('posture', 'unknown'), 'dominant_signal_path': (intel.get('signal_path') or {}).get('dominant_signal_path', 'unknown'), 'corroborated': (intel.get('signal_path') or {}).get('is_corroborated', False), 'is_noisy': (intel.get('signal_path') or {}).get('is_noisy', False), 'next_pivot': (intel.get('signal_path') or {}).get('next_pivot_recommendation', 'unknown'), 'branch_verdict': (intel.get('branch_value') or {}).get('branch_verdict', 'unknown'), 'risk_score': (intel.get('correlation') or {}).get('risk_score', 0.0), 'hypothesis_count': (intel.get('hypothesis_pack') or {}).get('hypothesis_count', 0), 'first_action': (intel.get('sprint_verdict') or {}).get('first_action', ''), 'confidence': (intel.get('sprint_verdict') or {}).get('confidence', ''), 'runtime_truth_level': inp.runtime_truth_level, 'checkpoint_zero_category': inp.ckpt_category, 'checkpoint_zero_reason': inp.checkpoint_zero_reason, 'observed_run_tuple': inp.observed_run_tuple, 'canonical_sprint_owner': 'runtime.sprint_entrypoint.run_sprint', 'canonical_path_used': 'run_sprint', 'effective_source_mix': inp.src_mix_str, 'effective_parallelism': len(inp.live_feed_urls), 'effective_timeouts': {}, 'active_iteration_count': result.cycles_completed, **_extract_result_fields(result, inp.export_finish_status), 'timing_truth': inp.timing_truth, **inp.acq_payload}
    return ExportHandoff(sprint_id=ctx.sprint_id, scorecard=scorecard, top_nodes=inp.top_seed_nodes, phase_durations=inp.phase_durations, runtime_truth=inp.runtime_truth, execution_context={'query': inp.query, 'requested_duration_s': inp.duration_s, 'actual_duration_s': round(inp.actual_duration, 2), 'source_count': len(inp.live_feed_urls), 'sources': inp.live_feed_urls, 'platform': {'python_version': _PLATFORM_INFO['python_version'], 'macos_version': _PLATFORM_INFO['macos_version']}, 'report_path': str(ctx.report_path), 'git_snapshot': 'unknown', 'export_dir': str(ctx.report_path.parent)}, canonical_run_summary=canonical_run_summary, synthesis_outcome_payload=None, sprint_verdict=intel.get('sprint_verdict'), analyst_brief=ctx.scheduler.get_analyst_brief() if ctx.scheduler else None, timer_events=getattr(result, 'timer_events', None), uncertainty_flags=getattr(result, 'synthesis_uncertainty_flags', None) or None)
...
async def _teardown_transports(ctx: SprintRunContext) -> None:
    ...
    with _fail_safe_async('debug', 'parallel_close_async.transports'):
        ...
        _transport_close_errors = await parallel_close_async([('httpx', close_httpx_client_async), ('curl_cffi', close_curl_cffi_sessions_async), ('public_fetcher', close_public_fetcher_sessions_async), ('aiohttp', close_aiohttp_session_async)], concurrency=4, ctx='teardown.transports')
...
async def _teardown_cleanup(ctx: SprintRunContext) -> None:
    ...
    if ctx.store is not None:
        with _fail_safe_async('debug', 'tot_checkpoint.cleanup'):
            ...
            _cleanup_ckpt = TransactionalToTCheckpointer(sprint_id=ctx.sprint_id, duckdb_store=ctx.store, interval_s=30.0, lmdb_incremental=True, fs_fallback=True)
...
async def run_sprint(query: str, duration_s: float=1800.0, export_dir: str=str(Path.home() / '.hledac' / 'reports'), aggressive_mode: bool=False, deep_probe_enabled: bool=False, deep_research: bool=False, extreme_mode: bool=False, _no_communication: bool=False, ui_mode: bool=False, windup_lead_s: float | None=None, acquisition_profile: str | None=None, rl_train_mode: bool=False, force: bool=False, flags: SprintFlags | None=None, shutdown_event: asyncio.Event | None=None, resume: bool=True, prng_seed: int | None=None, replay_seed: int | None=None, warc_dir: str | None=None) -> None:
    """
    ...
    try:
        ...
        await _run_sprint_boot(ctx=ctx, query=query, duration_s=duration_s, windup_lead_s=windup_lead_s, flags=flags, force=force, aggressive_mode=aggressive_mode, resume=resume, prng_seed=prng_seed, replay_seed=replay_seed)
        await _run_sprint_execute(ctx=ctx, query=query, duration_s=duration_s, aggressive_mode=aggressive_mode, deep_research=deep_research, extreme_mode=extreme_mode, acquisition_profile=acquisition_profile, flags=flags, rl_train_mode=rl_train_mode, ui_mode=ui_mode, export_dir=export_dir)
        await _run_sprint_windup(ctx=ctx, query=query, duration_s=duration_s, export_dir=export_dir, deep_probe_enabled=deep_probe_enabled)
    except asyncio.CancelledError:
...
class _SignalHandlerContext:
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, shutdown_event: asyncio.Event) -> None:
        self.loop = loop
...
def _install_signal_handler_for_loop(loop: asyncio.AbstractEventLoop, shutdown_event: asyncio.Event) -> Callable[[], None]:
    """
...
async def _cancel_all_tasks(timeout_s: float=5.0) -> None:
    ...
    try:
        ...
        stragglers = []
    for t in stragglers:
        logger.warning('[SHUTDOWN] Task %s did not drain in %ss — abandoning', t.get_name(), timeout_s)
...
def _run_sprint_loop(args: argparse.Namespace) -> None:
    ...
    if _build_runtime is None:
        from hledac.universal._core.composition_root import build_runtime as _br, run_runtime as _rr
        _build_runtime = _br
        _run_runtime = _rr
    sprint_flags = SprintFlags(force=args.force, no_communication=getattr(args, 'no_communication', False), no_stealth=getattr(args, 'no_stealth', False), no_ghost=getattr(args, 'no_ghost', False), no_coordination=getattr(args, 'no_coordination', False), production=getattr(args, 'production', False), hermes_force=getattr(args, 'force_hermes', False), blitz_mode=args.duration <= 1800)
    loop, sprint_task, _shutdown_event, restore_signals = _build_runtime(query=args.query, duration_s=float(args.duration), export_dir=args.export_dir, aggressive_mode=args.aggressive, deep_probe_enabled=args.deep_probe, deep_research=args.deep_research, extreme_mode=args.extreme, no_communication=getattr(args, 'no_communication', False), ui_mode=getattr(args, 'ui', False), windup_lead_s=None, acquisition_profile=args.acquisition_profile, rl_train_mode=args.rl_train, force=args.force, flags=sprint_flags, prng_seed=getattr(args, 'seed', None), replay_seed=getattr(args, 'replay_seed', None), warc_dir=getattr(args, 'warc_dir', None))
    try:
...
def _main_dispatch() -> None:
    ...
    parser.add_argument('--duration', type=int, default=1800, help='Sprint duration in seconds (default: 1800 = 30min)')
    parser.add_argument('--export-dir', type=str, default=str(Path.home() / '.hledac' / 'reports'))
    parser.add_argument('--ct-pivot', type=str, default=None, help='Run CT log pivot for a domain via crt.sh')
    parser.add_argument('--pivot', type=str, default=None, help='Sprint 8SB: semantic pivot — find similar findings via ANN search')
    parser.add_argument('--pivot-k', type=int, default=10, help='Number of results for --pivot (default: 10)')
    parser.add_argument('--aggressive', action='store_true', help='Sprint F195B: Enable aggressive mode with 8s branch budgets')
    parser.add_argument('--deep-probe', action='store_true', help='Run deep probe research post-sprint (deep web, S3 buckets, IPFS)')
    parser.add_argument('--deep-research', action='store_true', help='F11: Run enhanced deep research advisory post-sprint')
    parser.add_argument('--extreme', action='store_true', help='F11: Enable EXHAUSTIVE depth for deep research (implies --deep-research)')
    parser.add_argument('--force', action='store_true', help='F221-ABORT: Override the pre-flight guard that aborts sprints whose active-window budget would be below MIN_ACTIVE_WINDOW_S=30s. Emits a [F221-FORCED] warning instead of exiting with code 2. Use only for explicit dry-runs / smoke tests where zero evidence is acceptable.')
    parser.add_argument('--acquisition-profile', type=str, default='default', choices=['default', 'nonfeed_diagnostic', 'deep_osint_m1'], help='F216B/F251D: Acquisition runtime profile (default | nonfeed_diagnostic | deep_osint_m1)')
    parser.add_argument('--rl-train', action='store_true', help='RL F257: Enable QMIX training mode (updates Q-network weights every 10 sprints). Default is inference-only after 124 sprint warmup.')
    parser.add_argument('--rl-no-train', action='store_true', help='RL F261QMIX: Force inference-only mode (overrides HLEDAC_ENABLE_RL=1). Use for production runs where Q-network must NOT be updated.')
    parser.add_argument('--rl-train-interval', type=int, default=None, help='RL F261QMIX: Override HLEDAC_RL_TRAIN_INTERVAL (default 10 sprints per QMIX training step).')
    parser.add_argument('--no-communication', action='store_true', help='F26X-3: Skip CommunicationLayer injection in run_sprint(). Default ON, mirroring --no-coordination opt-out contract from F26X-2.')
    parser.add_argument('--no-ghost', action='store_true', help='F260: Skip GhostLayer injection in run_sprint(). Default ON, mirroring --no-coordination/--no-communication opt-out contract.')
    parser.add_argument('--no-stealth', action='store_true', help='F260: Skip StealthLayer injection in run_sprint(). Default ON, mirroring --no-coordination/--no-communication opt-out contract.')
    parser.add_argument('--list-sources', action='store_true', help='F270: Print DeepSourceRegistry catalog (curated beyond-surface sources) and exit.')
    parser.add_argument('--tier', type=str, default=None, choices=['surface', 'dark', 'archive', 'p2p', 'academic'], help='F270: Filter --list-sources output by source tier.')
    parser.add_argument('--production', action='store_true', help='F272B: Production mode. Sprint aborts with exit code 2 if the pre-run health check finds fetch_coordinator_ok=False (not_initialized). Default OFF: advisory-degraded mode continues and only logs the warning. Use for CI / orchestration where a degraded run is worse than no run.')
    parser.add_argument('--force-hermes', action='store_true', help="F273D: Force-load Hermes3 model at sprint start even if HLEDAC_ENABLE_HERMES_SYNTHESIS != '1'. Overrides the lazy hermes gate. Result exposes hermes_model_loaded (bool) and hermes_load_reason (str) so you can verify whether the model is actually resident. M1 8GB: Hermes-3-3B-4bit ~2GB -- use only when you need synthesis output, not for routine OSINT sprints.")
    parser.add_argument('--seed', type=int, default=None, help='ULTIMATE-001: Explicit 64-bit PRNG seed for deterministic cognitive replay. If not provided, a random seed is generated and logged. Use this with --replay-seed <seed> --warc-dir <path> to reconstruct the identical ToT tree and synthesis path for forensic verification.')
    parser.add_argument('--replay-seed', type=int, default=None, help='ULTIMATE-001: Replay mode. Takes a previously captured seed to reconstruct the exact cognitive path from a prior sprint. Requires --warc-dir pointing to the WARC archive captured during the original run.')
    parser.add_argument('--warc-dir', type=str, default=None, help='ULTIMATE-001: WARC archive directory for replay mode. When combined with --replay-seed, the system replays HTTP responses from the WARC file instead of fetching live, enabling deterministic forensic replay.')
    ...
    if args.list_sources:
        ...
        for src in sorted(sources, key=lambda s: (s.source_tier, s.name)):
            print(f'{src.source_id:<18} {src.source_tier:<10} {src.transport_required:<10} {src.data_type:<14} {src.reliability:<5.2f} {src.name[:28]:<30} {src.base_url}')
        print('-' * 110)
