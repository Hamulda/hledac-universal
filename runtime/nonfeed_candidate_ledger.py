...
# F289: Max conceptual domain candidates generated by MLX for no-domain queries
...
# F289: Fail-safe conceptual domain generator for no-domain queries
...
async def _generate_conceptual_domains_mlx(
    ...
    try:
        ...
        system_prompt = (
            ...
            "generate up to 5 plausible domain names that might be relevant for investigating that topic. "
            "Focus on common infrastructure patterns: threat actor leak sites, "
            "ransomware negotiation portals, breach forums, dark web marketplaces, "
            "or public reconnaissance platforms. "
            "Return ONLY a JSON list of domain strings, nothing else. "
            'Example: ["ransomware-leak.site", "breach-forum.onion", "osint-scanner.io"]'
        )
        ...
        try:
            ...
            array_match = _re.search(r"\[.*\]", response, _re.DOTALL)
            if not array_match:
                return []
            try:
        ...
        # Fail-safe: never raise, return empty
        return []


async def generate_conceptual_domain_candidates(
    query: str,
) -> list[DomainCandidate]:
    """
    ...
    Returns:
        List of DomainCandidate (may be empty)
    """
    # Fast path: regex already found candidates — no need for MLX
    candidates = extract_domain_candidates_from_text(query, source_family=FAMILY_PUBLIC)
    if candidates:
        return []
    # Slow path: MLX generates candidates from concept
