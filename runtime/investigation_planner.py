...
def build_planner_state_from_report(report: dict) -> dict:
    ...
    if not seed_available and (domains or ips or urls or hashes or cves):
        seed_available = True
    seed_context: dict = {'available': seed_available, 'source': 'acquisition_report' if acq else 'top_level', 'iocs': seed_iocs, 'domains': domains, 'ips': ips, 'urls': urls, 'hashes': hashes, 'cves': cves}
