...
def _is_feed_dominant(source_family_outcomes: dict) -> bool:
    ...
    if not source_family_outcomes:
        return False
    feed_accepted = sum((v.get('accepted', 0) for k, v in source_family_outcomes.items() if 'feed' in k.lower() or k in ('ct_log', 'certificate')))
    nonfeed_accepted = sum((v.get('accepted', 0) for k, v in source_family_outcomes.items() if 'feed' not in k.lower() and k not in ('ct_log', 'certificate')))
...
def _handle_feed_dominant_actions(actions: list, source_family_outcomes: dict, current_query: str) -> None:
    ...
    if _is_feed_dominant(source_family_outcomes):
        actions.append(InvestigationAction(action='extract_more_seeds_from_duckdb', target=current_query, priority=0.85, reason='Feed-dominant evidence; need nonfeed corroboration', lane='public'))


def _handle_domain_seed_actions(actions: list, domains: list, source_family_outcomes: dict, mem_critical: bool, ct_has_findings: bool, ct_timed_out: bool, doh_terminal: bool, wayback_is_missing: bool, pdns_is_missing: bool) -> None:
    """Handle actions when domain seeds are available."""
    if domains and (not mem_critical):
        primary_domain = domains[0]
        if not doh_terminal:
            actions.append(InvestigationAction(action='run_doh_on_domain', target=primary_domain, priority=0.9, reason='Domain seed found; DOH lookup for DNS records', lane='infra'))
        ct_priority = 0.88
        ...
        if ct_priority > 0:
            actions.append(InvestigationAction(action='run_ct_on_domain', target=primary_domain, priority=ct_priority, reason=ct_reason, lane='infra'))
        if wayback_is_missing:
            actions.append(InvestigationAction(action='run_wayback_on_url', target=f'https://{primary_domain}', priority=0.8, reason='Domain seed found; Wayback for historical snapshots (lane missing/blank)', lane='passive'))
        if pdns_is_missing:
            actions.append(InvestigationAction(action='run_passivedns_on_domain_or_ip', target=primary_domain, priority=0.82, reason='Domain seed found; PassiveDNS for historical resolutions (lane missing/blank)', lane='passive'))
...
def _handle_no_domains_actions(actions: list, domains: list, current_query: str, mem_critical: bool) -> None:
    ...
    if not domains and (not mem_critical):
        actions.append(InvestigationAction(action='extract_more_seeds_from_duckdb', target=current_query, priority=0.82, reason='No domain seeds found; extract seeds from DuckDB', lane='public'))
...
def _handle_ip_seed_actions(actions: list, ips: list, mem_critical: bool) -> None:
    ...
    if ips and (not mem_critical):
        primary_ip = ips[0]
        actions.append(InvestigationAction(action='run_passivedns_on_domain_or_ip', target=primary_ip, priority=0.78, reason='IP seed found; PassiveDNS for historical resolutions', lane='passive'))
...
def _handle_url_seed_actions(actions: list, urls: list, mem_critical: bool) -> None:
    ...
    if urls and (not mem_critical):
        primary_url = urls[0]
        actions.append(InvestigationAction(action='run_wayback_on_url', target=primary_url, priority=0.76, reason='URL seed found; Wayback for historical snapshots', lane='passive'))


def _handle_missing_lanes_actions(actions: list, missing_lanes: list, domains: list, ips: list, current_query: str, mem_critical: bool, max_actions: int) -> None:
    ...
    for lane in missing_lanes:
        ...
        if lane == 'ct' and domains:
            actions.append(InvestigationAction(action='run_ct_on_domain', target=domains[0], priority=0.65, reason='CT lane missing; trying CT for domain', lane='infra'))
        elif lane == 'passivedns':
            target = domains[0] if domains else ips[0] if ips else current_query
            actions.append(InvestigationAction(action='run_passivedns_on_domain_or_ip', target=target, priority=0.65, reason='PassiveDNS lane missing; trying passive lookup', lane='passive'))
        elif lane == 'wayback':
            target = f'https://{domains[0]}' if domains else current_query
            actions.append(InvestigationAction(action='run_wayback_on_url', target=target, priority=0.6, reason='Wayback lane missing; trying archive lookup', lane='passive'))


def _handle_synthesis_actions(actions: list, source_count: int, has_corr: bool, mem_low: bool, corroboration_scores: dict, current_query: str) -> None:
    ...
    if source_count >= _MIN_SOURCES_FOR_SYNTHESIS and has_corr and (not mem_low):
        actions.append(InvestigationAction(action='synthesize_with_llm', target=current_query, priority=0.7, reason='Cross-source corroboration found; synthesis warranted', lane='synthesis'))
    if source_count >= 3 and has_corr:
        actions.append(InvestigationAction(action='stop_enough_evidence', target=current_query, priority=0.95, reason='Strong corroboration across >=3 independent sources', lane='stop'))
...
def plan_next_investigation_actions(state: dict, *, max_actions: int=MAX_ACTIONS) -> list[InvestigationAction]:
    ...
    """
    if not state:
        return [InvestigationAction(action='stop_enough_evidence', target='', priority=0.01, reason='Empty state; default stop', lane='stop')]
    ...
    if public_down and seed_context and (not mem_low):
        actions.append(InvestigationAction(action='public_bootstrap_from_seed', target=current_query, priority=0.72, reason='Public provider unavailable; bootstrap from seed context', lane='public'))

    # Delegate to helper functions
    _handle_feed_dominant_actions(actions, source_family_outcomes, current_query)
    _handle_domain_seed_actions(actions, domains, source_family_outcomes, mem_critical, ct_has_findings, ct_timed_out, doh_terminal, wayback_is_missing, pdns_is_missing)
...
def _terminal_from_source_family_outcomes(sfo_entry: dict) -> dict:
    ...
    if not terminal:
        if error:
            ...
            terminal = 'unknown'
    return {'terminal_state': terminal, 'error': error, 'attempted': bool(attempted), 'raw_count': accepted + rejected + pending}
...
def _extract_seed_context(pick, domains: list, ips: list, urls: list, hashes: list, cves: list, seed_available: bool) -> dict:
    ...
    if isinstance(sfo_raw, list):
        ...
        for entry in sfo_raw:
            ...
            if family:
                ...
                sfo_dict[family] = {'accepted': entry.get('accepted_count', 0) or entry.get('accepted', 0), 'rejected': entry.get('rejected_count', 0) or entry.get('rejected', 0), 'pending': entry.get('pending_count', 0) or entry.get('pending', 0), 'terminal_state': terminal_meta['terminal_state'], 'error': terminal_meta['error'], 'attempted': terminal_meta['attempted'], 'raw_count': terminal_meta['raw_count']}
...
def summarize_planner_actions(actions: list[InvestigationAction]) -> list[dict]:
    ...
    for action in actions:
        if len(summarized) >= MAX:
            break
        summarized.append({'action': action.action, 'target': action.target, 'priority': round(action.priority, 4), 'reason': action.reason, 'lane': action.lane})
    return summarized