...
async def run_ct_pivot(domain: str) -> CTPivotResult:
    ...
    try:
        ...
        return CTPivotResult(domain=result['domain'], accepted_findings=_accepted, cert_count=result['cert_count'], first_cert=result['first_cert'], last_cert=result['last_cert'], san_names=_san_names, issuers=result['issuers'])
    finally:
