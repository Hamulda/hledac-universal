...
def _merge_outcomes(base: AdvisoryRunOutcome, other: AdvisoryRunOutcome) -> AdvisoryRunOutcome:
    ...
    return AdvisoryRunOutcome(
        ...
        local_search_source=base.local_search_source
        if base.local_search_source != "none"
        else other.local_search_source,
...
def build_search_documents_from_findings(findings: list) -> list:
    ...
    for f in findings:
        ...
        try:
            ...
            source_type = getattr(f, "source_type", "unknown") or (
                f.get("source_type", "unknown") if isinstance(f, dict) else "unknown"
            )
...
class SprintAdvisoryRunner:
    ...
    __slots__ = ("_analyst_workbench", "_duckdb_store", "_governor", "_scheduler")

    def __init__(
        self, scheduler: Any, duckdb_store: Any = None, governor: Any = None, analyst_workbench: Any = None
    ) -> None:
    ...
    async def run_all_advisories(self) -> AdvisoryRunOutcome:
        ...
        try:
            ...
            if parallel_ok is not None:
                ...
                parallel_results = await parallel_ok(
    ...
    async def _collect_feedback_summary(self) -> Any:
        ...
        try:
            ...
            adapter = HypothesisFeedbackAdapter(
                duckdb_store=store, target_id=getattr(self._scheduler, "sprint_id", "") or "default"
            )
    ...
    def _compute_pivots(
        self, planner: Any, findings: list, hermes_outputs: list, graph_stats: dict, feedback_summary: Any
    ) -> list:
    ...
    async def _run_pivot_executor_advisory(self, outcome: AdvisoryRunOutcome) -> AdvisoryRunOutcome:
        ...
        try:
            ...
            executor = AutonomousPivotExecutor(
                duckdb_store=store, resource_governor=self._governor, feedback_adapter=feedback_adapter
            )
    ...
    async def _run_federated_research_advisory(self, outcome: AdvisoryRunOutcome) -> AdvisoryRunOutcome:
        ...
        try:
            ...
            elapsed = (perf_counter() - t0) * 1000
            log.debug(
                "[F350M-FED-P3-FOLLOWUP] advisory done: updates=%d persists=%d mode=%s dur=%.2fms",
                updates_emitted,
                persists_emitted,
