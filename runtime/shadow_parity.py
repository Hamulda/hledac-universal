...
def _check_phase_field_merge(
    bundle: LifecycleSnapshotBundle, mismatches: list[str], mismatch_details: dict[str, Any]
) -> None:
    """
    ...
    if wf == "WINDUP" and bundle.windup_local_phase is None:
        if "PHASE_FIELD_MERGE" not in mismatches:
            ...
            mismatch_details["phase_field_merge"] = (
                "workflow_phase=WINDUP but windup_local_phase=None — windup_local synthesis mode missing in WINDUP context"
            )
    if wf != "WINDUP" and bundle.windup_local_phase is not None:
        if "PHASE_FIELD_MERGE" not in mismatches:
            mismatches.append("PHASE_FIELD_MERGE")
            mismatch_details["phase_field_merge"] = (
                f"workflow_phase={wf} but windup_local_phase is set to {bundle.windup_local_phase.mode} — windup_local leaked outside WINDUP"
            )


def _check_graph_capability(
    graph_bundle: GraphSummaryBundle, mismatches: list[str], mismatch_details: dict[str, Any]
) -> None:
    """
    ...
    if graph_backend == "unknown":
        ...
        if stability == "STABLE":
            ...
            mismatch_details["graph_backend"] = (
                f"unknown backend (fact_stability={stability}) — insufficient input for capability determination"
            )
