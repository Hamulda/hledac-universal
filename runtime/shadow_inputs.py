...
class ControlPhase(Struct, frozen=True):
    ...
    @classmethod
    def from_lifecycle(
        cls, lifecycle: SprintLifecycleManager, now_monotonic: float | None = None, thermal_state: str = "nominal"
    ) -> ControlPhase:
...
class LifecycleSnapshotBundle(Struct):
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "windup_local_synthesis_engine": self.windup_local_phase.synthesis_engine
            if self.windup_local_phase
            else None,
...
def collect_lifecycle_snapshot(
    ...
    if wf_phase.phase == "WINDUP":
        windup_local = WindupLocalPhase(
            mode=windup_synthesis_mode, error_encountered=windup_error, synthesis_engine=windup_engine
        )
        fact_stability = "COMPAT"
...
def collect_model_control_facts(
    analyzer_result: AnalyzerResult | None = None, raw_profile: dict[str, Any] | None = None
    ...
    return ModelControlFactsBundle(
        fact_stability="UNKNOWN", __compat_note__="no analyzer_result and no raw_profile provided"
    )


def collect_export_handoff_facts(
    handoff: ExportHandoff | None = None, scorecard: dict[str, Any] | None = None, sprint_id: str = "unknown"
) -> dict[str, Any]:
    """
...
def collect_provider_runtime_facts(
    model_manager: Any = None, lifecycle_status: dict[str, Any] | None = None
    ...
    if lifecycle_status is not None:
        try:
            ...
            pass
    return ProviderRuntimeFactsBundle(
        fact_stability="UNKNOWN", __compat_note__="no model_manager and no lifecycle_status provided"
    )
