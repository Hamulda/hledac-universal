...
from hledac.universal.network.session_runtime import CT_CONNECT_TIMEOUT_S, CT_READ_TIMEOUT_S, async_get_aiohttp_session
...
class _CTLogScanner:
    ...
    __slots__ = tuple(('_write_lock', 'allow_external', 'cache_ttl_days'))
    ...
    async def get_subdomains(self, domain: str, *, async_session: httpx.AsyncClient | None=None) -> list[str]:
        ...
        async def _fetch_with_session(session: httpx.AsyncClient) -> list[str]:
            ...
            async with session.get(url, timeout=httpx.Timeout(connect=CT_CONNECT_TIMEOUT_S, read=CT_READ_TIMEOUT_S)) as resp:
                if resp.status != 200:
                    return []
    ...
    async def _flush_pending_writes(self):
        ...
        try:
            with sqlite3.connect(self.CACHE_DB) as conn:
                conn.executemany('INSERT OR REPLACE INTO ct_cache (domain, subdomains, fetched_at) VALUES (?, ?, ?)', writes)
                conn.commit()
