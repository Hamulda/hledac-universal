"""
probe_f207j_nonfeed_finding_bridge/nonfeed_finding_bridge.py
============================================================

Sprint F207J-B: Non-feed adapter → CanonicalFinding candidate bridge.
...
__all__ = ['ct_results_to_findings', 'wayback_results_to_findings', 'passive_dns_results_to_findings', 'Rejection', 'MAX_BRIDGE_OUTPUT']
Rejection = str
...
_URL_SCHEME_RE = re.compile('^https?://', re.IGNORECASE)
_URL_TRAILING_SLASH_RE = re.compile('/+$')
...
def _make_blake2b_hex(value: str, salt: str='', max_len: int=16) -> str:
    ...
    """
    b2 = hashlib.blake2b((salt + value).encode('utf-8'), digest_size=16)
    return b2.hexdigest()[:max_len]

def _canonical_finding(*, finding_id: str, source_type: str, query: str, confidence: float, ts: float, provenance: tuple[str, ...], payload_text: str | None=None) -> Any | None:
    """
    ...
    try:
        return CanonicalFinding(finding_id=finding_id, query=query[:500], source_type=source_type, confidence=confidence, ts=ts, provenance=prov, payload_text=payload)
    except Exception:  # noqa: BLE001 — CanonicalFinding validation errors are expected in probe context
        return None
...
def _ts_from_wayback_timestamp(ts: str) -> float:
    ...
    except ValueError:  # noqa: BLE001 — invalid timestamp format → return 0.0
        return 0.0
_PRIVATE_HOSTNAMES: frozenset[str] = frozenset({'localhost', 'invalid', 'test', 'example'})
...
def _extract_domain_from_ct_hit(url: str, title: str) -> str | None:
    ...
    Returns None if no domain-like string is found. Does NOT filter
    private hosts — filtering is done in the caller.
    """
...
def ct_results_to_findings(batch_result: Any, outcome: Any, query: str, sprint_id: str) -> tuple[list[Any], list[RejectionReason]]:
    """
    ...
    The domain is extracted from hit.url (format: "https://subdomain.example.com/").
    ...
    for hit in capped:
        ...
        provenance: tuple[str, ...] = (f'ct:{domain}', f'query:{query[:200]}', f'sprint:{sprint_id[:16]}', f'title:{title[:200]}')
        payload_text = snippet if snippet else None
        finding = _canonical_finding(finding_id=finding_id, source_type=_CT_SOURCE_TYPE, query=query, confidence=_CT_CONFIDENCE, ts=ts, provenance=provenance, payload_text=payload_text)
...
def wayback_results_to_findings(diff_result: Any, query: str, sprint_id: str) -> tuple[list[Any], list[RejectionReason]]:
    """
    ...
    for event in capped:
        ...
        provenance: tuple[str, ...] = (f'wayback:{url[:300]}', f'digest:{digest[:64]}', f'change:{change_type}', f'ts:{timestamp}', f'sprint:{sprint_id[:16]}')
        payload_parts = [f'change_type: {change_type}', f'url: {url}', f'digest: {digest}', f'timestamp: {timestamp}']
        if evidence_url:
            payload_parts.append(f'evidence_url: {evidence_url}')
        if url:
            payload_parts.append(f'source_domain: {_strip_url_scheme(url)}')
        payload_text = '\n'.join(payload_parts)
        finding = _canonical_finding(finding_id=finding_id, source_type=_WAYBACK_SOURCE_TYPE, query=query, confidence=_WAYBACK_CONFIDENCE, ts=ts, provenance=provenance, payload_text=payload_text)
        if finding is not None:
            findings.append(finding)
    return (findings, rejections)
...
def passive_dns_results_to_findings(ips: list[str], outcome: Any, query: str, sprint_id: str) -> tuple[list[Any], list[RejectionReason]]:
    """
    ...
    if not ips:
        rejections.append(REJECTION_MISSING_VALUE)
        return ([], rejections)
    _PRIVATE_IP_PREFIXES = ('10.', '172.16.', '172.17.', '172.18.', '172.19.', '172.20.', '172.21.', '172.22.', '172.23.', '172.24.', '172.25.', '172.26.', '172.27.', '172.28.', '172.29.', '172.30.', '172.31.', '192.168.', '127.', '0.', '255.', '169.254.', '::1', 'fe80:', 'fc00:', 'fd00:')
    ...
    for ip in capped:
        ...
        provenance: tuple[str, ...] = (f'domain:{query_stripped}', f'ip:{ip_stripped}', f'sprint:{sprint_id[:16]}', f'source:circl_pdns')
        payload_text = f'domain: {query_stripped}\nip: {ip_stripped}\nsource: CIRCL PDNS'
        finding = _canonical_finding(finding_id=finding_id, source_type=_PDNS_SOURCE_TYPE, query=query, confidence=_PDNS_CONFIDENCE, ts=now, provenance=provenance, payload_text=payload_text)
        if finding is not None:
            findings.append(finding)
    return (findings, rejections)