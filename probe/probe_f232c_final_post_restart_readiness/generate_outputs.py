...
commands_md = """# POST_RESTART_COMMANDS — Sprint F232C
...
**Live Allowed:** """ + str(result.live_allowed) + """
**Next Action:** `""" + result.next_action.value + """`
**Swap:** """ + f"{result.swap_used_gib:.3f} GiB (tier: {result.swap_policy_tier})" + """
**F231 Inventory:** """ + result.f231_inventory_verdict + """
**F231H Gate:** """ + result.f231h_gate_verdict + """
...
with open(os.path.join(out_dir, 'POST_RESTART_COMMANDS.md'), 'w') as f:
    f.write(commands_md)
print(f'POST_RESTART_COMMANDS.md written')
...
print(f'post_restart_commands.json written')
