...
PROBE_ROOT = pathlib.Path(
    "/Users/vojtechhamada/PycharmProjects/Hledac/hledac/universal/probe_f229d_next_action_import_compat"
)
...
def test_lsm_file_exists() -> None:
    check("lsm_file_exists", LSM_PATH.exists(), str(LSM_PATH))


def test_nam_file_exists() -> None:
    check("nam_file_exists", NAM_PATH.exists(), str(NAM_PATH))


def test_lsm_imports_derive_next_action() -> None:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module == "benchmarks.live_measurement_next_action":
            ...
            check("lsm_imports_derive_next_action", "_derive_next_action" in names, f"import names: {names}")
            return
    check("lsm_imports_derive_next_action", False, "import not found")


def test_lsm_imports_next_action_input() -> None:
    src = LSM_PATH.read_text()
    tree = ast.parse(src)
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module == "benchmarks.live_measurement_next_action":
            names = [a.name for a in node.names]
            check("lsm_imports_next_action_input", "NextActionInput" in names, f"import names: {names}")
            return
    check("lsm_imports_next_action_input", False, "import not found")


def test_lsm_imports_was_family_attempted() -> None:
    src = LSM_PATH.read_text()
    tree = ast.parse(src)
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module == "benchmarks.live_measurement_next_action":
            names = [a.name for a in node.names]
            check("lsm_imports_was_family_attempted", "_was_family_attempted" in names, f"import names: {names}")
            return
    check("lsm_imports_was_family_attempted", False, "import not found")


def test_lsm_no_local_next_action_input() -> None:
    src = LSM_PATH.read_text()
    tree = ast.parse(src)
    local = [n.name for n in ast.walk(tree) if isinstance(n, ast.ClassDef) and n.name == "NextActionInput"]
    check("lsm_no_local_next_action_input", len(local) == 0, f"found: {local}" if local else "")


def test_lsm_no_local_rule_helpers() -> None:
    src = LSM_PATH.read_text()
    tree = ast.parse(src)
    local = [n.name for n in ast.walk(tree) if isinstance(n, ast.FunctionDef) and n.name.startswith("_rule")]
    check("lsm_no_local_rule_helpers", len(local) == 0, f"found: {local}" if local else "")


def test_lsm_no_local_was_family_attempted() -> None:
    src = LSM_PATH.read_text()
    tree = ast.parse(src)
    local = [n.name for n in ast.walk(tree) if isinstance(n, ast.FunctionDef) and n.name == "_was_family_attempted"]
    check("lsm_no_local_was_family_attempted", len(local) == 0, f"found: {local}" if local else "")


def test_lsm_no_local_derive_next_action() -> None:
    src = LSM_PATH.read_text()
    tree = ast.parse(src)
    local = [n.name for n in ast.walk(tree) if isinstance(n, ast.FunctionDef) and n.name == "_derive_next_action"]
    check("lsm_no_local_derive_next_action", len(local) == 0, f"found: {local}" if local else "")


def test_nam_exports_expected_symbols() -> None:
            ...
            for t in node.targets:
                if isinstance(t, ast.Name) and t.id == "__all__":
                    if isinstance(node.value, ast.List):
                        ...
                        check("nam_exports_expected_symbols", expected.issubset(set(names)), f"found: {names}")
                        return
    check("nam_exports_expected_symbols", False, "__all__ not found")


def test_nam_import_succeeds() -> None:
...
def test_nam_derive_next_action_callable() -> None:
    sys.path.insert(0, str(UNIVERSAL_ROOT))
    sys.path.insert(0, str(UNIVERSAL_ROOT / "benchmarks"))
    try:
        import benchmarks.live_measurement_next_action as m
...
def test_nam_next_action_input_fields() -> None:
    sys.path.insert(0, str(UNIVERSAL_ROOT))
    sys.path.insert(0, str(UNIVERSAL_ROOT / "benchmarks"))
    try:
        import benchmarks.live_measurement_next_action as m
        ...
        check("nam_next_action_input_fields", required.issubset(fields), f"missing: {required - fields}")
...
def test_nam_rule_count() -> None:
    src = NAM_PATH.read_text()
    tree = ast.parse(src)
    rules = [n.name for n in ast.walk(tree) if isinstance(n, ast.FunctionDef) and n.name.startswith("_rule")]
    check("nam_rule_count", len(rules) == 8, f"found {len(rules)}: {rules}")


def test_nam_was_family_attempted_callable() -> None:
    sys.path.insert(0, str(UNIVERSAL_ROOT))
    sys.path.insert(0, str(UNIVERSAL_ROOT / "benchmarks"))
    try:
        import benchmarks.live_measurement_next_action as m
...
def test_nam_minimal_input_produces_tuple() -> None:
    sys.path.insert(0, str(UNIVERSAL_ROOT))
    sys.path.insert(0, str(UNIVERSAL_ROOT / "benchmarks"))
    try:
        import benchmarks.live_measurement_next_action as m
        ...
        result = m._derive_next_action(
            ...
            ct_findings=0,
            runtime_truth=rt,
            feed_dominance_score=None,
            top_public_reject_reason=None,
            nonfeed_starvation_suspected=False,
            prewindup_barrier_checked=False,
            prewindup_barrier_satisfied=False,
            prewindup_required_lanes=None,
            prewindup_attempted_lanes=None,
            ...
            acquisition_prelude_checked=False,
            ...
            acquisition_prelude_duration_s=None,
        ...
        check(
            "nam_minimal_input_produces_tuple",
...
def test_nam_was_family_attempted_behavior() -> None:
    sys.path.insert(0, str(UNIVERSAL_ROOT))
    sys.path.insert(0, str(UNIVERSAL_ROOT / "benchmarks"))
    try:
        import benchmarks.live_measurement_next_action as m
