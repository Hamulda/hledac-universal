...
_F231_BLOCKING_PROBES = [
    ...
    "probe_f231c_advisory_evidence_surface",
    "probe_f231d_research_quality_v2",
    "probe_f231e_research_quality_comparable_field",
    "probe_f231f_evidence_depth_aliases",
    "probe_f231g_quality_sanity_bundle_smoke",
]
...
def check_live_allowed(decision_data: dict, artifact_pack: dict) -> tuple[str, list[str]]:
    ...
    decision_data.get("live_allowed", False)
    missing_required = artifact_pack.get("missing_required", [])
    artifact_pack.get("overall", "UNKNOWN")
    return (gate_decision, missing_required)


def derive_verdict(
    gate_decision: str, f231_missing: list[str], swap_gib: float, missing_artifacts: list[str]
) -> ReadinessResult:
    """Derive final operator verdict."""
    if gate_decision == "BLOCKED_BY_PROVIDER_SURFACE":
        return ReadinessResult(
            verdict="BLOCKED_BY_PROVIDER_SURFACE",
            next_action="fix_provider_surface",
            next_action_detail="probe_f217c_public_bootstrap and/or probe_f217d_ct_provider_resilience missing",
        )
    if gate_decision == "BLOCKED_BY_MEMORY":
        return ReadinessResult(
            verdict="BLOCKED_BY_MEMORY",
            next_action="restart_then_run_live",
            next_action_detail=f"swap={swap_gib:.2f}GiB — restart required before live",
        )
    if f231_missing:
        return ReadinessResult(
            verdict="BLOCKED_BY_ARTIFACTS",
            next_action="run_missing_probe",
            next_action_detail=",".join(f"probe_{m.lower()}" for m in f231_missing),
            f231_missing=f231_missing,
        )
    if missing_artifacts:
        return ReadinessResult(
            verdict="BLOCKED_BY_ARTIFACTS",
            next_action="run_missing_probe",
            next_action_detail=",".join(missing_artifacts),
        )
    if swap_gib <= _CLEAN_SWAP_MAX_GIB:
        return ReadinessResult(
            verdict="READY_TO_RUN_NOW",
            next_action="run_live_now",
            next_action_detail="nonfeed_diagnostic180 — exact command below",
            uma_swap_gib=swap_gib,
        )
    elif swap_gib <= _DIAGNOSTIC_SWAP_MAX_GIB:
        return ReadinessResult(
            verdict="READY_DIAGNOSTIC_ONLY",
            next_action="run_with_hardware_taint",
            next_action_detail=f"swap={swap_gib:.2f}GiB in ({_CLEAN_SWAP_MAX_GIB:.1f}, {_DIAGNOSTIC_SWAP_MAX_GIB:.1f}]GiB — hardware taint",
            uma_swap_gib=swap_gib,
        )
    else:
        return ReadinessResult(
            ...
            next_action="restart_then_run_live",
            next_action_detail=f"swap={swap_gib:.2f}GiB > {_DIAGNOSTIC_SWAP_MAX_GIB:.1f}GiB — restart required",
            uma_swap_gib=swap_gib,
        )
...
def render_markdown(result: ReadinessResult, profile: str, query: str) -> str:
    ...
    lines = [
        "# Pre-Live Cockpit Final Readiness — Sprint F231L",
        "",
    ...
    lines.extend(
        [
            ...
            "## Component Status",
            "",
            f"- **F231 Inventory Loaded:** `{result.f231_inventory_loaded}`",
            f"- **F231 Missing:** `{result.f231_missing or 'none'}`",
            f"- **Gate Decision:** `{result.gate_decision}`",
            f"- **UMA Swap:** `{result.uma_swap_gib:.2f}GiB`",
            f"- **Missing Artifacts:** `{result.missing_artifacts or 'none'}`",
            "",
            "## Next Actions",
        ]
    )
    if result.verdict == "READY_TO_RUN_NOW":
        lines.extend(
            [
                "",
                "Ready to run nonfeed_diagnostic180 live:",
                "```bash",
                f"python -m core --profile nonfeed_diagnostic180 --query {repr(query)} --live",
                "```",
            ]
        )
        ...
        lines.extend(
            [
                "",
                "Swap too high. Restart then run:",
                "```bash",
                "# restart terminal / session first",
                f"python -m core --profile nonfeed_diagnostic180 --query {repr(query)} --live",
                "```",
            ]
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "-d",
        type=Path,
        default=BASE / "probe_f219f_prelive_decision_gate/prelive_decision.json",
        help="Decision gate JSON path",
    )
    parser.add_argument(
        "--artifact-pack-json",
        "-a",
        type=Path,
        default=BASE / "probe_f219i_prelive_artifact_pack/artifact_pack.json",
        help="Artifact pack JSON path",
    )
    parser.add_argument(
        "--f231-inventory-json",
        "-f",
        type=Path,
        default=BASE / "probe_f231j_artifact_inventory/f231_artifact_inventory.json",
        help="F231J artifact inventory JSON path",
    )
    ...
    parser.add_argument(
        "--query", default="mozilla.org certificate transparency subdomains april 2026", help="Query for report header"
    )
    return parser
