...
def run_decision_gate() -> tuple[str, dict]:
    ...
    result = run_gate(REPO_ROOT, 'nonfeed_diagnostic', 'mozilla.org certificate transparency subdomains april 2026')
    return (result.decision.value, {'live_allowed': result.live_allowed, 'reasons': result.reasons, 'warnings': result.warnings, 'f224_core_ready': result.f224_core_ready, 'missing_f224_artifacts': result.missing_f224_artifacts, 'f231_core_ready': result.f231_core_ready, 'missing_f231_artifacts': result.missing_f231_artifacts, 'swap_policy_tier': result.swap_policy_tier, 'swap_gate_reason': result.swap_gate_reason, 'hardware_constrained': result.hardware_constrained, 'uma_swap_gib': result.uma.get('swap_used_gib', 0.0), 'uma_state': result.uma.get('uma_state', 'unknown'), 'suggested_live_command': result.suggested_live_command})

def run_cockpit_check() -> tuple[str, str]:
    """F231L prelive cockpit final readiness verdict."""
    cockpit_json = os.path.join(REPO_ROOT, 'probe_f231l_prelive_cockpit_final_readiness', 'prelive_cockpit_final_readiness.json')
...
def check_f219_aliases() -> dict[str, bool]:
    ...
    p219d = os.path.join(REPO_ROOT, 'probe_f219d_public_session_seal', 'public_session_seal.json')
    d = load_json(p219d)
...
def determine_verdict(gate_decision: str, gate_data: dict, f224_blocking: list[str], f231_blocking: list[str], uma_swap: float, uma_state: str, inventory_ready: bool, inventory_verdict: str, f231h_gate: str) -> ReadinessResult:
    ...
    if uma_swap > 4.0:
        blockers.append(Blocker(category='memory', detail=f'swap={uma_swap:.3f}GiB > 4.0GiB threshold'))
    ...
    for reason in gate_data.get('reasons', []):
        if 'BLOCKED_BY_CONTRACT' in reason and 'Hermes Metal' in reason:
            blockers.append(Blocker(category='contract', detail=reason, probe='probe_f219b_hermes_metal_finalizer'))
            ...
            blockers.append(Blocker(category='contract', detail=reason, probe='probe_f224d_confidence_policy'))
            merge_log.append(f'blocker=CONTRACT {reason}')
    for probe in f224_blocking:
        blockers.append(Blocker(category='artifacts', detail=f'{probe} missing — required for nonfeed_diagnostic', probe=probe))
        merge_log.append(f'blocker=F224_ARTIFACT {probe}')
    for probe in f231_blocking:
        blockers.append(Blocker(category='artifacts', detail=f'{probe} missing — required evidence lift pack', probe=probe))
    ...
    if memory_blocks and (not (contract_blocks or artifact_blocks)):
        live_allowed = False
        suggested = f'python -m core --profile nonfeed_diagnostic180 --query "mozilla.org certificate transparency subdomains april 2026" --live --require-memory-ok'
        merge_log.append('verdict=READY_TO_RESTART_AND_RUN (memory only)')
        return ReadinessResult(verdict=Verdict.READY_TO_RESTART_AND_RUN, live_allowed=live_allowed, blockers=blockers, uma_swap_gib=uma_swap, uma_state=uma_state, suggested_command=suggested, merge_log=merge_log, f224_blocking=f224_blocking, f231_blocking=f231_blocking, f231_inventory_verdict=inventory_verdict, f231h_gate_verdict=f231h_gate)
    elif contract_blocks:
        live_allowed = False
        failing_probe = next((b.probe for b in blockers if b.category == 'contract' and b.probe), 'probe_f219b_hermes_metal_finalizer')
        ...
        return ReadinessResult(verdict=Verdict.BLOCKED_BY_CONTRACT, live_allowed=live_allowed, blockers=blockers, uma_swap_gib=uma_swap, uma_state=uma_state, suggested_command=suggested, merge_log=merge_log, f224_blocking=f224_blocking, f231_blocking=f231_blocking, f231_inventory_verdict=inventory_verdict, f231h_gate_verdict=f231h_gate)
    elif artifact_blocks:
        live_allowed = False
        suggested = 'python -m pytest tests/probe_f224d_confidence_policy -v --tb=short && python -m pytest tests/probe_f231a_public_candidate_ledger -v --tb=short'
        merge_log.append('verdict=BLOCKED_BY_ARTIFACTS')
        return ReadinessResult(verdict=Verdict.BLOCKED_BY_ARTIFACTS, live_allowed=live_allowed, blockers=blockers, uma_swap_gib=uma_swap, uma_state=uma_state, suggested_command=suggested, merge_log=merge_log, f224_blocking=f224_blocking, f231_blocking=f231_blocking, f231_inventory_verdict=inventory_verdict, f231h_gate_verdict=f231h_gate)
    else:
        live_allowed = True
        suggested = f'python -m core --profile nonfeed_diagnostic180 --query "mozilla.org certificate transparency subdomains april 2026" --live --require-memory-ok'
        merge_log.append('verdict=READY_TO_RUN_NOW')
        return ReadinessResult(verdict=Verdict.READY_TO_RUN_NOW, live_allowed=live_allowed, blockers=blockers, uma_swap_gib=uma_swap, uma_state=uma_state, suggested_command=suggested, merge_log=merge_log, f224_blocking=f224_blocking, f231_blocking=f231_blocking, f231_inventory_verdict=inventory_verdict, f231h_gate_verdict=f231h_gate)
...
def run() -> ReadinessResult:
    ...
    result = determine_verdict(gate_decision=gate_decision, gate_data=gate_data, f224_blocking=f224_blocking, f231_blocking=f231_blocking, uma_swap=uma_swap, uma_state=uma_state, inventory_ready=inv_ready, inventory_verdict=inv_verdict, f231h_gate=f231h_status)
...
if __name__ == '__main__':
    ...
    with open(output_json, 'w') as f:
        json.dump({'sprint': 'F231T', 'verdict': result.verdict.value, 'live_allowed': result.live_allowed, 'blockers': [{'category': b.category, 'detail': b.detail, 'probe': b.probe} for b in result.blockers], 'f231_inventory_verdict': result.f231_inventory_verdict, 'f231h_gate_verdict': result.f231h_gate_verdict, 'f224_blocking': result.f224_blocking, 'f231_blocking': result.f231_blocking, 'uma_swap_gib': result.uma_swap_gib, 'uma_state': result.uma_state, 'suggested_command': result.suggested_command, 'merge_log': result.merge_log}, f, indent=2)
