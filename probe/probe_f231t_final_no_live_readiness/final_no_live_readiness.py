...
def run_cockpit_check() -> tuple[str, str]:
    ...
    cockpit_json = os.path.join(
        REPO_ROOT, "probe_f231l_prelive_cockpit_final_readiness", "prelive_cockpit_final_readiness.json"
    )
...
def determine_verdict(
    ...
    for reason in gate_data.get("reasons", []):
        if "BLOCKED_BY_CONTRACT" in reason and "Hermes Metal" in reason:
            ...
            merge_log.append("blocker=CONTRACT Hermes Metal finalizer FAILED")
    ...
    for probe in f224_blocking:
        blockers.append(
            Blocker(category="artifacts", detail=f"{probe} missing — required for nonfeed_diagnostic", probe=probe)
        )
        merge_log.append(f"blocker=F224_ARTIFACT {probe}")
    for probe in f231_blocking:
        blockers.append(
            Blocker(category="artifacts", detail=f"{probe} missing — required evidence lift pack", probe=probe)
        )
    ...
    if memory_blocks and (not (contract_blocks or artifact_blocks)):
        live_allowed = False
        suggested = 'python -m core --profile nonfeed_diagnostic180 --query "mozilla.org certificate transparency subdomains april 2026" --live --require-memory-ok'
        ...
        live_allowed = False
        failing_probe = next(
            (b.probe for b in blockers if b.category == "contract" and b.probe), "probe_f219b_hermes_metal_finalizer"
        )
...
if __name__ == "__main__":
    result = run()
    print(f"{'=' * 60}")
    print("  F231T Final No-Live Readiness")
    print(f"{'=' * 60}")
        ...
        json.dump(
            {
                ...
                "live_allowed": result.live_allowed,
                "blockers": [{"category": b.category, "detail": b.detail, "probe": b.probe} for b in result.blockers],
