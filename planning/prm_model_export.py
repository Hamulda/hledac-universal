...
def _create_numpy_model() -> dict[str, np.ndarray]:
    ...
    weights = {
        "w1": rng.normal(
            0, np.sqrt(2.0 / (_PRM_FEATURE_DIM + _PRM_HIDDEN_DIM)), (_PRM_FEATURE_DIM, _PRM_HIDDEN_DIM)
        ).astype(np.float32),
        "b1": np.zeros(_PRM_HIDDEN_DIM, dtype=np.float32),
        "w2": rng.normal(
            0, np.sqrt(2.0 / (_PRM_HIDDEN_DIM + _PRM_OUTPUT_DIM)), (_PRM_HIDDEN_DIM, _PRM_OUTPUT_DIM)
        ).astype(np.float32),
