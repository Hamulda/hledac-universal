...
class PRMFeatureIdx:
    ...
    DEAD_END_PROB = 7
    IOC_DISCOVERY_RATE = 8
    UNCERTAINTY = 9
    SPRINT_URGENCY = 10
    TIME_ELAPSED = 11
    SEMANTIC_GRAVITY = 12
    CHILD_COUNT = 13
    SIBLING_DIVERSITY = 14
...
class PRMFeatureVector:
    ...
    def __post_init__(self) -> None:
        if len(self.features) != _PRM_FEATURE_DIM:
            raise ValueError(f'Expected {_PRM_FEATURE_DIM} features, got {len(self.features)}')
...
class PRMFeatureExtractor:
    ...
    def __init__(self, max_depth: int=8, max_cost: float=32.0) -> None:
        self._max_depth = max_depth
        self._max_cost = max_cost
        self._avg_branching: float = 2.0
    ...
    def extract(self, node: ThoughtNode, context: PRMInferenceContext) -> PRMFeatureVector:
        ...
        f[PRMFeatureIdx.AVG_BRANCHING_FACTOR] = np.clip((self._avg_branching - 1.0) / 5.0, 0.0, 1.0)
...
class PRMInference:
    ...
    """
    __slots__ = ('_model_path', '_model', '_model_loaded', '_model_warmed_up', '_coreml_available', '_rust_ane_available', '_numpy_weights', '_telemetry', '_use_rust_ane', '_warmup_features')

    def __init__(self, model_path: Path | None=None, use_rust_ane: bool=True) -> None:
        ...
        self._telemetry = {'inference_calls': 0, 'coreml_calls': 0, 'rust_ane_calls': 0, 'numpy_fallback_calls': 0, 'warmup_calls': 0}
    ...
    def _prepare_warmup_features(self) -> None:
        ...
        warmup[PRMFeatureIdx.DEPTH] = 0.5
        warmup[PRMFeatureIdx.PARENT_VALUE] = 0.5
        warmup[PRMFeatureIdx.UNCERTAINTY] = 0.3
        warmup[PRMFeatureIdx.SPRINT_URGENCY] = 0.2
        warmup[PRMFeatureIdx.EXPLORATION_BONUS] = 0.1
    ...
    def _load_rust_ane(self) -> bool:
        ...
        try:
            ...
            if raw is None or not hasattr(raw, 'ane'):
                return False
            result = raw.ane.load_model('prm_step', str(self._model_path), _PRM_FEATURE_DIM, 1)
            self._model_loaded = True
    ...
    def _load_numpy_fallback(self) -> None:
        ...
        self._numpy_weights = {'w1': rng.normal(0, 0.1, (16, 32)).astype(np.float32), 'b1': np.zeros(32, dtype=np.float32), 'w2': rng.normal(0, 0.05, (32, 1)).astype(np.float32), 'b2': np.zeros(1, dtype=np.float32)}

    def predict_step_reward(self, features: PRMFeatureVector) -> float:
        """
    ...
    def _predict_numpy(self, features: PRMFeatureVector) -> float:
        ...
        if self._numpy_weights is None:
            self._load_numpy_fallback()
        w1, b1, w2, b2 = (self._numpy_weights['w1'], self._numpy_weights['b1'], self._numpy_weights['w2'], self._numpy_weights['b2'])
        h = np.maximum(features.features @ w1 + b1, 0.0)
        out = float(h @ w2 + b2)
        return np.clip(out, -1.0, 1.0)

    def predict_batch(self, features_list: list[PRMFeatureVector]) -> list[float]:
        """
        ...
        if self._numpy_weights is None:
            self._load_numpy_fallback()
        batch_matrix = np.stack([f.features for f in features_list])
        w1 = self._numpy_weights['w1']
        b1 = self._numpy_weights['b1']
        w2 = self._numpy_weights['w2']
        b2 = self._numpy_weights['b2']
    ...
    def get_info(self) -> dict[str, Any]:
        ...
        """
        return {'model_path': str(self._model_path), 'model_exists': self._model_path.exists(), 'model_loaded': self._model_loaded, 'model_warmed_up': self._model_warmed_up, 'coreml_available': self._coreml_available, 'rust_ane_available': self._rust_ane_available, 'inference_path': self.get_inference_path(), 'telemetry': self._telemetry.copy(), 'ane_constraints': {'max_models': _ANE_MAX_MODELS, 'max_batch_size': _ANE_MAX_BATCH_SIZE}, 'platform': {'system': platform.system(), 'machine': platform.machine(), 'ane_capable': platform.system() == 'Darwin' and platform.machine() == 'arm64'}}
...
class CumulativePRMScorer:
    ...
    """
    __slots__ = ('_prm_inference', '_feature_extractor', '_discount_factor', '_step_rewards_cache', '_total_reward_cache')

    def __init__(self, prm_inference: PRMInference | None=None, feature_extractor: PRMFeatureExtractor | None=None, discount_factor: float=0.95) -> None:
...
def create_default_prm_scorer() -> CumulativePRMScorer:
    ...
    return CumulativePRMScorer(prm_inference=prm, feature_extractor=extractor, discount_factor=0.95)
...
class RustPRMScorer:
    ...
    def _get_numpy_weights(self) -> dict[str, np.ndarray]:
        ...
        rng = np.random.default_rng(42)
        return {'w1': rng.normal(0, 0.1, (16, 32)).astype(np.float32), 'b1': np.zeros(32, dtype=np.float32), 'w2': rng.normal(0, 0.05, (32, 1)).astype(np.float32), 'b2': np.zeros(1, dtype=np.float32)}
    ...
    def _numpy_batch_inference(self, batch_matrix: np.ndarray, weights: dict[str, np.ndarray]) -> list[float]:
