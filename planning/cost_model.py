...
class OnlineRidge:
    ...
    __slots__ = ("n_features", "alpha", "A", "A_inv", "b", "coef_", "n_samples")
...
class RunningNormalizer:
    ...
    __slots__ = ("count", "decay", "dim", "mean", "var")

    def __init__(self, dim: int, decay: float = 0.99) -> None:
        self.dim = dim
        self.decay = decay
...
class AdaptiveCostModel:
    ...
    def __init__(self, governor, evidence_log, feature_dim: int = 64, hidden_dim: int = 32, lr: float = 0.001) -> None:
    ...
    def _load_mlx_model(self) -> None:
        ...
        if has_mamba:
            ...
            class _MambaBlock(nn.Module):
                __slots__ = ("_mamba", "out_proj")

                def __init__(self, d_model, d_state, d_conv, expand_factor) -> None:
                    super().__init__()
    ...
    def _load_mlx_optimizer(self) -> None:
        """Load MLX optimizer lazily."""
        import mlx.optimizers as optim

        self._optimizer = optim.Adam(learning_rate=self.lr)

    def set_sprint_context(
        self, remaining_time_s: float, total_duration_s: float = 300.0, remaining_actions: int = 10
    ) -> None:
    ...
    def predict(
        self, task_type: str, params: dict, system_state: dict
    ...
    async def predict_async(
        ...
        try:
            return await asyncio.to_thread(self._predict_impl, task_type, params, system_state)
        except Exception:
    ...
    def predict_overrun_risk(self, cost_estimate: dict) -> float:
        ...
        time_cost, _ram_cost, _net_cost, _value, uncertainty = self.predict("other", params, sys_state)
    ...
    async def update(
        self, task_type: str, params: dict, system_state: dict, actual: tuple[float, float, float, float]
    ) -> None:
        x_raw = self._build_features(task_type, params, system_state)
