...
class CDXSearchResult(msgspec.Struct, gc=False):
    ...
    def to_finding_dict(self) -> dict:
        return {'source': 'wayback_cdx', 'url': self.original, 'timestamp': self.timestamp, 'mimetype': self.mimetype, 'status_code': self.status_code, 'length': self.length, 'replay_url': self.replay_url}
    ...
    def to_canonical_finding(self, query: str, _sprint_id: str='') -> CanonicalFinding | None:
        ...
        try:
            ...
            return CanonicalFinding(finding_id=f'cdx-{(self.digest[:16] if self.digest else self.timestamp[:12])}', source_type='wayback_cdx', confidence=0.8, query=query[:128], ts=self._parse_timestamp(), payload_text=payload, provenance=(f'url:{self.original}', f'ts:{self.timestamp}', f'mimetype:{self.mimetype}', f'status:{self.status_code}'))
        except Exception:
            return None
...
async def cdx_deep_search(domain: str, session: httpx.AsyncClient, *, match_type: str='domain', from_date: str | None=None, to_date: str | None=None, limit: int=MAX_CDX_RESULTS) -> list[CDXSearchResult]:
    """
    ...
    if match_type == 'domain':
        ...
        url_param = domain
    params: dict[str, Any] = {'url': url_param, 'matchType': match_type, 'output': 'json', 'fl': 'timestamp,original,mimetype,statuscode,length,digest', 'filter': 'statuscode:200', 'collapse': 'urlkey', 'limit': str(limit)}
    if from_date:
...
def _parse_cdx_response(raw: list[list[str]]) -> list[CDXSearchResult]:
    ...
    for row in raw[1:]:
        if len(row) < 6:
            continue
        result = CDXSearchResult(timestamp=row[0], original=row[1], mimetype=row[2], status_code=row[3], length=row[4], digest=row[5] if len(row) > 5 else '')
        results.append(result)
    return results

async def cdx_deep_search_batch(domains: list[str], session: httpx.AsyncClient, *, match_type: str='domain', concurrency: int=3, rate_limit_s: float=RATE_LIMIT_S) -> list[CDXSearchResult]:
    """
...
class WaybackCDXDeepSearch:
    ...
    async def search(self, domains_or_urls: list[str], *, match_type: str='domain', from_date: str | None=None, to_date: str | None=None, limit_per_domain: int=200, concurrency: int=3) -> CDXDeepSearchResult:
        """
