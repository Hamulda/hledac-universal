...
async def resolve_doh(
    ...
    *,
    ...
    for answer in data.get("Answer", []):
        ...
        f = DOHFinding(
            domain=domain, record_type=record_type.value, value=raw_data, ttl=answer.get("TTL", 0), provider=provider
        )
        if record_type == RecordType.TXT:
...
async def full_doh_profile(
    domain: str, session: httpx.AsyncClient, *, limit: int = 500, timeout: float = 10.0
) -> list[DOHFinding]:
    """
...
async def subdomain_probe(
    domain: str, session: httpx.AsyncClient, wordlist: list[str] | None = None, *, timeout: float = 5.0
) -> list[str]:
    """
