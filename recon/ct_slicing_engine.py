...
class CTSlicingEngine:
    ...
    async def slice_domain(
        self, domain: str, providers: list[CTProvider] | None = None, session: Any | None = None
    ) -> CTSlicingResult:
    ...
    async def stop(self) -> None:
        ...
        logger.info(
            f"[CT] Stopped. Stats: {self._stats.total_requests} requests, {self._stats.total_entries_extracted} entries"
        )
...
def create_ct_slicing_engine(
    watch_domains: list[str], cache_dir: str | None = None, ioc_graph: Any | None = None
) -> CTSlicingEngine:
