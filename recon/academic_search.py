...
from hledac.universal.utils.two_pass_pipeline import consumer_fn_to_thread
...
class SourcePerformance(Struct):
    ...
    def update(self, success: bool, response_time_ms: float) -> None:
...
class BaseSourceAdapter(ABC):
    ...
    @abstractmethod
    async def search(
        self, query: str, max_results: int = 10, analysis: QueryAnalysis | None = None
    ) -> list[SearchResult]:
        """Search the source with the given query."""

    async def execute_search(
        self, query: str, max_results: int = 10, analysis: QueryAnalysis | None = None, **kwargs
        ...
        try:
            ...
            execution_time = (time.time() - start_time) * 1000
            self.logger.warning(
                f"[ADAPTER_COMPAT] Search failed for {adapter_name}: error_type={type(e).__name__}, async_session_supported={async_session_supported}, skipped_kwargs={list(skipped_kwargs.keys())}"
            )
        ...
        try:
            ...
            async def _do_search(session: httpx.AsyncClient) -> list[SearchResult]:
                ...
                resp = await session.get(
                    self.base_url, params=params, headers=headers, timeout=httpx.Timeout(self.config.timeout_seconds)
                )
                try:
                    if resp.status_code != 200:
    ...
    async def search(
        ...
        try:
            ...
            async def _do_search(session: httpx.AsyncClient) -> list[SearchResult]:
                ...
                resp = await session.get(
                    url, params=params, headers=headers, timeout=httpx.Timeout(self.config.timeout_seconds)
                )
                try:
...
class AcademicSearchEngine:
    ...
    def __init__(
        self, config: dict[str, Any] | None = None, enable_expansion: bool = True, enable_deduplication: bool = True
    ) -> None:
    ...
    def _init_sources(self) -> None:
        ...
        source_configs = {
            "arxiv": SourceConfig(
                name="arxiv", base_url="http://export.arxiv.org/api/query", max_results=10, rate_limit_per_minute=30
            ),
            "crossref": SourceConfig(
                name="crossref", base_url="https://api.crossref.org/works", max_results=10, rate_limit_per_minute=50
            ),
    ...
    async def search(
        ...
        try:
            ...
            self.logger.info(
                f"Search completed: {len(all_results)} total, {len(ranked_results)} unique from {successful_sources} sources in {execution_time:.0f}ms"
            )
            return result
        except Exception as e:
        ...
        async def search_with_limit(source_name: str, adapter: BaseSourceAdapter, query: str):
            async with semaphore:
                return await adapter.execute_search(
                    query, max_results=adapter.config.max_results, analysis=analysis, async_session=async_session
                )
    ...
    async def _rank_results(self, results: list[SearchResult], query: str) -> list[SearchResult]:
        ...
        for i in range(0, len(results), batch_size):
            batch = results[i : i + batch_size]
            try:
                batch_scored = await parallel_ok(
                    *[asyncio.to_thread(score_one, r) for r in batch], label=f"academic_rank:{i}"
                )
    ...
    async def _deduplicate_and_rank(self, results: list[SearchResult], query: str) -> list[SearchResult]:
        ...
        async def build_dedup_items() -> list[DedupItem]:
            ...
            for i in range(0, len(results), 50):
                ...
                try:
                    batch_items = await parallel_ok(
                        *[make_dedup_item(r) for r in batch], label=f"academic_dedup_rank:build:{i}"
                    )
