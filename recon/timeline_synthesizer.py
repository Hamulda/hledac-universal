...
class SynthesizedTimeline(Struct, frozen=True):
    """
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'entity_id': self.entity_id, 'metadata': {'total_events': self.metadata.total_events, 'oldest_event_ts': self.metadata.oldest_event_ts, 'newest_event_ts': self.metadata.newest_event_ts, 'event_types': self.metadata.event_types, 'sources': self.metadata.sources}, 'events': [e.to_dict() for e in self.events]}
...
class TimelineSynthesizer:
    ...
    """
    __slots__ = tuple(('_events', '_stats'))

    def __init__(self) -> None:
        self._events: list[TimelineEvent] = []
        self._stats: dict[str, int] = {'ct_events_added': 0, 'archive_events_added': 0, 'document_events_added': 0, 'finding_events_added': 0, 'invalid_skipped': 0, 'events_output': 0}
    ...
    def add_ct_events(self, findings: list[Any]) -> int:
        ...
        for f in findings:
            try:
                ...
                event = TimelineEvent(ts=ts, event_type='ct_observed', source='ct_log', description=f'CT cert observed: {entity_id}', entity_id=entity_id, confidence=confidence, evidence=[f'finding:{fid}'] if fid else [])
    ...
    def add_archive_events(self, archive_results: list[Any]) -> int:
        ...
        for ar in archive_results:
            try:
                ...
                event = TimelineEvent(ts=ts, event_type='archive_snapshot', source=source, description=f'Archive snapshot: {title[:60]}', entity_id=url[:128], confidence=0.7, evidence=[url] if url else [])
                self._events.append(event)
                count += 1
            except Exception:
                self._stats['invalid_skipped'] += 1
    ...
    def add_document_timestamps(self, doc_metadata: list[Any]) -> int:
        ...
        for doc in doc_metadata:
            try:
                ...
                event = TimelineEvent(ts=ts, event_type='document_dated', source='metadata_extractor', description=f'Document: {doc_type} created', entity_id=path[:128] if path else doc_type, confidence=0.8, evidence=[path] if path else [])
                self._events.append(event)
                count += 1
            except Exception:
                self._stats['invalid_skipped'] += 1
    ...
    def add_finding_events(self, findings: list[Any], source_label: str='finding') -> int:
        ...
        for f in findings:
            try:
                ...
                query = getattr(f, 'query', '') or ''
                event = TimelineEvent(ts=ts, event_type='finding_accepted', source=source_label, description=f'{src_type} finding: {query[:50]}', entity_id=fid[:32] if fid else '', confidence=getattr(f, 'confidence', 0.5) or 0.5, evidence=[f'finding:{fid}'] if fid else [])
                self._events.append(event)
                count += 1
            except Exception:
                self._stats['invalid_skipped'] += 1
    ...
    def build(self, entity_id: str='') -> SynthesizedTimeline:
        ...
        metadata = TimelineMetadata(total_events=len(valid_events), oldest_event_ts=oldest, newest_event_ts=newest, event_types=event_types, sources=sources)
        return SynthesizedTimeline(events=valid_events, metadata=metadata, entity_id=entity_id)
...
__all__ = ['TimelineSynthesizer', 'TimelineEvent', 'TimelineMetadata', 'SynthesizedTimeline', 'create_timeline_synthesizer', 'MAX_TIMELINE_EVENTS', 'MAX_EVENT_AGE_DAYS']