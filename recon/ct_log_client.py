...
class CTLogClient:
    ...
    __slots__ = tuple(('_cache_dir', '_last_certstream_request', '_last_request', '_lock'))
    ...
    def _extract_candidate_domains(query: str) -> list[str]:
        ...
        domains = [d.lstrip('www.').lower() for d in raw if d.lower() not in _INVALID_DOMAINS and '.' in d and (len(d) < 253)]
        seen: set[str] = set()
        result = []
        for d in domains:
            if d not in seen:
    ...
    async def search(self, query: str, session: httpx.AsyncClient) -> list[dict]:
        ...
        if not domains:
            if query and len(query) > 3:
                logger.info(f'[CT] No domains extracted from query (CT requires domains, not org names). Query: {query[:80]!r}. Consider adding domain names for CT enrichment.')
            else:
        ...
        async with self._lock:
            ...
            try:
                ...
                if raw is None:
                    ...
                    return {'domain': domain, 'san_names': [], 'cert_count': 0, 'issuers': [], 'first_cert': 0.0, 'last_cert': 0.0, 'ct_provider_selected': 'all_failed'}
            finally:
    ...
    async def _fetch_ct_with_fallback(self, domain: str, session: httpx.AsyncClient) -> tuple[list | None, str | None]:
        ...
        if crtsh_decision.allowed:
            ...
            raw, _status, err = await checked_aiohttp_get(session, url, timeout=httpx.Timeout(5.0), failure_kind='crtsh_ct')
        ...
        if not certspotter_decision.allowed:
            logger.warning(f'certspotter.io circuit breaker open ({certspotter_decision.reason}), skipping to crt.sh identity')
        else:
        ...
        if crtsh_identity_decision.allowed:
            ...
            logger.warning(f'CT crt.sh identity {domain}: circuit breaker open ({crtsh_identity_decision.reason}), skipping identity fallback')
        return (None, None)
    ...
    async def _fetch_certspotter(self, domain: str, session: httpx.AsyncClient) -> list | None:
        ...
        raw, _status, err = await checked_aiohttp_get(session, url, timeout=httpx.Timeout(15.0), failure_kind='certspotter_ct')
        if err:
    ...
    async def _fetch_certificates_with_fallback(self, domain: str, session: httpx.AsyncClient) -> list | None:
        ...
        if crtsh_decision.allowed:
            url = self._CRT_SH_URL.format(domain=domain)
            raw, _status, err = await checked_aiohttp_get(session, url, timeout=httpx.Timeout(5.0), failure_kind='crtsh_certs')
        ...
        crtsh_identity_decision = domain_breaker_check('crt.sh.identity')
        if crtsh_identity_decision.allowed:
            raw, _status, err = await checked_aiohttp_get(session, self._CRT_SH_IDENTITY_URL.format(domain=domain), timeout=httpx.Timeout(5.0), failure_kind='crtsh_certs_identity')
            if err:
            ...
            logger.warning(f'CT crt.sh identity fetch_certificates {domain}: circuit breaker open ({crtsh_identity_decision.reason})')
        return None
    ...
    def _parse_certs(self, raw: list) -> list[dict]:
        ...
        for entry in raw:
            try:
                ...
                certs.append({'subject_common_name': cn, 'issuer': issuer_cn, 'valid_from': valid_from, 'valid_to': valid_to, 'alt_names': sorted(set(alt_names))})
            except Exception:
                continue
        return certs
    ...
    def to_canonical_findings(ct_result: dict, query: str) -> list:
        ...
        for san in san_names[:MAX]:
            ...
            findings.append(CanonicalFinding(finding_id=finding_id, query=query, source_type='ct_log', confidence=0.75, ts=ts, provenance=('ct_log', domain), payload_text=msgspec.json.encode({'issuer': issuer, 'cert_count': ct_result.get('cert_count', 0), 'domain': domain, 'san': san})))
        return findings