...
class TemporalAnalyzer:
    ...
    def _analyze_trend(self, timestamps: list[datetime], values: list[float]) -> TrendAnalysis:
        """
    ...
    def _generate_scenarios(self, timestamps: list[datetime], values: list[float], query: str) -> list[Scenario]:
        """
    ...
    def _project_linear(self, values: list[float], slope: float, horizon_days: int) -> list[float]:
        """Simple linear projection."""
        last_value = values[-1]
        return [last_value + slope * day for day in range(1, horizon_days + 1, 7)]

    def _project_arima(self, values: list[float], horizon_days: int) -> list[float]:
        """
        ...
        if len(diff) > 1:
            ...
            ar_coeff = np.clip(ar_coeff, -0.9, 0.9)
        else:
            ar_coeff = 0.0
        ma_coeff = 0.3
        predictions = []
    ...
    def _project_exponential_smoothing(self, values: list[float], horizon_days: int) -> list[float]:
        """
    ...
    def _project_bayesian(self, values: list[float], horizon_days: int) -> dict[str, list[float]]:
        ...
        for step in range(n_steps):
            ...
            pred_var = posterior_var * (1 + step * 0.1)
