...
class DataLeakHunter:
    ...
    BREACH_APIS = {
        ...
        "leaklookup": {"url": "https://leak-lookup.com/api/search", "rate_limit": 1},
        "dehashed": {"url": "https://api.dehashed.com/search", "rate_limit": 2},
        "intelligencex": {"url": "https://2.intelx.io/intel/search", "rate_limit": 2},
        "grep_app": {"url": "https://grep.app/api/search", "rate_limit": 3},
    }
                    ...
                    if data.get("found"):
                        for source in data.get("sources", []):
                            alert = LeakAlert(
                                ...
                                severity=AlertSeverity.HIGH,
                                breach_name=source,
                                leaked_data={"database": source, "total_results": data.get("count", 0)},
                            )
                            alerts.append(alert)
            await asyncio.sleep(config["rate_limit"])
        except Exception as e:
    ...
    async def _check_intelligencex(self, value: str, target_type: str) -> list[LeakAlert]:
        ...
        try:
            async with self._session.post(
                config["url"], headers=headers, json=search_payload, timeout=httpx.Timeout(total=30)
            ) as resp:
                self._api_calls += 1
                if resp.status == 429:
        ...
        async def _poll_once() -> None:
            async with self._session.get(result_url, headers=headers, timeout=httpx.Timeout(total=30)) as resp:
                if resp.status == 429:
                    raise _RateLimitedError("IntelligenceX rate limited")
                if resp.status != 200:
                    ...
                    for hit in hits:
                        try:
                            ...
                            alert = LeakAlert(
                                ...
                                leaked_data={"repo": repo, "file": file, "snippet": snippet},
                                url=src.get("url", ""),
                            )
                            alerts.append(alert)
                        except Exception:
    ...
    async def _process_alert(self, alert: LeakAlert) -> None:
        ...
        alert_hash = hashlib.sha256(f"{alert.target}:{alert.breach_name}:{alert.source.value}".encode()).hexdigest()[
            :16
        ]
        ...
        logger.warning(
            f"🚨 LEAK ALERT [{alert.severity.value.upper()}]: {alert.target} found in {alert.breach_name} ({alert.source.value})"
        )
    ...
    def get_alerts(
        self, target: str | None = None, severity: AlertSeverity | None = None, limit: int = 100
    ) -> list[LeakAlert]:
