...
_SECRET_PATTERNS: list[tuple[str, str]] = [
    ...
    ("\\bsk_live_[0-9a-zA-Z]{24}\\b", "sk_live_[REDACTED]"),
    ("Bearer\\s+[A-Za-z0-9_\\.\\-]{20,}", "Bearer [REDACTED]"),
    ("-----BEGIN[^\\n]+-----", "[REDACTED:PRIVATE KEY]"),
    (
        "(?i)(?:api[_-]?key|secret|password|passwd|token)\\s*[=:]\\s*[\"\\']?[A-Za-z0-9_\\.\\-]{8,32}[\"\\']?",
        "[REDACTED:CREDENTIAL]",
    ),
    ("\\bAIza[0-9A-Za-z\\-_]{35}\\b", "AIza[REDACTED]"),
]
...
def _build_evidence_envelope(
    source: str, evidence_pointers: list[str], signal_facets: dict[str, float], audit_reason: str
) -> str:
...
class LeakSentinelAdapter:
    ...
    async def scan(self, query: str) -> list[CanonicalFinding]:
        ...
        if "/" in query and len(query) > 3:
            t = safe_create_task(
                _fetch_github_secret_findings(query, self._semaphore), name="leak_sentinel:github_findings"
            )
        ...
        logger.debug(
            f"LeakSentinel: {self._stats.findings_produced} findings from {self._stats.sources_succeeded}/{self._stats.sources_run} sources in {self._stats.elapsed_s:.1f}s"
        )
        return all_findings
