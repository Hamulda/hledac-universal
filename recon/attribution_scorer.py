...
class AttributionConfidenceScorer:
    ...
    def _email_domain_match_score(self, left: IdentityCandidate, right: IdentityCandidate) -> AttributionFactor | None:
        """
    ...
    def score_pair(self, left: IdentityCandidate, right: IdentityCandidate, context: dict[str, Any] | None=None) -> AttributionScore:
        ...
        if not self._check_limit():
            return AttributionScore(confidence=0.0, factors=(), evidence_ids=(), factor_weights=self._weights)
    ...
    def score_candidates(self, candidates: list[IdentityCandidate]) -> dict[str, AttributionScore]:
        """
