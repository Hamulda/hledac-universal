...
class CorrelationReport(Struct, frozen=True):
...
class Anomaly(Struct, frozen=True):
...
class SharedContext(Struct, frozen=True):
...
class ComprehensiveReport(Struct, frozen=True):
...
class WorkflowPlan(Struct, frozen=True):
...
class WorkflowOrchestrator:
    ...
    __slots__ = ("_execution_timeline", "_module_registry", "config", "orchestrator")

    def __init__(self, orchestrator: Any, config: IntelligenceConfig | None = None) -> None:
    ...
    def _add_timeline_event(self, event_type: str, details: dict[str, Any]) -> None:
        ...
        self._execution_timeline.append(
            {"timestamp": datetime.now(UTC).isoformat(), "type": event_type, "details": details}
        )
    ...
    async def execute_workflow(self, workflow: WorkflowPlan, input_data: Any) -> ComprehensiveReport:
        ...
        try:
            ...
            self._add_timeline_event(
                "modules_complete", {"completed": len(results), "failed": len(workflow.modules) - len(results)}
            )
            ...
            if self.config.enable_correlation:
                ...
                self._add_timeline_event(
                    ...
                    {"findings": len(correlations.cross_module_findings), "risk_score": correlations.risk_score},
                )
    ...
    async def _execute_parallel(
        self, module_groups: list[list[str]], input_data: Any, context: SharedContext
    ) -> dict[str, Any]:
    ...
    def _generate_report(
        self, results: dict[str, Any], correlations: CorrelationReport, anomalies: list[Anomaly], context: SharedContext
    ) -> ComprehensiveReport:
    ...
    def _generate_recommendations(
        self, results: dict[str, Any], correlations: CorrelationReport, anomalies: list[Anomaly]
    ) -> list[str]:
...
class CorrelationResult(Struct, frozen=True):
...
def _extract_and_correlate_entities(
...
def _compute_cluster_metrics(
    themes: dict[str, list[dict[str, Any]]], normalized: list[dict[str, Any]]
    ...
    for theme, fndgs in themes.items():
        score = sum(
            ...
            for x in fndgs
            if x["severity"].lower() in ("critical", "high")
        )
        if score > 0:
...
def _compute_top_priority_items(
    normalized: list[dict[str, Any]],
...
def _build_correlation_result(
    ...
    return CorrelationResult(
        ...
        what_matters_first=what_matters,
        operator_shortlist=operator_list,
...
def correlate_findings(
    findings: list[dict[str, Any]], *, risk_thresholds: dict[str, float] | None = None, max_themes: int = 10
) -> CorrelationResult:
    ...
    campaign_hints = _find_campaign_hints(normalized, themes)
    _corr_iocs, _camp_conf = _compute_top_priority_items(
        normalized, repeated_iocs, theme_source_overlap, campaign_hints
    )
    ...
    return CorrelationResult(
        ...
        corroborated_iocs=_corr_iocs,
        top_priority_pivots=_get_top_priority_pivots(normalized, dominant_cluster, high_risk_branch, repeated_domains),
        campaign_confidence=_camp_conf,
...
def _extract_entities(
    ...
    from hledac.universal._core.ioc_patterns import DOMAIN_RE, HASH_RE, IPV4_RE, URL_RE
...
def _find_campaign_hints(
    findings: list[dict[str, Any]], _themes: dict[str, list[dict[str, Any]]]
) -> list[dict[str, Any]]:
    ...
    high_conf_findings = [
        f for f in findings if f["confidence"] > 0.8 and f["severity"].lower() in ("high", "critical")
    ]
...
def _find_coupling_pairs(entities: list[dict[str, Any]]) -> list[tuple[str, str]]:
    ...
    for _dtype, dvals in list(by_type.items())[:2]:
        for _itype, ivals in list(by_type.items())[1:]:
            for dv in dvals[:5]:
                for iv in ivals[:5]:
                    pairs.append((dv, iv))
...
def _calc_cross_source_confidence(
    findings: list[dict[str, Any]], theme_source_overlap: dict[str, list[str]], campaign_hints: list[dict[str, Any]]
) -> float:
    ...
    has_repeated = any(
        f.get("description", "")
        for f in findings
        if any(ioc in f.get("description", "").lower() for ioc in ("domain", "ip", "hash", "url"))
    )
...
def _get_top_priority_pivots(
    ...
    if dominant_cluster:
        pivots.append(
            {"pivot_type": "dominant_cluster", "description": f"Primary cluster: {dominant_cluster}", "priority": 1}
        )
...
def _calc_campaign_confidence(
    campaign_hints: list[dict[str, Any]], theme_source_overlap: dict[str, list[str]]
) -> float:
