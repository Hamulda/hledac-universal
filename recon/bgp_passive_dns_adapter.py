...
async def _rate_limited_text(
    session: httpx.AsyncClient, url: str, last_request: float, timeout: float = TIMEOUT_PER_REQUEST
) -> str | None:
...
async def ripestat_lookup_ip(
    ip: str, session: httpx.AsyncClient, rate_limit_ref: float | None = None
) -> BGPFinding | None:
    """
    RIPE Stat API: IP → ASN, prefix, holder, country, org.
...
async def bgptools_prefix_history(
    prefix: str, session: httpx.AsyncClient, rate_limit_ref: float = 0.0
) -> list[BGPFinding]:
    """
...
async def hackertarget_pdns(domain: str, session: httpx.AsyncClient, rate_limit_ref: float = 0.0) -> list[PDNSRecord]:
    ...
    try:
        ...
        if not text or text.startswith("#"):
            logger.debug(
                "PassiveDNS HackerTarget empty response for domain=%r (status_code=%d)", domain, resp.status_code
            )
            return []
        records = []
        for line in text.splitlines()[:MAX_PDNS_RECORDS]:
            ...
            if rec_type in ("A", "AAAA", "MX", "NS", "TXT", "CNAME", "PTR"):
                records.append(
                    PDNSRecord(domain=domain, record_type=rec_type, value=value, source="hackertarget", ts=time.time())
                )
        return records
    except Exception:
        return []
