...
_API_PATTERNS: list[tuple[str, re.Pattern]] = [
    ("aws_access_key", re.compile("\\bAKIA[0-9A-Z]{16}\\b")),
    ("google_api_key", re.compile("\\bAIza[0-9A-Za-z\\-_]{35}\\b")),
    ("stripe_secret_key", re.compile("\\bsk_live_[0-9a-zA-Z]{24}\\b")),
    ("slack_token", re.compile("\\bxox[baprs]-[0-9]{10,13}-[0-9]{10,13}-[A-Za-z0-9]{24,32}\\b")),
    (
        "aws_secret_key",
        re.compile("\\b(?:aws)?_?secret_?access?_?key\\s*[=:]\\s*['\\\"]?([A-Za-z0-9/+=]{40})['\\\"]?", re.IGNORECASE),
    ),
    ("private_key", re.compile("-----BEGIN (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----", re.IGNORECASE)),
    (
        "generic_api_key",
        re.compile(
    ...
    ("anthropic_api_key", re.compile("\\bsk-ant-[a-zA-Z0-9\\-_]{95}\\b")),
    ("openai_api_key_2024", re.compile("\\bsk-proj-[a-zA-Z0-9\\-_]{100}\\b")),
    ("huggingface_token", re.compile("\\bhf_[a-zA-Z0-9]{34}\\b")),
    ("doppler_secret", re.compile("\\bdp\\.pt\\.[a-zA-Z0-9]{43}\\b")),
    ("infisical_token", re.compile("\\binf-[a-zA-Z0-9]{43}\\b")),
    ("vercel_token", re.compile("\\b[a-zA-Z0-9]{24,}\\b")),
    ("supabase_service_key", re.compile("\\beyJ[a-zA-Z0-9\\-_]+\\.[a-zA-Z0-9\\-_]+\\.[a-zA-Z0-9\\-_]+\\b")),
]
...
async def _gh_get(url: str, session: httpx.AsyncClient, params: dict | None = None) -> dict | None:
    ...
    headers = {"Accept": "application/vnd.github.v3+json", "User-Agent": "hledac-osint/1.0"}
    data, status_code, err = await checked_aiohttp_get(
        session, url, params=params, headers=headers, timeout=httpx.Timeout(total=30), failure_kind="github_api"
    )
...
def _scan_line_for_secrets(line: str, file_path: str, line_no: int) -> list[SecretFinding]:
    ...
    for pattern_label, compiled_re in _API_PATTERNS:
        ...
        for _ in matches:
            ...
            findings.append(
                SecretFinding(pattern=pattern_label, file_path=file_path, line=line_no, context=masked_line)
            )
    return findings
                ...
                for line_no, line in enumerate(patch.splitlines(), start=1):
                    ...
                    for pattern_label, compiled_re in _API_PATTERNS:
                        ...
                        for _ in matches:
                            ...
                            yield SecretFinding(
                                pattern=pattern_label, file_path=file_path, line=line_no, context=masked_line
                            )
                            logger.debug(
                                f"GitHub secret in {repo_full_name} commit {file_path}:{line_no} pattern={pattern_label}"
                            )
    finally:
        ...
        for pattern_label, compiled_re in _API_PATTERNS:
            ...
            async def _fetch_one(item: dict) -> tuple[dict, str | None]:
                async with _fetch_sem:
                    item.get("path") or item.get("name") or "unknown"
                ...
                if content:
                    for line_no, line in enumerate(content.splitlines(), start=1):
                        ...
                        for _secret in matches:
                            ...
                            logger.debug(
                                f"GitHub secret in {repo_full_name}/{file_path}:{line_no} pattern={pattern_label} context={masked_line[:80]}"
                            )
                else:
                    findings.append(
                        SecretFinding(
                            pattern=pattern_label, file_path=file_path, line=0, context=f"[found in {html_url}]"
                        )
                    )
    return findings
