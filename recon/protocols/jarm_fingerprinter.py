...
class _JARMFingerprinter:
    ...
    def _build_payloads(self) -> list[bytes]:
        ...
        payloads.append(self._build_client_hello(self.TLS_1_2, ciphers, extensions=None))
        ...
        payloads.append(self._build_client_hello(self.TLS_1_2, ciphers, extensions=True))
        ...
        ciphers = self.CIPHERS_FORWARD[:80]
        payloads.append(self._build_client_hello(self.TLS_1_1, ciphers, extensions=None))

        # 6. TLS 1.3, ALL ciphers forward, no extensions
        ciphers = self.CIPHERS_FORWARD[:80]
        payloads.append(self._build_client_hello(self.TLS_1_3, ciphers, extensions=None))

        # 7. TLS 1.2, NO_GREASE forward, ALPN=h2
        ciphers_no_grease = [c for c in self.CIPHERS_FORWARD[:80] if c not in self.GREASE_VALUES]
        payloads.append(self._build_client_hello(self.TLS_1_2, ciphers_no_grease, alpn=h2_ext))

        # 8. TLS 1.2, ALL ciphers reverse, ALPN=http/1.1
        ciphers = self.CIPHERS_REVERSE[:80]
        payloads.append(self._build_client_hello(self.TLS_1_2, ciphers, alpn=http11_ext))
        ...
        ciphers = self.CIPHERS_REVERSE[:80]
        payloads.append(self._build_client_hello(self.TLS_1_3, ciphers, extensions=True))

        return payloads

    def _build_client_hello(
        self, tls_version: bytes, ciphers: list[int], extensions: bool | None = None, alpn: bytes | None = None
    ) -> bytes:
        ...
        random_bytes = bytes(
            [(int(time.time()) >> (8 * i)) & 0xFF for i in range(4)] + [_JARM_RNG.randint(0, 255) for _ in range(28)]
        )
    ...
    def _send_tls_packet(self, domain: str, port: int, payload: bytes) -> bytes | None:
    ...
    def _get_cached(self, domain: str) -> str | None:
        ...
        try:
            conn = self._get_db()
            cursor = conn.execute("SELECT hash, ts FROM jarm_cache WHERE domain = ?", (domain,))
            row = cursor.fetchone()
            if row:
                jarm_hash, ts = row
