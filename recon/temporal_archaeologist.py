...
class ArchivedVersion(msgspec.Struct, gc=False):
...
class EntitySnapshot(msgspec.Struct, gc=False):
...
class IdentityChange(msgspec.Struct, gc=False):
...
class TemporalGap(msgspec.Struct, frozen=True, gc=False):
...
class EntityTimeline(msgspec.Struct, frozen=True, gc=False):
...
class TemporalAnomaly(msgspec.Struct, frozen=True, gc=False):
...
class TemporalCorrelation(msgspec.Struct, frozen=True, gc=False):
...
class RecoveryResult(msgspec.Struct, frozen=True, gc=False):
...
class TemporalArchaeologist:
    ...
    async def recover_deleted_content(self, url: str, sources: list[str] | None=None, from_date: datetime | None=None, to_date: datetime | None=None, include_content: bool=True) -> RecoveryResult:
        ...
        results = await parallel_ok(*tasks, label='temporal_archaeologist:391')
    ...
    def detect_temporal_anomalies(self, timeline: EntityTimeline) -> list[TemporalAnomaly]:
        ...
        if not timeline.snapshots:
            return anomalies
        for _detector_name, _detector_fn in [('disappearance', self._detect_disappearances), ('content_wipe', self._detect_content_wipes), ('activity_gap', self._detect_activity_gaps), ('sudden_change', self._detect_sudden_changes), ('frequency_shift', self._detect_frequency_shifts)]:
            try:
    ...
    async def cross_temporal_correlation(self, entity_a: str, entity_b: str, id_type: str='url') -> TemporalCorrelation:
        ...
        results = await parallel_ok(self.reconstruct_version_history(entity_a, id_type), self.reconstruct_version_history(entity_b, id_type), label='cross_temporal_correlation:reconstruct')
        if len(results) < 2:
            logger.warning(f'cross_temporal_correlation: only {len(results)}/2 timelines succeeded for {entity_a} vs {entity_b}')
            return TemporalCorrelation(entity_a=entity_a, entity_b=entity_b, correlation_score=0.0, overlapping_periods=[], shared_attributes={}, temporal_proximity=[])
    ...
    async def deep_historical_search(self, query: str, time_range: tuple[datetime, datetime], sources: list[str] | None=None) -> list[ArchivedVersion]:
        ...
        async def _search_source(source: str) -> tuple[str, list[ArchivedVersion]]:
            ...
            try:
                ...
                return (source, [])
        results_gathered = await parallel_ok(*[_search_source(s) for s in sources], label='deep_historical_search')
        all_results = []
        ...
        try:
            async with self._session.get(self.WAYBACK_CDX_URL, params=params) as resp:
                ...
                if include_content:
                    ...
                    results = await parallel_ok(*tasks, label='temporal_archaeologist:_recover_from_wayback')
