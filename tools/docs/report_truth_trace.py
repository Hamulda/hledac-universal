...
class BoundarySnapshot(msgspec.Struct):
...
class TraceResult(msgspec.Struct, frozen=True):
    verdict: str
...
def trace_verdict(snapshots: dict, raw_benchmark: dict | None=None, raw_internal: dict | None=None) -> tuple[str, str | None, dict]:
    ...
    extended = {'terminality_satisfied': None, 'missing_lanes': None, 'benchmark_shape_gaps': None, 'internal_runtime_failures': None, 'terminality_source_outcome_mismatch': None}
    if bench is None:
    ...
    if bench_acq is not None and bench_acq != 'MISSING' and (bench_acq != 'null'):
        ...
        terminality_ok = term_checked not in (None, 'MISSING', 'null', False) and term_satisfied not in (None, 'MISSING', 'null', False) and (term_satisfied is True)
        gaps = []
...
def _find_terminality_stale_lanes(raw_internal: dict | None) -> list:
...
def trace_boundaries(benchmark_path: str, validation_path: str | None, output_json_path: str, output_md_path: str) -> TraceResult:
    ...
    if benchmark is None:
        ...
        result = TraceResult(verdict=verdict, drop_boundary=drop, boundary_snapshots={}, acquisition_missing_in=[], terminality_state={}, details={'error': f'benchmark JSON not readable: {benchmark_path}'}, **extended)
    else:
        ...
        result = TraceResult(verdict=verdict, drop_boundary=drop, boundary_snapshots={k: snap.present if snap else None for k, snap in snapshots.items()}, acquisition_missing_in=missing_in, terminality_state=terminality, details={'measurement_id': benchmark.get('measurement_id'), 'status': benchmark.get('status'), 'run_quality_verdict': benchmark.get('run_quality_verdict'), 'benchmark_has_acquisition_report': benchmark.get('acquisition_report') is not None, 'internal_report_has_acquisition_report': internal_raw.get('acquisition_report') is not None if internal_raw else False, 'validation_verdict': validation_path and (load_json(validation_path) or {}).get('overall_verdict')}, **extended)
    output_data = {'verdict': result.verdict, 'drop_boundary': result.drop_boundary, 'measurement_id': result.details.get('measurement_id'), 'status': result.details.get('status'), 'run_quality_verdict': result.details.get('run_quality_verdict'), 'acquisition_report_in_benchmark': result.details.get('benchmark_has_acquisition_report'), 'acquisition_report_in_internal': result.details.get('internal_report_has_acquisition_report'), 'boundary_snapshots': result.boundary_snapshots, 'acquisition_missing_in': result.acquisition_missing_in, 'terminality_state': result.terminality_state, 'validation_verdict': result.details.get('validation_verdict'), 'terminality_satisfied': result.terminality_satisfied, 'missing_lanes': result.missing_lanes, 'benchmark_shape_gaps': result.benchmark_shape_gaps, 'internal_runtime_failures': result.internal_runtime_failures, 'terminality_source_outcome_mismatch': result.terminality_source_outcome_mismatch}
