...
def trace_verdict(
    snapshots: dict, raw_benchmark: dict | None = None, raw_internal: dict | None = None
        ...
        if not terminality_ok:
            if stale_lanes:
                return (
                    ...
                    "internal_report_json (terminality snapshot stale before nonfeed predispatch)",
                    extended,
                )
        ...
        if internal is not None:
            ...
            if not _is_nullish(int_acq):
                return (
                    ...
                    "benchmark_json (internal report has it, benchmark doesn't)",
                    extended_base,
                )
            if _is_nullish(int_acq):
                return (
                    "TRACE_DROP_BEFORE_EXPORT",
                    "internal_report_json (scheduler never populated acquisition_report)",
                    extended_base,
                )
        return ("TRACE_DROP_BEFORE_EXPORT", "scheduler (acquisition_report never written to benchmark)", extended_base)
    ...
    if validation is not None:
        ...
        if val_failures and all("acquisition_report" in f.get("field_path", "") for f in val_failures):
            return (
                ...
                "validation_json (only validator field paths, no real acquisition data)",
                extended_base,
            )
...
def _extract_attempted_lanes(acq: dict) -> set[str]:
    ...
    return {
        ...
        if isinstance(outcome, dict) and outcome.get("family") and outcome.get("attempted") is True
    }
...
def _build_result_details(
    ...
    return {
        ...
        "internal_report_has_acquisition_report": (bool(internal_raw and internal_raw.get("acquisition_report"))),
