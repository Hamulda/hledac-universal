...
class APIDocGenerator:
    ...
    __slots__ = ("class_hierarchy", "cross_references", "doc_categories", "modules", "package_path")

    def __init__(self, package_path: str = "hledac") -> None:
    ...
    def parse_function(
        self, node: ast.FunctionDef | ast.AsyncFunctionDef, module_name: str, class_name: str | None = None
    ):
    ...
    def generate_markdown_documentation(self, output_dir: str = "docs") -> None:
    ...
    def generate_api_reference(self, output_path: Path) -> None:
        ...
        for module in self.modules.values():
            for cls in module.classes:
                if any(name in cls.name.lower() for name in ["agent", "orchestrator", "manager", "system"]):
                    ...
                    content.append(
                        f"| [{cls.name}](api/{self.categorize_module(module)}.md#{cls.name.lower()}) | {description[:100]}... | {module.name} |"
                    )
        api_content = "\n".join(content)
        with open(output_path / "API_REFERENCE_GENERATED.md", "w", encoding="utf-8") as f:
            f.write(api_content)

    def generate_category_doc(self, category: str, modules: list[APIModule], output_path: Path) -> None:
    ...
    def generate_class_doc(self, cls: APIClass, content: list[str]) -> None:
        ...
        if cls.properties:
            ...
            for prop in cls.properties:
                content.append(
                    f"- `{prop.name}`: {prop.type_hint}" + (f" - {prop.docstring}" if prop.docstring else "")
                )
            content.append("")
        if cls.methods:
    ...
    def generate_function_doc(self, func: APIMethod, content: list[str]) -> None:
        """Generate documentation for a function."""
        self.generate_method_doc(func, content)

    def generate_method_doc(self, method: APIMethod, content: list[str]) -> None:
    ...
    def generate_cross_reference_index(self, output_path: Path) -> None:
        ...
        for name, module_name, cls in all_classes:
            content.append(
                f"- [{name}](api/{self.categorize_module(self.modules[module_name])}.md#{name.lower()}) - `{module_name}`"
            )
        content.append("")
        content.append("### Functions")
        content.append("")
    ...
    def generate_examples(self, output_path: Path) -> None:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate API documentation for Hledac", suggest_on_error=True, color=True
    )
