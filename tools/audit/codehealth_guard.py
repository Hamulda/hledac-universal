...
class GuardResult(msgspec.Struct, gc=False):
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'verdict': self.verdict.value, 'function_name': self.function_name, 'explicit_args': self.explicit_args, 'source_lines': self.source_lines, 'is_wrapper_delegate': self.is_wrapper_delegate, 'has_input_dataclass': self.has_input_dataclass, 'rule_helper_count': self.rule_helper_count, 'has_rule_classes': self.has_rule_classes, 'compatibility_wrapper_detected': self.compatibility_wrapper_detected, 'owner_delegated_detected': self.owner_delegated_detected, 'owner_imported_detected': self.owner_imported_detected, 'owner_module': self.owner_module, 'imported_symbol': self.imported_symbol, 'error_message': self.error_message}
...
def _count_explicit_args(func_node: ast.FunctionDef | ast.AsyncFunctionDef) -> int:
    ...
    return sum((1 for arg in func_node.args.args if arg.arg != 'self'))
...
def run_guard(file_path: str, symbol: str) -> GuardResult:
    """Run the code-health guard on a specific function in a file."""
    path = Path(file_path)
    if not path.exists():
        return GuardResult(verdict=GuardVerdict.FAIL_FILE_NOT_FOUND, function_name=symbol, explicit_args=0, source_lines=0, is_wrapper_delegate=False, has_input_dataclass=False, rule_helper_count=0, has_rule_classes=False, error_message=f'File not found: {file_path}')
    source_text = path.read_text(encoding='utf-8')
    try:
        tree = ast.parse(source_text)
    except SyntaxError as e:
        return GuardResult(verdict=GuardVerdict.FAIL_SYNTAX_ERROR, function_name=symbol, explicit_args=0, source_lines=0, is_wrapper_delegate=False, has_input_dataclass=False, rule_helper_count=0, has_rule_classes=False, error_message=f'Syntax error in source file: {e}')
    ...
    if func_node is None:
        ...
        if owner_module is not None:
            return GuardResult(verdict=GuardVerdict.PASS_OWNER_IMPORTED, function_name=symbol, explicit_args=0, source_lines=0, is_wrapper_delegate=False, has_input_dataclass=False, rule_helper_count=0, has_rule_classes=False, compatibility_wrapper_detected=False, owner_delegated_detected=False, owner_imported_detected=True, owner_module=owner_module, imported_symbol=imported_symbol, error_message=None)
        return GuardResult(verdict=GuardVerdict.FAIL_SYMBOL_MISSING, function_name=symbol, explicit_args=0, source_lines=0, is_wrapper_delegate=False, has_input_dataclass=False, rule_helper_count=0, has_rule_classes=False, error_message=f"Function '{symbol}' not found in {file_path}")
    ...
    if is_owner_delegated:
        return GuardResult(verdict=GuardVerdict.PASS_OWNER_DELEGATED, function_name=symbol, explicit_args=explicit_args, source_lines=source_lines, is_wrapper_delegate=is_wrapper, has_input_dataclass=has_input_dc, rule_helper_count=rule_helper_count, has_rule_classes=has_rule_classes, compatibility_wrapper_detected=False, owner_delegated_detected=True, error_message=None)
    if explicit_args > 8:
        if is_compat_wrapper:
            return GuardResult(verdict=GuardVerdict.PASS_COMPAT_WRAPPER, function_name=symbol, explicit_args=explicit_args, source_lines=source_lines, is_wrapper_delegate=is_wrapper, has_input_dataclass=has_input_dc, rule_helper_count=rule_helper_count, has_rule_classes=has_rule_classes, compatibility_wrapper_detected=True, error_message=None)
        return GuardResult(verdict=GuardVerdict.FAIL_TOO_MANY_ARGS, function_name=symbol, explicit_args=explicit_args, source_lines=source_lines, is_wrapper_delegate=is_wrapper, has_input_dataclass=has_input_dc, rule_helper_count=rule_helper_count, has_rule_classes=has_rule_classes, error_message=f'Function has {explicit_args} explicit args (max 8)')
    if source_lines > 80 and (not is_wrapper):
        return GuardResult(verdict=GuardVerdict.FAIL_TOO_LONG, function_name=symbol, explicit_args=explicit_args, source_lines=source_lines, is_wrapper_delegate=is_wrapper, has_input_dataclass=has_input_dc, rule_helper_count=rule_helper_count, has_rule_classes=has_rule_classes, error_message=f'Function has {source_lines} source lines (max 80)')
    if has_rule_classes:
        return GuardResult(verdict=GuardVerdict.FAIL_POLICY_CLASS_OVERENGINEERING, function_name=symbol, explicit_args=explicit_args, source_lines=source_lines, is_wrapper_delegate=is_wrapper, has_input_dataclass=has_input_dc, rule_helper_count=rule_helper_count, has_rule_classes=has_rule_classes, error_message='Policy class overengineering detected (.*Rule class names found)')
    if not has_input_dc:
        return GuardResult(verdict=GuardVerdict.FAIL_MISSING_INPUT_DATACLASS, function_name=symbol, explicit_args=explicit_args, source_lines=source_lines, is_wrapper_delegate=is_wrapper, has_input_dataclass=has_input_dc, rule_helper_count=rule_helper_count, has_rule_classes=has_rule_classes, error_message='NextActionInput dataclass not found in source')
    if rule_helper_count < 4:
        return GuardResult(verdict=GuardVerdict.FAIL_MISSING_RULE_HELPERS, function_name=symbol, explicit_args=explicit_args, source_lines=source_lines, is_wrapper_delegate=is_wrapper, has_input_dataclass=has_input_dc, rule_helper_count=rule_helper_count, has_rule_classes=has_rule_classes, error_message=f'Only {rule_helper_count} _rule_* helpers found (min 4)')
    return GuardResult(verdict=GuardVerdict.PASS, function_name=symbol, explicit_args=explicit_args, source_lines=source_lines, is_wrapper_delegate=is_wrapper, has_input_dataclass=has_input_dc, rule_helper_count=rule_helper_count, has_rule_classes=has_rule_classes)

def _render_markdown(result: GuardResult) -> str:
    status_icon = '✅' if result.verdict in (GuardVerdict.PASS, GuardVerdict.PASS_COMPAT_WRAPPER, GuardVerdict.PASS_OWNER_DELEGATED, GuardVerdict.PASS_OWNER_IMPORTED) else '❌'
    lines = [f'# NextAction Code Health Guard — `{result.function_name}`', '', f'**Verdict:** {status_icon} `{result.verdict.value}`', '', '## Metrics', f'- Explicit args: {result.explicit_args} (max 8, compat wrapper exception applies if >8)', f'- Source lines: {result.source_lines} (max 80, unless wrapper delegate)', f'- Is wrapper delegate: {result.is_wrapper_delegate}', f'- Is compat wrapper: {result.compatibility_wrapper_detected}', f'- Is owner delegated: {result.owner_delegated_detected}', f'- Is owner imported: {result.owner_imported_detected}', f"- Owner module: {result.owner_module or 'N/A'}", f"- Imported symbol: {result.imported_symbol or 'N/A'}", f'- Has NextActionInput dataclass: {result.has_input_dataclass}', f'- _rule_* helper count: {result.rule_helper_count} (min 4)', f'- Has .*Rule classes: {result.has_rule_classes}', '']
    if result.error_message:
        lines.append(f'**Error:** {result.error_message}\n')
    lines.append('## Verdict Definition\n')
    verdicts = {GuardVerdict.PASS: 'Function passes all code-health checks.', GuardVerdict.PASS_COMPAT_WRAPPER: 'Function has >8 args but is a thin compatibility wrapper (NextActionInput construction + rule helper delegation).', GuardVerdict.PASS_OWNER_DELEGATED: 'Function delegates to the canonical next_action owner (live_measurement_next_action.py). Target file is the compatibility shim, not the implementation.', GuardVerdict.PASS_OWNER_IMPORTED: 'Symbol not locally defined because ownership moved to benchmarks.live_measurement_next_action.py.', GuardVerdict.FAIL_TOO_MANY_ARGS: 'Function has too many explicit arguments (>8) and is not an acceptable compatibility wrapper.', GuardVerdict.FAIL_TOO_LONG: 'Function source exceeds 80 lines and is not a wrapper delegate.', GuardVerdict.FAIL_POLICY_CLASS_OVERENGINEERING: 'Policy class overengineering detected (.*Rule class names).', GuardVerdict.FAIL_MISSING_INPUT_DATACLASS: 'NextActionInput dataclass not found in source.', GuardVerdict.FAIL_MISSING_RULE_HELPERS: f'Only {result.rule_helper_count} _rule_* helpers found (min 4).', GuardVerdict.FAIL_FILE_NOT_FOUND: 'Source file does not exist — guard cannot inspect.', GuardVerdict.FAIL_SYNTAX_ERROR: 'Source file has syntax errors — guard cannot parse.', GuardVerdict.FAIL_SYMBOL_MISSING: 'Target function/symbol not found in source file.'}
...
def main(argv: list[str] | None=None) -> int:
    parser = argparse.ArgumentParser(description='Hermetic code-health guard for NextAction functions.')
