...
def _load_file_content(
    repo_root: Path, file_path: Path, claim_id: str, original_text: str
...
def check_ane_embedder_not_implemented(repo_root: Path) -> list[ClaimResult]:
    ...
    results, content = _load_file_content(
        repo_root, repo_root / "brain" / "ane_embedder.py", "P1-ane_embedder", original_text
    )
    if content is None:
        return results
    in_embed = False
    ...
    if not has_raise_in_embed:
        results.append(
            ClaimResult(
                ...
                evidence="F228B: embed() method rewritten. No 'raise NotImplementedError' found in embed(). Three inference paths: CoreML → fallback embedder → hash fallback. Truthful docstring: 'Sprint F228B: Truthful embed — no NotImplementedError in production.'",
...
def check_worker_pool_module_level_process_pool_executor(repo_root: Path) -> list[ClaimResult]:
    ...
    results, content = _load_file_content(
        repo_root, repo_root / "utils" / "worker_pool.py", "P2-worker_pool", original_text
    )
    if content is None:
        return results
    ...
    if has_module_level_executor and has_deprecation_comment:
        results.append(
            ClaimResult(
                ...
                original_text=original_text + " — ProcessPoolExecutor singleton",
                status=ClaimStatus.LEGACY_DEPRECATED,
                evidence="Module-level _executor singleton exists but is lazy (None until first get_executor() call). No processes spawned on import. File itself declares DEPRECATED with 'zero callers' and 'kept for potential future ThreadPoolExecutor migration'. F224A already deprecated it — confirmed by file header comment.",
...
def check_claims_coordinator_placeholder(repo_root: Path) -> list[ClaimResult]:
    ...
    results, content = _load_file_content(
        repo_root,
        repo_root / "coordinators" / "claims_coordinator.py",
        "P4-claims_coordinator-placeholder",
        original_text,
    )
    if content is None:
        return results
        ...
        if has_extract_claims and has_process_evidence and has_derive_confidence:
            results.append(
                ClaimResult(
                    ...
                    original_text=original_text,
                    status=ClaimStatus.FIXED,
                    evidence="Placeholder comment exists but method _extract_claims() is fully implemented (220+ lines) with deterministic claim extraction, polarity derivation, confidence scoring, bounded output. _derive_confidence() implements heuristic confidence with URL/provenance/title-agreement bonuses.",
...
def check_discovery_planner_provider_capability_state(repo_root: Path) -> list[ClaimResult]:
    ...
    results, content = _load_file_content(
        repo_root,
        repo_root / "discovery" / "discovery_planner.py",
        "P3-discovery_planner_provider_state",
        original_text,
    )
    if content is None:
        return results
    ...
    if has_provider_capability_state and has_production_state:
        results.append(
            ClaimResult(
                ...
                original_text=original_text,
                status=ClaimStatus.FIXED,
                evidence="ProviderCapabilityState Enum exists with PRODUCTION, ADVISORY_STUB, NOT_WIRED, DISABLED states. get_provider_state() function resolves provider capability with proper priority logic. discovery_planner.py is now fully wired with ProviderCapabilityState.",
...
def check_confidence_policy_seam(repo_root: Path) -> list[ClaimResult]:
    ...
    results, content = _load_file_content(
        repo_root, repo_root / "intelligence" / "confidence_policy.py", "P3-confidence_policy_seam", original_text
    )
    if content is None:
        return results
    ...
    if has_compute_confidence and has_source_baselines and has_planner_constant:
        results.append(
            ClaimResult(
                ...
                original_text=original_text,
                status=ClaimStatus.FIXED,
                evidence="F224D canonical seam: compute_confidence() is fully implemented with source baselines (FEED=0.65, PUBLIC=0.60, CT=0.70, WAYBACK=0.55, PASSIVE_DNS=0.68, SOCIAL=0.50, PLANNER=0.75, STEALTH=0.58), bonuses (PROVENANCE_BONUS=0.05, IOC_BONUS=0.10, CORROBORATION_BONUS=0.05), rejection penalty, and hard-clamp to [MIN_CONFIDENCE, MAX_CONFIDENCE]. __all__ exports all constants and the function.",
...
def check_social_identity_miner_if_false(repo_root: Path) -> list[ClaimResult]:
    ...
    original_text = (
        "intelligence/social_identity_miner.py:31 'if False: from ..knowledge.duckdb_store import DuckDBShadowStore'"
    )
    results, content = _load_file_content(
        repo_root,
        repo_root / "intelligence" / "social_identity_miner.py",
        "P4-social_identity_miner_if_false",
        original_text,
    )
    if content is None:
        return results
    ...
    if has_if_false:
        results.append(
            ClaimResult(
                ...
                original_text=original_text + " — dead import",
                status=ClaimStatus.OPEN,
...
def check_sidecar_bus_if_false(repo_root: Path) -> list[ClaimResult]:
    ...
    original_text = (
        "runtime/sidecar_bus.py:36 'if False: from hledac.universal.knowledge.duckdb_store import DuckDBShadowStore'"
    )
    results, content = _load_file_content(
        repo_root, repo_root / "runtime" / "sidecar_bus.py", "P4-sidecar_bus_if_false", original_text
    )
    if content is None:
        return results
    has_if_false = re.search("if\\s+False\\s*:", content) is not None
