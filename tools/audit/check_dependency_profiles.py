...
SYNC_PROFILES: dict[str, ProfileCheck] = {
    ...
    "osint-html": ProfileCheck(
        name="osint-html", uv_sync_args=["--extra", "osint-html"], import_smoke=["selectolax", "curl_cffi"]
    ),
    "transport": ProfileCheck(name="transport", uv_sync_args=["--extra", "transport"], import_smoke=["h2"]),
    "dev": ProfileCheck(name="dev", uv_sync_args=["--extra", "dev"], import_smoke=["pytest", "ruff", "mypy"]),
}
NO_SYNC_PROFILES: dict[str, ProfileCheck] = {
    "no-torch-default": ProfileCheck(
        name="no-torch-default", uv_sync_args=[], import_smoke=[], verify_fn="_check_torch_not_in_default"
    ),
    "no-browser-default": ProfileCheck(
        name="no-browser-default", uv_sync_args=[], import_smoke=[], verify_fn="_check_browser_not_in_default"
    ),
}
...
def get_uv_tracked_packages() -> set[str]:
    ...
    result = subprocess.run(
        ["uv", "pip", "list", "--format=freeze"], capture_output=True, text=True, cwd=str(_PROJECT_ROOT)
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--profile", action="append", dest="profiles", help="Profile to check (can be repeated). Default: all profiles."
    )
    parser.add_argument(
        "--drift", action="store_true", help="Run drift check only (compare uv pip list vs site-packages)."
    )
    parser.add_argument(
        "--strict", action="store_true", help="Make drift check a failure (exit 1 if untracked packages found)."
    )
