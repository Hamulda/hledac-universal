...
class OSINTFrameworkRunner:
    ...
    async def run_theharvester(self, target: str) -> list[dict]:
        ...
        try:
            proc_check = await asyncio.create_subprocess_exec(
                "theHarvester", "--help", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
    ...
    async def run_sherlock(self, username: str) -> list[dict]:
        ...
        try:
            proc_check = await asyncio.create_subprocess_exec(
                "sherlock", "--help", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
            async with asyncio.timeout(5):
                await proc_check.communicate()
        except (TimeoutError, FileNotFoundError) as e:
    ...
    async def run_maigret(self, username: str) -> list[dict]:
        ...
        try:
            proc_check = await asyncio.create_subprocess_exec(
                "maigret", "--help", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
            async with asyncio.timeout(5):
                await proc_check.communicate()
        except (TimeoutError, FileNotFoundError) as e:
