...
def _find_local_onnx() -> Path | None:
    ...
    candidates = [Path('~/.hledac/models/bge-small-en-v1.5.onnx').expanduser(), Path('models/bge-small-en-v1.5.onnx'), Path('/tmp/bge-small-en-v1.5.onnx')]
    for p in candidates:
        if p.exists():
            return p
    return None

def export_bge_to_coreml(model_id: str=DEFAULT_MODEL_ID, output_path: Path | None=None, compute_units: ComputeUnit=ComputeUnit.ALL, quantize: bool=False, dry_run: bool=False) -> Path | dict:
    """
    ...
    if dry_run:
        result = {'status': 'ok', 'model_id': model_id, 'embedding_dim': embedding_dim, 'compute_units': str(compute_units), 'quantize': quantize, 'output_path': str(output_path)}
    ...
    mlmodel = ct.convert(traced, compute_units=compute_units, source='pytorch')
    if quantize:
