...
_PROVIDER_SURFACE_ALIASES = {
    ...
    "probe_f217d_ct_provider_resilience": [("probe_f219e_ct_provider_cooldown", "ct_provider_cooldown.json")],
}
...
def _collect_required_probes(repo_root: Path, acc: _GateAccumulator) -> tuple[_GateAccumulator, _ContractProbeState]:
    ...
    checked = {
        ...
        "probe_f216i_zero_findings_quality": {
            ...
            "detail": zf_quality.data.get("confirmation_zero_findings_stay_failed", {}).get("grade")
            if zf_quality.found
            else None,
        },
    ...
    if not mig_report.found:
        acc = acc.add_required("probe_m218e_memory_integration_guard").add_reason(
            "BLOCKED_BY_CONTRACT: memory integration guard missing"
        )
    elif not mig_pass:
    ...
    if not nrg_report.found:
        acc = acc.add_required("probe_f216h_nonfeed_recovery_guard").add_reason(
            "BLOCKED_BY_CONTRACT: nonfeed recovery guard missing"
        )
    elif not nrg_pass:
...
def _collect_optional_probes(repo_root: Path, acc: _GateAccumulator) -> tuple[_GateAccumulator, _OptionalProbeState]:
    ...
    acc = acc.update_checked(
        {
            "probe_f219e_ct_provider_cooldown": {
                "found": ct_cooldown_report.found if ct_cooldown_report else False,
                "pass": ct_cooldown[0],
                "detail": ct_cooldown[1],
            }
        }
    )
    if ct_cooldown_report and not ct_cooldown_report.found:
        acc = acc.add_optional("probe_f219e_ct_provider_cooldown").add_warning(
            "optional CT cooldown report absent — skipped"
        )
    ...
    if hmf_report and not hmf_report.found:
        acc = acc.add_optional("probe_f219b_hermes_metal_finalizer").add_warning(
            "optional Hermes Metal finalizer absent — skipped"
        )
    ...
    if pss_report and not pss_report.found:
        acc = acc.add_optional("probe_f219d_public_session_seal").add_warning(
            "optional public session seal absent — skipped"
        )
    ...
    optional_state = _OptionalProbeState(
        ...
        surface_contract=(sc_pass, sc_detail, sc_report),
...
def _compute_capability_flags(
    ...
    capability_blocked = (
        blocks["has_provider_surface"]
        or blocks["has_memory"]
        or blocks["f224_blocks"]
        or blocks["f231_blocks"]
        or fallback_blocked
    )
...
def run_gate(repo_root: Path, profile: str, query: str) -> DecisionResult:
    ...
    feed_baseline_allowed = (
        not blocks["has_provider_surface"]
        and not blocks["has_memory"]
        and swap_policy_tier in ("clean", "diagnostic")
        and not acc.missing_required
    )
    ...
    next_action_feed = (
        live_cmd
        if feed_baseline_allowed
        else (highswap_cmd if swap_policy_tier != "hard_block" else "restart required — memory pressure")
    )
    ...
    return DecisionResult(
        ...
        missing_required_reports=acc.missing_required,
        missing_optional_reports=acc.missing_optional,
        uma=uma,
        checked_reports=acc.checked,
        ...
        nonfeed_block_reason=_build_nonfeed_block_reason(blocks["f224_blocks"], blocks["f231_blocks"], profile),
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Pre-Live Decision Gate — Sprint F219F", formatter_class=argparse.RawDescriptionHelpFormatter
    )
...
def _render_alias_table_section(result: DecisionResult) -> list[str]:
    ...
    lines = [
        ...
        "---",
        "",
        "## Provider Surface Alias Table (F217→F219)",
        "",
...
def main() -> int:
    ...
    if args.write_report:
        out_clean = {
            ...
            "checked_reports": {
                k: {kk: vv for kk, vv in v.items() if kk not in ("data",)} for k, v in result.checked_reports.items()
            },
    ...
    md_path = args.output_markdown or (
        args.write_report.parent / "REPORT_PRELIVE_DECISION_GATE.md" if args.write_report else None
    )
    if md_path:
