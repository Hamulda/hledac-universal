...
FEED_ONLY_SCOREBOARD = {
    "source_family_outcomes": [{"family": "feed", "terminal_state": "COMPLETED", "accepted_count": 20}],
...
PROVIDER_YIELD_SCOREBOARD = {
    ...
    "corroboration_penalties": ["feed_only_no_nonfeed", "nonfeed_attempted_no_positive_evidence"],
    "corroborating_families": ("feed",),
...
FIXTURE_DESCRIPTIONS: dict[str, str] = {
    ...
    "provider_yield": "CT/DOH timeout/error — provider yield issue. Produces provider_yield_seed.",
    "ioc_followup_domain": "Multi-source run with domain IOCs. IOC follow-up seeds with domain values.",
...
def _derive_operator_summary(
    gaps: list[str], planner_actions: list[dict], scorecard: dict[str, Any], verdict: str
) -> dict[str, Any]:
    """
    ...
    return {
        ...
        "memory_note": f"replay | {verdict} | accepted={scorecard.get('accepted_findings', 0)} | corrob={scorecard.get('corroboration_score', 0.0):.2f}",
...
def _run_research_loop(scorecard: dict[str, Any], query: str, profile: str, sprint_id: str) -> dict[str, Any]:
    ...
    capability_synthesis = _build_capability_synthesis(
        ...
        acquisition_report={"terminality": {"satisfied": bool(scorecard.get("terminal_families"))}},
        research_depth=None,
    )
    top_nodes: list[dict] = []
    ...
    pye = _compute_provider_yield_signals(
        scorecard, nonfeed_missing_expected_lanes=scorecard.get("nonfeed_expected_lanes", [])
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--fixture",
        type=str,
        choices=["feed_only", "provider_yield", "ioc_followup_domain", "corroborated", "nonfeed_attempted_no_positive"],
