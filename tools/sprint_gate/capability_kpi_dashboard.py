...
def score_qoder_reality(artifact: dict[str, Any] | None) -> DomainResult:
    ...
    return DomainResult(
        ...
        blockers=blockers,
        recommended_action="Resolve DEPRECATED capabilities; ensure all active paths documented"
        if blockers
        else "Maintain active capability coverage",
        raw=artifact,
    )
...
def score_transport_authority(artifact: dict[str, Any] | None) -> DomainResult:
    ...
    return DomainResult(
        ...
        blockers=blockers,
        recommended_action="Wire remaining critical consumers to circuit breaker"
        if blockers
        else "Transport authority fully seamed",
        raw=artifact,
    )
...
def score_stealth_safety(
    stealth_manager: dict[str, Any] | None, stealth_crawler: dict[str, Any] | None
) -> DomainResult:
    ...
    if stealth_crawler is None:
        ...
        evidence.append(
            f"stealth_crawler: cb_used={cb_used}, canonical_transport={canonical_transport}, helper={helper}"
        )
        if not cb_used:
    ...
    return DomainResult(
        ...
        blockers=blockers,
        recommended_action="Wire stealth breaker seams for both manager and crawler"
        if blockers
        else "Stealth safety fully seamed",
...
def score_memory_safety(artifact: dict[str, Any] | None) -> DomainResult:
    ...
    if uma_thresholds:
        evidence.append(
            f"UMA_total={uma_thresholds.get('UMA_total_MB', '?')}MB, EMERGENCY={uma_thresholds.get('EMERGENCY_threshold_MB', '?')}MB"
        )
    ...
    return DomainResult(
        ...
        blockers=blockers,
        recommended_action="Resolve memory conflicts before aggressive mode"
        if blockers
        else "Memory authority healthy",
        raw=artifact,
    )
...
def score_graph_authority(artifact: dict[str, Any] | None) -> DomainResult:
    ...
    return DomainResult(
        ...
        blockers=blockers,
        recommended_action="Canonicalize remaining graph authority modules"
        if blockers
        else "Graph authority fully canonical",
        raw=artifact,
    )
...
def resolve_readiness(domains: dict[str, DomainResult]) -> tuple[DomainStatus, DomainStatus, DomainStatus]:
    ...
    r300_green = all(
        domains[k].status in (DomainStatus.GREEN, DomainStatus.YELLOW) for k in r300_required if k in domains
    )
    readiness_300s = DomainStatus.GREEN if r300_green and all(s >= 40 for s in r300_scores) else DomainStatus.RED
    stealth_ok = (
        domains.get("stealth_safety", DomainResult(DomainStatus.UNKNOWN, 0, [], [], "")).status == DomainStatus.GREEN
    )
    stealth_ok2 = domains.get("stealth_safety", DomainResult(DomainStatus.UNKNOWN, 0, [], [], "")).score >= 70
    readiness_stealth = (
        DomainStatus.GREEN if readiness_300s == DomainStatus.GREEN and stealth_ok and stealth_ok2 else DomainStatus.RED
    )
    memory_ok = (
        domains.get("memory_safety", DomainResult(DomainStatus.UNKNOWN, 0, [], [], "")).status == DomainStatus.GREEN
    )
    live_ok = (
        domains.get("live_measurement", DomainResult(DomainStatus.UNKNOWN, 0, [], [], "")).status == DomainStatus.GREEN
    )
    readiness_aggressive = (
        DomainStatus.GREEN if readiness_stealth == DomainStatus.GREEN and memory_ok and live_ok else DomainStatus.RED
    )
...
def compute_next_big_move(domains: dict[str, DomainResult], readinesses: tuple) -> str:
    ...
    if r300s != DomainStatus.GREEN:
        critical = [
            d
            for d in ["runtime_authority", "graph_authority", "acquisition_strategy"]
            if domains.get(d, DomainResult(DomainStatus.UNKNOWN, 0, [], [], "")).status
            in (DomainStatus.RED, DomainStatus.UNKNOWN)
        ]
        if critical:
...
def compute_dashboard() -> DashboardOutput:
    ...
    runtime = score_runtime_authority(
        domains["transport_authority"], domains["stealth_safety"], domains["memory_safety"]
    )
...
def render_md(dashboard: DashboardOutput) -> str:
    ...
    for name, domain in sorted(dashboard.domains.items()):
        ...
        lines.append(
            f"| {name} | {domain.score}/100 | {domain.status.value.upper()} | {blockers[:60]} | {domain.recommended_action[:40]} |"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Capability KPI Dashboard — Sprint F206BL", suggest_on_error=True, color=True
    )
