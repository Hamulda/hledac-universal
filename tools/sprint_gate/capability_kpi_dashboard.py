...
class DashboardOutput(Struct, frozen=True):
    sprint: str
    date: str
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'sprint': self.sprint, 'date': self.date, 'capability_score': self.capability_score, 'readiness_for_300s_live': self.readiness_for_300s_live.value, 'readiness_for_stealth_live': self.readiness_for_stealth_live.value, 'readiness_for_aggressive_mode': self.readiness_for_aggressive_mode.value, 'domains': {k: {'status': v.status.value, 'score': v.score, 'evidence_artifacts': v.evidence_artifacts, 'blockers': v.blockers, 'recommended_action': v.recommended_action} for k, v in self.domains.items()}, 'next_big_move': self.next_big_move, 'blockers': self.blockers}
...
def score_qoder_reality(artifact: dict[str, Any] | None) -> DomainResult:
    ...
    if artifact is None:
        ...
        return DomainResult(status=DomainStatus.UNKNOWN, score=0, evidence_artifacts=[], blockers=blockers, recommended_action='Run qoder reality probe to generate artifact')
    ...
    return DomainResult(status=status, score=score, evidence_artifacts=['probe_qoder_reality/qoder_reality_matrix.json'], blockers=blockers, recommended_action='Resolve DEPRECATED capabilities; ensure all active paths documented' if blockers else 'Maintain active capability coverage', raw=artifact)
...
def score_transport_authority(artifact: dict[str, Any] | None) -> DomainResult:
    ...
    if artifact is None:
        ...
        return DomainResult(status=DomainStatus.UNKNOWN, score=0, evidence_artifacts=[], blockers=blockers, recommended_action='Run transport authority probe')
    ...
    evidence = [f'verdict={verdict}', f'complete={complete}/{total}', f'critical_remaining={critical_remaining}', f'canonical_cb={canonical_status}', f'production_cb={production_status}']
    ...
    if blockers:
        ...
        status = DomainStatus.YELLOW
    return DomainResult(status=status, score=min(100, score), evidence_artifacts=['probe_transport_authority_f206bc/transport_authority_status_refreshed.json'] + evidence, blockers=blockers, recommended_action='Wire remaining critical consumers to circuit breaker' if blockers else 'Transport authority fully seamed', raw=artifact)
...
def score_acquisition_strategy(artifact: dict[str, Any] | None) -> DomainResult:
    ...
    if artifact is None:
        ...
        return DomainResult(status=DomainStatus.UNKNOWN, score=0, evidence_artifacts=[], blockers=blockers, recommended_action='Run acquisition strategy probe')
    ...
    evidence = [f'lanes={len(lanes)}, high_risk={len(high_risk_lanes)}, disabled={len(disabled_lanes)}', f'tests={passed}/{total_tests} passed']
    if failed > 0:
    ...
    if blockers or failed > 0:
        ...
        status = DomainStatus.YELLOW
    return DomainResult(status=status, score=min(100, max(0, score)), evidence_artifacts=['probe_acquisition_strategy_f206bg/acquisition_strategy_snapshot.json'] + evidence, blockers=blockers, recommended_action='Address high-risk lanes and failing tests' if blockers else 'Acquisition strategy healthy', raw=artifact)

def score_stealth_safety(stealth_manager: dict[str, Any] | None, stealth_crawler: dict[str, Any] | None) -> DomainResult:
    ...
    if not blockers:
        ...
        score = 20
        status = DomainStatus.RED
    return DomainResult(status=status, score=score, evidence_artifacts=['probe_stealth_manager_f206be/stealth_manager_breaker_seam.json', 'probe_stealth_crawler_f206bf/stealth_crawler_breaker_seam.json'], blockers=blockers, recommended_action='Wire stealth breaker seams for both manager and crawler' if blockers else 'Stealth safety fully seamed', raw={'stealth_manager': stealth_manager or {}, 'stealth_crawler': stealth_crawler or {}})
...
def score_memory_safety(artifact: dict[str, Any] | None) -> DomainResult:
    ...
    if artifact is None:
        ...
        return DomainResult(status=DomainStatus.UNKNOWN, score=0, evidence_artifacts=[], blockers=blockers, recommended_action='Run M1 memory authority probe')
    ...
    evidence = [f'conflicts={len(conflicts)}, active={len(active_conflicts)}', f'ram_guards={len(ram_guards)}, skips={len(guard_skips)}']
    if active_conflicts:
    ...
    if uma_thresholds:
        evidence.append(f"UMA_total={uma_thresholds.get('UMA_total_MB', '?')}MB, EMERGENCY={uma_thresholds.get('EMERGENCY_threshold_MB', '?')}MB")
    ...
    if active_conflicts:
        ...
        status = DomainStatus.YELLOW
    return DomainResult(status=status, score=score, evidence_artifacts=['probe_m1_memory_authority/m1_memory_authority_matrix.json'], blockers=blockers, recommended_action='Resolve memory conflicts before aggressive mode' if blockers else 'Memory authority healthy', raw=artifact)
...
def score_graph_authority(artifact: dict[str, Any] | None) -> DomainResult:
    ...
    if artifact is None:
        ...
        return DomainResult(status=DomainStatus.UNKNOWN, score=0, evidence_artifacts=[], blockers=blockers, recommended_action='Run graph authority probe')
    ...
    canonical_count = sum((1 for m in [truth_writer, analytics_writer, sprint_facts] if m.get('canonical', False)))
    reset_exists = reset_verif.get('exists', False)
    reset_status = reset_verif.get('status', 'UNKNOWN')
    evidence = [f'canonical_writers={canonical_count}/3', f'reset_session: exists={reset_exists}, status={reset_status}', f'write_path_call_sites={len(write_paths)}']
    ...
    if blockers:
        ...
        status = DomainStatus.YELLOW
    return DomainResult(status=status, score=score, evidence_artifacts=['probe_graph_authority/graph_authority_matrix.json'] + evidence, blockers=blockers, recommended_action='Canonicalize remaining graph authority modules' if blockers else 'Graph authority fully canonical', raw=artifact)

def score_live_measurement(live_run: dict[str, Any] | None, dry_run: dict[str, Any] | None) -> DomainResult:
    ...
    if live_run is None and dry_run is None:
        ...
        return DomainResult(status=DomainStatus.UNKNOWN, score=0, evidence_artifacts=[], blockers=blockers, recommended_action='Run live sprint measurement probe (F206BJ)')
    ...
    if verdict == 'PASS' or verdict == 'GREEN':
        ...
        score = 50
        status = DomainStatus.UNKNOWN
    return DomainResult(status=status, score=score, evidence_artifacts=['probe_live_sprint_measurement_f206bj/live_run.json', 'probe_live_sprint_measurement_f206bj/dry_run.json'], blockers=blockers, recommended_action='Address runtime errors in live measurement' if blockers else 'Live measurement passed', raw=artifact)

def score_runtime_authority(transport: DomainResult, stealth: DomainResult, memory: DomainResult) -> DomainResult:
    ...
    if all_green:
        ...
        status = DomainStatus.YELLOW
    return DomainResult(status=status, score=avg_score, evidence_artifacts=['composite: transport + stealth + memory'], blockers=blockers, recommended_action='Address domain-level blockers' if blockers else 'Runtime authority fully operational')

def resolve_readiness(domains: dict[str, DomainResult]) -> tuple[DomainStatus, DomainStatus, DomainStatus]:
    """
...
def compute_next_big_move(domains: dict[str, DomainResult], readinesses: tuple) -> str:
    ...
    if r300s != DomainStatus.GREEN:
        critical = [d for d in ['runtime_authority', 'graph_authority', 'acquisition_strategy'] if domains.get(d, DomainResult(DomainStatus.UNKNOWN, 0, [], [], '')).status in (DomainStatus.RED, DomainStatus.UNKNOWN)]
        if critical:
...
def compute_dashboard() -> DashboardOutput:
    ...
    domains: dict[str, DomainResult] = {'qoder_truth': score_qoder_reality(artifacts.get('qoder_reality')), 'transport_authority': score_transport_authority(artifacts.get('transport_authority')), 'acquisition_strategy': score_acquisition_strategy(artifacts.get('acquisition_strategy')), 'stealth_safety': score_stealth_safety(stealth_manager, stealth_crawler), 'memory_safety': score_memory_safety(artifacts.get('m1_memory')), 'graph_authority': score_graph_authority(artifacts.get('graph_authority')), 'live_measurement': score_live_measurement(live_run, dry_run)}
    runtime = score_runtime_authority(domains['transport_authority'], domains['stealth_safety'], domains['memory_safety'])
    ...
    return DashboardOutput(sprint='F206BL', date=datetime.now(UTC).isoformat(), capability_score=overall_score, readiness_for_300s_live=readinesses[0], readiness_for_stealth_live=readinesses[1], readiness_for_aggressive_mode=readinesses[2], domains=domains, next_big_move=next_move, blockers=all_blockers)

def render_md(dashboard: DashboardOutput) -> str:
    lines = ['# Capability KPI Dashboard — F206BL', '', f'**Generated**: {dashboard.date}', f'**Overall Capability Score**: {dashboard.capability_score}/100', '', '## Readiness Dimensions', '', '| Dimension | Status |', '|-----------|--------|', f'| 300s Live | {dashboard.readiness_for_300s_live.value.upper()} |', f'| Stealth Live | {dashboard.readiness_for_stealth_live.value.upper()} |', f'| Aggressive Mode | {dashboard.readiness_for_aggressive_mode.value.upper()} |', '', '## Domain Breakdown', '', '| Domain | Score | Status | Blockers | Recommended Action |', '|---------|-------|--------|----------|--------------------|']
    for name, domain in sorted(dashboard.domains.items()):
        blockers = '; '.join(domain.blockers) if domain.blockers else 'none'
        lines.append(f'| {name} | {domain.score}/100 | {domain.status.value.upper()} | {blockers[:60]} | {domain.recommended_action[:40]} |')
    ...
    if dashboard.blockers:
        ...
        for b in dashboard.blockers:
            lines.append(f'- {b}')
    lines += ['', '## Next Big Move', '', f'{dashboard.next_big_move}']
    return '\n'.join(lines)
...
def main() -> int:
    ...
    try:
        ...
        error_result = {'sprint': 'F206BL', 'date': datetime.now(UTC).isoformat(), 'error': str(e), 'capability_score': 0}
