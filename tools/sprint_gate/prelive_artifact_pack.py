...
def render_collision_warning(report: SprintCollisionReport) -> list[str]:
    ...
    for coll in report.collisions:
        ...
        lines.append('')
        lines.append(f'**Action:** Use full alias (e.g. `{coll.aliases[0]}`) to disambiguate. Live is NOT blocked — required artifacts are unambiguous when paths are explicit.')
        lines.append('')
    return lines

class ArtifactPackResult(Struct, frozen=True):
    required: list
    optional: list
    commands: list
...
class ProbeArtifact(Struct, frozen=True):
    probe_dir: str
    filename: str
    full_path: str
    found: bool
...
def check_artifact(repo_root: Path, probe_dir: str, filename: str, *, required: bool=True) -> ProbeArtifact:
    """Check a single artifact file exists and is valid JSON."""
    full_path = str(repo_root / probe_dir / filename)
    artifact = ProbeArtifact(probe_dir=probe_dir, filename=filename, full_path=full_path, found=False)
    p = Path(full_path)
    if not p.exists():
...
def build_regeneration_commands(required: list[ProbeArtifact]) -> list[str]:
    """Build pytest commands to regenerate missing artifacts."""
    commands = []
    missing_required_dirs = {(a.probe_dir, a.filename) for a in required if a.status != ArtifactStatus.READY_FOR_PRELIVE_GATE}
...
def render_markdown(required: list[ProbeArtifact], commands: list[str], overall: ArtifactStatus) -> str:
    """Render the markdown report."""
    lines = ['# Prelive Artifact Pack Report', '', f'**Status:** `{overall.value}`', '', '## Required Artifacts', '', '| Probe | Artifact | Status |', '|-------|----------|--------|']
    for a in required:
    ...
    lines.extend(['', '## Pytest Commands Reference', '', '```bash', '# Check all artifacts:', 'python -m pytest tests/probe_f219i_prelive_artifact_pack -v --tb=short', '', '# Regenerate specific probe lane:', '# python -m pytest tests/probe_f217c_public_bootstrap -v --tb=short', '# python -m pytest tests/probe_f217d_ct_provider_resilience -v --tb=short', '```', ''])
    return '\n'.join(lines)

def render_json(required: list[ProbeArtifact], commands: list[str], overall: ArtifactStatus, f224: list[ProbeArtifact] | None=None) -> dict:
    """Render the JSON report."""
    f224_section = {}
    if f224 is not None:
        f224_section = {'f224_artifacts': [{'probe_dir': a.probe_dir, 'filename': a.filename, 'path': a.full_path, 'found': a.found, 'status': a.status.value, 'parse_error': a.parse_error} for a in f224], 'f224_all_present': all((a.status == ArtifactStatus.READY_FOR_PRELIVE_GATE for a in f224)), 'f224_present_count': sum((1 for a in f224 if a.status == ArtifactStatus.READY_FOR_PRELIVE_GATE)), 'f224_total': len(f224)}
    return {'status': overall.value, 'overall': overall.value, 'required_artifacts': [{'probe_dir': a.probe_dir, 'filename': a.filename, 'path': a.full_path, 'found': a.found, 'status': a.status.value, 'parse_error': a.parse_error} for a in required], 'missing_required': [f'{a.probe_dir}/{a.filename}' for a in required if a.status != ArtifactStatus.READY_FOR_PRELIVE_GATE], 'regeneration_commands': commands, 'probe_count': {'total_required': len(required), 'found': sum((1 for a in required if a.status == ArtifactStatus.READY_FOR_PRELIVE_GATE)), 'missing_required': sum((1 for a in required if a.status == ArtifactStatus.MISSING_REQUIRED)), 'stale_or_corrupt': sum((1 for a in required if a.status == ArtifactStatus.STALE_OR_CORRUPT))}, **f224_section}

def main() -> int:
    parser = argparse.ArgumentParser(description='Validate and regenerate prelive artifact pack.', formatter_class=argparse.RawDescriptionHelpFormatter, epilog=textwrap.dedent('            Examples:\n              # Inspect only (default mode - no tests run)\n              python tools/prelive_artifact_pack.py --repo-root .\n\n              # Inspect and generate reports\n              python tools/prelive_artifact_pack.py --repo-root . \\\n                --output-json probe_f219i_prelive_artifact_pack/artifact_pack.json \\\n                --output-md probe_f219i_prelive_artifact_pack/REPORT_PRELIVE_ARTIFACT_PACK.md\n\n              # Inspect and print regeneration commands without running them\n              python tools/prelive_artifact_pack.py --repo-root . --run-probes\n\n              # Dry-run: inspect and emit commands only (same as default)\n              python tools/prelive_artifact_pack.py --repo-root .\n        '))
    ...
    parser.add_argument('--run-probes', action='store_true', help='Execute pytest commands to regenerate missing artifacts. Without this flag, only inspects and reports (default behavior).')
    parser.add_argument('--verbose', '-v', action='store_true', help='Print detailed status per artifact.')
    ...
    if args.run_probes and commands:
        ...
        for cmd in commands:
            print(f'\n$ {cmd}')
            cmd_parts = cmd.split()
            result = subprocess.run(cmd_parts, shell=False, cwd=str(repo_root), capture_output=False)
    ...
    if args.output_json:
        ...
        if collision_report.has_collisions:
            json_out['sprint_id_collisions'] = {'has_collisions': True, 'collision_count': len(collision_report.collisions), 'total_probes_scanned': collision_report.total_probes_scanned, 'collisions': [{'sprint_id': c.sprint_id, 'aliases': c.aliases, 'probe_dirs': c.probe_dirs, 'report_paths': c.report_paths, 'json_paths': c.json_paths} for c in collision_report.collisions]}
