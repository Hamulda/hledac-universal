...
def render_collision_warning(report: SprintCollisionReport) -> list[str]:
    ...
    lines.append(
        f"**{len(report.collisions)} collision(s)** detected across {report.total_probes_scanned} probes scanned."
    )
    lines.append("")
    for coll in report.collisions:
        ...
        lines.append("")
        lines.append(
            f"**Action:** Use full alias (e.g. `{coll.aliases[0]}`) to disambiguate. Live is NOT blocked — required artifacts are unambiguous when paths are explicit."
        )
        lines.append("")
    return lines
...
def check_all_artifacts_with_f224(
    repo_root: Path,
) -> tuple[list[ProbeArtifact], list[ProbeArtifact], list[ProbeArtifact]]:
...
def build_regeneration_commands(required: list[ProbeArtifact]) -> list[str]:
    ...
    missing_required_dirs = {
        (a.probe_dir, a.filename) for a in required if a.status != ArtifactStatus.READY_FOR_PRELIVE_GATE
    }
...
def render_markdown(required: list[ProbeArtifact], commands: list[str], overall: ArtifactStatus) -> str:
    ...
    if commands:
        ...
        lines.extend(
            ["", "## Regeneration Commands", "", "All required artifacts present. No regeneration needed.", ""]
        )
    lines.extend(
        [
            "",
...
def render_json(
    required: list[ProbeArtifact], commands: list[str], overall: ArtifactStatus, f224: list[ProbeArtifact] | None = None
) -> dict:
    ...
    return {
        ...
        "missing_required": [
            f"{a.probe_dir}/{a.filename}" for a in required if a.status != ArtifactStatus.READY_FOR_PRELIVE_GATE
        ],
...
def main() -> int:
    ...
    parser.add_argument(
        "--run-probes",
        action="store_true",
        help="Execute pytest commands to regenerate missing artifacts. Without this flag, only inspects and reports (default behavior).",
    )
