...
def _check_nonfeed_evidence_missing(b: BenchmarkSurface) -> tuple[bool, str | None]:
    ...
    if b.run_quality_verdict in _NONFEED_EVIDENCE_MISSING_VERDICTS:
        return (
            False,
            f"Nonfeed evidence missing: run_quality_verdict={b.run_quality_verdict} — terminality satisfied but nonfeed evidence insufficient",
        )
    return (True, None)
...
def _check_public_surface_present(b: BenchmarkSurface) -> tuple[bool, str | None]:
    ...
    if isinstance(ar_sfo, list):
        ...
        if canonical_attempted:
            return (
                False,
                "public_terminal_stage indicates PUBLIC was attempted but PUBLIC is absent from acquisition_report.source_family_outcomes",
            )
...
def _check_hardware_constrained_comparable(b: BenchmarkSurface, q: QualitySurface) -> tuple[bool, str | None]:
    ...
    if hardware_constrained and q.research_quality_comparable is not False:
        return (
            False,
            f"hardware_constrained={hardware_constrained} but research_quality_comparable={q.research_quality_comparable} — hardware-constrained runs must not be marked comparable",
        )
    return (True, None)
...
def _check_swap_gate_comparable(b: BenchmarkSurface) -> tuple[bool, str | None]:
    ...
    if b.swap_gate_triggered and b.comparable_result is not False:
        return (
            False,
            f"swap_gate_triggered={b.swap_gate_triggered} but comparable_result={b.comparable_result} — active300/600 with high swap must not be marked comparable",
        )
    return (True, None)
...
def _run_check(
    ...
    *extra_args: Any,
    ...
    if not ok:
        if msg is None:
            raise RuntimeError(
                f"Sanity check {check_name!r} returned False but msg is None — internal invariant violated"
            )
...
_SANITY_CHECKS: tuple[_CheckSpec, ...] = (
    _CheckSpec("benchmark_fail_validator_pass", _check_benchmark_fail_validator_pass, lambda p: ()),
    _CheckSpec("stale_terminality", _check_stale_terminality, lambda p: (p.allow_stale_trace,)),
    _CheckSpec("benchmark_shape_source_family_outcomes", _check_benchmark_missing_source_family_outcomes, lambda p: ()),
    _CheckSpec("wallclock_budget", _check_wallclock_budget, lambda p: ()),
    _CheckSpec("feed_only_nonfeed_attempted", _check_feed_only_accepted_nonfeed_attempted, lambda p: ()),
    _CheckSpec("nonfeed_evidence_missing", _check_nonfeed_evidence_missing, lambda p: ()),
    _CheckSpec("research_quality", _check_research_quality, lambda p: (p.min_quality_grade, p.allow_feed_only)),
    _CheckSpec("ct_loss_stage_present", _check_ct_loss_stage_present, lambda p: ()),
    _CheckSpec("public_surface_present", _check_public_surface_present, lambda p: ()),
    _CheckSpec("hardware_constrained_comparable", _check_hardware_constrained_comparable, lambda p: ()),
    _CheckSpec("swap_gate_comparable", _check_swap_gate_comparable, lambda p: ()),
)
...
_VERDICT_KEYWORDS: tuple[tuple[frozenset[str], SanityVerdict], ...] = (
    ...
    (
        frozenset({"ct_loss_stage", "public_surface", "hardware_constrained", "comparable"}),
        SanityVerdict.SANITY_FAIL_SURFACE_DISAGREEMENT,
    ),
...
def sanity_check(
    ...
    params = SanityParams(
        ...
        trace_raw=trace_raw,
        allow_stale_trace=allow_stale_trace,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--quality-json", type=Path, help="Path to research quality score JSON from research_quality_score.py"
    )
    parser.add_argument(
        "--min-quality-grade",
        type=str,
