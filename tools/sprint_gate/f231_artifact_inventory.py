...
class ArtifactResult(msgspec.Struct, gc=False):
    name: str
    exists: bool
...
class PackInventory(msgspec.Struct, frozen=True, gc=False):
    verdict: str
    gate_status: str
...
def inspect_artifact(repo_root: str, probe_dir: str, artifact_file: str) -> ArtifactResult:
    ...
    try:
        ...
        with open(path) as fh:
            data = json.load(fh)
        return ArtifactResult(name=name, exists=True, valid_json=True, size_bytes=size, test_count=data.get('test_count'), verdict=data.get('verdict'))
...
def run_inventory(repo_root: str, gate_artifact_path: str | None=None) -> PackInventory:
    ...
    inv = PackInventory(verdict=verdict, gate_status=gate_status, present=present, missing=missing, malformed=malformed, missing_blocking=missing_blocking, artifacts=artifacts)
...
def _apply_gate_artifact_stale(inv: PackInventory, gate_artifact_path: str | None, repo_root: str) -> PackInventory:
    ...
    if not gate_artifact_path:
        gate_artifact_path = os.path.join(repo_root, 'probe_f231h_prelive_evidence_lift_gate', 'prelive_evidence_lift_gate.json')
    ...
    if gate_blocking_set & inventory_blocking_set:
        return PackInventory(verdict=inv.verdict, gate_status='GATE_STALE', present=inv.present, missing=inv.missing, malformed=inv.malformed, missing_blocking=inv.missing_blocking, artifacts=inv.artifacts)
    return inv
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--output-md', metavar='PATH', help='Write markdown report to PATH')
    p.add_argument('--gate-artifact', metavar='PATH', help='Path to F231H gate JSON (overrides default probe dir lookup)')
    return p

def _render_md(inv: PackInventory) -> str:
    lines = ['# F231 Artifact Inventory', '', f'**Verdict:** `{inv.verdict}`', f'**Gate Status:** `{inv.gate_status}`', '', '## Artifact Table', '', '| Artifact | Status | Size (bytes) | Test Count | Verdict |', '|:---------|:-------|-------------:|------------|:--------|']
    ...
    lines += ['', '## Gate Cross-Check', '', f'- Blocking set required by F231H: `{sorted(GATE_BLOCKING_SET)}`', f'- Present: `{sorted(inv.present)}`', f'- Missing: `{sorted(inv.missing)}`', f'- Malformed: `{sorted(inv.malformed)}`', f'- Missing blocking artifacts: `{inv.missing_blocking}`', '', f'**Conclusion:** `{inv.gate_status}`']
    return '\n'.join(lines)
...
def main(argv: list[str] | None=None) -> int:
    ...
    if not gate_path:
        gate_path = os.path.join(repo_root, 'probe_f231h_prelive_evidence_lift_gate', 'prelive_evidence_lift_gate.json')
