...
def _apply_gate_artifact_stale(inv: PackInventory, gate_artifact_path: str | None, repo_root: str) -> PackInventory:
    ...
    if not gate_artifact_path:
        gate_artifact_path = os.path.join(
            repo_root, "probe_f231h_prelive_evidence_lift_gate", "prelive_evidence_lift_gate.json"
        )
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--gate-artifact", metavar="PATH", help="Path to F231H gate JSON (overrides default probe dir lookup)"
    )
    return p
