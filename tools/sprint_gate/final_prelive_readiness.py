...
_CONTRACT_FALSE_POSITIVES = {'probe_f219b_hermes_metal_finalizer': 'prelive_decision_gate._is_pass() schema mismatch — F219B report uses tests.all_passed=True, gate checks status=PASS|ready_for_controlled_smoke', 'probe_f224d_confidence_policy': 'prelive_decision_gate._check_f224_artifacts() hardcodes probe_f224d_confidence_policy but actual probe directory is probe_f224d_sprint_id_collision'}
    ...
    for probe_dir, filename in _F224_BLOCKING_PROBES:
        ...
        if os.path.exists(path):
            try:
                ...
                missing_blocking.append(probe_dir)
        elif probe_dir == 'probe_f224d_sprint_id_collision':
...
def run_f231_inventory(repo_root: str) -> tuple[str, list[str], list[str]]:
    ...
    try:
        ...
        from hledac.universal.tools.f231_artifact_inventory import run_inventory
...
def run_f231h_gate(repo_root: str) -> tuple[str, str, list[str]]:
    ...
    if not data:
        return ('GATE_ABSENT', '', [])
    return (data.get('verdict', 'UNKNOWN'), data.get('gate_status', ''), data.get('blocking_probes', []))
...
def run_gate_fresh(repo_root: str, profile: str, query: str) -> dict | None:
    ...
    try:
        os.chdir(repo_root)
        sys.path.insert(0, repo_root)
        from hledac.universal.tools.prelive_decision_gate import run_gate
        ...
        return {'decision': result.decision.value if hasattr(result.decision, 'value') else str(result.decision), 'live_allowed': result.live_allowed, 'reasons': list(result.reasons) if result.reasons else [], 'warnings': list(result.warnings) if result.warnings else [], 'f224_core_ready': result.f224_core_ready if hasattr(result, 'f224_core_ready') else False, 'missing_f224_artifacts': list(result.missing_f224_artifacts) if hasattr(result, 'missing_f224_artifacts') else [], 'f231_core_ready': result.f231_core_ready if hasattr(result, 'f231_core_ready') else False, 'missing_f231_artifacts': list(result.missing_f231_artifacts) if hasattr(result, 'missing_f231_artifacts') else [], 'swap_policy_tier': result.swap_policy_tier if hasattr(result, 'swap_policy_tier') else 'unknown', 'swap_gate_reason': result.swap_gate_reason if hasattr(result, 'swap_gate_reason') else '', 'hardware_constrained': result.hardware_constrained if hasattr(result, 'hardware_constrained') else False, 'fallback_schema_blocked': result.fallback_schema_blocked if hasattr(result, 'fallback_schema_blocked') else False, 'checked_reports': checked_reports_clean}
    except Exception as e:
        return {'_error': str(e)}
    finally:
        os.chdir(prev_cwd)
...
def compute_verdict(gate_decision: str, gate_data: dict, f224_blocking: list[str], f231_blocking: list[str], f224_core_ready: bool, f231_core_ready: bool, swap_used: float, uma_state: str, inventory_ready: bool, inventory_verdict: str, f231h_gate: str, f231h_status: str, provider_surface_ok: bool, stripped_reasons: list[str], profile: str, query: str) -> ReadinessResult:
    ...
    if swap_used > HARD_BLOCK_SWAP_GIB:
        blockers.append(Blocker(category='memory', severity='HARD_BLOCK', detail=f'swap={swap_used:.3f}GiB > {HARD_BLOCK_SWAP_GIB}GiB threshold (hard_block tier)', current_swap_gib=swap_used, threshold_gib=HARD_BLOCK_SWAP_GIB))
        merge_log.append(f'blocker=MEMORY swap={swap_used:.3f}GiB')
    if f231_blocking:
        blockers.append(Blocker(category='artifacts', severity='HARD_BLOCK', detail=f"F231 blocking artifacts missing: {', '.join(f231_blocking)}"))
        merge_log.append(f'blocker=ARTIFACTS missing_f231={f231_blocking}')
    if f224_blocking and (not f224_core_ready):
        blockers.append(Blocker(category='contract', severity='HARD_BLOCK', detail=f"F224 blocking artifacts missing: {', '.join(f224_blocking)}"))
    ...
    if not provider_surface_ok and gate_decision == 'BLOCKED_BY_PROVIDER_SURFACE':
        blockers.append(Blocker(category='provider_surface', severity='HARD_BLOCK', detail='Provider surface gaps detected (public_bootstrap, ct_provider_resilience)'))
    ...
    if has_real_contract_blocker and (not f231_blocking) and (not f224_blocking):
        blockers.append(Blocker(category='contract', severity='HARD_BLOCK', detail=f"Contract gate failed: {'; '.join(stripped_reasons)}"))
    ...
    return ReadinessResult(verdict=verdict, live_allowed=live_allowed, next_action=next_action, next_action_detail=next_action_detail, blockers=blockers, f231_inventory_verdict=inventory_verdict, f231_inventory_present=[], f231_inventory_missing=f231_blocking, f231h_gate_verdict=f231h_gate, f231h_gate_status=f231h_status, swap_used_gib=swap_used, uma_state=uma_state, swap_policy_tier=swap_policy_tier, swap_gate_reason=swap_gate_reason, hardware_constrained=hardware_constrained, gate_decision=gate_decision, gate_live_allowed=gate_data.get('live_allowed', False), gate_reasons=gate_data.get('reasons', []), gate_warnings=gate_data.get('warnings', []), f224_core_ready=f224_core_ready, f224_blocking=f224_blocking, f231_core_ready=f231_core_ready, f231_blocking=f231_blocking, provider_surface_ok=provider_surface_ok, post_restart_command=_build_post_restart_command(profile, query), merge_log=merge_log)

def _build_post_restart_command(profile: str, query: str) -> str:
    """Build the post-restart live command."""
    return f'python -m core --profile {profile} --query "{query}" --live --require-memory-ok'

def run_final_readiness(repo_root: str, profile: str, query: str) -> ReadinessResult:
    merge_log: list[str] = []
    f231t_data = read_f231t_result(repo_root)
    if f231t_data:
        merge_log.append(f"f231t_read: verdict={f231t_data.get('verdict')} swap={f231t_data.get('memory', {}).get('swap_used_gib')}")
    ...
    merge_log.append(f"gate_fresh: decision={gate_decision} live_allowed={gate_data.get('live_allowed')}")
    ...
    merge_log.append(f'provider_surface_ok={provider_surface_ok}')
    swap_used, uma_state, swap_policy_tier, swap_gate_reason, hardware_constrained = _extract_uma_from_gate(gate_data)
    if swap_used == 0.0:
    ...
    result = compute_verdict(gate_decision=gate_decision, gate_data=gate_data, f224_blocking=f224_blocking, f231_blocking=f231_blocking, f224_core_ready=f224_core_ready, f231_core_ready=f231_core_ready, swap_used=swap_used, uma_state=uma_state, inventory_ready=inventory_ready, inventory_verdict=inv_verdict, f231h_gate=f231h_gate, f231h_status=f231h_status, provider_surface_ok=provider_surface_ok, stripped_reasons=stripped_reasons, profile=profile, query=query)
...
def _get_current_swap_gib() -> float:
    ...
    try:
        import subprocess
        out = subprocess.check_output(['sysctl', '-n', 'vm.swapusage'], stderr=subprocess.DEVNULL, text=True)
        parts = out.split()
...
def _make_unknown_result(reason: str, merge_log: list[str]) -> ReadinessResult:
    return ReadinessResult(verdict=Verdict.BLOCKED_BY_UNKNOWN, live_allowed=False, next_action=NextAction.FIX_CONTRACT_GATE, next_action_detail=reason, merge_log=merge_log)

def render_markdown(result: ReadinessResult, profile: str, query: str) -> str:
    lines = ['# Final Pre-Live Readiness — F232C', '', f'**Profile:** `{profile}`', f'**Query:** `{query}`', '**Date:** 2026-05-10', '', '---', '', f'## Verdict: `{result.verdict.value}`', '', f'**Live Allowed:** {result.live_allowed}', f'**Next Action:** {result.next_action.value}']
    ...
    lines.extend(['', '---', '', '## Swap / Memory', '', '| Metric | Value |', '|--------|-------|', f'| swap_used_gib | {result.swap_used_gib:.3f} |', f'| uma_state | {result.uma_state} |', f'| swap_policy_tier | {result.swap_policy_tier} |', f'| swap_gate_reason | {result.swap_gate_reason} |', f'| hardware_constrained | {result.hardware_constrained} |', '', f'**Thresholds:** clean<{CLEAN_SWAP_MAX_GIB}GiB, diagnostic<{DIAGNOSTIC_SWAP_MAX_GIB}GiB, hard_block>={HARD_BLOCK_SWAP_GIB}GiB'])
    lines.extend(['', '## F231 Artifact Inventory', '', '| Check | Value |', '|-------|-------|', f'| verdict | {result.f231_inventory_verdict} |', f'| F231H gate | {result.f231h_gate_verdict} ({result.f231h_gate_status}) |', f'| F231 core ready | {result.f231_core_ready} |', f"| F231 blocking missing | {result.f231_blocking or 'none'} |"])
    if result.f231_inventory_present:
        lines.append(f"| F231 present | {', '.join(result.f231_inventory_present)} |")
    lines.extend(['', '## Gate Decision', '', '| Check | Value |', '|-------|-------|', f'| gate_decision | {result.gate_decision} |', f'| gate_live_allowed | {result.gate_live_allowed} |', f'| F224 core ready | {result.f224_core_ready} |', f"| F224 blocking missing | {result.f224_blocking or 'none'} |", f'| provider_surface_ok | {result.provider_surface_ok} |'])
    ...
    if result.verdict == Verdict.READY_TO_RESTART_AND_RUN:
        lines.extend(['', '---', '', '## Post-Restart Command Pack', '', '⚠️ **ABORT RULE:** If `final_prelive_readiness` does not return `READY_TO_RUN_NOW` after restart, do NOT run live.', '', '**Memory instruction:** Restart Mac, open only terminal, run readiness first.', '', '```bash', '# 1. Run final pre-live readiness (post-restart)', 'python -m tools.final_prelive_readiness \\', '  --repo-root . \\', f'  --profile {profile} \\', f'  --query "{query}" \\', '  --output-json probe_f232c_final_post_restart_readiness/final_readiness.json \\', '  --output-md probe_f232c_final_post_restart_readiness/FINAL_READINESS.md', '', '# 2. If READY_TO_RUN_NOW — run live nonfeed_diagnostic180', f'python -m core --profile {profile} --query "{query}" --live --require-memory-ok', '', '# 3. Research quality score', 'python -m tools.research_quality_score \\', f'  --repo-root . --profile {profile} --sprint-id F232C \\', '  --output-json probe_f232c_final_post_restart_readiness/research_quality_score.json \\', '  --output-md probe_f232c_final_post_restart_readiness/RESEARCH_QUALITY_SCORE.md', '', '# 4. Live result sanity', 'python -m tools.live_result_sanity \\', f'  --repo-root . --profile {profile} \\', '  --output-json probe_f232c_final_post_restart_readiness/live_result_sanity.json', '', '# 5. Evidence delta memory', 'python -m tools.evidence_delta_memory \\', f'  --repo-root . --profile {profile} \\', '  --output-json probe_f232c_final_post_restart_readiness/evidence_delta_memory.json', '', '# 6. F231 artifact inventory (final)', 'python -m tools.f231_artifact_inventory \\', '  --repo-root . \\', '  --output-json probe_f232c_final_post_restart_readiness/f231_artifact_inventory.json \\', '  --output-md probe_f232c_final_post_restart_readiness/F231_ARTIFACT_INVENTORY.md', '```'])
    lines.extend(['', '---', '', '## Merge Log', ''])
    for entry in result.merge_log:
        lines.append(f'- {entry}')
    return '\n'.join(lines)

def main() -> int:
    parser = argparse.ArgumentParser(description='Sprint F232C — Final Pre-Live Readiness', formatter_class=argparse.RawDescriptionHelpFormatter)
