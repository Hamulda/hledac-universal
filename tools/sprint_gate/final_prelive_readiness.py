...
_CONTRACT_FALSE_POSITIVES = {
    "probe_f219b_hermes_metal_finalizer": "prelive_decision_gate._is_pass() schema mismatch — F219B report uses tests.all_passed=True, gate checks status=PASS|ready_for_controlled_smoke",
    "probe_f224d_confidence_policy": "prelive_decision_gate._check_f224_artifacts() hardcodes probe_f224d_confidence_policy but actual probe directory is probe_f224d_sprint_id_collision",
}
...
def run_gate_fresh(repo_root: str, profile: str, query: str) -> dict | None:
    ...
    try:
        ...
        return {
            ...
            "missing_f224_artifacts": list(result.missing_f224_artifacts)
            if hasattr(result, "missing_f224_artifacts")
            else [],
            "f231_core_ready": result.f231_core_ready if hasattr(result, "f231_core_ready") else False,
            "missing_f231_artifacts": list(result.missing_f231_artifacts)
            if hasattr(result, "missing_f231_artifacts")
            else [],
            ...
            if hasattr(result, "fallback_schema_blocked")
            else False,
...
def compute_verdict(
    ...
    if memory_blocker and (not artifact_blocker) and (not contract_blocker) and (not provider_blocker):
        ...
        next_action_detail = (
            f"swap={swap_used:.3f}GiB in ({CLEAN_SWAP_MAX_GIB}GiB, {DIAGNOSTIC_SWAP_MAX_GIB}GiB] — hardware taint"
        )
        live_allowed = True
...
def run_final_readiness(repo_root: str, profile: str, query: str) -> ReadinessResult:
    ...
    if f231t_data:
        merge_log.append(
            f"f231t_read: verdict={f231t_data.get('verdict')} swap={f231t_data.get('memory', {}).get('swap_used_gib')}"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Sprint F232C — Final Pre-Live Readiness", formatter_class=argparse.RawDescriptionHelpFormatter
    )
