...
class ValidationResult(Struct, frozen=True):
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "failures": [
                {"verdict": f.verdict.value, "reason": f.reason, "field_path": f.field_path} for f in self.failures
            ],
...
TERMINAL_OUTCOME_STATES: frozenset[str] = frozenset(
    ["ATTEMPTED", "SKIPPED", "ERROR", "TIMEOUT", "TERMINAL", "COMPLETED", "SATISFIED", "EXHAUSTED", "NEVER_ATTEMPTED"]
)
...
def _validate_prelude_checks(failures: list, prelude: dict, profile: str, query_type: str) -> None:
    ...
    prelude.get("prelude_terminal_lanes")
    if prelude_checked is None:
        pass
        ...
        failures.append(
            ValidationFailure(
                ...
                f"acquisition_prelude.prelude_checked is {prelude_checked!r}, expected true",
...
def _validate_source_outcomes(failures: list, sf_outcomes, branch_mix, live_kpi) -> None:
    ...
    if not sf_outcomes or (isinstance(sf_outcomes, dict) and not sf_outcomes):
        failures.append(
            ValidationFailure(
                ...
                "source_family_outcomes is empty",
    ...
    if not (feed_count > 0 or feed_in_outcomes):
        failures.append(
            ValidationFailure(Verdict.FAIL_MISSING_SOURCE_OUTCOMES, "No feed findings attempted", "branch_mix.feed")
        )
...
def _validate_lane_terminality(failures: list, data: dict, live_kpi: dict, profile: str, query_type: str) -> None:
    ...
    for lane, field in [("PUBLIC", "public_terminal_state"), ("CT", "ct_terminal_state")]:
        ...
        if state == "NEVER_ATTEMPTED":
            failures.append(
                ValidationFailure(
                    Verdict.FAIL_PUBLIC_NOT_TERMINAL if lane == "PUBLIC" else Verdict.FAIL_CT_NOT_TERMINAL,
                    f"{lane} lane never attempted for domain query",
                    field,
                )
            )
            ...
            failures.append(
                ValidationFailure(
                    Verdict.FAIL_PUBLIC_NOT_TERMINAL if lane == "PUBLIC" else Verdict.FAIL_CT_NOT_TERMINAL,
                    f"{lane} terminal_state '{state}' is not terminal",
                    field,
                )
            )
...
def _validate_hardware_and_schema(failures: list, acq_report, data, live_kpi, allow_hardware_constrained) -> None:
    ...
    if "hardware-constrained" in quality_verdict or "hardware_constrained" in quality_verdict:
        if not allow_hardware_constrained:
            failures.append(
                ValidationFailure(Verdict.FAIL_HARDWARE_TAINTED, "hardware constraint detected", "run_quality_verdict")
            )
    schema_version = acq_report.get("schema_version") if isinstance(acq_report, dict) else None
    if not schema_version:
        failures.append(
            ValidationFailure(
                Verdict.FAIL_TERMINALITY_NOT_CHECKED, "schema_version missing", "acquisition_report.schema_version"
            )
        )


def _validate_guards(
    failures: list, windup_count, windup_reason, windup_not_applicable, return_guard_checked, scheduler_exit
) -> None:
    """Validate scheduler and windup guards."""
    if not scheduler_exit or not str(scheduler_exit).strip():
        failures.append(
            ValidationFailure(Verdict.FAIL_SCHEDULER_EXIT_MISSING, "scheduler_exit empty", "scheduler_exit_path")
        )
    if return_guard_checked is not True:
        failures.append(
            ValidationFailure(
                Verdict.FAIL_RETURN_GUARD_MISSING,
                f"return_guard_checked is {return_guard_checked!r}",
    ...
    if not (windup_count > 0 or has_reason):
        failures.append(
            ValidationFailure(
                Verdict.FAIL_TERMINALITY_NOT_CHECKED, "windup_guard not called", "windup_guard_call_count"
            )
        )


def _failures_from_dict(
    data: dict, profile: str, query_type: str, allow_hardware_constrained: bool
    ...
    windup_count, windup_reason, windup_not_applicable, return_guard_checked, scheduler_exit = _extract_guard_fields(
        data
    )
    live_kpi = _extract_live_kpi(data)
    ...
    if mismatched_lanes:
        failures.append(
            ValidationFailure(
                ...
                f"lanes mismatch: {missing_lanes}",
    ...
    if verdict_kpi in (Verdict.FAIL_PUBLIC_ACCEPTANCE_KPI, Verdict.WARN_PUBLIC_ACCEPTANCE_KPI):
        failures.append(
            ValidationFailure(verdict_kpi, f"public_acceptance_kpi: {msg_kpi}", "live_kpi.public_acceptance_*")
        )
    verdict_fetch, msg_fetch = _check_public_fetch_telemetry(data, _get_safe)
    if verdict_fetch == Verdict.WARN_PUBLIC_ACCEPTANCE_KPI:
        failures.append(
            ValidationFailure(verdict_fetch, f"public_fetch_telemetry: {msg_fetch}", "live_kpi.public_fetch_*")
        )
    return failures
...
def _check_public_acceptance_kpi(data: dict, _get=_get_safe) -> tuple[Verdict | None, str]:
    ...
    if attempted and attempted > 100:
        ...
        if rate < 0.01:
            issues.append(
                f"CRITICAL: acceptance_rate={rate:.1%} (accepted={accepted}/{attempted}) — systemic rejection"
            )
        elif rate < 0.1:
    ...
    if reasons and isinstance(reasons, dict):
        ...
        if top_count > (attempted or 0) * 0.5:
            issues.append(
                f"top reject reason={top_reason!r} accounts for {top_count}/{attempted} ({top_count / (attempted or 1):.0%})"
            )
    ...
    return (
        Verdict.PASS_MULTISOURCE_TERMINALITY,
        f"public_acceptance OK rate={(accepted or 0) / (attempted or 1):.1%} next_action={next_act}",
    )
...
def _check_public_fetch_telemetry(data: dict, _get=_get_safe) -> tuple[Verdict | None, str]:
    ...
    if fetch_attempted > 0 and accept_attempted is not None:
        ...
        if pass_rate < 0.05:
            return (
                Verdict.WARN_PUBLIC_ACCEPTANCE_KPI,
                f"pre-acceptance filter drops {1 - pass_rate:.0%} of fetched URLs (fetch={fetch_attempted}, acceptance_input={accept_attempted})",
            )
    return (
        Verdict.PASS_MULTISOURCE_TERMINALITY,
        f"fetch telemetry OK fetch={fetch_attempted} acceptance_input={accept_attempted}",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--allow-hardware-constrained", action="store_true", help="Allow hardware-constrained runs to pass"
    )
