...
def _get_repo_root_reality() -> dict:
    ...
    _cwd_warning = (
        f"WARNING: CWD={_cwd} is outside expected universal root ({_universal}). Artifact scans may glob wrong directory. Use --repo-root {_UNIVERSAL_ROOT} or run from {_UNIVERSAL_ROOT}."
...
_F223_ARTIFACT_ALIASES: dict[str, list[tuple[str, str]]] = {
    ...
    "F223B_TERMINALITY_VERDICT_SSOT": [("probe_f223b_terminality_verdict_ssot", "terminality_verdict_ssot.json")],
    ...
    "F223D_PRODUCT_VALUE_REALITY": [("probe_f223d_product_value_reality", "product_value_reality.json")],
    "F223H_CWD_INVOCATION_GUARD": [("probe_f223h_cwd_invocation_guard", "cwd_invocation_guard.json")],
    "F223E_ASYNC_RESOURCE_HYGIENE": [("probe_f223e_async_resource_hygiene", "async_resource_hygiene.json")],
    "F223F_ANALYST_BRIEF_REALITY": [("probe_f223f_analyst_brief_reality", "analyst_brief_reality.json")],
    "F223G_PERSISTENT_DEDUP_AUDIT": [("probe_f223g_persistent_dedup_audit", "persistent_dedup_audit.json")],
}
...
def _check_all_f223_artifacts(
    repo_root: Path,
) -> tuple[list[F223ArtifactResult], list[F223ArtifactResult], list[F223ArtifactResult]]:
    """
...
_RUN_MODE_TEMPLATES: dict[tuple[str, bool], tuple[bool, bool, bool, str, int, int, str]] = {
    ...
    ("hard_block", False): (
        ...
        0,
        0,
        "swap_hard_block: swap={swap_gib:.3f}GiB > {diag_max}GiB",
    ),
    ("diagnostic", True): (
        True,
        True,
        ...
        0,
        0,
        "swap_diagnostic: swap={swap_gib:.3f}GiB in ({clean_max}, {diag_max}]GiB",
    ),
    ("diagnostic", False): (
        False,
        True,
...
def _compute_run_mode(
    ...
    if "{swap_gib}" in reason_tpl:
        investigation_reason = reason_tpl.format(
            swap_gib=swap_gib, clean_max=CLEAN_SWAP_MAX_GIB, diag_max=DIAGNOSTIC_SWAP_MAX_GIB
        )
...
def _render_capability_section(result: OneButtonResult) -> list[str]:
    ...
    if result.capability_live_allowed:
        lines.extend(
            ["", "### Exact Command (Nonfeed Capability)", "", "```bash", result.live_command.get("command", ""), "```"]
        )
        ...
        lines.extend(
            [
                ...
                "```bash",
                result.live_command.get("command", ""),
                "```",
            ]
        )
    else:
...
def _run_self_test(repo_root: Path, profile: str, query: str) -> SelfTestResult:
    ...
    reality = _get_repo_root_reality()
    cwd_contract_ok = (
        reality["cwd_is_universal_root"] and reality["universal_root_exists"] and (not reality["cwd_warning"])
    )
    if reality["cwd_warning"]:
    ...
    if profile == "nonfeed_diagnostic180" and expected_acquisition != "nonfeed_diagnostic":
        ...
        blocking_reasons.append(
            f"acquisition_profile={expected_acquisition} != nonfeed_diagnostic for benchmark profile nonfeed_diagnostic180"
        )
    ...
    if profile == "nonfeed_diagnostic":
        warnings.append(
            "profile is 'nonfeed_diagnostic' — did you mean 'nonfeed_diagnostic180'? nonfeed_diagnostic180 is the benchmark profile that maps to nonfeed_diagnostic acquisition."
        )
...
def _render_self_test_markdown(result: SelfTestResult, profile: str, query: str) -> str:
    ...
    for a in result.artifact_matrix:
        ...
        lines.append(
            f"| {a['probe_dir']} | {a['filename']} | {a['category']} | {found_icon} | {valid_icon} | {blocks_icon} |"
        )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to prelive_decision.json (from prelive_decision_gate.py). If omitted, provider surface check is skipped.",
    )
    p.add_argument(
        "--last-live-triage",
        type=Path,
    ...
    p.add_argument(
        "--self-test",
        action="store_true",
        help="Run self-test mode: validates artifact resolution and expected assertion contract without running live. Never loads MLX or makes network calls. Emits machine-checkable JSON readiness matrix.",
    )
    return p
...
def main() -> int:
    ...
    _write_outputs(
        args.output_json, args.output_md, result.to_dict(), md_text=_render_markdown(result, args.profile, args.query)
    )
