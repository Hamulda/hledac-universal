...
class CommonCrawlAdapter:
    ...
    _index_fetch_failed: bool = False
                ...
                async for line in response.content.iter_lines():
                    ...
                    try:
                        ...
                        if not raw_url or self._is_noise_url(raw_url):
                            continue
                        findings.append(RawFinding(text=raw_url, source=SOURCE_NAME, url=raw_url, metadata={'timestamp': data.get('timestamp')}))
                    except Exception:
                        continue
        except Exception as e:
    ...
    async def search(self, query: str, max_results: int=20) -> list[dict]:
        ...
        """
        clean = re.sub('^(site|domain):', '', query.strip(), flags=re.IGNORECASE).strip()
        if not clean:
            return []
        results = await self.fetch(clean, max_results=max_results)
        return [{'title': f'[CC] {r.url}', 'url': r.url, 'snippet': f"Archived: {(r.metadata or {}).get('timestamp', 'unknown')}", 'source': SOURCE_NAME, 'timestamp': (r.metadata or {}).get('timestamp', '')} for r in results]
