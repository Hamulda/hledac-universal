    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> ast.ClassDef:
            ...
            if isinstance(dc_dec, ast.Call):
                ...
                if not has_slots_kw and (not has_existing_slots):
                    ...
                    self.changes.append(
                        {
                            "type": "dataclass",
                            "file": getattr(node, "_filepath", "?"),
                            "line": node.lineno,
                            "name": node.name,
                            "action": "added slots=True",
                        }
                    )
            else:
                idx = node.decorator_list.index(dc_dec)
                new_dec = ast.Call(
                    func=ast.Name(id="dataclass", ctx=ast.Load()), args=[ast.Constant(value=True)], keywords=[]
                )
                ...
                self.changes.append(
                    {
                        ...
                        "line": node.lineno,
                        "name": node.name,
                        "action": "bare @dataclass → @dataclass(slots=True)",
                    }
                )
        ...
        slots_node = ast.Assign(
            ...
            value=ast.Call(
                ...
                args=[ast.Tuple(elts=[ast.Constant(value=a) for a in attrs], ctx=ast.Load())],
                keywords=[],
            ),
        )
        insert_idx = 0
        ...
        self.changes.append(
            {
                ...
                "line": node.lineno,
                "name": node.name,
                "action": f"added __slots__={attrs}",
                "count": len(attrs),
            }
        )
        return self.generic_visit(node)
