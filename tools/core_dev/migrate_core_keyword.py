...
def migrate_file(src_path: Path, dry_run: bool = True) -> tuple[int, int]:
    ...
    content = re.sub(r'(?<![a-zA-Z])from core(\.[a-zA-Z_]\w*)* import', lambda m: m.group(0).replace('from core', 'from _core'), content)
    ...
    substitutions = original.count("from core import") + original.count("import core") + original.count("hledac.universal.core")
