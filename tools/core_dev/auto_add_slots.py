...
class SlotsFixer(ast.NodeTransformer):
            ...
            if isinstance(dec, ast.Name) and dec.id == "dataclass":
                ...
                self.changes.append(
                    Change(
                        ...
                        line=node.lineno,
                        change_type="dataclass_slots",
                        old_decorator="@dataclass",
                        new_decorator="@dataclass(slots=True)",
                    )
                )
                break
                ...
                if not has_slots:
                    ...
                    self.changes.append(
                        Change(
                            file=self.filepath, class_name=node.name, line=node.lineno, change_type="dataclass_slots"
                        )
                    )
                break
    ...
    def _add_slots(self, node: ast.ClassDef, attrs: list[str]) -> None:
        ...
        slots_node = ast.Assign(
            targets=[ast.Name(id="__slots__", ctx=ast.Store())], value=slots_value, lineno=node.lineno, col_offset=0
        )
        insert_idx = 0
        ...
        self.changes.append(
            Change(
                file=self.filepath, class_name=node.name, line=node.lineno, change_type="add_slots", attributes=attrs
            )
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Automatically add __slots__ to classes",
