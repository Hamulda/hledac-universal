...
class SlotsComplianceChecker(ast.NodeVisitor):
            ...
            if isinstance(item, ast.Assign):
                for target in item.targets:
                    if isinstance(target, ast.Name) and target.id == "__slots__":
                        return True
                    if isinstance(target, ast.Assign) and any(
                        isinstance(t, ast.Name) and t.id == "__slots__" for t in target.targets
                    ):
                        return True
        return False
    ...
    def _get_dataclass_info(self, node: ast.ClassDef) -> dict | None:
...
def scan_directory(root: str, exclude_dirs: set[str] | None = None) -> list[str]:
...
def generate_summary(reports: list[FileReport]) -> dict:
    ...
    return {
        ...
        "files_with_slots": files_with_slots,
        "files_plain_no_slots": files_plain_no_slots,
        "total_classes": total_classes,
        "with_explicit_slots": with_slots,
        "with_dataclass_or_msgspec": with_dataclass_slots,
        "plain_no_slots": plain_no_slots,
        "skipped": skipped,
        "slots_compliance_pct": round(with_slots / total_classes * 100 if total_classes > 0 else 0, 1),
        "overall_optimized_pct": round(
            (with_slots + with_dataclass_slots) / total_classes * 100 if total_classes > 0 else 0, 1
        ),
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Check __slots__ compliance across the codebase",
    ...
    if args.json:
            ...
            "files": [
                {
                    ...
                    "with_slots": r.with_slots,
                    "with_dataclass_slots": r.with_dataclass_slots,
                    "plain_no_slots": r.plain_no_slots,
                    "classes": [
                        {
                            "name": c.name,
                            "line": c.line,
                        ...
                        for c in r.classes
                        if c.category == "plain_no_slots"
                    ],
                }
                for r in reports
                if r.plain_no_slots > 0
            ],
        }
        if args.output:
        ...
        print(
            f"  With explicit __slots__:    {summary['with_explicit_slots']:>6}  ({summary['slots_compliance_pct']}%)"
        )
