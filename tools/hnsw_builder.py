...
class IncrementalHNSW:
    ...
    def __init__(
        self, dim: int, max_elements: int = 100000, ef_construction: int = 200, M: int = 16, ef_search: int = 50
    ) -> None:
        """
        ...
        self.index = usearch.index.Index(
            ndim=dim, metric="cos", dtype="f32", connectivity=M, expansion_add=exp_add, expansion_search=ef_search
        )
    ...
    async def add_items(self, vectors: np.ndarray, ids: list[str]) -> None:
        """
        ...
        async with self._lock:
            for id_str, vec in zip(ids, vectors, strict=False):
