...
# =============================================================================
# Keyword Configuration (Externalizable to TOML)
# =============================================================================

# Language keywords with bilingual support (EN/CS)
TOOL_KEYWORDS: dict[str, tuple[str, ...]] = {
    'stealth_crawler': (
        'dark web', 'tor', 'onion', 'hidden service', '.onion', 'darknet',
        'deep web', 'hidden web', 'temný web', 'skrytá služba',
    ),
    'archive_discovery': (
        'archive', 'wayback', 'historical', 'deleted', 'past', 'archiv',
        'historický', 'smazaný', 'minulost', 'web archive', 'internet archive',
        'snapshot', 'cached',
    ),
    'leak_hunter': (
    ...
    'blockchain_analyzer': (
        'blockchain', 'crypto', 'bitcoin', 'ethereum', 'wallet', 'transaction',
        'btc', 'eth', 'cryptocurrency', 'token', 'smart contract', 'defi',
        'nft', 'blockchain analysis',
    ),
    'academic_search': (
        'research', 'paper', 'study', 'scholar', 'arxiv', 'academic', 'výzkum',
        'studie', 'článek', 'publikace', 'vědecký', 'journal', 'conference',
        'thesis', 'dissertation', 'peer reviewed',
    ),
    'identity_stitching': (
        'identity', 'person', 'profile', 'who is', 'individual', 'identita',
        'osoba', 'profil', 'kdo je', 'jednotlivec', 'background check',
        'people search', 'find person',
    ),
    'relationship_discovery': (
        'connection', 'relationship', 'network', 'linked to', 'spojení', 'vztah',
        'síť', 'propojený', 'associated with', 'affiliated', 'related to',
        'connected to',
    ),
    'pattern_mining': (
        'pattern', 'trend', 'anomaly', 'correlation', 'vzor', 'trend', 'anomálie',
        'korelace', 'pattern analysis', 'behavioral pattern', 'usage pattern',
        'frequency analysis',
    ),
    'web_intelligence': (
        'website', 'web', 'online', 'internet', 'domain', 'webová stránka',
        'web', 'online', 'internet', 'doména', 'site analysis', 'web scraping',
        'web monitoring',
    ),
    'news_analyzer': (
        'news', 'article', 'media', 'report', 'press', 'novinky', 'článek',
        'média', 'zpráva', 'tisk', 'breaking news', 'headline', 'journalism',
        'news source',
    ),
    'forum_hunter': (
        'forum', 'discussion', 'community', 'reddit', 'thread', 'fórum', 'diskuze',
        'komunita', 'vlákno', 'message board', 'online community', 'user discussion',
    ),
    'social_mapper': (
        'social media', 'facebook', 'twitter', 'instagram', 'linkedin', 'twitter',
        'x.com', 'tiktok', 'youtube', 'telegram', 'sociální sítě', 'social profile',
        'social account', 'follower', 'post',
    ),
    'document_analyzer': (
        'document', 'pdf', 'file', 'report', 'doc', 'dokument', 'soubor',
        'zpráva', 'pdf analysis', 'document parsing', 'text extraction', 'file analysis',
    ),
    'image_forensics': (
        'image', 'photo', 'picture', 'visual', 'exif', 'obrázek', 'fotografie',
        'foto', 'vizuální', 'image analysis', 'photo analysis', 'reverse image search',
    ),
    'temporal_analyzer': (
        'timeline', 'history', 'when', 'date', 'time', 'časová osa', 'historie',
        'kdy', 'datum', 'čas', 'chronology', 'time series', 'temporal pattern',
        'event sequence',
    ),
    'geolocation_tracker': (
        'location', 'where', 'place', 'geo', 'gps', 'lokace', 'kde', 'místo',
        'geolokace', 'poloha', 'coordinates', 'address', 'geographic', 'mapping',
    ),
    'metadata_extractor': (
        'metadata', 'exif', 'header', 'properties', 'metadata', 'hlavička',
        'vlastnosti', 'file properties', 'file metadata', 'document properties',
        'technical data',
    ),
    'threat_assessor': (
        'threat', 'risk', 'danger', 'malicious', 'attack', 'hrozba', 'riziko',
        'nebezpečí', 'škodlivý', 'útoku', 'security threat', 'cyber threat',
        'risk assessment',
    ),
    'vulnerability_scanner': (
        'vulnerability', 'exploit', 'cve', 'security flaw', 'zranitelnost', 'exploit',
        'bezpečnostní chyba', 'security vulnerability', 'zero day', 'patch', 'bug',
    ),
    'reputation_analyzer': (
        'reputation', 'review', 'rating', 'feedback', 'reputace', 'recenze',
        'hodnocení', 'zpětná vazba', 'online reputation', 'brand reputation',
        'customer review',
    ),
    'cross_reference_engine': (
        'cross reference', 'verify', 'corroborate', 'confirm', 'křížový odkaz',
        'ověřit', 'potvrdit', 'verifikovat', 'fact check', 'verification',
...
PRIVACY_KEYWORDS: dict[str, tuple[str, ...]] = {
    'MAXIMUM': (
        ...
        'maximum privacy', 'total anonymity', 'untraceable', 'temný web',
        'skrytá služba', 'únik', 'citlivý', 'důvěrný',
    ),
    'HIGH': (
        'identity', 'person', 'profile', 'who is', 'private', 'personal data',
        'pii', 'high privacy', 'anonymous', 'identita', 'osoba', 'soukromý',
        'osobní údaje',
    ),
}
...
SOURCE_KEYWORDS: dict[str, tuple[str, ...]] = {
    'surface': (
        'web', 'website', 'internet', 'google', 'search', 'news', 'article',
        'blog', 'forum', 'social media',
    ),
    'archive': (
        'archive', 'wayback', 'historical', 'deleted', 'past', 'archiv',
        'historický', 'snapshot', 'cached',
    ),
    'academic': (
        'research', 'paper', 'study', 'scholar', 'arxiv', 'academic', 'journal',
        'science', 'publication', 'výzkum', 'studie', 'vědecký', 'publikace',
    ),
    'dark': (
        'dark web', 'tor', 'onion', 'darknet', 'hidden', 'temný web', 'skrytý',
        'skrytá služba',
    ),
    'leaked': (
        'leak', 'breach', 'exposed', 'dump', 'compromised', 'unik', 'únik',
        'zveřejněný', 'kompromitovaný',
    ),
}

DEPTH_KEYWORDS: dict[str, tuple[str, ...]] = {
    'EXHAUSTIVE': (
        'comprehensive', 'exhaustive', 'complete', 'thorough', 'everything',
        'all information', 'full analysis', 'deep dive', 'komplexní', 'exhaustivní',
        'úplný', 'důkladný', 'maximum detail', 'in-depth', 'extensive',
    ),
    'DEEP': (
        'deep', 'detailed', 'extensive', 'comprehensive', 'analyze', 'investigation',
        'research', 'study', 'hluboký', 'detailní', 'rozsáhlý', 'analýza',
        'thorough analysis', 'detailed report',
    ),
    'QUICK': (
        'quick', 'brief', 'summary', 'overview', 'fast', 'rychlý', 'stručný',
        'přehled', 'shrnutí', 'quick check', 'brief overview', 'short summary',
    ),
}
...
# =============================================================================
# Compiled Pattern Cache (Memory-Efficient, Thread-Safe)
# =============================================================================
...
def _compile_patterns_for_group(
    group: str, keywords: tuple[str, ...]
...
class AutoResearchProfile:
    ...
    """
    __slots__ = (
        'use_tot', 'tot_mode', 'tools', 'sources', 'privacy_level',
        'use_tor', 'models_needed', 'depth', 'max_time', 'reasoning',
    )
...
# =============================================================================
# Complexity Analyzer (Strategy Pattern)
# =============================================================================
...
class ComplexityAnalyzer:
    ...
    MULTI_STEP_KEYWORDS: tuple[str, ...] = (
        'analyze', 'compare', 'evaluate', 'assess', 'explain', 'determine',
        'investigate', 'explore', 'examine', 'analyzovat', 'porovnat', 'zhodnotit',
    )
    
    ALTERNATIVE_KEYWORDS: tuple[str, ...] = (
        'options', 'pros and cons', 'advantages', 'alternatives', 'compare',
        'versus', 'vs', 'trade-off', 'možnosti', 'výhody', 'different approaches',
        'multiple strategies', 'various methods',
    )
    
    COMPLEXITY_INDICATORS: tuple[str, ...] = (
        'considering', 'factors', 'aspects', 'dimensions', 'trade-offs',
...
class AutonomousAnalyzer:
    ...
    """
    __slots__ = (
        '_tot_integration', '_complexity_analyzer',
        '_tool_patterns', '_privacy_patterns', '_source_patterns', '_depth_patterns',
    )
    ...
    def _determine_privacy(self, query: str, tools: set[str]) -> tuple[str, bool]:
        ...
        # Check tool-based privacy requirements
