...
REPLACEMENT_MAP = {
    ...
    "ASSIGN_NO_RET_EXC_BUG": "safe_gather_ok",
    "RETURN_WITH_RET_EXC": "safe_gather_ok",
    "RETURN_NO_RET_EXC_BUG": "safe_gather_ok",
    "BUG_BARE_NO_RET_EXC": "safe_gather_fire_and_forget",
}
SKIP_PATH_PARTS = {
    ".venv",
    "venv",
...
_RE_FROM_HLEDAC_ASYNCX = re.compile(
    "^(\\s*from\\s+hledac\\.universal\\.utils\\.asyncx\\s+import\\s+)(?P<names>[^\\n]+)$", re.MULTILINE
)
