...
def _find_import_sites(content: str, filepath: str) -> list[ImportSite]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.If):
            ...
            if node.module and (
                node.module == "hledac_rust_extensions" or node.module.startswith("hledac_rust_extensions.")
            ):
                names = []
...
def _is_submodule_import(names: tuple[tuple[str, str | None], ...]) -> bool:
    ...
    return all(
        not name[0].startswith("_") and name[0][0].islower() for name in names if name[0] not in KNOWN_SUBMODULES
    )
...
def _replace_in_source(content: str, sites: list[ImportSite]) -> tuple[str, list[str]]:
    ...
    for site in sorted_sites:
        ...
        if start_line < len(lines):
            ...
            re.match(r"(\s*)", lines[start_line]).group(1) if lines[start_line].strip() else ""
            lines[start_line] = indented

        # Remove following lines if multi-line
        while (
            len(lines) > start_line + 1
            and lines[start_line + 1].strip()
            and not lines[start_line + 1].strip().startswith("#")
        ):
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Migrate hledac_rust_extensions imports to facade pattern")
