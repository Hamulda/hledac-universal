...
def _clear_hledac_modules() -> None:
...
def _root_import_benchmark(*, runs: int = 3, warmups: int = 1, quick: bool = False) -> dict[str, Any]:
    ...
    with package_import_context():
        ...
        cold_result = time_many(
            lambda: (_clear_hledac_modules(), gc.collect(), importlib.import_module("hledac.universal")),
            runs=runs,
            warmups=warmups,
        )
        ...
        return {
            ...
            "first_access_summary": {
                attr: summarize_samples(samples) for attr, samples in first_access_samples.items()
            },
            "cached_access_summary": {},
        }


def _print_import_summary(s: dict[str, float], prefix: str = "    ") -> None:
    print(
        f"{prefix}min={s['min_ms']:.3f}ms  median={s['median_ms']:.3f}ms  p95={s['p95_ms']:.3f}ms  max={s['max_ms']:.3f}ms  runs={s['runs']}"
    )
...
def _config_import_benchmark(*, runs: int = 3, warmups: int = 1, quick: bool = False) -> dict[str, Any]:
    ...
    with package_import_context():
        ...
        for label, mod_name in [
            ("config", "hledac.universal.config"),
            ("project_types", "hledac.universal.project_types"),
        ]:
            key_ms = f"{label}_ms"
...
def _zstd_benchmark(*, runs: int = 3, warmups: int = 1, quick: bool = False) -> dict[str, Any]:
    ...
    print(
        f"  raw={raw_size / 1024:.1f} KiB  compressed={compressed_size / 1024:.1f} KiB  ratio={_fmt_ratio(raw_size, compressed_size)}"
    )
...
def _json_benchmark(*, runs: int = 3, warmups: int = 1, quick: bool = False) -> dict[str, Any]:
    ...
    speedup_d = (
        json_dumps_result["summary"]["median_ms"] / orjson_dumps_result["summary"]["median_ms"]
        if orjson_dumps_result["summary"]["median_ms"] > 0
        else 0
    )
    speedup_l = (
        json_loads_result["summary"]["median_ms"] / orjson_loads_result["summary"]["median_ms"]
        if orjson_loads_result["summary"]["median_ms"] > 0
        else 0
    )
...
def _topk_benchmark(*, runs: int = 3, warmups: int = 1, quick: bool = False) -> dict[str, Any]:
    ...
    speedup = (
        sorted_result["summary"]["median_ms"] / heapq_result["summary"]["median_ms"]
        if heapq_result["summary"]["median_ms"] > 0
        else 0
    )
...
async def _run_async_semaphore(*, runs: int = 3, warmups: int = 1, quick: bool = False) -> dict[str, Any]:
    ...
    overhead_pct = (
        (sem_s["median_ms"] - plain_s["median_ms"]) / plain_s["median_ms"] * 100 if plain_s["median_ms"] > 0 else 0
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--runs", type=int, default=None, help="Number of measured runs per benchmark. Default: 3 (quick), 7 (full)"
    )
    parser.add_argument(
        "--warmups", type=int, default=None, help="Number of warm-up runs. Default: 1 (quick), 2 (full)"
    )
