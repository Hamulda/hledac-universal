...
PROJECT_ROOT = BENCH_FILE.parents[3]
...
def summarize_samples(samples_ms: list[float]) -> dict[str, float]:
    ...
    median_idx = n // 2
    return {'min_ms': round(sorted_samples[0], 4), 'median_ms': round(sorted_samples[median_idx], 4), 'mean_ms': round(sum(sorted_samples) / n, 4), 'p95_ms': round(sorted_samples[p95_idx], 4), 'max_ms': round(sorted_samples[-1], 4), 'runs': n}
...
def time_many(fn: Callable[[], Any], *, runs: int=7, warmups: int=1) -> dict[str, Any]:
    ...
    return {'status': 'ok', 'samples_ms': [round(s, 4) for s in samples_ms], 'summary': summarize_samples(samples_ms), 'warmups': warmups, 'runs': runs}
...
def _root_import_benchmark(*, runs: int=3, warmups: int=1, quick: bool=False) -> dict[str, Any]:
    ...
    with package_import_context():
        import importlib
        cold_result = time_many(lambda: (_clear_hledac_modules(), gc.collect(), importlib.import_module('hledac.universal')), runs=runs, warmups=warmups)
        ...
        premature = [m for m in ('hledac.universal.fetching.public_fetcher', 'hledac.universal.knowledge.duckdb_store', 'hledac.universal.patterns.pattern_matcher', 'hledac.universal.config') if m in sys.modules]
        if premature:
        ...
        for attr in lazy_attrs:
            ...
            _print_import_summary(s, f'    {attr}: ')
        return {'cold_import_ms': import_ms_samples, 'cold_import_summary': cold_summary, 'cached_import_summary': cached_summary, 'first_access_summary': {attr: summarize_samples(samples) for attr, samples in first_access_samples.items()}, 'cached_access_summary': {}}
...
def _duckdb_store_benchmark(*, runs: int=3, warmups: int=1, quick: bool=False) -> dict[str, Any]:
    ...
    with package_import_context():
        ...
        def cold_first_access():
            for name in list(sys.modules):
                if name == 'hledac' or name.startswith('hledac.universal') or name == 'duckdb' or name.startswith('duckdb.') or (name == 'orjson'):
                    del sys.modules[name]
            gc.collect()
            importlib.import_module('hledac.universal')
        ...
        _print_import_summary(cached_summary, '    ')
        return {'cold_first_access_summary': cold_summary, 'cached_access_summary': cached_summary, 'cold_samples_ms': cold_result['samples_ms'], 'cached_samples_ms': cached_result['samples_ms']}
...
async def _hash_identifier_impl(n_calls: int) -> float:
    ...
    hi = HashIdentifier()
    test_hashes = ['$2a$12$LQv3c1yqBWVHxkd0EZHAKmeQ3N0cMfhOLPq7Z0eNH6T2d3.QVJDXG', '$argon2id$v=19$m=19456,t=2,p=1$', '5f4dcc3b5aa765d61d8327deb882cf99', 'e802d3f60fa8f8c8f5e9a9d9e0e0e0e0e0e0e0e0', '$pbkdf2_sha256$60000$', '$1$2y$10$', '$sha512$', 'e802d3f60fa8f8c8f5e9a9d9e0e0e0e0e0e0e0e0', 'aGVsbG93b3JsZGhlbGxvd29ybGRoZWxsb3dvcmxkaGVsbG8=']
...
async def _run_hash_identifier(*, runs: int=3, warmups: int=1, quick: bool=False) -> dict[str, Any]:
    ...
    n_calls = 500 if quick else 10000
    ...
    return {'status': 'ok', 'samples_ms': [round(s, 4) for s in samples_ms], 'summary': s, 'warmups': warmups, 'runs': runs, 'calls_per_sec': round(calls_per_sec, 1), 'n_calls': n_calls}
_HASH_TEMPLATES = ['$2a${len}$12$LQv3c1yqBWVHxkd0EZHAKmeQ3N0cMfhO', '$argon2id$v=19$m=19456,t=2,p=1$YWFhYWFhYWFhYWFh', '5f4dcc3b5aa765d61d8327deb882cf99', 'e802d3f60fa8f8c8f5e9a9d9e0e0e0e0e0e0e0e0', '$pbkdf2_sha256$60000$dmFsaWQ', '$1$2y$10$VVVVVVVVVVVVVVVVVVVVVVVVVV', 'aGVsbG93b3JsZGhlbGxvd29ybGRoZWxsb3dvcmxkaGVsbG8=', '0000000000000000000000000000000000000000', '$H$1$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA', '$P$B$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA']
...
async def _run_hash_identifier_file(*, runs: int=3, warmups: int=1, quick: bool=False) -> dict[str, Any]:
    ...
    from text.hash_identifier import HashIdentifier
    n_lines = 100 if quick else 1000
    ...
    try:
        ...
        return {'n_lines': n_lines, 'findings': findings_count, 'summary': result_summary, 'samples_ms': samples_ms, 'status': status}
    finally:
        try:
            os.unlink(tmp_path)
        except OSError:
...
def _zstd_benchmark(*, runs: int=3, warmups: int=1, quick: bool=False) -> dict[str, Any]:
    ...
    payload = {'evidence': [{'title': f'Investigation Report Case {i}', 'content': 'Lorem ipsum dolor sit amet consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam quis nostrud exercitation ullamco laboris.', 'tags': ['osint', 'threat-intel', 'domain', f'case-{i % 50}'], 'hash': 'a' * 64} for i in range(200)], 'metadata': {'source': 'F214 benchmark harness', 'version': '1.0', 'timestamp': '2026-05-14T00:00:00Z'}}
    ...
    return {'raw_size_bytes': raw_size, 'compressed_size_bytes': compressed_size, 'compression_ratio_pct': round((raw_size - compressed_size) / raw_size * 100, 1) if raw_size > 0 else 0, 'compress_summary': compress_result['summary'], 'decompress_summary': decompress_result['summary'], 'orjson_summary': orjson_ms['summary'], 'compress_samples_ms': compress_result['samples_ms'], 'decompress_samples_ms': decompress_result['samples_ms']}
...
def _json_benchmark(*, runs: int=3, warmups: int=1, quick: bool=False) -> dict[str, Any]:
    ...
    n_items = 1000 if quick else 5000
    payload = [{'id': i, 'name': f'item_{i}', 'status': 'active' if i % 2 == 0 else 'pending', 'score': round(i * 0.123, 4), 'tags': ['osint', 'threat', f'tag-{i % 20}']} for i in range(n_items)]
    ...
    return {'n_items': n_items, 'json_dumps_summary': json_dumps_result['summary'], 'json_loads_summary': json_loads_result['summary'], 'orjson_dumps_summary': orjson_dumps_result['summary'], 'orjson_loads_summary': orjson_loads_result['summary'], 'orjson_speedup_dumps': round(speedup_d, 2), 'orjson_speedup_loads': round(speedup_l, 2)}

def _topk_benchmark(*, runs: int=3, warmups: int=1, quick: bool=False) -> dict[str, Any]:
    _section('7. Top-K Benchmark (sorted vs heapq)')
    import heapq
    import random
    n_items = 10000 if quick else 50000
    k = 20
    ...
    return {'n_items': n_items, 'k': k, 'sorted_summary': sorted_result['summary'], 'heapq_summary': heapq_result['summary'], 'speedup': round(speedup, 2)}
...
async def _async_semaphore_impl(n_tasks: int, sem_limit: int):
    ...
    async def plain_gather():
        return await parallel_ok(*(plain_task(i) for i in range(n_tasks)), label='bench_f214_python314_runtime:748')

    async def semaphore_gather():
        sem = asyncio.Semaphore(sem_limit)
        return await parallel_ok(*(sem_task(i, sem) for i in range(n_tasks)), label='bench_f214_python314_runtime:752')
    t0 = time.perf_counter()
...
async def _run_async_semaphore(*, runs: int=3, warmups: int=1, quick: bool=False) -> dict[str, Any]:
    ...
    return {'n_tasks': n_tasks, 'semaphore_limit': sem_limit, 'plain_summary': plain_s, 'semaphore_summary': sem_s, 'overhead_pct': round(overhead_pct, 2), 'plain_samples_ms': [round(s, 4) for s in plain_samples], 'semaphore_samples_ms': [round(s, 4) for s in sem_samples]}
...
def _memory_snapshot(quick: bool=False) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {'ru_maxrss_raw': rss_raw, 'ru_maxrss_kib': round(rss_kib, 3), 'ru_maxrss_mib': round(rss_kib / 1024, 3), 'units': {'ru_maxrss_raw': 'bytes on darwin / KiB on linux (unconverted)'}}
    if _HAS_PSUTIL:
...
def _run_sync_benchmark(name: str, fn: Callable[..., dict[str, Any]], **kwargs) -> dict[str, Any]:
    try:
        return fn(**kwargs)
    except Exception as exc:
...
async def _run_async_benchmark(name: str, fn: Callable[..., Any], **kwargs) -> dict[str, Any]:
    try:
...
async def _run_all(quick: bool, runs: int, warmups: int) -> dict[str, Any]:
    ...
    sync_benchmarks: list[tuple[str, Callable[..., dict[str, Any]]]] = [('root_import', _root_import_benchmark), ('duckdb_store', _duckdb_store_benchmark), ('config_import', _config_import_benchmark), ('zstd', _zstd_benchmark), ('json', _json_benchmark), ('topk', _topk_benchmark), ('memory_snapshot', _memory_snapshot)]
    ...
    async_benchmarks: list[tuple[str, Callable[..., Any]]] = [('hash_identifier', _run_hash_identifier), ('hash_identifier_file', _run_hash_identifier_file), ('async_semaphore', _run_async_semaphore)]
...
def main() -> None:
    ...
    parser.add_argument('--runs', type=int, default=None, help='Number of measured runs per benchmark. Default: 3 (quick), 7 (full)')
    parser.add_argument('--warmups', type=int, default=None, help='Number of warm-up runs. Default: 1 (quick), 2 (full)')
...
def _run_main(quick: bool, runs: int, warmups: int, args: argparse.Namespace, real_stdout: Any) -> None:
    ...
    if args.json:
        output = {'status': 'ok', 'platform': sys.platform, 'python_version': f'{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}', 'quick': quick, 'runs': runs, 'warmups': warmups, 'has_orjson': _HAS_ORJSON, 'has_psutil': _HAS_PSUTIL, 'has_zstd': _HAS_ZSTD, 'benchmarks': results}
        json_bytes = orjson.dumps(output, option=orjson.OPT_INDENT_2) if _HAS_ORJSON else json.dumps(output, indent=2)
