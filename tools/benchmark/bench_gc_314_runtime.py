...
def get_gc_snapshot() -> GCSnapshot:
    try:
        ...
        gc.collect()
    return GCSnapshot(threshold=gc.get_threshold(), count=gc.get_count(), stats=gc.get_stats() if hasattr(gc, 'get_stats') else [], collections_total=sum((s.get('collections', 0) for s in gc.get_stats())) if hasattr(gc, 'get_stats') else 0)
...
def get_mem_snapshot() -> MemorySnapshot:
    ...
    return MemorySnapshot(rss_mb=mem_info.rss / (1024 * 1024), swap_mb=swap_mb, vm_used_gb=vm.used / 1024 ** 3, vm_available_gb=vm.available / 1024 ** 3)
...
async def _lightweight_sprint(duration_s: float=15) -> dict:
    ...
    return {'elapsed_s': elapsed, 'cycles': cycle, 'errors': errors}
...
async def _boot_smoke(duration_s: float=35) -> dict:
    ...
    elapsed = time.monotonic() - start
    return {'elapsed_s': elapsed, 'modules_loaded': len(succeeded), 'modules_failed': len(failed), 'errors': errors}
...
def render_report(report: BenchmarkReport) -> str:
    ...
    lines = ['# F214G: Python 3.14.4 vs 3.14.5+ GC Reality Benchmark', '', f"**Generated:** {time.strftime('%Y-%m-%d %H:%M:%S')}", '**Platform:** MacBook Air M1 8GB, Darwin', '', '## Environment', '', '| Item | Value |', '|------|-------|', f'| Python version | `{report.python_version}` |', f'| Python version_info | `{report.python_version_info}` |', f'| gc.threshold | `{report.gc_threshold}` |', f'| psutil available | `{psutil is not None}` |', '', '## GC Threshold Interpretation', '', '```', f'gc.get_threshold() = {report.gc_threshold}', '```', '', '**3.14.4 incremental GC** threshold: `(2000, 10, 0)` —', 'This means: minor collection every 2000 allocations, full collection every', '2000 × 10 = 20,000 allocations. With the default 700 RSS match working set,', 'incremental GC introduces per-collection pauses.', '', '**3.13 generational GC** threshold: `(700, 10, 10)` —', 'Classic generational: minor (gen0) every 700, promoted to gen1 every', '700 × 10 = 7000, promoted to gen2 every 700 × 10 × 10 = 70,000.', '', '## GC Sites Audit', '', f'Total `gc.collect()` call sites found: **{report.gc_sites_audited}**', '', '| Category | Count |', '|----------|-------|']
    ...
    for phase in report.phases:
        ...
        lines.extend([f'### Phase: {phase.name}', '', '| Metric | Value |', '|--------|-------|', f'| Wall clock | {phase.wall_clock_s:.2f}s |', f'| GC collections delta | {phase.gc_collections_delta} |', f'| gc.threshold before | `{phase.gc_before.threshold}` |', f'| gc.threshold after | `{phase.gc_after.threshold}` |', f'| gc.count before | `{phase.gc_before.count}` |', f'| gc.count after | `{phase.gc_after.count}` |', f'| RSS before | {phase.mem_before.rss_mb:.1f} MB |', f'| RSS after | {phase.mem_after.rss_mb:.1f} MB |', f'| RSS delta | {delta_mem:+.1f} MB |', f'| RSS peak | {phase.mem_peak_mb:.1f} MB |', f'| Swap used peak | {report.swap_peak_mb:.1f} MB |'])
        ...
        lines.append('')
    lines.extend(['## Swap Analysis', '', f'Swap peak during benchmark: **{report.swap_peak_mb:.1f} MB**', '', 'If swap > 0 MB during any phase, the M1 8GB UMA is under memory pressure.', 'Incremental GC (3.14.4) can trigger more frequent minor collections,', 'which may push the working set above the UMA ceiling.', ''])
    lines.extend(['## Recommendation', '', f'**{report.recommendation}**', ''])
    if report.recommendation == 'NO_PATCH':
        lines.extend(['NO GC policy change is warranted based on this benchmark.', 'Criteria for NO_PATCH:', '  - Swap remained ≤ 10 MB throughout all phases', '  - Wall clock within 10% of expected', '  - No SIGINT cleanup warnings', '  - gc.collections_delta within expected range for workload', '', 'The incremental GC in 3.14.4 does not materially degrade Hledac runtime', 'on M1 8GB UMA under the tested workloads.'])
    else:
        lines.extend(['Evidence suggests GC policy review is warranted.', 'Criteria for PATCH (document which gc.collect sites can be deferred', 'or removed, and which emergency sites must be preserved):', '  - Swap appeared during any lightweight phase', '  - Wall clock significantly exceeded expectations', '  - gc.collections_delta indicates thrashing', '  - SIGINT cleanup warnings present'])
    lines.extend(['', '## Notes', ''])
    for note in report.notes:
        lines.append(f'- {note}')
    lines.extend(['', '## Rerun Instructions for Python 3.14.5+', '', '```bash', 'cd /Users/vojtechhamada/PycharmProjects/Hledac', 'uv python install 3.14.5', 'cd hledac/universal', 'rm -rf .venv', 'uv venv --python 3.14.5 --managed-python', 'source .venv/bin/activate', 'uv sync', 'PYTHONPATH=/Users/vojtechhamada/PycharmProjects/Hledac \\', 'PYTHON_DISABLE_REMOTE_DEBUG=1 \\', 'python tools/bench_gc_314_runtime.py', '```', '', '## 3.14.4 vs 3.14.5 Expected Differences', '', '| Metric | 3.14.4 (incremental) | 3.14.5+ (generational) |', '|--------|---------------------|----------------------|', '| gc.threshold | `(2000, 10, 0)` | `(700, 10, 10)` |', '| GC pause model | incremental, more frequent | generational, batched |', '| M1 8GB impact | possible RSS pressure | similar or better |', '| Swap sensitivity | higher | lower |', ''])
    return '\n'.join(lines)
_g_sigint_count = 0
...
async def main() -> int:
    ...
    parser.add_argument('--label', default='auto', help='Label for this benchmark run (default: auto = python version)')
    parser.add_argument('--out', default=None, help='Optional JSON file to write raw BenchmarkReport as JSON')
    ...
    gc_categories = {'B) Emergency UMA GC': 3, 'C) Periodic runtime GC': 10, 'A) Shutdown / teardown GC': 5, 'D) Tests only': 0, 'E) Dead / legacy': 2}
    gc_sites_audited = 24
    ...
    phase_import = await run_phase('Module Import', lambda: _import_hledac_modules(), timeout_s=30)
    ...
    phase_boot = await run_phase('Boot Smoke 35s', lambda: _boot_smoke(duration_s=35), timeout_s=45)
    ...
    phase_sprint = await run_phase('Lightweight Sprint 15s', lambda: _lightweight_sprint(duration_s=15), timeout_s=25)
    ...
    phase_gc = await run_phase('Post-Sprint GC', lambda: _sprint_gc_pressure(), timeout_s=30)
    ...
    swap_appeared = swap_delta_mb > 100
    boot_ok = 30 <= phase_boot.wall_clock_s <= 40
    sprint_ok = 10 <= phase_sprint.wall_clock_s <= 20
    notes: list[str] = [f'SIGINT cleanup warnings: {_g_sigint_count}', f'Swap peak: {swap_peak_mb:.1f} MB', f'Boot phase wall clock: {phase_boot.wall_clock_s:.2f}s (expected 35s ±5s)', f'Sprint phase wall clock: {phase_sprint.wall_clock_s:.2f}s (expected 15s ±5s)']
    if swap_appeared:
    ...
    overall_pass = not swap_appeared and boot_ok and sprint_ok and (not sigint_warnings)
    ...
    report = BenchmarkReport(python_version=python_version, python_version_info=version_info, gc_threshold=gc_threshold, phases=phases, gc_sites_audited=gc_sites_audited, gc_categories=gc_categories, swap_peak_mb=swap_peak_mb, overall_pass=overall_pass, recommendation=recommendation, notes=notes)
    if out_path:
        report_json = {'python_version': report.python_version, 'python_version_info': list(report.python_version_info), 'gc_threshold': list(report.gc_threshold), 'swap_peak_mb': report.swap_peak_mb, 'overall_pass': report.overall_pass, 'recommendation': report.recommendation, 'notes': report.notes, 'gc_sites_audited': report.gc_sites_audited, 'gc_categories': report.gc_categories, 'phases': [{'name': p.name, 'wall_clock_s': p.wall_clock_s, 'gc_collections_delta': p.gc_collections_delta, 'gc_threshold_before': list(p.gc_before.threshold), 'gc_threshold_after': list(p.gc_after.threshold), 'rss_peak_mb': p.mem_peak_mb, 'errors': p.errors} for p in phases]}
