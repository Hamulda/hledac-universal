...
class SourceReputation(Struct):
    ...
    def compute_rates(self) -> None:
        ...
        self.overall_score = max(0.0, min(1.0, 0.45 * self.corroboration_rate - 0.25 * self.contested_rate - 0.15 * self.drift_rate - 0.15 * self.blocked_rate))
...
class ToolRegistry:
    ...
    """
    __slots__ = tuple(('_call_counts', '_semaphores', '_tools'))
    ...
    def get_tools_by_risk(self, max_risk: RiskLevel) -> list[Tool]:
        ...
        return [tool for tool in self._tools.values() if risk_order[tool.cost_model.risk_level] <= max_level]

    def get_network_tools(self) -> list[Tool]:
        """Get all tools requiring network access."""
        return [tool for tool in self._tools.values() if tool.cost_model.network]

    def get_high_memory_tools(self, threshold_mb: int=500) -> list[Tool]:
        """Get tools with high memory requirements."""
        return [tool for tool in self._tools.values() if tool.cost_model.ram_mb_est >= threshold_mb]
...
__all__ = ['ToolRegistry', 'Tool', 'CostModel', 'CostSummary', 'BudgetLimits', 'RateLimits', 'RiskLevel', 'SourceReputation']