...
class DeepWebHintsExtractor:
    ...
    JS_MARKERS = {
        "__NEXT_DATA__": "__NEXT_DATA__\\s*=\\s*",
        "__NUXT__": "__NUXT__\\s*=\\s*",
    ...
    __slots__ = ("_api_patterns", "_js_pattern", "logger")
    ...
    def _compute_hints_hash(self, hints: DeepWebHints) -> str:
        ...
        stable = {
            "forms": [
                {"a": f.get("action", ""), "m": f.get("method", ""), "n": len(f.get("fields", []))} for f in hints.forms
            ],
            "api": sorted(hints.api_candidates)[:10],
