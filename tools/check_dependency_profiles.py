...
SYNC_PROFILES: dict[str, ProfileCheck] = {'default': ProfileCheck(name='default', uv_sync_args=[], import_smoke=['aiohttp', 'duckdb', 'lmdb', 'msgspec', 'xxhash', 'ahocorasick'], skip_guard="sys.platform != 'darwin' or platform.machine() != 'arm64'"), 'm1-local': ProfileCheck(name='m1-local', uv_sync_args=['--extra', 'm1-local', '--extra', 'dev'], import_smoke=['mlx', 'selectolax', 'duckdb', 'pyarrow', 'rapidfuzz']), 'graph-storage': ProfileCheck(name='graph-storage', uv_sync_args=['--extra', 'graph-storage'], import_smoke=['duckdb', 'lancedb', 'pyarrow', 'polars']), 'osint-html': ProfileCheck(name='osint-html', uv_sync_args=['--extra', 'osint-html'], import_smoke=['selectolax', 'curl_cffi']), 'transport': ProfileCheck(name='transport', uv_sync_args=['--extra', 'transport'], import_smoke=['h2']), 'dev': ProfileCheck(name='dev', uv_sync_args=['--extra', 'dev'], import_smoke=['pytest', 'ruff', 'mypy'])}
NO_SYNC_PROFILES: dict[str, ProfileCheck] = {'no-torch-default': ProfileCheck(name='no-torch-default', uv_sync_args=[], import_smoke=[], verify_fn='_check_torch_not_in_default'), 'no-browser-default': ProfileCheck(name='no-browser-default', uv_sync_args=[], import_smoke=[], verify_fn='_check_browser_not_in_default')}
...
def get_uv_tracked_packages() -> set[str]:
    ...
    """
    result = subprocess.run(['uv', 'pip', 'list', '--format=freeze'], capture_output=True, text=True, cwd=str(_PROJECT_ROOT))
...
def get_site_packages_dirs() -> set[str]:
    ...
    """
    code = 'import importlib.metadata; for d in importlib.metadata.distributions():     print(d.name)'
    result = subprocess.run([str(_VENV_PYTHON), '-c', code], capture_output=True, text=True)
    if result.returncode != 0:
        return set()
    packages: set[str] = set()
...
def check_drift(strict: bool=False) -> tuple[bool, str, list[str]]:
    ...
    benign = {'__pycache__', '.cache', 'python_bootstrap', 'python3.14', 'python3.13', 'python3.12', 'distutils', 'setuptools', 'pip', 'wheel', '_yaml', '_multiprocess', '_pytest', '_polars_runtime_32', '_sounddevice_data', '_duckdb_stubs'}
...
def run_sync_profile(profile: ProfileCheck, verbose: bool=False) -> tuple[bool, str]:
    ...
    if profile.skip_guard:
        guard_code = f'import sys, platform; sys.exit(0 if ({profile.skip_guard}) else 1)'
        guard_result = subprocess.run([str(_VENV_PYTHON), '-c', guard_code], capture_output=True, text=True)
    ...
    sync_result = subprocess.run(sync_cmd, capture_output=True, text=True, cwd=str(_PROJECT_ROOT))
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Dependency profile smoke checks for uv-managed hledac/universal')
    parser.add_argument('--profile', action='append', dest='profiles', help='Profile to check (can be repeated). Default: all profiles.')
    parser.add_argument('--drift', action='store_true', help='Run drift check only (compare uv pip list vs site-packages).')
    parser.add_argument('--strict', action='store_true', help='Make drift check a failure (exit 1 if untracked packages found).')
