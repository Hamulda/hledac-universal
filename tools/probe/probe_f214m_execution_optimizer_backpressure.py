...
async def execute_parallel_bounded(tasks: list[Any], max_pending: int, thread_pool) -> list[Any]:
    ...
    for i in range(0, len(tasks), max_pending):
        ...
        chunk_results = await asyncio.gather(
            *[t() if inspect.iscoroutinefunction(t) else loop.run_in_executor(thread_pool, t) for t in chunk],
...
async def probe_len_tasks_histogram() -> None:
    ...
    print(
        f"  → Range: {min(coordinator_tasks)}-{max(coordinator_tasks)}, P95~{sorted(coordinator_tasks)[int(len(coordinator_tasks) * 0.95)]:.0f}"
    )
...
async def probe_memory_pressure() -> None:
    ...
    async def measure(
        name: str, task_count: int, task_factory: Callable, use_bounded: bool = False, max_pending: int = 32
    ) -> None:
        gc.collect()
        tracemalloc.start()
        start = time.time()
        ...
        if use_bounded:
            ...
            await asyncio.gather(
                *[t() if inspect.iscoroutinefunction(t) else loop.run_in_executor(thread_pool, t) for t in tasks],
                return_exceptions=True,
            )
        ...
        print(
            f"  {name:45s} | tasks={task_count:3d} | time={elapsed * 1000:8.1f}ms | traced_peak={peak / 1024:7.1f}KB | RSS_peak={mem_peak:7.1f}MB"
        )
...
async def probe_exception_behavior() -> None:
...
async def probe_strategy_comparison() -> None:
    ...
    for label, mp, is_serial in configs:
        ...
        tracemalloc.stop()
        print(
            f"  {label:40s} | time={elapsed * 1000:7.1f}ms | traced_peak={peak / 1024:7.1f}KB | results={len(results)}"
        )


async def probe_caller_audit() -> None:
