...
async def execute_parallel_bounded(tasks: list[Any], max_pending: int, thread_pool) -> list[Any]:
    ...
    for i in range(0, len(tasks), max_pending):
        ...
        chunk_results = await asyncio.gather(*[t() if inspect.iscoroutinefunction(t) else loop.run_in_executor(thread_pool, t) for t in chunk], return_exceptions=True)
...
async def probe_memory_pressure():
    ...
    async def measure(name: str, task_count: int, task_factory: Callable, use_bounded: bool=False, max_pending: int=32):
        ...
        if use_bounded:
            ...
            loop = asyncio.get_running_loop()
            await asyncio.gather(*[t() if inspect.iscoroutinefunction(t) else loop.run_in_executor(thread_pool, t) for t in tasks], return_exceptions=True)
        elapsed = time.time() - start
        _, peak = tracemalloc.get_traced_memory()
        tracemalloc.stop()
        mem_peak = process.memory_info().rss / 1024 / 1024
        print(f'  {name:45s} | tasks={task_count:3d} | time={elapsed * 1000:8.1f}ms | traced_peak={peak / 1024:7.1f}KB | RSS_peak={mem_peak:7.1f}MB')
...
async def probe_exception_behavior():
    ...
    loop = asyncio.get_running_loop()
    results = await asyncio.gather(*[t() if inspect.iscoroutinefunction(t) else loop.run_in_executor(thread_pool, t) for t in tasks], return_exceptions=True)
...
async def probe_strategy_comparison():
    ...
    configs = [('unbounded asyncio.gather (current)', None, False), ('bounded max_pending=32', 32, False), ('bounded max_pending=16', 16, False), ('serial (baseline)', None, True)]
    for label, mp, is_serial in configs:
        ...
        if is_serial:
            ...
            results = await parallel_ok(*[t() for t in tasks], label='probe_f214m_execution_optimizer_backpressure:238')
        elapsed = time.time() - start
        _, peak = tracemalloc.get_traced_memory()
        tracemalloc.stop()
...
async def probe_caller_audit():
    ...
    print('=' * 70)
    callers = [('execution_coordinator._execute_parallel_processing', '_parallel_max_tasks = 5; _calculate_task_count(confidence, 5) → 1-5 tasks', 'LOW', 'I/O-bound'), ('resource_allocator.optimize_parallel_execution', 'max_parallel_tasks = min(capacity.cpu_cores, capacity.memory_gb/2, 8) → ≤8 tasks', 'LOW', 'I/O-bound'), ('legacy/autonomous_orchestrator.execute_parallel_search', 'scale unknown; needs grep audit in legacy/ directory', 'MEDIUM', 'mixed')]
