...
def benchmark_compression(raw_data: bytes, name: str, n_runs: int = 100) -> dict[str, CompressionResult]:
    ...
    results["gzip_l1"] = CompressionResult(
        ...
        ratio=gzip_size / len(raw_data),
        rss_peak_kb=rss_gzip,
        verdict="BASELINE",
    )
    if has_zstd:
        for level, level_name in [(1, "zstd_l1"), (3, "zstd_l3")]:
            ...
            results[level_name] = CompressionResult(
                ...
                ratio=zstd_size / len(raw_data),
                rss_peak_kb=rss_zstd,
...
def print_candidate_table(candidates: list[CandidateReport]) -> None:
    ...
    print()
    print(
        f"{'Candidate':<45} {'Format':<12} {'RawB':>7} {'CmprB':>7} {'Ratio':>7} {'CmpUs':>8} {'DcpUs':>8} {'RSSKB':>7} {'Verdict':<15}"
    )
    print("-" * 100)
    for c in candidates:
        for _key, r in c.results.items():
            print(
                f"{c.file_line:<45} {r.format + '_' + str(r.level):<12} {r.raw_bytes:>7} {r.compressed_bytes:>7} {r.ratio:>7.1%} {r.compress_us:>8.1f} {r.decompress_us:>8.1f} {r.rss_peak_kb:>7} {r.verdict:<15}"
            )
        print()
    print("-" * 100)
    print()
    ...
    for c in candidates:
        print(
            f"{c.file_line:<45} {c.artifact_type:<20} {str(c.transient):<10} {str(c.read_path_exists):<10} {str(c.migration_needed):<10} {str(c.gate_passed):<8} {c.patch_decision}"
        )
    print()
...
def write_report(candidates: list[CandidateReport]) -> Path:
    ...
    for c in candidates:
        for _key, r in c.results.items():
            lines.append(
                f"| `{c.file_line}` | {r.format}_l{r.level} | {r.raw_bytes} | {r.compressed_bytes} | {r.ratio:.1%} | {r.compress_us:.1f} | {r.decompress_us:.1f} | {r.rss_peak_kb} | {r.verdict} |"
            )
    lines.extend(
        [
            "",
    ...
    for c in candidates:
        reason_short = (
            c.patch_decision_reason[:80] + "..." if len(c.patch_decision_reason) > 80 else c.patch_decision_reason
        )
        lines.append(
            f"| `{c.file_line}` | {c.artifact_type} | {c.transient} | {c.read_path_exists} | {c.migration_needed} | {c.gate_passed} | **{c.patch_decision}** | {reason_short} |"
        )
    lines.extend(
        [
            "",
