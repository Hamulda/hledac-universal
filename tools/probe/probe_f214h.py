...
def _process_file(
    path: str, entry: os.DirEntry, root_dir: str, file_cache: dict, errors: list, prefix_hash_bytes: int = 4096
) -> dict[str, Any] | None:
    try:
        stat = entry.stat()
...
def run_scan_and_cache_baseline(root_dir: str, max_workers: int = 4) -> dict[str, Any]:
    ...
    if use_parallel:
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(_process_file, p, e, root_dir, file_cache, errors, prefix_hash_bytes): (p, e)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            try:
                for result in executor.map(
                    ...
                    [root_dir] * len(candidates),
                    [file_cache] * len(candidates),
                    [errors] * len(candidates),
                    [prefix_hash_bytes] * len(candidates),
...
if __name__ == "__main__":
    ...
    print("")
    print("BASELINE (submit-all + as_completed):")
    print(f"  Wall time:        {(t1 - t0) * 1000:.1f}ms")
    print(f"  Peak RSS:         {peak_baseline_mb:.1f}MB")
    print("")
    print("BOUNDED (executor.map buffersize=8):")
    print(f"  Wall time:        {(t3 - t2) * 1000:.1f}ms")
    print(f"  Peak RSS:         {peak_bounded_mb:.1f}MB")
    print("")
    print("DIFF:")
    print(
        f"  Memory diff:      {peak_baseline_mb - peak_bounded_mb:+.1f}MB ({('bounded wins' if peak_bounded_mb < peak_baseline_mb else 'baseline wins')})"
    )
    ...
    print("")
    print(
        f"CONCLUSION: {('PATCH IS SAFE — use executor.map(buffersize=8)' if baseline_hashes == bounded_hashes else 'SEMANTIC MISMATCH — do not patch')}"
    )
