...
def gen_text_items(n: int, seed: int = 42) -> list[str]:
    ...
    for i in range(n):
        ...
        noisy = (
            base[:noise_pos] + base[noise_pos : noise_pos + noise_len] + f" item_{i} unique_{rng.randint(0, 999999)}"
        )
        items.append(noisy)
    return items
...
def benchmark_candidate(
    name: str, func: Callable, workload: list, n_iter: int = 3, workers: int = 4
) -> CandidateResult:
...
def run_probe() -> list[CandidateResult]:
    ...
    candidates = [
        (
            "normalize_text (scoring.py)",
            candidate_normalize_text,
            text_items,
            "lowercase, strip, re.sub punctuation, normalize whitespace",
        ),
        ("rrf_fuse (ranking.py)", candidate_rrf_fuse, rrf_items, "Reciprocal Rank Fusion, 50 docs per list"),
        (
            "entity_confidence (entity_extractor.py)",
            candidate_entity_confidence,
            entity_items,
            "Pattern confidence scoring across 8 pattern types",
        ),
        (
            "lang_fallback_detect (language.py)",
            candidate_lang_detect,
            text_items,
            "Pure Python fallback language detection",
        ),
        (
            "extract_keywords (validation.py)",
            candidate_extract_keywords,
            text_items,
            "Keyword extraction with stopword filtering",
        ),
        (
            "html_text_extract (content_extractor.py)",
            candidate_html_extract,
            html_items,
        ...
        ("aho_scan (aho_extractor.py)", candidate_aho_scan, text_items, "Aho-Corasick multi-pattern scan"),
        (
            "regex_scan (aho_extractor.py)",
            candidate_regex_scan,
            text_items,
            "Ground truth substring scan for comparison",
        ),
        ("jaccard_similarity (validation.py)", candidate_jaccard_sim, jaccard_items, "Jaccard text similarity"),
        (
            "markdown_render (sprint_markdown_reporter.py)",
            candidate_markdown_render,
            markdown_items,
