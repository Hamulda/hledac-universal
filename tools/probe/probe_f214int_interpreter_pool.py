...
def candidate_markdown_render(items: list) -> list[str]:
    ...
    for item in items:
        try:
            ...
            results.append('')
    return results
LOREM_TEMPLATE = "This is a sample document about machine learning and artificial intelligence systems. Another article discussing the latest developments in Python programming and software engineering. A report on data analysis and statistical methods for research and development purposes. Content related to web development and cloud computing technologies and infrastructure. Documentation about API design and software architecture patterns and best practices. Machine learning models require careful tuning and validation before deployment to production. Python's ecosystem provides excellent tools for data science and numerical computing. Web scraping and information extraction are core capabilities of modern research tools. Natural language processing enables intelligent analysis of unstructured text data. Information retrieval systems help find relevant documents in large collections efficiently. " * 20
HTML_TEMPLATE = "<html><head><title>Test Document</title></head><body><script>console.log('evil');</script><style>.hidden{display:none;}</style><div class='content'><h1>Heading</h1><p>Paragraph text with <strong>bold</strong> and <em>italic</em> content. More text here to make it realistic.</p><ul><li>Item one</li><li>Item two</li><li>Item three</li></ul><a href='https://example.com'>Link</a><img src='img.png' alt='image'/><noscript>NoScript content</noscript></div></body></html>" * 50
...
def gen_entity_items(n: int, seed: int=42) -> list[tuple]:
    ...
    random.Random(seed)
    patterns = [('email', 'user@example.com'), ('email', 'admin@company.org'), ('domain', 'example.com'), ('domain', 'test.example.org'), ('url', 'https://example.com/page'), ('url', 'http://test.org/api'), ('ipv4', '192.168.1.1'), ('ipv4', '10.0.0.255')]
    items = []
    for i in range(n):
...
def gen_markdown_items(n: int, seed: int=42) -> list[dict]:
    ...
    items = []
    for i in range(n):
        item = {'sprint_id': f'F214X_{i:04d}', 'query': f'test_query_{i}', 'total_findings': rng.randint(10, 500), 'accepted_findings': rng.randint(5, 400), 'rejected_findings': rng.randint(1, 100), 'duration_seconds': rng.randint(60, 3600), 'sources_touched': rng.randint(1, 20), 'findings': [{'finding_id': f'F{i:06d}_{j}', 'source': rng.choice(['ct', 'dns', 'scraped', ' passive']), 'type': rng.choice(['domain', 'ip', 'url', 'cert']), 'confidence': rng.uniform(0.3, 0.99), ' IOC': f'192.168.1.{j}'} for j in range(rng.randint(5, 20))]}
        items.append(item)
    return items

def benchmark_candidate(name: str, func: Callable, workload: list, n_iter: int=3, workers: int=4) -> CandidateResult:
    """Run serial + ThreadPool + InterpreterPool + ProcessPool benchmark."""
    result = CandidateResult(name=name, module_path='', func_name=name, wall_time_ms=0.0, rss_delta_kb=0, serialization_overhead_ms=0.0, import_time_ms=0.0, thread_speedup=0.0, interp_speedup=0.0, process_speedup=0.0, thread_fail='', interp_fail='', process_fail='', notes='')
    gc.collect()
    ...
    try:
        ...
        for _ in range(n_iter):
            ...
            with ThreadPoolExecutor(max_workers=workers) as exc:
                ...
                futures = [exc.submit(func, workload[i * chunk_size:(i + 1) * chunk_size]) for i in range(workers)]
    ...
    return result
_ALLOWED_IMPORT_MODULES = frozenset({'utils.ranking', 'utils.entity_extractor', 'utils.language', 'utils.validation', 'utils.aho_extractor', 'tools.content_extractor', 'export.sprint_markdown_reporter'})
...
def run_probe() -> list[CandidateResult]:
    ...
    print('=' * 60)
    text_items = gen_text_items(10000, seed=42)
    bytes_items = gen_bytes_items(200, seed=42)
    rrf_items = gen_rrf_items(500, seed=42)
    entity_items = gen_entity_items(10000, seed=42)
    html_items = gen_html_items(5000, seed=42)
    jaccard_items = gen_jaccard_items(5000, seed=42)
    markdown_items = gen_markdown_items(500, seed=42)
    candidates = [('normalize_text (scoring.py)', candidate_normalize_text, text_items, 'lowercase, strip, re.sub punctuation, normalize whitespace'), ('rrf_fuse (ranking.py)', candidate_rrf_fuse, rrf_items, 'Reciprocal Rank Fusion, 50 docs per list'), ('entity_confidence (entity_extractor.py)', candidate_entity_confidence, entity_items, 'Pattern confidence scoring across 8 pattern types'), ('lang_fallback_detect (language.py)', candidate_lang_detect, text_items, 'Pure Python fallback language detection'), ('extract_keywords (validation.py)', candidate_extract_keywords, text_items, 'Keyword extraction with stopword filtering'), ('html_text_extract (content_extractor.py)', candidate_html_extract, html_items, 'HTML text extraction without bs4'), ('shannon_entropy (bytes)', candidate_entropy, bytes_items, 'Shannon entropy in bits per byte, proper log2 formula'), ('aho_scan (aho_extractor.py)', candidate_aho_scan, text_items, 'Aho-Corasick multi-pattern scan'), ('regex_scan (aho_extractor.py)', candidate_regex_scan, text_items, 'Ground truth substring scan for comparison'), ('jaccard_similarity (validation.py)', candidate_jaccard_sim, jaccard_items, 'Jaccard text similarity'), ('markdown_render (sprint_markdown_reporter.py)', candidate_markdown_render, markdown_items, 'Markdown report rendering from sprint results dict')]
...
def main():
    ...
    results_data = [{'name': r.name, 'module_path': r.module_path, 'func_name': r.func_name, 'wall_time_ms': r.wall_time_ms, 'rss_delta_kb': r.rss_delta_kb, 'serialization_overhead_ms': r.serialization_overhead_ms, 'import_time_ms': r.import_time_ms, 'thread_speedup': r.thread_speedup, 'interp_speedup': r.interp_speedup, 'process_speedup': r.process_speedup, 'thread_fail': r.thread_fail, 'interp_fail': r.interp_fail, 'process_fail': r.process_fail, 'notes': r.notes} for r in results]
