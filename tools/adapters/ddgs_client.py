...
def search_text_sync(
    query: str, backends: tuple[str, ...] = DEFAULT_TEXT_BACKENDS, max_results_per_backend: int = 4, timeout: int = 6
) -> list[dict]:  # noqa: E501
...
def search_news_sync(
    query: str, backends: tuple[str, ...] = DEFAULT_NEWS_BACKENDS, max_results_per_backend: int = 3, timeout: int = 6
) -> list[dict]:  # noqa: E501
    if not DDGS_AVAILABLE:
