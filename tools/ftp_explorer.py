...
class FTPExplorer:
    ...
    __slots__ = ("logger", "max_bytes", "max_depth", "max_entries", "timeout")
    ...
    async def list(
        self, ftp_url: str, max_depth: int | None = None, max_entries: int | None = None
    ) -> list[FTPListingItem]:
        """
    ...
    async def _list_recursive(
        self, client: aioftp.Client, path: str, remaining_depth: int, entries_limit: int
    ) -> list[FTPListingItem]:
...
async def list_ftp_directory(
    ftp_url: str, max_depth: int = MAX_DEPTH, max_entries: int = MAX_ENTRIES
) -> list[FTPListingItem]:
    """
