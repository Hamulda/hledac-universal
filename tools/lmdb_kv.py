...
class LMDBKVStore:
    ...
    __slots__ = ("_env", "_finalizer", "_map_size", "_max_keys", "_path", "_critical")
    ...
    def __init__(
        ...
        if _USE_CANONICAL and open_lmdb is not None:
            self._env = open_lmdb(
                self._path, map_size=map_size, max_dbs=1, writemap=writemap, metasync=metasync, readahead=False
            )
        else:
            self._env = lmdb.open(
                str(self._path),
...
class AsyncLMDBKVStore:
    ...
    __slots__ = ("_env", "_use_async", "map_size", "path")
