    ...
    *,
    ...
    if not evidence_stripped:
        return GroundingValidationResult(
            grounded=False, checked_items=0, matched_items=0, missing_items=(), reason=REASON_EMPTY_EVIDENCE
        )
    if not claimed_items:
        return GroundingValidationResult(
            grounded=False, checked_items=0, matched_items=0, missing_items=(), reason=REASON_NO_CLAIMED_ITEMS
        )
    ...
    return GroundingValidationResult(
        grounded=all_grounded, checked_items=checked, matched_items=matched, missing_items=tuple(missing), reason=reason
    )
