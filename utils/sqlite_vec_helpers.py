# utils/sqlite_vec_helpers.py
# Phase 7.3: sqlite-vec ANN — M1 8GB native, zero-process overhead
"""
...
# Lazy import — sqlite-vec is optional extra
...
class SqliteVecStore:
    ...
    def __init__(
        self,
        sprint_id: str = _DEFAULT_SPRINT_ID,
        dim: int = MAX_DIM,
    ) -> None:
    ...
    async def upsert(
        ...
        try:
            ...
            self._conn.execute(
                f"DELETE FROM {self.TABLE_NAME} WHERE item_id = ?",
                (item_id,),
            )
            self._conn.execute(
                f"DELETE FROM {self.TABLE_NAME}_meta WHERE item_id = ?",
                (item_id,),
            )
            self._conn.execute(
                f"INSERT INTO {self.TABLE_NAME} (embedding, item_id, metadata) VALUES (?, ?, ?)",
                (vec_bytes, item_id, meta_json),
            )
            self._conn.execute(
                f"INSERT INTO {self.TABLE_NAME}_meta (item_id, text, created_at) VALUES (?, ?, ?)",
                (item_id, text, now),
            )
            self._conn.commit()
            return True
    ...
    # -------------------------------------------------------------------------
    # Entity / Identity helpers (Phase 11.2: consolidate LanceDB → sqlite-vec)
    # -------------------------------------------------------------------------
    ...
    async def upsert_entity(
        ...
        try:
            ...
            self._conn.execute(
                f"INSERT INTO {self.TABLE_NAME} (embedding, item_id, metadata) VALUES (?, ?, ?)",
                (vec_bytes, entity_id, meta_json),
            )

            # Upsert entity metadata (aliases for text/alias search)
            self._conn.execute(
                f"""
    ...
    async def search_entities(
        ...
        try:
            ...
            rows = self._conn.execute(
                ...
                LIMIT ?
                """,
                (vec_bytes, top_k * 3),  # Over-fetch to allow filtering
            ).fetchall()
            ...
            for item_id, distance, meta_json in rows:
                ...
                if item_type != "entity" and not entity_type:
                    # Skip non-entity items if no type filter
                    continue
                results.append(
                    {
                        "id": item_id,
                        "item_id": item_id,
                        "distance": distance,
                        "similarity": 1.0 - distance,  # vec0 distance → similarity
                        "metadata": meta,
                    }
                )
    ...
    async def get_entity(self, entity_id: str) -> dict[str, Any] | None:
        ...
        try:
            row = self._conn.execute(
                f"SELECT item_id, metadata FROM {self.TABLE_NAME} WHERE item_id = ?",
                (entity_id,),
            ).fetchone()
            if not row:
                return None
            ...
            text_row = self._conn.execute(
                f"SELECT text FROM {self.TABLE_NAME}_meta WHERE item_id = ?",
                (entity_id,),
            ).fetchone()
