...
class SqliteVecStore:
    ...
    __slots__ = ("_conn", "_dim", "_path", "_sprint_id")
    ...
    async def upsert(self, item_id: str, embedding: list[float], metadata: dict[str, Any]) -> bool:
        ...
        try:
            ...
            self._conn.execute(
                f"INSERT INTO {self.TABLE_NAME}_meta (item_id, text, created_at) VALUES (?, ?, ?)", (item_id, text, now)
            )
            self._conn.commit()
            return True
        except Exception:
    ...
    async def upsert_entity(
        self, entity_id: str, embedding: list[float], aliases: list[str], extra_metadata: dict[str, Any] | None = None
    ) -> bool:
        """
    ...
    async def search_entities(
        self, query_embedding: list[float], top_k: int = 10, threshold: float = 0.0, entity_type: str | None = None
    ) -> list[dict[str, Any]]:
        """
    ...
    async def get_entity(self, entity_id: str) -> dict[str, Any] | None:
        ...
        try:
            row = self._conn.execute(
                f"SELECT item_id, metadata FROM {self.TABLE_NAME} WHERE item_id = ?", (entity_id,)
            ).fetchone()
            if not row:
                return None
            ...
            text_row = self._conn.execute(
                f"SELECT text FROM {self.TABLE_NAME}_meta WHERE item_id = ?", (entity_id,)
            ).fetchone()
