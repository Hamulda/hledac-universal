...
class ImportOptimizer:
    ...
    __slots__ = tuple(('check_only', 'dry_run', 'files_with_issues'))
    ...
    def _organize_imports(self, content: str, filepath: Path) -> str:
        ...
        stdlib_modules = {'asyncio', 'json', 'time', 'logging', 'pickle', 'uuid', 'hashlib', 'os', 're', 'tempfile', 'shutil', 'zipfile', 'tarfile', 'gzip', 'bz2', 'lzma', 'math', 'sqlite3', 'collections', 'datetime', 'pathlib', 'contextlib', 'enum', 'dataclasses', 'typing', 'urllib'}
    ...
    def _remove_unused_imports(self, content: str, filepath: Path) -> str:
        ...
        try:
            result = subprocess.run([sys.executable, '-m', 'autoflake', '--remove-all-unused-imports', '--remove-unused-variables', '--remove-duplicate-keys', '--stdin-display-name', str(filepath)], input=content.encode('utf-8'), capture_output=True)
...
def main():
    ...
    parser.add_argument('path', nargs='?', default='hledac/', help='Path to optimize (default: hledac/)')
    parser.add_argument('--dry-run', action='store_true', help='Show what would be changed without making changes')
    parser.add_argument('--check-only', action='store_true', help='Check if files need fixing (exit code 1 if issues found)')
