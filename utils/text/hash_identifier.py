...
PATTERN_HASHES: dict[str, str] = {
    ...
    "^\\$P\\$": "phpBB3/WordPress MD5",
    "^\\$H\\$": "phpBB3/WordPress MD5",
    "^\\*[A-F0-9]{40}$": "MySQL5",
    "^sha1\\$": "SHA1 (Django)",
    "^\\{SHA\\}": "SHA1 (Base64)",
    "^\\{SSHA\\}": "SSHA",
...
_COMPILED_PATTERN_HASHES: tuple[tuple[re.Pattern[str], str, str], ...] = tuple(
    ((re.compile(pattern), algo, pattern) for pattern, algo in PATTERN_HASHES.items())
)
_HEX_HASH_SCAN_RE = re.compile("\\b[0-9a-fA-F]{32,128}\\b")
_COMPILED_SCAN_PATTERN_HASHES: tuple[tuple[re.Pattern[str], str], ...] = tuple(
    (re.compile(pattern + "\\S+"), pattern) for pattern in PATTERN_HASHES.keys()
)
        ...
        try:
            ...
            for match in _HEX_HASH_SCAN_RE.finditer(content):
                ...
                if matches:
                    ...
                    findings.append(
                        HashFinding(position=match.start(), hash_string=hash_string, matches=matches, context=context)
                    )
            for compiled_re, _ in _COMPILED_SCAN_PATTERN_HASHES:
                for match in compiled_re.finditer(content):
                    ...
                    if matches:
                        ...
                        findings.append(
                            HashFinding(
                                position=match.start(), hash_string=hash_string, matches=matches, context=context
                            )
                        )
