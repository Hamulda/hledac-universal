...
class FailureEntry:
    ...
    def create(
        ...
        failure_path: Optional[tuple[str, ...]] = None,
    ) -> "FailureEntry":
...
class ComponentHealth:
    ...
    severity_counts: dict[FailureSeverity, int] = field(
        default_factory=lambda: {s: 0 for s in FailureSeverity}
    )
...
class FailureRegistry:
    ...
    async def get_failures(
        ...
        min_severity: Optional[FailureSeverity] = None,
        limit: int = 100,
    ) -> list[FailureEntry]:
    ...
    async def get_summary(self) -> dict[str, Any]:
        ...
        async with self._lock:
            high_critical = [
                ...
                if f.severity.value >= FailureSeverity.HIGH.value
            ]
            return {
                ...
                "component_details": {
                    name: {
                        ...
                        "high_critical": sum(
                            h.severity_counts[s]
                            for s in [FailureSeverity.HIGH, FailureSeverity.CRITICAL]
                        ),
                    }
...
class SprintHealthLedger:
    ...
    def __init__(
        ...
        # Wire up degradation mode updates on failure
    ...
    def _on_failure(self, entry: FailureEntry) -> None:
        ...
        try:
            ...
            logger.debug(
                "[DEGRADATION] No event loop for failure recording, degradation state unchanged"
            )

    def _on_mode_transition(self, old: DegradedMode, new: DegradedMode) -> None:
        """Handle degradation mode transition."""
        self._transitions.append(ModeTransition(
            timestamp=time.monotonic(),
            old_mode=old,
            new_mode=new,
        ...
        logger.warning(
            "[%s] ⚠️ DEGRADATION MODE CHANGE: %s → %s",
            self.sprint_id,
            old.label,
            new.label,
        )
