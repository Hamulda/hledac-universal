...
from hledac.universal.utils.resilience.degradation_modes import DegradationState, DegradedMode, DegradationThresholds, FailureSeverity, ModeTransition, get_degradation_action
...
class ComponentHealth:
    ...
    severity_counts: dict[FailureSeverity, int] = field(default_factory=lambda: {s: 0 for s in FailureSeverity})
...
class FailureRegistry:
    ...
    async def record(self, component: str, severity: FailureSeverity, error: BaseException, context: Optional[dict[str, Any]]=None, failure_path: Optional[tuple[str, ...]]=None) -> FailureEntry:
        """
        ...
        """
        entry = FailureEntry.create(component=component, severity=severity, error=error, context=context, failure_path=failure_path)
        ...
        logger.log(log_level, '[%s] %s failure in %s: %s - %s', self.sprint_id, severity.label, component, entry.error_type, entry.error_message)
        return entry
    ...
    async def get_failures(self, component: Optional[str]=None, min_severity: Optional[FailureSeverity]=None, limit: int=100) -> list[FailureEntry]:
    ...
    async def get_summary(self) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            return {'sprint_id': self.sprint_id, 'total_failures': len(self._failures), 'high_critical_count': len(high_critical), 'components_affected': len(self._components), 'component_details': {name: {'total': h.total_failures, 'last_failure_ts': h.last_failure_ts, 'last_error': h.last_error_type, 'high_critical': sum((h.severity_counts[s] for s in [FailureSeverity.HIGH, FailureSeverity.CRITICAL]))} for name, h in self._components.items()}}
...
class SprintHealthLedger:
    ...
    Usage:
        ...
        ledger.end_sprint()
    """
    _current: Optional['SprintHealthLedger'] = None
    _lock = asyncio.Lock()
    __slots__ = ('_active', '_degradation_state', '_registry', '_transitions', 'end_time', 'sprint_id', 'start_time')

    def __init__(self, sprint_id: str, thresholds: Optional[DegradationThresholds]=None) -> None:
        self.sprint_id = sprint_id
    ...
    @classmethod
    async def start_sprint(cls, sprint_id: str, thresholds: Optional[DegradationThresholds]=None) -> 'SprintHealthLedger':
    ...
    @classmethod
    def start_sprint_sync(cls, sprint_id: str, thresholds: Optional[DegradationThresholds]=None) -> 'SprintHealthLedger':
    ...
    @classmethod
    def get_current(cls) -> Optional['SprintHealthLedger']:
    ...
    def _on_failure(self, entry: FailureEntry) -> None:
        ...
        try:
            ...
            safe_create_task(self._degradation_state.record_failure(entry.severity), name='failure_registry:degradation')
        except RuntimeError:
    ...
    def _on_mode_transition(self, old: DegradedMode, new: DegradedMode) -> None:
        ...
        self._transitions.append(ModeTransition(timestamp=time.monotonic(), old_mode=old, new_mode=new, trigger_counts=dict(self._degradation_state._failure_counts)))
        logger.warning('[%s] ⚠️ DEGRADATION MODE CHANGE: %s → %s', self.sprint_id, old.label, new.label)

    async def record_failure(self, component: str, severity: FailureSeverity, error: BaseException, context: Optional[dict[str, Any]]=None) -> FailureEntry:
    ...
    async def get_health_summary(self) -> dict[str, Any]:
        ...
        return {'sprint_id': self.sprint_id, 'elapsed_seconds': elapsed, 'status': 'ACTIVE' if self._active else 'ENDED', 'degradation': self._degradation_state.to_dict(), 'registry': registry_summary, 'transitions': [{'timestamp': t.timestamp, 'from': t.old_mode.name, 'to': t.new_mode.name} for t in self._transitions], 'health_indicators': self._compute_health_indicators()}

    def _compute_health_indicators(self) -> dict[str, Any]:
        """Compute derived health indicators."""
        mode = self.degradation_mode
        return {'can_continue': mode.value < DegradedMode.EMERGENCY.value, 'sidecars_enabled': not mode.should_skip_sidecars(), 'mlx_enabled': not mode.should_skip_mlx_inference(), 'errors_should_propagate': mode.should_propagate_errors(), 'requires_attention': mode.value >= DegradedMode.DEGRADED.value, 'critical_infrastructure_intact': mode.value < DegradedMode.EMERGENCY.value}
    ...
    async def end_sprint(self) -> None:
        ...
        async with self._lock:
            ...
            logger.info('[%s] Sprint ended. Duration: %.1fs, Mode: %s, Failures: %d', self.sprint_id, summary['elapsed_seconds'], self.degradation_mode.label, summary['registry']['total_failures'])
    ...
    @asynccontextmanager
    async def sprint_context(self) -> AsyncGenerator['SprintHealthLedger', None]:
    ...
    @asynccontextmanager
    async def failure_context(self, component: str, severity: FailureSeverity=FailureSeverity.MEDIUM) -> AsyncGenerator[None, None]:
        """
...
def get_current_ledger() -> Optional[SprintHealthLedger]:
