...
class HealthScore:
    ...
    _DEGRADATION_SCORES: dict[DegradedMode, int] = {DegradedMode.HEALTHY: 100, DegradedMode.DEGRADED: 70, DegradedMode.IO_ONLY: 40, DegradedMode.EMERGENCY: 10}
    ...
    async def from_ledger_async(cls, ledger: SprintHealthLedger) -> 'HealthScore':
        ...
        return cls(total=total, components_score=components_score, degradation_score=degradation_score, throughput_score=throughput_score, grade=cls._compute_grade(total), issues=issues)
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'total': round(self.total, 1), 'components_score': round(self.components_score, 1), 'degradation_score': round(self.degradation_score, 1), 'throughput_score': round(self.throughput_score, 1), 'grade': self.grade, 'issues': self.issues}
...
class HealthReporter:
    ...
    async def generate_snapshot(self) -> dict[str, Any]:
        ...
        return {'timestamp': time.time(), 'sprint_id': self.ledger.sprint_id, 'elapsed_seconds': summary['elapsed_seconds'], 'health_score': score.to_dict(), 'status': score.status(), 'degradation_mode': self.ledger.degradation_mode.label, 'can_continue': summary['health_indicators']['can_continue'], 'components': await self._summarize_components(), 'recent_failures': await self._get_recent_failures(limit=5)}
    ...
    async def _summarize_components(self) -> list[dict[str, Any]]:
        ...
        return [{'name': name, 'failures': h.total_failures, 'status': '⚠️' if h.total_failures > 0 else '✅', 'last_error': h.last_error_type} for name, h in sorted(components.items(), key=lambda x: x[1].total_failures, reverse=True)]
    ...
    async def _get_recent_failures(self, limit: int=5) -> list[dict[str, Any]]:
        ...
        return [{'timestamp': f.timestamp, 'component': f.component, 'severity': f.severity.label, 'error': f'{f.error_type}: {f.error_message[:50]}'} for f in failures]
    ...
    async def generate_completion_report(self) -> dict[str, Any]:
        ...
        return {'sprint_id': self.ledger.sprint_id, 'outcome': outcome, 'outcome_detail': outcome_detail, 'duration_seconds': summary['elapsed_seconds'], 'health_score': score.to_dict(), 'status': score.status(), 'degradation_mode': self.ledger.degradation_mode.label, 'total_failures': registry_summary['total_failures'], 'high_critical_failures': registry_summary['high_critical_count'], 'components_affected': registry_summary['components_affected'], 'mode_transitions': summary['transitions'], 'can_trust_results': score.total >= 70, 'requires_review': score.total < 90}
...
def format_health_status(ledger: SprintHealthLedger) -> str:
    ...
    lines = [f'═══ Sprint Health ═══', f'Sprint: {ledger.sprint_id}', f'Status: {score.status()}', f'Score: {score.total:.0f}/100 (Grade: {score.grade})', f'Mode: {ledger.degradation_mode.label}']
    if score.issues:
        lines.append('Issues:')
...
def format_completion_summary(ledger: SprintHealthLedger, duration: float) -> str:
    ...
    score = HealthScore.from_ledger(ledger)
    lines = ['╔══════════════════════════════════════════════════════════╗', '║                  SPRINT COMPLETION REPORT                ║', '╚══════════════════════════════════════════════════════════╝', f'  Sprint ID:      {ledger.sprint_id}', f'  Duration:       {duration:.1f}s', '', f'  ══ Health ══', f'  Score:          {score.total:.0f}/100 (Grade: {score.grade})', f'  Status:         {score.status()}', f'  Mode:           {ledger.degradation_mode.label}', f"  Can Continue:   {('Yes' if ledger.degradation_state.to_dict()['recovery_available'] else 'No')}", '', f'  ══ Failures ══', f"  Total:          {(score.issues if hasattr(score, 'issues') else 'N/A')}"]
    return '\n'.join(lines)

class AlertThresholds:
    """Configurable alert thresholds."""
    __slots__ = ('consecutive_component_failures', 'degradation_to_degraded', 'degradation_to_emergency', 'degradation_to_io_only', 'failure_count_critical', 'failure_count_high')

    def __init__(self, failure_count_high: int=10, failure_count_critical: int=25, degradation_to_degraded: int=3, degradation_to_io_only: int=5, degradation_to_emergency: int=8, consecutive_component_failures: int=3) -> None:
...
def _check_degradation_mode_alerts(mode: DegradedMode) -> list[dict[str, Any]]:
    ...
    if mode == DegradedMode.DEGRADED:
        alerts.append({'level': 'warning', 'component': 'system', 'message': 'Sprint in DEGRADED mode', 'action': 'Monitor closely, consider abort if worsens'})
    elif mode == DegradedMode.IO_ONLY:
        alerts.append({'level': 'error', 'component': 'system', 'message': 'Sprint in IO_ONLY mode', 'action': 'Results will be incomplete, consider aborting'})
    elif mode == DegradedMode.EMERGENCY:
        alerts.append({'level': 'critical', 'component': 'system', 'message': 'Sprint in EMERGENCY mode', 'action': 'CRITICAL: Abort sprint immediately'})
    return alerts
...
async def check_alerts_async(ledger: SprintHealthLedger) -> list[dict[str, Any]]:
    ...
    for name, health in components.items():
        if health.total_failures >= thresholds.failure_count_critical:
            alerts.append({'level': 'critical', 'component': name, 'message': f'Component has {health.total_failures} failures', 'action': f'Circuit breaker should be open for {name}'})
        elif health.total_failures >= thresholds.failure_count_high:
            alerts.append({'level': 'warning', 'component': name, 'message': f'Component has {health.total_failures} failures', 'action': f'Monitor {name} closely'})
    return alerts
...
def check_alerts(ledger: SprintHealthLedger) -> list[dict[str, Any]]:
    ...
    if total_failures >= thresholds.failure_count_critical:
        alerts.append({'level': 'warning', 'component': 'system', 'message': f'Total failures: {total_failures} (use check_alerts_async for details)', 'action': 'Call check_alerts_async for per-component breakdown'})
    return alerts