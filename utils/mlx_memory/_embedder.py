...
class MetalBufferPool:
    ...
    __slots__ = tuple(('_allocated', '_allocated_bytes', '_buffers'))
    ...
    def allocate(self, max_batch: int=_MAX_BATCH_SIZE, seq_len: int=_MAX_SEQ_LEN, hidden: int=_HIDDEN_DIM) -> bool:
        """Allocate pre-defined Metal buffers. Idempotent (guards against double-init)."""
        if self._allocated:
            return True
        try:
            import mlx.core as mx
            buffers_def = [('input_ids', (max_batch, seq_len), 'int32'), ('attention_mask', (max_batch, seq_len), 'int32'), ('output', (max_batch, seq_len, hidden), 'float32')]
...
def init_metal_embedder_buffers(max_batch: int=_MAX_BATCH_SIZE, seq_len: int=_MAX_SEQ_LEN, hidden: int=_HIDDEN_DIM) -> dict[str, Any]:
    """
