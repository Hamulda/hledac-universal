...
class _LazyModule:
    ...
    For parallel loading:
        ...
        # F314: migrated asyncio.gather -> parallel_ok
        await parallel_ok(*(m.ensure_loaded() for m in light_modules), label="capability_prober:load_light")
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if self._module is None:
            raise RuntimeError(f"Module '{self._name}' not loaded. Call await ensure_loaded() first")
    ...
    async def ensure_loaded(self) -> Any:
        ...
        if self._name in _LazyModule._cache:
            ...
            if cached is not None:
                ...
                if self._load_error is not None:
                    raise self._load_error
                raise RuntimeError(f"Module '{self._name}' cached as None without load error")
        try:
