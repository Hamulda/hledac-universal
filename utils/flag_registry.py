...
FlagGroup = Literal[
    "network", "brain", "storage", "dark_surface", "intelligence_apis", "forensics", "stealth", "system"
]
...
class FlagSpec(Struct, frozen=True):
...
def register(spec: FlagSpec) -> FlagSpec:
    ...
    if spec.group not in VALID_GROUPS:
        raise FlagRegistryError(
            f"FlagSpec {spec.name!r}: invalid group {spec.group!r}; expected one of {sorted(VALID_GROUPS)}"
        )
    if spec.name in FLAG_REGISTRY:
        existing = FLAG_REGISTRY[spec.name]
        raise FlagRegistryError(
            f"FlagSpec {spec.name!r} already registered (group={existing.group!r}); duplicate registration"
        )
...
def _register_symmetric_conflict(a: FlagSpec, b: FlagSpec) -> tuple[FlagSpec, FlagSpec]:
    ...
    a = FlagSpec(
        ...
        default=a.default,
        implies=a.implies,
        conflicts_with=a_conflicts,
        requires_daemon=a.requires_daemon,
        min_ram_mb=a.min_ram_mb,
    ...
    b = FlagSpec(
        ...
        default=b.default,
        implies=b.implies,
        conflicts_with=b_conflicts,
        requires_daemon=b.requires_daemon,
        min_ram_mb=b.min_ram_mb,
...
register(
    FlagSpec(
        name="HLEDAC_ENABLE_BGP", group="intelligence_apis", min_ram_mb=40, description="BGP enrichment sidecar (F234)."
    )
)
register(
    FlagSpec(
...
register(
    FlagSpec(name="HLEDAC_OFFLINE", group="system", min_ram_mb=0, description="Offline mode (skip all network egress).")
)
register(
    FlagSpec(
...
register(
    FlagSpec(name="HLEDAC_HTTP3", group="network", min_ram_mb=0, description="HTTP/3 (QUIC) experimental transport.")
)
register(
    FlagSpec(
