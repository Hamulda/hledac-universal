...
def register(spec: FlagSpec) -> FlagSpec:
    ...
    if spec.group not in VALID_GROUPS:
        raise FlagRegistryError(
            f"FlagSpec {spec.name!r}: invalid group {spec.group!r}; "
    ...
    if spec.name in FLAG_REGISTRY:
        ...
        raise FlagRegistryError(
            ...
            f"(group={existing.group!r}); duplicate registration"
        )
...
# ---------------------------------------------------------------------------
# Internal helper for symmetric conflict registration
# ---------------------------------------------------------------------------

def _register_symmetric_conflict(
    a: FlagSpec, b: FlagSpec
) -> tuple[FlagSpec, FlagSpec]:
...
# ---------------------------------------------------------------------------
# Discovery helpers (Phase 3 wiring surface)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Built-in registrations — top 27 flags from
# docs/flag_analysis/FLAGS_TAXONOMY_AND_VALIDATION.md sections 2.1, 3.1, 3.2
# ---------------------------------------------------------------------------

# ── network ────────────────────────────────────────────────────────────
register(FlagSpec(
    name="HLEDAC_ENABLE_TOR",
    group="network",
    requires_daemon="tor",
    min_ram_mb=50,
    description="Tor SOCKS5 transport (requires running tor daemon).",
))
register(FlagSpec(
    name="HLEDAC_ENABLE_I2P",
    group="network",
    requires_daemon="i2p",
    min_ram_mb=50,
    description="I2P SAM bridge transport (requires i2pd daemon).",
))
register(FlagSpec(
    name="HLEDAC_ENABLE_NYM",
    group="network",
    requires_daemon="nym",
    min_ram_mb=80,
    description="Nym mixnet transport (requires nym-client daemon).",
))
register(FlagSpec(
    name="HLEDAC_ENABLE_IPFS",
    group="network",
    min_ram_mb=30,
    description="IPFS gateway content fetch (sprint sidecar).",
))

# ── network — HTTP transport mutual-exclusion (CURL_CFFI ↔ HTTPX_H2) ──
_register_symmetric_conflict(
    FlagSpec(
        name="HLEDAC_ENABLE_CURL_CFFI",
        group="network",
        min_ram_mb=50,
        description="curl_cffi HTTP transport (JA3 fingerprinting).",
    ),
    FlagSpec(
        name="HLEDAC_ENABLE_HTTPX_H2",
        group="network",
        min_ram_mb=10,
        description="httpx HTTP/2 backend (conflicts with curl_cffi).",
    ),
)

# ── network — browser mutual-exclusion (NODRIVER ↔ HEAVY_BROWSER) ─────
_register_symmetric_conflict(
    FlagSpec(
        name="HLEDAC_ENABLE_NODRIVER",
        group="network",
        min_ram_mb=400,
        description="nodriver headless browser (Chrome binary required).",
    ),
    FlagSpec(
        name="HLEDAC_ENABLE_HEAVY_BROWSER",
        group="network",
        min_ram_mb=1500,
        description="Playwright (M1 RAM critical, 1.5GB+ headroom).",
    ),
)

# ── brain ──────────────────────────────────────────────────────────────
register(FlagSpec(
    name="HLEDAC_ENABLE_LLM",
    group="brain",
    min_ram_mb=2200,
    description="MLX/Hermes3 4bit LLM inference (M1 2.2GB RSS).",
))
register(FlagSpec(
    name="HLEDAC_ENABLE_DSPY",
    group="brain",
    implies=("HLEDAC_ENABLE_LLM",),
    min_ram_mb=200,
    description="DSPy compiled hypothesis programs (requires LLM).",
))
# NOTE: HLEDAC_ENABLE_HERMES_SYNTHESIS is registered later in the
# SYNTHESIS ↔ HERMES_SYNTHESIS mutual-exclusion block so the spec
# can carry both ``implies`` and ``conflicts_with`` in one entry.
register(FlagSpec(
    name="HLEDAC_ENABLE_HYPOTHESIS",
    group="brain",
    implies=("HLEDAC_ENABLE_LLM",),
    min_ram_mb=200,
    description="Hypothesis-driven pivot planner (requires LLM).",
))
register(FlagSpec(
    name="HLEDAC_ENABLE_GRAPH_RAG",
    group="brain",
    implies=("HLEDAC_ENABLE_LLM", "HLEDAC_ENABLE_GRAPH_ANALYSIS"),
    min_ram_mb=300,
    description="Graph RAG embeddings (requires LLM + graph analysis).",
))

# ── storage ────────────────────────────────────────────────────────────
register(FlagSpec(
    name="HLEDAC_ENABLE_GRAPH_ANALYSIS",
    group="storage",
    min_ram_mb=200,
    description="Leiden community detection (DuckPGQ graph).",
))
register(FlagSpec(
    name="HLEDAC_ENABLE_GRAPH_PATHS",
    group="storage",
    implies=("HLEDAC_ENABLE_GRAPH_ANALYSIS",),
    min_ram_mb=150,
    description="Quantum pathfinder (requires graph analysis).",
))
register(FlagSpec(
    name="HLEDAC_HTTP_CACHE",
    group="storage",
    min_ram_mb=20,
    description="HTTP response cache (enabled by default in .env.example).",
))

# ── dark_surface ───────────────────────────────────────────────────────
register(FlagSpec(
    name="HLEDAC_ENABLE_DARK_PIVOTS",
    group="dark_surface",
    min_ram_mb=80,
    description="Orchestrated Tor/I2P/IPFS pivots in hypothesis engine.",
))
register(FlagSpec(
    name="HLEDAC_ENABLE_DHT",
    group="dark_surface",
    min_ram_mb=100,
    description="DHT discovery sidecar (used for dark surface content).",
))
register(FlagSpec(
    name="HLEDAC_ENABLE_FEDERATED",
    group="dark_surface",
    min_ram_mb=150,
    description="Federated sidecar activation (gateway).",
))

# ── dark_surface — federated mode mutual-exclusion ────────────────────
_register_symmetric_conflict(
    FlagSpec(
        name="HLEDAC_ENABLE_FEDERATED_HYBRID",
        group="dark_surface",
        implies=("HLEDAC_ENABLE_FEDERATED",),
        min_ram_mb=200,
        description="Federated hybrid (P2P + bridge; requires FEDERATED).",
    ),
    FlagSpec(
        name="HLEDAC_ENABLE_FEDERATED_P2P",
        group="dark_surface",
        min_ram_mb=200,
        description="Federated pure P2P mode (conflicts with hybrid).",
    ),
)

# ── intelligence_apis ──────────────────────────────────────────────────
register(FlagSpec(
    name="HLEDAC_ENABLE_BGP",
    group="intelligence_apis",
    min_ram_mb=40,
    description="BGP enrichment sidecar (F234).",
))
register(FlagSpec(
    name="HLEDAC_ENABLE_BGP_PDNS",
    group="intelligence_apis",
    implies=("HLEDAC_ENABLE_BGP",),
    min_ram_mb=60,
    description="Passive DNS via BGP (requires BGP parent).",
))
register(FlagSpec(
    name="HLEDAC_ENABLE_ACADEMIC",
    group="intelligence_apis",
    min_ram_mb=80,
    description="Academic research lane (arXiv, PubMed).",
))
register(FlagSpec(
    name="HLEDAC_ENABLE_LEAKSENTINEL",
    group="intelligence_apis",
    min_ram_mb=30,
    description="Paste/GitHub/breach signal scanning.",
))

# ── system — cross-cutting ─────────────────────────────────────────────
register(FlagSpec(
    name="HLEDAC_CONTENT_HASHER",
    group="system",
    min_ram_mb=10,
    description="Content hash registry (Rust extension).",
))
register(FlagSpec(
    name="HLEDAC_ENABLE_STEALTH_LAYER",
    group="stealth",
    min_ram_mb=40,
    description="Stealth mode (UA rotation, jitter variance).",
))

# ── system — deprecated alias (SYNTHESIS ↔ HERMES_SYNTHESIS) ──────────
# HERMES_SYNTHESIS lives in the brain group but is registered here
# (in the system conflict pair) so it carries both ``implies`` and
# ``conflicts_with`` in one frozen spec.
_register_symmetric_conflict(
    FlagSpec(
        name="HLEDAC_ENABLE_SYNTHESIS",
        group="system",
        min_ram_mb=0,
        description="DEPRECATED alias — use HLEDAC_ENABLE_HERMES_SYNTHESIS.",
    ),
    FlagSpec(
        name="HLEDAC_ENABLE_HERMES_SYNTHESIS",
        group="brain",
        implies=("HLEDAC_ENABLE_LLM",),
        min_ram_mb=200,
        description="Hermes3 synthesis lane (preferred over SYNTHESIS).",
    ),
)
...
# ---------------------------------------------------------------------------
# Phase 3: Fail-fast combo validation
# ---------------------------------------------------------------------------

# M1 8GB UMA budget: macOS ~2.5GB + orchestrátor ~1GB + LLM ~2GB + KV ~0.75GB
# Soft warning at 5500MB (5.5GB), hard fatal at 7000MB (7GB).
...
def validate_flag_combo() -> tuple[list[str], list[str]]:
    ...
    try:
        # Snapshot active flag set. is_enabled() is fail-soft.
        active: set[str] = {
            name
            for name, spec in FLAG_REGISTRY.items()
            if is_enabled(name, spec.default)
        }
        ...
        for name in sorted(active):
            ...
            # 1. Implication check (soft).
            for implied in spec.implies:
                if implied in FLAG_REGISTRY and not is_enabled(
                    implied, FLAG_REGISTRY[implied].default
                ):
                    warnings.append(
                        f"{name} implies {implied} but it is disabled"
                    )
            ...
            for conflict in spec.conflicts_with:
                if conflict in active:
                    errors.append(
                        f"CONFLICT: {name} ↔ {conflict} cannot both be active"
                    )

        # 3. Compound RAM budget.
        total_ram = sum(
            FLAG_REGISTRY[f].min_ram_mb
            for f in active
            if f in FLAG_REGISTRY
        )

        if total_ram > _RAM_FATAL_MB:
            errors.append(
                f"FATAL: estimated RAM {total_ram}MB exceeds M1 8GB budget"
            )
        elif total_ram > _RAM_WARN_MB:
            warnings.append(
                f"WARNING: estimated RAM {total_ram}MB approaching M1 limit"
            )
    except (AttributeError, TypeError, KeyError) as exc:
        # Defensive: validation itself must never raise.
