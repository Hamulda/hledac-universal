...
class MultiTierRotatingBloomFilter:
    ...
    __slots__ = (
        '_per_host_capacity', '_global_capacity', '_max_tiers',
        '_max_fill_ratio', '_global_filter', '_host_tiers',
        '_tier_stats', '_lru_order', '_lock', '_is_rust',
        '_mmap_path', '_mmap_enabled'
    )
    ...
    def evict_idle_tiers(self, max_idle_seconds: float = 3600.0) -> int:
        ...
        with self._lock:
            to_evict = [
                host for host, stats in self._tier_stats.items()
    ...
    # === Async methods for async pipeline support ===
    # These are synchronous wrappers that release the GIL via asyncio.to_thread
    # for better concurrency in async pipelines.
...
class MmapBloomFilter(BloomFilter):
    ...
    def __init__(
        ...
        # Initialize base BloomFilter first
