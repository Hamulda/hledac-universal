...
class EntityExtractor:
    ...
    def _compile_patterns(self) -> None:
        ...
        self.patterns = {
            PatternType.EMAIL: re.compile("\\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Z|a-z]{2,}\\b", re.IGNORECASE),
            PatternType.BTC_ADDRESS: re.compile("\\b(?:1|3)[a-km-zA-HJ-NP-Z1-9]{25,34}\\b|\\bbc1[a-z0-9]{39,59}\\b"),
            PatternType.ETH_ADDRESS: re.compile("\\b0x[a-fA-F0-9]{40}\\b"),
            PatternType.XMR_ADDRESS: re.compile("\\b4[0-9AB][1-9A-HJ-NP-Za-km-z]{93}\\b"),
            PatternType.AWS_KEY: re.compile("\\bAKIA[0-9A-Z]{16}\\b|\\bASIA[0-9A-Z]{16}\\b"),
            PatternType.GOOGLE_KEY: re.compile("\\bAIza[0-9A-Za-z_-]{35}\\b"),
            PatternType.STRIPE_KEY: re.compile("\\bsk_live_[0-9a-zA-Z]{24,}\\b|\\bpk_live_[0-9a-zA-Z]{24,}\\b"),
            PatternType.IP_ADDRESS: re.compile(
                "\\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\b"
            ),
            PatternType.ONION_LINK: re.compile("\\b[a-z2-7]{16}\\.onion\\b|\\b[a-z2-7]{56}\\.onion\\b"),
            PatternType.PRIVATE_KEY: re.compile("-----BEGIN (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----"),
            PatternType.PASSWORD: re.compile(
                "(?i)(?:password|passwd|pwd)\\s*[=:]\\s*[\"\\']?([^\"\\'\\s]+)", re.IGNORECASE
            ),
            PatternType.API_KEY_GENERIC: re.compile(
                "\\b(?:api[_-]?key|apikey)\\s*[=:]\\s*[\"\\']?([a-zA-Z0-9_-]{16,})[\"\\']?", re.IGNORECASE
            ),
            PatternType.URL: re.compile(
                "https?://(?:[-\\w.])+(?:[:\\d]+)?(?:/(?:[\\w/_.])*(?:\\?(?:[\\w&=%.])*)?(?:#(?:[\\w.])*)?)?"
            ),
            PatternType.PHONE: re.compile("\\b(?:\\+?1[-.\\s]?)?\\(?[0-9]{3}\\)?[-.\\s]?[0-9]{3}[-.\\s]?[0-9]{4}\\b"),
        }
