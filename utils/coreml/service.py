...
class _ModelCache:
    __slots__ = ("_cache", "_lock", "_max_size", "_meta")
    ...
    async def predict(
        self, name: str, inputs: dict[str, Any], compute_unit: ComputeUnit = ComputeUnit.ALL
    ...
    async def predict_batch(
        self, name: str, inputs_list: list[dict[str, Any]], compute_unit: ComputeUnit = ComputeUnit.ALL
        ...
        total / len(results) if results else 0.0
...
async def health() -> HealthResult:
    ...
    models = await _cache.list_models()
    return HealthResult(
        status="ok", version=ct.__version__, ane=ane_available, models_loaded=len(models), cache_max=_MAX_CACHE
    )
...
async def predict_batch(req: BatchPredictRequest) -> BatchPredictResult:
    ...
    try:
        ...
        return BatchPredictResult(
            results=results, total_latency_ms=total, avg_latency_ms=avg, compute_unit_used=cu_used
        )
    except HTTPException:
        raise
    except Exception as e:
