...
def _to_otlp_value(v: Any) -> dict[str, Any]:
    ...
    if isinstance(v, dict):
        return {
            "kvlistValue": {
                "values": [
                    {"key": str(k)[:128], "value": _to_otlp_value(val)} for k, val in list(v.items())[:_MAX_DICT]
                ]
            }
        }
...
def _span_to_otlp(span: Any, max_attrs: int = 32) -> dict[str, Any]:
        ...
        events_out.append(
            {
                ...
                "attributes": [
                    ...
                    for k, v in list((getattr(ev, "attributes", None) or {}).items())[:max_attrs]
                ],
            }
        )
    ...
    return {
        ...
        "parentSpanId": format(parent.span_id, "016x")
