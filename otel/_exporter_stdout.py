...
# OTLP/JSON shape mappers. Bounded for M1 8GB.
_MAX_STRING = 1024
_MAX_LIST = 32
_MAX_DICT = 32
...
def _to_otlp_value(v: Any) -> dict[str, Any]:
    ...
    if isinstance(v, int):
        if v < -(2**63) or v > 2**63 - 1:
            v = 0  # int64 overflow -> 0 (fail-soft)
    ...
    if isinstance(v, (list, tuple)):
        return {
            "arrayValue": {
                "values": [_to_otlp_value(x) for x in list(v)[:_MAX_LIST]],
            }
        }
    if isinstance(v, dict):
        return {
            "kvlistValue": {
                "values": [
                    {"key": str(k)[:128], "value": _to_otlp_value(val)}
                    for k, val in list(v.items())[:_MAX_DICT]
                ]
            }
        }
...
def _span_to_otlp(span: Any, max_attrs: int = 32) -> dict[str, Any]:
    ...
    otlp_attrs = [
        {"key": str(k)[:128], "value": _to_otlp_value(v)}
        for k, v in attrs.items()
    ]

    # Status code -> OTLP enum string
    ...
    for ev in (getattr(span, "events", None) or []):
        events_out.append(
            {
                "timeUnixNano": int(getattr(ev, "timestamp", 0) or 0),
                "name": str(getattr(ev, "name", ""))[:128],
                "attributes": [
                    {"key": str(k)[:128], "value": _to_otlp_value(v)}
                    for k, v in list((getattr(ev, "attributes", None) or {}).items())[
                        :max_attrs
                    ]
                ],
            }
    ...
    return {
        "traceId": format(ctx.trace_id, "032x") if ctx and ctx.trace_id else "0" * 32,
        "spanId": format(ctx.span_id, "016x") if ctx and ctx.span_id else "0" * 16,
        "parentSpanId": (
        ...
        "startTimeUnixNano": str(int(getattr(span, "start_time", 0) or 0)),
        "endTimeUnixNano": str(int(getattr(span, "end_time", 0) or 0)),
