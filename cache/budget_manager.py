...
class FrequencyTracker:
    ...
    def __init__(self, width=2**15, depth=4) -> None:
        self.width = width
        self.depth = depth
...
class BudgetManager:
    ...
    def __init__(self, config: BudgetConfig | None = None) -> None:
        ...
        logger.debug(
            f"BudgetManager initialized: max_iter={self.config.max_iterations}, max_docs={self.config.max_docs}, max_time={self.config.max_time_sec}s"
        )
    ...
    def _check_stagnation(self, evidence: IterationSnapshot) -> tuple[bool, StopReason, str]:
        ...
        if existing_stagnation or jaccard_stagnation:
            ...
            logger.debug(
                f"Stagnation detected: {self.state.stagnation_counter}/{self.config.stagnation_threshold} iterations (existing={existing_stagnation}, jaccard={jaccard_stagnation})"
            )
    ...
    def record_iteration(self, evidence: IterationSnapshot) -> None:
        ...
        logger.debug(
            f"Iteration {self.state.iteration} recorded: entities={len(evidence.entities)}, sources={len(evidence.sources)}, claims={len(evidence.claims)}, confidence={evidence.confidence:.2f}"
        )
...
def create_quick_budget() -> BudgetManager:
    ...
    return create_budget_manager(
        max_iterations=3, max_docs=10, max_time_sec=60, max_tool_calls=20, min_confidence=0.6, stagnation_threshold=1
    )
