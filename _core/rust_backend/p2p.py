...
class P2PHarvester:
    ...
    def __init__(
        self, max_concurrent_peers: int = 20, default_duration_s: int = 120, default_max_results: int = 100
    ) -> None:
        """
        ...
        if self._rust_module is None and self._bridge_module is None:
            logger.warning(
                "[P2P] Rust p2p_harvest module not available. Set HLEDAC_ENABLE_P2P_HARVEST=1 or compile with --features p2p_harvest"
            )
        ...
        if self._rust_module is not None:
            try:
                findings = await self._rust_module.harvest(
                    keyword=keyword, protocols=protocols, duration_s=duration_s, max_results=max_results
                )
    ...
    async def dht_crawl(
        self, keyword: str, duration_s: int | None = None, max_results: int | None = None
    ) -> list[CanonicalFinding]:
        """
    ...
    def _convert_to_canonical_findings(
        self, raw_findings: list[dict[str, Any]], keyword: str
    ) -> list[CanonicalFinding]:
        """
    ...
    def _protocol_to_source_type(self, protocol: str) -> SourceType:
        ...
        mapping = {
            ...
            "ipfs": SourceType.DHT_METADATA,
            "tor": SourceType.DHT_METADATA,
            "i2p": SourceType.DHT_METADATA,
        }
        return mapping.get(protocol, SourceType.DHT_METADATA)

    async def _python_fallback_harvest(
        self, keyword: str, protocols: list[str], duration_s: int, max_results: int
    ) -> list[CanonicalFinding]:
        """
        ...
        if "bt_dht" in protocols:
            try:
                ...
                results = await crawl_dht_for_keyword(
                    keyword=keyword, duration_s=duration_s, max_results=max_results, harvest_metadata=False
                )
                import time
...
async def harvest_p2p(
    keyword: str, protocols: list[str] | None = None, duration_s: int = 120, max_results: int = 100
) -> list[CanonicalFinding]:
    """
