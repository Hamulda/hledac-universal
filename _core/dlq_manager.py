...
Použití:
    from hledac.universal._core.dlq_manager import get_dlq_manager, dlq_catch
...
class DLQManager:
    ...
    def _get_connection(self) -> sqlite3.Connection:
        ...
        conn = sqlite3.connect(str(self.db_path), check_same_thread=False, isolation_level=None)
    ...
    async def _get_async_connection(self) -> aiosqlite.Connection:
        ...
        conn = await aiosqlite.connect(str(self.db_path), uri=True)
    ...
    def store_payload(self, payload_data: bytes, sprint_id: str, source: str, error: Exception, metadata: Optional[dict[str, Any]]=None) -> str:
        ...
        payload = DLQPayload(payload_id=payload_id, sprint_id=sprint_id, source=source, error_type=type(error).__qualname__, error_message=str(error)[:1000], payload_data=payload_data, metadata=metadata or {})
        conn = self._get_connection()
        try:
            conn.execute('\n                INSERT OR IGNORE INTO dlq_payloads\n                (payload_id, sprint_id, source, error_type, error_message,\n                 payload_data, metadata, created_at, attempt_count)\n                VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0)\n                ', (payload.payload_id, payload.sprint_id, payload.source, payload.error_type, payload.error_message, payload.payload_data.hex(), _json_encode(payload.metadata), payload.created_at.isoformat()))
            try:
                conn.execute('\n                    INSERT INTO dlq_stats (source, error_type, count, last_occurrence)\n                    VALUES (?, ?, 1, ?)\n                    ON CONFLICT(source, error_type) DO UPDATE SET\n                        count = count + 1,\n                        last_occurrence = ?\n                    ', (payload.source, payload.error_type, payload.created_at.isoformat(), payload.created_at.isoformat()))
            except Exception:
                pass
        except Exception as e:
    ...
    def get_stats(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return [{'source': row['source'], 'error_type': row['error_type'], 'count': row['count'], 'last_occurrence': row['last_occurrence']} for row in cursor.fetchall()]
        except Exception:
            return []
        finally:
            conn.close()
        ...
        if is_async:
            ...
            async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
                try:
                    ...
                    try:
                        dlq = get_dlq_manager()
                        dlq.store_payload(payload_data=payload_data, sprint_id=sprint_id, source=source, error=e, metadata=metadata)
