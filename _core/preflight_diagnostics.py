...
def _check_rust_extension() -> PreflightResult:
    ...
    try:
        ...
        has_any_symbol = any(hasattr(hledac_rust_extensions, sym) for sym in _CRITICAL_SYMBOLS)
        ...
        try:
            ...
            from hledac.universal._core.rust_backend._prober import _probe
...
def _check_memory_sys_metrics() -> PreflightResult:
    ...
    try:
        ...
        return PreflightResult(
            ...
            passed=True,
            duration_ms=duration_ms,
            details=f"total={mem_info.total_gib:.2f}GiB used={mem_info.used_gib:.2f}GiB ({mem_info.percent:.1f}%)",
        )
    except ImportError as exc:
        duration_ms = (time.perf_counter() - start) * 1000
        return PreflightResult(
...
def run_preflight_diagnostics(max_duration_ms: float = 2000.0) -> list[PreflightResult]:
    ...
    if failed_checks:
        logger.error(
            "[PREFLIGHT] FAILED: %d check(s) failed — exiting. Fix errors above and retry.",
            len(failed_checks),
        )
