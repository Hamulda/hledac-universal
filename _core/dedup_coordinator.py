...
class DedupCoordinator:
    ...
    def _evict_oldest_cache_entries(self, count: int = 1) -> None:
        ...
        while count > 0 and self._link_predict_cache:
            oldest_node_id = min(
                self._link_predict_cache.keys(),
                key=lambda nid: self._link_predict_cache[nid].timestamp
            )
    ...
    def _cache_prediction(self, node_id: int, predictions: list[dict[str, Any]]) -> None:
        ...
        with self._cache_lock:
            ...
            expired = [
                nid for nid, cached in self._link_predict_cache.items()
    ...
    def dedup_check(
        ...
        # Running loop exists - create task and wait
...
# =============================================================================
# R11: LinkPredictorDedupBridge - Connects StreamingLinkPredictor to DedupCoordinator
# =============================================================================
# PURPOSE:
#   - Bridges StreamingLinkPredictor (speculative prefetch) with DedupCoordinator
#   - Adds predict_links_for_node as supplementary signal to stream_predictions()
#   - M1 8GB bounds: top-100 nodes, top-20 candidates per node
#
# SIGNAL FLOW:
#   SpeculativePrefetcher.stream_predictions()
#       → LinkPredictorDedupBridge.run_dedup_predictions()
#           → DedupCoordinator._predict_links_for_node() [Rust FFI]
#               → Returns predictions as DedupSignal for quality assessment
# =============================================================================
    ...
    def run_dedup_predictions(
        ...
        for node_id in target_nodes:
            ...
            if predictions:
                ...
                results.append(DedupPredictionResult(
                    node_id=node_id,
...
# R11: Standalone Dedup Bridge (no DedupManager dependency)
# =============================================================================
# PURPOSE:
#   Standalone bridge that integrates Rust link_predictor for dedup quality
#   without requiring a full DedupCoordinator + DedupManager.
#
# ARCHITECTURE:
#   - SpeculativePrefetcher._init_dedup_bridge() creates this directly
#   - Uses Rust FFI for link prediction
#   - M1 8GB bounds: max 100 nodes, 20 candidates per node
# =============================================================================
