...
class ResourceType(Enum):
    ...
    FILE_DESCRIPTOR = auto()  # OS file descriptors
    MACH_PORT = auto()  # macOS Mach ports
    CHILD_PROCESS = auto()  # Subprocess PIDs
    MMAP_REGION = auto()  # Memory-mapped regions
    METAL_CACHE = auto()  # MLX Metal cache allocations
    THREAD = auto()  # Tokio/rayon threads
    TMP_VOLUME = auto()  # RAM-backed /tmp files
...
class ResourceAllocation:
    ...
    type: ResourceType
    handle: int | str  # OS handle (fd number, pid, port, etc.)
    owner: str  # Transport/service name (e.g., "tor", "i2p", "duckdb")
    allocated_at: float  # Monotonic timestamp
    size_bytes: int | None = None  # For mmap, metal_cache, tmp
...
class ResourceLimits:
    ...
    # File descriptors
    max_fds: int = 256  # Conservative (ulimit -n is 10240)
    fd_warn_threshold: float = 0.75  # Warn at 75% utilization

    # Mach ports (macOS only)
    max_mach_ports: int = 16384
    mach_port_warn_threshold: float = 0.80

    # Child processes
    max_child_processes: int = 8  # Tor + I2P + Nym + workers
    child_process_warn_threshold: float = 0.75

    # mmap regions
    max_mmap_regions: int = 128
    mmap_warn_threshold: float = 0.80

    # Metal cache
    max_metal_cache_bytes: int = int(1.5 * 1024**3)  # 1.5 GiB
    metal_cache_warn_threshold: float = 0.85

    # Threads
    max_threads: int = 32  # Tokio + rayon workers
    thread_warn_threshold: float = 0.80

    # /tmp volume
    max_tmp_volume_bytes: int = int(512 * 1024**2)  # 512 MiB
    tmp_volume_warn_threshold: float = 0.80
...
class ResourceLedger:
    ...
    def __init__(
        ...
        # FD snapshot for delta tracking
        self._system_fd_count: int = 0
        self._last_fd_check: float = 0.0

        # Mach port tracking (macOS only)
    ...
    def _count_fds(self) -> int:
        ...
        try:
            # Count FDs in /proc/self/fd (Linux) or use resource module (macOS)
            if hasattr(os, "pidfd_open"):  # Linux 5.6+
    ...
    def _get_rlimit_usage(self) -> int:
        ...
        try:
            ...
            return min(soft, hard)  # Use soft limit
        except Exception:
    ...
    def _count_mach_ports(self) -> int:
        ...
        if self._mach_port_sample_cache is not None:
            ...
            if now - cached_time < 2.0:  # 2s cache
                return cached_count

        try:
            # Use mach_task_basic_info to get task info
            TASK_INFO = 4  # MACH_TASK_BASIC_INFO
            ...
            # Use ctypes to call mach_task_info
            kinfo = ctypes.c_void_p()
            kinfo_size = ctypes.c_uint()

            # Simplified: use the known Mach port count from ledger
            ledger_count = self._count_by_type(ResourceType.MACH_PORT)

            # Estimate total from ledger + known socket FDs
            socket_count = self._count_by_type(ResourceType.FILE_DESCRIPTOR)
            total = ledger_count + socket_count + 50  # Base allocation
    ...
    def can_admit(
        ...
        mmap_regions: int = 0,
        metal_cache_bytes: int = 0,
        threads: int = 0,
        tmp_volume_bytes: int = 0,
    ) -> tuple[bool, str]:
        """
        ...
        """
        checks = [
            (ResourceType.FILE_DESCRIPTOR, fds, self._limits.max_fds, self._limits.fd_warn_threshold),
            (ResourceType.MACH_PORT, mach_ports, self._limits.max_mach_ports, self._limits.mach_port_warn_threshold),
            (ResourceType.CHILD_PROCESS, child_processes, self._limits.max_child_processes, self._limits.child_process_warn_threshold),
            (ResourceType.MMAP_REGION, mmap_regions, self._limits.max_mmap_regions, self._limits.mmap_warn_threshold),
            (ResourceType.METAL_CACHE, metal_cache_bytes, self._limits.max_metal_cache_bytes, self._limits.metal_cache_warn_threshold),
            (ResourceType.THREAD, threads, self._limits.max_threads, self._limits.thread_warn_threshold),
            (ResourceType.TMP_VOLUME, tmp_volume_bytes, self._limits.max_tmp_volume_bytes, self._limits.tmp_volume_warn_threshold),
        ]
    ...
    def admission(
        ...
        """
        can_admit, reason = self.can_admit(
            owner,
            fds=fds,
        ...
        try:
            # Acquire each resource type
            if fds > 0:
                for _ in range(fds):
                    self.allocate(ResourceType.FILE_DESCRIPTOR, -1, owner)  # -1 = placeholder
            ...
            # Rollback acquired resources on error
    ...
    def register_child_process(
        ...
        pid: int,
        owner: str,
        process: asyncio.subprocess.Process | subprocess.Popen | None = None,
    ) -> bool:
        """
    ...
    async def terminate_child_processes(
        self,
        owner: str | None = None,
        timeout_s: float = 5.0,
        force: bool = False,
    ...
    def telemetry(self) -> dict[str, Any]:
        ...
        with self._lock:
            result: dict[str, Any] = {
                ...
                "admission_denied_count": self._admission_denied_count,
                "resources": {},
                "warnings": [],
            }
            ...
            for resource_type in ResourceType:
                ...
                result["resources"][resource_type.name] = {
                    "current": current,
                    "limit": limit,
                    "peak": peak,
                    "utilization": utilization,
                    "tracked": self._count_by_type(resource_type),
                }

                # Add warning if high utilization
    ...
    def format_telemetry_report(self) -> str:
        ...
        tel = self.telemetry()
        lines = [
            "=== Resource Ledger Telemetry ===",
            f"Allocations: {tel['allocation_count']} | Releases: {tel['release_count']} | Denied: {tel['admission_denied_count']}",
            "",
