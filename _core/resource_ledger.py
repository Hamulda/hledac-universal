...
class ResourceLedger:
    ...
    __slots__ = (
        "_admission_denied_count",
        "_allocation_count",
        "_allocations",
        "_last_fd_check",
        "_limits",
        "_lock",
        "_mach_port_sample_cache",
        "_owner_allocations",
        "_peak_usage",
        "_release_count",
        "_system_fd_count",
    ...
    def allocate(
        ...
        with self._lock:
            if not self._check_limit(resource_type):
                ...
                logger.warning(
                    f"[ResourceLedger] Allocation denied: {resource_type.name} for {owner} (limit={self._get_limit(resource_type)})"
                )
                return False
            ...
            logger.debug(
                f"[ResourceLedger] Allocated: {resource_type.name}={handle} for {owner} (total={current}/{self._get_limit(resource_type)})"
            )
            return True
    ...
    def release(self, resource_type: ResourceType, handle: int | str, force: bool = False) -> bool:
        ...
        with self._lock:
            ...
            logger.debug(
                f"[ResourceLedger] Released: {resource_type.name}={handle} from {allocation.owner} (age={allocation.age_s():.1f}s)"
            )
            return True
    ...
    def _count_mach_ports(self) -> int:
        ...
        try:
            ...
            ctypes.mach_task_self()
            TaskBasicInfo()
            ctypes.c_uint32(TASK_INFO)
            ctypes.pythonapi.PyThreadState_Get().c_profile.h
            ctypes.c_int()
            ctypes.c_void_p()
            ctypes.c_uint()
    ...
    def can_admit(
        ...
        checks = [
            ...
            (
                ...
                child_processes,
                self._limits.max_child_processes,
                self._limits.child_process_warn_threshold,
            ),
            (ResourceType.MMAP_REGION, mmap_regions, self._limits.max_mmap_regions, self._limits.mmap_warn_threshold),
            (
                ResourceType.METAL_CACHE,
                metal_cache_bytes,
                self._limits.max_metal_cache_bytes,
                self._limits.metal_cache_warn_threshold,
            ),
            (ResourceType.THREAD, threads, self._limits.max_threads, self._limits.thread_warn_threshold),
            (
                ResourceType.TMP_VOLUME,
                tmp_volume_bytes,
        ...
        for resource_type, requested, limit, warn_threshold in checks:
            ...
            if new_total > limit:
                return (
                    False,
                    f"{resource_type.name} limit exceeded: current={current}, requested={requested}, limit={limit}",
                )
            if new_total > limit * warn_threshold:
                logger.warning(
                    f"[ResourceLedger] {resource_type.name} warning: {new_total}/{limit} ({new_total / limit:.0%})"
                )
    ...
    def register_child_process(
        self, pid: int, owner: str, process: asyncio.subprocess.Process | subprocess.Popen | None = None
    ) -> bool:
        """
    ...
    async def terminate_child_processes(
        self, owner: str | None = None, timeout_s: float = 5.0, force: bool = False
    ) -> int:
        """
            ...
            for resource_type in ResourceType:
                ...
                if utilization >= warn_threshold:
                    result["warnings"].append(
                        f"{resource_type.name} at {utilization:.0%} (threshold={warn_threshold:.0%})"
                    )
            return result
