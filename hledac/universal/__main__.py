...
- Full bootstrap (uvloop, boot guard, dotenv, signal handlers)
- Delegates to cli.parser.dispatch_async()
- Invoked by:
...
# Sprint Y3: uvloop is already gated darwin+arm64 via pyproject.toml apple-accel extra.
# Install early — before any async code or loop creation.
try:
    import platform as _platform
    import sys as _sys

    # Y3: Explicit darwin arm64 gate — uvloop is only in apple-accel extra,
    # which is only available on M-series Macs. Fallback to native asyncio otherwise.
    _is_darwin_arm = (
        _platform.system() == "Darwin"
        and getattr(_platform, "machine", lambda: "")().lower() in ("arm64", "aarch64")
    )
    if not _is_darwin_arm:
        logging.info("[RUNTIME] uvloop skipped: not darwin/arm64 — using native asyncio")
    elif _sys.version_info >= (3, 15):
        # Python 3.15+ has MJIT and uvloop compatibility is unverified
        ...
        _uvloop.install()
        # Y3: Persist to canonical RuntimeState so session_runtime can read it
        try:
            from hledac.universal.runtime.state import mark_uvloop_installed
            mark_uvloop_installed()
        except Exception:
            pass  # fail-soft: uvloop state is diagnostic, not functional
...
def main() -> None:
    ...
    try:
        ...
        logger.warning("boot_error", error=str(e))
