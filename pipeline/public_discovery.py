...
if TYPE_CHECKING:
    pass  # Constants imported lazily at runtime to avoid circular deps
...
def _compute_fetch_policy(
    ...
    """
    # Heavy JS sites get js_capable
    _JS_DOMAINS = ("google.com", "youtube.com", "twitter.com", "x.com",
...
def generate_rescue_urls(query: str, max_urls: int = 8) -> list:
    ...
    for name, base_url in _RESGUE_SOURCE_CANDIDATES[:max_urls]:
        ...
        hits.append(_make_discovery_hit(
            query=query,
            title=f"Rescue: {name}",
            url=url,
...
def _make_discovery_hit(
    query: str, title: str, url: str, snippet: str,
    score: float, reason: str, rank: int, source: str,
) -> Any:
...
# Sprint F223C: Bounded seed_context bootstrap
...
# ----------------------------------------------------------------------
# _extract_provider_surface — discovery result classification
# ----------------------------------------------------------------------
...
def _extract_provider_surface(
    ...
    for hit in hits:
        ...
        if reason in ("rescue_candidate", "bootstrap_root", "bootstrap_www",
                      "bootstrap_security_txt", "bootstrap_robots",
                      "bootstrap_sitemap", "seed_context_domain",
                      "seed_context_url"):
...
class _MinimalStealth:
    ...
    async def get(self, url: str) -> str:
        try:
            ...
            async with session.get(url, timeout=ClientTimeout(total=10.0)) as resp:
                return await resp.text()
        except Exception:
            return ""


_CC_SCANNER_LOOKUP: Any = None  # initialized lazily
...
async def _inject_onion_hits(hits: tuple, query: str, store: Any) -> int:
    ...
    try:
        ...
        text = await async_fetch_public_text(
            url=base_url,
            timeout_s=10.0,
            max_bytes=100_000,
        )
    ...
    for match in re.finditer(r"onion[^\s\"']+", text_str):
...
class _DiscoveryEngine:
    ...
    async def run(self, uma_state: str) -> tuple:
        ...
        # ---- Stage 1: Rescue URLs for threat queries ----
        rescue_hits = generate_rescue_urls(self.query, max_urls=8)

        # ---- Stage 2: Bootstrap URLs for domain queries ----
        bootstrap_urls = generate_bootstrap_urls(self.query)

        # ---- Stage 3: Seed context bootstrap ----
        seed_urls: list[str] = []
        ...
        for h in rescue_hits:
            all_hits.append(h)  # DiscoveryHit objects
        for url in bootstrap_urls:
            all_hits.append(_make_discovery_hit(
                query=self.query, title="Bootstrap", url=url,
                snippet=f"Bootstrap: {url}", score=0.5,
                reason="bootstrap", rank=-1, source="bootstrap",
            ))
        for url in seed_urls:
            all_hits.append(_make_discovery_hit(
                query=self.query, title="SeedContext", url=url,
                snippet=f"SeedContext: {url}", score=0.5,
                reason="seed_context", rank=-1, source="seed_context",
            ))
        ...
        for hit in filtered:
            ...
            if score <= 0.05:
                skipped.append(hit)
            elif reason in ("rescue_candidate", "bootstrap_root", "bootstrap_www",
                            "bootstrap_security_txt", "bootstrap_robots",
                            "bootstrap_sitemap", "seed_context_domain",
                            "seed_context_url"):
                stub.append(hit)
        ...
        if self.query and _CT_QUERY_IS_DOMAIN_RE.match(self.query):
            try:
                selected, *_ = await _inject_ct_subdomain_hits(
        ...
        if _query_looks_like_domain_for_cc(self.query):
            try:
                selected, *_ = await _inject_commoncrawl_hits(
                    tuple(selected), self.query
                )
        ...
        # ---- Academic search (nested function extracted to module level) ----
...
# ----------------------------------------------------------------------
# limited_academic_search — extracted from inside _DiscoveryEngine.run
#   (originally L4027-5353, 1326 lines nested inside the run() method)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# run_tot_with_timeout — Tree-of-Thought reasoning
# ----------------------------------------------------------------------
