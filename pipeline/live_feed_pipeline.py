...
class FeedIocStage:
    ...
    async def process(self, feed_items: list[dict]) -> list[dict]:
        ...
        try:
            ...
            for item, iocs in zip(feed_items, results, strict=False):
                item["iocs"] = iocs
        except Exception:
...
async def async_run_live_feed_pipeline(
            ...
            if _rust_results:
                ...
                for _idx, _entry_url, _combined_hits, _t_hits, _d_hits, _phase in _rust_results:
                    ...
                    for _hit in _converted_hits:
                        ...
                        _finding = _pattern_hit_to_finding(feed_url, _entry_url, _hit, query_context, "{{_hit.value}}")
                        if _finding:
    ...
    if prefilted:
        ...
        build = await parallel(
            entry_tasks,
            concurrency=_MAX_FEED_PATTERN_TASKS,
            policy="collect",
            taskgroup=True,
            ctx="live_feed_pipeline:entry_processing",
        )
