...
from hledac.universal.discovery.duckduckgo_adapter import DiscoveryHit, classify_discovery_error
from hledac.universal.discovery.duckduckgo_adapter import search_multi_engine as _search_multi_engine_bootstrap
from hledac.universal.fetching.public_fetcher import classify_fetch_error
...
class DiscoveryEngine(Struct):
    ...
    async def run(self, uma_state: str) -> DiscoveryPhaseResult:
        ...
        if not hits:
            ...
            hits = kw_result['hits']
        discovery_telemetry = _build_discovery_telemetry(discovery_result=disc['result'], discovery_error=discovery_error, discovery_error_type=discovery_error_type, discovery_elapsed_s=discovery_elapsed_s, discovery_attempted=discovery_attempted, public_discovery_cache_hit=disc['cache_hit'], public_discovery_query_count=disc['query_count'], hits=hits, pub_bootstrap_order=bs['bootstrap_order'], pub_bootstrap_prevented_discovery_timeout=bs['prevented_timeout'], pub_bootstrap_first_fetch_attempted=bs['first_fetch_attempted'], pub_bootstrap_candidates_count=bs['candidates_count'], pub_bootstrap_fetch_attempted=bs['fetch_attempted'], pub_bootstrap_fetch_success=bs['fetch_success'], pub_bootstrap_accepted_findings=0, pub_bootstrap_errors=0, pub_rescue_candidates_count=bs['rescue_count'], pub_rescue_fetch_attempted=0, pub_rescue_fetch_success=0, pub_rescue_accepted_findings=0, pub_rescue_errors=0, pub_rescue_order=bs['rescue_order'], keyword_seed_fallback_triggered=bs['keyword_fallback_triggered'], pub_keyword_bootstrap_candidates_count=kw_result['candidates_count'], pub_keyword_bootstrap_fetch_attempted=kw_result['fetch_attempted'], pub_keyword_bootstrap_fetch_success=kw_result['fetch_success'], pub_keyword_bootstrap_order=kw_result['bootstrap_order'], pub_keyword_bootstrap_errors=kw_result['errors'], pub_build_success_count=0, pub_build_failure_count=0, pub_duplicate_count=0, pub_provider_selected=disc['provider_selected'], pub_provider_skipped=disc['provider_skipped'], pub_provider_stub=disc['provider_stub'], pub_provider_errors=disc['provider_errors'], pub_query_variants=disc['query_variants'], pub_provider_timeout_count=disc['provider_timeout_count'], pub_provider_import_error_count=disc['provider_import_error_count'], pub_discovery_empty_reason=disc['discovery_empty_reason'], public_candidates_discovered=0, public_candidates_fetch_attempted=0, public_candidates_fetch_success=0, public_candidates_parse_success=0, public_candidates_pattern_matched=0, public_candidates_built=0, public_candidates_store_attempted=0, public_candidates_stored=0, public_candidates_rejected=0, stage_failure=None, stage_failure_reason=None)
        if not hits:
            return DiscoveryPhaseResult(hits=(), discovery_result=None, discovery_error=discovery_error, discovery_error_type=discovery_error_type, discovery_elapsed_s=discovery_elapsed_s, discovery_attempted=discovery_attempted, discovery_telemetry=discovery_telemetry, academic_findings_count=0, ct_injected=0, cc_injected=0, onion_findings_count=0, pastebin_findings_count=0, github_secrets_count=0, keyword_seed_fallback_triggered=bs['keyword_fallback_triggered'])
        ...
        return DiscoveryPhaseResult(hits=hits, discovery_result=disc['result'], discovery_error=discovery_error, discovery_error_type=discovery_error_type, discovery_elapsed_s=discovery_elapsed_s, discovery_attempted=discovery_attempted, discovery_telemetry=discovery_telemetry, academic_findings_count=augmented_result['academic_findings_count'], ct_injected=augmented_result['ct_injected'], cc_injected=augmented_result['cc_injected'], onion_findings_count=onion_findings_count, pastebin_findings_count=augmented_result['pastebin_findings_count'], github_secrets_count=augmented_result['github_secrets_count'], keyword_seed_fallback_triggered=bs['keyword_fallback_triggered'])
    ...
    async def _run_augmentation_phases(self, hits: tuple) -> dict:
        ...
        return {'hits': enriched_hits, 'academic_findings_count': academic_findings_count, 'ct_injected': ct_injected, 'cc_injected': cc_injected, 'pastebin_findings_count': pastebin_findings_count, 'github_secrets_count': github_secrets_count}
        ...
        if self.public_bootstrap_enabled:
            try:
                ...
                for idx, url in enumerate(bootstrap_urls):
                    bootstrap_hits.append(DiscoveryHit(query=self.query, title=f'Bootstrap {idx + 1}', url=url, snippet=f'Deterministic bootstrap URL: {url}', score=0.85, reason='deterministic_bootstrap', rank=-1, source='bootstrap', retrieved_ts=0.0))
            except Exception:
                candidates_count = 0
            ...
            if candidates_count == 0 and rescue_count == 0 and (self.seed_context is not None):
                try:
                    seed_urls = generate_seed_context_bootstrap_urls(self.seed_context, max_candidates=_MAX_SEED_CONTEXT_BOOTSTRAP)
                    candidates_count = len(seed_urls)
                    for idx, url in enumerate(seed_urls):
                        bootstrap_hits.append(DiscoveryHit(query=self.query, title=f'SeedBootstrap {idx + 1}', url=url, snippet=f'Seed context bootstrap URL: {url}', score=0.8, reason='seed_context_bootstrap', rank=-1, source='seed_bootstrap', retrieved_ts=0.0))
                except Exception:
                    candidates_count = 0
        return {'bootstrap_hits': bootstrap_hits, 'rescue_hits': rescue_hits, 'candidates_count': candidates_count, 'bootstrap_order': bootstrap_order, 'prevented_timeout': prevented_timeout, 'first_fetch_attempted': first_fetch_attempted, 'fetch_attempted': fetch_attempted, 'fetch_success': fetch_success, 'rescue_count': rescue_count, 'rescue_order': rescue_order, 'keyword_fallback_triggered': keyword_fallback_triggered}
    ...
    async def _run_discovery_phase(self, bootstrap_hits: list, rescue_hits: list) -> dict:
        ...
        try:
            ...
            query_count = 1
            _extract_provider_surface(discovery_result, provider_selected, provider_skipped, provider_stub, provider_errors, provider_timeout_count, provider_import_error_count, discovery_empty_reason)
            ...
            discovery_error_type = classify_discovery_error(discovery_error, elapsed_s=discovery_elapsed_s, timeout_s=35.0, hits_count=len(hits))
        except asyncio.CancelledError:
            discovery_elapsed_s = time.monotonic() - _discovery_start if _discovery_start else None
            discovery_error_type = classify_discovery_error(asyncio.CancelledError('cancelled'), elapsed_s=discovery_elapsed_s, hits_count=0)
            raise
        except Exception as exc:
            discovery_elapsed_s = time.monotonic() - _discovery_start if _discovery_start else None
            discovery_error = f'discovery_exception:{type(exc).__name__}:{exc}'
            discovery_error_type = classify_discovery_error(discovery_error, elapsed_s=discovery_elapsed_s, hits_count=0)
            hits = ()
        return {'hits': hits, 'result': discovery_result, 'error': discovery_error, 'error_type': discovery_error_type, 'elapsed': discovery_elapsed_s, 'attempted': discovery_attempted, 'cache_hit': cache_hit, 'query_count': query_count, 'provider_selected': provider_selected, 'provider_skipped': provider_skipped, 'provider_stub': provider_stub, 'provider_errors': provider_errors, 'query_variants': query_variants, 'provider_timeout_count': provider_timeout_count, 'provider_import_error_count': provider_import_error_count, 'discovery_empty_reason': discovery_empty_reason}
    ...
    async def _run_keyword_fallback(self) -> dict:
        ...
        errors = 0
        try:
            keyword_hits = await generate_keyword_bootstrap_urls(self.query, max_urls=_MAX_KEYWORD_BOOTSTRAP_URLS)
            ...
            errors = 1
            candidates_count = 0
        return {'hits': hits, 'candidates_count': candidates_count, 'bootstrap_order': bootstrap_order, 'fetch_attempted': fetch_attempted, 'fetch_success': fetch_success, 'errors': errors}
...
async def _run_phase1_augmentation(hits: tuple, query: str, store: Any) -> tuple:
    ...
    _build_p1 = await parallel([_ct_wrapper(), _cc_wrapper(), _pastebin_github_wrapper()], concurrency=4, policy='collect', ctx='live_public_pipeline:issue32_phase1')
...
async def _run_pastebin_github_scan(query: str, store: Any) -> tuple[int, int]:
    ...
    _DOMAIN_ORG_RE = _re.compile('(?:[a-zA-Z0-9](?:[a-zA-Z0-9\\-]{0,61}[a-zA-Z0-9])?\\.)+[a-zA-Z]{2,}')
    try:
        ...
        if _paste_findings:
            ...
            for _pf in _paste_findings:
                ...
                _p20_findings.append(CanonicalFinding(finding_id=_pf_id, query=query, source_type='pastebin_monitor', confidence=0.6, ts=time.time(), provenance=('pastebin', _pf.source, _target), payload_text=f'uri={_pf.uri}\nemails={_pf.emails}\nips={_pf.ip_addresses}\nmasked_secrets={_pf.masked_secrets()}\nsnippet={_pf.context_snippet[:300]}'))
        ...
        if _gh_results:
            ...
            for _gf in _gh_results:
                ...
                _gh_findings.append(CanonicalFinding(finding_id=_gf_id, query=query, source_type='github_secret_scanner', confidence=0.55, ts=time.time(), provenance=('github', _gf.pattern, _org), payload_text=f'pattern={_gf.pattern}\nfile={_gf.file_path}\nline={_gf.line}\ncontext={_gf.context[:300]}'))
...
def _build_discovery_telemetry(discovery_result: Any, discovery_error: str | None, discovery_error_type: str | None, discovery_elapsed_s: float | None, discovery_attempted: bool, public_discovery_cache_hit: int, public_discovery_query_count: int, hits: tuple, pub_bootstrap_order: str, pub_bootstrap_prevented_discovery_timeout: bool, pub_bootstrap_first_fetch_attempted: bool, pub_bootstrap_candidates_count: int, pub_bootstrap_fetch_attempted: int, pub_bootstrap_fetch_success: int, pub_bootstrap_accepted_findings: int, pub_bootstrap_errors: int, pub_rescue_candidates_count: int, pub_rescue_fetch_attempted: int, pub_rescue_fetch_success: int, pub_rescue_accepted_findings: int, pub_rescue_errors: int, pub_rescue_order: str, keyword_seed_fallback_triggered: bool, pub_keyword_bootstrap_candidates_count: int, pub_keyword_bootstrap_fetch_attempted: int, pub_keyword_bootstrap_fetch_success: int, pub_keyword_bootstrap_order: str, pub_keyword_bootstrap_errors: int, pub_build_success_count: int, pub_build_failure_count: int, pub_duplicate_count: int, pub_provider_selected: list, pub_provider_skipped: list, pub_provider_stub: list, pub_provider_errors: list, pub_query_variants: list, pub_provider_timeout_count: list, pub_provider_import_error_count: list, pub_discovery_empty_reason: list, public_candidates_discovered: int, public_candidates_fetch_attempted: int, public_candidates_fetch_success: int, public_candidates_parse_success: int, public_candidates_pattern_matched: int, public_candidates_built: int, public_candidates_store_attempted: int, public_candidates_stored: int, public_candidates_rejected: int, stage_failure: str | None=None, stage_failure_reason: str | None=None) -> dict:
    """Build discovery telemetry dict from collected counters."""
    return {'discovery_result': discovery_result, 'public_stage_failure': stage_failure or ('discovery_empty' if not hits else None), 'public_stage_failure_reason': stage_failure_reason or (discovery_error if discovery_error else 'no URLs returned from discovery'), 'public_discovery_raw_count': len(hits), 'public_discovery_deduped_count': public_candidates_discovered, 'public_discovery_attempted': discovery_attempted, 'public_discovery_cache_hit': public_discovery_cache_hit, 'public_discovery_query_count': public_discovery_query_count, 'public_bootstrap_order': pub_bootstrap_order or 'disabled', 'public_bootstrap_prevented_discovery_timeout': pub_bootstrap_prevented_discovery_timeout, 'public_bootstrap_first_fetch_attempted': pub_bootstrap_first_fetch_attempted, 'public_bootstrap_candidates_count': pub_bootstrap_candidates_count, 'public_bootstrap_fetch_attempted': pub_bootstrap_fetch_attempted, 'public_rescue_candidates_count': pub_rescue_candidates_count, 'public_rescue_fetch_attempted': pub_rescue_fetch_attempted, 'public_rescue_order': pub_rescue_order, 'keyword_seed_fallback_triggered': keyword_seed_fallback_triggered, 'public_keyword_bootstrap_candidates_count': pub_keyword_bootstrap_candidates_count, 'public_keyword_bootstrap_fetch_attempted': pub_keyword_bootstrap_fetch_attempted, 'public_keyword_bootstrap_fetch_success': pub_keyword_bootstrap_fetch_success, 'public_keyword_bootstrap_order': pub_keyword_bootstrap_order, 'public_keyword_bootstrap_errors': pub_keyword_bootstrap_errors, 'public_build_success_count': pub_build_success_count, 'public_build_failure_count': pub_build_failure_count, 'public_duplicate_count': pub_duplicate_count, 'public_provider_selected': list(pub_provider_selected), 'public_provider_skipped': list(pub_provider_skipped), 'public_provider_stub': list(pub_provider_stub), 'public_provider_errors': list(pub_provider_errors), 'public_query_variants': list(pub_query_variants), 'public_provider_timeout_count': pub_provider_timeout_count[0], 'public_provider_import_error_count': pub_provider_import_error_count[0], 'public_discovery_empty_reason': pub_discovery_empty_reason[0] if pub_discovery_empty_reason else '', 'discovery_error_type': discovery_error_type or '', 'discovery_elapsed_s': round(discovery_elapsed_s, 3) if discovery_elapsed_s else None, 'public_candidates_discovered': public_candidates_discovered, 'public_candidates_fetch_attempted': public_candidates_fetch_attempted, 'public_candidates_fetch_success': public_candidates_fetch_success, 'public_candidates_parse_success': public_candidates_parse_success, 'public_candidates_pattern_matched': public_candidates_pattern_matched, 'public_candidates_built': public_candidates_built, 'public_candidates_store_attempted': public_candidates_store_attempted, 'public_candidates_stored': public_candidates_stored, 'public_candidates_rejected': public_candidates_rejected}
...
_CT_QUERY_IS_DOMAIN_RE: re.Pattern = re.compile('^(?:\\*\\.)?[a-zA-Z0-9][a-zA-Z0-9.*-]*\\.[a-zA-Z]{2,}$')
'Regex to detect domain-like query strings suitable for CT subdomain lookup.'
_CC_QUERY_IS_DOMAIN_RE: re.Pattern = re.compile('^(?:\\*\\.)?[a-zA-Z0-9][a-zA-Z0-9.*-]*\\.[a-zA-Z]{2,}$|^(?:site|domain):')
...
_BOOTSTRAP_DEFAULT_URLS: list[str] = ['', '/www.', '/.well-known/security.txt', '/robots.txt', '/sitemap.xml']
'Ordered list of URL path templates for deterministic bootstrap.'
_RESGUE_SOURCE_CANDIDATES: list[tuple[str, str]] = [('ThreatFox', 'https://threatfox.abuse.ch/browse.php?search='), ('ID Ransomware', 'https://id-ransomware.malwarehunterteam.com/'), ('BleepingComputer', 'https://www.bleepingcomputer.com/search/?search='), ('The Hacker News', 'https://thehackernews.com/search?q='), ('Krebs on Security', 'https://krebsonsecurity.com/?s='), ('CISA KEV', 'https://www.cisa.gov/known-exploited-vulnerabilities-catalog?search='), ('URLhaus', 'https://urlhaus.abuse.ch/'), ('AlienVault OTX', 'https://otx.alienvault.com/api/v1/search?q='), ('Maltiverse', 'https://maltiverse.com/search?keyword='), ('Onyphe', 'https://www.onyphe.io/search/?query='), ('GreyNoise', 'https://greynoise.io/viz/share/'), ('AbuseIPDB', 'https://www.abuseipdb.com/check/')]
'Static rescue source list for non-domain threat/malware/ransomware queries.'
_SHOPPING_NOISE_DOMAINS: tuple[str, ...] = ('trendyol.com', 'pazarama.com', 'amazon.com.tr', 'n11.com', 'hepsiburada.com', 'gittigidiyor.com', 'cimri.com', 'akakce.com')
_SHOPPING_NOISE_PATHS: tuple[str, ...] = ('/gp/bestsellers/', '/gp/bestsellers', '/bestsellers/', '/best-seller', '/matkap', '/category/', '/product/', '/products/', '/shop/', '/shopping/', '/cart/', '/checkout/', '/buy/', '/sale/', '/offers/', '/home-improvement', '/home-and-garden')
_SHOPPING_NOISE_PATHS_STRICT: tuple[str, ...] = ('/cart/', '/checkout/', '/buy/', '/sale/', '/offers/')
_CTI_NEWS_ALLOWED_DOMAINS: tuple[str, ...] = ('cisa.gov', 'krebsonsecurity.com', 'bleepingcomputer.com', 'thehackernews.com', 'abuse.ch', 'threatfox.abuse.ch', 'id-ransomware.malwarehunterteam.com', 'malwarehunterteam.com', 'cyberscoop.com', 'darkreading.com', 'threatpost.com', 'therecord.media', 'securityweek.com', 'inforisktoday.com', 'helpnetsecurity.com', 'ransomwarewiki.com', 'cybercrime-tracker.net', 'malware-traffic-analysis.net', 'unit42.paloaltonetworks.com', 'securityaffairs.com', 'thecyberwire.com', 'bleepinguid.com', 'ransomware.live')
...
def _check_ip_cve(q: str) -> bool:
    ...
    import re as _re
    if _re.match('^\\d{1,3}(?:\\.\\d{1,3}){3}(?:\\/\\d{1,2})?$|^[0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{0,4}){2,7}(?::\\d{1,3})?(?:\\/\\d{1,2})?$', q):
        return True
    if _re.match('^CVE-\\d{4}-\\d{4,}$', q, _re.IGNORECASE):
        return True
    return False
...
def _check_threat_patterns(q: str, first_token: str) -> bool:
    ...
    import re as _re
    THREAT_PAT = _re.compile('^(?:lockbit|conti|revil|clop|darkside|blackcat|alphv|ransomware|apt[_\\s]?\\d+|apt[_-]\\w+|sidecopy|callback|triangle|temp|wanna[_\\s]?cry|wannacry|petya|notpetya|badrabbit|emotet|trickbot|cobalt[_\\s]?strike|koadic|metasploit|fin7|carbanak|finacrypt|prodaft|labyrinth|zCrypt|poisonivy|plugx|gh0st|gain|wellmess|whispergate|hermetic)$', _re.IGNORECASE)
    if THREAT_PAT.match(q):
        return True
    EXTENDED_PAT = _re.compile('^(?:meterpreter|sandworm|lazarus|log4shell|finacrypt|prodaft|labyrinth|zcrypt|poisonivy|plugx|gh0st|gain|wellmess|whispergate|hermetic|sidecopy|callback|triangle|temp|sofacy|平原)$', _re.IGNORECASE)
    for token in _re.split('[\\s\\-_]+', q):
...
def _check_generic_keywords(q: str, first_token: str) -> bool:
    ...
    import re as _re
    THREAT_KW = _re.compile('^(?:ransomware|malware|threat[_-]?actor|cobalt[_\\s]?strike|breach|exploit|0day|zero[_\\s]?day|vulnerability|phishing|spam|botnet|trojan|rootkit|keylogger|Ransomware|Malware|ThreatActor|CVE|APT)$', _re.IGNORECASE)
    OSINT_KW = _re.compile('^(?:osint|osint infrstructure|infrastructure|telemetry|leak|dark[_\\s]?web|exposure|credential|breach|darkweb|onion|leakdb|intel|threat|hunting|recon|scanning|fingerprint|iot|ics|scada)$', _re.IGNORECASE)
...
def generate_rescue_urls(query: str, max_urls: int=8) -> list[DiscoveryHit]:
    ...
    for name, base_url in _RESGUE_SOURCE_CANDIDATES[:max_urls]:
        ...
        hits.append(DiscoveryHit(query=query, title=f'Rescue: {name}', url=url, snippet=f'Rescue search via {name}: {query}', score=0.7, reason='rescue_candidate', rank=-1, source='rescue', retrieved_ts=0.0))
    return hits
...
def generate_bootstrap_urls(query: str, max_urls: int=_MAX_BOOTSTRAP_URLS) -> list[str]:
    ...
    urls: list[str] = [f'https://www.{domain}' if path == '/www.' else f'https://{domain}{path}' if path else f'https://{domain}' for path in paths]
    return urls
...
async def generate_keyword_bootstrap_urls(query: str, max_urls: int=_MAX_KEYWORD_BOOTSTRAP_URLS) -> list[DiscoveryHit]:
    ...
    for engine in _PUBLIC_BOOTSTRAP_SEARCH_ENGINES:
        try:
            raw_results = await _search_multi_engine_bootstrap(query, max_results=max_urls)
            if not raw_results:
                continue
            hits: list[DiscoveryHit] = []
            for i, item in enumerate(raw_results[:max_urls]):
                ...
                if not url:
                    continue
                hits.append(DiscoveryHit(query=query, title=title or f'{engine.capitalize()} result {i + 1}', url=url, snippet=snippet or f'Keyword bootstrap via {engine}: {query}', score=0.75, reason=f'keyword_bootstrap_{engine}', rank=i, source=engine, retrieved_ts=time.time()))
            if hits:
                return hits
        except Exception:
            continue
...
def _is_valid_domain(q: str) -> bool:
    ...
    if not q or '.' not in q:
        return False
    if _re.match('^\\d{1,3}(\\.\\d{1,3}){3}$', q):
        return False
    if not _re.match('^[a-zA-Z0-9.\\-]+$', q):
        return False
...
def _extract_provider_surface(discovery_result, selected_out, skipped_out, stub_out, errors_out, timeout_count_out, import_error_count_out, empty_reason_out) -> None:
    ...
    if psd and isinstance(psd, list):
        for entry in psd:
            _process_provider_status_entry(entry, selected_out, skipped_out, stub_out)
    ...
    if error_str:
        ...
        timeout_count_out[0] += t_inc
        import_error_count_out[0] += i_inc
        if empty_reason and (not empty_reason_out):
            empty_reason_out.append(empty_reason)
    if not selected_out and (not psd) and (not empty_reason_out):
            ...
            try:
                ...
                for entry in history:
                    ...
                    if isinstance(value, dict):
                        ...
                        if payload:
                            rag_context.append({'query': value.get('query', ''), 'payload': payload[:500], 'timestamp': value.get('timestamp', 0)})
            except Exception:
                rag_context = []
        ctx.uma_state = ctx.uma_state
        return PipelineContext(**{**ctx.__dict__, 'session_id': session_id, '_rag_context': rag_context, '_persistence_enabled': persistence_enabled, '_persistence_restored': persistence_restored, '_persistence_saved': False})
...
class Phase2_ResourceGovernance:
    ...
    async def run(self, ctx: PipelineContext) -> PipelineContext:
        ...
        from hledac.universal._core.resource_governor import UMA_STATE_CRITICAL, UMA_STATE_EMERGENCY, UMA_STATE_OK
        ...
        return PipelineContext(**{**ctx.__dict__, 'uma_state': uma_state, 'effective_concurrency': effective_concurrency, '_semaphore': asyncio.Semaphore(effective_concurrency), '_is_emergency': uma_state == UMA_STATE_EMERGENCY})
...
class Phase3_DiscoveryRunner:
    ...
    async def run(self, ctx: PipelineContext) -> tuple[PipelineContext, DiscoveryPhaseResult]:
        ...
        """
        discovery_info = await DiscoveryEngine(query=ctx.query, store=ctx.store, max_results=ctx.max_results, public_bootstrap_enabled=ctx.public_bootstrap_enabled, seed_context=ctx.seed_context).run(uma_state=ctx.uma_state)
        return (PipelineContext(**{**ctx.__dict__, 'hits': discovery_info.hits, 'discovery_telemetry': discovery_info.discovery_telemetry}), discovery_info)
...
def _extract_hit_metadata(hit) -> dict:
    ...
    return {'url': hit_url, 'title': hit_title, 'snippet': hit_snippet, 'score': hit_score, 'reason': hit_reason, 'rank': hit_rank}
...
class Phase4_FetchOrchestrator:
    ...
    async def run(self, ctx: PipelineContext, discovery: DiscoveryPhaseResult) -> tuple[PipelineContext, list]:
        ...
        if noise_rejections:
            logger.debug('[F265C] Noise filter rejected %d/%d hits', len(noise_rejections), len(hits) + len(noise_rejections))
        ...
        for hit in hits[:500]:
            ...
            seen_url_count += 1
            task = safe_create_task(_fetch_and_process_page(semaphore=ctx._semaphore, query=ctx.query, hit_url=meta['url'], hit_title=meta['title'], hit_snippet=meta['snippet'], hit_rank=meta['rank'], fetch_timeout_s=ctx.fetch_timeout_s, fetch_max_bytes=ctx.fetch_max_bytes, store=ctx.store, memory_manager=ctx.memory_manager, session_id=ctx.session_id, discovery_score=meta['score'], discovery_reason=meta['reason'], vector_store=ctx.vector_store, graph=ctx.graph), name='fetch:public_page')
            tasks.append(task)
        ...
        return (PipelineContext(**{**ctx.__dict__, 'all_page_results': all_page_results, '_seen_url_count': seen_url_count, '_noise_reject_reasons': noise_reject_reasons, '_error_results': error_results}), all_page_results)

class _BasicCounter:
    """F361: Compute basic discovery/fetch/pattern counts."""

    def run(self, results: list) -> dict:
        return {'total_discovered': len(results), 'total_fetched': sum((1 for p in results if p.fetched)), 'total_matched': sum((p.matched_patterns for p in results)), 'total_accepted': sum((p.accepted_findings for p in results)), 'total_stored': sum((p.stored_findings for p in results))}
...
class _AcceptanceClassifier:
    ...
    def run(self, fetched: list, noise_reasons: dict) -> dict:
        ...
        return {'acceptance_reject_reasons': reject_reasons, 'accepted_urls': accepted, 'rejected_urls': rejected}
        ...
        for p in results:
            ...
            if tr is None:
                ...
                if tr.startswith('skipped_') and len(skipped) < 5:
                    ...
                    rejected.append(p.url)
        return {'terminal_reason_counts': dict(counter), 'skipped_samples': skipped, 'rejected_samples': rejected}
...
class _ValueAnalyzer:
    ...
    def run(self, results: list, fetched: list) -> dict:
        ...
        weak_skipped = sum((1 for p in results if p.quality_reason and p.quality_reason.startswith('SKIP_WEAK')))
        low_value = sum((1 for p in fetched if p.matched_patterns == 0 and p.quality_reason in ('weak_low_signal', 'ok:no_query_signal')))
        disc_strong_weak = sum((1 for p in results if p.discovery_signal and p.matched_patterns == 0))
        disc_strong_content = sum((1 for p in results if p.discovery_signal and p.matched_patterns > 0))
        squandered = sum((1 for p in results if p.discovery_score is not None and p.discovery_score >= 0.85 and (p.quality_reason in ('weak_low_signal', 'SKIP_WEAK:weak_discovery', 'SKIP_WEAK:very_low_text'))))
        ...
        for p in results:
            ...
            if cat in waste_cats:
                waste_cats[cat] += 1
        health_ratio = round(sum((1 for p in fetched if getattr(p, 'structural_quality', '') == 'healthy')) / max(len(fetched), 1), 3) if fetched else 0.0
        waste_code = max(waste_cats, key=lambda k: waste_cats[k]) if any((v > 0 for v in waste_cats.values())) else ''
        waste_breakdown = '|'.join((f'{v}{k[:3]}' for k, v in sorted(waste_cats.items()) if v > 0)) if any((v > 0 for v in waste_cats.values())) else 'none'
        ...
        return {'strong_pages': strong, 'weak_pages_skipped': weak_skipped, 'low_value_fetches': low_value, 'discovery_strong_content_weak': disc_strong_weak, 'discovery_and_content_strong': disc_strong_content, 'discovery_squandered': squandered, 'noise_fetch_ratio': noise_ratio, 'waste_ratio': noise_ratio, 'value_ratio': round(disc_strong_content / max(len(results), 1), 3) if results else 0.0, 'public_branch_hint': hint, 'public_next_action': action, 'public_confidence_note': confidence, 'quality_mix': mix, 'top_waste_pattern': top_waste, 'waste_category_counts': waste_cats, 'structural_health_ratio': health_ratio, 'run_waste_pattern_code': waste_code, 'waste_reason_breakdown': waste_breakdown}
...
class _ProofGradeCalculator:
    ...
    def run(self, stored: int, fetched: list, noise_ratio: float, error_ratio: float) -> dict:
        ...
        if backend_degraded:
            ...
            grade = 'empty'
        return {'backend_degraded': backend_degraded, 'public_proof_grade': grade}
...
class _CandidateLedger:
    ...
    def run(self, results: list, fetched: list) -> dict:
        ...
        fetch_success = sum((1 for p in fetched if not (p.error and p.error.startswith('fetch_text_none_or_empty'))))
        ...
        if not discovered:
            ...
            terminal = 'accepted'
        return {'candidates_discovered': discovered, 'candidates_fetch_attempted': fetch_attempted, 'candidates_fetch_success': fetch_success, 'candidates_parse_success': parse_success, 'candidates_pattern_matched': pattern_matched, 'candidates_built': built, 'candidates_store_attempted': store_attempted, 'candidates_stored': stored, 'candidates_rejected': rejected, 'rejection_summary': rej_sum, 'terminal_stage': terminal}

class _FetchSkipAnalyzer:
    """F361: Analyze why pages were skipped during fetch."""

    def run(self, results: list) -> dict:
        skip_reasons = [p.fetch_blocked_reason for p in results if not p.fetched and p.fetch_blocked_reason]
        skip_reason = Counter(skip_reasons).most_common(1)[0][0] if skip_reasons else None
        accessibility_stages = {'connection', 'tls', 'http'}
        accessibility_blocker = any((getattr(p, 'failure_stage', None) in accessibility_stages for p in results))
        return {'public_fetch_skip_reason': skip_reason, 'fetch_accessibility_blocker': accessibility_blocker}
...
class Phase5_TelemetryAggregator:
    ...
    """
    __slots__ = ('_acceptance', '_basic', '_ledger', '_proof', '_skip', '_terminal', '_value')
    ...
    def run(self, ctx: PipelineContext, discovery: DiscoveryPhaseResult) -> dict:
        ...
        proof = self._proof.run(basic['total_stored'], fetched, value['noise_fetch_ratio'], error_ratio)
        ...
        build_success = sum((1 for p in fetched if p.matched_patterns > 0 or p.accepted_findings > 0))
        build_fail = sum((1 for p in fetched if p.matched_patterns > 0 and p.stored_findings == 0))
        acceptance_ratio = build_success / max(build_success + build_fail, 1) if build_success + build_fail > 0 else 0.0
        return {**basic, 'seen_url_count': seen_count, **value, **acceptance, **terminal, **ledger, **proof, **skip, 'bootstrap_candidates': dt.get('public_bootstrap_candidates_count', 0), 'bootstrap_fetch_attempted': dt.get('public_bootstrap_fetch_attempted', 0), 'bootstrap_fetch_success': dt.get('public_bootstrap_fetch_success', 0), 'bootstrap_accepted': dt.get('public_bootstrap_accepted_findings', 0), 'bootstrap_errors': dt.get('public_bootstrap_errors', 0), 'bootstrap_order': dt.get('public_bootstrap_order', 'disabled'), 'bootstrap_prevented': dt.get('public_bootstrap_prevented_discovery_timeout', False), 'bootstrap_first_attempted': dt.get('public_bootstrap_first_fetch_attempted', False), 'rescue_candidates': dt.get('public_rescue_candidates_count', 0), 'rescue_fetch_attempted': dt.get('public_rescue_fetch_attempted', 0), 'rescue_fetch_success': dt.get('public_rescue_fetch_success', 0), 'rescue_accepted': dt.get('public_rescue_accepted_findings', 0), 'rescue_errors': dt.get('public_rescue_errors', 0), 'rescue_order': dt.get('public_rescue_order', 'disabled'), 'build_success': pub_build_success, 'build_failure': pub_build_failure, 'duplicate_count': dt.get('public_duplicate_count', 0), 'acceptance_ratio': acceptance_ratio, 'discovery_empty_reason': dt.get('public_discovery_empty_reason', '')}
...
class Phase6_ReportGenerator:
    ...
    async def run(self, ctx: PipelineContext, all_page_results: list) -> tuple[PipelineContext, str, int]:
        ...
        if ctx.hermes_engine is not None and all_page_results:
            try:
                generated_report = await _generate_and_store_report(query=ctx.query, pages=tuple(all_page_results), store=ctx.store, hermes_engine=ctx.hermes_engine, vector_store=ctx.vector_store)
            except Exception:
                generated_report = ''
        ...
        if ctx.store is not None and ctx.hermes_engine is not None:
            try:
                ...
                pass
        return (PipelineContext(**{**ctx.__dict__, 'generated_report': generated_report, 'tot_solution_count': tot_solution_count}), generated_report, tot_solution_count)
...
async def _run_rl_loop(ctx: PipelineContext, all_page_results: list) -> dict:
    ...
    while True:
        ...
        try:
            if action == 'hypothesis_generation' and ctx.hermes_engine is not None:
                ...
                if hasattr(ctx.hermes_engine, 'generate_hypotheses_async'):
                    hyp_strings = await ctx.hermes_engine.generate_hypotheses_async(context=ctx_h, hermes_engine=getattr(ctx.hermes_engine, '_inference_engine', None))
        ...
        rl_next_state = (ctx.query[:20] if len(ctx.query) > 20 else ctx.query, min(step_count // 2, 5), min(new_findings_count // 10, 10), 6, action == 'tot_reasoning')
        ...
        if ctx.store is not None and action_findings:
            try:
                ...
                for finding_data in action_findings:
                    ...
                    rl_finding_buffer.append(CanonicalFinding(finding_id=finding_id, query=ctx.query, source_type='rl_research', confidence=0.7, ts=time.time(), provenance=('rl', action), payload_text=str(finding_data)[:500]))
        ...
        if ctx.memory_manager is not None and ctx.session_id is not None:
            try:
                await ctx.memory_manager.put(ctx.session_id, f'rl_result:{step_count}', {'action': action, 'reward': reward, 'findings_count': len(action_findings), 'timestamp': time.time()})
            except Exception:
                pass
...
async def _run_hypothesis_tot(ctx: PipelineContext, all_page_results: list) -> dict:
    ...
    if not recent_findings:
        return {'tot_solution_count': 0}
    hypo_context = {'query': ctx.query, 'stored_findings_count': ctx.total_stored if hasattr(ctx, 'total_stored') else 0, 'findings': [{'finding_id': f.finding_id if hasattr(f, 'finding_id') else str(f.get('finding_id', '')), 'source_type': f.source_type if hasattr(f, 'source_type') else str(f.get('source_type', '')), 'confidence': f.confidence if hasattr(f, 'confidence') else float(f.get('confidence', 0.0)), 'provenance': f.provenance if hasattr(f, 'provenance') else f.get('provenance', '')} for f in recent_findings[:20]]}
    hypotheses = await hypo_engine.generate_hypotheses_async(context=hypo_context, hermes_engine=ctx.hermes_engine)
    ...
    for coro in asyncio.as_completed(tasks):
        ...
        if tot_result:
            ...
            try:
                tot_finding_buffer.append(CanonicalFinding(finding_id=f'tot_{hashlib.sha256(tot_result.encode()).hexdigest()[:16]}', query=ctx.query, source_type='tot_synthesis', confidence=0.7, ts=time.time(), provenance=('tot', hypo[:100]), payload_text=tot_result[:1000]))
            except Exception:
                pass
            if ctx.enqueue_hypothesis_pivot is not None:
                try:
                    ...
                    for term in pivot_seed:
                        ctx.enqueue_hypothesis_pivot(ioc_value=term.lower(), ioc_type='hypothesis', confidence=0.6, depth=1)
                except Exception:
                    pass
        ...
        try:
            ...
            for pr in ctx.all_page_results:
                if pr.accepted_findings > 0:
                    findings_for_synth.append({'content': (pr.quality_reason or '')[:500], 'title': pr.url or '', 'source_type': 'public_lane', 'confidence': 0.5, 'url': pr.url or ''})
            ...
            async with asyncio.timeout(90.0):
                report = await runner.synthesize_findings(query=ctx.query, findings=findings_for_synth, max_findings=10, force_synthesis=False)
            await runner.close()
            if report is not None:
                from hledac.universal.knowledge.duckdb_store import CanonicalFinding
                report_id = f'synth_{hashlib.md5(ctx.query.encode()).hexdigest()[:12]}'
                synthesis_finding = CanonicalFinding(finding_id=report_id, query=ctx.query, source_type='llm_synthesis', confidence=getattr(report, 'confidence', 0.7) or 0.7, ts=time.time(), payload_text=f"Threat actors: {', '.join(getattr(report, 'threat_actors', []) or [])} | {getattr(report, 'threat_summary', '')[:500]}", provenance=('synthesis', getattr(report, 'query', ctx.query)[:50]))
        ...
        try:
            ...
            if ctx.memory_manager is not None and ctx.session_id is not None:
                try:
                    ...
                    session_findings = []
            export_metadata = {'query': ctx.query, 'sources': sources, 'tags': ['hledac', 'osint', 'public-pipeline'], 'session_id': ctx.session_id, 'stored_findings': str(ctx.total_stored if hasattr(ctx, 'total_stored') else 0)}
            md_path = export_mgr.export_markdown(report=generated_report, findings=session_findings, file_path=None, metadata=export_metadata)
            if md_path:
                logger.info(f'[P18] Exported markdown to {md_path}')
            if ctx.graph is not None and ctx.graph.node_count() > 0:
                html_path = export_mgr.export_graph_html(graph_manager=ctx.graph, file_path=None, title=f'Hledac Graph - {ctx.query[:50]}')
                if html_path:
...
class Phase9_TemporalPersistence:
    ...
    def run(self) -> dict:
        ...
        try:
            from hledac.universal.layers import get_temporal_signal_summary, build_temporal_priority_hints, save_temporal_signal_snapshot
        ...
        return {'temporal_signal_summary': temporal_signal_summary, 'temporal_priority_hints': temporal_priority_hints, 'persistence_saved': persistence_saved}

class ResultBuilder:
    """Builds PipelineRunResult from phase outputs."""

    @staticmethod
    def build(ctx: PipelineContext, telemetry: dict, discovery: DiscoveryPhaseResult, public_stage_failure: str | None, public_stage_failure_reason: str | None, generated_report: str, tot_solution_count: int) -> PipelineRunResult:
        ...
        public_branch_verdict = {'waste_ratio': t['waste_ratio'], 'value_ratio': t['value_ratio'], 'public_branch_hint': t['public_branch_hint'], 'strong_pages': t['strong_pages'], 'weak_pages_skipped': t['weak_pages_skipped'], 'discovery_strong_content_weak': t['discovery_strong_content_weak'], 'discovery_and_content_strong': t['discovery_and_content_strong'], 'low_value_fetches': t['low_value_fetches'], 'discovery_squandered': t['discovery_squandered'], 'noise_fetch_ratio': t['noise_fetch_ratio'], 'corroboration_vs_burn': round((t['discovery_and_content_strong'] + t['strong_pages']) / max(total_discovered, 1), 3), 'public_next_action': t['public_next_action'], 'public_confidence_note': t['public_confidence_note'], 'backend_degraded': t['backend_degraded'], 'public_proof_grade': t['public_proof_grade'], 'discovery_error_detail': discovery.discovery_error}
        ...
        public_zero_hit_summary = {'zero_hit_accessible_fetch_count': sum((1 for p in ctx.all_page_results if p.fetched and p.matched_patterns == 0)), 'zero_hit_unique_reasons': list(zero_hit_reasons.keys()), 'zero_hit_has_substantive_content': any((p.fetched and p.matched_patterns == 0 and (getattr(p, 'structural_quality', '') == 'healthy') for p in ctx.all_page_results)), 'zero_hit_has_signalless': any((p.fetched and p.matched_patterns == 0 and (getattr(p, 'waste_category', '') == 'signalless') for p in ctx.all_page_results))}
        fallback_triggered = getattr(discovery.discovery_result, 'fallback_triggered', None)
        FALLBACK_STATE_MAP = {'primary_backend_failed_fallback_succeeded': 'primary_failed_fallback_succeeded', 'primary_backend_failed_fallback_failed': 'primary_failed_fallback_failed'}
        public_discovery_fallback_state = FALLBACK_STATE_MAP.get(fallback_triggered) or ('no_fallback_needed' if discovery.discovery_error is None else None)
        ...
        return PipelineRunResult(query=ctx.query, discovered=total_discovered, fetched=total_fetched, matched_patterns=total_matched, accepted_findings=total_accepted, stored_findings=total_stored, patterns_configured=_get_patterns_configured_count(), pages=tuple(ctx.all_page_results), error=run_error, strong_pages=t['strong_pages'], weak_pages_skipped=t['weak_pages_skipped'], low_value_fetches=t['low_value_fetches'], discovery_strong_content_weak=t['discovery_strong_content_weak'], discovery_and_content_strong=t['discovery_and_content_strong'], discovery_squandered=t['discovery_squandered'], noise_fetch_ratio=t['noise_fetch_ratio'], corroboration_vs_burn=public_branch_verdict['corroboration_vs_burn'], public_next_action=t['public_next_action'], public_confidence_note=t['public_confidence_note'], public_branch_verdict=public_branch_verdict, usable_findings_ratio=usable_findings_ratio, discovery_to_findings_efficiency=discovery_to_findings_efficiency, quality_mix=t['quality_mix'], public_proof_grade=t['public_proof_grade'], public_value_density=public_value_density, top_waste_pattern=t['top_waste_pattern'], discovery_false_positive_count=sum((1 for p in ctx.all_page_results if getattr(p, 'discovery_false_positive', False))), waste_category_counts=t['waste_category_counts'], structural_health_ratio=t['structural_health_ratio'], factual_value_density=public_value_density, run_waste_pattern_code=t['run_waste_pattern_code'], waste_reason_breakdown=t['waste_reason_breakdown'], backend_degraded=t['backend_degraded'], public_discovery_blocker=public_discovery_blocker, public_fetch_accessibility_blocker=t['fetch_accessibility_blocker'], public_discovery_fallback_state=public_discovery_fallback_state, dominant_public_failure_mode=dominant_failure_mode, public_stage_failure=public_stage_failure, public_stage_failure_reason=public_stage_failure_reason, public_discovery_attempted=discovery.discovery_attempted, public_discovery_raw_count=total_discovered, public_discovery_deduped_count=t['seen_url_count'], public_pages_fetched=total_fetched, public_pages_accepted=sum((1 for p in ctx.all_page_results if p.accepted_findings > 0)), public_pages_rejected=sum((1 for p in ctx.all_page_results if p.fetched and p.accepted_findings == 0)), public_findings_accepted=total_accepted, zero_hit_accessible_fetch_count=sum((1 for p in ctx.all_page_results if p.fetched and p.matched_patterns == 0)), zero_hit_quality_reason_counts=zero_hit_reasons, zero_hit_title_samples=tuple(zero_hit_titles), public_zero_hit_summary=public_zero_hit_summary, ct_subdomain_injected=discovery.ct_injected, cc_archive_injected=discovery.cc_injected, academic_findings_count=discovery.academic_findings_count, pastebin_findings_count=discovery.pastebin_findings_count, github_secrets_count=discovery.github_secrets_count, public_bootstrap_enabled=ctx.public_bootstrap_enabled, public_bootstrap_candidates_count=t['bootstrap_candidates'], public_bootstrap_fetch_attempted=t['bootstrap_fetch_attempted'], public_bootstrap_fetch_success=t['bootstrap_fetch_success'], public_bootstrap_accepted_findings=t['bootstrap_accepted'], public_bootstrap_errors=t['bootstrap_errors'], public_bootstrap_order=t['bootstrap_order'], public_bootstrap_prevented_discovery_timeout=t['bootstrap_prevented'], public_bootstrap_first_fetch_attempted=t['bootstrap_first_attempted'], public_rescue_candidates_count=t['rescue_candidates'], public_rescue_fetch_attempted=t['rescue_fetch_attempted'], public_rescue_fetch_success=t['rescue_fetch_success'], public_rescue_accepted_findings=t['rescue_accepted'], public_rescue_errors=t['rescue_errors'], public_rescue_order=t['rescue_order'], keyword_seed_fallback_triggered=discovery.keyword_seed_fallback_triggered, public_discovered=total_discovered, public_fetch_attempted=total_fetched, public_fetch_skipped=total_discovered - t['seen_url_count'], public_fetch_skip_reason=t['public_fetch_skip_reason'], public_js_renderer_unavailable=sum((1 for p in ctx.all_page_results if p.fetched and p.js_renderer_skipped_reason == 'browser_unavailable')), public_xml_or_rss_detected=sum((1 for p in ctx.all_page_results if p.fetched and p.js_renderer_skipped_reason in ('xml_or_feed_url', 'xml_recovered'))), public_fetch_timeout_count=sum((1 for p in ctx.all_page_results if not p.fetched and p.fetch_blocked_reason == 'timeout')), public_fetch_blocked_by_memory=sum((1 for p in ctx.all_page_results if not p.fetched and p.fetch_blocked_reason == 'uma_memory')), public_discovery_cache_hit=discovery.discovery_telemetry.get('public_discovery_cache_hit', 0), public_discovery_query_count=discovery.discovery_telemetry.get('public_discovery_query_count', 0), public_fetch_candidate_count=t['seen_url_count'], public_fetch_gate=public_fetch_gate, public_fetch_attempted_urls_sample=tuple((p.url for p in ctx.all_page_results if p.fetched))[:5], public_acceptance_attempted=t['candidates_fetch_attempted'], public_acceptance_accepted=sum((1 for p in ctx.all_page_results if p.fetched and p.accepted_findings > 0)), public_acceptance_rejected=sum((1 for p in ctx.all_page_results if p.fetched and p.accepted_findings == 0)), public_acceptance_reject_reasons=t['acceptance_reject_reasons'], public_accepted_url_sample=tuple(t['accepted_urls']), public_rejected_url_sample=tuple(t['rejected_urls']), public_build_success_count=t['build_success'], public_build_failure_count=t['build_failure'], public_duplicate_count=t['duplicate_count'], public_acceptance_ratio=t['acceptance_ratio'], public_terminal_classified_count=sum((1 for v in t['terminal_reason_counts'].values() if v > 0)), public_unclassified_count=len(ctx.all_page_results) - sum((1 for v in t['terminal_reason_counts'].values() if v > 0)), public_terminal_reason_counts=t['terminal_reason_counts'], public_fetch_success=t['candidates_fetch_success'], public_fetch_failed=t['candidates_fetch_attempted'] - t['candidates_fetch_success'], public_skipped_duplicate=total_discovered - t['seen_url_count'], public_skipped_unsupported_scheme=t['terminal_reason_counts'].get('skipped_unsupported_scheme', 0), public_skipped_memory_gate=t['terminal_reason_counts'].get('skipped_memory_gate', 0), public_skipped_quality_gate=t['terminal_reason_counts'].get('skipped_quality_gate', 0), public_skipped_browser_unavailable=t['terminal_reason_counts'].get('skipped_browser_unavailable', 0), public_skipped_xml_or_feed=t['terminal_reason_counts'].get('skipped_xml_or_feed', 0), public_skipped_timeout=t['terminal_reason_counts'].get('skipped_timeout', 0), public_skipped_fetch_error=t['terminal_reason_counts'].get('skipped_fetch_error', 0), public_rejected_no_pattern_match=t['terminal_reason_counts'].get('rejected_no_pattern_match', 0), public_rejected_low_information=t['terminal_reason_counts'].get('rejected_low_information', 0), public_rejected_duplicate=t['terminal_reason_counts'].get('rejected_duplicate', 0), public_rejected_storage_rejected=t['terminal_reason_counts'].get('rejected_storage_rejected', 0), public_skipped_url_sample=tuple(t['skipped_samples']), public_rejected_url_samples=tuple(t['rejected_samples']), public_candidates_discovered=t['candidates_discovered'], public_candidates_fetch_attempted=t['candidates_fetch_attempted'], public_candidates_fetch_success=t['candidates_fetch_success'], public_candidates_parse_success=t['candidates_parse_success'], public_candidates_pattern_matched=t['candidates_pattern_matched'], public_candidates_built=t['candidates_built'], public_candidates_store_attempted=t['candidates_store_attempted'], public_candidates_stored=t['candidates_stored'], public_candidates_rejected=t['candidates_rejected'], public_rejection_summary=t['rejection_summary'], public_terminal_stage=t['terminal_stage'], public_discovery_empty_reason=t['discovery_empty_reason'])
...
def _pattern_context(text: str, start: int, end: int, radius: int=_FINDING_ID_CONTEXT_RADIUS) -> str:
...
def _js_confidence_from_verdict(verdict: str, status_code: int | None=None, content_length: int | None=None) -> float:
...
def _enrich_text_with_metadata(title: str, snippet: str, extracted_text: str) -> str:
...
def _score_page_quality(*, hit_url: str, hit_title: str, hit_snippet: str, hit_rank: int, query: str, extracted_text: str, discovery_score: float | None=None, discovery_reason: str | None=None) -> str:
    ...
    """
    has_discovery_signal = discovery_score is not None and discovery_score >= _DISCOVERY_SIGNAL_SCORE_THRESHOLD or (discovery_reason is not None and discovery_reason.strip() != '')
    strong_discovery = discovery_score is not None and discovery_score >= 0.7
    query_lower = query.lower()
    ...
    signals_good = sum([title_has_query, snippet_has_query, url_has_path, text_is_meaningful])
    if strong_discovery:
        signals_good += 1
    _is_bootstrap = discovery_reason in ('deterministic_bootstrap', 'seed_context_bootstrap', 'rescue') or (discovery_reason or '').startswith('keyword_bootstrap_')
    if _is_bootstrap:
        signals_good += 1
...
def _compute_page_usable_fields(*, fetched: bool, matched_patterns: int, stored_findings: int, quality_reason: str | None, discovery_signal: bool, discovery_score: float | None, error: str | None, extracted_text_len: int=0) -> tuple[bool, str, str, bool, str, str]:
...
async def _build_public_finding(*, query: str, url: str, page_text: str, hit_title: str, hit_snippet: str, discovery_score: float | None, discovery_reason: str | None, http_status_code: int=0) -> tuple:
    ...
    provenance_parts = ['source_family:public', f'url:{url[:300]}', 'label:public_surface']
    ...
    finding_id = _make_finding_id(query=query, url=url, label='public_surface', pattern='content_only', value=payload_text[:100])
    try:
        finding = CanonicalFinding(finding_id=finding_id, query=query[:500], source_type=_PUBLIC_SOURCE_TYPE, confidence=0.65, ts=time.time(), provenance=provenance, payload_text=payload_text)
        return (finding,)
    except Exception:
        return ()

async def _extract_live_public_findings_from_page(*, query: str, url: str, hit_label: str, hit_pattern: str, hit_value: str, hit_start: int, hit_end: int, page_text: str, discovery_score: float | None=None) -> tuple:
    ...
    finding = CanonicalFinding(finding_id=finding_id, query=query, source_type=_SOURCE_TYPE, confidence=confidence, ts=time.time(), provenance=provenance, payload_text=context)
    return (finding,)

async def _fetch_and_process_page(*, semaphore: asyncio.Semaphore, query: str, hit_url: str, hit_title: str, hit_snippet: str, hit_rank: int, fetch_timeout_s: float, fetch_max_bytes: int, store: Any | None, memory_manager: Any | None=None, session_id: str | None=None, discovery_score: float | None=None, discovery_reason: str | None=None, vector_store: Any | None=None, graph: Any | None=None) -> PipelinePageResult:
    """Delegate to public_fetch module (extracted from this file)."""
    from .public_fetch import _fetch_and_process_page as _impl
    return await _impl(semaphore=semaphore, query=query, hit_url=hit_url, hit_title=hit_title, hit_snippet=hit_snippet, hit_rank=hit_rank, fetch_timeout_s=fetch_timeout_s, fetch_max_bytes=fetch_max_bytes, store=store, memory_manager=memory_manager, session_id=session_id, discovery_score=discovery_score, discovery_reason=discovery_reason, vector_store=vector_store, graph=graph)
...
async def _generate_and_store_report(query: str, pages: tuple, store: Any | None, hermes_engine: Any | None, vector_store: Any | None=None) -> str:
...
async def _store_report_and_inference(store, query: str, report_text: str, report_id: str, hermes_engine) -> None:
    ...
    from hledac.universal.knowledge.duckdb_store import CanonicalFinding
    try:
        report_finding = CanonicalFinding(finding_id=report_id, query=query, source_type=_REPORT_SOURCE_TYPE, confidence=0.7, ts=time.time(), provenance=('source_family:public', 'report_generation', hermes_engine.__class__.__name__), payload_text=report_text)
    ...
    try:
        ...
        hermes_output = HermesInferenceOutput(output_id=report_id, source_finding_id=report_id, inference_type='report_synthesis', timestamp=time.time(), primary_text=report_text, confidence=0.7, key_iocs=key_iocs, key_entities=key_entities, pivot_suggestions=key_iocs[:10], bounded=False, tokens_used=0, model_name=hermes_engine.__class__.__name__, source_hints=('public',))
        hermes_finding = CanonicalFinding(finding_id=hermes_output.output_id, query=query, source_type='hermes_inference', confidence=hermes_output.confidence, ts=hermes_output.timestamp, provenance=('source_family:public', 'hermes_inference', hermes_engine.__class__.__name__), payload_text=_json.encode(hermes_output.to_dict()).decode('utf-8')[:4096])
...
async def _extract_iocs_from_report(report_text: str) -> tuple[list[str], list[str]]:
    ...
    ioc_json = re.search('<IOC_JSON>\\s*(\\{.*?\\})\\s*</IOC_JSON>', report_text, re.DOTALL)
    if ioc_json:
        try:
...
async def _inject_ct_subdomain_hits(hits: tuple, query: str) -> tuple:
    ...
    ct_hits = tuple((_CTHit(f'https://{subdomain}', idx) for idx, subdomain in enumerate(subdomains)))
    return ct_hits + hits
...
async def _inject_commoncrawl_hits(hits: tuple, query: str) -> tuple:
    ...
    if _CC_SCANNER_LOOKUP is None:
        try:
            ...
            return hits
    import re
    clean_domain = re.sub('^(site|domain):', '', query.strip(), flags=re.IGNORECASE).strip()
    if not clean_domain:
        return hits
    try:
    ...
    cc_hits = tuple((_CCHit(url=r.get('url', ''), title=r.get('title', ''), snippet=r.get('snippet', ''), rank=idx) for idx, r in enumerate(cc_results[:20])))
    return cc_hits + hits
...
async def _inject_onion_hits(hits: tuple, query: str, store: DuckDBShadowStore) -> int:
    ...
    async def _fetch_one_onion(onion_url: str) -> CanonicalFinding | None:
        try:
            ...
            return CanonicalFinding(finding_id=pf_id, query=query, source_type='onion_discovery', confidence=0.55, ts=ts_now, provenance=('onion_discovery', onion_url), payload_text=content[:500] if content else None)
        except Exception as e:
            logger.debug(f'[F193A] Onion fetch {onion_url}: {e}')
            return None
    _result = await parallel([_fetch_one_onion(url) for url in onion_urls], policy='collect', concurrency=_onion_cap, ctx='onion_hits')
...
async def async_run_live_public_pipeline(query: str, store: DuckDBShadowStore | None=None, max_results: int=10, fetch_timeout_s: float=35.0, fetch_max_bytes: int=2000000, fetch_concurrency: int=8, hermes_engine: Any | None=None, graph: Any | None=None, memory_manager: Any | None=None, session_id: str | None=None, vector_store: Any | None=None, run_loop: bool=False, rl_steps: int=0, enqueue_hypothesis_pivot: Any | None=None, public_bootstrap_enabled: bool=False, seed_context: Any | None=None, fetch_fn: Any | None=None, match_fn: Any | None=None, discovery_fn: Any | None=None, ct_subdomains_fn: Any | None=None, clear_query_cache_fn: Any | None=None, export_dir: str | None=None, _sprint_id: str='') -> PipelineRunResult:
    ...
    _ensure_patched()
    ctx = PipelineContext(query=query, store=store, max_results=max_results, fetch_timeout_s=fetch_timeout_s, fetch_max_bytes=fetch_max_bytes, fetch_concurrency=fetch_concurrency, hermes_engine=hermes_engine, graph=graph, memory_manager=memory_manager, session_id=session_id, vector_store=vector_store, run_loop=run_loop, rl_steps=rl_steps, enqueue_hypothesis_pivot=enqueue_hypothesis_pivot, public_bootstrap_enabled=public_bootstrap_enabled, seed_context=seed_context, export_dir=export_dir, _sprint_id=_sprint_id, clear_query_cache_fn=clear_query_cache_fn)
    ...
    return ResultBuilder.build(ctx=ctx, telemetry=telemetry, discovery=discovery, public_stage_failure=public_stage_failure, public_stage_failure_reason=public_stage_failure_reason, generated_report=generated_report, tot_solution_count=tot_solution_count)

def _build_emergency_result(ctx: PipelineContext) -> PipelineRunResult:
    """Build emergency abort result for UMA emergency state."""
    return PipelineRunResult(query=ctx.query, discovered=0, fetched=0, matched_patterns=0, accepted_findings=0, stored_findings=0, patterns_configured=_get_patterns_configured_count(), pages=(), error='uma_emergency_abort', public_discovery_blocker='uma_emergency_abort', public_fetch_accessibility_blocker=False, public_discovery_fallback_state=None, dominant_public_failure_mode='uma_emergency_abort', public_stage_failure='uma_emergency', public_stage_failure_reason='UMA emergency state blocks all public lane processing', public_discovery_attempted=False, public_discovery_raw_count=0, public_discovery_deduped_count=0, public_pages_fetched=0, public_pages_accepted=0, public_pages_rejected=0, public_findings_accepted=0, public_fetch_gate='emergency_blocked', public_discovered=0, public_fetch_attempted=0, public_fetch_skipped=0, public_fetch_candidate_count=0, public_fetch_attempted_urls_sample=(), public_acceptance_attempted=0, public_acceptance_accepted=0, public_acceptance_rejected=0, public_acceptance_reject_reasons={}, public_accepted_url_sample=(), public_rejected_url_sample=(), public_terminal_classified_count=0, public_unclassified_count=0, public_terminal_reason_counts={}, public_fetch_success=0, public_fetch_failed=0, public_skipped_duplicate=0, public_skipped_unsupported_scheme=0, public_skipped_memory_gate=0, public_skipped_quality_gate=0, public_skipped_browser_unavailable=0, public_skipped_xml_or_feed=0, public_skipped_timeout=0, public_skipped_fetch_error=0, public_rejected_no_pattern_match=0, public_rejected_low_information=0, public_rejected_duplicate=0, public_rejected_storage_rejected=0, public_build_success_count=0, public_build_failure_count=0, public_duplicate_count=0, public_acceptance_ratio=0.0, public_skipped_url_sample=(), public_rejected_url_samples=(), public_candidates_discovered=0, public_candidates_fetch_attempted=0, public_candidates_fetch_success=0, public_candidates_parse_success=0, public_candidates_pattern_matched=0, public_candidates_built=0, public_candidates_store_attempted=0, public_candidates_stored=0, public_candidates_rejected=0, public_rejection_summary={}, public_rescue_candidates_count=0, public_rescue_fetch_attempted=0, public_rescue_fetch_success=0, public_rescue_accepted_findings=0, public_rescue_errors=0, public_rescue_order='disabled', public_terminal_stage='uma_emergency')
