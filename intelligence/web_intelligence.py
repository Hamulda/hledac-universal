    ...
    async def infer_tech_from_jobs(self, entity_name: str) -> TechIntelligence:
        ...
        async def fetch_hn_jobs() -> None:
            ...
            try:
                session = await httpx.AsyncClient()
                async with asyncio.timeout(15.0):
                    async with session.get('https://hacker-news.firebaseio.com/v0/topstories.json', timeout=aiohttp.ClientTimeout(total=15)) as resp:
                        if resp.status != 200:
                            return
                        ...
                        for story_id in story_ids[:100]:
                            if scanned >= 20:
                                break
                            scanned += 1
                            async with session.get(f'https://hacker-news.firebaseio.com/v0/item/{story_id}.json', timeout=aiohttp.ClientTimeout(total=5)) as item_resp:
                                ...
                                if re.search('Who is Hiring\\?.*\\d{4}', title, re.IGNORECASE):
                                    if 'text' in item and item['text']:
                                        ...
                                        async with session.get(item['url'], timeout=aiohttp.ClientTimeout(total=10)) as text_resp:
        ...
        async def fetch_remoteok_jobs() -> None:
            ...
            try:
                ...
                session = await httpx.AsyncClient()
                async with asyncio.timeout(15.0):
                    async with session.get(url, headers={'User-Agent': 'Mozilla/5.0', 'Accept': 'application/json'}, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                        if resp.status != 200:
                            return
                        try:
                            import orjson
