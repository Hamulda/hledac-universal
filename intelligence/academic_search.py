    ...
    async def search(self, query: str, max_results: int=10, analysis: QueryAnalysis | None=None, async_session: httpx.AsyncClient | None=None) -> list[SearchResult]:
        ...
        try:
            ...
            async def _do_search(session: httpx.AsyncClient) -> list[SearchResult]:
                ...
                async with session.get(url, headers=headers, timeout=httpx.Timeout(total=self.config.timeout_seconds)) as response:
    ...
    async def search(self, query: str, max_results: int=10, analysis: QueryAnalysis | None=None, async_session: httpx.AsyncClient | None=None) -> list[SearchResult]:
        ...
        try:
            ...
            async def _do_search(session: httpx.AsyncClient) -> list[SearchResult]:
                ...
                async with session.get(self.base_url, params=params, headers=headers, timeout=httpx.Timeout(total=self.config.timeout_seconds)) as response:
                    if response.status != 200:
...
class SemanticScholarAdapter(BaseSourceAdapter):
    ...
    async def search(self, query: str, max_results: int=10, analysis: QueryAnalysis | None=None, async_session: httpx.AsyncClient | None=None) -> list[SearchResult]:
        ...
        try:
            ...
            async def _do_search(session: httpx.AsyncClient) -> list[SearchResult]:
                ...
                async with session.get(url, params=params, headers=headers, timeout=httpx.Timeout(total=self.config.timeout_seconds)) as response:
    ...
    async def get_paper_details(self, paper_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            _sess = await httpx.AsyncClient()
            async with _sess as sess:
                async with sess.get(url, params=params, headers=headers, timeout=httpx.Timeout(total=10)) as response:
                    if response.status == 200:
...
class SemanticScholarClient:
    ...
    async def search_ss(self, query: str, session: httpx.AsyncClient, limit: int=10) -> list[dict]:
        ...
        try:
            async with session.get(self._SS_URL, params=params, timeout=httpx.Timeout(total=12)) as r:
                if r.status == 429:
    ...
    async def search_arxiv(self, query: str, session: httpx.AsyncClient, max_results: int=5) -> list[dict]:
        ...
        try:
            async with session.get(self._ARXIV_URL, params=params, timeout=httpx.Timeout(total=12)) as r:
                r.raise_for_status()
                text = await r.text()
        except Exception as e:
