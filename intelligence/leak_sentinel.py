    ...
    try:
        async with semaphore:
            ...
            try:
                ...
                from hledac.universal.intelligence.pastebin_monitor import run as run_pastebin
            except ImportError:
...
async def _fetch_github_secret_findings(query: str, semaphore: asyncio.Semaphore) -> LeakSourceResult:
    ...
    try:
        async with semaphore:
            start = time.monotonic()
            try:
                from hledac.universal.intelligence.github_secret_scanner import SecretFinding, scan_repo
            except ImportError:
...
async def _fetch_breach_findings(query: str, semaphore: asyncio.Semaphore) -> LeakSourceResult:
    ...
    try:
        async with semaphore:
            start = time.monotonic()
            try:
                from hledac.universal.intelligence.data_leak_hunter import BreachAPIConfig, DataLeakHunter
            except ImportError:
