...
- No images >50MB stored (N/A for academic search)

Uses existing AcademicSearchEngine from academic_search.py internally.
"""
...
async def search_academic_all(
    ...
    """
    # Use semaphore to enforce rate limiting (5 concurrent = ~100 req/min with 2 req/source)
    semaphore = asyncio.Semaphore(5)

    async def limited_search(source: str, search_func):
    ...
    return {
        ...
        "crossref": results[1] if not isinstance(results[1], Exception) else [],
