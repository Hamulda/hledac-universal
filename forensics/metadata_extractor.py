            ...
            if "vbaProject.bin" in name:
                try:
                    ...
                    for _, vba_line in vba_parser.extract_macros():
                        if vba_line:
                            urls = _URL_PATTERN.findall(
                                vba_line.encode("utf-8", errors="ignore") if isinstance(vba_line, str) else vba_line
                            )
...
class ScrubbingAnalysis(Struct):
...
class SteganalysisMetadata(Struct):
...
class MetadataCache:
    ...
    MAX_ENTRIES = 10000
    __slots__ = ("_duckdb_store", "_conn", "_lock", "db_path")
...
class UniversalMetadataExtractor:
        ...
        async with self._semaphore:
            ...
            try:
                ...
                match ext_type:
                    ...
                    case "cad":
                        result.cad = await (
                            self._extract_svg_metadata if ext == ".svg" else self._extract_dxf_metadata
                        )(file_path)
                    case "email":
                ...
                return MetadataResult(
                    file_path=file_path, success=False, error=str(e), extraction_time=time.time() - start_time
                )
    ...
    async def extract_batch(self, file_paths: list[str]) -> list[MetadataResult]:
        ...
        for i in range(0, len(file_paths), self.batch_size):
            ...
            tasks = [self._extract_with_precomputed_hash(path, file_hashes.get(path)) for path in batch]
    ...
    async def _batch_precompute_file_hashes(self, file_paths: list[str]) -> dict[str, tuple[str, float, int]]:
        ...
        if len(file_paths) < self._BATCH_HASH_THRESHOLD:
            ...
            return {path: self._get_file_hash(path) for path in file_paths}
        ...
        try:
            ...
            hash_results = dict(zip(paths, blake3_hashes, strict=False))
        except Exception:
            # Fallback to serial hashlib
            hash_results = {
                path: hashlib.sha256(content).hexdigest() for path, (content, _, _) in file_contents.items()
            }
    ...
    async def _extract_with_precomputed_hash(
        ...
        async with self._semaphore:
            ...
            try:
                ...
                generic = await self._extract_generic_metadata_with_hash(file_path, precomputed)
    ...
    async def _extract_pdf_metadata(self, file_path: str) -> PDFMetadata | None:
        ...
        try:
            ...
            metadata = PDFMetadata(
                ...
                modification_date=datetime.fromisoformat(rust_meta.modification_date)
                if rust_meta.modification_date
                else None,
    ...
    async def _extract_pdf_mupdf(self, file_path: str) -> PDFMetadata | None:
        ...
        try:
            ...
            with fitz_mod.open(file_path) as doc:
                metadata = PDFMetadata(
                    num_pages=len(doc), pdf_version=doc.pdf_version() if hasattr(doc, "pdf_version") else None
                )
                info = doc.metadata
                if info:
                    metadata.title = info.get("title")
    ...
    async def _extract_steganography(self, file_path: str) -> SteganalysisMetadata | None:
        ...
        """
        try:
            from .steganography_detector import analyze_image_steganography
        except ImportError:
            return None
        try:
    ...
    def _parse_pptx_core_xml(self, core_xml: bytes, metadata: PPTXMetadata) -> None:
        ...
        try:
            ...
            metadata.author = (
                root.find(".//dc:creator", ns).text if root.find(".//dc:creator", ns) is not None else None
            )
    ...
    async def _extract_dxf_metadata(self, file_path: str) -> CADMetadata | None:
        ...
        try:
            ...
            for line in content.split("\n"):
                ...
                if (
                    line == "SECTION"
                    and "HEADER" in content[content.find(line) + len(line) : content.find(line) + len(line) + 20]
                ):
                    in_header = True
                elif line == "ENDSEC":
                    in_header = False
        ...
        try:
            ...
            if olefile.isOleFile(file_path):
                ...
                if ole.exists("__substg1.0_0042001F"):
                    metadata.subject = (
                        ole.openstream("__substg1.0_0042001F")
                        .read()
                ...
                if ole.exists("__substg1.0_0C1F001F"):
                    metadata.from_addr = (
                        ole.openstream("__substg1.0_0C1F001F")
                        .read()
                        .decode("utf-16-le", errors="ignore")
                        .rstrip("\x00")
                    )
    ...
    def _extract_exif_timeline(self, exif: dict) -> list[TimelineEvent]:
        ...
        for key, event_type in [
            ("DateTime", "captured"),
            ("DateTimeOriginal", "captured_original"),
    ...
    def _check_image_scrubbing(self, result, indicators, missing) -> None:
    ...
    def _result_from_dict(self, data: dict[str, Any]) -> MetadataResult:
        ...
        if data.get("pdf"):
            ...
            result.pdf = PDFMetadata(
                ...
                modification_date=datetime.fromisoformat(pdf["modification_date"])
                if pdf.get("modification_date")
                else None,
