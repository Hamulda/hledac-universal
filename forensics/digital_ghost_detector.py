...
_GHOST_PATTERN_GROUPS: list[tuple[str, str]] = [
    ("ts_0", "created.*modified.*0000"),
    ("ts_1", "last.*access.*1970"),
    ("ts_2", "deleted.*\\d{4}-\\d{2}-\\d{2}"),
    ("frag_0", "\\{[^{}]*\\}"),
    ("frag_1", "<[^>]+>"),
    ("frag_2", "[a-zA-Z0-9]{20,}"),
    ("shadow_0", "ref.*deleted"),
    ("shadow_1", "moved.*permanently"),
    ("shadow_2", "404.*not.*found"),
    ("shadow_3", "previously.*available"),
    ("fs_0", "\\.tmp$"),
    ("fs_1", "~$"),
    ("fs_2", "\\.bak$"),
    ("fs_3", "\\.old$"),
...
class DigitalGhostDetector:
    ...
    def analyze_file(self, file_path: str | Path) -> DigitalGhostAnalysis:
        ...
        try:
            ...
            logger.info(
                f"Ghost analysis complete: {len(result.ghost_signals)} signals, {len(result.recovered_content)} recovered fragments"
            )
    ...
    def _detect_ghost_signals(self, location: str, text_content: str, raw_content: bytes) -> list[GhostSignal]:
        ...
        for group_name, pattern in _GHOST_PATTERN_GROUPS:
            if group_name.startswith("fs_") and _re.search(pattern, location):
                sig_type, confidence, ind_list = _GHOST_GROUP_TO_TYPE[group_name]
                signals.append(
                    GhostSignal(signal_type=sig_type, location=location, confidence=confidence, indicators=ind_list)
                )
                break
                ...
                patterns.append(
                    {
                        ...
                        "confidence": 0.7 if avg_diff < 3600 else 0.5,
                    }
                )
        return patterns
        ...
        if result.ghost_signals:
            ...
            if high_conf_signals:
                recommendations.append(
                    f"High-confidence ghost signals detected ({len(high_conf_signals)}). Consider forensic recovery tools."
                )
        if result.recovered_content:
            recommendations.append(
                f"{len(result.recovered_content)} content fragments potentially recoverable. Review recovered content for sensitive information."
            )
        if result.deletion_indicators:
            recommendations.append(
                f"{len(result.deletion_indicators)} deletion indicators found. Content may have been incompletely wiped."
            )
