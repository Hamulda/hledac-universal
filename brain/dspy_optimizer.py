...
class DSPyOptimizer:
        ...
        if len(examples) < 10:
            seed_examples: list[tuple] = [
                (
                    ...
                    '{"indicator": "example.onion", "type": "darknet_domain", "risk": "medium", "findings": ["potential Tor hidden service", "requires Nym or I2P access"]}',
                ),
                (
                    "Check if this URL shows signs of compromise: http://185.220.101.47/",
                    '{"indicator": "185.220.101.47", "type": "ipv4", "risk": "high", "findings": ["high-risk Tor exit node IP", "potential hostile scanner"]}',
                ),
                (
                    "Investigate this email pattern for breach correlation: user@company.com",
                    '{"indicator": "user@company.com", "type": "email", "risk": "medium", "findings": ["email pattern matches corporate naming convention"]}',
                ),
                (
                    "Analyze this URL structure for infrastructure tracking: http://target.com/api/v2/internal/users",
                    '{"indicator": "target.com/api/v2/internal/users", "type": "url", "risk": "high", "findings": ["internal API endpoint exposed", "potential information disclosure"]}',
                ),
                (
                    "Check this IP for threat intelligence: 194.5.249.253",
                    '{"indicator": "194.5.249.253", "type": "ipv4", "risk": "critical", "findings": ["known infrastructure", "APT associated", "immediate block recommended"]}',
                ),
                (
                    "Analyze this GitHub repository for secrets: https://github.com/org/repo",
                    '{"indicator": "github.com/org/repo", "type": "github", "risk": "high", "findings": ["repository name suggests internal tooling", "requires auth for deep scan"]}',
                ),
                (
                    "Investigate this ASN for infrastructure mapping: AS212238",
                    '{"indicator": "AS212238", "type": "asn", "risk": "medium", "findings": ["Censys-infrastructure ASN", "known VPN/proxy provider"]}',
                ),
                (
                    "Check this certificate for certificate pinning issues: *.example.com",
                    '{"indicator": "*.example.com", "type": "cert_fingerprint", "risk": "low", "findings": ["wildcard cert observed", "certificate transparency log available"]}',
                ),
                (
                    "Analyze this Pastebin paste for exposed credentials: https://pastebin.com/raw/abc123",
                    '{"indicator": "pastebin.com/abc123", "type": "pastebin", "risk": "critical", "findings": ["contains potential API keys", "requires immediate takedown check"]}',
                ),
                (
                    "Investigate domain for DNS history: suspicious-domain.com",
                    '{"indicator": "suspicious-domain.com", "type": "domain", "risk": "high", "findings": ["recently registered", "nameservers in offshore jurisdiction"]}',
                ),
            ]
    ...
    async def _load_training_examples(self, limit: int = 1000) -> list[tuple]:
        ...
        try:
            if EVIDENCE_ROOT.exists():
                evidence_files = sorted(EVIDENCE_ROOT.glob("*.jsonl"), key=attrgetter("stat")().st_mtime, reverse=True)[
                    :10
                ]
        except Exception as e:
        ...
        for ev_file in evidence_files:
            ...
            try:
                ...
                for line in lines:
                    ...
                    query = (
                        payload.get("query")
                        or payload.get("params", {}).get("query")
                        or payload.get("action_params", {}).get("query")
                        or ""
                    )
                    result = (
                        payload.get("result")
                        or payload.get("action_result")
                        or payload.get("response", {}).get("content", "")
                        or str(payload.get("response", ""))
                        or ""
                    )
                    if query and result:
            ...
            if new_prompts:
                ...
                for task, prompt in new_prompts.items():
                    ...
                    self._prompt_versions[task].append(
                        {"version": ver, "prompt": prompt, "trained_at": time.time(), "examples": len(examples)}
                    )
    ...
    def _dspy_optimize_mipro(self, examples: list[tuple]) -> dict:
        ...
        try:
            ...
            if instr is None:
                logger.warning(
                    "DSPy optimizer: could not extract instructions from optimized module — using default fallback"
                )
    ...
    def record_performance(self, task: str, score: float) -> None:
...
def load_compiled_program(name: str) -> Any | None:
    ...
    if state is not None:
        ...
        if program is not None:
            ...
            logger.info(
                "Loaded compiled DSPy program '%s' (%d demos)", name, len(demos) if isinstance(demos, list) else 0
            )
            return program
