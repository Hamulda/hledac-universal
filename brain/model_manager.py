...
def _check_rss_before_load(model_key: str) -> float:
    ...
    if current_rss > threshold:
        raise MemoryPressureError(
            f"[MODEL MEMORY] RSS {current_rss:.2f}GB > threshold {threshold:.2f}GB (max_rss_gb={_model_max_rss_gb}, model={model_key}, size~{model_size}GB). Skipping model load."
        )
    return current_rss
...
def _verify_rss_after_unload(model_key: str, rss_before: float) -> None:
    ...
    if rss_before < noop_threshold:
        logger.debug(
            f"[MODEL MEMORY] Unload was a no-op for {model_key} (rss_before={rss_before:.2f}GB < expected~{model_size:.2f}GB); skipping RSS drop verification."
        )
        return
    if dropped < noop_threshold:
        logger.warning(
            f"[MODEL MEMORY] RSS did not drop expected amount after unload: dropped={dropped:.2f}GB, expected~{model_size:.2f}GB (RSS before={rss_before:.2f}GB, after={rss_after:.2f}GB, model={model_key})"
        )
    else:
...
async def model_lifecycle(model_name: ModelName) -> ModelLifecycle:
    ...
    if manager._current_model is not None:
        ...
        if current != model_name:
            logger.warning(
                f"[MODEL CONFLICT] Requested '{model_name}' but '{current}' is loaded. Releasing current model first."
            )
...
class ModelManager:
    ...
    def _create_gliner_engine(self) -> NEREngine:
        ...
        try:
            ...
            class NEREngine:
                ...
                def extract(
                    self, text: str, labels: list[str], relations: list[dict] | None = None, threshold: float = 0.5
                ) -> dict[str, Any]:
    ...
    def _check_memory_admission(self) -> None:
        ...
        try:
            ...
            if status.state == UMA_STATE_EMERGENCY:
                raise RuntimeError(
                    f"[MEMORY ADMISSION] EMERGENCY state ({status.system_used_gib:.2f} GiB) — model load BLOCKED to prevent OOM. Free up memory before retrying."
                )
            if status.state == UMA_STATE_CRITICAL:
                raise RuntimeError(
                    f"[MEMORY ADMISSION] CRITICAL state ({status.system_used_gib:.2f} GiB) — model load BLOCKED to prevent OOM. Free up memory before retrying."
                )
        except RuntimeError:
            raise
        except Exception:
    ...
    async def _convert_modernbert_to_coreml(self, embedder: Any) -> bool:
        ...
        try:
            ...
            try:
                ct_model = ct.convert(
                    mlx_model, source="pytorch", convert_to="mlprogram", compute_units=ct.ComputeUnit.CPU_AND_NE
                )
            except Exception as e:
    ...
    async def _begin_model_unload(
        self, model_name: str, model_type: ModelType
    ) -> DeepHermes3Engine | ModernBertAdapter | CoreMLEmbedder | None:
    ...
    async def _prepare_hermes_quantization(self, model_type: ModelType) -> None:
        ...
        try:
            ...
            logger.info(
                f"[F203J] Hermes quantization selected: {budget.quantization} (tokens={budget.max_tokens}, latency={budget.max_latency_ms}ms, reason={budget.reason})"
            )
        except Exception as e:
    ...
    async def _unload_model_with_verification(
        self, model: Any, model_type: ModelType, model_name: str, rss_before_unload: float
    ) -> None:
        ...
        if model is not None and hasattr(model, "unload"):
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "[P1E-B] Model unload timed out after %.1fs for %s — continuing shutdown", timeout_s, model_name
                    )
                except Exception as e:
...
class _SyncCompatibilityWrapper:
    ...
    def acquire(self, model_name: str) -> bool:
        ...
        try:
            ...
            if loop.is_running():
                raise RuntimeError(
                    "Cannot use sync acquire() in async context. Use: model = await manager.load_model('hermes')"
                )
            else:
    ...
    def release(self, model_name: str) -> bool:
        ...
        try:
            ...
            if loop.is_running():
                raise RuntimeError(
                    "Cannot use sync release() in async context. Use: await manager.release_model('hermes')"
                )
            else:
