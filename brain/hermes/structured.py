...
async def generate_structured(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            last_error = e
            logger.debug(
                f"[STRUCTURED] Attempt {attempt + 1} failed: {e}"
            )
...
async def _structured_with_outlines(
    engine,
    prompt: str,
    response_model: type[T],
    temperature: float,
...
async def _structured_with_parse(
    engine,
    prompt: str,
    response_model: type[T],
    temperature: float,
...
def _parse_json_response(text: str, response_model: type[T]) -> T:
    """
...
async def structured_with_correction(
    engine,
    prompt: str,
    response_model: type[T],
    temperature: float,
...
Generate corrected output:"""
    ...
    raise RuntimeError(
        f"structured_with_correction failed after {max_corrections + 1} attempts"
    )
