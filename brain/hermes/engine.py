...
class DeepHermesConfig:
    ...
    def __init__(
        ...
        self.model_path = model_path or os.getenv(
            "HLEDAC_HERMES_MODEL",
            "mlx-community/Hermes-3-Llama-3.2-3B-Flash-Bf16"
        )
...
class DeepHermes3Engine:
    ...
    async def generate_stream(
        ...
        *,
        ...
        async for token in stream_tokens(
            self, formatted, max_tokens, temperature or 0.7
        ):
            yield token
    ...
    async def generate_structured(
        ...
        return await generate_structured(
            self, prompt, response_model,
            temperature, max_tokens, system_msg, max_retries, priority
        )
    ...
    async def warmup_prefix_cache(
        ...
        if cache_path.exists():
            return await restore_warmup_cache_async(
                self,
                cache_path,
                cache_path.stem.removeprefix("warmup_")
            )
        return False
