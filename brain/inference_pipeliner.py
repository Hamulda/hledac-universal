...
class InferencePipeliner:
    ...
    async def generate(
        ...
        """
        future = self.submit(
            prompt=prompt, temperature=temperature, max_tokens=max_tokens, system_msg=system_msg, thinking=thinking
        )
        return await future
        ...
        try:
            ...
            if self._worker_thread is not None and self._worker_thread.is_active():
                coro = engine.generate(
                    prompt=preprocessed, temperature=req.temperature, max_tokens=req.max_tokens, system_msg=None
                )
