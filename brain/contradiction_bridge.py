...
_IOC_PROTOCOL_MAP: dict[str, list[str]] = {'ip': ['shodan', 'censys', 'bgp', 'ct'], 'domain': ['ct', 'passive_dns', 'shodan', 'censys'], 'hash': ['url', 'ct'], 'url': ['url'], 'email': ['url'], 'cve': ['url']}
...
class ContradictionBridge:
    ...
    def set_retract_callback(self, callback: Any) -> None:
        ...
        if callback is not None:
            logger.info('[ContradictionBridge] META-008 retract callback registered: %s', getattr(callback, '__name__', repr(callback)))
    ...
    def _detect_tri_source(self, contradiction: Any, ioc_value: str, findings: list[dict[str, Any]]) -> TriSourceContrd | None:
        """
        ...
        try:
            ...
            if len(true_sources) == 2 and len(false_sources) == 1:
                return TriSourceContrd(affected_ioc=ioc_value, dissenter_source=false_sources[0], supporting_sources=true_sources, severity=float(getattr(contradiction, 'severity', 0.7)), contradiction_type=getattr(contradiction, 'contradiction_type', 'factual'))
            elif len(true_sources) == 1 and len(false_sources) == 2:
                return TriSourceContrd(affected_ioc=ioc_value, dissenter_source=true_sources[0], supporting_sources=false_sources, severity=float(getattr(contradiction, 'severity', 0.7)), contradiction_type=getattr(contradiction, 'contradiction_type', 'factual'))
            ...
            if sole_dissenter_count >= cutoff:
                try:
                    ...
                    if facts_retracted > 0:
                        ...
                        logger.info("[ContradictionBridge] META-008 AUTO-RETRACTED systematic dissenter '%s': sole_dissenter_count=%d (≥%d), facts_retracted=%d", source_id, sole_dissenter_count, cutoff, facts_retracted)
                except Exception as e:
                    logger.debug("[ContradictionBridge] META-008 retract '%s' failed: %s", source_id, e)
        self._tri_source_index.clear()
        return retracted

    def build_alerts(self, contradictions: list[Any], ioc_entities: list[Any], findings: list[dict[str, Any]], sprint_id: str='') -> list[Any]:
        """
        ...
        for c in capped:
            ...
            alert = EntropyAlert(entity_id=ioc_value, entropy=round(1.0 - severity, 3), threshold_exceeded=SEVERITY_THRESHOLD, confidence=round(max(0.0, severity - 0.2), 3), risk_level='high', metadata={'reason': 'propositional_contradiction', 'sources': affected_sources, 'severity': severity, 'contradiction_type': getattr(c, 'contradiction_type', 'factual'), 'claim_a': getattr(c, 'claim_a', '')[:200], 'claim_b': getattr(c, 'claim_b', '')[:200], 'alternative_protocols': alt_protocols, 'trigger_path': 'adversarial_contradiction', 'sprint_id': sprint_id}, contradiction_source_id=tri.dissenter_source if tri else None)
            alerts.append(alert)
            self._stats.alerts_emitted += 1
        if tri_source_events:
            logger.debug('[ContradictionBridge] Detected %d tri-source contradictions across %d potential dissenters', len(tri_source_events), len(self._tri_source_index))
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {'emit_count': self._stats.emit_count, 'contradictions_seen': self._stats.contradictions_seen, 'alerts_emitted': self._stats.alerts_emitted, 'sources_auto_retracted': self._stats.sources_auto_retracted, 'last_emit_ts': self._stats.last_emit_ts, 'active_tri_source_sources': len(self._tri_source_index)}
