...
# -- Bounds (M1 8GB safe) -----------------------------------------------------
SEVERITY_THRESHOLD: float = 0.7  # Only emit if severity > this
MAX_EMIT_CONTRADICTIONS: int = 20  # Cap per synthesis call
AUTO_RETRACT_DISSENT_THRESHOLD: int = 3  # [META-008] ≥3 tri-source = systematic dissenter

# -- Alternative protocol map (duplicated from synthesis_runner.py to avoid circular import)
...
# Env gate — shared with HLEDAC_ENABLE_CONTRADICTION_FEEDBACK (both gates must agree)
...
class ContradictionBridge:
    ...
    __slots__ = (
        "_enabled",
        "_stats",
        "_tri_source_index",  # source_id → list[TriSourceContrd]
        "_retract_callback",   # async callable(source_id) -> int (facts retracted)
    )

    def __init__(self) -> None:
        self._enabled: bool = _ENABLED
        self._stats: ContradictionEmitStats = ContradictionEmitStats()
        # [META-008] Tri-source index: source_id → list of tri-source events
        self._tri_source_index: dict[str, list[TriSourceContrd]] = {}
        # [META-008] Retraction callback (JTMS.retract_source or IOCGraph.retract_source)
    ...
    def _build_affected_sources(
        ...
        for field_name in ("evidence_supporting_a", "evidence_supporting_b"):
            ...
            if ev_ids:
                for eid in ev_ids:
                    # evidence_id format: "ev_<12hex>" — derive source hint
                    # In practice, source comes from the finding dicts passed separately
                    if isinstance(eid, str) and eid.startswith("ev_"):
                        # Strip evidence prefix to get source hint
                        # The actual source tracking happens via tri-source analysis
                        sources.append(str(eid)[:50])
        return sources[:10]  # Cap at 10 sources per contradiction
    ...
    def _detect_tri_source(
        ...
        try:
            ...
            # Build source map from findings: source → normalized claim
            ...
            if sole_dissenter_count >= cutoff:
                try:
                    ...
                    if facts_retracted > 0:
                        ...
                        logger.info(
                            "[ContradictionBridge] META-008 AUTO-RETRACTED "
                            "systematic dissenter '%s': sole_dissenter_count=%d "
                            "(≥%d), facts_retracted=%d",
                            source_id,
    ...
    def build_alerts(
        ...
        # Cap at MAX_EMIT_CONTRADICTIONS
        ...
        for c in capped:
            ...
            if ioc_value in seen_entities:
                continue  # Deduplicate: one alert per entity per synthesis call
            seen_entities.add(ioc_value)

            # Build affected sources list
            affected_sources = self._build_affected_sources(c)

            # Detect tri-source pattern for [META-008] auto-retraction
            # _tri_source_index mutations happen in _auto_retract_systematic_dissenters
            # (called after build_alerts returns), so no lock needed here
            ...
            # [META]-011: Resolve alternative protocols for this IOC type
            # Find the matching IOC entity to get its type
            ...
            alert = EntropyAlert(
                entity_id=ioc_value,
                entropy=round(1.0 - severity, 3),  # Invert: high severity → high entropy
                threshold_exceeded=SEVERITY_THRESHOLD,
                confidence=round(max(0.0, severity - 0.2), 3),  # Conf = sev - penalty
                risk_level="high",
                metadata={
        ...
        # Trigger async auto-retraction of systematic dissenters
        # Note: called synchronously here; caller should await if needed
        if tri_source_events:
            logger.debug(
                "[ContradictionBridge] Detected %d tri-source contradictions "
