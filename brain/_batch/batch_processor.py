...
class BatchConfig:
    ...
    age_bump_interval: float = 5.0
    tie_breaker: float = 1e-06
...
class BatchProcessor:
    ...
    """
    __slots__ = ('_config', '_flush_cycle', '_last_age_bump', '_lock', '_queue', '_shutting_down', '_stats', '_telemetry', '_worker_task')
    ...
    def get_stats(self) -> BatchStats:
        ...
        return BatchStats(processed_count=self._stats.processed_count, failed_count=self._stats.failed_count, queue_depth=len(self._queue), avg_latency_ms=self._stats.avg_latency_ms, last_flush_at=self._stats.last_flush_at, retried_count=self._stats.retried_count, sharded_count=self._stats.sharded_count)
    ...
    def is_batch_safe(self, response_model: type | None, priority: float, stream: bool, timeout: float) -> bool:
        """
    ...
    def current_flush_interval(self) -> float:
        ...
        if pressure > 0.8:
            return self._config.default_flush_interval * 0.5
        elif pressure > 0.5:
    ...
    async def flush(self, timeout: float=5.0) -> int:
        ...
        processed = sum((1 for _, result in results if not isinstance(result, Exception)))
        return processed
    ...
    async def _process_batch_with_shard_retry(self, items: list[BatchItem]) -> list[tuple[BatchItem, Any]]:
        """
        ...
        try:
            ...
            for item in items:
                item_result: Any = batch_error
                for attempt in range(self._config.max_item_retries + 1):
                    try:
                        item_result = await safe_wait_for(self._process_single(item), timeout=item.timeout)
                        ...
                        if attempt < self._config.max_item_retries:
                            ...
                            logger.debug(f'[BatchProcessor] Item {item.id} retry {attempt + 1}/{self._config.max_item_retries}')
                        else:
                            self._stats.failed_count += 1
                            logger.warning(f'[BatchProcessor] Item {item.id} exhausted retries: {item_error}')
