...
MICRO_MODELS: dict[str, MicroModelSpec] = {
    'qwen_coder': MicroModelSpec(
        ...
        code_patterns=(
            ...
            r'from\s+\w+\s+import', r'function\s*\w*\s*\(', r'=>\s*\{',
            r'const\s+\w+\s*=', r'let\s+\w+\s*=', r'var\s+\w+\s*=',
            r'#include', r'public\s+class', r'private\s+void',
            r'async\s+def', r'@\w+\s*\(', r'```\w*',
        ),
        sql_patterns=(
            r'SELECT\s+.+\s+FROM', r'INSERT\s+INTO', r'UPDATE\s+\w+\s+SET',
            r'DELETE\s+FROM', r'CREATE\s+TABLE', r'ALTER\s+TABLE',
            r'DROP\s+TABLE', r'JOIN\s+\w+\s+ON', r'WHERE\s+\w+',
        ),
    ),
