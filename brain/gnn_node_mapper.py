...
class MappingLRUCache:
    ...
    def __init__(self, max_size: int = MAX_MAPPED_NODES) -> None:
    ...
    def set_embedding_ref(self, kuzu_id: str, table_name: str, row_id: int, dimension: int) -> None:
        """Set embedding reference for a node (for LanceDB lookup)."""
        self._embedding_refs[kuzu_id] = EmbeddingReference(
            table_name=table_name, row_id=row_id, dimension=dimension, updated_at=0.0
        )
...
async def fetch_node_embeddings(
    mapping: NodeMapping, lancedb_path: Path, table_name: str = "node_embeddings", batch_size: int = 1000
) -> dict[str, list[float]]:
...
def build_gnn_feature_matrix(
    mapping: NodeMapping, node_ids: list[str], embeddings: dict[str, list[float]], embedding_dim: int = 64
