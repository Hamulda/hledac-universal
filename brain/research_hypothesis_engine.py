...
class HypothesisEngine:
    ...
    def _extract_iocs_from_text(self, text: str, source_type: str, finding_id: str, ts: float) -> list[CausalEntity]:
        ...
        return self._causal_reasoner._extract_iocs_from_text(text, source_type, finding_id, ts)
    ...
    def _attempt_adversarial_falsification(self, hypothesis: Hypothesis) -> FalsificationResult:
        ...
        all_evidence_ids = hypothesis.supporting_evidence + hypothesis.conflicting_evidence
        all_evidence = [self._evidence.get(eid) for eid in all_evidence_ids if eid in self._evidence]
    ...
    def merge_hypotheses(self, h1: Hypothesis, h2: Hypothesis) -> Hypothesis | None:
        """
    ...
    def suggest_next_queries(self, findings: list[str] | str, context: dict[str, Any] | None=None, max_queries: int=5) -> list[dict[str, str]]:
        ...
        if len(queries) < max_queries:
            model_queries = self._model_assisted_query_suggestion(findings, context, max_queries - len(queries))
            if model_queries:
    ...
    def _ner_capability_probe(self, text: str, heuristic_entities: list[str], heuristic_iocs: list[tuple[str, str]]) -> tuple[list[str], list[tuple[str, str]]]:
        ...
        try:
            ...
            return (merged_entities[:12], heuristic_iocs)
        except Exception:
            return (heuristic_entities, heuristic_iocs)
