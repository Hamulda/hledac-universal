...
CONSISTENCY_SEVERITY_THRESHOLD: float = 0.6  # Re-fetch if severity >= this
...
class PropositionalContradictionAlert:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            # EntropyAlert compatibility fields
            "alert_type": "propositional_contradiction",
            "entropy": 0.0,  # Not applicable for propositional
            "threshold_exceeded": CONSISTENCY_SEVERITY_THRESHOLD,
            "confidence": 1.0 - self.severity,  # Inverted: high severity = low confidence
        }
...
class PropositionalConsistencyBridge:
    ...
    async def check_batch(
        ...
        try:
            # Call Rust/Python consistency verifier
            ...
            logger.info(
                "[CONSISTENCY] Check #%d: %d findings -> %d contradictions "
                "(%d alerts emitted) in %.1fms | batch_score=%.3f",
    ...
    async def _emit_alerts(self, alerts: list[PropositionalContradictionAlert]) -> None:
        ...
        try:
            ...
            # Convert to EntropyAlert for EntropyFetchBridge compatibility
            for alert in alerts:
                entropy_alert = EntropyAlert(
                    entity_id=alert.entity_id,
                    entropy=alert.severity,  # Use severity as pseudo-entropy
                    threshold_exceeded=CONSISTENCY_SEVERITY_THRESHOLD,
