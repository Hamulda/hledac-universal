...
class MLXModelPool:
    ...
    """
    __slots__ = (
        "_budget_bytes", "_loaded", "_lock", "_config",
        "_total_evictions", "_total_hits", "_total_misses",
        "_last_eviction_at", "_eviction_history", "_loaded_count",
    )
    ...
    async def acquire(
        self,
        model_id: str,
        loader: Callable[[], Awaitable[tuple[Any, Any | None]]] | Callable[[], tuple[Any, Any | None]],
    ) -> tuple[Any, Any | None]:
        async with self._lock:
            ...
            try:
                ...
                e = ModelEntry(
                    ...
                    size_bytes=size, loaded_at=time.time(), access_count=1,
                )
