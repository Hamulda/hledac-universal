...
class MLXModelPool:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            "budget_gb": self._config.budget_gb,
            "budget_bytes": self._budget_bytes,
            "loaded_count": self._loaded_count,
            "total_bytes_used": self.total_bytes_used,
            "total_evictions": self._total_evictions,
            "hit_rate_pct": self._total_hits / total * 100 if total > 0 else 0,
            "models": {
                mid: {"size_mb": e.size_bytes / 1024**2, "access_count": e.access_count}
                for mid, e in self._loaded.items()
            },
        }
    ...
    async def acquire(
        self,
        model_id: str,
        loader: Callable[[], Awaitable[tuple[MLXModel, MLXTokenizer | None]]]
        | Callable[[], tuple[MLXModel, MLXTokenizer | None]],
    ) -> tuple[MLXModel, MLXTokenizer | None]:
    ...
    def _select_lru_candidate(self) -> str | None:
        ...
        q8_candidates = [mid for mid in self._loaded.keys() if self._get_model_priority(mid) < 5]
        if q8_candidates:
    ...
    async def _evict_if_needed(self) -> None:
        while self.total_bytes_used >= self._budget_bytes and self._loaded:
            ...
            if elapsed < self._config.min_eviction_interval_s and len(self._loaded) > 1:
                logger.warning("[MLX_POOL] Eviction throttled")
                break
            mid = self._select_lru_candidate()
            if mid is None:
                break
    ...
    async def preload_async(
        self, model_id: str, loader: Callable[[], Awaitable[tuple[MLXModel, MLXTokenizer | None]]]
    ) -> None:
        """
                ...
                try:
                    ...
                    async with self._lock:
                        if model_id not in self._loaded:
                            size = self._estimate_model_size(model, tokenizer)
                            e = ModelEntry(
                                model=model, tokenizer=tokenizer, size_bytes=size, loaded_at=time.time(), access_count=1
                            )
                            self._loaded[model_id] = e
                            self._loaded.move_to_end(model_id)
                            self._loaded_count = len(self._loaded)
