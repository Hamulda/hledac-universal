...
class GraphSAGEModel:
    ...
    def _init_mlx_layers(self) -> None:
...
class ANEGNNEngine:
    ...
    def __init__(self, config: GNNConfig | None = None, model_path: Path | None = None) -> None:
        ...
        if _ANERUST_AVAILABLE and _rust_ane is not None:
            try:
                ...
                _rust_ane.load_model(
                    "graphsage_ane", str(self.model_path), self.config.out_dim, self.config.max_batch_nodes
                )
    ...
    def _run_coreml_inference(self, features: np.ndarray, adj: np.ndarray) -> np.ndarray:
        ...
        input_feature = _CoreML.MLMultiArray.alloc().initWithShape_dataType_error_(
            feat_shape, _CoreML.MLMultiArrayDataTypeFloat32, None
        )
    ...
    async def run_inference(
        self, node_ids: list[str], features: np.ndarray, edges: list[tuple[int, int]]
    ) -> GNNBatchResult:
...
class HybridLinkPredictor:
    ...
    def __init__(
        self, gnn_engine: ANEGNNEngine | None = None, duckdb_path: Path | None = None, gnn_weight: float = 0.6
    ) -> None:
    ...
    def _init_rust_heuristics(self) -> None:
    ...
    def _compute_heuristic_score(
        self, src_idx: int, dst_idx: int, adjacency: dict[int, list[int]], degrees: dict[int, int]
    ...
    async def predict_links(
        ...
        degrees: dict[int, int] = dict.fromkeys(range(n_nodes), 0)
...
def export_graphsage_to_coreml(config: GNNConfig | None = None, output_path: Path | None = None) -> Path:
    ...
    model = GraphSAGEModel(
        in_dim=config.in_dim, hidden_dim=config.hidden_dim, out_dim=config.out_dim, num_layers=config.num_layers
    )
...
if __name__ == "__main__":
    ...
    if args.export:
        ...
        engine = ANEGNNEngine()
        print("ANE-GNN Engine initialized")
