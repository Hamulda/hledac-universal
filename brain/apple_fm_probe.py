...
def _check_apple_intelligence_enabled() -> tuple[bool, str | None]:
    ...
    try:
        result = subprocess.run(
            ["system_profiler", "SPApplicationsFeedbackAssistantDataType"], capture_output=True, text=True, timeout=5
        )
...
def get_nl_entities(text: str) -> list:
    ...
    def _block(tag, token_range, _stop) -> bool:
        if tag:
    ...
    tagger.enumerateTagsInRange_unit_scheme_options_usingBlock_(
        (0, len(text)), NaturalLanguage.NLTokenUnit.word, NaturalLanguage.NLTagScheme.nameType, 0, _block
    )
    return entities
