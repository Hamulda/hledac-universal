...
class DarkQuerySignature:
    ...
    if _DSPY_AVAILABLE:
        ...
        dark_queries = dspy.OutputField(
            desc="JSON list of {type, query, priority, reasoning}. type ∈ {onion,ipfs,paste,i2p}. priority 0-1."
        )
...
class DarkQueryProgram:
    ...
    def forward(
        self, ioc_brief: str, available_transports: str = "tor+stealth", max_queries: int = 10
    ) -> dspy.Prediction:
...
def load_compiled_program(name: str) -> Any | None:
    ...
    if not path.exists():
        logger.info(
            "DSPy: No compiled program for '%s' — running zero-shot. Compile with: python scripts/dspy_compile.py --program %s --train gold_data/dark_queries.jsonl",
            name,
            name,
        )
        return None
...
class ContradictionResolverProgram:
    ...
    def forward(self, contradictory_findings: list[dict], context: str = "") -> dspy.Prediction:
        ...
        findings_text = (
            "\n".join(f"- [{f.get('conflict_mass', 0):.2f}] {f.get('finding', '')[:150]}" for f in limited)
            if limited
            else "No contradictory findings"
        )
...
class MultiHopDeepResearchChain:
    ...
    def __init__(self, max_hops: int | None = None, graph_rag: Any = None) -> None:
        """
        ...
        for hop_n in range(1, effective_max_hops + 1):
            ...
            try:
                ...
                if confidence < self.CONFIDENCE_THRESHOLD:
                    logger.debug(
                        f"MultiHop: stopping at hop {hop_n} (confidence={confidence:.2f} < {self.CONFIDENCE_THRESHOLD})"
                    )
                    break
    ...
    def _fetch_graph_evidence(self, graph_rag: Any, query: str, hop_number: int) -> list[str]:
        ...
        try:
            ...
            result = graph_rag.multi_hop_search(
                query=query, hops=self.MAX_HOPS_PER_SEARCH, max_nodes=self.MAX_NODES_PER_HOP
            )
            findings = []
