...
class BoundedInferencePipeline:
    ...
    def __init__(self, engine: DeepHermes3Engine) -> None:
        ...
        self._input_queue: asyncio.Queue[PipelineItem] = asyncio.Queue(maxsize=_INPUT_QUEUE_MAX)
        self._prep_to_inf: asyncio.Queue[PipelineItem] = asyncio.Queue(maxsize=_PREP_TO_INF_MAX)
        self._inf_to_post: asyncio.Queue[PipelineItem] = asyncio.Queue(maxsize=_INF_TO_POST_MAX)
            ...
            while not queue.empty():
                try:
                    ...
                    if not item.future.done():
                        item.future.set_exception(RuntimeError("pipeline shutdown"))
        ...
        while not self._closed:
            ...
            try:
                ...
                if item.formatted_prompt is None:
                    if not item.future.done():
                        item.future.set_exception(RuntimeError("formatted_prompt is None"))
                    self._prep_to_inf.task_done()
                    self._stats["failed"] = int(self._stats["failed"]) + 1
                    continue

                try:
                    gen_tokens = self._engine._tokenizer.encode(item.formatted_prompt)
                except Exception:
                    gen_tokens = None
                ...
                if gen_tokens is not None:
                    _max_total = self._engine.config.context_window - item.max_tokens - 50
                    if len(gen_tokens) > _max_total:
                        ...
                        formatted = self._engine._format_chatml(
                            system_msg=(item._system_used or "You are a helpful research assistant."),
                            user_msg=truncated,
                        )
                        try:
                            gen_tokens = self._engine._tokenizer.encode(formatted)
                        except Exception:
                            gen_tokens = None
                        logger.warning(
                ...
                prefix_cache = self._engine._resolve_kv_cache(item.system_msg, formatted)
                ...
                logger.warning("[Pipeline] inference timeout for %s", item.request_id)
                if not item.future.done():
                    item.future.set_exception(TimeoutError(f"inference timeout after {_INF_TIMEOUT_S}s"))
                self._stats["failed"] = int(self._stats["failed"]) + 1
            except Exception as e:
        ...
        while not self._closed:
            ...
            try:
                ...
                if item.raw_text is None:
                    if not item.future.done():
                        item.future.set_exception(RuntimeError("raw_text is None"))
                    self._stats["failed"] = int(self._stats["failed"]) + 1
                ...
                self._stats["completed"] = int(self._stats["completed"]) + 1
