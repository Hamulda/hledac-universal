...
try:
    ...
    class ExtractionSignature(dspy.Signature):
        ...
        relations: list[dict] = dspy.OutputField(desc="Extracted relations: list of {source, target, type, confidence}")
    ...
    class HypothesisSignature(dspy.Signature):
        ...
        hypotheses = dspy.OutputField(desc="List of hypotheses: type, statement, prior_probability, status")
    ...
    class DeepResearchHopSignature(dspy.Signature):
        ...
        next_query: str = dspy.OutputField(desc="Most promising next research direction to explore")
        reasoning: str = dspy.OutputField(desc="Why this direction reduces epistemic uncertainty about the query")
        confidence: float = dspy.OutputField(desc="Confidence this hop will yield new findings (0.0 to 1.0)")
