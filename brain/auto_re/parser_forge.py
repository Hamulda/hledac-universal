...
_FORBIDDEN_PATTERNS = [
    re.compile("\\bos\\b"),
    re.compile("\\bsubprocess\\b"),
    re.compile("\\beval\\b"),
    re.compile("\\bexec\\b"),
    re.compile("\\b__import__\\b"),
    re.compile("\\bgetattr\\b"),
    re.compile("\\bsetattr\\b"),
    re.compile("\\bopen\\s*\\("),
    re.compile("\\bcompile\\s*\\("),
    re.compile("\\bbreakpoint\\b"),
    re.compile("\\bglobals\\b"),
    re.compile("\\blocals\\b"),
    re.compile("\\binput\\b"),
]
...
class AutoREEngine:
    ...
    __slots__ = ("_catalog", "_attempt_count", "_enabled")
    ...
    async def process_unknown_binary(self, file_path: str, content: bytes) -> AutoREResult:
        ...
        (time.monotonic() - stage_a_start) * 1000
    ...
    async def _sandboxed_execute(self, parser_python: str, data: bytes) -> list[dict[str, Any]]:
        ...
        try:
            ...
            result_str = await asyncio.wait_for(
                loop.run_in_executor(None, self._run_subprocess_sandbox, wrapper_path, data),
