...
class ParsedIOC(Struct, frozen=True):
...
@dataclass(slots=True)
class AutoREResult(Struct, frozen=True):
    """
    ...
    error: str = ''
    stage: str = ''
    hermes3_ms: float = 0.0
    sandbox_ms: float = 0.0
    timestamp: float = field(default_factory=time.time)
_AUDIT_DIR = Path.home() / '.cache' / 'hledac' / 'auto_re'
_MAX_CODE_BYTES = 2048
_MAX_ATTEMPTS_PER_SPRINT = 3
_SANDBOX_TIMEOUT_S = 5.0
_FORBIDDEN_PATTERNS = [re.compile('\\bos\\b'), re.compile('\\bsubprocess\\b'), re.compile('\\beval\\b'), re.compile('\\bexec\\b'), re.compile('\\b__import__\\b'), re.compile('\\bgetattr\\b'), re.compile('\\bsetattr\\b'), re.compile('\\bopen\\s*\\('), re.compile('\\bcompile\\s*\\('), re.compile('\\bbreakpoint\\b'), re.compile('\\bglobals\\b'), re.compile('\\blocals\\b'), re.compile('\\binput\\b')]
...
class AutoREEngine:
    ...
    """
    __slots__ = tuple(('_catalog', '_attempt_count', '_enabled'))
    ...
    async def process_unknown_binary(self, file_path: str, content: bytes) -> AutoREResult:
        """
        ...
        if len(content) > 1048576:
            return AutoREResult(success=False, file_hash=hashlib.sha256(content).hexdigest(), format_family='unknown', error='File exceeds 1 MB size limit for AutoRE', stage='A')
        ...
        prompt = self.catalog.build_hermes3_prompt(header512=header512, entropy=entropy, ascii_ratio=ascii_ratio, file_path=file_path, format_family=chosen_family)
        hermes3_ms = 0.0
        format_hypothesis = ''
        parser_python = ''
        engine = _get_hermes3_engine()
        if engine is None:
            return AutoREResult(success=False, file_hash=file_hash, format_family=family_name, error='Hermes3Engine unavailable', stage='B')
        try:
            hermes3_start = time.monotonic()
            response = await engine.generate(prompt, system_msg='You are a forensic binary format analyst. Always output valid JSON in <|constrain|> tags.', max_tokens=1024, temperature=0.3)
            ...
            return AutoREResult(success=False, file_hash=file_hash, format_family=family_name, error=f'Hermes3 call failed: {e}', stage='B')
        try:
            ...
            return AutoREResult(success=False, file_hash=file_hash, format_family=family_name, error=f'Parse Hermes3 response failed: {e}', stage='B')
        sandbox_ms = 0.0
        ...
        try:
            ...
            return AutoREResult(success=False, file_hash=file_hash, format_family=family_name, format_hypothesis=format_hypothesis, parser_source=parser_python, error=f'Sandbox execution failed: {e}', stage='C', hermes3_ms=hermes3_ms)
        iocs: list[ParsedIOC] = []
        try:
            ...
            iocs = []
        success = len(iocs) > 0
        result = AutoREResult(success=success, file_hash=file_hash, format_family=family_name, format_hypothesis=format_hypothesis, iocs=iocs, parser_source=parser_python, error='' if success else 'Stage D: no valid IOCs after gate', stage='D', hermes3_ms=hermes3_ms, sandbox_ms=sandbox_ms)
        await self._save_audit_cache(file_hash, result)
        return result
    CONSTRAIN_PATTERN = re.compile('<\\|constrain\\|>\\s*(.*?)\\s*<\\|message\\|>', re.DOTALL)
    ...
    def _validate_code_length(self, code_bytes: bytes) -> None:
        ...
        if len(code_bytes) > _MAX_CODE_BYTES:
            raise ValueError(f'Generated code exceeds {_MAX_CODE_BYTES} bytes (got {len(code_bytes)})')
    ...
    async def _sandboxed_execute(self, parser_python: str, data: bytes) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            result_str = await asyncio.wait_for(loop.run_in_executor(None, self._run_subprocess_sandbox, wrapper_path, data), timeout=_SANDBOX_TIMEOUT_S + 1.0)
        finally:
            try:
    ...
    def _run_subprocess_sandbox(self, script_path: Path, data: bytes) -> str:
        """
        ...
        """
        import base64
        env = {'PATH': '/usr/bin:/bin', 'PYTHONDONTWRITEBYTECODE': '1', 'PYTHONUNBUFFERED': '1'}
        data_b64 = base64.b64encode(data).decode('ascii')
        try:
            proc = subprocess.run([sys.executable, str(script_path)], input=data_b64.encode('ascii'), capture_output=True, timeout=_SANDBOX_TIMEOUT_S, env=env)
    ...
    async def _validate_iocs(self, parser_output: list[dict[str, Any]]) -> list[ParsedIOC]:
        """
        ...
        if rust is None:
            return [ParsedIOC(ioc_type=str(item.get('ioc_type', 'unknown')), ioc_value=str(item.get('ioc_value', '')), confidence=min(float(item.get('confidence', 0.3)), 1.0), context=str(item.get('context', ''))) for item in parser_output if item.get('ioc_value')]
        ioc_values = [str(item.get('ioc_value', '')) for item in parser_output if item.get('ioc_value')]
        if not ioc_values:
            return []
        combined = '\n'.join(ioc_values)
        loop = asyncio.get_running_loop()
        rust_iocs = await loop.run_in_executor(None, lambda: rust.extract_iocs_simd(combined))
        ...
        for item in parser_output:
            ...
            if ioc_value in rust_confirmed:
                validated.append(ParsedIOC(ioc_type=ioc_type, ioc_value=ioc_value, confidence=min(float(item.get('confidence', 0.7)), 1.0), context=str(item.get('context', ''))))
        return validated
    ...
    async def _save_audit_cache(self, file_hash: str, result: AutoREResult) -> None:
        ...
        try:
            ...
            logging.getLogger(__name__).warning('[AUTO-RE] Stage E: failed to save audit cache: %s', e)
