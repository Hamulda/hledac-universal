...
class MicroModelPool:
    ...
    def __init__(
        self, memory_budget_mb: int | None = None, eviction_threshold: float = 0.9, preload_all: bool = True
    ) -> None:
    ...
    def _batch_preload(self) -> None:
        ...
        print(
            f"[MicroModelPool] ISSUE-022-06: UMA fragmentation score: {report['status']} ({report['fragmentation_score']:.3f})"
        )
    ...
    def _load_model_weights_only(self, model_id: str) -> LoadedMicroModel:
        ...
        try:
            model, tokenizer = mlx_lm.load(
                spec.full_path, tokenizer_mode="slow" if spec.task_type == TaskType.EMBEDDINGS else "auto"
            )
    ...
    def generate(self, model_id: str, prompt: str, max_tokens: int = 256, temp: float = 0.7, **kwargs) -> str:
        ...
        response = mlx_lm.generate(
            loaded.model, loaded.tokenizer, prompt=prompt, max_tokens=max_tokens, temp=temp, **kwargs
        )
        return response
