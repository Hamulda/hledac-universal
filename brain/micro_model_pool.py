...
class UmaFragmentationMonitor:
    ...
    """
    __slots__ = ('_enabled', '_fragmentation_score', '_snapshots')
    ...
    def snapshot(self, label: str) -> dict[str, Any]:
        ...
        """
        snapshot = {'label': label, 'timestamp': time.time(), 'active_memory_mb': self.get_active_memory() / (1024 * 1024), 'wired_memory_mb': self.get_wired_memory() / (1024 * 1024)}
    ...
    def get_report(self) -> dict[str, Any]:
        ...
        return {'fragmentation_score': score, 'status': self._get_status(score), 'snapshots': self._snapshots, 'recommendations': self._get_recommendations(score)}
...
class IMicroModelPool(Protocol):
    ...
    def get_model(self, model_id: str, use_warmup: bool=True) -> LoadedMicroModel | None:
        """Get a loaded model by ID, loading it if necessary."""
        ...

    def swap_to(self, target_model_id: str) -> tuple[ModelT, TokenizerT, bool] | tuple[None, None, False]:
...
class MicroModelPool:
    ...
    """
    __slots__ = ('_batch_preload_done', '_eviction_threshold', '_fully_preloaded', '_load_queue', '_loaded', '_loader_thread', '_lock', '_lru', '_main_model', '_main_model_lock', '_memory_budget', '_pending_loads', '_preload_all', '_total_loaded_bytes', '_uma_monitor', '_wired_models')

    def __init__(self, memory_budget_mb: int | None=None, eviction_threshold: float=0.9, preload_all: bool=True):
    ...
    def preload(self, model_ids: list[str] | None=None) -> None:
        ...
        self._loader_thread = threading.Thread(target=self._batch_preload, daemon=True, name='MicroModelBatchLoader')
    ...
    def _load_model_weights_only(self, model_id: str) -> LoadedMicroModel:
        ...
        start = time.time()
        try:
            model, tokenizer = mlx_lm.load(spec.full_path, tokenizer_mode='slow' if spec.task_type == TaskType.EMBEDDINGS else 'auto')
            loaded = LoadedMicroModel(spec=spec, model=model, tokenizer=tokenizer, is_warmed=False, is_wired=False, load_time=time.time() - start)
            with self._lock:
    ...
    def load_model(self, model_id: str, warmup: bool=True, wire_memory: bool=True) -> LoadedMicroModel:
        ...
        try:
            ...
            loaded = LoadedMicroModel(spec=spec, model=model, tokenizer=tokenizer, is_warmed=False, is_wired=wire_memory, load_time=time.time() - start)
            if warmup:
                try:
                    self._warmup_model(loaded)
                except Exception:
    ...
    def _warmup_model(self, loaded: LoadedMicroModel) -> None:
        ...
        try:
            if loaded.spec.task_type == TaskType.EMBEDDINGS:
                ...
                mlx_lm.generate(loaded.model, loaded.tokenizer, prompt=warmup_text, max_tokens=3, temp=0.1)
    ...
    def generate(self, model_id: str, prompt: str, max_tokens: int=256, temp: float=0.7, **kwargs) -> str:
        ...
        response = mlx_lm.generate(loaded.model, loaded.tokenizer, prompt=prompt, max_tokens=max_tokens, temp=temp, **kwargs)
        return response

    def _generate_embeddings(self, loaded: LoadedMicroModel, text: str, pool_type: str='mean', normalize: bool=True, **kwargs) -> str:
        ...
        try:
            ...
            return _json_dumps(embeddings.tolist())
        except Exception as e:
    ...
    def get_stats(self) -> dict[str, Any]:
        """Get pool statistics for monitoring."""
        with self._lock:
            return {'memory_budget_mb': self._memory_budget / (1024 * 1024), 'memory_used_mb': self._total_loaded_bytes / (1024 * 1024), 'memory_pressure': self.memory_pressure, 'loaded_count': len(self._loaded), 'wired_count': len(self._wired_models), 'fully_preloaded': self._fully_preloaded, 'swap_type': 'pointer_swap', 'loaded_models': [{'id': mid, 'spec': lm.spec.name, 'memory_mb': lm.spec.memory_mb, 'use_count': lm.use_count, 'last_used': lm.last_used, 'is_warmed': lm.is_warmed, 'is_wired': lm.is_wired} for mid, lm in self._loaded.items()], 'uma_fragmentation': self._uma_monitor.get_report()}
__all__ = ['MicroModelPool', 'MicroModelSpec', 'LoadedMicroModel', 'TaskType', 'IMicroModelPool', 'MICRO_MODELS', 'UmaFragmentationMonitor', 'get_uma_monitor']