...
def _heuristic_expand_concept(query: str) -> list[SyntheticDomainCandidate]:
    ...
    for n in (3, 2, 1):  # prefer longer n-grams
        for window in zip(*[words[i:] for i in range(n)], strict=False):
...
async def expand_concept_domains(
    ...
    _engine_loaded = hermes_engine is not None and hasattr(hermes_engine, "_model") and hermes_engine._model is not None
