...
_RE_AWS_SECRET_KEY = re.compile(
    "\\b(?:aws)?_?secret_?access?_?key\\s*[=:]\\s*[\\'\"]?([A-Za-z0-9/+=]{40})[\\'\"]?", re.IGNORECASE
)
...
_RE_GENERIC_API_KEY = re.compile(
    "\\b(?:api[_-]?key|apikey|api_secret)\\s*[=:]\\s*[\\'\"]?([A-Za-z0-9_\\-]{20,64})[\\'\"]?", re.IGNORECASE
)
_RE_PRIVATE_KEY = re.compile("-----BEGIN (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----", re.IGNORECASE)  # noqa: security/hardcoded-secret
...
_PATTERNS: list[tuple[str, re.Pattern, str]] = [
    ...
    ("private_key", _RE_PRIVATE_KEY, "-----BEGIN PRIVATE KEY-----"),  # noqa: security/hardcoded-secret
...
class OutputDLPFilter:
    ...
    def __init__(
        self, *, mask_emails: bool = False, mask_all_ips: bool = False, mask_internal_paths: bool = True
    ) -> None:
    ...
    def scan(self, text: str) -> list[dict[str, Any]]:
        ...
        if self.mask_emails:
            for match in _RE_EMAIL.finditer(text):
                findings.append(
                    {"pattern": "email", "match": match.group(), "start": match.start(), "end": match.end()}
                )
        return findings
