...
class ModernBertEngine:
    ...
    async def score_pivots_by_similarity(self, pivot_candidates: list[dict], finding_texts: list[str], top_k: int=10) -> list[tuple[dict, float]]:
        """
        ...
        try:
            ...
            if finding_emb.shape[0] == 0:
                return []
            finding_centroid = finding_emb.mean(axis=0, keepdims=True)
            f_norm = finding_centroid / (np.linalg.norm(finding_centroid, axis=1, keepdims=True) + 1e-08)
            pivot_texts = []
            ...
            if not pivot_texts:
                return []
            pivot_emb = self._embed_sync(pivot_texts)
            if pivot_emb.shape[0] == 0:
                return []
            p_norm = pivot_emb / (np.linalg.norm(pivot_emb, axis=1, keepdims=True) + 1e-08)
            sims = (p_norm @ f_norm.T).flatten()
