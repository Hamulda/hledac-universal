...
_ANE_MAX_MODELS = 2
_ANE_MAX_BATCH_SIZE = 16
_ANE_MAX_DIM = 1024
_ANE_MODEL_FOOTPRINT_MB = 50
_ANE_COMPILE_TIMEOUT_S = 120.0
_MODEL_CONFIGS: dict[str, dict[str, Any]] = {'bge-small': {'hf_id': 'BAAI/bge-small-en-v1.5', 'dim': 384, 'max_seq_len': 512, 'description': 'BGE-small-en-v1.5 — 384d, fast ANE small-batch'}, 'modernbert-embed': {'hf_id': 'nomic-ai/modernbert-embed-base', 'dim': 768, 'max_seq_len': 512, 'description': 'ModernBERT-embed-base — 768d, ANE-efficient for ≤16 batch'}, 'all-minilm-l6-v2': {'hf_id': 'sentence-transformers/all-MiniLM-L6-v2', 'dim': 384, 'max_seq_len': 256, 'description': 'all-MiniLM-L6-v2 — 384d, legacy ANE model (pre-compiled)'}}
...
class _ModelCache:
    ...
    __slots__ = ('_access_order', '_models')
    ...
    def put(self, key: str, path: Path, dim: int, max_seq_len: int) -> _CachedModel:
        ...
        model = _CachedModel(key=key, path=path, dim=dim, max_seq_len=max_seq_len, compiled_at=time_module.monotonic())
...
def _compile_hf_model_to_coreml(hf_id: str, output_path: Path, dim: int, max_seq_len: int) -> bool:
    ...
    try:
        ...
        inputs = tokenizer(dummy_text, return_tensors='pt', padding=True, truncation=True, max_length=max_seq_len)
        with torch.no_grad():
            traced = torch.jit.trace(model, (inputs['input_ids'], inputs['attention_mask']))
        input_shape = ct.Shape((1, ct.RangeDim(1, max_seq_len)))
        mlmodel = ct.convert(traced, inputs=[ct.TensorType(name='input_ids', shape=input_shape, dtype=np.int32), ct.TensorType(name='attention_mask', shape=input_shape, dtype=np.int32)], outputs=[ct.TensorType(name='last_hidden_state', dtype=np.float32)], compute_units=ct.ComputeUnit.NEURAL_ENGINE, minimum_deployment_target=ct.target.iOS18)
...
def _compile_mlx_model_to_coreml(mlx_model: Any, output_path: Path, dim: int, max_seq_len: int) -> bool:
    ...
    try:
        ...
        logger.warning('[ANE:compile-mlx] Full MIL builder not yet implemented — use _compile_hf_model_to_coreml() for now')
        return False
    except Exception as e:
...
class ANEInferenceEngine:
    ...
    Usage:
        ...
        embeddings = await engine.embed_batch_ane(texts, model_key="bge-small")
    """
    __slots__ = ('_cache', '_loaded_models', '_model_metadata', '_compile_lock', '_compiling')
    ...
    async def ensure_loaded(self, model_key: str='bge-small') -> bool:
        ...
        try:
            ...
            if compiled_path.exists():
                ...
                if mlmodel is not None:
                    self._loaded_models[model_key] = mlmodel
                    meta = self._cache.put(model_key, compiled_path, config['dim'], config['max_seq_len'])
            ...
            async with self._get_compile_lock():
                ...
                success = await asyncio.to_thread(_compile_hf_model_to_coreml, config['hf_id'], compiled_path, config['dim'], config['max_seq_len'])
                if not success:
    ...
    async def embed_batch_ane(self, texts: list[str], model_key: str='bge-small', *, normalize: bool=True) -> np.ndarray | None:
        ...
        try:
            ...
            def _run_ane_inference() -> np.ndarray:
                ...
                for text in texts:
                    inputs = tokenizer(text[:2048], return_tensors='np', padding='max_length', truncation=True, max_length=meta.max_seq_len)
                    ...
                    hidden = np.squeeze(hidden, axis=0)
                    mask = np.squeeze(attention_mask, axis=0)
                ...
                if normalize:
                    ...
                    result_array = result_array / (norms + 1e-09)
                return result_array
    ...
    async def _get_tokenizer(self, hf_id: str) -> Any | None:
        ...
        try:
            ...
            tokenizer = await asyncio.to_thread(AutoTokenizer.from_pretrained, hf_id)
    ...
    def get_model_info(self, model_key: str) -> dict[str, Any] | None:
        ...
        cached = self._cache.get(model_key)
        if cached is not None:
            return {'key': cached.key, 'dim': cached.dim, 'max_seq_len': cached.max_seq_len, 'path': str(cached.path), 'loaded': model_key in self._loaded_models}
        return None
...
def unload_ane_engine() -> None:
    ...
    if _ANE_ENGINE is not None:
        _ANE_ENGINE.unload()
        _ANE_ENGINE = None
__all__ = ['ANEInferenceEngine', 'get_ane_engine', 'unload_ane_engine', 'is_ane_available', '_ANE_MAX_BATCH_SIZE', '_ANE_MAX_DIM']