...
def _compile_mlx_model_to_coreml(mlx_model: Any, output_path: Path, dim: int, max_seq_len: int) -> bool:
    ...
    try:
        ...
        if not params:
            ...
            return False
        logger.warning(
            "[ANE:compile-mlx] Full MIL builder not yet implemented — use _compile_hf_model_to_coreml() for now"
        )
        return False
    except Exception as e:
...
class ANEInferenceEngine:
        ...
        try:
            ...
            async with self._get_compile_lock():
                ...
                success = await asyncio.to_thread(
                    _compile_hf_model_to_coreml, config["hf_id"], compiled_path, config["dim"], config["max_seq_len"]
                )
                if not success:
    ...
    async def embed_batch_ane(
        self, texts: list[str], model_key: str = "bge-small", *, normalize: bool = True
    ) -> np.ndarray | None:
