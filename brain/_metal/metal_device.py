...
class MetalDevice:
    ...
    _TIER_THRESHOLDS: dict[str, tuple[int, int]] = field(default_factory=lambda: {'low': (256 * 1024 ** 2, 512 * 1024 ** 2), 'medium': (512 * 1024 ** 2, 1024 * 1024 ** 2), 'high': (1024 * 1024 ** 2, 2 * 1024 ** 3), 'critical': (2 * 1024 ** 3, float('inf'))})
    ...
    def get_stats(self) -> MetalMemoryStats:
        ...
        return MetalMemoryStats(active_bytes=active, active_gb=round(active / 1024 ** 3, 3), peak_bytes=peak, peak_gb=round(peak / 1024 ** 3, 3), metal_tier=tier, pressure_level=self._get_pressure_level(active))
    ...
    def is_memory_available(self, required_bytes: int) -> bool:
        ...
        soft_ceiling = UmaBudget.MISSION_PEAK_RSS_GIB * 1024 ** 3
