...
class TestManifestGeneration:
    ...
    def test_class_structure(self, manifest: dict[str, Any]):
    ...
    def test_function_structure(self, manifest: dict[str, Any]):
        """Each function should have required fields."""
        for full_name, func_data in manifest.get("functions", {}).items():
            assert "name" in func_data
            assert "module" in func_data
            assert "signature" in func_data
            assert "return_type" in func_data

    def test_field_has_rust_and_python_types(self, manifest: dict[str, Any]):
...
class TestPyiGeneration:
    ...
    def test_pyi_has_slots(self, pyi_content: str, manifest: dict[str, Any]):
    ...
    def test_pyi_field_definitions(self, pyi_content: str, manifest: dict[str, Any]):
        ...
        for full_name, class_data in manifest.get("classes", {}).items():
            ...
            for field_data in class_data.get("fields", []):
                if field_data.get("has_get"):
                    ...
                    assert f"    {field_name}:" in pyi_content or \
                           f"{field_name}:" in pyi_content, \
...
class TestPyClassSlots:
    ...
    def test_all_get_fields_are_slots(self, manifest: dict[str, Any]):
        ...
        for full_name, class_data in manifest.get("classes", {}).items():
            ...
            assert get_fields.issubset(slot_fields), \
                f"{full_name}: get fields {get_fields - slot_fields} not in slots"

    def test_slots_are_get_fields(self, manifest: dict[str, Any]):
        """Slots should only contain fields that have get."""
        for full_name, class_data in manifest.get("classes", {}).items():
            slots = set(class_data.get("slots", []))
            all_fields = {f["name"] for f in class_data["fields"]}
            assert slots.issubset(all_fields), \
                f"{full_name}: slots {slots - all_fields} not in field definitions"

    def test_pyi_slots_match_manifest(self, pyi_content: str, manifest: dict[str, Any]):
...
class TestCLI:
    ...
    def test_validator_runs(self, repo_root: Path):
        """Validator should run without errors."""
        result = subprocess.run(
            [sys.executable, "stub_validator.py"],
            cwd=repo_root,
            capture_output=True,
            text=True
        )
    ...
    def test_validator_with_strict_mode(self, repo_root: Path):
        """Validator should accept --ci flag."""
        result = subprocess.run(
            [sys.executable, "stub_validator.py", "--ci"],
            cwd=repo_root,
            capture_output=True,
            text=True
        )
...
class TestBuildIntegration:
    ...
    def test_ffi_manifest_generated_at_build_time(self, repo_root: Path):
    ...
    def test_manifest_regeneration_on_rust_change(self, repo_root: Path):
...
class TestValidationRules:
    ...
    def test_validation_rules_present(self, manifest: dict[str, Any]):
...
class TestTypeMappingCompleteness:
    ...
    def test_common_types_mapped(self):
        ...
        for rust_type in required_types:
            assert rust_type in RUST_TO_PYTHON_TYPE, \
                f"Missing type mapping for {rust_type}"

    def test_type_mapping_function(self):
        """rust_type_to_python should handle all mapped types."""
        from ffi_type_manifest import rust_type_to_python, RUST_TO_PYTHON_TYPE
