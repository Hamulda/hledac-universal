...
def batch_dumps(
    ...
    if _rust_available and _rust_module is not None:
        ...
        try:
            if sort_keys:
                return [
                    _rust_module.serde_json_pretty_sorted(s) if pretty 
                    else _rust_module.serde_json_compact(s)
                    for s in json_strs
                ]
            else:
                return [
                    _rust_module.serde_json_pretty(s) if pretty
                    else _rust_module.serde_json_compact(s)
                    for s in json_strs
                ]
        except Exception as e:
...
# Import uuid for STIX bundle ID generation
import uuid
