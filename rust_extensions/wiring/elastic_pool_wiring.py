...
def _get_elastic_rust() -> dict | None:
    ...
    try:
        ...
        required = ["resize_cpu_pool", "resize_io_pool", "init_elastic_pools", "get_cpu_threads", "get_io_threads"]
...
class ElasticPoolWired:
    ...
    def get_all_pool_threads(self) -> int:
        ...
        try:
            ...
            return (
                self.get_cpu_pool_threads()
                + self.get_io_pool_threads()
                + self.get_simd_pool_threads()
                + self.get_mlx_pool_threads()
    ...
    def adaptive_resize(self, memory_pressure: float) -> bool:
        ...
        if target != current:
            ...
            logger.info(
                f"[elastic_pool_wiring] Adaptive resize: cpu {current} -> {actual} (pressure={memory_pressure:.2f})"
            )
