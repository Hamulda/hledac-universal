...
fn peel_region(region: &str, depth: u8, max_depth: u8) -> Option<DecodedCandidate> {
    ...
    if best.layers == depth {
        let decoded_bytes = best.decoded.as_bytes();
...
pub fn register(m: &Bound<'_, pyo3::types::PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(decode_ioc_candidates, m)?)?;
    m.add_function(wrap_pyfunction!(batch_decode_ioc_candidates, m)?)?;
    m.add_function(wrap_pyfunction!(deobfuscate_telemetry, m)?)?;
    m.add_function(wrap_pyfunction!(deobfuscate_telemetry_reset, m)?)?;
