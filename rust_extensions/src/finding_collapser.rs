...
static STOP_WORDS: LazyLock<HashSet<&'static str>> = LazyLock::new(|| {
    ...
    [
        ...
        "or", "yet", "if", "because", "until", "while", "about", "against", "this", "that",
        "these", "those", "it", "its", "they", "them", "their", "what", "which", "who", "whom",
        "whose", "him", "her", "his", "we", "us", "our", "you", "your", "i", "me", "my", "mine",
        "am", "get", "got", "like", "back", "up", "down", "out", "over", "off", "any", "new",
        "first", "last", "see", "known", "known", "seen", "via", "per", "via",
    ]
    .into_iter()
    .for_each(|w| {
        set.insert(w);
...
fn collapse_findings_core(
    ...
    for f in findings {
        ...
        if entry.entity.is_empty() {
            ...
            entry.ioc_type = f.ioc_type.as_deref().unwrap_or("unknown").to_string();
        }
        // Collect source tuple
        let source_url = f.source_url.as_deref().unwrap_or_default().to_string();
        let conf = f.confidence.unwrap_or(0.0);
        let text = f.text.as_deref().unwrap_or_default().to_string();
    ...
    for g in &mut group_list {
        ...
        g.sources.retain(|(url, _, text)| {
            let key = format!("{}:{}", url, text.chars().take(40).collect::<String>());
...
pub fn collapse_findings(
    ...
    let _guard = _COLLAPSE_GLOBAL_LOCK.lock();
...
pub fn compress_prompt_core(text: &str, config: &CompressionConfig) -> String {
    if text.is_empty() {
        return String::new();
    }
...
mod tests {
    ...
    fn test_collapse_deterministic() {
        ...
        for _ in 0..99 {
            let (again, _) =
                collapse_findings_core(&serde_json::from_slice(&json).unwrap(), 12, 400);
            assert_eq!(
                first.len(),
                again.len(),
