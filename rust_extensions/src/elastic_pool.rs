...
fn build_simd_pool() -> Result<ThreadPool, String> {
    ...
    ThreadPoolBuilder::new()
        ...
        .spawn_fifo() // NEXTGEN-03: FIFO scheduling to minimize cross-pool stealing
        .spawn_handler(move |builder| {
...
pub fn register_functions(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(get_simd_pool_threads_py, m)?)?;
    m.add_function(wrap_pyfunction!(get_mlx_pool_threads_py, m)?)?;
    m.add_function(wrap_pyfunction!(get_graph_pool_threads_py, m)?)?;
    m.add_function(wrap_pyfunction!(get_all_pool_threads_py, m)?)?;
    Ok(())
}
