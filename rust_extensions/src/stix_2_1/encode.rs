...
pub fn encode_indicator(finding: &Map<String, Value>) -> Option<Value> {
    ...
    let source_type = finding.get("source_type").and_then(|v| v.as_str()).unwrap_or("web");
    let confidence = finding
        .get("confidence")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.5);
    let query = finding.get("query").and_then(|v| v.as_str()).unwrap_or("");
    let finding_id = finding.get("finding_id").and_then(|v| v.as_str()).unwrap_or("");
    ...
    indicator.insert("name".to_string(), Value::String(format!(
        "{} indicator: {}",
        source_type, query
    )));
    indicator.insert("description".to_string(), Value::String(format!(
        "OSINT indicator extracted from {} source. Query: {}",
        source_type, query
    )));
    indicator.insert("pattern".to_string(), Value::String(pattern));
    indicator.insert("pattern_type".to_string(), Value::String("stix".to_string()));
    ...
    indicator.insert("labels".to_string(), Value::Array(vec![
    ...
    indicator.insert(
        ...
        Value::Array(vec![Value::String("marking-definition--613f2e26-407d-48f7-9f50-60798f4e9e5e".to_string())]),
    );
...
fn build_sco(stix_type: &str, ioc_value: &str, _ioc_type: &str) -> Option<Value> {
    ...
    match stix_type {
        "file-hash" => {
            ...
            hashes.insert(
                algorithm.to_string(),
                Value::String(hash_value.to_string()),
            );
...
fn build_cybox_pattern(ioc_type: &str, ioc_value: &str) -> Option<String> {
    ...
    match ioc_type.to_lowercase().as_str() {
        ...
        "ipv4" | "ipv4-addr" => Some(format!(
            "ipv4-addr:value = '{encoded_value}'"
        )),
        "ipv6" | "ipv6-addr" => Some(format!(
            "ipv6-addr:value = '{encoded_value}'"
        )),
        "domain" | "domain-name" => Some(format!(
            "domain-name:value = '{encoded_value}'"
        )),
        "email" | "email-addr" => Some(format!(
            "email-addr:value = '{encoded_value}'"
        )),
        "md5" => Some(format!(
            "file-hash:hashes.MD5 = '{encoded_value}'"
        )),
        "sha1" => Some(format!(
            "file-hash:hashes.'SHA-1' = '{encoded_value}'"
        )),
        "sha256" => Some(format!(
            "file-hash:hashes.'SHA-256' = '{encoded_value}'"
        )),
        "sha512" => Some(format!(
            "file-hash:hashes.'SHA-512' = '{encoded_value}'"
        )),
        "mutex" => Some(format!(
            "mutex:name = '{encoded_value}'"
        )),
        "registry" => Some(format!(
            "registry-key:key = '{encoded_value}'"
        )),
        "cve" => Some(format!(
            "vulnerability:cve = '{encoded_value}'"
        )),
        _ => None,
    }
}
...
pub fn encode_finding(finding_py: &PyAny, py: Python<'_>) -> PyResult<Py<PyBytes>> {
    ...
    // Encode as note
    let query = finding.get("query").and_then(|v| v.as_str()).unwrap_or("");
    let payload = finding.get("payload_text").and_then(|v| v.as_str()).unwrap_or("");
    let finding_id = finding.get("finding_id").and_then(|v| v.as_str()).unwrap_or("");
...
pub fn encode_findings_batch(findings_py: &PyAny, py: Python<'_>) -> PyResult<Py<PyBytes>> {
    ...
    let findings: Vec<Map<String, Value>> = match serde_json::from_str::<Vec<Map<String, Value>>>(&list_str) {
        Ok(v) => v,
        _ => return Ok(PyBytes::new(py, &[]).into()),
    };
...
fn iso8601_timestamp() -> String {
    ...
    let y_str = if y < 10 { format!("000{y}") }
        else if y < 100 { format!("00{y}") }
        else if y < 1000 { format!("0{y}") }
        else { y.to_string() };
    let m_str = if m < 10 { format!("0{m}") } else { m.to_string() };
    let d_str = if d < 10 { format!("0{d}") } else { d.to_string() };
    let h_str = if h < 10 { format!("0{h}") } else { h.to_string() };
    let mn_str = if mn < 10 { format!("0{mn}") } else { mn.to_string() };
    let s_str = if s < 10 { format!("0{s}") } else { s.to_string() };
...
fn future_timestamp(days: i64) -> String {
    ...
    let y_str = if year < 10 { format!("000{year}") }
        else if year < 100 { format!("00{year}") }
        else if year < 1000 { format!("0{year}") }
        else { year.to_string() };
    let m_str = if month < 10 { format!("0{month}") } else { month.to_string() };
    let d_str = if day < 10 { format!("0{day}") } else { day.to_string() };
    let h_str = if hour < 10 { format!("0{hour}") } else { hour.to_string() };
    let mn_str = if minute < 10 { format!("0{minute}") } else { minute.to_string() };
    let s_str = if second < 10 { format!("0{second}") } else { second.to_string() };
...
mod tests {
    ...
    fn test_cybox_pattern_sha256() {
        let pattern = build_cybox_pattern("sha256", "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855").unwrap();
    ...
    fn test_encode_indicator() {
        ...
        finding.insert("ioc_value".to_string(), Value::String("https://evil.com".to_string()));
        finding.insert("source_type".to_string(), Value::String("web".to_string()));
        finding.insert("confidence".to_string(), Value::Number(serde_json::Number::from_f64(0.85).unwrap()));
        finding.insert("query".to_string(), Value::String("test query".to_string()));
        finding.insert("finding_id".to_string(), Value::String("finding-123".to_string()));
