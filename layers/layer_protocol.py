...
class Layer(Protocol):
    ...
    async def on_event(self, ctx: LayerContext, event: LayerEvent) -> LayerEvent | None:
        """
...
class LayerStack:
    ...
    async def on_event(self, ctx: LayerContext, event: LayerEvent) -> LayerEvent:
        ...
        for layer in self._layers:
            ...
            name = getattr(layer, 'layer_name', repr(layer))
            try:
                result = await safe_wait_for(layer.on_event(ctx, event), timeout=30.0, label=f'on_event:{name}')
                if result is None:
                    event.halt()
            except Exception as e:
...
async def create_uds_server(path: str, handler: Callable[[Any], Coroutine[Any, Any, None]], *, backlog: int=16) -> asyncio.AbstractServer:
    """
