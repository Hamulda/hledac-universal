...
class _KeyState(msgspec.Struct, gc=False):
    ...
    ring_gaps: deque[float] = field(default_factory=lambda: deque(maxlen=DEFAULT_RING_SIZE))
    ring_sources: deque[str] = field(default_factory=lambda: deque(maxlen=DEFAULT_RING_SIZE))
...
class TemporalSignalLayer:
    ...
    """
    __slots__ = tuple(('_bocpd_max_run', '_edge_candidates', '_half_life_s', '_lru_order', '_max_keys', '_ring_size', '_states', '_sync_window', '_synchrony_window_s'))

    def __init__(self, max_keys: int=DEFAULT_MAX_KEYS, ring_size: int=DEFAULT_RING_SIZE, half_life_s: float=DEFAULT_HALF_LIFE_S, synchrony_window_s: float=DEFAULT_SYNCHRONY_WINDOW_S, bocpd_max_run: int=DEFAULT_BOCPD_MAX_RUN):
    ...
    def observe(self, event: TemporalEvent) -> TemporalScore:
        ...
        if key not in self._states:
            ...
            self._states[key] = _KeyState(ring_gaps=deque(maxlen=self._ring_size), ring_sources=deque(maxlen=self._ring_size))
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        for k, v in self._states.items():
            ls = v.last_score
            states_serializable[k] = {'last_ts': v.last_ts, 'event_count': v.event_count, 'ewma_rate': v.ewma_rate, 'ewma_gap': v.ewma_gap, 'gap_variance': v.gap_variance, 'ring_gaps': list(v.ring_gaps), 'ring_sources': list(v.ring_sources), 'confirmation_weight': v.confirmation_weight, 'last_updated': v.last_updated, 'ph_cumsum': v.ph_cumsum, 'ph_mean': v.ph_mean, 'bocpd_run_length': v.bocpd_run_length, 'bocpd_log_odds': v.bocpd_log_odds, 'last_score': {'key': ls.key, 'family': ls.family, 'event_count': ls.event_count, 'anomaly_score': ls.anomaly_score, 'burst_score': ls.burst_score, 'periodicity_score': ls.periodicity_score, 'change_point_score': ls.change_point_score, 'source_synchrony_score': ls.source_synchrony_score, 'rate_score': ls.rate_score, 'cv_isi': ls.cv_isi, 'mean_gap_s': ls.mean_gap_s, 'autocorr_lag1': ls.autocorr_lag1, 'reason': ls.reason} if ls else None}
        edge_candidates_serializable = [{'src_key': c.src_key, 'dst_key': c.dst_key, 'edge_type': c.edge_type, 'score': c.score, 'window_start': c.window_start, 'window_end': c.window_end, 'reason': c.reason} for c in self._edge_candidates]
        return {'max_keys': self._max_keys, 'ring_size': self._ring_size, 'half_life_s': self._half_life_s, 'synchrony_window_s': self._synchrony_window_s, 'bocpd_max_run': self._bocpd_max_run, 'states': states_serializable, 'lru_order': self._lru_order, 'edge_candidates': edge_candidates_serializable, 'sync_window': [(w_ts, src, tuple(keys)) for w_ts, src, keys in self._sync_window]}

    @classmethod
    def from_snapshot(cls, snapshot: dict[str, Any]) -> TemporalSignalLayer:
        layer = cls(max_keys=snapshot.get('max_keys', DEFAULT_MAX_KEYS), ring_size=snapshot.get('ring_size', DEFAULT_RING_SIZE), half_life_s=snapshot.get('half_life_s', DEFAULT_HALF_LIFE_S), synchrony_window_s=snapshot.get('synchrony_window_s', DEFAULT_SYNCHRONY_WINDOW_S), bocpd_max_run=snapshot.get('bocpd_max_run', DEFAULT_BOCPD_MAX_RUN))
        for key, state_data in snapshot.get('states', {}).items():
            state = _KeyState(last_ts=state_data['last_ts'], event_count=state_data['event_count'], ewma_rate=state_data['ewma_rate'], ewma_gap=state_data['ewma_gap'], gap_variance=state_data['gap_variance'], ring_gaps=deque(state_data['ring_gaps'], maxlen=layer._ring_size), ring_sources=deque(state_data['ring_sources'], maxlen=layer._ring_size), confirmation_weight=state_data['confirmation_weight'], last_updated=state_data['last_updated'], ph_cumsum=state_data.get('ph_cumsum', 0.0), ph_mean=state_data.get('ph_mean', 0.0), bocpd_run_length=state_data.get('bocpd_run_length', 0), bocpd_log_odds=state_data.get('bocpd_log_odds', 0.0))
            ls_data = state_data.get('last_score')
            if ls_data is not None:
                state.last_score = TemporalScore(key=ls_data['key'], family=ls_data.get('family', ''), event_count=ls_data['event_count'], anomaly_score=ls_data['anomaly_score'], burst_score=ls_data['burst_score'], periodicity_score=ls_data['periodicity_score'], change_point_score=ls_data['change_point_score'], source_synchrony_score=ls_data['source_synchrony_score'], rate_score=ls_data['rate_score'], cv_isi=ls_data['cv_isi'], mean_gap_s=ls_data['mean_gap_s'], autocorr_lag1=ls_data['autocorr_lag1'], reason=ls_data.get('reason', 'restored'))
            elif state.event_count > 0:
                state.last_score = TemporalScore(key=key, family='', event_count=state.event_count, anomaly_score=0.0, burst_score=0.0, periodicity_score=0.0, change_point_score=0.0, source_synchrony_score=0.0, rate_score=0.0, cv_isi=0.0, mean_gap_s=state.ewma_gap, autocorr_lag1=0.0, reason='restored')
            layer._states[key] = state
        layer._lru_order = deque(snapshot.get('lru_order', []))
        layer._edge_candidates = deque([TemporalEdgeCandidate(src_key=c['src_key'], dst_key=c['dst_key'], edge_type=c['edge_type'], score=c['score'], window_start=c['window_start'], window_end=c['window_end'], reason=c['reason']) for c in snapshot.get('edge_candidates', [])], maxlen=256)
        layer._sync_window = deque([(w_ts, src, frozenset(keys)) for w_ts, src, keys in snapshot.get('sync_window', [])], maxlen=256)
        return layer
    ...
    def _update_edge_candidates(self, state: _KeyState, ts: float, burst_score: float, source_synchrony_score: float) -> None:
        if burst_score > 0.6:
            for other_key, other_state in self._states.items():
                ...
                if other_state.last_score and other_state.last_score.burst_score > 0.4:
                    if not state.last_score:
                        continue
                    window_start = ts - self._synchrony_window_s
                    candidate = TemporalEdgeCandidate(src_key=state.last_score.key, dst_key=other_key, edge_type='co_burst', score=(burst_score + other_state.last_score.burst_score) / 2.0, window_start=window_start, window_end=ts, reason='co_burst')
            ...
            if len(recent_keys) >= 2:
                ...
                for i in range(min(len(recent_list), 4)):
                    for j in range(i + 1, min(len(recent_list), 4)):
                        candidate = TemporalEdgeCandidate(src_key=recent_list[i], dst_key=recent_list[j], edge_type='source_synchrony', score=source_synchrony_score, window_start=window_start, window_end=ts, reason='source_synchrony')
                        self._edge_candidates.append(candidate)
...
def event_from_finding_like(obj: Any) -> TemporalEvent | None:
    ...
    try:
        ...
        return TemporalEvent(ts=float(ts), key=str(key), family=str(family), source=str(source), weight=float(weight), labels=labels)
