"""
Layer Protocol - Canonical Interface for All Layers
================================================

Design principles:
- Each layer is a Protocol (PEP 544): mount(ctx), unmount(ctx), process(data)
- LayerRegistry manages ordered execution
- No global state — LayerRegistry is injected where needed
...
class Layer(Protocol):
    ...
    Lifecycle:
        ...
        process() → process data through layer
        rollback() → on failure (optional)

    M1 8GB: All heavy imports are lazy-loaded in __init__ or methods.
    """

    # Canonical layer name for registry lookup
    layer_name: str

    async def mount(self, ctx: LayerContext) -> None:
        """Mount layer — called when layer is added to registry."""
        ...

    async def unmount(self, ctx: LayerContext) -> None:
        """Unmount layer — called when layer is removed or registry shuts down."""
        ...

    async def process(self, ctx: LayerContext, data: Any) -> Any:
        """
        Process data through layer.

        Args:
            ctx: Layer context with shared services
            data: Data to process

        Returns:
            Processed data (may be modified)
        """
        ...

    async def rollback(self, ctx: LayerContext, error: Exception) -> None:
        """Rollback on failure — default no-op."""
        ...
...
class LayerStack:
    ...
    async def process_all(self, ctx: LayerContext, data: Any) -> Any:
        """
        Process data through all layers in order.

        Each layer receives the data and can modify it.
        ...
        Returns:
            Processed data after all layers
        """
        result = data
        for layer in self._layers:
            name = getattr(layer, "layer_name", repr(layer))
