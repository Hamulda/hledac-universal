...
class TuneResult(Struct, frozen=True):
    ...
    def changed(self) -> bool:
        ...
        return (
            self.triggered
            and self.success
            and (self.new_partitions != self.old_partitions or self.new_num_sub_vectors != self.old_num_sub_vectors)
        )
...
class IVFPQAutoTuner:
    ...
    def _load_state(self) -> TuneState:
        ...
        try:
            ...
            return TuneState(
                ...
                last_num_partitions=int(data.get("last_num_partitions", DEFAULT_NUM_PARTITIONS)),
                ...
                last_num_sub_vectors=int(data.get("last_num_sub_vectors", DEFAULT_NUM_SUB_VECTORS)),
    ...
    def measure_recall(
        self, table: _TableLike, *, sample_size: int = DEFAULT_RECALL_SAMPLES, k: int = RECALL_TOP_K
    ) -> tuple[float, float]:
        ...
        for s_idx, q_idx in enumerate(sample_idx):
            ...
            try:
                ann_rows = (
                    table.search(q_list, vector_column_name=self._vector_column).metric("cosine").limit(k_eff).to_list()
                )
            except Exception as e:
    ...
    def compute_optimal_partitions(
        self, *, current: int, recall: float, avg_search_ms: float, row_count: int, prev_recall_ema: float = 0.0
    ) -> int:
    ...
    def compute_optimal_sub_vectors(
        self, *, current: int, recall: float, avg_search_ms: float, embedding_dim: int
    ) -> int:
        ...
        if recall < self._SUB_VEC_TOO_FEW:
            ...
            recall >= self._SUB_VEC_EXCELLENT
    ...
    def retrain(self, table: _TableLike, *, new_num_partitions: int, num_sub_vectors: int | None = None) -> bool:
        ...
        n_sub = max(
            MIN_NUM_SUB_VECTORS,
            min(MAX_NUM_SUB_VECTORS, int(num_sub_vectors) if num_sub_vectors is not None else self._num_sub_vectors),
        )
        try:
            ...
            try:
                ...
                table.create_index(**index_kwargs)
            logger.info(
                f"[LANCEDB-AUTOTUNE] retrained table={self._table_name} num_partitions={n_part} num_sub_vectors={n_sub} max_iterations={M1_MAX_ITERATIONS}"
            )
            try:
    ...
    def tune_if_due(
        ...
        *,
        ...
        cur_sub = (
            int(current_num_sub_vectors) if current_num_sub_vectors is not None else self._state.last_num_sub_vectors
        )
        ...
        new_sub_vec = self.compute_optimal_sub_vectors(
            current=cur_sub, recall=recall, avg_search_ms=avg_ms, embedding_dim=self._embedding_dim
        )
        success = True
        err: str | None = None
        ...
        if partitions_changed or sub_vec_changed:
            ...
            logger.info(
                f"[LANCEDB-AUTOTUNE] measured table={self._table_name} recall={recall:.3f} recall_ema={recall_ema:.3f} avg_ms={avg_ms:.2f} partitions={current_num_partitions} sub_vec={cur_sub} rows={row_count} (no change)"
            )
        new_state = TuneState(
    ...
    async def tune_if_due_async(
        ...
        *,
        ...
        try:
            result = await offload_to(
                "cpu_blocking_pool",
                self.tune_if_due,
                table,
                current_num_partitions,
                current_num_sub_vectors,
                inserts_delta,
            )
            return result
        except RuntimeError:
