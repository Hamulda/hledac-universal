...
class KnowledgeGraphBuilder:
    ...
    def _init_patterns(self) -> dict[str, list[re.Pattern]]:
        ...
        return {
            "is_a": [
                re.compile("(\\w+(?:\\s+\\w+)*)\\s+is\\s+(?:a|an)\\s+(\\w+(?:\\s+\\w+)*)", re.IGNORECASE),
                re.compile("(\\w+(?:\\s+\\w+)*)\\s+are\\s+(\\w+(?:\\s+\\w+)*)", re.IGNORECASE),
            ],
            "causes": [
                re.compile("(\\w+(?:\\s+\\w+)*)\\s+causes?\\s+(\\w+(?:\\s+\\w+)*)", re.IGNORECASE),
                re.compile("(\\w+(?:\\s+\\w+)*)\\s+lead(?:s)?\\s+to\\s+(\\w+(?:\\s+\\w+)*)", re.IGNORECASE),
            ],
            "located_in": [
                re.compile("(\\w+(?:\\s+\\w+)*)\\s+is\\s+located\\s+in\\s+(\\w+(?:\\s+\\w+)*)", re.IGNORECASE),
                re.compile("(\\w+(?:\\s+\\w+)*)\\s+is\\s+situated\\s+in\\s+(\\w+(?:\\s+\\w+)*)", re.IGNORECASE),
            ],
            "part_of": [
                re.compile("(\\w+(?:\\s+\\w+)*)\\s+is\\s+part\\s+of\\s+(\\w+(?:\\s+\\w+)*)", re.IGNORECASE),
                re.compile("(\\w+(?:\\s+\\w+)*)\\s+belongs\\s+to\\s+(\\w+(?:\\s+\\w+)*)", re.IGNORECASE),
            ],
            "contains": [
                re.compile("(\\w+(?:\\s+\\w+)*)\\s+contains?\\s+(\\w+(?:\\s+\\w+)*)", re.IGNORECASE),
                re.compile("(\\w+(?:\\s+\\w+)*)\\s+includes?\\s+(\\w+(?:\\s+\\w+)*)", re.IGNORECASE),
            ],
        }
    ...
    def process_and_store(self, content: str, metadata: dict[str, Any], knowledge_layer) -> list[str]:
        ...
        for fact in facts:
            ...
            knowledge_layer.add_relation(
                ...
                target_id=target_id,
                edge_type=edge_type,
                metadata={"context": fact["context"], "extracted_from": metadata.get("url", "unknown")},
            )
        if "author" in metadata and "url" in metadata:
            ...
            author_node = knowledge_layer.add_knowledge(
                content=metadata["author"], node_type=NodeType.ENTITY, metadata={"type": "author"}
            )
            node_ids.append(author_node)
            url_node = knowledge_layer.add_knowledge(
                content=metadata["url"], node_type=NodeType.URL, metadata={"type": "source"}
            )
            node_ids.append(url_node)
            knowledge_layer.add_relation(
                source_id=author_id, target_id=url_id, edge_type=EdgeType.MENTIONS, metadata={"relation": "wrote"}
            )
