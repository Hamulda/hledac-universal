...
class KnowledgeGraphBuilder:
    ...
    def _init_patterns(self) -> dict[str, list[re.Pattern]]:
        ...
        """
        return {'is_a': [re.compile('(\\w+(?:\\s+\\w+)*)\\s+is\\s+(?:a|an)\\s+(\\w+(?:\\s+\\w+)*)', re.IGNORECASE), re.compile('(\\w+(?:\\s+\\w+)*)\\s+are\\s+(\\w+(?:\\s+\\w+)*)', re.IGNORECASE)], 'causes': [re.compile('(\\w+(?:\\s+\\w+)*)\\s+causes?\\s+(\\w+(?:\\s+\\w+)*)', re.IGNORECASE), re.compile('(\\w+(?:\\s+\\w+)*)\\s+lead(?:s)?\\s+to\\s+(\\w+(?:\\s+\\w+)*)', re.IGNORECASE)], 'located_in': [re.compile('(\\w+(?:\\s+\\w+)*)\\s+is\\s+located\\s+in\\s+(\\w+(?:\\s+\\w+)*)', re.IGNORECASE), re.compile('(\\w+(?:\\s+\\w+)*)\\s+is\\s+situated\\s+in\\s+(\\w+(?:\\s+\\w+)*)', re.IGNORECASE)], 'part_of': [re.compile('(\\w+(?:\\s+\\w+)*)\\s+is\\s+part\\s+of\\s+(\\w+(?:\\s+\\w+)*)', re.IGNORECASE), re.compile('(\\w+(?:\\s+\\w+)*)\\s+belongs\\s+to\\s+(\\w+(?:\\s+\\w+)*)', re.IGNORECASE)], 'contains': [re.compile('(\\w+(?:\\s+\\w+)*)\\s+contains?\\s+(\\w+(?:\\s+\\w+)*)', re.IGNORECASE), re.compile('(\\w+(?:\\s+\\w+)*)\\s+includes?\\s+(\\w+(?:\\s+\\w+)*)', re.IGNORECASE)]}
            ...
            for relation_type, patterns in self._patterns.items():
                for pattern in patterns:
                    ...
                    for match in matches:
                        if len(match) == 2:
                            source, target = match
                            facts.append({'source': source.strip(), 'target': target.strip(), 'relation': relation_type, 'context': sentence})
    ...
    def process_and_store(self, content: str, metadata: dict[str, Any], knowledge_layer) -> list[str]:
        """
        ...
        for fact in facts:
            ...
            source_node = knowledge_layer.add_knowledge(content=fact['source'], node_type=NodeType.ENTITY, metadata={'extracted_from': metadata.get('url', 'unknown')})
            node_ids.append(source_node)
            target_node = knowledge_layer.add_knowledge(content=fact['target'], node_type=NodeType.ENTITY, metadata={'extracted_from': metadata.get('url', 'unknown')})
            node_ids.append(target_node)
            edge_type_map = {'is_a': EdgeType.RELATED, 'causes': EdgeType.CAUSES, 'located_in': EdgeType.RELATED, 'part_of': EdgeType.PART_OF, 'contains': EdgeType.CONTAINS}
            edge_type = edge_type_map.get(fact['relation'], EdgeType.RELATED)
            knowledge_layer.add_relation(source_id=source_id, target_id=target_id, edge_type=edge_type, metadata={'context': fact['context'], 'extracted_from': metadata.get('url', 'unknown')})
        if 'author' in metadata and 'url' in metadata:
            ...
            author_node = knowledge_layer.add_knowledge(content=metadata['author'], node_type=NodeType.ENTITY, metadata={'type': 'author'})
            node_ids.append(author_node)
            url_node = knowledge_layer.add_knowledge(content=metadata['url'], node_type=NodeType.URL, metadata={'type': 'source'})
            node_ids.append(url_node)
            knowledge_layer.add_relation(source_id=author_id, target_id=url_id, edge_type=EdgeType.MENTIONS, metadata={'relation': 'wrote'})
        logger.info(f'Processed and stored {len(facts)} facts, {len(node_ids)} nodes')
        return node_ids

    def process_document(self, document: str, url: str, author: str | None=None, knowledge_layer=None) -> list[str]:
        """
        ...
        Returns:
            List of created node IDs
        """
        metadata = {'url': url, 'type': 'document'}
        if author:
