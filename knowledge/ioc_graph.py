...
from hledac.universal.brain.jtms import JTMS, apply_temporal_decay
...
IOC_TYPES: frozenset[str] = frozenset(
    (
        ...
        "malware_family",
        "email",
        "ssh_key",  # 3.1 FIX: git forensics IOCs
    )
)
_RE_IP_PUBLIC = re.compile(
    "\\b(?!10\\.|127\\.|169\\.254\\.|172\\.(?:1[6-9]|2\\d|3[01])\\.|192\\.168\\.)(?:\\d{1,3}\\.){3}\\d{1,3}\\b"
)
...
class IOCGraph:
    ...
    __slots__ = (
        ...
        "_jtms",
        "_decay_lambda",
        "_schema_has_temporal",
        "_schema_has_embeddings",
        "_schema_has_gnn_scores",
        "_OBS_BUFFER_FLUSH_SIZE",
        "_schema_has_multimedia",
        "_face_buffer",
        "_voice_buffer",
        "_FACE_BUFFER_FLUSH_SIZE",
        "_VOICE_BUFFER_FLUSH_SIZE",
    )

    def __init__(self, db_path: Path | None = None, decay_lambda: float = 0.01, *, memory_mode: bool = False) -> None:
    ...
    async def retract_source(self, source_id: str) -> dict[str, int]:
        ...
        logging.info(
            f"[IOCGraph] retract_source({source_id}): {facts_retracted} facts retracted, {iocs_updated} IOCs updated"
        )
    ...
    async def auto_retract_unreliable_sources(self) -> list[str]:
        ...
        for source in unreliable:
            try:
                ...
                logging.warning(
                    "[IOCGraph] auto_retract_unreliable_sources failed for %s: %s",
                    source.source_id,
                    e,
                )
    ...
    async def _recompute_ioc_confidences(self) -> int:
        ...
        for ioc_id, confidences in ioc_confidences.items():
            ...
            try:
                await asyncio.to_thread(self._update_ioc_confidence_sync, ioc_id, aggregated_conf, now)
                iocs_updated += 1
            except Exception as e:
                import logging
        ...
        if self._jtms._facts and self._decay_lambda > 0:
            ...
            for ioc_type, value, base_conf, obs_at in ioc_copy:
                ...
                if fact_data:
                    ...
                    decayed_conf = apply_temporal_decay(base_conf, justification.timestamp, self._decay_lambda, now)
    ...
    def _init_schema_sync(self) -> None:
        ...
        try:
            self._conn.execute(
                "CREATE REL TABLE OBSERVED(FROM IOC TO IOC, finding_id STRING, source_type STRING, first_seen DOUBLE, last_seen DOUBLE)"
            )
        except Exception:  # noqa: BLE001
            pass
        ...
        try:
            self._conn.execute("MATCH (n:IOC) RETURN n.embedding_table, n.embedding_row_id LIMIT 1")
        ...
        try:
            self._conn.execute("MATCH ()-[r:PREDICTED]->() RETURN r.gnn_score, r.combined_score LIMIT 1")
        ...
        if not self._schema_has_gnn_scores:
            ...
            try:
                self._conn.execute("ALTER (FROM)->(TO) ADD gnn_score DOUBLE DEFAULT 0.0")
                self._conn.execute("ALTER (FROM)->(TO) ADD combined_score DOUBLE DEFAULT 0.0")
                self._schema_has_gnn_scores = True
                ...
                # Try to verify again
                try:
                    self._conn.execute("MATCH ()-[r:PREDICTED]->() RETURN r.gnn_score LIMIT 1")
                    self._schema_has_gnn_scores = True
                except Exception:
                    self._schema_has_gnn_scores = False
    ...
    def _create_schema(self, conn) -> None:
        ...
        try:
            conn.execute(
                "CREATE NODE TABLE IOC(id STRING PRIMARY KEY, ioc_type STRING, value STRING, first_seen DOUBLE, last_seen DOUBLE, confidence DOUBLE, earliest_observed DOUBLE, latest_observed DOUBLE, observation_count INTEGER)"
            )
        except Exception:
            pass
        try:
        ...
        try:
            conn.execute(
                "CREATE REL TABLE PREDICTED(FROM IOC TO IOC, confidence DOUBLE, adamic_adar DOUBLE, jaccard DOUBLE, pref_attach DOUBLE, common_neighbors INTEGER, method STRING, created_at DOUBLE, verified BOOLEAN DEFAULT false)"
            )
        except Exception:
            pass
    ...
    def _create_multimedia_schema(self, conn) -> None:
        """
    ...
    def _copy_nodes(self, target_conn):
        ...
        if self._schema_has_temporal:
            res = self._conn.execute(
                "MATCH (n:IOC) RETURN n.id, n.ioc_type, n.value, n.first_seen, n.last_seen, n.confidence, n.earliest_observed, n.latest_observed, n.observation_count"
            )
            while res.has_next():
                row = res.get_next()
                target_conn.execute(
                    "CREATE (:IOC {id: $id, ioc_type: $t, value: $v, first_seen: $fs, last_seen: $ls, confidence: $c, earliest_observed: $eo, latest_observed: $lo, observation_count: $oc})",
                    {
                        "id": row[0],
                        "t": row[1],
                        "v": row[2],
            ...
            res = self._conn.execute(
                "MATCH (n:IOC) RETURN n.id, n.ioc_type, n.value, n.first_seen, n.last_seen, n.confidence"
            )
            while res.has_next():
                row = res.get_next()
                target_conn.execute(
                    "CREATE (:IOC {id: $id, ioc_type: $t, value: $v, first_seen: $fs, last_seen: $ls, confidence: $c, earliest_observed: $fs, latest_observed: $ls, observation_count: 1})",
                    {"id": row[0], "t": row[1], "v": row[2], "fs": row[3], "ls": row[4], "c": row[5]},
                )
                ioc_count += 1
    ...
    def _copy_observed_edges(self, target_conn):
        ...
        obs_count = 0
        res = self._conn.execute(
            "MATCH (a:IOC)-[r:OBSERVED]->(b:IOC) RETURN a.id, b.id, r.finding_id, r.source_type, r.first_seen, r.last_seen"
        )
        while res.has_next():
            row = res.get_next()
            target_conn.execute(
                "MATCH (a:IOC {id: $aid}), (b:IOC {id: $bid}) CREATE (a)-[:OBSERVED {finding_id: $fid, source_type: $st, first_seen: $fs, last_seen: $ls}]->(b)",
                {"aid": row[0], "bid": row[1], "fid": row[2], "st": row[3], "fs": row[4], "ls": row[5]},
            )
            obs_count += 1
    ...
    def _copy_predicted_edges(self, target_conn):
        ...
        try:
            res = self._conn.execute(
                "MATCH (a:IOC)-[r:PREDICTED]->(b:IOC) RETURN a.id, b.id, r.confidence, r.adamic_adar, r.jaccard, r.pref_attach, r.common_neighbors, r.method, r.created_at, r.verified"
            )
            while res.has_next():
                row = res.get_next()
                target_conn.execute(
                    "MATCH (a:IOC {id: $aid}), (b:IOC {id: $bid}) CREATE (a)-[:PREDICTED {confidence: $conf, adamic_adar: $aa, jaccard: $jac, pref_attach: $pa, common_neighbors: $cn, method: $method, created_at: $ts, verified: $ver}]->(b)",
                    {
                        "aid": row[0],
                        "bid": row[1],
                        "conf": row[2],
    ...
    async def persist_to_disk(self, target_path: Path) -> int:
        ...
        try:
            ...
            total = (
                self._copy_nodes(target_conn)
                + self._copy_observed_edges(target_conn)
    ...
    def _upsert_ioc_sync(self, node_id: str, ioc_type: str, value: str, confidence: float, observed_at: float) -> str:
        ...
        if not self._schema_has_temporal:
            ...
            if not res.has_next():
                conn.execute(
                    "CREATE (:IOC {id: $id, ioc_type: $t, value: $v, first_seen: $ts, last_seen: $ts, confidence: $c})",
                    {"id": node_id, "t": ioc_type, "v": value, "ts": observed_at, "c": confidence},
                )
            else:
                conn.execute("MATCH (n:IOC) WHERE n.id = $id SET n.last_seen = $ts", {"id": node_id, "ts": observed_at})
            return node_id

        # Temporal schema
        res = conn.execute(
            "MATCH (n:IOC) WHERE n.id = $id RETURN n.earliest_observed, n.latest_observed", {"id": node_id}
        )
        if not res.has_next():
            conn.execute(
    ...
    async def record_observation(
        self, ioc_id_a: str, ioc_id_b: str, finding_id: str, ts: float, source_type: str
    ) -> None:
        """
    ...
    def _record_observation_sync(
        ...
        assert conn is not None
        res = conn.execute(
            "MATCH (a:IOC)-[r:OBSERVED]->(b:IOC) WHERE a.id = $ida AND b.id = $idb RETURN r.first_seen",
            {"ida": ioc_id_a, "idb": ioc_id_b},
        )
        if not res.has_next():
            conn.execute(
                "MATCH (a:IOC), (b:IOC) WHERE a.id = $ida AND b.id = $idb CREATE (a)-[r:OBSERVED {finding_id: $fid, source_type: $st, first_seen: $ts, last_seen: $ts}]->(b)",
                {"ida": ioc_id_a, "idb": ioc_id_b, "fid": finding_id, "st": source_type, "ts": ts},
            )
        else:
            conn.execute(
                "MATCH (a:IOC)-[r:OBSERVED]->(b:IOC) WHERE a.id = $ida AND b.id = $idb SET r.last_seen = $ts",
    ...
    async def upsert_ioc_batch(
        ...
        for item in iocs:
            if len(item) == 4:
                ...
                normalized.append(
                    (ioc_type, value, confidence, obs if obs is not None else (observed_at or time.time()))
                )
            else:
    ...
    def _upsert_legacy(self, conn, node_ids: list[str], iocs: list[tuple], now: float) -> list[str]:
        ...
        new_iocs = [
            (nid, t, v, c) for nid, (t, v, c, _) in zip(node_ids, iocs, strict=False) if nid not in existing_ids
        ]
        ...
        if existing:
            try:
                conn.execute(
                    "UNWIND $ids AS nid MATCH (n:IOC) WHERE n.id = nid SET n.last_seen = $ts",
                    {"ids": existing, "ts": now},
                )
            except Exception:
                for nid in existing:
    ...
    def _load_existing_info(self, conn, node_ids) -> dict:
        ...
        try:
            res = conn.execute(
                "UNWIND $ids AS nid MATCH (n:IOC) WHERE n.id = nid RETURN n.id, n.earliest_observed, n.latest_observed",
                {"ids": node_ids},
            )
            info = {}
            while res.has_next():
    ...
    def _create_new_nodes(self, conn, new_nodes) -> list:
        ...
        try:
            ...
            conn.execute(
                "UNWIND $data AS row CREATE (:IOC {id: row.id, ioc_type: row.t, value: row.v, first_seen: row.eo, last_seen: row.lo, confidence: row.c, earliest_observed: row.eo, latest_observed: row.lo, observation_count: 1})",
                {"data": data},
            )
            ...
            for nid, (t, val, c, ts) in new_nodes:
                try:
                    conn.execute(
                        "CREATE (:IOC {id: $id, ioc_type: $t, value: $v, first_seen: $eo, last_seen: $lo, confidence: $c, earliest_observed: $eo, latest_observed: $lo, observation_count: 1})",
    ...
    def _update_existing_nodes(self, conn, existing_to_update) -> None:
        ...
        try:
            ...
            conn.execute(
                "UNWIND $data AS row MATCH (n:IOC) WHERE n.id = row.id SET n.last_seen = row.ts, n.latest_observed = CASE WHEN row.ts > n.latest_observed THEN row.ts ELSE n.latest_observed END, n.observation_count = n.observation_count + 1",
                {"data": data},
            )
        except Exception:
            for nid, obs_at, _ in existing_to_update:
                try:
                    conn.execute(
                        "MATCH (n:IOC) WHERE n.id = $id SET n.last_seen = $ts, n.latest_observed = CASE WHEN $ts > n.latest_observed THEN $ts ELSE n.latest_observed END, n.observation_count = n.observation_count + 1",
                        {"id": nid, "ts": obs_at},
                    )
                except Exception:
                    pass
    ...
    def _upsert_ioc_batch_sync(
        ...
        if not node_ids:
            return []
        return (
            self._upsert_legacy(conn, node_ids, iocs, now)
            if not self._schema_has_temporal
            else self._upsert_temporal(conn, node_ids, iocs, now)
        )
    ...
    def _check_existing_edges(self, conn, obs_pairs) -> set:
        ...
        try:
            res = conn.execute(
                "UNWIND $obs AS pair MATCH (a:IOC)-[r:OBSERVED]->(b:IOC) WHERE a.id = pair[0] AND b.id = pair[1] RETURN pair[0], pair[1]",
                {"obs": obs_pairs},
            )
            existing = set()
            while res.has_next():
    ...
    def _create_missing_edges(self, conn, missing) -> None:
        ...
        try:
            ...
            conn.execute(
                "UNWIND $data AS row MATCH (a:IOC), (b:IOC) WHERE a.id = row.ida AND b.id = row.idb CREATE (a)-[r:OBSERVED {finding_id: row.fid, source_type: row.st, first_seen: row.ts, last_seen: row.ts}]->(b)",
                {"data": data},
            )
        except Exception:
    ...
    def _update_existing_edges(self, conn, existing_obs) -> None:
        ...
        try:
            ...
            conn.execute(
                "UNWIND $data AS row MATCH (a:IOC)-[r:OBSERVED]->(b:IOC) WHERE a.id = row.ida AND b.id = row.idb SET r.last_seen = row.ts",
                {"data": data},
            )
        except Exception:
    ...
    def _add_seed_node(self, conn, seed_id, ioc_type, ioc_value, node_set) -> None:
        """Add seed node to node set."""
        try:
            if (
                seed_res := conn.execute(
                    "MATCH (n:IOC) WHERE n.id = $id RETURN n.ioc_type, n.value, n.confidence, n.first_seen, n.last_seen",
                    {"id": seed_id},
                )
            ).has_next():
    ...
    def _collect_edges_arrow(self, conn, node_ids, node_set, max_edges):
        ...
        edges, truncated, degree_map = [], False, dict.fromkeys(node_ids, 0)
        if len(node_ids) < 2:
            return edges, truncated, degree_map
        edge_set = set()
        try:
            arrow_table = conn.execute(
                "UNWIND $ids AS nid MATCH (a:IOC)-[r:OBSERVED]->(b:IOC) WHERE a.id = nid AND b.id IN $ids RETURN a.id AS src, b.id AS dst, r.finding_id AS finding_id, r.source_type AS source_type, r.last_seen AS last_seen LIMIT $limit",
            ...
            for rec in arrow_table.to_pylist():
                if len(edge_set) >= max_edges:
                    ...
                    break
                if (
                    (src := str(rec.get("src", ""))) in node_set
                    and (dst := str(rec.get("dst", ""))) in node_set
                    and (pair := (src, dst)) not in edge_set
                ):
                    edge_set.add(pair)
                    edges.append(
                        {
    ...
    def _extract_k_hop_subgraph_arrow_sync(
        self, ioc_value: str, ioc_type: str, k: int, max_nodes: int, max_edges: int
    ) -> dict:
        """Arrow-accelerated subgraph extraction using Kuzu getAsArrow()."""
        if not _PYARROW_AVAILABLE or _pa is None:
            return self._extract_k_hop_subgraph_sync(ioc_value, ioc_type, k, max_nodes, max_edges)
    ...
    def _collect_nodes_sync(self, conn, ioc_value, ioc_type, k, neighbor_limit):
        ...
        try:
            res = conn.execute(
                f"MATCH (n:IOC)-[r*1..{k}]-(m:IOC) WHERE n.value = $v AND n.ioc_type = $t AND n.id <> m.id RETURN DISTINCT m.id AS id, m.ioc_type AS ioc_type, m.value AS value, m.confidence AS confidence, m.first_seen AS first_seen, m.last_seen AS last_seen LIMIT {neighbor_limit + 1}",
                {"v": ioc_value, "t": ioc_type},
            )
            col_names = res.get_column_names()
            while res.has_next():
                row = res.get_next()
    ...
    def _add_seed_node_sync(self, conn, seed_id, ioc_type, ioc_value, node_set) -> None:
        """Add seed node."""
        try:
            if (
                seed_res := conn.execute(
    ...
    def _collect_edges_sync(self, conn, node_ids, node_set, max_edges):
        ...
        edges, degree_map, truncated = [], dict.fromkeys(node_ids, 0), False
        if len(node_ids) < 2:
            return edges, degree_map, truncated
        edge_set = set()
        try:
            edge_res = conn.execute(
                "UNWIND $ids AS nid MATCH (a:IOC)-[r:OBSERVED]->(b:IOC) WHERE a.id = nid AND b.id IN $ids RETURN a.id, b.id, r.finding_id, r.source_type, r.last_seen LIMIT $limit",
                {"ids": node_ids, "limit": max_edges * 2},
            )
            while edge_res.has_next():
                ...
                if (src := str(row[0]) if row[0] else "") in node_set and (
                        ...
                        edges.append(
                            {
                                "source_id": src,
                                "target_id": dst,
                                "finding_id": str(row[2]) if row[2] else "",
                                "source_type": str(row[3]) if row[3] else "unknown",
                                "confidence": 1.0,
                                "last_seen": float(row[4]) if row[4] is not None else 0.0,
                            }
                        )
    ...
    def _export_stix_bundle_sync(self) -> list[dict[str, Any]]:
        ...
        try:
            res = conn.execute(
                "MATCH (n:IOC) RETURN n.id, n.ioc_type, n.value, n.confidence, n.first_seen ORDER BY n.first_seen DESC"
            )
            while res.has_next():
                row = res.get_next()
    ...
    def _load_nodes_for_communities(self, conn):
        ...
        try:
            ...
            logging.warning(f"[IOCGraph] Failed to load nodes: {e}")
    ...
    def _load_edges_for_communities(self, conn, value_to_id):
        ...
        try:
            ...
            logging.warning(f"[IOCGraph] Failed to load edges: {e}")
        return edges
    ...
    def _get_communities_igraph(self, nodes, edges):
        ...
        try:
            ...
            logging.debug(f"igraph fallback failed: {e}")
        return None
    ...
    def _get_communities_sync(self) -> dict[str, int]:
        ...
        if not edges:
            return {value: i for i, (_, value, _) in enumerate(nodes)}
        if rust_result := self._get_communities_rust(nodes, edges):
            return rust_result
    ...
    def _extract_edges(self, conn, node_map) -> tuple:
        ...
        edges, degree_map, edge_set = [], dict.fromkeys(node_map, 0), set()
        try:
            res = conn.execute(
    ...
    def _compute_igraph_communities(self, node_map, edges) -> tuple:
        ...
        id_to_idx, idx_to_nid = {nid: i for i, nid in enumerate(node_map)}, dict(enumerate(node_map))
        edge_list = [
        ...
        if edge_list:
            ...
            return (
                {
                    nid: int(membership.membership[i] if hasattr(membership, "membership") else membership[i])
    ...
    def _try_rust_communities(self, communities, node_map, edges, id_to_idx, idx_to_nid) -> bool:
        ...
        if len(set(communities.values())) <= 1 and edges:
            try:
                ...
                nodes_compact = [
                    (i + 1, node_map[nid]["value"], node_map[nid]["ioc_type"]) for i, nid in enumerate(node_map)
                ]
                edges_compact = [
                ...
                if nodes_compact and edges_compact:
                    ...
                    if rust_result and (rust_comm := rust_result.get("communities", {})):
                        communities.clear()
                        communities.update(
                            {idx_to_nid[int(k) - 1]: int(v) for k, v in rust_comm.items() if int(k) - 1 in idx_to_nid}
                        )
                        return True
            except Exception:
                pass
    ...
    def _detect_communities(self, node_map, edges, max_community_size) -> tuple:
        ...
        try:
            ...
            communities = dict.fromkeys(node_map, 0)
    ...
    def _export_graph_topology_sync(self, *, max_nodes: int, max_community_size: int, include_centrality: bool) -> dict:
        ...
        centrality = self._compute_topology_for_export(node_map, edges, degree_map, include_centrality)

        return self._assemble_topology_result(node_map, edges, community_info, centrality, community_groups, degree_map)
    ...
    def _detect_topology_communities(self, node_map, edges, max_community_size: int) -> tuple:
        ...
        try:
            ...
            idx_to_nid = dict(enumerate(node_map))
            edge_list = [
                (id_to_idx[e["source"]], id_to_idx[e["target"]])
                for e in edges
                if e["source"] in id_to_idx and e["target"] in id_to_idx
    ...
    def _assemble_topology_result(
        self, node_map, edges, community_info, centrality, community_groups, degree_map
    ) -> dict:
        """Phase 5: Assemble final topology result."""
        nodes_result = self._assemble_topology_nodes_with_sort(node_map, community_groups, degree_map, centrality)
        stats = self._compute_topology_stats(node_map, edges, community_groups, degree_map)
    ...
    def _try_rust_centrality(self, centrality, node_ids, node_map, edges, value_to_id) -> bool:
        ...
        try:
            ...
            adj_list: list[tuple[int, list[int]]] = list(adjacency.items())
            if adj_list:
    ...
    def _try_igraph_centrality(self, centrality, node_ids, edges, node_map, n) -> bool | None:
        ...
        try:
            ...
            for metric, func in [
                ("pagerank", g.pagerank),
                ("betweenness", g.betweenness if n <= 2000 else None),
                ...
                if func:
                    try:
                        scores = (
                            func(directed=False)
                            if metric != "pagerank"
                            else func(weights="weight" if g.is_weighted() else None)
                        )
    ...
    def _get_igraph_pagerank(self, nodes, edges, value_to_id, target_id) -> float:
        ...
        try:
            ...
            if target_idx is None:
                return 0.0
            return float(
                ig.Graph(n=len(nodes), edges=edge_list, directed=True).pagerank(damping=0.85, directed=True)[target_idx]
            )
        except Exception:
            return 0.0
    ...
    def _get_centrality_sync(self, ioc_value: str) -> float:
        ...
        if not nodes:
            return 0.0
        if (target_id := value_to_id.get(ioc_value)) is None:
            return 0.0
        if score := self._get_rust_pagerank(nodes, edges, target_id):
            return score
