...
class DeepExplainer:
    ...
    __slots__ = tuple(('gnn', 'governor'))

    def __init__(self, gnn_predictor, governor: ResourceGovernor):
        self.gnn = gnn_predictor
        self.governor = governor

    async def explain(self, node: str, target_prediction: str | None=None, max_nodes: int=10, optimize_features: bool=False) -> dict:
        """
        ...
        async with self.governor.reserve({'ram_mb': 200, 'gpu': True}, Priority.NORMAL):
            ...
            if USE_NATIVE and hasattr(self.gnn, 'explain'):
                try:
                    ...
                    data = _mxg.data.Data(x=_mx.array(subgraph['node_features']), edge_index=_mx.array(subgraph['edges']).T, edge_weight=_mx.array(subgraph.get('edge_weights', [1.0] * len(subgraph['edges']))), y=_mx.array([subgraph['target_idx']]))
                    explanation = self.gnn.explain(data, target_idx=subgraph['target_idx'])
                    return {'node': node, 'important_edges': explanation.edge_importance, 'feature_importance': explanation.feature_importance if optimize_features else None}
