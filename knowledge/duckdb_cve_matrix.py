...
class CveCorrelationMatrix:
    ...
    def _row_to_cve_match(self, row: tuple) -> CveMatch:
        ...
        return CveMatch(cve_id=str(row[0]) if row[0] else '', technology=str(row[1]) if row[1] else '', version_pattern=str(row[2]) if row[2] else None, cvss_score=float(row[3]) if row[3] is not None else None, cwe_id=str(row[4]) if row[4] else None, description_snippet=str(row[5]) if row[5] else '', published_date=str(row[6]) if row[6] else None)
    ...
    async def match_batch_async(self, technologies: list[tuple[str, str | None]]) -> dict[str, list[CveMatch]]:
        """Async batch lookup."""
        loop = asyncio.get_running_loop()
    ...
    def load_cve_data(self, cve_data: list[dict[str, Any]]) -> int:
        ...
        for cve in cve_data:
            try:
                conn.execute('\n                    INSERT OR IGNORE INTO cve_matrix\n                    (cve_id, technology, version_pattern, cvss_score, cwe_id, description_snippet, published_date)\n                    VALUES (?, ?, ?, ?, ?, ?, ?)\n                ', [cve.get('cve_id', ''), cve.get('technology', '').lower(), cve.get('version_pattern'), cve.get('cvss_score'), cve.get('cwe_id'), cve.get('description_snippet', '')[:500], cve.get('published_date')])
                loaded += 1
            except Exception as e:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {'enabled': _INMEMORY_CVE_ENABLED, 'db_path': str(self._db_path), 'db_exists': self._db_path.exists(), 'initialized': self._initialized, 'cache_size': len(self._lru_cache), 'cache_max': self._cache_size, 'cached_technologies': list(self._lru_cache.keys())[:10]}
...
_get_cve_matrix = module_singleton_getter(singleton_name='_CVE_MATRIX_INSTANCE', factory=CveCorrelationMatrix)
