...
class LanceDBRAGEngine:
    ...
    def __init__(
        ...
        self._quantize_enabled = (
            enable_quantize if enable_quantize is not None else os.getenv("HLEDAC_LANCEDB_QUANTIZE", "0") == "1"
        )
                ...
                try:
                    ...
                    if not any(getattr(idx, "name", "") == "content_idx" for idx in existing):
                        self._table.create_fts_index(
                            "content", replace=False, with_position=True, tokenizer_name="en_stem"
                        )
    ...
    async def add_document(self, doc: RAGDocument) -> bool:
        ...
        try:
            await offload_to(
                "cpu_blocking_pool",
                self._table.add,
                [{"id": doc.id, "content": doc.content, "metadata": metadata_json, "embedding": emb_norm}],
            )
            return True
        except Exception as e:
    ...
    async def add_documents_batch(self, docs: list[RAGDocument], batch_size: int = _MAX_BATCH_SIZE) -> int:
        ...
        for i in range(0, len(docs), batch_size):
            ...
            for d, emb in zip(batch, emb_norm, strict=True):
                rows.append(
                    {"id": d.id, "content": d.content, "metadata": orjson.dumps(d.metadata).decode(), "embedding": emb}
                )
            try:
    ...
    async def search(
        self, query: str, top_k: int = 10, use_mmr: bool = True, mmr_lambda: float = 0.7
    ) -> list[RetrievedChunk]:
        """
        ...
        for r in results:
            ...
            try:
                ...
                metadata = {}
            doc = RAGDocument(
                id=r.get("id", ""), content=r.get("content", ""), metadata=metadata, embedding=r.get("embedding")
            )
