...
from hledac.universal.utils.asyncx import bounded_parallel_map, safe_wait_for
...
def _resolve_lancedb_cache_size() -> int:
    ...
    if raw:
        try:
            ...
            if mb <= 0:
                logger.warning(
                    f"[LANCEDB_CACHE] Invalid {_HLEDAC_CACHE_MB_VAR}={raw}, using default {_HLEDAC_DEFAULT_CACHE_MB}MB"
                )
                mb = _HLEDAC_DEFAULT_CACHE_MB
            elif not override_enabled and mb > _HLEDAC_HARD_MAX_CACHE_MB:
                logger.warning(
                    f"[LANCEDB_CACHE] {mb}MB exceeds hard max {_HLEDAC_HARD_MAX_CACHE_MB}MB without {_HLEDAC_LARGE_OVERRIDE_VAR}=1, capping"
                )
                mb = _HLEDAC_HARD_MAX_CACHE_MB
            return mb * 1024 * 1024
        except ValueError:
            logger.warning(
                f"[LANCEDB_CACHE] Non-integer {_HLEDAC_CACHE_MB_VAR}={raw}, using default {_HLEDAC_DEFAULT_CACHE_MB}MB"
            )
...
async def _write_batch_to_table(writer_id: int, qname: str, batches: list) -> None:
    ...
    if table is not None:
        try:
            await safe_wait_for(
                asyncio.to_thread(table.add, all_records), timeout=30.0, label=f"lancedb_table_add:{qname}"
            )
...
class LanceDBIdentityStore:
    ...
    __slots__ = (
        ...
        "_embedding_dim",
        "_mlx_embeddings",
        "_mlx_embeddings_total_count",
        "_mlx_id_to_idx",
        "_mlx_ids",
        "_mlx_load_chunk_size",
        "_orch",
        "_table",
        "db",
        "uri",
        ...
        "_index_cache",
        "_index_cache_time",
        "_index_build_deferred",
        "_metrics",
        "_large_override_enabled",
        "_ivfpq_enabled",
    ...
    def __init__(self, uri: str = str(_DEFAULT_URI), orchestrator=None) -> None:
        """
        ...
        self._ivfpq_num_partitions: int = max(
            8, min(256, int(os.environ.get("HLEDAC_LANCEDB_IVFPQ_NUM_PARTITIONS", "256")))
        )
        self._ivfpq_num_sub_vectors: int = max(
            4, min(64, int(os.environ.get("HLEDAC_LANCEDB_IVFPQ_NUM_SUB_VECTORS", "8")))
        )
    ...
    async def _initialize_embedder(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"[EMBEDDER] Using shared MLXEmbeddingManager: {self._mlx_embed_manager.model_path}, dim={self._mlx_embed_manager.EMBEDDING_DIM}"
            )
            return True
        except ImportError:
    ...
    async def _embed_batch(self, texts: list[str], batch_size: int = 16) -> list[list[float]]:
        ...
        if _get_uma_budget()():
            logger.warning(
                f"[EMBED] Skipping {len(texts)} embeddings — M1 critical memory pressure ({len(texts)} texts, batch_size={batch_size})"
            )
    ...
    async def _get_cached_embedding(self, text_hash: str) -> list[float] | None:
        ...
        async with self._writeback_lock:
            ...
            if len(self._writeback_buffer) > _WRITEBACK_MAX:
                items_to_evict = sorted(self._writeback_buffer.items(), key=lambda x: self._access_counts.get(x[0], 0))[
            ...
            async with self._writeback_lock:
                ...
                if len(self._writeback_buffer) > _WRITEBACK_MAX:
                    items_to_evict = sorted(
                        self._writeback_buffer.items(), key=lambda x: self._access_counts.get(x[0], 0)
                    )[:_WRITEBACK_BATCH_SIZE]
                    flush_items = items_to_evict
                    for flush_key, _ in flush_items:
                        self._writeback_buffer.pop(flush_key, None)
    ...
    async def _load_embeddings_to_mlx(self) -> None:
        ...
        try:
            ...
            try:
                ...
                if available_gb < 3.0:
                    logger.debug(
                        f"[LANCEDB_MLX] Skipping MLX load — only {available_gb:.1f}GB available (need >3GB for embedding table)"
                    )
                    return
            except Exception:  # noqa: BLE001
                pass
            ...
            if total_count > MAX_MLX_ROWS:
                logger.debug(
                    f"[LANCEDB_MLX] Capping load from {total_count} to {MAX_MLX_ROWS} rows (M1 8GB GPU memory budget)"
                )
            ...
            for offset in range(0, total_count, chunk_size):
                try:
                    ...
                    if uma.swap_detected:
                        logger.debug(
                            f"[LANCEDB_MLX] Aborting load at chunk offset {offset} — swap detected mid-load (used {uma.swap_used_gib:.1f}GiB)"
                        )
                        break
                except Exception:  # noqa: BLE001
                    pass
                ...
                chunk_data = (
                    self._table.to_lance().to_table(columns=["embedding", "id"], offset=offset, limit=limit).to_pydict()
                )
                ...
                if raw_emb and isinstance(raw_emb[0], (list, tuple)):
                    ...
                    emb_np = (
                        np.ascontiguousarray(raw_emb, dtype=np.float32) if raw_emb else np.array([], dtype=np.float32)
                    )
    ...
    def _mmr(
        self, candidates: list[dict], query_emb: list[float], lambda_param: float = 0.5, top_k: int = 30
    ) -> list[dict]:
        ...
        while len(selected) < top_k and remaining:
            ...
            for doc in remaining:
                ...
                sim_to_query = np.dot(query_emb_np, doc_emb) / (
                    np.linalg.norm(query_emb_np) * np.linalg.norm(doc_emb) + 1e-08
                )
                max_sim_to_selected = 0
                if selected:
                    # MRL-2 FIX: Column name is 'embedding' not '_embedding'
                    selected_embs = np.array(
                        [s.get("embedding", s.get("_embedding", [0] * len(query_emb))) for s in selected]
                    )
                    sims = np.dot(selected_embs, doc_emb) / (
                        np.linalg.norm(selected_embs, axis=1) * np.linalg.norm(doc_emb) + 1e-08
                    )
    ...
    def _validate_table_schema(self, expected_dim: int) -> bool:
        ...
        try:
            ...
            emb_field = next((f for f in lance_schema.fields if f.name == "embedding"), None)
    ...
    def _log_table_opened(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"[LANCEDB] lancedb.table_opened table=entities rows={row_count} size_mb={size_mb:.2f} uri={self.uri}"
            )
        except Exception as e:
    ...
    async def _ensure_ivf_pq_index_async(self) -> None:
        ...
        async with self._ivfpq_lock:
            ...
            try:
                ...
                if row_count < 256:
                    logger.debug(
                        f"[LANCEDB] IVF-PQ skipped: only {row_count} rows (need >= 256 for meaningful PQ training)"
                    )
                ...
                self._ivfpq_trained = True
                logger.info(
                    f"[LANCEDB] IVF-PQ trained: table=entities rows={row_count} num_partitions={num_partitions} num_sub_vectors={num_sub_vectors}"
                )
            except Exception as e:
                self._ivfpq_trained = True
    ...
    async def add_entity(self, entity_id: str, embedding: list[float], aliases: list[str]) -> bool:
        ...
        try:
            ...
            try:
                ...
                await safe_wait_for(
                    queue.put((data, deadline)),
                    timeout=max(0.0, deadline - time.monotonic()),
                    label="lancedb_queue_put",
                )
            except TimeoutError:
            ...
            if tuner is not None and getattr(self, "_ivfpq_enabled", False):
                try:
                    ...
                    if result.changed():
                        ...
                        logger.info(
                            f"[LANCEDB] auto-tune adjusted num_partitions={result.old_partitions}->{result.new_partitions} num_sub_vectors={result.old_num_sub_vectors}->{result.new_num_sub_vectors} recall={result.recall:.3f} avg_ms={result.avg_search_ms:.2f}"
                        )
        ...
        try:
            ...
            def _search():
                if _qt == "hybrid" and _txt and self._lancedb_has_fts:
                    ...
                    logger.debug(
                        "[LANCEDB:H] text_hint=%r ignored — FTS not supported in local LanceDB", str(_txt)[:50]
                    )
                    # M1 8GB optimization: probe only ~3% of IVF_PQ partitions
                    _nprobes = self._ivfpq_nprobes
                    try:
                        return (
                            self._table.search(_emb, vector_column_name="embedding")
                            .nprobes(_nprobes)
                            .limit(_lim)
                            .to_polars()
                        )
            ...
            if "_relevance_score" in df.columns:
                ...
                df = df.with_columns((1 - pl.col("_distance")).alias("similarity")).filter(
                    ...
                    for idx, r in enumerate(rows):
                        ...
                        if idx < len(embeddings) and embeddings[idx]:
                            self._table.merge_insert("id").on("id").execute(
                                [{"id": entity_id, "embedding": embeddings[idx]}]
                            )
                            stats["reembedded"] += 1
                        else:
                            stats["skipped"] += 1
    ...
    async def search_similar_adaptive(self, query_text: str, query_emb: list[float], top_k: int = 10) -> list[dict]:
        ...
        try:
            candidates = await self.search_similar(
                query_emb, text_hint=query_text or "", limit=200, query_type="auto", threshold=0.0
            )
        except Exception:
    ...
    async def search_with_mmr(
        self, query_text: str, query_emb: list[float], top_k: int = 10, lambda_mult: float = 0.5, fetch_k: int = 30
    ) -> list[dict]:
        """
        ...
        try:
            candidates = await self.search_similar(
                query_emb, text_hint=query_text or "", limit=fetch_k, query_type="auto", threshold=0.0
            )
        except Exception:
            if self._usearch_index is not None:
        ...
        selected_indices = maximal_marginal_relevance(
            query_vector=query_emb_np, candidate_vectors=candidate_embs, top_k=top_k, lambda_param=lambda_mult
        )
...
class SqliteVecIdentityStore:
    ...
    MAX_DIM = 384
    __slots__ = ("_embedding_manager", "_orch", "_sprint_id", "_store")
    ...
    async def search_similar(
        ...
        try:
            results = await self._store.search_entities(
                query_embedding=embedding, top_k=limit, threshold=1.0 - threshold
            )
            now = datetime.now(UTC)
            return [
                {
    ...
    async def search_similar_adaptive(
        self, query_text: str, query_emb: list[float], top_k: int = 10
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            results = await self._store.search_entities(
                query_embedding=query_emb, top_k=min(top_k * 2, 100), threshold=0.0
            )
        except Exception:
            return []
        if not results:
        ...
        try:
            ...
            if query_emb_np.ndim == 1:
                query_emb_np = query_emb_np.reshape(1, -1)
            entity_vecs = [
                np.array([e.get("embedding", []) if isinstance(e.get("embedding"), list) else []], dtype="float32")
                for e in entities
            ]
            selected_indices = maximal_marginal_relevance(
                query_vector=query_emb_np, candidate_vectors=entity_vecs, top_k=top_k, lambda_param=0.5
            )
...
class LanceDBAcademicStore:
    ...
    async def _embed_texts(self, texts: list[str]) -> list[list[float]]:
        ...
        if self._embedder is None:
            raise RuntimeError(
                "_embed_texts called before _init_embedder succeeded. Check that initialize() was awaited and a backend is installed."
            )
        import asyncio
    ...
    async def search_similar(
        self, query: str, top_k: int = 10, filters: dict | None = None, query_type: str = "auto"
    ) -> list[AcademicPaper]:
        """
        ...
        try:
            ...
            def _search():
                if _qt == "hybrid" and _q and self._lancedb_has_fts:
                    ...
                    builder = (
                        self._table.search(
                            query_type="hybrid", vector_column_name="embedding", fts_columns=["title", "abstract"]
                        )
                        .vector(_emb)
                        .text(_q)
                        .limit(_k)
    ...
    async def get_citation_context(self, paper_id: str, max_papers: int = 20) -> list[AcademicPaper]:
        ...
        try:
            ...
            try:
                _base = (
                    self._table.search([0.0] * self._dim, vector_column_name="embedding")
                    .nprobes(_nprobes)
            ...
            try:
                similar = (
                    self._table.search(_base[0].get("embedding", [0.0] * self._dim), vector_column_name="embedding")
                    .nprobes(_nprobes)
                    .where(f"citation_count > {_base[0].get('citation_count', 0) * 0.5}")
                    .limit(max_papers)
                    .to_list()
                )
            except TypeError:
