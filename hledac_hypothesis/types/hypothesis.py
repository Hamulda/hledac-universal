...
class InferenceEngineProtocol(Protocol):
    ...
    async def abductive_reasoning(self, observations: list[object], context: dict[str, Any]) -> list[dict[str, Any]]:
        """Generate possible explanations (hypotheses) from observations."""
        ...

    async def evidence_chaining(self, hypothesis: object, context: dict[str, Any]) -> list[dict[str, Any]]:
        """Chain evidence to design tests for a hypothesis."""
        ...

    async def belief_update(self, hypothesis: object, new_evidence: object) -> float:
        """Calculate updated belief given new evidence."""
        ...


def _to_operator_shortlist(raw: list[dict[str, Any]], max_items: int = 3) -> list[dict[str, Any]]:
