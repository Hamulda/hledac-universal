...
async def _get_ipfs_session(
    host: str, *, timeout: httpx.Timeout, transport: httpx.AsyncHTTPTransport | None = None
) -> httpx.AsyncClient:
    """
...
async def fetch_directory_recursive(
    cid: str, max_depth: int = MAX_DIR_DEPTH, current_depth: int = 0, seen_cids: set[str] | None = None
) -> list[dict]:
    """
                        ...
                        results.append(
                            {
                                ...
                                "size": link_size,
                                "type": "dir" if link_type == 2 else "file",
                            }
                        )
...
async def find_via_ipfs_search(query: str) -> list[str]:
    ...
    try:
        ...
        resp, err = await _ipfs_checked_get(
            session, IPFS_SEARCH_GATEWAY, timeout=client_timeout, failure_kind="ipfs_search"
        )
        if err is None and resp is not None and (resp.status_code == 200):
            data = await resp.json()
            hits = data.get("hits", [])
...
async def search_via_estuary(query: str) -> list[str]:
    ...
    try:
        ...
        session = await _get_ipfs_session(host, timeout=client_timeout)
        resp, err = await _ipfs_checked_get(
            session, ESTUARY_SEARCH, timeout=client_timeout, failure_kind="estuary_search"
        )
        if err is None and resp is not None and (resp.status_code == 200):
            data = await resp.json()
            if isinstance(data, list):
    ...
    for name, base_url in IPFS_GATEWAYS:
        try:
            ...
            if len(body) > MAX_FILE_SIZE_BYTES:
                logger.warning(
                    f"IPFS file {cid} from {name} exceeds 10MB limit after download ({len(body)} bytes) → skipping"
                )
                return None
            get_breaker(host).record_success()
...
def ipfs_content_to_finding_dict(
    ...
    if gateway:
        provenance = provenance + (
            f"https://{gateway}.ipfs.example/{cid}" if not gateway.startswith("http") else f"{gateway}/{cid}",
        )
    return {
        "finding_id": finding_id,
        "query": query,
...
async def ipfs_search_as_findings(query: str, timeout_per_result: int = 30) -> list:
    ...
    results = await parallel(
        [_fetch_one_cid(cid) for cid in cids[:20]], policy="log", concurrency=5, ctx="ipfs:_fetch_cids"
    )
    return [r for r in results if r is not None]
