...
async def _get_ipfs_session(host: str, *, timeout: httpx.Timeout, transport: httpx.AsyncHTTPTransport | None=None) -> httpx.AsyncClient:
    """
...
def get_ipfs_pool_status() -> dict:
    ...
    return {'pool_size': len(_SESSION_POOL), 'max_pool_size': MAX_POOL_SIZE, 'hosts': list(_SESSION_POOL.keys())}

async def _ipfs_checked_get(session: httpx.AsyncClient, url: str, *, timeout: httpx.Timeout | None=None, failure_kind: str='ipfs_fetch_error') -> tuple[httpx.Response | None, str | None]:
    """
...
IPFS_GATEWAYS: list[tuple[str, str]] = [('local', 'http://localhost:8080/ipfs/'), ('cloudflare', 'https://cloudflare-ipfs.com/ipfs/'), ('ipfs.io', 'https://ipfs.io/ipfs/')]
IPNS_API_GATEWAYS: list[str] = ['https://ipfs.io/api/v0/name/resolve?arg=', 'https://cloudflare-ipfs.com/api/v0/name/resolve?arg=']
IPNS_TIMEOUT: int = 15
...
async def resolve_ipns(name: str, timeout: int=IPNS_TIMEOUT) -> str | None:
    ...
    for api_base in IPNS_API_GATEWAYS:
        try:
            ...
            resp, err = await _ipfs_checked_get(session, url, failure_kind='ipns_resolve')
            if err is None and resp is not None and (resp.status_code == 200):
                data = await resp.json()
                path = data.get('Path', '')
                cid_match = re.search('/ipfs/([a-zA-Z0-9]+)', path)
                if cid_match:
...
async def fetch_directory_recursive(cid: str, max_depth: int=MAX_DIR_DEPTH, current_depth: int=0, seen_cids: set[str] | None=None) -> list[dict]:
    """
    ...
    for name, gateway_base in IPFS_GATEWAYS:
        try:
            ...
            if api_host == host:
                resp, err = await _ipfs_checked_get(session, api_url, failure_kind='ipfs_dir_ls')
            else:
                api_session = await _get_ipfs_session(api_host, timeout=client_timeout)
                resp, err = await _ipfs_checked_get(api_session, api_url, failure_kind='ipfs_dir_ls')
            if err is None and resp is not None and (resp.status_code == 200):
                data = await resp.json()
                ...
                if objects:
                    ...
                    for link in links[:MAX_DIR_FILES]:
                        ...
                        results.append({'cid': link_cid, 'path': f'{cid}/{link_name}', 'size': link_size, 'type': 'dir' if link_type == 2 else 'file'})
                        if link_type == 2 and current_depth < max_depth:
                            sub_results = await fetch_directory_recursive(link_cid, max_depth, current_depth + 1, seen_cids)
...
async def find_via_ipfs_search(query: str) -> list[str]:
    ...
    try:
        ...
        session = await _get_ipfs_session(host, timeout=client_timeout)
        resp, err = await _ipfs_checked_get(session, IPFS_SEARCH_GATEWAY, timeout=client_timeout, failure_kind='ipfs_search')
        if err is None and resp is not None and (resp.status_code == 200):
            data = await resp.json()
            hits = data.get('hits', [])
...
async def search_via_estuary(query: str) -> list[str]:
    ...
    try:
        ...
        session = await _get_ipfs_session(host, timeout=client_timeout)
        resp, err = await _ipfs_checked_get(session, ESTUARY_SEARCH, timeout=client_timeout, failure_kind='estuary_search')
        if err is None and resp is not None and (resp.status_code == 200):
            data = await resp.json()
            if isinstance(data, list):
...
async def ipfs_directory_as_findings(cid: str, query: str, max_depth: int=3) -> list:
    """
    ...
    for entry in entries:
        ...
        finding = CanonicalFinding(finding_id=finding_id, query=query, source_type='ipfs_directory', confidence=0.7, ts=_time.time(), provenance=(f'ipfs://{entry_cid}',), payload_text=payload[:4096] if payload else None)
...
async def fetch_ipfs(cid: str, timeout: int=30) -> bytes | None:
    ...
    for name, base_url in IPFS_GATEWAYS:
        try:
            ...
            session = await _get_ipfs_session(host, timeout=client_timeout, transport=transport)
            try:
                ...
                continue
            resp, err = await _ipfs_checked_get(session, url, failure_kind='ipfs_fetch')
            ...
            if len(body) > MAX_FILE_SIZE_BYTES:
                logger.warning(f'IPFS file {cid} from {name} exceeds 10MB limit after download ({len(body)} bytes) → skipping')
                return None
            get_breaker(host).record_success()
...
async def search_ipfs(query: str) -> list[str]:
    ...
    if not keywords:
        return []
    for kw in keywords:
        cid_candidate = f"Qm{kw[:44].ljust(44, '0')}"
        if cid_candidate not in seen:
            seen.add(cid_candidate)
            cids.append(cid_candidate)
    return cids

def ipfs_content_to_finding_dict(cid: str, content: bytes, query: str, source_type: str='ipfs_fetch', gateway: str | None=None, ts: float | None=None, finding_id_prefix: str='ipfs') -> dict:
    """
    ...
    return {'finding_id': finding_id, 'query': query, 'source_type': source_type, 'confidence': 0.75, 'ts': _ts, 'provenance': provenance, 'payload_text': payload_text, 'accepted': True, 'reason': source_type, 'entropy': 0.0, 'normalized_hash': None, 'duplicate': False}
...
async def fetch_findings_from_cids(cids: list[str], query: str, timeout_per_cid: int=10) -> list[CanonicalFinding]:
    """
    ...
    results = await parallel_ok(*tasks, label='ipfs_client:858')
...
async def ipfs_search_as_findings(query: str, timeout_per_result: int=30) -> list:
    ...
    async def _fetch_one_cid(cid: str) -> CanonicalFinding | None:
        try:
            ...
            return CanonicalFinding(finding_id=finding_dict['finding_id'], query=finding_dict['query'], source_type=finding_dict['source_type'], confidence=finding_dict['confidence'], ts=finding_dict['ts'], provenance=finding_dict['provenance'], payload_text=finding_dict.get('payload_text'))
        except Exception:
            return None
