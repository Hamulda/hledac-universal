...
def _create_mamba_fusion_instance(
    vision_dim: int = 1280,
    text_dim: int = 768,
    graph_dim: int = 64,
    hidden: int = 256,
    ...
    async def _lazy_load(self) -> None:
        ...
        async with self._get_lock():
            ...
            if os.environ.get(_MOBILECLIP_ENV_GATE, "").lower() not in ("1", "true", "yes"):
                raise RuntimeError(
                    f"{_MOBILECLIP_ENV_GATE} not set — MobileCLIP gated off. Set {_MOBILECLIP_ENV_GATE}=1 to enable (loads ~150 MB)."
                )

            # Try pool first
            if self._pool is not None:
                try:
                    model, tok = await self._pool.acquire(_MOBILECLIP_POOL_ID, lambda: _load_mobileclip_from_pool())
                    self._model = model
                    self._tokenizer = tok
                    self._initialized = True
...
class IdentityFusion:
    ...
    async def fuse_identity(
        ...
        matched_identities = await self._find_matching_identities(face_vector, voice_vector, text_iocs, confidence)
        ...
        try:
            ...
            if hasattr(self._graph, "link_face_to_voice"):
                ...
                if face_id and voice_id:
                    ...
                    face_weight = (
                        self._face_weight / (self._face_weight + self._voice_weight)
                        if (self._face_weight + self._voice_weight) > 0
                        else 0.5
                    )
