...
class StealthManager:
    ...
    def __init__(self, config: StealthManagerConfig | None = None) -> None:
    ...
    def get_stealth_transport_telemetry(self) -> dict[str, Any]:
        ...
        return {
            ...
            "canonical_curl_runtime_used": False,
...
class StealthSession:
    ...
    __slots__ = ("_closed", "_cookies", "_request_count", "_session", "manager")

    def __init__(self, manager: StealthManager) -> None:
    ...
    async def _execute_request_attempt(
        self, session: Any, method: str, url: str, stealth_headers: dict[str, str], data: Any, **kwargs
        ...
        try:
            response = await session.request(
                method=method.upper(), url=url, headers=stealth_headers, data=data, **kwargs
            )
    ...
    def _is_onion_url(url: str) -> bool:
        ...
        try:
            ...
            return netloc.endswith(".onion") or netloc.startswith("tor:") or netloc.endswith(".onion.")
        except Exception:
            return False
            ...
            with Controller.from_port(address="127.0.0.1", port=9051) as controller:
                try:
                    ...
                    if "authentication failed" in str(e).lower():
                        logger.warning(
                            "Tor authentication failed. Ensure torrc config uses CookieAuthSocket or HashedControlPassword and the control port is accessible."
                        )
                    raise
...
class TokenBucketController:
    ...
    __slots__ = ("_capacity", "_cond", "_last_refill", "_rate", "_tokens")
