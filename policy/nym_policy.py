...
class LinUCBArm:
    __slots__ = tuple(('A_inv', 'alpha', 'b', 'dim'))
...
class NymPolicy:
    __slots__ = tuple(('_last_context', 'alpha', 'bandit_nym', 'bandit_tor', 'exploration_rate', 'governor', 'lambda_cost', 'nym', 'nym_latency', 'tor', 'tor_latency'))

    def __init__(self, governor: ResourceGovernor, tor_transport: TorTransport, nym_transport: NymTransport, alpha: float=0.5, lambda_cost: float=0.01, exploration_rate: float=0.1):
    ...
    def _extract_features(self, risk_level: RiskLevel, time_budget: float, sensitivity: float, transport: str) -> np.ndarray:
    ...
    async def select_transport(self, risk_level: RiskLevel, time_budget: float, sensitivity: float, need_cover_traffic: bool=False, request_id: str | None=None) -> tuple[Transport, dict]:
