...
class LinUCBArm:
    __slots__ = ("A_inv", "alpha", "b", "dim")

    def __init__(self, dim: int = 4, alpha: float = 0.5) -> None:
        self.dim = dim
        self.alpha = alpha
...
class NymPolicy:
    ...
    def _extract_features(
        self, risk_level: RiskLevel, time_budget: float, sensitivity: float, transport: str
    ) -> np.ndarray:
    ...
    async def update_reward(self, request_id: str, success: bool, latency: float) -> None:
