# report/renderers/svg_renderer.py
# Issue 12.1: SVG renderer using graphviz system binary
"""
...
class SVGRenderer:
    ...
    def _build_dot_graph(
        ...
        for node in nodes[:200]:  # Cap at 200 nodes
        ...
        for edge in edges[:400]:  # Cap at 400 edges
    ...
    def _safe_id(node_id: str) -> str:
        ...
        safe = re.sub(r"[^a-zA-Z0-9_]", "_", str(node_id))[:40]
        return safe or ""

    def _render_mermaid_fallback(self, graph_data: Any) -> str:
        """Render as Mermaid SVG when graphviz unavailable."""
        nodes = self._extract_nodes(graph_data)
        self._extract_edges(graph_data)  # consumed for compatibility

        lines = [
            "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 800 600'>",
            "<style>text{font-family:system-ui,sans-serif;font-size:12px;fill:#e6edf3}rect{fill:#161b22;stroke:#30363d;rx:4}</style>",
        ...
        for node in nodes[:20]:
            ...
            y = 60 + (count // 4) * 80
            lines.append(
                f"<rect x='{x}' y='{y}' width='140' height='36'/><text x='{x+70}' y='{y+22}' text-anchor='middle'>{label}</text>"
            )
            count += 1
            if count >= 20:
                break
