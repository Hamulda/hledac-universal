...
class PDFRenderer:
    ...
    __slots__ = ("_weasyprint_available",)
    ...
    def _render_webkit_fallback(html_content: str) -> bytes:
        ...
        if sys.platform != "darwin":
            raise RuntimeError(
                "PDF rendering unavailable: no backend (weasyprint not installed, webkit only available on macOS)"
            )
        try:
            import asyncio
            ...
            raise RuntimeError(
                "PDF rendering unavailable: weasyprint not installed and macos_webkit_renderer not available"
            )
