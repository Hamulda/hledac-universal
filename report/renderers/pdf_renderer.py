# report/renderers/pdf_renderer.py
# Issue 12.1: PDF renderer using weasyprint
"""
...
class PDFRenderer:
    ...
    def render_markdown_to_file(
        self, markdown: str, path: Path | str, *, html_renderer: Any = None
    ) -> Path:
    ...
    def _render_webkit_fallback(html_content: str) -> bytes:
        ...
        if sys.platform != "darwin":
            raise RuntimeError(
                "PDF rendering unavailable: no backend (weasyprint not installed, "
        ...
        try:
            ...
            # We can't directly render HTML to PDF with fetch_with_macos_webkit
            # since it fetches URLs. Use temp file approach.
            import asyncio
            ...
            try:
                # Write HTML to temp, then use WebKit to get PDF
                # This is a best-effort fallback
                result = asyncio.run(
                    fetch_with_macos_webkit(f"file://{temp_path}", timeout_s=30.0)
                )
            ...
            raise RuntimeError(
                "PDF rendering unavailable: weasyprint not installed and "
                "macos_webkit_renderer not available"
            )
