# report/renderers/markdown_renderer.py
# Issue 12.1: Markdown renderer using jinja2 templates
"""
...
class MarkdownRenderer:
    ...
    def __init__(self, template_dir: Path | str | None = None) -> None:
        self._template_dir = (
            Path(template_dir) if template_dir else self._default_template_dir()
        )
    ...
    def _render_fallback(self, template_name: str, context: dict[str, Any]) -> str:
        ...
        # Minimal fallback — preserves existing sprint_markdown_reporter behavior
        lines: list[str] = []
        ...
        if src_yield:
            ...
            for src, cnt in sorted(
                src_yield.items(), key=lambda x: x[1], reverse=True
            )[:10]:
