...
class _DHTBootstrapProtocol(asyncio.DatagramProtocol):
    ...
    def __init__(self, loop, node_id: str) -> None:
        self._loop = loop
...
class BEP5UDPProtocol(asyncio.DatagramProtocol):
    ...
    def _cleanup_pending(self) -> None:
        ...
        expired_tids = [
            tid
            for tid, ts in list(self._pending_created.items())
            if (
...
async def crawl_dht_for_keyword(
    keyword: str, duration_s: int = 120, max_results: int = 100, harvest_metadata: bool = False
) -> list[dict]:
    """
    ...
    node = KademliaNode(
        node_id=f"hledac-crawl-{uuid.uuid7().hex[:8]}", governor=governor, bootstrap_nodes=BOOTSTRAP_PEERS
    )
    try:
...
class KademliaNode:
    __slots__ = (
        "_bep5_protocol",
        "_bep5_transport",
        "_nodes_since_snapshot",
        "_pending_rpcs",
        "_pending_rpcs_created",
        "_refresh_task",
        "_routing_loaded",
        "_running",
        "_transport",
    ...
    def __init__(
        ...
        k: int = 20,
        alpha: int = 3,
        local_graph_store: LocalGraphStore | None = None,
    ) -> None:
    ...
    async def start_udp(self, port: int = 0) -> bool:
        ...
        try:
            ...
            transport, protocol = await loop.create_datagram_endpoint(
                lambda: BEP5UDPProtocol(self._handle_message), local_addr=("0.0.0.0", port)
            )
    ...
    def _update_routing(self, peer_id: str, peer_info: dict[str, Any] | None = None) -> None:
    ...
    def _cleanup_pending_rpcs(self) -> None:
        """
        ...
        expired_rpc_ids = [
            rid
            for rid, fut in list(self._pending_rpcs.items())
            if fut.done()
            or fut.cancelled()
    ...
    def _launch_find_value_rpcs(
        self, shortlist: list, queried: set[str], key: str
    ...
    async def _send_store(self, peer_id: str, key: str, value: Any) -> None:
        if not self._transport:
            return
        try:
            import orjson
    ...
    async def _send_find_value(self, peer_id: str, key: str, rpc_id: str) -> None:
        if not self._transport:
            return
    ...
    async def _handle_store(self, data: dict[str, Any]) -> None:
        sender = data.get("sender")
        payload = data.get("payload", {})
        if sender:
            self._update_routing(sender)
    ...
    async def _handle_find_value(self, data: dict[str, Any]) -> None:
        sender = data.get("sender")
        payload = data.get("payload", {})
        key = payload.get("key")
        rpc_id = payload.get("rpc_id")
    ...
    async def _handle_find_value_resp(self, data: dict[str, Any]) -> None:
        sender = data.get("sender")
        payload = data.get("payload", {})
        rpc_id = payload.get("rpc_id")
        if sender:
                ...
                if pid:
                    ...
                    if not ok:
                        self.routing_table[bucket_idx] = [
                            p for p in self.routing_table.get(bucket_idx, []) if p.get("id") != pid
                        ]
    ...
    async def _dht_send_get_peers(self, sock: socket.socket, host: str, port: int, info_hash: bytes) -> dict | None:
        ...
        msg = {
            ...
            "y": "q",
            "q": "get_peers",
            "a": {"id": self.node_id.encode()[:20].ljust(20, b"\x00"), "info_hash": info_hash[:20].ljust(20, b"\x00")},
        }
        try:
            loop = asyncio.get_running_loop()
            await loop.sock_sendto(sock, self._bencode(msg), (host, port))
    ...
    async def _handle_get_peers_response(
        self, response: dict, info_hash: bytes, keyword: str, results: list, seen_hashes: set
    ) -> None:
    ...
    async def _perform_bep9_handshake(
        ...
        handshake = (
            protocol + bytes(8) + info_hash[:20].ljust(20, b"\x00") + self.node_id.encode()[:20].ljust(20, b"\x00")
        )
    ...
    async def _store_dht_results(self, keyword: str, results: list) -> None:
    ...
    def _refresh_routing_from_results(self) -> None:
