...
class LocalGraphStore:
    __slots__ = tuple(('_mxg', 'bucket_id', 'db_path', 'env', 'graph', 'key_manager'))
    ...
    async def put_node(self, node_id: str, features: mx.array, neighbors: list[str]) -> None:
        ...
        def _put():
            putmulti_bounded(self.env, [(node_id.encode(), encrypted), (f'neighbors:{node_id}'.encode(), encode(neighbors[:1000]))], overwrite=True)
