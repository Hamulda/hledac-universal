...
class TorrentMetadataFetcher(Struct, frozen=True):
    ...
    async def fetch_metadata(
        self, infohash: bytes, peers: list[tuple[str, int]], timeout: float = BEP_9_TIMEOUT
    ) -> TorrentInfo | None:
    ...
    def _parse_torrent_info(self, info: dict, _raw_info: bytes) -> TorrentInfo:
        ...
        return TorrentInfo(
            name=name, files=files, total_size=total_size, piece_length=piece_length, pieces=pieces, trackers=trackers
        )
