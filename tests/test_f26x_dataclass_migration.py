...
class TestHypothesisEngineTypes:
    ...
    def test_evidence_is_msgspec_struct(self):
    ...
    def test_evidence_mutable(self):
        from brain.hypothesis_engine._types import Evidence
    ...
    def test_dark_query_is_frozen(self):
    ...
    def test_dark_query_tuple_default(self):
        from brain.hypothesis_engine._types import DarkQuery, DarkQueryType
        dq = DarkQuery(DarkQueryType.IPFS, "query", 0.5)
        assert dq.source_iocs == ()

    def test_falsification_result(self):
    ...
    def test_test_design_default_factory(self):
    ...
    def test_contradiction_timestamp_default(self):
    ...
    def test_adversarial_report_nested_structs(self):
    ...
    def test_source_credibility_still_dataclass(self):
    ...
    def test_test_result_still_dataclass(self):
...
class TestAcquisitionStrategyTypes:
    ...
    def test_lane_spec_frozen(self):
        from runtime.acquisition_strategy import LaneSpec
        spec = LaneSpec(50, 30, "low")
        with pytest.raises(AttributeError):
            spec.max_items = 100

    def test_lane_rule_is_msgspec(self):
        from runtime.acquisition_strategy import LaneRule, LaneSpec
        from runtime.acquisition_strategy import AcquisitionContext
        from runtime.acquisition_strategy import RiskLevel
    ...
    def test_acquisition_context_still_dataclass(self):
        """AcquisitionContext kept as dataclass (uses field(default=...))."""
        from dataclasses import is_dataclass
        from runtime.acquisition_strategy import AcquisitionContext
        assert is_dataclass(AcquisitionContext)

    def test_acquisition_context_field_defaults(self):
        from runtime.acquisition_strategy import AcquisitionContext
        ctx = AcquisitionContext(
            query="test", duration_s=300, aggressive_mode=False,
            uma_state="ok", swap_detected=False, hardware_critical=False,
            has_domain=True, has_url=False, has_crypto=False,
    ...
    def test_feed_dominance_budget_still_msgspec(self):
    ...
    def test_feed_dominance_budget_methods(self):
        from runtime.acquisition_strategy import FeedDominanceBudget
