...
class TestPyClassSlots:
    ...
    def test_all_classes_have_name(self, manifest) -> None:
    ...
    def test_all_classes_have_module(self, manifest) -> None:
        """Every PyClass should have a module."""
        for full_name, class_data in manifest["classes"].items():
            assert "module" in class_data, f"Class {full_name} missing 'module'"

    def test_field_types_are_valid(self, manifest) -> None:
    ...
    def test_get_set_consistency(self, manifest) -> None:
            ...
            for field in class_data.get("fields", []):
                if field.get("has_get"):
                    assert field["name"] in slots, (
                        f"Field {field['name']} in {full_name} has has_get=True but is not in slots list"
                    )
...
class TestFunctionSignatures:
    ...
    def test_all_functions_have_name(self, manifest) -> None:
...
class TestPyiGeneration:
    ...
    def test_pyi_exists(self) -> None:
        """PYI stub should exist after manifest generation."""
        assert _PYI_PATH.exists(), (
            f"PYI stub not found at {_PYI_PATH}. Run: python rust_extensions/ffi_type_manifest.py"
        )

    def test_pyi_header_comments(self, pyi_content) -> None:
    ...
    def test_pyi_has_typing_imports(self, pyi_content) -> None:
        """PYI should have typing imports."""
        assert "from typing import" in pyi_content
        assert "Any" in pyi_content

    def test_pyi_class_definitions(self, pyi_content, manifest) -> None:
        """PYI should contain class definitions for all PyClasses."""
        classes = manifest.get("classes", {})

        for _full_name, class_data in classes.items():
            class_name = class_data["name"]
            assert f"class {class_name}:" in pyi_content, f"Class {class_name} not found in .pyi stub"

    def test_pyi_function_definitions(self, pyi_content, manifest) -> None:
        ...
        for _full_name, func_data in sample.items():
...
class TestBuildIntegration:
    ...
    def test_ffi_type_manifest_runs(self) -> None:
        ...
        assert result.returncode == 0, f"ffi_type_manifest.py failed:\nstdout: {result.stdout}\nstderr: {result.stderr}"

    def test_stub_validator_runs(self) -> None:
...
class TestRuntimeSlotsValidation:
    ...
    def test_known_classes_exist(self, manifest) -> None:
        ...
        try:
            ...
            for full_name in list(manifest["classes"].keys())[:5]:
                ...
                assert hasattr(ext, class_name), f"Class {class_name} not found in compiled extension"
        except ImportError:
    ...
    def test_known_functions_exist(self, manifest) -> None:
        ...
        try:
            ...
            for full_name in list(manifest["functions"].keys())[:10]:
                ...
                assert hasattr(ext, func_name), f"Function {func_name} not found in compiled extension"
        except ImportError:
            pytest.skip("Extension not available")
...
class TestTypeCompatibility:
    ...
    def test_no_unknown_types_in_manifest(self, manifest) -> None:
        ...
        assert len(unknown_types) == 0, f"Found fields with unknown types: {unknown_types}"

    def test_validation_rules_present(self, manifest) -> None:
...
class TestCLI:
    ...
    def test_validator_cli_help(self) -> None:
        ...
        result = subprocess.run(
            [sys.executable, str(_RUST_EXT / "stub_validator.py"), "--help"], capture_output=True, text=True, timeout=10
        )
...
class TestMaturinHook:
    ...
    def test_pyproject_has_develop_hook(self) -> None:
        """pyproject.toml should have develop-hooks configured."""
        pyproject = _RUST_EXT / "pyproject.toml"
        content = pyproject.read_text()

        assert "develop-hooks" in content, "pyproject.toml missing [tool.maturin.develop-hooks]"
        assert "ffi_validate" in content, "pyproject.toml missing ffi_validate hook"

    def test_hook_command_correct(self) -> None:
