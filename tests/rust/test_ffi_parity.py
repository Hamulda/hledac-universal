...
class TestFFIParity:
    ...
    def test_manifest_exists(self) -> None:
        ...
        assert ABI_MANIFEST.exists(), (
            f"Manifest not found at {ABI_MANIFEST}. Run: python rust_extensions/build_ffi_manifest.py"
        )
    ...
    def test_no_class_name_drift(self) -> None:
        ...
        class_names = {name for name in specs if name[0].isupper() and "_" not in name}
    ...
    def test_register_functions_coverage(self) -> None:
        ...
        if missing_modules:
            ...
            pytest.fail(
                ...
                + "\n".join(f"  {mod}: {', '.join(funcs)}" for mod, funcs in sorted(by_domain.items()))
            )
