...
class TestF271CWindupLead:
    ...
    def test_f250_clamp_produces_non_negative_active_window(
        ...
        effective_windup_s = float(
            max(_F250_WINDUP_CLAMP_MIN_S, min(_F250_WINDUP_CLAMP_MAX_S, raw_windup))
        )
        active_window_s = float(duration_s) - effective_windup_s

        # F271C: hard invariant
        assert active_window_s >= 0.0, (
            f"F271C INVARIANT VIOLATED: active_window_s={active_window_s} "
        ...
        for duration_s in (30, 45, 60, 90, 120, 180, 240, 600, 1800, 7200):
            ...
            if active < MIN_ACTIVE_WINDOW_S:
                assert duration_s < 60, (
                    f"duration={duration_s} should abort (active={active})"
                )
                    ...
                    for kw in node.keywords:
                        if kw.arg == "timeout":
                            if (
                                ...
                                and kw.value.value == 35.0
                            ):
                                found_timeout_35 = True
                    found_wait_for = True
        assert found_wait_for, (
            "F271B: pipeline/live_public_pipeline.py must use asyncio.wait_for "
            "to bound discovery coroutine"
        )
        assert found_timeout_35, (
            "F271B: asyncio.wait_for must use timeout=35.0 to match the "
            "classify_discovery_error contract"
        )

    def test_discovery_does_not_raise_on_outer_timeout(self) -> None:
...
class TestF271EStaleText:
    ...
    def test_recommended_action_no_longer_mentions_extract_runtime_timing(self) -> None:
        ...
        assert "_extract_runtime_timing" not in action, (
            f"F271E: stale reference still present: {action!r}"
        )
...
class TestF271FEagerStartUloop:
    ...
    def test_detection_respects_active_loop_policy(self) -> None:
        ...
        try:
            assert ah._EAGER_START_SUPPORTED is False, (
                "F271F: with uvloop installed, eager_start must be False "
                f"(got {ah._EAGER_START_SUPPORTED})"
            )
        finally:
    ...
    def test_export_dir_forwarded_by_root_dispatcher(self) -> None:
        ...
        assert '"--export-dir"' in src, (
            "F271F: root __main__.py parser must expose --export-dir"
        )
        ...
        assert "export_dir=" in branch, (
            "F271F: --export-dir must be forwarded inside the sprint "
            "branch (not only in dry-run)"
        )
