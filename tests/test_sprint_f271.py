...
class TestF271CWindupLead:
    ...
    @pytest.mark.parametrize('duration_s,expected_windup,expected_active', [(60, 30.0, 30.0), (90, 30.0, 60.0), (180, 54.0, 126.0), (1800, 180.0, 1620.0)])
    def test_f250_clamp_produces_non_negative_active_window(self, duration_s: int, expected_windup: float, expected_active: float) -> None:
        ...
        _F250_WINDUP_LEAD_FRAC: float = 0.3
        ...
        assert active_window_s >= 0.0, f'F271C INVARIANT VIOLATED: active_window_s={active_window_s} for duration_s={duration_s}'
...
class TestF271DGuardMinLanes:
    ...
    def test_guard_breaks_only_when_two_or_more_lanes_have_entries(self) -> None:
        ...
        assert len(entries_a) < _MIN_LANES_FOR_EARLY_WINDUP, '1 lane with entries should NOT satisfy early windup threshold'
        entries_b: dict[str, int] = {'ct': 12, 'public': 5}
        assert len(entries_b) >= _MIN_LANES_FOR_EARLY_WINDUP, '2 lanes with entries should satisfy early windup threshold'
...
class TestF271ARelationshipImport:
    ...
    def test_relationship_not_required_in_graph_service(self) -> None:
        ...
        assert hasattr(graph_service, 'upsert_relation'), 'graph_service must export upsert_relation() for F271A callback'
    ...
    def test_upsert_relation_signature_compatible(self) -> None:
        ...
        try:
            ...
            upsert('f271a-test-src', 'f271a-test-dst', 'f271a_test', weight=0.5, evidence='f271a_unit_test')
...
class TestF271BCoroutineLeak:
    ...
    def test_discovery_uses_asyncio_wait_for(self) -> None:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                func = node.func
                if isinstance(func, ast.Name) and func.id == 'safe_wait_for':
                    for kw in node.keywords:
                        if kw.arg == 'timeout':
                            if isinstance(kw.value, ast.Constant) and kw.value.value == 35.0:
                                found_timeout_35 = True
                    found_wait_for = True
        assert found_wait_for, 'F271B: pipeline/live_public_pipeline.py must use safe_wait_for to bound discovery coroutine'
        assert found_timeout_35, 'F271B: safe_wait_for must use timeout=35.0 to match the classify_discovery_error contract'
    ...
    def test_discovery_does_not_raise_on_outer_timeout(self, session_event_loop: asyncio.AbstractEventLoop) -> None:
        ...
        assert captured.get('caught') == 'TimeoutError', f"F271B: outer must catch TimeoutError, got {captured.get('caught')}"
...
class TestF271EStaleText:
    ...
    def test_recommended_action_no_longer_mentions_extract_runtime_timing(self) -> None:
        from hledac.universal.export.components.signal_builder import _compute_runtime_diagnosis
        ...
        assert '_build_signals_dict' in action or '_finalize_result_truth' in action, f'F271E: recommended_action must point to canonical seam, got: {action!r}'
    ...
    def test_root_cause_includes_payload_type_name(self) -> None:
        ...
        assert 'list' in d_list['root_cause'], f"root_cause must include 'list' for list payload, got: {d_list['root_cause']!r}"
...
class TestF271FEagerStartUloop:
    ...
    def test_detection_respects_active_loop_policy(self) -> None:
        ...
        importlib.reload(ah)
        try:
            assert ah._EAGER_START_SUPPORTED is False, f'F271F: with uvloop installed, eager_start must be False (got {ah._EAGER_START_SUPPORTED})'
        finally:
            import asyncio as _asyncio
            _asyncio.set_event_loop_policy(_asyncio.DefaultEventLoopPolicy())
            importlib.reload(ah)

    def test_safe_gather_dropin_under_uvloop_does_not_raise(self, session_event_loop: asyncio.AbstractEventLoop) -> None:
        ...
        try:
            ...
            async def main() -> None:
                ...
                result = await ah.parallel_ok(*coros, label='f271f-uvloop')
    ...
    def test_export_dir_forwarded_by_root_dispatcher(self) -> None:
        ...
        assert 'export_dir=' in src, 'F271F: root __main__.py dispatcher must forward export_dir to core run_sprint (regression: was dropped silently)'
        ...
        assert 'export_dir=' in branch, 'F271F: --export-dir must be forwarded inside the sprint branch (not only in dry-run)'