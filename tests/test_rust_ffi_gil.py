...
class TestRustFFIGILRelease:
    ...
    async def test_gil_not_held_during_ffi(self) -> None:
        ...
        assert progress["python_task_ran"], "Python task did not run - GIL may not have been released"
        ...
        assert python_duration < ffi_duration * 0.8, f"Python task took {python_duration:.3f}s (sequential execution?)"
    ...
    async def test_concurrent_ffi_calls(self) -> None:
        ...
        results = await asyncio.gather(*[mock_ffi_call(i) for i in range(10)])
