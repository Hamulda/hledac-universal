...
class TestDspyEvidenceSeam:
    ...
    @pytest.mark.asyncio
    async def test_load_training_examples_file_reading(self, mock_evidence_dir) -> None:
    ...
    @pytest.mark.asyncio
    async def test_load_training_examples_graceful_skip_empty_file(self, tmp_path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_filter_training_examples_error_results(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_filter_training_examples_valid_inputs(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_optimization_no_crash_on_zero_examples(self) -> None:
        """_run_optimization fails gracefully when 0 examples after filtering."""
        from hledac.universal.brain.dspy_optimizer import DSPyOptimizer

        optimizer = DSPyOptimizer(brain_manager=MagicMock())

        with patch.object(optimizer, "_load_training_examples", return_value=[]):
            await optimizer._run_optimization()

        assert optimizer._failure_count == 0

    @pytest.mark.asyncio
    async def test_run_optimization_circuit_breaker_on_dspy_failure(self) -> None:
        ...
        with (
            patch.object(optimizer, "_load_training_examples", return_value=valid_examples),
            patch.object(optimizer, "_dspy_optimize_mipro", side_effect=RuntimeError("DSPy MIPROv2 failed")),
        ):
            await optimizer._run_optimization()
        ...
        assert time.time() < optimizer._circuit_open_until, "Circuit breaker should be open"
