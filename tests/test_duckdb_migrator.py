...
class TestSchemaMigratorFreshDB:
    ...
    def test_migrate_applies_all_migrations(self, temp_db_path: Path, duckdb_conn) -> None:

        m = SchemaMigrator(duckdb_conn)

        applied = m.migrate()

        assert applied == _CURRENT_SCHEMA_VERSION, f"Expected {_CURRENT_SCHEMA_VERSION} migrations, got {applied}"

        # Verify schema_version was recorded
        rows = duckdb_conn.execute("SELECT version, description FROM schema_version ORDER BY version").fetchall()
        ...
        indexes = duckdb_conn.execute(
            "SELECT indexname FROM pg_indexes WHERE tablename = 'canonical_findings'"
        ).fetchall()
        index_names = {r[0] for r in indexes}
        assert "idx_canonical_findings_ts" in index_names

    def test_migrate_idempotent_second_call(self, temp_db_path: Path, duckdb_conn) -> None:
        import duckdb
...
class TestSchemaMigratorLegacyDB:
    ...
    def test_legacy_db_bootstraps_version_1(self, temp_db_path: Path, duckdb_conn) -> None:
        import duckdb
        ...
        duckdb_conn.execute("CREATE TABLE IF NOT EXISTS sprint_delta (sprint_id TEXT PRIMARY KEY, ts DOUBLE)")
        duckdb_conn.commit()
        ...
        assert 1 in applied_versions, f"Expected version 1 bootstrapped for legacy DB, got {applied_versions}"
    ...
    def test_legacy_db_sprint_delta_alter_columns(self, temp_db_path: Path, duckdb_conn) -> None:
        import duckdb
        ...
        cols = conn2.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_name = 'sprint_delta'"
        ).fetchall()
...
class TestSchemaMigratorCurrentDB:
    ...
    def test_current_db_no_migrations_applied(self, temp_db_path: Path, duckdb_conn) -> None:
        import duckdb
        ...
        duckdb_conn.execute("INSERT INTO schema_version VALUES (1, 0.0, 'legacy')")
        duckdb_conn.execute("INSERT INTO schema_version VALUES (2, 0.0, 'legacy')")
        duckdb_conn.execute("CREATE TABLE IF NOT EXISTS canonical_findings (id VARCHAR PRIMARY KEY)")
        duckdb_conn.commit()
        duckdb_conn.close()
