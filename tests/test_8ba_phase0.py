...
class TestSprint8BAPhase0:
    ...
    def test_stealth_request_deleted_or_explicitly_live(self) -> None:
        ...
        assert not os.path.exists(path), f"stealth_request.py should not exist at {path}"

    def test_basepolicy_score_url_is_abstract_if_live(self) -> None:
        """Verify BasePolicy.score_url is @abc.abstractmethod."""
        from tools.policies import BasePolicy

        # BasePolicy must inherit from ABC
        assert issubclass(BasePolicy, abc.ABC), "BasePolicy must inherit from abc.ABC"
        # score_url must be abstract
        assert "score_url" in BasePolicy.__abstractmethods__, "score_url must be in __abstractmethods__"
        # Cannot instantiate abstract class
        with pytest.raises(TypeError):
            BasePolicy("test")

    def test_basepolicy_concrete_implementations_work(self) -> None:
        ...
        for url, policy_cls, expected_min in test_urls:
            ...
            assert score >= expected_min, f"{policy_cls.__name__} score for {url} should be >= {expected_min}"

    def test_lock_audit_result_is_explicit(self) -> None:
    ...
    def test_no_remaining_eager_scipy_networkx_in_owned_files(self) -> None:
    ...
    def test_module_level_mutable_state_audit_is_explicit(self) -> None:
                    ...
                    for target in node.targets:
                        if isinstance(target, ast.Name):
                            ...
                            if val_src.strip().startswith(("[", "{")):
                                if not val_src.strip().startswith(
                                    ("{[", "{{")
                                ):  # Allow dict/set literals for some cases  # noqa: E501
                                    pass  # These are typically tuples converted to frozenset or similar

    def test_import_baseline_unchanged(self) -> None:
