...
@pytest.mark.asyncio
async def test_execute_research_plan_parallel_vs_serial() -> None:
    """
    ...
    AsyncMock(return_value={"source": "academic", "results": []})
    AsyncMock(return_value={"source": "archive", "results": []})
    AsyncMock(return_value={"source": "crawl", "results": []})
    ...
    with (
        patch.object(cls, "search_academic", mock_search_academic),
...
@pytest.mark.asyncio
async def test_execute_research_plan_exception_isolation() -> None:
    """
