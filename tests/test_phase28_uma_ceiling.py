...
class TestUmaBudgetSSOT:
    ...
    def test_uma_hard_ceiling_is_class_variable(self):
...
class TestDerivedConstants:
    ...
    def test_total_gib_is_alias(self):
    ...
    def test_tracked_allocation_budget(self):
    ...
    def test_mission_peak_rss_derived(self):
        ...
        assert actual == expected, \
            f"MISSION_PEAK_RSS_GIB should be {expected} (6.25 * 0.88), got {actual}"

    def test_threshold_constants_are_derived(self):
    ...
    def test_threshold_ratios_are_consistent(self):
...
class TestM1FetchCeiling:
    ...
    def test_m1_fetch_soft_ceiling_matches_mission_peak(self):
    ...
    def test_module_level_uma_hard_ceiling(self):
        """Module-level UMA_HARD_CEILING_GIB must match UmaBudget.UMA_HARD_CEILING_GIB."""
        from utils.uma_budget import UmaBudget, UMA_HARD_CEILING_GIB
...
class TestNoHardcodedCeilings:
    ...
    def test_no_hardcoded_6_25_in_coordinators(self):
        ...
        filtered_violations = [
            ...
            if "uma_budget.py" not in v
            and "test_" not in v
        ]

        assert len(filtered_violations) == 0, \
            f"Hardcoded 6.25 found outside UmaBudget:\n" + "\n".join(filtered_violations)

    def test_no_hardcoded_5_5_in_uma_context(self):
...
class TestM1Optimization:
    ...
    def test_gc_false_on_budget_classes(self):
    ...
    def test_uma_budget_is_msgspec_compatible(self):
