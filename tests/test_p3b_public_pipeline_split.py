...
class TestPublicConstants:
    ...
    def test_is_threat_query(self, query, expected) -> None:
        assert _is_threat_query(query) == expected, f"query={query!r}"

    @pytest.mark.parametrize(
        "url,is_threat,expected_noise",
        [
            # Shopping domains
    ...
    def test_is_shopping_noise_url(self, url, is_threat, expected_noise) -> None:
...
class TestPublicPatterns:
    ...
    def test_make_finding_id_deterministic(self) -> None:
    ...
    def test_make_finding_id_different_inputs(self) -> None:
    ...
    def test_html_to_text(self, html, expected) -> None:
        result = _html_to_text(html)
        assert result == expected, f"html={html!r}"

    def test_score_page_quality_very_good(self) -> None:
    ...
    def test_score_page_quality_weak(self) -> None:
        tier = _score_page_quality(
    ...
    def test_js_confidence_from_verdict(self) -> None:
    ...
    def test_enrich_text_with_metadata(self) -> None:
    ...
    def test_pattern_context(self) -> None:
...
class TestPublicAcceptance:
    ...
    @pytest.mark.parametrize(
        "query,expected_tuple",
        [
            ("", ()),  # empty query → no finding
        ],
    )
    def test_build_public_finding_empty(self, query, expected_tuple) -> None:
...
class TestPipelineStructs:
    ...
    def test_pipeline_page_result_fields(self) -> None:
...
class TestP3BParityInvariant:
    ...
    def test_is_threat_query_parity(self) -> None:
    ...
    def test_shopping_noise_parity(self) -> None:
        ...
        for url, is_threat, expected_noise, expected_reason in test_cases:
            is_noise, reason = _is_shopping_noise_url(url, is_threat)
            assert is_noise == expected_noise, (
                f"Parity: noise({url!r}, threat={is_threat}) = {is_noise}, expected {expected_noise}"
            )
            assert reason == expected_reason, f"Parity: reason({url!r}) = {reason!r}, expected {expected_reason!r}"

    def test_filter_public_noise_parity(self) -> None:
