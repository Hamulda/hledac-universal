...
class TestToolSchemaValidation:
    ...
    def registry(self):
        ...
        reg.register_with_schema(name="web_search", handler=lambda **kwargs: {"results": []}, schema=WebSearchArgs)
        reg.register_with_schema(
            name="entity_extract", handler=lambda **kwargs: {"entities": []}, schema=EntityExtractArgs
        )

        return reg

    def test_valid_args_pass(self, registry) -> None:
    ...
    def test_invalid_args_fail_fast(self, registry) -> None:
    ...
    def test_missing_required_field(self, registry) -> None:
    ...
    def test_pattern_validation(self, registry) -> None:
    ...
    def test_unknown_tool_fails(self, registry) -> None:
...
class TestToolExecutionFlow:
    ...
    def orchestrator(self):
        ...
        registry.register_with_schema(
            name="web_search", handler=lambda **kwargs: {"results": ["doc1", "doc2"]}, schema=WebSearchArgs
        )

        return FullyAutonomousOrchestrator(hermes=hermes, tools=registry)

    @pytest.mark.asyncio
    async def test_valid_plan_executes(self, orchestrator) -> None:
    ...
    @pytest.mark.asyncio
    async def test_invalid_args_blocked(self, orchestrator) -> None:
        """Nevalidní argumenty → tool se nezavolá, chyba v evidenci."""
        orchestrator.hermes.generate = Mock(
            return_value={
...
class TestSchemaDefinition:
    ...
    def test_tool_definition_creation(self) -> None:
        ...
        tool_def = ToolDefinition(name="test_tool", handler=handler, schema=WebSearchArgs, description="Test tool")
...
class TestSchemaEdgeCases:
    ...
    def test_empty_args_for_optional_only(self, registry) -> None:
        ...
        reg = ToolRegistry()
        reg.register_with_schema(name="all_optional", handler=lambda **kwargs: {"ok": True}, schema=AllOptional)
    ...
    def test_nested_model_validation(self) -> None:
        ...
        reg = ToolRegistry()
        reg.register_with_schema(name="create_person", handler=lambda **kwargs: {"created": True}, schema=Person)

        # Validní vnořený model
        reg.validate_args("create_person", {"name": "John", "address": {"street": "Main St", "city": "NYC"}})
    ...
    def test_list_validation(self) -> None:
        ...
        reg = ToolRegistry()
        reg.register_with_schema(name="with_list", handler=lambda **kwargs: {"ok": True}, schema=WithList)
    ...
    def test_union_types(self) -> None:
        ...
        reg = ToolRegistry()
        reg.register_with_schema(name="with_union", handler=lambda **kwargs: {"ok": True}, schema=WithUnion)
...
class TestSchemaDocumentation:
    ...
    def test_description_extraction(self) -> None:
        ...
        class DocumentedArgs(BaseModel):
            ...
            query: str = Field(..., description="Vyhledávací dotaz", examples=["python tutorial"])

        reg = ToolRegistry()
        reg.register_with_schema(name="documented", handler=lambda **kwargs: {"ok": True}, schema=DocumentedArgs)

        schema = reg.get_schema("documented")
        assert "query" in schema["properties"]

    def test_examples_in_schema(self) -> None:
