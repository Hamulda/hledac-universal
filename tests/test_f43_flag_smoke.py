...
class TestF43FlagSmoke:
    ...
    def test_runner_discovers_real_flags(self) -> None:
    ...
    def test_runner_categorizes_known_pass_flag(self) -> None:
        ...
        assert "PASS" in result.stdout, (
            f"expected PASS, got rc={result.returncode}; stdout={result.stdout!r}; stderr={result.stderr!r}"
        )
        assert "HLEDAC_ENABLE_DSPY" in result.stdout

    def test_runner_rejects_nonexistent_flag(self) -> None:
    ...
    def test_runner_regex_accepts_real_flag(self) -> None:
    ...
    def test_runner_clean_env_after_check(self) -> None:
