...
class TestMoELoadAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_mlx_worker_does_not_freeze_event_loop(self) -> None:
        """
    ...
    async def test_mlx_worker_concurrent_load_serialized(self) -> None:
        """
        ...
        """
        from hledac.universal._core.mlx_inference_lock import _get_mlx_worker
    ...
    async def test_run_in_mlx_worker_api(self) -> None:
        ...
        # Non-blocking call
        result = await asyncio.wait_for(run_in_mlx_worker(dummy_sync_op, 10, 20), timeout=2.0)
        self.assertEqual(result, 30)
        print("[M-04 API] run_in_mlx_worker() funguje správně")

    async def test_mlx_worker_is_active(self) -> None:
