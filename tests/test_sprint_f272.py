...
class TestF272BProductionPreflight:
    ...
    def test_production_flag_parsed_by_argparse(self):
...
class TestF272CLaneNameNormalization:
    ...
    def test_canonical_lane_name_uppercases_strings(self):
        assert canonical_lane_name("public") == "PUBLIC"
        assert canonical_lane_name("Public") == "PUBLIC"
        assert canonical_lane_name("PUBLIC") == "PUBLIC"

    def test_canonical_lane_name_handles_enum(self):
        """Enum values must be unwrapped before uppercase."""
        fake_enum = MagicMock()
        fake_enum.value = "i2p"
        assert canonical_lane_name(fake_enum) == "I2P"

    def test_canonical_lane_name_handles_non_string(self):
    ...
    def test_normalization_mixed_input_output_uniform(self):
...
class TestF272DAdvisoryLruDedup:
    ...
    def test_100_duplicates_only_emit_once(self):
    ...
    def test_lru_evicts_oldest_at_16_keys(self):
    ...
    def test_lru_hit_does_not_evict(self):
    ...
    def test_different_keys_each_emit_once(self):
        log = MagicMock()
        for i in range(5):
            _log_advisory_dedup(log, f"k{i}", "[T] %s", i)
...
class TestF272EMaxCyclesAdaptive:
    ...
    def test_ema_state_lazy_init_defaults(self):
    ...
    def test_ema_clamps_pathological_outliers(self):
    ...
    def test_effective_max_cycles_floor_at_50(self):
    ...
    def test_effective_max_cycles_ceiling_at_300(self):
    ...
    def test_typical_60s_sprint_yields_45_to_450_cycles(self):
...
class TestF272GHealthcheckNotInitialized:
    ...
    def test_summary_reports_not_initialized_when_fetch_down(self):
    ...
    def test_summary_full_string_contains_no_na(self):
        """Hard invariant: no 'NA' substring anywhere in summary output."""
        hr = HealthReport(
            duckdb_ok=False, hermes_ok=False,
            fetch_coordinator_ok=False, graph_service_ok=False,
        )
        s = hr.summary()
...
class TestF272CrossCutting:
    ...
    def test_all_fixes_fail_soft(self):
