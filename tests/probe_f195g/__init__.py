...
class TestF195G_Shodan_CanonicalFindings:
    ...
    def test_f195g_search_shodan_to_findings_returns_tuple(self):
        ...
        # Mock search_shodan to return synthetic results
        mock_raw = [
            {
                "ip": "1.2.3.4",
                "port": 80,
        ...
        with patch("hledac.universal.intelligence.shodan_wrapper.search_shodan",
            ...
            findings, raw_results = search_shodan_to_findings("apache", limit=10)
    ...
    def test_f195g_shodan_finding_source_type_contract(self):
        ...
        with patch("hledac.universal.intelligence.shodan_wrapper.search_shodan",
                   new=AsyncMock(return_value=mock_raw)):
            findings, _ = search_shodan_to_findings("apache", limit=10)
    ...
    def test_f195g_shodan_finding_confidence_richness(self):
        """F195G-1c: Confidence scales with banner richness."""
        from hledac.universal.intelligence.shodan_wrapper import search_shodan_to_findings

        # Short banner → lower confidence
        with patch("hledac.universal.intelligence.shodan_wrapper.search_shodan",
                   new=AsyncMock(return_value=[{"ip": "1.2.3.4", "port": 80, "banner": "short", "hostnames": []}])):
            findings, _ = search_shodan_to_findings("apache", limit=10)
        assert findings[0].confidence == 0.65

        # Long banner → higher confidence
        with patch("hledac.universal.intelligence.shodan_wrapper.search_shodan",
                   new=AsyncMock(return_value=[{"ip": "1.2.3.4", "port": 80, "banner": "x" * 200, "hostnames": []}])):
            findings, _ = search_shodan_to_findings("apache", limit=10)
        assert findings[0].confidence == 0.75

        # Banner + hostname → highest
        with patch("hledac.universal.intelligence.shodan_wrapper.search_shodan",
                   new=AsyncMock(return_value=[{"ip": "1.2.3.4", "port": 80, "banner": "x" * 200, "hostnames": ["ex.com"]}])):
            findings, _ = search_shodan_to_findings("apache", limit=10)
        assert findings[0].confidence == 0.80

        # Common service ports with banner → highest tier
        with patch("hledac.universal.intelligence.shodan_wrapper.search_shodan",
                   new=AsyncMock(return_value=[{"ip": "1.2.3.4", "port": 443, "banner": "TLS banner", "hostnames": []}])):
            findings, _ = search_shodan_to_findings("apache", limit=10)
...
class TestF195G_BGP_CanonicalFindings:
    ...
    def test_f195g_bgp_handler_graceful_fallback_no_bgp_available(self, mock_scheduler):
        """F195G-4: bgp_routing_history degrades gracefully when BGP_AVAILABLE=False."""
        from hledac.universal.discovery.ti_feed_adapter import _handle_bgp_routing_history

        with patch("hledac.universal.network.bgp_monitor", None):
            with patch.dict("hledac.universal.discovery.ti_feed_adapter.__dict__",
                            {"BGP_AVAILABLE": False}, clear=False):
                # Re-import to pick up the patched BGP_AVAILABLE
                import importlib
                import hledac.universal.discovery.ti_feed_adapter as tfa
                importlib.reload(tfa)

                # Mock query_bgp_routing_history to return history
                with patch.object(tfa, "query_bgp_routing_history",
                                  new=AsyncMock(return_value={"resource": "1.2.3.4", "history": [{"ts": 123}]})):
                    task = FakePivotTask(task_type="bgp_routing_history", ioc_value="1.2.3.4")
                    # This should not raise even without pybgpstream
                    # Note: full test requires careful mocking of the ImportError path
    ...
    async def test_f195g_bgp_routing_history_pivots_and_persists(self, mock_scheduler):
        ...
        with patch("hledac.universal.discovery.ti_feed_adapter.query_bgp_routing_history",
            ...
            # Mock monitor_bgp to return events
            mock_events = [
            ...
            with patch("hledac.universal.discovery.ti_feed_adapter.monitor_bgp",
