...
class TestDeepProbeInvariants:
    ...
    def test_max_bucket_scan_is_50(self) -> None:
...
class TestDeepProbeRunnerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_run_deep_probe_returns_result_dict(self, mock_store, mock_scanner) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_deep_probe_stores_with_correct_source_type(self, mock_store, mock_scanner) -> None:
        """invariant_1: findings stored via store have source_type='deep_probe'."""
        call_records = []

        async def mock_record_finding(finding_id, query, source_type, confidence) -> bool:
            call_records.append(
        ...
        with patch("hledac.universal.deep_probe.DeepProbeScanner", return_value=mock_scanner):
            with patch("hledac.universal.deep_probe.scan_ipfs", AsyncMock(return_value=[])):
                ...
                for record in call_records:
                    assert record["source_type"] == "deep_probe", (
                        f"Expected 'deep_probe' but got {record['source_type']}"
                    )  # noqa: E501

    @pytest.mark.asyncio
    async def test_run_deep_probe_is_fail_safe_on_scanner_error(self, mock_store) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_deep_probe_timeout_is_bounded(self, mock_store) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_deep_probe_if_enabled_returns_none_when_disabled(self, mock_store) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_deep_probe_if_enabled_runs_when_enabled(self, mock_store, mock_scanner) -> None:
...
class TestDeepProbeExportNonBlocking:
    ...
    @pytest.mark.asyncio
    async def test_deep_probe_called_after_export_completes(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_deep_probe_does_not_block_export(self) -> None:
...
class TestDeepProbeProbeRunnerImports:
    ...
    def test_probe_runner_imports_deep_probe(self) -> None:
    ...
    def test_deep_probe_exports_correct_api(self) -> None:
    ...
    def test_generate_ipfs_dorks_returns_list_of_strings(self) -> None:
