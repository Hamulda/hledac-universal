"""
Unit tests for BatchScheduler — pure asyncio, no MLX/GPU.

Tests cover:
- Queue initialization and startup
- Priority ordering (priority=0 bypasses)
- Schema boundary segregation
- Prompt hash boundary segregation
- Length bin boundary segregation
- Age bump anti-starvation
- Adaptive flush interval (3-tier)
- Batch max size limit
- Shutdown (poison pill + pending futures)
- is_batch_safe routing
- Telemetry EMA + counters
...
class TestBatchSchedulerBasics(unittest.TestCase):
    ...
    def test_zero_mlx_imports(self):
        ...
        import brain.batch_scheduler as bs
