...
async def stream_to_thread(
    source: AsyncGenerator[Any], func: Callable[[Any], Any], *, max_concurrent: int = 4
) -> AsyncGenerator[Any]:
...
async def export_sprint_streaming(
    ...
    try:
        ...
        ioc_row_count = 0
        async with _aclose(
            stream_ioc_table_section(_get_findings_with_iocs(store, handoff), max_rows=max_ioc_rows, chunk_size=50)
        ) as ioc_gen:
    ...
    try:
        ...
        async with _aclose(
            stream_graph_viz_section(
                _get_graph_manager(store, handoff), max_nodes=max_graph_nodes, max_edges=max_graph_edges
            )
        ) as graph_gen:
