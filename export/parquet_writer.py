...
class AsyncParquetStreamingReader:
    ...
    async def stream_batches(self, path: str | Path, *, batch_size: int=4096, columns: list[str] | None=None) -> AsyncIterator[Any]:
        """
            ...
            for file_path in files:
                try:
                    ...
                    for rg_idx in range(num_row_groups):
                        batch: Any = await loop.run_in_executor(None, lambda: pf.read_row_group(rg_idx, columns=columns).to_batches(batch_size)[0])
                        yield batch
...
def _check_duckdb_available() -> bool:
    try:
        ...
        return False
_PARQUET_SCHEMA: list[tuple[str, str]] = [('id', 'string'), ('query', 'string'), ('source_type', 'string'), ('confidence', 'float64'), ('ts', 'float64'), ('provenance_json', 'string'), ('payload_text', 'string'), ('claims_json', 'string')]

def _get_pyarrow_schema() -> Any:
    """Get PyArrow schema for Parquet writing (lazy import)."""
    import pyarrow as pa
    return pa.schema([('id', pa.string()), ('query', pa.string()), ('source_type', pa.string()), ('confidence', pa.float64()), ('ts', pa.float64()), ('provenance_json', pa.string()), ('payload_text', pa.string()), ('claims_json', pa.string())])
...
class ParquetExporter:
    ...
    async def _export_via_duckdb_copy(self, findings: list['CanonicalFinding'], output_path: Path, compression: str='zstd') -> Path | None:
        """
        ...
        try:
            ...
            rows = []
            for f in findings:
                rows.append((f.finding_id, f.query, f.source_type, f.confidence, f.ts or 0.0, orjson.dumps(list(f.provenance)).decode('utf-8') if f.provenance else '[]', f.payload_text or '', getattr(f, 'claims_json', None) or '[]'))
            ...
            if self._pa is not None:
                ...
                batch = self._pa.record_batch([arr_id, arr_query, arr_st, arr_conf, arr_ts, arr_prov, arr_payload, arr_claims], names=['id', 'query', 'source_type', 'confidence', 'ts', 'provenance_json', 'payload_text', 'claims_json'])
    ...
    async def _export_via_rust_arrow(self, findings: list['CanonicalFinding'], output_path: Path, compression: str='zstd') -> Path | None:
        """
        ...
        try:
            ...
            for f in findings:
                findings_dicts.append({'id': f.finding_id, 'query': f.query, 'source_type': f.source_type, 'confidence': f.confidence, 'ts': f.ts or 0.0, 'provenance_json': __import__('orjson').dumps(list(f.provenance)).decode('utf-8') if f.provenance else '[]', 'payload_text': f.payload_text or '', 'claims_json': getattr(f, 'claims_json', None) or '[]'})
            try:
    ...
    async def _export_via_polars(self, findings: list['CanonicalFinding'], output_path: Path, compression: str='zstd') -> Path | None:
        """
    ...
    async def export_findings(self, findings: list['CanonicalFinding'], output_dir: Path, filename_base: str='findings', compression: str='zstd') -> list[Path]:
        """
    ...
    async def export_from_duckdb_query(self, query: str, output_path: Path, duckdb_path: Path | str | None=None, compression: str='zstd') -> Path | None:
        """
...
async def export_findings_parquet(findings: list['CanonicalFinding'], output_dir: Path, filename_base: str='findings', duckdb_path: Path | str | None=None, compression: str='zstd') -> list[Path]:
    """
    ...
    try:
        return await exporter.export_findings(findings=findings, output_dir=output_dir, filename_base=filename_base, compression=compression)
    finally:
        await exporter.aclose()

def export_parquet_to_path(findings: list['CanonicalFinding'], output_path: Path, compression: str='zstd') -> Path | None:
    """
    ...
    def _sync_export() -> Path | None:
        try:
            ...
            batch = exporter._pa.record_batch([arr_id, arr_query, arr_st, arr_conf, arr_ts, arr_prov, arr_payload, arr_claims], names=['id', 'query', 'source_type', 'confidence', 'ts', 'provenance_json', 'payload_text', 'claims_json'])
            df: object = exporter._pl.from_arrow(batch)
            if not hasattr(df, 'write_parquet'):
                return None
            data_frame = cast(Any, df)
            output_path.parent.mkdir(parents=True, exist_ok=True)
            _COMP_TABLE: dict[str, Literal['zstd', 'snappy', 'gzip', 'uncompressed']] = {'zstd': 'zstd', 'snappy': 'snappy', 'gzip': 'gzip', 'none': 'uncompressed'}
