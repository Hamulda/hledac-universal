...
class RAGOrchestrator:
    ...
    def __init__(self, *args: Any, **_kwargs: Any) -> None:
        ...
        self._backend_mode: str = "auto"  # resolved at init
    ...
    async def initialize(self) -> None:
        ...
        async with self._init_lock:
            ...
            # Primary: sqlite-vec (M1 native, zero-process)
            if self._backend_mode in ("auto", "sqlitevec"):
                try:
                    ...
                    if ok:
                        ...
                        logger.info(
                            "RAGOrchestrator: sqlite-vec primary activated "
                            "(sprint_id=%s)", self._sprint_id
                        )
                    else:
            ...
            # Fallback: LanceDB (only if RAM headroom)
            if self._backend_mode in ("auto", "lancedb") and self._sqlite_vec_store is None:
                if _has_ram_headroom(_LANCEDB_RAM_THRESHOLD_GB):
                    ...
                    errors.append(
                        f"RAM headroom < {_LANCEDB_RAM_THRESHOLD_GB}GB, "
                        "LanceDB fallback skipped"
                    )
            ...
            if self._sqlite_vec_store is None and self._lancedb_store is None:
                ...
                logger.warning(
                    "RAGOrchestrator.initialize failed: %s", self._init_error
                )
            else:
    ...
    async def research_and_answer(
        ...
        if self._sqlite_vec_store is None and self._lancedb_store is None:
            return self._empty_result(
                error=self._init_error or "no backend available",
                started=started,
            )
        ...
        # LanceDB fallback (fill gaps if sqlite-vec returned few results)
        if self._lancedb_store is not None and len(results) < _MAX_SOURCES:
            try:
                lancedb_results = await self._lancedb_store.search_similar_adaptive(
                    ...
                    query_emb=embedding,
                    top_k=min(5, _MAX_SOURCES - len(results)),
                )
                # Merge, avoiding duplicates by item_id
        ...
        if not results:
            return self._empty_result(
                error=f"no results from any backend: {'; '.join(search_errors) or 'unknown'}",
                started=started,
            )
        ...
        for r in results:
            ...
            if score < confidence_threshold:
                continue
            # Prefer metadata text, fall back to text field
            text = (
                (r.get("metadata") or {}).get("text")
                or r.get("text")
                or ""
            ).strip()[:_TOKEN_CHARS_PER_SOURCE]
            if not text:
                continue
