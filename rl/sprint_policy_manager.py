...
class SprintPolicyManager:
    ...
    __slots__ = (
        ...
        "_pending_feedback",
        "_policy_path",
        "_policy_path_explicit",
        "_q_cache_timestamp",
        "_q_value_cache",
        "_qmix_train_interval",
        "_qmix_trainer",
        "_replay_buffer",
        "_reward_history",
        "_rl_train_mode",
    ...
    def enable_training_mode(self) -> None:
        ...
        log.info(
            "[SprintPolicyManager] Training mode ENABLED — QMIX updates every %d sprints", self._qmix_train_interval
        )
    ...
    def _init_qmix(self) -> None:
        ...
        try:
            ...
            self._qmix_trainer = QMIXJointTrainer(
                agents=self._agents, mixer=mixer, target_mixer=target_mixer, gamma=0.99, tau=0.005
            )
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            if not getattr(self, "_policy_path_explicit", False):
                log.debug(
                    "[SprintPolicyManager] Default policy_path — discarding stale disk state to avoid cross-session contamination"
                )
                return
    ...
    def _compute_reward(self, result: SprintSchedulerResult) -> float:
        ...
        try:
            ...
            findings_accepted = (
                float(_fa) if isinstance(_fa, (int, float)) else float(getattr(result, "accepted_findings", 0) or 0)
            )
    ...
    def update(self, result: SprintSchedulerResult) -> None:
        ...
        if self._replay_buffer is not None and self._state_extractor is not None:
            try:
                ...
                self._replay_buffer.push(
                    state=state, actions=action_vector, reward=reward, next_state=next_state, done=False
                )
                log.debug(
                    "[SprintPolicyManager] Replay buffer size: %d, last reward=%.3f", self._replay_buffer.size, reward
                )
            except Exception as e:
        ...
        if (
            ...
            log.info(
                "[SprintPolicyManager] AUTO-WARMUP: enabling rl_train_mode=True (sprint=%d, replay=%d, min=%d)",
                self._state.sprint_sequence_number,
        ...
        if _ebm is not None:
            log.info(
                "[SprintPolicyManager] sprint=%d epistemic_bonus_mean=%.4f", self._state.sprint_sequence_number, _ebm
            )
        self._save()
    ...
    def _update_q_cache(self, batch: Any) -> None:
        ...
        try:
            ...
            if cache_states is None or self._agents is None:
                return
            self._q_value_cache = {
                _aid: float(mx.mean(_agent.q_net(cache_states)).item()) for _aid, _agent in self._agents.items()
            }
    ...
    def _run_qmix_training(self) -> None:
        ...
        now = time.monotonic()
        if (
            now - self._last_train_at < _TRAIN_COOLDOWN_S
            or self._train_steps_this_sprint >= _MAX_TRAIN_STEPS_PER_SPRINT
        ):
            return
        ...
        try:
            ...
            log.info(
                "[SprintPolicyManager] QMIX train_step %d: loss=%.4f mean_q=%.3f replay=%d cum_steps=%d step_dt=%.2fs",
                self._state.sprint_sequence_number,
                loss,
                mean_q,
                self._replay_buffer.size,
    ...
    def _accumulate_feedback(self, src_key, total_count, accepted_count) -> None:
    ...
    def get_qmix_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "epistemic_bonus_mean": sum(self._state.epistemic_strength_history[-10:])
