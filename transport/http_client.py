...
class _BackendResolver:
    ...
    def _profile_to_policy(profile: Profile, tls_profile: str):
        ...
        _map: dict[Profile, TransportPolicy] = {
            ...
            Profile.STEALTH: POLICY_STEALTH_CHROME
            if tls_profile == "chrome136"
            else POLICY_STEALTH_SAFARI
            if tls_profile in ("safari17_4", "safari")
            ...
            Profile.H3: POLICY_H3_CHROME if tls_profile == "chrome136" else POLICY_H3_SAFARI,
        }
        return _map[profile]
...
class HttpTransport:
        ...
        for attempt in range(self._retry_engine.max_retries + 1):
                ...
                if ttfb_timeout_s is not None and ttfb_timeout_s > 0:
                    try:
                        result = await safe_wait_for(
                            self._do_fetch(url, timeout_s, max_bytes, headers), timeout=ttfb_timeout_s
                        )
                    except TimeoutError:
                        result = HttpResult(
                            url=url,
            ...
            if attempt < self._retry_engine.max_retries:
                ...
                logger.debug(
                    ...
                    attempt + 1,
                    self._retry_engine.max_retries,
                    url,
                    sleep_s,
                    result.error,
                )
    ...
    async def _fetch_httpx(
        self, url: str, timeout_s: float, max_bytes: int, headers: dict[str, str], t0: float
    ) -> HttpResult:
        """Execute fetch via httpx AsyncClient."""
        client: httpx.AsyncClient = self._client
        try:
            resp = await client.get(
                url, headers=headers, timeout=timeout_s, follow_redirects=self._config.follow_redirects
            )
    ...
    async def _fetch_curl_cffi(
        ...
        from hledac.universal.transport.unified_transport import _I2P_PROXY, _TOR_PROXY, TransportKind
        ...
        try:
            resp = await session.get(
                url, headers=headers, timeout=timeout_s, proxies=proxies, follow_redirects=self._config.follow_redirects
            )
            body = resp.content[:max_bytes]
            elapsed = (time.monotonic() - t0) * 1000
            try:
    ...
    @classmethod
    async def fetch_one(
        cls, url: str, profile: str | Profile = "default", qos: str | QoS = "normal", **kwargs
    ) -> HttpResult:
