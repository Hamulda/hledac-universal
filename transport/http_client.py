...
class HttpTransportConfig(Struct, frozen=True):
...
class _RetryEngine:
    ...
    """
    __slots__ = ('_jitter', 'base_backoff_s', 'max_backoff_s', 'max_retries')

    def __init__(self, max_retries: int=2, base_backoff_s: float=0.5, max_backoff_s: float=8.0, jitter: random.Random | None=None) -> None:
    ...
    def retryable_error_patterns(self) -> tuple[str, ...]:
        return ('timed out', 'timeout', 'ttfb_timeout', 'connection refused', 'connection reset', 'connection aborted', 'broken pipe', 'no route to host', 'host is unreachable', 'network is unreachable', 'temporary failure in name resolution', 'name or service not known', 'getaddrinfo failed', 'eof occurred', 'incomplete chunked read', 'peer closed connection', 'connection reset by peer', 'curl error', 'server disconnected', 'handshake failure')
...
class _CircuitBreakerGuard:
    ...
    async def is_open(self, url: str) -> tuple[bool, str]:
        ...
        if not domain:
            return (False, '')
        try:
            from hledac.universal.transport.circuit_breaker import CircuitBreaker, DomainCircuitBreakerRegistry
...
class _BackendResolver:
    ...
    def _profile_to_policy(profile: Profile, tls_profile: str):
        ...
        from hledac.universal.transport.unified_transport import POLICY_CLEARNET_H2, POLICY_H3_CHROME, POLICY_H3_SAFARI, POLICY_I2P, POLICY_STEALTH_CHROME, POLICY_STEALTH_SAFARI, POLICY_TOR, TransportKind, TransportPolicy
        _map: dict[Profile, TransportPolicy] = {Profile.DEFAULT: POLICY_CLEARNET_H2, Profile.STEALTH: POLICY_STEALTH_CHROME if tls_profile == 'chrome136' else POLICY_STEALTH_SAFARI if tls_profile in ('safari17_4', 'safari') else TransportPolicy(kind=TransportKind.CURL_CFFI, tls_profile=tls_profile, timeout_s=15.0), Profile.TOR: POLICY_TOR, Profile.I2P: POLICY_I2P, Profile.H3: POLICY_H3_CHROME if tls_profile == 'chrome136' else POLICY_H3_SAFARI}
        return _map[profile]

    async def resolve(self, profile: Profile, url: str, tls_profile: str='chrome136') -> tuple[bool, Any, str, object]:
...
class HttpTransport:
    ...
    """
    __slots__ = ('_config', '_profile', '_qos', '_retry_engine', '_sem_acquired', '_gsem', '_hsem', '_client', '_backend_name', '_policy')

    def __init__(self, profile: str | Profile='default', qos: str | QoS='normal', timeout_s: float=15.0, max_bytes: int=2000000, ttfb_timeout_s: float | None=1.5, headers: dict[str, str] | None=None, follow_redirects: bool=True, max_redirects: int=10, verify_ssl: bool=True, tls_profile: str='chrome136') -> None:
        ...
        """
        self._config = HttpTransportConfig(profile=str(profile), qos=str(qos), timeout_s=timeout_s, max_bytes=max_bytes, ttfb_timeout_s=ttfb_timeout_s, headers=headers or {}, follow_redirects=follow_redirects, max_redirects=max_redirects, verify_ssl=verify_ssl, tls_profile=tls_profile)
        self._profile = Profile(str(profile))
        self._qos = QoS[str(qos).upper()] if isinstance(qos, str) else qos
        self._retry_engine = _RetryEngine(max_retries=self._qos.max_retries, base_backoff_s=self._qos.base_backoff_s)
    ...
    async def _ensure_backend(self, url: str) -> tuple[bool, str]:
        ...
        if self._client is not None:
            return (True, '')
        ok, client, backend_name, policy = await _backend_resolver.resolve(self._profile, url, tls_profile=self._config.tls_profile)
    ...
    async def fetch(self, url: str, **kwargs) -> HttpResult:
        ...
        if cb_open:
            return HttpResult(url=url, error=f'circuit_breaker_open:{cb_reason}', failure_stage='circuit_breaker', circuit_breaker_open=True, profile_used=str(self._profile), elapsed_ms=(time.monotonic() - t0_total) * 1000)
        ...
        for attempt in range(self._retry_engine.max_retries + 1):
            ...
            try:
                if ttfb_timeout_s is not None and ttfb_timeout_s > 0:
                    try:
                        result = await safe_wait_for(self._do_fetch(url, timeout_s, max_bytes, headers), timeout=ttfb_timeout_s)
                    except asyncio.TimeoutError:
                        result = HttpResult(url=url, error=f'ttfb_timeout:{ttfb_timeout_s:.1f}s', failure_stage='ttfb_timeout', retry_count=retry_count, profile_used=str(self._profile), elapsed_ms=(time.monotonic() - t0_total) * 1000)
                else:
                    result = await self._do_fetch(url, timeout_s, max_bytes, headers)
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                result = HttpResult(url=url, error=f'{type(exc).__name__}:{exc}', failure_stage='fetch_exception', retry_count=retry_count, profile_used=str(self._profile), elapsed_ms=(time.monotonic() - t0_total) * 1000)
            last_result = result
            if result.ok:
            ...
            if attempt < self._retry_engine.max_retries:
                ...
                logger.debug('[HttpTransport] retry %d/%d for %s after %.2fs (error=%s)', attempt + 1, self._retry_engine.max_retries, url, sleep_s, result.error)
                await asyncio.sleep(sleep_s)
        await _cb_guard.record_failure(url)
        if last_result is not None:
            return last_result
        return HttpResult(url=url, error='retry_exhausted', failure_stage='retry_loop', retry_count=retry_count, profile_used=str(self._profile), elapsed_ms=(time.monotonic() - t0_total) * 1000)

    async def _do_fetch(self, url: str, timeout_s: float, max_bytes: int, headers: dict[str, str]) -> HttpResult:
        """Execute the actual HTTP request through the resolved backend."""
        t0 = time.monotonic()
        ok, err = await self._ensure_backend(url)
        if not ok:
            return HttpResult(url=url, error=err, failure_stage='backend_init', profile_used=str(self._profile), elapsed_ms=(time.monotonic() - t0) * 1000)
        if self._profile == Profile.JS:
        ...
        return HttpResult(url=url, error=f'unknown_backend:{self._backend_name}', failure_stage='backend_dispatch', profile_used=str(self._profile), elapsed_ms=(time.monotonic() - t0) * 1000)

    async def _fetch_httpx(self, url: str, timeout_s: float, max_bytes: int, headers: dict[str, str], t0: float) -> HttpResult:
        """Execute fetch via httpx AsyncClient."""
        client: httpx.AsyncClient = self._client
        try:
            resp = await client.get(url, headers=headers, timeout=timeout_s, follow_redirects=self._config.follow_redirects)
            body = resp.content[:max_bytes]
            elapsed = (time.monotonic() - t0) * 1000
            return HttpResult(url=url, final_url=str(resp.url), status_code=resp.status_code, text=body.decode('utf-8', errors='replace') if body else '', content=bytes(body), content_type=resp.headers.get('Content-Type', ''), fetched_bytes=len(body), declared_length=int(resp.headers.get('Content-Length', -1)), elapsed_ms=elapsed, headers=dict(resp.headers), backend='httpx', profile_used=str(self._profile))
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            elapsed = (time.monotonic() - t0) * 1000
            return HttpResult(url=url, error=f'{type(exc).__name__}:{exc}', failure_stage='httpx_fetch', backend='httpx', profile_used=str(self._profile), elapsed_ms=elapsed)

    async def _fetch_curl_cffi(self, url: str, timeout_s: float, max_bytes: int, headers: dict[str, str], t0: float) -> HttpResult:
        """Execute fetch via curl_cffi AsyncSession."""
        from hledac.universal._core.env_config import ENV
        from hledac.universal.transport.unified_transport import TransportKind, _I2P_PROXY, _TOR_PROXY
        ...
        if policy is not None:
            try:
                ...
                pass
        try:
            resp = await session.get(url, headers=headers, timeout=timeout_s, proxies=proxies, follow_redirects=self._config.follow_redirects)
            body = resp.content[:max_bytes]
            elapsed = (time.monotonic() - t0) * 1000
            try:
            ...
            return HttpResult(url=url, final_url=final_url, status_code=resp.status_code, text=body.decode('utf-8', errors='replace') if body else '', content=bytes(body), content_type=resp_headers.get('Content-Type', ''), fetched_bytes=len(body), declared_length=int(resp_headers.get('Content-Length', -1)), elapsed_ms=elapsed, headers=resp_headers, backend='curl_cffi', profile_used=str(self._profile))
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            elapsed = (time.monotonic() - t0) * 1000
            return HttpResult(url=url, error=f'{type(exc).__name__}:{exc}', failure_stage='curl_cffi_fetch', backend='curl_cffi', profile_used=str(self._profile), elapsed_ms=elapsed)

    async def _fetch_js(self, url: str, timeout_s: float, max_bytes: int, t0: float) -> HttpResult:
        ...
        try:
            ...
            html = await safe_wait_for(_fetch_with_playwright(url, timeout=timeout_s), timeout=timeout_s + 10)
            elapsed = (time.monotonic() - t0) * 1000
            if html:
                body = html.encode('utf-8')[:max_bytes]
                return HttpResult(url=url, final_url=url, status_code=200, text=html[:max_bytes], content=body, content_type='text/html', fetched_bytes=len(body), elapsed_ms=elapsed, backend='playwright', profile_used='js')
            return HttpResult(url=url, error='playwright_empty_response', failure_stage='js_render', backend='playwright', profile_used='js', elapsed_ms=elapsed)
        except asyncio.CancelledError:
            raise
        except asyncio.TimeoutError:
            elapsed = (time.monotonic() - t0) * 1000
            return HttpResult(url=url, error='playwright_timeout', failure_stage='js_timeout', backend='playwright', profile_used='js', elapsed_ms=elapsed)
        except Exception as exc:
            elapsed = (time.monotonic() - t0) * 1000
            return HttpResult(url=url, error=f'{type(exc).__name__}:{exc}', failure_stage='js_render', backend='playwright', profile_used='js', elapsed_ms=elapsed)
    ...
    @classmethod
    async def fetch_one(cls, url: str, profile: str | Profile='default', qos: str | QoS='normal', **kwargs) -> HttpResult:
    ...
    @classmethod
    async def fetch_many(cls, urls: list[str], profile: str | Profile='default', qos: str | QoS='normal', concurrency: int | None=None, **kwargs) -> list[HttpResult]:
        ...
        async def _fetch_one(idx: int, url: str) -> None:
            async with sem:
                try:
                    ...
                    results[idx] = HttpResult(url=url, error=f'{type(exc).__name__}:{exc}', failure_stage='batch_fetch')
        try:
        ...
        for i in range(len(results)):
            if results[i] is None:
                results[i] = HttpResult(url=urls[i], error='batch_fetch_failed', failure_stage='batch_fetch')
        return results
...
class _ResponseAdapter:
    ...
    def __repr__(self) -> str:
        return f'<Response [{self.status_code}] backend={self._result.backend} profile={self._result.profile_used}>'

def get_semaphore_telemetry() -> dict[str, Any]:
    """Return semaphore telemetry for monitoring."""
    return {'global_limit': _semaphore_mgr.global_limit, 'host_sems_count': len(_semaphore_mgr._host_sems)}
__all__ = ['HttpTransport', 'HttpResult', 'HttpTransportConfig', 'Profile', 'QoS', '_ResponseAdapter', 'get_semaphore_telemetry']