...
class GopherTransport:
    ...
    async def fetch(
        self, url: str, *, timeout_s: float = TIMEOUT_S, max_bytes: int = MAX_RESPONSE_BYTES
    ) -> GopherResponse:
        """
        Fetch a gopher:// URL.
        ...
        if parsed is None:
            return GopherResponse(
                selector="", content=b"", content_type="unknown", items=[], error=f"Invalid gopher URL: {url}"
            )
    ...
    async def _fetch(self, host: str, port: int, selector: str, timeout_s: float, max_bytes: int) -> GopherResponse:
        ...
        try:
            ...
            return GopherResponse(
                selector=selector, content=b"", content_type="unknown", items=[], error=f"Connection error: {e}"
            )
        try:
            request = f"{selector}\r\n".encode()
    ...
    async def list_directory(
        self, host: str, port: int = DEFAULT_PORT, selector: str = "/", timeout_s: float = TIMEOUT_S
    ) -> list[GopherItem]:
        """
    ...
    async def get_hole_index(self) -> list[GopherItem]:
        ...
        response = await self._fetch(
            "gopher.floodgap.com", DEFAULT_PORT, "/", timeout_s=15.0, max_bytes=MAX_RESPONSE_BYTES
        )
        return response.items
        ...
        try:
            ...
            for item in response.items[:max_results]:
                if item.item_type in (GTYPE_FILE, GTYPE_DIRECTORY, GTYPE_SEARCH):
                    content = ""
                    if item.item_type == GTYPE_FILE:
                        content_resp = await self._fetch(
                            item.host, item.port, item.selector, timeout_s=10.0, max_bytes=1000000
                        )
