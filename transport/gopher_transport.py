...
class GopherFinding(msgspec.Struct, gc=False):
    ...
    item_type: str
    source_server: str
GOPHER_BOOTSTRAP_SERVERS: list[tuple[str, int]] = [('gopher.floodgap.com', DEFAULT_PORT), ('gopher.quux.org', DEFAULT_PORT)]
GOPHER_LINES: dict[str, str] = {'0': 'file', '1': 'directory', '2': 'CSO phone-book', '3': 'error', '4': 'BinHex file', '5': 'DOS file', '6': 'Uuencoded file', '7': 'search', '8': 'Telnet', '9': 'binary', '+': 'mirror', 'T': 'TN3270', 'g': 'gif', 'I': 'image', 'h': 'HTML', 's': 'wav', 'e': 'event', 'M': 'MIME'}
...
class GopherTransport:
    ...
    async def fetch(self, url: str, *, timeout_s: float=TIMEOUT_S, max_bytes: int=MAX_RESPONSE_BYTES) -> GopherResponse:
        """
        Fetch a gopher:// URL.
        ...
        if parsed is None:
            return GopherResponse(selector='', content=b'', content_type='unknown', items=[], error=f'Invalid gopher URL: {url}')
        host, port, selector = parsed
        return await self._fetch(host, port, selector, timeout_s, max_bytes)

    async def _fetch(self, host: str, port: int, selector: str, timeout_s: float, max_bytes: int) -> GopherResponse:
        ...
        if not decision.allowed:
            return GopherResponse(selector=selector, content=b'', content_type='unknown', items=[], error=f'Circuit open for {host}: {decision.reason}')
        try:
            async with asyncio.timeout(timeout_s):
                reader, writer = await asyncio.open_connection(host, port)
        except TimeoutError:
            get_breaker(host).record_failure(is_timeout=True, failure_kind='timeout')
            return GopherResponse(selector=selector, content=b'', content_type='unknown', items=[], error=f'Timeout connecting to {host}:{port}')
        except OSError as e:
            get_breaker(host).record_failure(is_timeout=False, failure_kind='connection_error')
            return GopherResponse(selector=selector, content=b'', content_type='unknown', items=[], error=f'Connection error: {e}')
        try:
            request = f'{selector}\r\n'.encode()
            ...
            return GopherResponse(selector=selector, content=content, content_type=content_type, items=items)
        except Exception as e:
            get_breaker(host).record_failure(is_timeout=False, failure_kind='read_error')
            return GopherResponse(selector=selector, content=b'', content_type='unknown', items=[], error=str(e))
        finally:
            writer.close()
    ...
    def _analyze_content(self, content: bytes) -> tuple[str, list[GopherItem]]:
    ...
    def _parse_directory(self, content: bytes) -> list[GopherItem]:
        ...
        for line in lines:
            ...
            if len(parts) < 5:
                continue
            try:
                item = GopherItem(item_type=parts[0].decode('ascii', errors='replace'), display_string=parts[1].decode('utf-8', errors='replace'), selector=parts[2].decode('utf-8', errors='replace'), host=parts[3].decode('ascii', errors='replace'), port=int(parts[4].decode('ascii', errors='replace')))
                items.append(item)
    ...
    def _is_binary_content(self, content: bytes) -> bool:
        ...
        if null_count > len(content) * 0.1:
            return True
        binary_signatures = [b'GIF87a', b'GIF89a', b'\x89PNG', b'\xff\xd8\xff', b'PK\x03\x04', b'%PDF', b'MZ']
        for sig in binary_signatures:
            if content.startswith(sig):
                return True
        return False

    async def list_directory(self, host: str, port: int=DEFAULT_PORT, selector: str='/', timeout_s: float=TIMEOUT_S) -> list[GopherItem]:
        """
    ...
    async def fetch_text(self, url: str, timeout_s: float=TIMEOUT_S, max_bytes: int=1024 * 1024) -> str | None:
        """
        ...
        if response.content_type == 'directory':
            ...
            for item in response.items:
                ...
                lines.append(f'{prefix}] {item.display_string} ({item.item_type})')
            return '\n'.join(lines)
    ...
    async def crawl_gopherspace(self, start_host: str='gopher.floodgap.com', start_port: int=DEFAULT_PORT, start_selector: str='/', max_hops: int=MAX_CRAWL_HOPS, max_items: int=MAX_CRAWL_ITEMS, max_time: float=MAX_CRAWL_TIME) -> list[GopherFinding]:
        """
        ...
        while queue and _time.monotonic() - start_time < max_time:
            ...
            async with sem:
                try:
                    response = await self._fetch(host, port, selector, timeout_s=10.0, max_bytes=1000000)
                    if response.items:
                        for item in response.items[:max_items]:
                            ...
                            if item.item_type == GTYPE_FILE:
                                findings.append(GopherFinding(title=item.display_string, content='', url=f'gopher://{item.host}:{item.port}{item.selector}', item_type='file', source_server=host))
                    elif response.content and response.content_type == 'file':
                        findings.append(GopherFinding(title=selector.split('/')[-1] or 'root', content=response.text[:5000], url=url, item_type='content', source_server=host))
                except Exception as e:
    ...
    async def search(self, query: str, *, timeout_s: float=30.0) -> GopherResponse:
        ...
        return await self._fetch(self.VERONICA_HOST, self.VERONICA_PORT, selector, timeout_s, max_bytes=1000000)

    def item_to_finding(self, item: GopherItem, *, query: str | None=None, sprint_id: str | None=None) -> dict:
        """
        ...
        """
        finding = {'source_type': 'gopher_content', 'ioc_type': 'url', 'ioc_value': f'gopher://{item.host}:{item.port}/{item.selector}', 'confidence': 0.7, 'confidence_signal': 'gopher_menu_item', 'finding_data': {'item_type': item.item_type, 'display_string': item.display_string, 'selector': item.selector, 'host': item.host, 'port': item.port, 'is_directory': item.is_directory, 'is_file': item.is_file}, 'payload_text': f'[Gopher] {item.display_string} ({item.item_type}) — gopher://{item.host}:{item.port}/{item.selector}'}
        if query:
        ...
        try:
            ...
            for item in response.items[:max_results]:
                if item.item_type in (GTYPE_FILE, GTYPE_DIRECTORY, GTYPE_SEARCH):
                    content = ''
                    if item.item_type == GTYPE_FILE:
                        content_resp = await self._fetch(item.host, item.port, item.selector, timeout_s=10.0, max_bytes=1000000)
                        content = content_resp.text[:4096] if content_resp.text else ''
                    finding = CanonicalFinding(finding_id=f'gopher-{int(time.time() * 1000)}', query=query, source_type='gopher_content', confidence=0.7, ts=time.time(), provenance=(f'gopher://{item.host}:{item.port}/{item.selector}',), payload_text=content if content else None)
