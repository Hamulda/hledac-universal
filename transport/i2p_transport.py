    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if "RESULT=OK" not in reply_str:
                logger.warning(f"SAM v3 handshake failed: {reply_str[:120]}")
        ...
        async with self._lock:
            ...
            try:
                ...
                if result == "OK":
                    ...
                    logger.info(f"SAM v3 session created: {session_name} dest={dest[:20] if dest else 'TRANSIENT'}...")
                    return dest
                else:
                    msg = _extract_sam_field(reply_str, "MESSAGE") or "unknown"
                    logger.warning(f"SAM v3 session create failed: RESULT={result} MESSAGE={msg}")
                    return None
            except TimeoutError:
    ...
    async def connect_stream(self, destination: str) -> tuple[asyncio.StreamReader, asyncio.StreamWriter] | None:
        ...
        async with self._lock:
            try:
                ...
                connect_cmd = f"STREAM CONNECT ID={self._session_name} DESTINATION={destination} SILENT=false\n"
                ...
                if result == "OK":
                    ...
                    logger.debug(f"SAM v3 stream connected to {destination[:40]}...")
                    return (self._reader, self._writer)  # type: ignore[return-value]
                else:
                    msg = _extract_sam_field(reply_str, "MESSAGE") or ""
                    logger.debug(f"SAM v3 stream connect failed: RESULT={result} MESSAGE={msg[:80]}")
                    return None
            except TimeoutError:
                logger.debug(f"SAM v3 stream connect timeout to {destination[:40]}...")
                return None
            except OSError as e:
...
class I2PTransport(Transport):
    ...
    async def _try_sam_mode(self) -> bool:
        ...
        try:
            ...
            if dest:
                ...
                logger.info(f"I2PTransport: SAM v3 session active (session={session_id}, dest={dest[:24]}...)")
                return True
    ...
    def register_handler(self, msg_type: str, handler: Callable[..., object]) -> None:
        ...
        if self.transport_mode == "sam":
            logger.debug(f"SAM v3 register_handler: {msg_type} (inbound STREAM ACCEPT not yet implemented)")
        else:
            raise NotImplementedError(
                "I2P SAM v3 streaming session not implemented; use SOCKS5 mode (rdns=True) for .i2p hostname resolution"
            )
    ...
    async def send_message(
        ...
        if self.transport_mode == "sam" and self._sam_v3_client:
            try:
                ...
                logger.warning(f"SAM v3 message send failed, falling back to proxy: {e}")
        ...
        if self.transport_mode == "sam" and self._sam_v3_client:
            try:
                ...
                if result:
                    ...
                    logger.debug(f"SAM v3 fetch failed for {config.url[:60]}, falling back to proxy")
                    # Fall through to proxy modes
            except Exception as e:
                logger.warning(f"SAM v3 fetch error for {config.url[:60]}: {e}, falling back to proxy")
                # Fall through to proxy modes
