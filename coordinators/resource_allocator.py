...
from hledac.universal.utils.asyncx import _check_gathered, safe_create_task
...
class IntelligentResourceAllocator:
    ...
    def __init__(self, config_path: str | None = None) -> None:
        ...
        _max_completed = int(os.environ.get(_MAX_COMPLETED_ALLOCATIONS_ENV, _MAX_COMPLETED_ALLOCATIONS_DEFAULT))
    ...
    async def _can_allocate(self, request: ResourceRequest, capacity: ResourceCapacity) -> bool:
        ...
        return (
            request.cpu_cores <= available_cpu
            and request.memory_gb <= available_memory
            and (request.gpu_memory is None or request.gpu_memory <= available_gpu)
        )

    async def _create_allocation(
        self, request: ResourceRequest, capacity: ResourceCapacity
    ...
    async def _handle_resource_anomaly(self, history_index: int) -> None:
        ...
        if history_index < len(self.resource_history):
            ...
            if anomaly_data["cpu_usage"] > self.scale_up_threshnew:
                low_priority_tasks = [
                    task_id for task_id, alloc in self.active_allocations.items() if alloc.efficiency_score < 0.5
                ]
    ...
    def get_allocation_statistics(self) -> dict[str, Any]:
        ...
        stats = {
            "total_requests": len(self._pending_requests_dict)
            + len(self.active_allocations)
            + len(self.completed_allocations),
    ...
    def export_allocation_report(self, filepath: str) -> None:
        ...
        report = {
            ...
            "recent_allocations": [
                ...
                for alloc in list(self.completed_allocations)[-20:]
            ],
        }
...
class ResourceAwareScheduler:
    ...
    async def schedule_task(self, task_id: str, task_func: callable, resource_request: ResourceRequest) -> bool:
        ...
        task = safe_create_task(
            self._execute_task(task_id, task_func), name=f"resource_allocator:execute_task:{task_id}"
        )
