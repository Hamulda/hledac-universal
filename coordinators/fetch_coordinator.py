...
from ..utils.async_helpers import BoundedPerHostGate, async_getaddrinfo  # noqa: E402
...
class FetchCoordinator(UniversalCoordinator):
        ...
        if os.environ.get("HLEDAC_ENABLE_TOR") == "1":
            try:
                ...
                if self._tor_transport_enabled:
                    ...
                    logger.info(
                        "  Circuit rotation after {self._tor_transport._max_circuit_requests} requests",
                        _max_circuit_requests=self._tor_transport._max_circuit_requests,
                    )
    ...
    async def _record_domain_failure(self, domain: str) -> None:
        ...
        if len(self._domain_failures) > 1000:
            ...
            stale_domains = [
                d
                for d, ts in self._domain_failure_timestamps.items()
                if ts < cutoff and d not in self._domain_blocked_until
            ]  # noqa: E501
        ...
        if failures >= self._failure_threshold:
            ...
            # Sprint F360: Wire into unified metrics dashboard
            try:
                from hledac.universal.metrics_registry import get_metrics_registry

                get_metrics_registry().record_fetch_telemetry(
                    blocked_domains=self._telemetry["circuit_breaker_active"],
                    circuit_open=len(self.get_blocked_domains()) > 0,
                )
            except Exception:  # noqa: BLE001
                pass  # fail-soft: metrics never crash fetch
            logger.warning(
    ...
    async def _do_step(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        for url, result in zip(urls_to_fetch, results, strict=False):
            if isinstance(result, Exception):
                ...
                logger.debug(
                    "[BATCH] fetch exception for {url}: {type(result).__name__}: {result}",
                    url=url,
                    result=result,
                    _ev0=type(result).__name__,
                )
    ...
    async def _fetch_url(self, url: str, attempt: int = 0) -> dict[str, Any] | None:
        ...
        if not dns_safe:
            ...
            logger.warning(
                "DNS rebinding defense blocked: {dns_meta.get('blocked_reason')} for {domain}",
                domain=domain,
                _ev0=dns_meta.get("blocked_reason"),
            )
        ...
        try:
            while attempt <= max_retries:
                ...
                if not dns_safe:
                    ...
                    logger.warning(
                        "DNS rebinding defense blocked: {blocked_reason} for {domain}",
                        domain=domain,
                        blocked_reason=_blocked_reason,
                    )
                    trace_fetch_end(
                        url, "dns_rebind_defense", "blocked", 0.0, {"reason": dns_meta.get("blocked_reason")}
                    )
                    break
                ...
                if not canonical_allowed:
                    self._telemetry["circuit_breaker_active"] = len(self.get_blocked_domains())
                    logger.debug(
                        "[F206AS] Canonical circuit breaker open for {domain}: {canonical_reason} (retry in {canonical_retry_after:.1f}s)",
                        domain=domain,
                        ...
                        async with session.head(url, allow_redirects=True, cookies=session_cookies) as resp:
                            ...
                            if content_type.startswith("text/html"):
                                async with session.get(
                                    url, cookies=session_cookies, timeout=preview_timeout
                                ) as get_resp:
                ...
                if result is None or result.get("error") == "timeout" or result.get("status_code", 200) >= 500:
                    if attempt < max_retries:
                        ...
                        logger.debug(
                            "[RETRY] Attempt {attempt_number}/{max_retries} for {url} after {delay}s",
        ...
        if result and result.get("content"):
            ...
            if len(content) < 5000 and self._paywall_bypass:  # type: ignore[ty:invalid-argument-type]  # stale type: ty can't refine content: str from upstream decode branch
                ...
                if bypass_result:
                    ...
                    logger.info(
                        "[PAYWALL] Bypassed via {bypass_result.get('bypassed')}", _ev0=bypass_result.get("bypassed")
                    )
            ...
            if ct.startswith("image/") and len(content_bytes) < 200 * 1024:  # type: ignore[ty:invalid-argument-type]  # stale type: result.get()/result[] return Any, ty can't refine to str/bytes
                ...
                try:
                    if self._captcha_detector.is_captcha(content_bytes, url_for_check):
                        logger.debug(
                            "[CAPTCHA] CAPTCHA detected at {url_for_check}, skipping", url_for_check=url_for_check
                        )
        ...
        try:
            ...
            for part, label in [
                ...
                if isinstance(part, list):
                    ...
                    logger.debug(
                        "[DEEP] {label} failed: {type(part).__name__}: {part}",
                        label=label,
                        part=part,
                        _ev0=type(part).__name__,
                    )
            ...
            logger.info(
                "[DEEP] query={query!r} → {raw_rows} raw rows → {fused_rows} fused",
                query=query,
                raw_rows=len(rows),
    ...
    async def _maybe_fire_cover_traffic(self, transport: str) -> None:
        ...
        try:
            if random.random() < _COVER_RATE:
                ...
                logger.debug(
                    "[COVER] fired cover traffic #{self._cover_count} for transport={transport}",
                    _cover_count=self._cover_count,
                    transport=transport,
                )
        except Exception:  # noqa: BLE001
