...
class PrivacyBudgetEntry:
    ...
    def is_exhausted(self, max_requests: int, max_entropy: float) -> bool:
        ...
        return (
            self.request_count >= max_requests or
            self.entropy_used >= max_entropy
        )

    def get_remaining_budget(self, max_requests: int, max_entropy: float) -> tuple[int, float]:
        """Get remaining budget (requests, entropy)."""
        return (
            max(0, max_requests - self.request_count),
            max(0.0, max_entropy - self.entropy_used)
        )
...
class PrivacyAllocatorService:
    ...
    def __post_init__(self) -> None:
        # Initialize privacy lane semaphores
    ...
    async def record_request(self, url: str, content_size: int = 0, privacy_level: PrivacyLevel = PrivacyLevel.CLEAR) -> None:
        """
    ...
    def cleanup_old_entries(self, max_age_s: float = 86400.0) -> int:
        ...
        now = time.monotonic()
        to_remove = [
            domain for domain, entry in self._budgets.items()
            if now - entry.last_request_ts > max_age_s
        ]
