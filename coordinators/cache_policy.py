...
class ByteBoundedLRU[K, V]:
    ...
    """
    __slots__ = tuple(('_l1', '_l2', '_l3', '_lmdb_env', '_lmdb_path', '_lock', '_metrics', 'demote_fraction', 'max_l1_bytes', 'max_l2_bytes', 'max_l3_bytes', 'pressure_gate'))

    def __init__(self, max_l1_bytes: int=128 * 1024 * 1024, max_l2_bytes: int=512 * 1024 * 1024, max_l3_bytes: int=1024 * 1024 * 1024, lmdb_path: str | None=None, demote_fraction: float=0.1, pressure_gate: Callable[[], float] | None=None) -> None:
        ...
        if lmdb_path and lmdb is not None:
            try:
                ...
                self._lmdb_env = lmdb.open(lmdb_path, map_size=max_l3_bytes, readahead=False, sync=False)
    ...
    async def put(self, key: K, value: V, metadata: dict[str, Any] | None=None) -> bool:
        """
        ...
        now = time.time()
        entry = _CacheEntry(value=value, size_bytes=size_bytes, created_at=now, last_accessed=now, access_count=1, metadata=metadata or {})
        with self._lock:
    ...
    def stats(self) -> CacheMetrics:
        ...
        with self._lock:
            return CacheMetrics(hits=self._metrics.hits, misses=self._metrics.misses, evictions=self._metrics.evictions, promotions=self._metrics.promotions, demotions=self._metrics.demotions, evicted_bytes=self._metrics.evicted_bytes, stored_bytes=self._metrics.stored_bytes, l1_size_bytes=self._metrics.l1_size_bytes, l2_size_bytes=self._metrics.l2_size_bytes, l3_size_bytes=self._metrics.l3_size_bytes)
    ...
    def _evict_l1_if_needed(self, incoming_bytes: int) -> None:
        ...
        while self._metrics.l1_size_bytes + incoming_bytes > self.max_l1_bytes and self._l1:
    ...
    def _evict_l2_if_needed(self, incoming_bytes: int) -> None:
        ...
        while self._metrics.l2_size_bytes + incoming_bytes > self.max_l2_bytes and self._l2:
    ...
    def _evict_l3_if_needed(self, incoming_bytes: int) -> None:
        ...
        while self._metrics.l3_size_bytes + incoming_bytes > self.max_l3_bytes and self._l3:
    ...
    def _serialize_entry(self, entry: _CacheEntry[V]) -> bytes:
        ...
        return encode({'value': entry.value, 'size_bytes': entry.size_bytes, 'created_at': entry.created_at, 'last_accessed': entry.last_accessed, 'access_count': entry.access_count, 'metadata': entry.metadata})
    ...
    def _deserialize_entry(self, data: bytes) -> _CacheEntry[V] | None:
        ...
        try:
            ...
            if not isinstance(raw, dict):
                return None
            return _CacheEntry(value=raw.get('value'), size_bytes=raw.get('size_bytes', 0), created_at=raw.get('created_at', 0.0), last_accessed=raw.get('last_accessed', 0.0), access_count=raw.get('access_count', 0), metadata=raw.get('metadata') or {})
        except Exception:
            return None
...
class ByteBoundedARC[K, V]:
    ...
    """
    __slots__ = tuple(('_b1', '_b2', '_c0', '_c1', '_lock', '_metrics', '_t1', '_t2', 'ghost_fraction', 'max_bytes', 'pressure_gate'))

    def __init__(self, max_bytes: int=128 * 1024 * 1024, ghost_fraction: float=0.1, pressure_gate: Callable[[], float] | None=None) -> None:
        if lmdb is None:
            raise ImportError('lmdb is required for ByteBoundedARC')
        self.max_bytes = max_bytes
        self.ghost_fraction = ghost_fraction
        self.pressure_gate = pressure_gate
        self._t1: OrderedDict[K, _CacheEntry[V]] = OrderedDict()
        self._t2: OrderedDict[K, _CacheEntry[V]] = OrderedDict()
        self._b1: OrderedDict[K, int] = OrderedDict()
    ...
    def get(self, key: K) -> V | None:
        ...
        with self._lock:
            ...
            if (entry := self._t1.get(key)):
                ...
                entry = self._replace_entry_val(entry)
                self._t2[key] = entry
        ...
        while total + size_bytes > target:
            if self._b1 and self._b2:
                ...
                if self._c0 > self._c1 and b1_len > 0:
                    ...
                    self._evict_from_t2()
                elif self._c0 >= self._c1 and self._t1:
                    self._evict_from_t1()
                elif self._t2:
                    self._evict_from_t2()
            elif self._t1:
    ...
    def _replace_entry_val(self, entry: _CacheEntry[V]) -> _CacheEntry[V]:
        ...
        return _CacheEntry(value=entry.value, size_bytes=entry.size_bytes, created_at=entry.created_at, last_accessed=time.time(), access_count=entry.access_count, metadata=entry.metadata)
    ...
    def stats(self) -> CacheMetrics:
        with self._lock:
            s = CacheMetrics(hits=self._metrics.hits, misses=self._metrics.misses, evictions=self._metrics.evictions, evicted_bytes=self._metrics.evicted_bytes, stored_bytes=self._metrics.stored_bytes, l1_size_bytes=self._c0, l2_size_bytes=self._c1)
            return s
