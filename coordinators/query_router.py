...
class QueryRouter:
    ...
    def __init__(
        self, source_mask: SourceMask | None = None, max_results_per_source: int = 5, timeout_s: float = 30.0
    ) -> None:
    ...
    async def query(self, query: str, remaining_budget_s: float = 30.0) -> list[dict]:
        ...
        if not tasks:
            return []
        gathered = await safe_wait_for(
            parallel_ok(*tasks, label="query_router"), timeout=self._timeout * 2, label="query_router_gather"
        )
        merged: list[dict] = []
