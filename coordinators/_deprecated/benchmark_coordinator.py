...
class BenchmarkConfig:
    ...
    sample_queries: list[str] = field(default_factory=lambda: ['machine learning algorithms', 'quantum computing research', 'climate change impact', 'renewable energy systems', 'artificial intelligence ethics'])
...
class MemoryProfiler:
    ...
    __slots__ = tuple(('_active', '_process', '_snapshots'))

    def __init__(self):
        self._snapshots: deque[tuple[float, float]] = deque(maxlen=MAX_SNAPSHOT_HISTORY)
        self._active = False
...
class AgentBenchmarker:
    ...
    """
    __slots__ = tuple(('_system_baseline', 'config', 'memory_profiler'))
    ...
    async def benchmark_agent(self, agent_name: str, agent: AgentProtocol, custom_queries: list[str] | None=None) -> AgentBenchmarkResult:
        """
        ...
        for iteration in range(self.config.benchmark_iterations):
            ...
            try:
                ...
                if hasattr(agent, 'search'):
                    results = await agent.search(query=query, max_results=10, timeout=self.config.timeout_seconds, run_id=f'benchmark_{agent_name}_{iteration}')
        ...
        result = self._calculate_benchmark_results(agent_name, execution_times, memory_snapshots, successful_executions, total_results, self.config.benchmark_iterations)
        logger.info(f'Benchmark completed for {agent_name}: {result.performance_tier} tier')
        return result

    async def benchmark_multiple_agents(self, agents: dict[str, AgentProtocol], concurrent: bool=False) -> BenchmarkReport:
        """
        ...
        if concurrent:
            ...
            for agent_name, agent in agents.items():
                task = asyncio.create_task(self.benchmark_agent(agent_name, agent), name=f'benchmark:agent:{agent_name}')
            ...
            for agent_name, task in tasks:
                try:
                    ...
                    failed_result = AgentBenchmarkResult(agent_name=agent_name, total_executions=self.config.benchmark_iterations, successful_executions=0, failed_executions=self.config.benchmark_iterations, success_rate=0.0, avg_execution_time=self.config.timeout_seconds, min_execution_time=self.config.timeout_seconds, max_execution_time=self.config.timeout_seconds, p95_execution_time=self.config.timeout_seconds, p99_execution_time=self.config.timeout_seconds, avg_memory_usage=0.0, peak_memory_usage=0.0, memory_growth_rate=0.0, avg_results_per_second=0.0, total_results_returned=0, performance_tier='failed', bottlenecks=['execution_failure'])
    ...
    async def _warmup_agent(self, agent: AgentProtocol, queries: list[str]) -> None:
        ...
        for i, query in enumerate(queries[:self.config.warmup_iterations]):
            try:
                if hasattr(agent, 'search'):
                    await agent.search(query=query, max_results=5, timeout=self.config.timeout_seconds, run_id=f'warmup_{i}')
                elif hasattr(agent, 'run'):
    ...
    def _calculate_benchmark_results(self, agent_name: str, execution_times: list[float], memory_snapshots: list[float], successful_executions: int, total_results: int, total_executions: int) -> AgentBenchmarkResult:
        """Calculate comprehensive benchmark results."""
        if not execution_times:
            return AgentBenchmarkResult(agent_name=agent_name, total_executions=total_executions, successful_executions=0, failed_executions=total_executions, success_rate=0.0, avg_execution_time=0.0, min_execution_time=0.0, max_execution_time=0.0, p95_execution_time=0.0, p99_execution_time=0.0, avg_memory_usage=0.0, peak_memory_usage=0.0, memory_growth_rate=0.0, avg_results_per_second=0.0, total_results_returned=0, performance_tier='failed', bottlenecks=['no_executions'])
        ...
        performance_tier = self._determine_performance_tier(avg_time, success_rate, avg_memory, avg_results_per_second)
        bottlenecks = self._identify_bottlenecks(avg_time, success_rate, avg_memory, memory_growth)
        return AgentBenchmarkResult(agent_name=agent_name, total_executions=total_executions, successful_executions=successful_executions, failed_executions=total_executions - successful_executions, success_rate=success_rate, avg_execution_time=avg_time, min_execution_time=min_time, max_execution_time=max_time, p95_execution_time=p95_time, p99_execution_time=p99_time, avg_memory_usage=avg_memory, peak_memory_usage=peak_memory, memory_growth_rate=memory_growth, avg_results_per_second=avg_results_per_second, total_results_returned=total_results, performance_tier=performance_tier, bottlenecks=bottlenecks, execution_times=execution_times, memory_snapshots=memory_snapshots)

    def _determine_performance_tier(self, avg_time: float, success_rate: float, avg_memory: float, throughput: float) -> str:
    ...
    def _identify_bottlenecks(self, avg_time: float, success_rate: float, avg_memory: float, memory_growth: float) -> list[str]:
    ...
    def _generate_comparative_analysis(self, results: dict[str, AgentBenchmarkResult]) -> dict[str, Any] | None:
        ...
        sorted_by_speed = sorted(results.items(), key=lambda x: x[1].avg_execution_time)
        sorted_by_reliability = sorted(results.items(), key=lambda x: x[1].success_rate, reverse=True)
        sorted_by_memory = sorted(results.items(), key=lambda x: x[1].avg_memory_usage)
        all_bottlenecks = []
        for result in results.values():
        ...
        for bottleneck in all_bottlenecks:
            ...
            return {'performance_tier_distribution': tier_counts, 'fastest_agents': [(name, result.avg_execution_time) for name, result in sorted_by_speed[:5]], 'most_reliable_agents': [(name, result.success_rate) for name, result in sorted_by_reliability[:5]], 'most_memory_efficient': [(name, result.avg_memory_usage) for name, result in sorted_by_memory[:5]], 'common_bottlenecks': bottleneck_counts, 'performance_variance': self._calculate_performance_variance(results)}

    def _calculate_performance_variance(self, results: dict[str, AgentBenchmarkResult]) -> dict[str, float]:
        ...
        return {'execution_time_variance': time_variance, 'memory_variance': memory_variance, 'execution_time_std': time_variance ** 0.5, 'memory_std': memory_variance ** 0.5}
    ...
    async def _capture_system_baseline(self) -> dict[str, Any]:
        ...
        return {'cpu_count': psutil.cpu_count(), 'memory_total_gb': psutil.virtual_memory().total / 1024 / 1024 / 1024, 'memory_available_gb': psutil.virtual_memory().available / 1024 / 1024 / 1024, 'process_memory_mb': process.memory_info().rss / 1024 / 1024, 'process_cpu_percent': process.cpu_percent(), 'timestamp': time.time()}
    ...
    async def _capture_system_metrics(self) -> dict[str, Any]:
        ...
        if not PSUTIL_AVAILABLE or psutil is None:
            return {'error': 'psutil not available', 'timestamp': time.time()}
        process = psutil.Process()
        return {'memory_usage_mb': process.memory_info().rss / 1024 / 1024, 'memory_percent': process.memory_percent(), 'cpu_percent': process.cpu_percent(), 'open_files': len(process.open_files()), 'threads': process.num_threads()}
    ...
    def _generate_recommendations(self, report: BenchmarkReport) -> list[str] | None:
        ...
        poor_agents = [name for name, result in report.agent_results.items() if result.performance_tier in ['poor', 'failed']]
        if poor_agents:
...
async def run_agent_benchmarks(agents: dict[str, AgentProtocol], config: BenchmarkConfig | None=None, concurrent: bool=False) -> BenchmarkReport:
    """
...
async def run_quick_performance_check(agents: dict[str, AgentProtocol], sample_size: int=3) -> dict[str, Any]:
    """
    ...
    """
    config = BenchmarkConfig(warmup_iterations=1, benchmark_iterations=sample_size, sample_queries=['test query', 'sample search', 'example request'][:sample_size])
    report = await run_agent_benchmarks(agents, config, concurrent=False)
    return {'summary': {'agents_tested': len(report.agent_results), 'overall_success_rate': report.overall_success_rate, 'avg_execution_time': report.avg_execution_time, 'performance_tiers': report.comparative_analysis.get('performance_tier_distribution', {})}, 'top_performers': {'fastest': report.comparative_analysis.get('fastest_agents', [])[:3], 'most_reliable': report.comparative_analysis.get('most_reliable_agents', [])[:3]}, 'recommendations': report.recommendations[:5], 'detailed_results': {name: {'success_rate': result.success_rate, 'avg_time': result.avg_execution_time, 'memory_usage': result.avg_memory_usage, 'performance_tier': result.performance_tier} for name, result in report.agent_results.items()}}