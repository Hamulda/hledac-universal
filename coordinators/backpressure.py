...
class BackpressureMonitor:
    ...
    """
    __slots__ = tuple(('_decision', '_governor', '_last_evaluate', '_last_state', '_lock', '_max_clearnet', '_min_clearnet', '_state_changes'))

    def __init__(self, governor: object, min_clearnet: int=_MIN_CLEARNET, max_clearnet: int=_DEFAULT_CLEARNET_MAX):
        self._governor = governor
        self._min_clearnet = min_clearnet
        self._max_clearnet = max_clearnet
        self._decision: BackpressureDecision = BackpressureDecision(clearnet_max=max_clearnet, stealth_max=_DEFAULT_STEALTH_MAX, uma_state='ok', io_only=False)
    ...
    async def evaluate(self) -> BackpressureDecision:
        ...
        async with self._lock:
            ...
            try:
                ...
                self._decision = BackpressureDecision(clearnet_max=self._max_clearnet, stealth_max=_DEFAULT_STEALTH_MAX, uma_state='ok', io_only=False)
            ...
            new_decision = BackpressureDecision(clearnet_max=clearnet_max, stealth_max=stealth_max, uma_state=governor_decision.uma_state, io_only=io_only)
            if new_decision.uma_state != self._last_state:
                logger.info(f'[BACKPRESSURE] uma_state: {self._last_state} → {new_decision.uma_state} (clearnet_max={clearnet_max}, stealth_max={stealth_max})')
    ...
    def get_telemetry(self) -> dict:
        ...
        return {'clearnet_max': self._decision.clearnet_max, 'stealth_max': self._decision.stealth_max, 'uma_state': self._decision.uma_state, 'io_only': self._decision.io_only, 'state_changes': self._state_changes}