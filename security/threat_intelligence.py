...
_STATIC_IOC_PATTERNS: dict[str, list[str]] = {
    "domain": ["malware-c2.net", "phishing-site.io", "suspicious-cdn.com"],
    "ip": ["185.220.101.0/24", "192.0.2.0/24"],
    "hash": [],
    "url": ["http://malware-download.com/payload.exe", "https://phishing-site.io/steal"],
}
...
class ThreatIntelligence:
    ...
    __slots__ = ("_feed_source", "_initialized", "_iocs", "_threat_count")
    ...
    async def analyze_threats(
        self, context: dict[str, Any], priority_level: int = 5, security_level: int = 3
    ) -> dict[str, Any]:
        """
    ...
    async def lookup_ioc(self, ioc: str) -> dict[str, Any]:
        ...
        if techniques:
            result.update(
                {"found": True, "severity": "medium", "techniques": techniques, "sources": ["kill_chain_tagger"]}
            )
        ...
        if _looks_like_ip(ioc_str):
            try:
                ...
                async with session.get(
                    "https://data.iana.org/rdap/ipv4.json", timeout=httpx.Timeout(total=4)
                ) as boot_resp:
            ...
            try:
                ...
                async with session.get(
                    ...
                    if resp.status_code == 200:
                        ...
                        name = data.get("name", "")
                        result.update(
                            {"sources": result["sources"] + ["bgptools"], "asn": asn, "prefix": pfx, "asn_name": name}
                        )
            except Exception:  # noqa: BLE001
                pass
        return result
