...
# Format: (name, version, sha256, url_or_none, build_ref, min_size_bytes)
#
# url_or_none: GitHub release URL if available, else None (→ isolated build)
# build_ref: git tag/commit that was used for the verified build.
#   Use "RELEASE-X.Y" format for official releases, "BUILD-YYYY-MM-DD" for
#   locally-built artifacts.
#
# IMPORTANT: Update sha256 when building new verified artifacts!
#   Build command (isolated):
#     git clone --filter=blob:none --depth=1 --branch=<tag> <repo> /tmp/steg_src
#     make -C /tmp/steg_src
#     shasum -a 256 /tmp/steg_src/stegdetect  → copy hex into manifest

# TODO(INTERNAL-007): Populate with actual SHA-256 once CI produces first build.
# Placeholder values — MUST be replaced before production use.
_STEGDETECT_KNOWN_GOOD: tuple[str, str, str, str | None, int] = (
    # (name, version, sha256_hex, release_url_or_none, min_size_bytes)
    "stegdetect",
    "0.8.0",  # matches abeluck/stegdetect @ latest tag as of 2024
...
class ArtifactManifest:
    ...
    url: str | None  # download URL (GitHub release, etc.)
    min_size_bytes: int
    build_ref: str | None = None  # git tag/commit used for this build
...
class ArtifactInstallResult:
    ...
    error: str | None = None
    verified: bool = False  # SHA-256 match confirmed
...
class ArtifactVerifier:
    ...
    async def ensure_artifact(
        ...
        *,
        ...
        # ── 1. Cache hit: already installed and verified ────────────────────────
        cached_result = await self._check_cache(binary_path, expected_sha256, min_size)
        if cached_result is not None:
            return cached_result

        # ── 2. Release download (preferred if available) ───────────────────────
        ...
        # ── 3. Isolated build (git clone → sandboxed make → verify → install) ──
        ...
        # ── 4. All paths failed ────────────────────────────────────────────────
        error_msg = (
            f"[ADVERSARY-001] [INTERNAL-007] All installation paths failed for "
    ...
    async def _check_cache(
        ...
        try:
            ...
            if size < min_size:
                logger.warning(
                    "[ADVERSARY-001] [INTERNAL-007] Cached binary too small: "
                    "%s (%d < %d bytes). Rebuilding.",
                    binary_path,
                    size,
                    min_size,
                )
        ...
        # SHA-256 check (async to avoid blocking)
        actual_sha256 = await asyncio.to_thread(_compute_sha256, binary_path)
        if actual_sha256.lower() != expected_sha256.lower():
            logger.warning(
                "[ADVERSARY-001] [INTERNAL-007] Cached binary SHA-256 mismatch: "
    ...
    async def _download_and_verify(
        ...
        try:
            ...
            if actual_sha256.lower() != expected_sha256.lower():
                logger.warning(
                    "[ADVERSARY-001] [INTERNAL-007] Release SHA-256 mismatch: "
                    "expected=%s actual=%s",
                    expected_sha256[:16],
                    actual_sha256[:16],
                )
    ...
    async def _isolated_build(
        ...
        try:
            ...
            # ── Step 1: Shallow clone (minimal surface) ─────────────────────────
            proc = await asyncio.create_subprocess_exec(
                "git",
                "clone",
                "--filter=blob:none",  # No blob data during clone
                "--depth",
                "1",
                "--branch",
                branch,
            ...
            if not src_binary.exists():
                # stegdetect Makefile may put binary elsewhere
                # Search for executable in cloned dir
            ...
            if actual_sha256.lower() != expected_sha256.lower():
                logger.warning(
                    "[ADVERSARY-001] [INTERNAL-007] Isolated build SHA-256 mismatch: "
                    "expected=%s actual=%s. This indicates the source changed. "
            ...
            logger.info(
                "[ADVERSARY-001] [INTERNAL-007] Isolated build verified and installed: "
    ...
    async def _fallback_git_clone_make(
        self,
        name: str,
        repo_url: str,
    ) -> ArtifactInstallResult:
        ...
        """
        logger.warning(
            "[ADVERSARY-001] [INTERNAL-007] ⚠️ UNSAFE FALLBACK: "
            "Installing %s via git clone + make WITHOUT SHA-256 verification. "
        ...
        try:
            ...
            # Copy to bin dir (with path guard)
            try:
