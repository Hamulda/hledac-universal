...
_STEGDETECT_KNOWN_GOOD: tuple[str, str, str, str | None, int] = ('stegdetect', '0.8.0', 'REPLACE_WITH_ACTUAL_SHA256_AFTER_FIRST_CI_BUILD', None, 50000)
...
class ArtifactVerifier:
    ...
    async def ensure_artifact(self, name: str, *, repo_url: str='https://github.com/abeluck/stegdetect.git', branch: str='master', build_cmd: list[str] | None=None) -> ArtifactInstallResult:
        """
        ...
        if not _SIGNED_ARTIFACTS_ENABLED:
            logger.warning('[ADVERSARY-001] [INTERNAL-007] HLEDAC_ENABLE_STEGDETECT_SIGNED=0: artifact verification DISABLED. Falling back to original git+make.')
            return await self._fallback_git_clone_make(name, repo_url)
        manifest = _VERIFIED_ARTIFACTS.get(name)
        if manifest is None:
            logger.error('[ADVERSARY-001] [INTERNAL-007] Unknown artifact: %s. Add to VERIFIED_ARTIFACTS before use.', name)
        ...
        if release_url is not None:
            dl_result = await self._download_and_verify(url=release_url, dest=binary_path, expected_sha256=expected_sha256, min_size=min_size)
            if dl_result.success:
                return dl_result
        build_result = await self._isolated_build(name=name, repo_url=repo_url, branch=branch, build_cmd=build_cmd or ['make'], dest=binary_path, expected_sha256=expected_sha256, min_size=min_size)
    ...
    async def _check_cache(self, binary_path: Path, expected_sha256: str, min_size: int) -> ArtifactInstallResult | None:
        """
        ...
        try:
            ...
            if size < min_size:
                logger.warning('[ADVERSARY-001] [INTERNAL-007] Cached binary too small: %s (%d < %d bytes). Rebuilding.', binary_path, size, min_size)
                return None
        except OSError:
            return None
        actual_sha256 = await asyncio.to_thread(_compute_sha256, binary_path)
        if actual_sha256.lower() != expected_sha256.lower():
            logger.warning('[ADVERSARY-001] [INTERNAL-007] Cached binary SHA-256 mismatch: expected=%s actual=%s. Removing and rebuilding.', expected_sha256[:16], actual_sha256[:16])
            try:
                binary_path.unlink()
            except OSError:
                pass
    ...
    async def _download_and_verify(self, url: str, dest: Path, expected_sha256: str, min_size: int) -> ArtifactInstallResult:
        """
        Download binary from verified URL and verify SHA-256.
        """
        try:
            proc = await asyncio.create_subprocess_exec('curl', '-fsSL', '--proto', '=https', '-o', str(dest), url, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            stdout, stderr = await proc.communicate()
            if proc.returncode != 0:
                logger.warning('[ADVERSARY-001] [INTERNAL-007] Download failed (curl rc=%d): %s', proc.returncode, stderr.decode(errors='replace'))
            ...
            if actual_sha256.lower() != expected_sha256.lower():
                logger.warning('[ADVERSARY-001] [INTERNAL-007] Release SHA-256 mismatch: expected=%s actual=%s', expected_sha256[:16], actual_sha256[:16])
    ...
    async def _isolated_build(self, name: str, repo_url: str, branch: str, build_cmd: list[str], dest: Path, expected_sha256: str, min_size: int) -> ArtifactInstallResult:
        """
        ...
        try:
            ...
            proc = await asyncio.create_subprocess_exec('git', 'clone', '--filter=blob:none', '--depth', '1', '--branch', branch, repo_url, str(src_dir), stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            stdout, stderr = await proc.communicate()
            if proc.returncode != 0:
                clone_out = stderr.decode(errors='replace')
                logger.warning('[ADVERSARY-001] [INTERNAL-007] git clone failed (rc=%d): %s', proc.returncode, clone_out)
                return ArtifactInstallResult(success=False, error=f'git clone failed: {clone_out[:200]}')
            build_dir = src_dir
            proc = await asyncio.create_subprocess_exec(build_cmd[0], *build_cmd[1:], cwd=str(build_dir), stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            stdout, stderr = await proc.communicate()
            if proc.returncode != 0:
            ...
            if not src_binary.exists():
                ...
                if found is None:
                    return ArtifactInstallResult(success=False, error=f'build succeeded but binary not found: {src_binary}')
                src_binary = found
            actual_sha256 = await asyncio.to_thread(_compute_sha256, src_binary)
            if actual_sha256.lower() != expected_sha256.lower():
                logger.warning('[ADVERSARY-001] [INTERNAL-007] Isolated build SHA-256 mismatch: expected=%s actual=%s. This indicates the source changed. Update the manifest SHA-256 and re-run.', expected_sha256[:16], actual_sha256[:16])
                return ArtifactInstallResult(success=False, error=f'SHA-256 mismatch: build artifact differs from manifest. Expected {expected_sha256[:16]}... Got {actual_sha256[:16]}... Update VERIFIED_ARTIFACTS with new SHA-256.')
            os.makedirs(_BIN_DIR, exist_ok=True)
            shutil.copy2(src_binary, dest)
            dest.chmod(493)
            logger.info('[ADVERSARY-001] [INTERNAL-007] Isolated build verified and installed: %s (SHA-256=%s, method=isolated_build)', dest, actual_sha256[:16])
    ...
    async def _fallback_git_clone_make(self, name: str, repo_url: str) -> ArtifactInstallResult:
        ...
        """
        logger.warning('[ADVERSARY-001] [INTERNAL-007] ⚠️ UNSAFE FALLBACK: Installing %s via git clone + make WITHOUT SHA-256 verification. This path is vulnerable to supply-chain attacks. Set HLEDAC_ENABLE_STEGDETECT_SIGNED=1 to enable verification.', name)
        _BIN_DIR = (Path.home() / '.hledac' / 'bin').resolve()
        ...
        try:
            ...
            proc = await asyncio.create_subprocess_exec('git', 'clone', '--depth', '1', repo_url, str(src_dir), stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            ...
            try:
                ...
                return ArtifactInstallResult(success=False, error=f'Binary path outside allowed directory: {binary_path}')
