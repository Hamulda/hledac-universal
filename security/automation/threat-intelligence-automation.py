...
class ThreatIntelligenceAutomation:
    ...
    __slots__ = (
        ...
        "vulnerability_cache",
        "ml_models",
    )  # [C3] added ml_models to __slots__

    MAX_DEFENSE_ACTIONS = 10_000  # [C1] cap to prevent unbounded list growth

    def __init__(self, config_path: str = "config/security_enhancements.yaml") -> None:
        self.config_path = Path(config_path)
        self.threat_intel_db: LRUCache[str, ThreatIntelligence] = LRUCache(
            maxsize=50_000
        )  # [B2] threat DB cap, bounded eviction
        self.active_alerts = {}
        ...
        self.vulnerability_cache: LRUCache[str, dict] = LRUCache(
            maxsize=10_000
        )  # [B1] CVE hot-path cap, bounded eviction
    ...
    async def start_threat_intelligence_service(self) -> None:
    ...
    async def _gather_threat_intelligence(self) -> None:
    ...
    async def _gather_malware_domains(self, source_name: str, config: dict[str, Any]) -> None:
            ...
            if response.status_code == 200:
                ...
                for entry in data:
                    threat = ThreatIntelligence(
                        ...
                        first_seen=datetime.fromisoformat(entry.get("first_seen", datetime.now(UTC).isoformat())),
    ...
    async def _gather_file_reputation(self, source_name: str, config: dict[str, Any]) -> None:
        """Gather file reputation intelligence"""
        logger.debug(f"File reputation gathering not fully implemented for {source_name}")

    async def _gather_ioc_indicators(self, source_name: str, config: dict[str, Any]) -> None:
                ...
                for pulse in data.get("results", []):
                    for indicator in pulse.get("indicators", []):
                        threat = ThreatIntelligence(
                            ...
                            first_seen=datetime.fromisoformat(pulse.get("created", datetime.now(UTC).isoformat())),
    ...
    async def _gather_vulnerabilities(self, source_name: str, config: dict[str, Any]) -> None:
            ...
            if response.status_code == 200:
                ...
                for cve in data.get("vulnerabilities", []):
                    ...
                    threat = ThreatIntelligence(
                        ...
                        first_seen=datetime.fromisoformat(cve.get("published", datetime.now(UTC).isoformat())),
    ...
    async def _analyze_threats_and_alert(self) -> None:
    ...
    async def _analyze_application_threats(self) -> None:
            ...
            if threat.threat_type in ["malware_domain", "ioc"]:
                for indicator in threat.indicators:
                    if self._matches_app_endpoint(indicator, app_endpoints):
                        alert = SecurityAlert(
                            alert_id=f"app_{threat_id}_{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}",
    ...
    async def _analyze_network_threats(self) -> None:
    ...
    async def _analyze_access_log_for_threats(self, log_file: Path) -> None:
                ...
                for line_num, line in enumerate(f, 1):
                    for pattern in suspicious_patterns:
                        if re.search(pattern, line, re.IGNORECASE):
                            alert = SecurityAlert(
                                alert_id=f"log_threat_{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}_{line_num}",
                                threat_intelligence=ThreatIntelligence(
    ...
    async def _analyze_behavior_anomalies(self) -> None:
        ...
        try:
            ...
            for anomaly in anomalies:
                alert = SecurityAlert(
                    alert_id=f"behavior_{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}_{hashlib.md5(str(anomaly).encode()).hexdigest()[:8]}",
                    threat_intelligence=ThreatIntelligence(
    ...
    async def _apply_automated_defenses(self) -> None:
    ...
    async def _execute_defense_action(self, alert: SecurityAlert) -> None:
        ...
        defense_action = DefenseAction(
            action_id=f"defense_{alert.alert_id}_{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}",
    ...
    async def _block_malicious_entities(self, alert: SecurityAlert) -> None:
    ...
    async def _update_firewall_rules(self, entity: str, action: str) -> None:
        """Update firewall rules (placeholder implementation)"""
        logger.info(f"Firewall rule updated: {action} {entity}")

    async def _apply_rate_limiting(self, alert: SecurityAlert) -> None:
    ...
    async def _update_security_rules(self, alert: SecurityAlert) -> None:
    ...
    async def _cleanup_blocked_entities(self) -> None:
        """Clean up blocked entities exceeding retention policy [C2]"""
        self.config["threat_intelligence"]["retention_days"]
    ...
    def generate_threat_intelligence_report(
        self, output_file: str = "security_reports/threat_intelligence.json"
    ) -> None:
