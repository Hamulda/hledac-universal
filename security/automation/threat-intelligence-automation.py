...
class ThreatIntelligenceAutomation:
    ...
    def _initialize_threat_sources(self) -> dict[str, dict[str, Any]]:
        ...
        return {'abuse.ch': {'url': 'https://feodotracker.abuse.ch/downloads/feodotracker.json', 'type': 'malware_domains', 'enabled': True, 'api_key': None}, 'virustotal': {'url': 'https://www.virustotal.com/vtapi/v2', 'type': 'file_reputation', 'enabled': True, 'api_key': '${VIRUSTOTAL_API_KEY}'}, 'alienvault': {'url': 'https://otx.alienvault.com/api/v1', 'type': 'ioc_indicators', 'enabled': True, 'api_key': '${OTX_API_KEY}'}, 'cve': {'url': 'https://services.nvd.nist.gov/rest/json/cves/2.0', 'type': 'vulnerabilities', 'enabled': True, 'api_key': None}}
            ...
            if threat.threat_type in ['malware_domain', 'ioc']:
                for indicator in threat.indicators:
                    if self._matches_app_endpoint(indicator, app_endpoints):
                        alert = SecurityAlert(alert_id=f"app_{threat_id}_{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}", threat_intelligence=threat, affected_assets=app_endpoints, recommended_actions=['Monitor application logs for suspicious activity', 'Enhance endpoint security monitoring', 'Consider IP-based blocking if applicable'], automated_response=self._get_automated_response(threat), timestamp=datetime.now(UTC), status='new')
    ...
    async def _analyze_access_log_for_threats(self, log_file: Path):
        ...
        try:
            suspicious_patterns = ['POST.*login.*403', 'admin.*401', '\\.php.*200', 'union.*select', '<script.*>']
            with open(log_file) as f:
                for line_num, line in enumerate(f, 1):
                    for pattern in suspicious_patterns:
                        if re.search(pattern, line, re.IGNORECASE):
                            alert = SecurityAlert(alert_id=f"log_threat_{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}_{line_num}", threat_intelligence=ThreatIntelligence(threat_id=f'log_pattern_{hashlib.md5(pattern.encode()).hexdigest()[:8]}', threat_type='log_based_threat', severity='medium', source='access_log', indicators=[pattern], description=f'Suspicious pattern detected in access log: {pattern}', first_seen=datetime.now(UTC), last_seen=datetime.now(UTC), confidence=0.6, tags=['log_analysis', 'pattern_matching']), affected_assets=['web_server'], recommended_actions=['Review access logs for similar patterns', 'Investigate source IP addresses', 'Consider rate limiting or blocking'], automated_response='rate_limit', timestamp=datetime.now(UTC), status='new')
                            self.active_alerts[alert.alert_id] = alert
    ...
    async def _analyze_behavior_anomalies(self):
        ...
        try:
            ...
            for anomaly in anomalies:
                alert = SecurityAlert(alert_id=f"behavior_{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}_{hashlib.md5(str(anomaly).encode()).hexdigest()[:8]}", threat_intelligence=ThreatIntelligence(threat_id=f'anomaly_{hashlib.md5(str(anomaly).encode()).hexdigest()[:8]}', threat_type='behavioral_anomaly', severity='medium', source='behavior_analysis', indicators=[str(anomaly)], description=f'Behavioral anomaly detected: {anomaly}', first_seen=datetime.now(UTC), last_seen=datetime.now(UTC), confidence=0.7, tags=['behavior', 'anomaly']), affected_assets=['application'], recommended_actions=['Investigate unusual behavior patterns', 'Review system logs for context', 'Consider temporary monitoring enhancement'], automated_response='enhance_monitoring', timestamp=datetime.now(UTC), status='new')
                self.active_alerts[alert.alert_id] = alert
    ...
    async def _update_security_rules(self, alert: SecurityAlert):
        ...
        try:
            ...
            if security_config_path.exists():
                ...
                config['threat_intelligence'][alert.threat_intelligence.threat_id] = {'type': alert.threat_intelligence.threat_type, 'severity': alert.threat_intelligence.severity, 'indicators': alert.threat_intelligence.indicators, 'last_seen': datetime.now(UTC).isoformat()}
    ...
    def generate_threat_intelligence_report(self, output_file: str='security_reports/threat_intelligence.json'):
        ...
        report = {'timestamp': datetime.now(UTC).isoformat(), 'summary': {'total_threats': len(self.threat_intel_db), 'active_alerts': len([a for a in self.active_alerts.values() if a.status == 'new']), 'defense_actions_applied': len(self.defense_actions), 'blocked_ips': len(self.blocked_entities['ips']), 'blocked_domains': len(self.blocked_entities['domains']), 'rate_limited_entities': len(self.blocked_entities['rate_limited'])}, 'threat_intelligence': {threat_id: {'type': threat.threat_type, 'severity': threat.severity, 'source': threat.source, 'confidence': threat.confidence, 'indicators_count': len(threat.indicators), 'first_seen': threat.first_seen.isoformat(), 'last_seen': threat.last_seen.isoformat(), 'tags': threat.tags} for threat_id, threat in self.threat_intel_db.items()}, 'active_alerts': [{'alert_id': alert.alert_id, 'threat_type': alert.threat_intelligence.threat_type, 'severity': alert.threat_intelligence.severity, 'affected_assets': alert.affected_assets, 'status': alert.status, 'timestamp': alert.timestamp.isoformat(), 'automated_response': alert.automated_response} for alert in self.active_alerts.values()], 'defense_actions': [{'action_id': action.action_id, 'action_type': action.action_type, 'target': action.target, 'confidence': action.confidence, 'impact': action.impact, 'rollback_possible': action.rollback_possible, 'duration_hours': action.duration.total_seconds() / 3600} for action in self.defense_actions[-50:]], 'blocked_entities': {'ips': list(self.blocked_entities['ips']), 'domains': list(self.blocked_entities['domains']), 'rate_limited': list(self.blocked_entities['rate_limited'])}, 'recommendations': self._generate_security_recommendations()}
    ...
    def _generate_security_recommendations(self) -> list[str]:
        ...
        recent_actions = [a for a in self.defense_actions if a.action_id.startswith(f"defense_{datetime.now(UTC).strftime('%Y%m%d')}")]
        if len(recent_actions) > 20:
            recommendations.append('High defensive activity detected - investigate potential attack patterns')
        recommendations.extend(['Regularly update threat intelligence feeds', 'Implement multi-layered security monitoring', 'Conduct regular security assessments', 'Maintain incident response procedures'])
