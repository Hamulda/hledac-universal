    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            return (
                [],
                PassiveDNSOutcome(
                    attempted=True, query=domain, result_count=0, error=f"http_{resp.status_code}", duration_s=elapsed
                ),
            )
        if "error" in text.lower() or "quota" in text.lower() or (not text) or text.startswith("#"):
            ...
            return (
                [],
                PassiveDNSOutcome(
                    attempted=True, query=domain, result_count=0, error="hackertarget_empty", duration_s=elapsed
                ),
            )
        ips: list[str] = []
...
class CIRCLPDNSRecord(Struct, frozen=True):
...
class PassiveDNSOutcome(Struct, frozen=True):
    """
...
async def resolve_doh(
    ...
    session_provider: httpx.AsyncClient | None = None,
...
async def lookup_passive_dns(
    domain: str, session_provider: httpx.AsyncClient | None = None, fetch_func: Callable[..., Any] | None = None
) -> list[str]:
    """
...
async def call_lookup_passive_dns(
    ...
    if not domain or not domain.strip():
        ...
        outcome = PassiveDNSOutcome(
            attempted=True, query=domain, result_count=0, error=None, skip_reason="empty_query", duration_s=elapsed
        )
        return ([], outcome)
    ...
    try:
        if fetch_func is not None:
            ...
            if resp.status_code == 404:
                elapsed = time.monotonic() - start
                outcome = PassiveDNSOutcome(
                    attempted=True, query=domain_stripped, result_count=0, error=None, duration_s=elapsed
                )
            ...
            if status != 200:
                ...
                if status == 401:
                    ...
                    if not outcome.error:
                        ...
                        return (ips, outcome)
                outcome = PassiveDNSOutcome(
                    attempted=True, query=domain_stripped, result_count=0, error=f"http_{status}", duration_s=elapsed
                )
                await asyncio.sleep(CIRCL_RATE_LIMIT_SLEEP)
                return ([], outcome)
            text = resp.text
        ...
        elapsed = time.monotonic() - start
        outcome = PassiveDNSOutcome(
            attempted=True, query=domain_stripped, result_count=0, error="timeout", timeout=True, duration_s=elapsed
        )
        await asyncio.sleep(CIRCL_RATE_LIMIT_SLEEP)
        return ([], outcome)
    except Exception as e:
        elapsed = time.monotonic() - start
        outcome = PassiveDNSOutcome(
            attempted=True, query=domain_stripped, result_count=0, error=str(e), duration_s=elapsed
        )
        await asyncio.sleep(CIRCL_RATE_LIMIT_SLEEP)
        return ([], outcome)
    elapsed = time.monotonic() - start
    outcome = PassiveDNSOutcome(
        attempted=True, query=domain_stripped, result_count=len(ips), error=None, duration_s=elapsed
    )
    await asyncio.sleep(CIRCL_RATE_LIMIT_SLEEP)
    return (ips, outcome)
