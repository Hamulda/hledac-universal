...
class ToolExecLog:
    ...
    def __init__(
        self, run_dir: Path, enable_persist: bool = True, run_id: str = "default", silent_failure: bool = False
    ) -> None:
        """
        ...
        if enable_persist and (not silent_failure):
            self._init_persist()
        logger.info(
            f"ToolExecLog initialized: run_id={run_id}, persist={enable_persist}, silent_failure={silent_failure}"
        )
                    ...
                    if db is not None:
                        try:
                            await db.executemany(
                                "INSERT INTO events (seq_no, event_type, data, hash, ts) VALUES (?, ?, ?, ?, ?)", batch
                            )
                            await db.commit()
                        except Exception as e:
    ...
    def log(
        ...
        if self._closed:
            raise RuntimeError(
                "ToolExecLog.log() called after finalize()/close(): audit trail is closed, refusing to log event"
            )
    ...
    def verify_all(self) -> dict[str, Any]:
        ...
        for event in events:
            if event.prev_chain_hash != expected_head:
                errors.append(
                    f"Chain break at seq {event.seq_no}: expected prev={expected_head}, got {event.prev_chain_hash}"
                )
            ...
            if event.chain_hash != expected_chain:
                errors.append(
                    f"Hash mismatch at seq {event.seq_no}: expected {expected_chain[:16]}..., got {(event.chain_hash or '')[:16]}..."
                )
