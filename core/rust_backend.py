...
# P3-2: Path to FFI manifest — generated by rust_extensions/build_ffi_manifest.py
ABI_OUT = _Path(__file__).parent  # .../hledac/universal/core/rust_backend/
...
class RustBackend:
    ...
    def _try_load_rust_extension(self) -> None:
        ...
        # P3-2: FFI manifest is next to this module (core/rust_backend/_ffi_manifest.json).
        _MANIFEST_PATH = ABI_OUT / "_ffi_manifest.json"
        ...
        try:
            ...
            if ver < _RUST_MIN_VERSION:
                ...
                # P3-2: FFI parity drift check — run once after successful load,
                # before any domain is used. Catches missing Rust symbols that
                # would silently fall through to Python (wrong results, O(n) slower).
                self._check_ffl_parity()
                logger.debug(
    ...
    def _check_ffl_parity(self) -> None:
        """P3-2: Verify Rust extension exports all symbols RustBackend expects.

        Compares _Rust*Domain._spec MethodSpec names (what Python calls)
        against the generated _ffi_manifest.json (what Rust actually exports).

        DRIFT = symbol in _spec but NOT in manifest → silent Python fallback
        at runtime (wrong results, O(n) instead of O(1)). Logs error and
        continues (fail-soft per project invariants).
        """
        ...
        if not manifest_path.exists():
            # Manifest not generated yet — skip (CI should have generated it)
            logger.debug(
                "[FFI parity] _ffi_manifest.json not found; run "
                "rust_extensions/build_ffi_manifest.py to generate"
            )
            return
        ...
        try:
            ...
            logger.warning(f"[FFI parity] could not read manifest: {exc}")
            return

        rust_exports: set[str] = set(manifest.get("all_lib_rs", []))

        # Scan this module source for all MethodSpec("name") strings
        # (both in _spec lists and in module-level Python fallbacks).
        # This is conservative: Python fallback names are also checked
        # against Rust exports, which may trigger some false-positive
        # "extra" reports — but that's intentional for safety.
        with open(__file__, encoding="utf-8") as f:
            src = f.read()

        spec_names: set[str] = set(_re.findall(r'MethodSpec\("(\w+)"', src))

        # Drift: spec names NOT in Rust exports
        missing = spec_names - rust_exports

        if missing:
            missing_sorted = sorted(missing)
            logger.error(
                f"[FFI parity DRIFT] {len(missing_sorted)} symbol(s) expected "
                f"by RustBackend but NOT exported by Rust extension: "
                f"{missing_sorted[:5]}{' ...' if len(missing_sorted) > 5 else ''}"
            )
