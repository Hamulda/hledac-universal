...
NATIVE_DIM = 1024  # BGE-M3 native embedding dimension
MRL_TARGET_DIM = 256  # Truncate to 256d for USEARCH compatibility

# Memory constraints for M1 8GB
MAX_BATCH_SIZE = 32  # Conservative for unified memory
MAX_BATCH_SIZE_LOW = 8  # Low memory mode
MAX_SEQ_LEN = 512  # BGE-M3 max sequence length
...
class BGEBackend(Enum):
    ...
    MLX = auto()        # MLX with Metal GPU (preferred)
    ONNX_CPU = auto()   # ONNX Runtime CPU fallback
    TRANSFORMERS = auto()  # HuggingFace transformers CPU
...
class BGEInferenceResult:
    ...
    embeddings: np.ndarray           # Shape: (batch, native_dim) or (batch, mrl_dim)
    model_dim: int                   # Native model dimension
    truncated_dim: Optional[int]    # If MRL truncation applied
    language: Optional[str]          # Detected language (if provided)
...
class BGEM3Embedder:
    ...
    __slots__ = tuple((
        '_config', '_model', '_processor', '_tokenizer',
        '_backend', '_is_loaded', '_mrl_truncator',
        '_hf_model'  # HuggingFace transformers model (CPU fallback)
    ))
    ...
    def __init__(
        ...
        model_id: str = MODEL_ID,
        mrl_target_dim: int = MRL_TARGET_DIM,
        backend: Optional[BGEBackend] = None,
        lazy_load: bool = True,
        batch_size: int = MAX_BATCH_SIZE,
        batch_size_low: int = MAX_BATCH_SIZE_LOW
    ):
    ...
    def _detect_backend(self) -> BGEBackend:
        ...
        # Try MLX first (Apple Silicon GPU)
    ...
    def _load_mlx(self) -> bool:
        ...
        try:
            ...
            # MLX-embedding-models supports BGE-M3
            # Note: BGE-M3 may require custom loader if not in registry
            try:
                self._model = EmbeddingModel.from_registry(self._config.model_id)
            except ValueError:
                # BGE-M3 not in registry, try direct loader
