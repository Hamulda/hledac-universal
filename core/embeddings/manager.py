...
class MLXEmbeddingManager:
    ...
    def encode_adaptive(self, texts: str | list[str], initial_batch_size: int=32, min_batch_size: int=4, max_batch_size: int=128, normalize: bool=True, truncate_dim: int | None=None, memory_pressure_provider: Callable[[], float] | None=None) -> np.ndarray:
        """
    ...
    def _write_prewarm_marker(self, model_name: str) -> None:
        try:
            ...
            with _PREWARM_LOCK:
                ...
                marker_path.write_text(msgspec.json.encode({'model': model_name, 'loaded_at': time.time(), 'version': 1}).decode())
