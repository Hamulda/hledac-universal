...
try:
    ...
    _msgspec_decode = None
    _msgspec_encode = None
...
class CacheStats:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'hits': self.hits, 'misses': self.misses, 'l1_hits': self.l1_hits, 'l2_hits': self.l2_hits, 'evictions': self.evictions, 'memmap_errors': self.memmap_errors, 'encode_errors': self.encode_errors, 'hit_rate': round(self.hit_rate, 4)}
...
class EmbeddingCache:
    ...
    """
    VERSION = 1
    __slots__ = tuple(('_file_size', '_l1', '_l1_lock', '_memmap_path', '_meta_path', '_mmap', '_mmap_lock', 'cache_dir', 'dim', 'max_bytes', 'max_entries', 'stats'))

    def __init__(self, dim: int=256, max_entries: int=_MAX_ENTRIES, max_bytes: int=_MAX_BYTES, cache_dir: Path | None=None):
        self.dim = dim
        ...
        async with self._mmap_lock:
            try:
                if self._memmap_path.exists():
                    ...
                    self._mmap = np.memmap(str(self._memmap_path), dtype=np.float16, mode='r+', offset=_HEADER_SIZE, shape=(self._max_shape()[0], self.dim))
    ...
    async def _create_memmap(self) -> None:
        ...
        try:
            with open(self._memmap_path, 'wb') as f:
                header = {'version': self.VERSION, 'dim': dim, 'max_entries': max_entries, 'free_list': list(range(max_entries)), 'used': 0}
                header_bytes = or_msgspec_dumps_str(header) if orjson else str(header).encode()
                header_bytes = header_bytes.ljust(_HEADER_SIZE, b'\x00')
                f.write(header_bytes)
                f.seek(total_size - 1)
                f.write(b'\x00')
            self._mmap = np.memmap(str(self._memmap_path), dtype=np.float16, mode='r+', offset=_HEADER_SIZE, shape=(max_entries, dim))
        ...
        try:
            if self._meta_path.exists():
                ...
                for slot_idx in used_slots:
                    ...
                    self._l1[f'_slot_{slot_idx}'] = CacheEntry(offset=offset, length=self.dim, mtime=meta.get('slot_mtimes', {}).get(str(slot_idx), 0), text_hash=f'_slot_{slot_idx}')
        except Exception as e:
    ...
    async def _save_meta(self) -> None:
        try:
            ...
            meta = {'version': self.VERSION, 'dim': self.dim, 'max_entries': self._max_shape()[0], 'free_list': free_slots, 'used': len(self._l1), 'slot_mtimes': slot_mtimes}
        ...
        try:
            ...
            if self._mmap is not None:
                ...
                if hasattr(self._mmap, 'flush'):
                    self._mmap.flush()
            entry = CacheEntry(offset=slot_idx * self.dim * 2, length=self.dim, mtime=asyncio.get_running_loop().time(), text_hash=text_hash)
            async with self._l1_lock:
