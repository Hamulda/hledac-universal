    ...
    async with semaphore:
        try:
                ...
                if query.startswith("ipfs://") or query.startswith("ipns://"):
                    ...
                    if content:
                        ...
                        return (
                            [finding],
                            AltProtocolResult(
                                source_type=SourceType.IPFS_CONTENT, findings_count=1, success=True, error=None
                            ),
                        )
                    return (
                        [],
            ...
            return (
                findings,
                AltProtocolResult(
                    source_type=SourceType.IPFS_CONTENT, findings_count=len(findings), success=True, error=None
                ),
            )
        except TimeoutError:
            return (
                [],
                AltProtocolResult(
                    source_type=SourceType.IPFS_CONTENT, findings_count=0, success=False, error="timeout"
                ),
            )
            ...
            return (
                [],
                AltProtocolResult(source_type=SourceType.IPFS_CONTENT, findings_count=0, success=False, error=str(e)),
            )
    ...
    async with semaphore:
        try:
            ...
            return (
                findings,
                AltProtocolResult(
                    source_type=SourceType.GOPHER_CONTENT, findings_count=len(findings), success=True, error=None
                ),
            )
        except TimeoutError:
            return (
                [],
                AltProtocolResult(
                    source_type=SourceType.GOPHER_CONTENT, findings_count=0, success=False, error="timeout"
                ),
            )
        except Exception as e:
            ...
            return (
                [],
                AltProtocolResult(source_type=SourceType.GOPHER_CONTENT, findings_count=0, success=False, error=str(e)),
            )
    ...
    async with semaphore:
        try:
            ...
            return (
                findings,
                AltProtocolResult(
                    source_type=SourceType.GEMINI_CONTENT, findings_count=len(findings), success=True, error=None
                ),
            )
        except TimeoutError:
            return (
                [],
                AltProtocolResult(
                    source_type=SourceType.GEMINI_CONTENT, findings_count=0, success=False, error="timeout"
                ),
            )
        except Exception as e:
            ...
            return (
                [],
                AltProtocolResult(source_type=SourceType.GEMINI_CONTENT, findings_count=0, success=False, error=str(e)),
            )
    ...
    async with semaphore:
        try:
            ...
            return (
                findings,
                AltProtocolResult(
                    source_type=SourceType.I2P_DISCOVERY, findings_count=len(findings), success=True, error=None
                ),
            )
        except TimeoutError:
            return (
                [],
                AltProtocolResult(
                    source_type=SourceType.I2P_DISCOVERY, findings_count=0, success=False, error="timeout"
                ),
            )
        except Exception as e:
            ...
            return (
                [],
                AltProtocolResult(source_type=SourceType.I2P_DISCOVERY, findings_count=0, success=False, error=str(e)),
            )
...
async def _fetch_from_zeronet(query: str, semaphore: asyncio.Semaphore) -> tuple[list, AltProtocolResult]:
    ...
    if not _ZERONET_ENABLED:
        return (
            [],
            AltProtocolResult(source_type=SourceType.ZERONET, findings_count=0, success=True, error="zeronet_disabled"),
        )
        ...
        try:
            ...
            if not available:
                return (
                    [],
                    AltProtocolResult(
                        source_type=SourceType.ZERONET, findings_count=0, success=True, error="zeronet_unavailable"
                    ),
                )
            ...
            return (
                findings,
                AltProtocolResult(
                    source_type=SourceType.ZERONET_CONTENT, findings_count=len(findings), success=True, error=None
                ),
            )
        except TimeoutError:
            return (
                [],
                AltProtocolResult(
                    source_type=SourceType.ZERONET_CONTENT, findings_count=0, success=False, error="timeout"
                ),
            )
        except Exception as e:
            ...
            return (
                [],
                AltProtocolResult(
                    source_type=SourceType.ZERONET_CONTENT, findings_count=0, success=False, error=str(e)
                ),
            )
...
async def _fetch_from_freenet(query: str, semaphore: asyncio.Semaphore) -> tuple[list, AltProtocolResult]:
    ...
    if not _FREENET_ENABLED:
        return (
            [],
            AltProtocolResult(source_type=SourceType.FREENET, findings_count=0, success=True, error="freenet_disabled"),
        )
        ...
        try:
            ...
            if not available:
                return (
                    [],
                    AltProtocolResult(
                        source_type=SourceType.FREENET, findings_count=0, success=True, error="freenet_unavailable"
                    ),
                )
            ...
            return (
                findings,
                AltProtocolResult(
                    source_type=SourceType.FREENET_CONTENT, findings_count=len(findings), success=True, error=None
                ),
            )
        except TimeoutError:
            return (
                [],
                AltProtocolResult(
                    source_type=SourceType.FREENET_CONTENT, findings_count=0, success=False, error="timeout"
                ),
            )
        except Exception as e:
            ...
            return (
                [],
                AltProtocolResult(
                    source_type=SourceType.FREENET_CONTENT, findings_count=0, success=False, error=str(e)
                ),
            )
        ...
        try:
            ...
            try:
                ...
                return (
                    findings,
                    AltProtocolResult(
                        source_type=SourceType.FEDIVERSE, findings_count=len(findings), success=True, error=None
                    ),
                )
            finally:
                await adapter.close()
            ...
            return (
                [],
                AltProtocolResult(source_type=SourceType.FEDIVERSE, findings_count=0, success=False, error="timeout"),
            )
        except Exception as e:
            logger.debug(f"Fediverse alt fetch error: {e}")
            return (
                [],
                AltProtocolResult(source_type=SourceType.FEDIVERSE, findings_count=0, success=False, error=str(e)),
            )
        ...
        try:
            ...
            try:
                ...
                return (
                    findings,
                    AltProtocolResult(
                        source_type=SourceType.MATRIX_PUBLIC, findings_count=len(findings), success=True, error=None
                    ),
                )
            finally:
                await adapter.close()
            ...
            return (
                [],
                AltProtocolResult(
                    source_type=SourceType.MATRIX_PUBLIC, findings_count=0, success=False, error="timeout"
                ),
            )
        except Exception as e:
            ...
            return (
                [],
                AltProtocolResult(source_type=SourceType.MATRIX_PUBLIC, findings_count=0, success=False, error=str(e)),
            )
...
async def fetch_all_alt_protocols(
    query: str, max_concurrent: int | None = None
    ...
    logger.info(
        f"Alt protocols: {len(all_findings)} findings from {sum(1 for r in protocol_results if r.success)} protocols"
    )
...
def get_alt_protocols_status() -> dict:
    ...
    return {
        ...
        "protocols": {
            ...
            "i2p": {
                "enabled": True,
                "gate": "HLEDAC_ENABLE_ALT_PROTOCOLS",
                "requires_daemon": True,
                "concurrency": I2P_CONCURRENCY,
            },
            "zeronet": {
                "enabled": _ZERONET_ENABLED,
                "gate": "HLEDAC_ENABLE_ZERONET",
                "requires_daemon": True,
                "concurrency": ZERONET_CONCURRENCY,
            },
            "freenet": {
                "enabled": _FREENET_ENABLED,
                "gate": "HLEDAC_ENABLE_FREENET",
                "requires_daemon": True,
                "concurrency": FREENET_CONCURRENCY,
            },
