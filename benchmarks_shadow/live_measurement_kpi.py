...
def _derive_live_kpi_from_input(inp: LiveKpiInput) -> dict:
    ...
    if nonfeed_attempted_families == 'CANONICAL_FIELD_MISSING' and inp.run_quality_verdict == RunQualityVerdict.PASS_VALID_CAPABILITY_RUN.value and active_runtime_occurred and (feed_findings > 0) and (nonfeed_findings == 0) and (not starvation_suppressed):
        nonfeed_starvation_suspected = True
        nonfeed_starvation_reason = 'canonical_field_missing_indeterminate'
    if not nonfeed_starvation_suspected and inp.run_quality_verdict == RunQualityVerdict.PASS_VALID_CAPABILITY_RUN.value and nonfeed_attempted_families and active_runtime_occurred and (feed_findings > 0) and (nonfeed_findings == 0) and (not starvation_suppressed):
        ...
        if _public_not_timed and _public_disc == 0 and (_public_fetch == 0) and (_public_ts in ('ERROR', 'DISCOVERY_ERROR')):
            nonfeed_starvation_suspected = True
