...
def get_uma_snapshot() -> dict[str, Any]:
    ...
    try:
        ...
        s = sample_uma_status()
        return {'system_used_gib': s.system_used_gib, 'swap_used_gib': s.swap_used_gib, 'swap_detected': s.swap_detected, 'state': s.state, 'io_only': s.io_only}
    except Exception as exc:
        return {'error': str(exc), 'state': 'unknown'}

def _make_canned_entry(idx: int) -> dict[str, Any]:
    return {'entry_url': f'https://feed-{idx}.example.com/cve-{idx}', 'title': f'CVE-2026-{idx:04d}: Critical Vulnerability in Component-{idx}', 'summary': f'Vulnerability CVE-2026-{idx:04d} allows remote attackers to execute arbitrary code via crafted input to Component-{idx}. Severity: critical. Affects versions 1.0 through {idx % 10 + 1}.0.', 'published': '2026-04-24T00:00:00Z', 'source': 'test-source'}
...
def _canned_match_text(text: str) -> list[dict[str, Any]]:
    ...
    for i in range(count_cves_in_text(text)):
        findings.append({'ioc_type': 'cve', 'ioc_value': f'CVE-2026-{1000 + i:04d}', 'confidence': 0.9, 'source': 'test-pattern'})
    return findings
...
async def run_hermetic_sprint(duration_s: float=DEFAULT_DURATION_S) -> BenchmarkResult:
    ...
    async def _sprint_loop() -> None:
        ...
        while time.monotonic() - start_time < duration_s:
            ...
            if rss_peak_mb > M1_8GB_CEILING_MB:
                break
    await parallel([_sprint_loop()], taskgroup=True, policy='raise', ctx='benchmark:sprint_loop')
    ...
    return BenchmarkResult(status=status, duration_s=elapsed_s, cycles=cycle_count, findings_total=findings_total, findings_accepted=findings_accepted, acceptance_ratio=findings_accepted / findings_total if findings_total > 0 else 0.0, rss_peak_mb=rss_peak_mb, uma_state_summary=state_counts, model_lease_count=snap.model_denied_count, renderer_denied_count=snap.renderer_denied_count, fetch_limit_at_end=fetch_limit, findings_per_minute=findings_per_min, timestamp=datetime.now(UTC).isoformat())

def _format_summary(result: BenchmarkResult) -> str:
    """Format benchmark result as human-readable summary."""
    lines = ['=' * 60, 'M1 Sustained Sprint Benchmark — Hermetic', '=' * 60, f'  Status:               {result.status}', f'  Duration:             {result.duration_s:.1f}s ({result.duration_s / 60:.1f}min)', f'  Cycles:               {result.cycles}', f'  Findings (total):     {result.findings_total}', f'  Findings (accepted): {result.findings_accepted}', f'  Acceptance ratio:    {result.acceptance_ratio:.2%}', f'  Findings/min:        {result.findings_per_minute:.1f}', f'  RSS peak:            {result.rss_peak_mb:.0f} MB ({result.rss_peak_mb / 1024:.2f} GiB)', f'  UMA states:          {result.uma_state_summary}', f'  Model denied count:  {result.model_lease_count}', f'  Renderer denied:     {result.renderer_denied_count}', f'  Fetch limit (end):   {result.fetch_limit_at_end}', f'  Timestamp:           {result.timestamp}', '=' * 60]
    return '\n'.join(lines)
...
def main() -> int:
    ...
    parser.add_argument('--duration', type=float, default=DEFAULT_DURATION_S, help=f'Sprint duration in seconds (default: {DEFAULT_DURATION_S})')
    parser.add_argument('--output', type=str, default=DEFAULT_OUTPUT_PATH, help='Optional JSON output path')
