...
def make_result(idx: int, url: str | None=None, title: str='', content: str='') -> RankedResult:
    return RankedResult(id=f'result_{idx}', title=title or f'Result {idx} Title', content=content or f'Content for result {idx}', url=url, source='benchmark', score=1.0 / idx if idx > 0 else 1.0)
...
def make_duplicate_cluster(base_idx: int, cluster_size: int, vary_content: bool=True) -> list[RankedResult]:
    ...
    for i in range(cluster_size):
        if vary_content:
            ...
            title = base_title
        results.append(make_result(base_idx * 10 + i, url=base_url if i == 0 else f'{base_url}?v={i}', title=title, content=content))
    return results
...
def build_synthetic_dataset(size: int, dup_ratio: float=0.3) -> list[RankedResult]:
    ...
    for i in range(unique_count):
        results.append(make_result(i, url=f'https://unique.example.com/item{i}', title=f'Unique Document {i}', content=f'This is unique content for document {i} with ID {i}'))
...
def benchmark_remove_duplicates(sizes: list[int], num_runs: int=3) -> list[BenchmarkResult]:
    ...
    for size in sizes:
        ...
        results.append(BenchmarkResult(size=size, time_seconds=round(avg_time, 4), dedup_count=total_deduped // num_runs, output_count=total_output // num_runs))
...
def run_benchmark(args) -> dict:
    ...
    benchmark_data = {'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'), 'sizes_tested': sizes, 'num_runs': num_runs, 'results': [{'size': r.size, 'time_seconds': r.time_seconds, 'dedup_count': r.dedup_count, 'output_count': r.output_count} for r in results], 'complexity_analysis': complexity, 'hotspot': '_remove_duplicates O(n²) pairwise similarity', 'optimization': 'cached normalized token sets, exact URL match early exit'}
...
def main():
    ...
    parser.add_argument('--output-json', default='probe_f214opt_ranking_dedup/benchmark.json', help='JSON output path')
    parser.add_argument('--output-md', default='probe_f214opt_ranking_dedup/BENCHMARK.md', help='Markdown output path')
