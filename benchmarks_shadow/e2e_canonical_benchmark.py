...
HERMETIC_SIDECAR_COUNT: int = 11
SYNTHETIC_ACCEPT_RATE: float = 0.7
SIDECAR_LIGHT_LOAD_MS: float = 5.0
...
def _make_synthetic_finding(index: int, query: str='benchmark query') -> dict[str, Any]:
    ...
    return {'finding_id': f'bench-f205e-{index:04d}', 'query': query, 'source_type': 'ct_log', 'confidence': 0.5 + index % 10 * 0.05, 'ts': _time.time(), 'provenance': (f'hermetic_bench::{index}',), 'payload_text': json.dumps({'triage': {'title': f' synthetic finding {index}', 'author': 'bench'}, 'ioc_type': 'domain', 'ioc_value': f'example-{index:04d}.com'}), 'ioc_type': 'domain', 'ioc_value': f'example-{index:04d}.com'}

class MockTargetMemory:
    """Synthetic TargetMemory for hermetic benchmark."""
    __slots__ = tuple(('confidence_drift', 'cumulative_finding_count', 'entity_facets', 'exposure_facets', 'first_seen_ts', 'last_seen_ts', 'pivot_facets', 'sprint_count', 'target_id', 'updated_by_sprint_id'))

    def __init__(self, target_id: str, sprint_count: int=1, cumulative_finding_count: int=0) -> None:
        ...
        self.pivot_facets = {}
        self.confidence_drift = {'sprints': 1, 'total_findings': 0, 'avg_findings_per_sprint': 0.0, 'drift_ratio': 1.0}
...
class MockDuckDBStore:
    ...
    """
    __slots__ = tuple(('_accept_rate', '_seen_ids', '_stored', '_total_accepted', '_total_submitted'))
    ...
    async def async_ingest_findings_batch(self, findings: list[Any]) -> list[dict[str, Any]]:
        """
        ...
        for f in findings:
            ...
            if fid in self._seen_ids:
                ...
                fdict = dict(f) if isinstance(f, dict) else {k: getattr(f, k, None) for k in ('finding_id', 'query', 'source_type', 'confidence', 'ts', 'provenance', 'payload_text', 'ioc_type', 'ioc_value')}
    ...
    async def async_get_target_memory(self, target_id: str) -> MockTargetMemory | None:
        ...
        return MockTargetMemory(target_id=target_id, sprint_count=2, cumulative_finding_count=stored * 2)

    async def aclose(self) -> None:
        self._stored.clear()
        self._seen_ids.clear()

async def _light_runner(findings: list, store: MockDuckDBStore, query: str, *, delay_ms: float=SIDECAR_LIGHT_LOAD_MS) -> None:
    """
...
async def _run_hermetic_benchmark(num_findings: int=HERMETIC_MAX_FINDINGS, runs: int=HERMETIC_DEFAULT_RUNS) -> dict[str, Any]:
    """
    ...
    """
    from hledac.universal.runtime.sidecar_bus import SIDECAR_STAGES, FindingSidecarBus, SidecarBatch
    ...
    hermetic_target_id = hermetic_query
    for run_idx in range(runs):
        ...
        batch = SidecarBatch(sprint_id=f'bench-f205e-run{run_idx}', query=hermetic_query, source_branch='ct', findings=tuple(findings_list), created_ts=_time.time())
        t0 = _time.monotonic()
        ...
        try:
            ...
            brief = await workbench.build_sprint_brief(sprint_id=f'bench-f205e-run{run_idx}', target_id=hermetic_target_id, findings=findings_list, graph_signal={'graph_nodes': 0, 'graph_edges': 0}, governor=None, duckdb_store=store)
        ...
        run_metric = {'run': run_idx, 'submitted_findings': len(findings_list), 'accepted_findings': accepted, 'stored_findings': stored, 'dedup_ratio': round(dedup_ratio, 4), 'findings_per_minute': round(findings_per_min, 2), 'sidecar_total_ms': round(sidecar_total_ms, 2), 'per_sidecar_ms': {k: round(v, 2) for k, v in per_sidecar.items()}, 'peak_rss_mb': round(rss_peak, 1), 'rss_delta_mb': round(rss_delta, 1), 'memory_ceiling_ok': rss_peak < M1_8GB_CEILING_MB}
    ...
    for name in sorted(all_names):
        ...
        per_sidecar_agg[name] = {'avg_ms': round(sum(vals) / n, 2), 'min_ms': round(min(vals), 2), 'max_ms': round(max(vals), 2)}
    return {'metadata': {'probe': 'F205E', 'timestamp': datetime.now(UTC).isoformat().replace('+0000', 'Z'), 'mode': 'hermetic', 'runs': runs, 'uvloop_active': _UVLOOP_ACTIVE, 'python_version': sys.version.split()[0], 'hermetic_num_findings': num_findings, 'hermetic_accept_rate': SYNTHETIC_ACCEPT_RATE, 'hermetic_sidecar_load_ms': SIDECAR_LIGHT_LOAD_MS}, 'runs': run_metrics, 'aggregate': {'findings_per_minute': round(avg_fpm, 2), 'dedup_ratio': round(avg_dedup, 4), 'sidecar_total_ms': round(avg_sidecar_ms, 2), 'stored_count': round(avg_stored, 1), 'accepted_count': round(avg_accepted, 1), 'peak_rss_mb': round(avg_rss, 1), 'memory_ceiling_ok': all_memory_ok, 'target_memory_summary_present': target_memory_summary_present, 'analyst_brief_includes_memory': analyst_brief_includes_memory}, 'per_sidecar_ms': per_sidecar_agg, 'status': 'pass' if all_memory_ok else 'fail'}

async def run_benchmark(hermetic: bool=True, runs: int=HERMETIC_DEFAULT_RUNS, output_path: Path | None=None) -> dict[str, Any]:
    ...
    t0 = _time.monotonic()
    if hermetic:
        result = await _run_hermetic_benchmark(num_findings=HERMETIC_MAX_FINDINGS, runs=runs)
    else:
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Sprint F205E — Hermetic E2E Canonical Benchmark')
    parser.add_argument('--hermetic', action='store_true', default=True, help='Hermetic mode: synthetic data, no network/MLX (default: True)')
    parser.add_argument('--runs', type=int, default=HERMETIC_DEFAULT_RUNS, help=f'Number of benchmark runs (default: {HERMETIC_DEFAULT_RUNS})')
    parser.add_argument('--output', type=str, default=None, help='Output JSON path')
    args = parser.parse_args()
    output_path = Path(args.output) if args.output else None
    result = asyncio.run(run_benchmark(hermetic=args.hermetic, runs=args.runs, output_path=output_path))
