...
async def _run_single_sprint(query: str) -> dict[str, Any]:
    ...
    # run_sprint returns None — it writes results to ~/.hledac/reports
    await run_sprint(
        query=query,
        duration_s=10,  # short enough for benchmarks
...
async def _run_single_sprint_unsafe(query: str) -> dict[str, Any]:
    ...
    except Exception as exc:  # pragma: no cover — defensive, production code uses broad catch
    ...
    async def run(
        ...
        *,
        ...
        for i in range(iterations):
            ...
            # Merge wall-clock and RSS into the result
            row: dict[str, Any] = {
                "iteration": i + 1,
                "latency_s": iter_wall,
            ...
            if result.get("error"):
                ...
                row["latency_s"] = None  # ensure null on error
        ...
        report: dict[str, Any] = {
            ...
            "latency_s": _stats(latency_vals),
            "rss_delta_mb": _stats(rss_vals),
            "findings_count": _stats(findings_vals) if findings_vals else {"p50": 0.0, "p95": 0.0, "p99": 0.0, "mean": 0.0, "std": 0.0, "min": 0.0, "max": 0.0},
            "iterations_detail": raw_results,
            "error_count": sum(1 for r in raw_results if r.get("error")),
        }
