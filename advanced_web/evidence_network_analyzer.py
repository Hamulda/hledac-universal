...
MAX_GRAPH_PAYLOAD_SCAN: int = 8 * 1024
_RE_GRAPH_IPV4 = re.compile('\\b(?:\\d{1,3}\\.){3}\\d{1,3}\\b')
_RE_GRAPH_SHA256 = re.compile('\\b[a-f0-9]{64}\\b')
_RE_GRAPH_SHA1 = re.compile('\\b[a-f0-9]{40}\\b')
_RE_GRAPH_MD5 = re.compile('\\b[a-f0-9]{32}\\b')
_RE_GRAPH_CVE = re.compile('\\bCVE-\\d{4}-\\d{4,7}\\b', re.IGNORECASE)
_RE_GRAPH_DOMAIN = re.compile('\\b(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\\.)+[a-z]{2,}\\b', re.IGNORECASE)
_NEGATION_CUES: frozenset[str] = frozenset({'not', 'no', 'never', 'without', 'denies', 'denied', 'refutes', 'refuted', 'disproves', 'disproved', 'fake', 'hoax', 'false'})
_EXCLUSIVE_PAIRS: tuple[tuple[str, str], ...] = (('online', 'offline'), ('enabled', 'disabled'), ('active', 'inactive'), ('vulnerable', 'patched'), ('exposed', 'hidden'), ('public', 'private'), ('encrypted', 'plaintext'), ('legitimate', 'malicious'), ('benign', 'malicious'), ('safe', 'unsafe'), ('allowed', 'blocked'), ('open', 'closed'))
_RE_DATE = re.compile('\\b(\\d{4})(?:-(\\d{1,2})(?:-(\\d{1,2}))?)?\\b')
_RE_NUMERIC = re.compile('\\b(\\d+(?:\\.\\d+)?)\\s*([a-zA-Z%]+)?\\b')
_RE_TOKEN = re.compile('[a-z0-9][a-z0-9._/-]{1,40}')
...
def _looks_like_domain(value: str) -> bool:
    ...
    if not s or ' ' in s:
        return False
    return bool(re.match('^[a-z0-9.-]+\\.[a-z]{2,}$', s))
...
def _coerce_entity(entity: Any) -> dict[str, Any] | None:
    ...
    if not isinstance(sources, list):
        sources = []
    return {'type': etype, 'value': value, 'sources': [str(s)[:128] for s in sources[:8]]}
...
def _build_ig_graph(ig_mod: Any, entities: list[dict[str, Any]]) -> Any:
    ...
    for e in entities:
        ...
        g.add_vertex(key, etype=e['type'], value=e['value'], sources=tuple(e.get('sources', [])))
    return g

def _compute_relationships(entities: list[dict[str, Any]], threshold: float) -> list[dict[str, Any]]:
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if weight <= 0.0:
                continue
            edges.append({'src': keys[i], 'dst': keys[j], 'weight': round(float(weight), 4), 'type': rel_type})
...
def _dedupe_edges(edges: list[EvidenceGraphEdge]) -> list[EvidenceGraphEdge]:
    ...
    for e in edges:
        ...
        if key in seen:
            prev = seen[key]
            seen[key] = EvidenceGraphEdge(src=prev.src, dst=prev.dst, rel_type=prev.rel_type, weight=max(prev.weight, e.weight), evidence_count=prev.evidence_count + e.evidence_count)
        else:
            seen[key] = e
...
class EvidenceNetworkAnalyzer:
    ...
    __slots__ = tuple(('_call_count', '_graph', '_initialized', '_last_args_count', '_last_graph_size'))
    ...
    async def detect_contradictions(self, evidence_a: dict[str, Any] | None, evidence_b: dict[str, Any] | None) -> dict[str, Any] | None:
        """
    ...
    async def calculate_centrality(self, network: dict[str, Any] | None) -> dict[str, float]:
        """
    ...
    async def analyze_evidence_network(self, query: str, confidence_threshold: float=0.5, priority: int=5) -> dict[str, Any]:
        """
        ...
        try:
            ...
            return {'networks': [{'query': q, 'entities': net.get('entities', []), 'edges': net.get('edges', []), 'clusters': net.get('clusters', []), 'centrality': net.get('centrality', {}), 'contradictions': net.get('contradictions', [])}], 'confidence': float(net.get('confidence', 0.0)) * float(confidence_threshold or 0.5), 'priority': int(priority), 'not_implemented': False, 'call_count': self._call_count}
        except Exception as e:
    ...
    async def analyze(self, findings: list[Any], max_hops: int=MAX_GRAPH_HOPS) -> EvidenceGraph:
        ...
        try:
            ...
            for iocs in finding_iocs:
                for ioc_type, value, src in iocs:
                    ...
                    if key in node_map:
                        ...
                        new_sources = tuple(dict.fromkeys(list(existing.sources) + ([src] if src else [])))[:8]
                        node_map[key] = EvidenceGraphNode(node_id=f'{ioc_type}:{value}', ioc_type=ioc_type, value=value, confidence=existing.confidence, sources=new_sources)
                    else:
                        node_map[key] = EvidenceGraphNode(node_id=f'{ioc_type}:{value}', ioc_type=ioc_type, value=value, confidence=0.5, sources=(src,) if src else ())
                ...
                if len(iocs) >= 2:
                    ...
                    for other in iocs[1:]:
                        if anchor[0] == other[0] and anchor[1] == other[1]:
                            continue
                        edges.append(EvidenceGraphEdge(src=f'{anchor[0]}:{anchor[1]}', dst=f'{other[0]}:{other[1]}', rel_type='co_occurrence', weight=0.6, evidence_count=1))
            ...
            confidence = sum((e.weight for e in edges)) / len(edges) if edges else 0.0
            self._last_graph_size = len(nodes) + len(edges)
            return EvidenceGraph(nodes=tuple(nodes), edges=tuple(edges), confidence=round(float(confidence), 4), finding_count=finding_count)
        except Exception as e:
    ...
    async def _query_connected_async(self, nodes: list[EvidenceGraphNode], max_hops: int) -> list[EvidenceGraphEdge]:
        ...
        for src_value, conn_list in connected_map.items():
            ...
            for c in conn_list:
                ...
                edges.append(EvidenceGraphEdge(src=f'unknown:{src_value[:MAX_VALUE_LEN]}', dst=f'{ioc_type}:{dst_value[:MAX_VALUE_LEN]}', rel_type='graph_connected', weight=round(weight, 4), evidence_count=1))
                if len(edges) >= MAX_GRAPH_EDGES:
                    return edges
        return edges
    ...
    def _extract_iocs_from_finding(self, f: Any) -> list[tuple[str, str, str]]:
        ...
        try:
            ...
            for ioc_type, pattern in (('cve', _RE_GRAPH_CVE), ('hash_sha256', _RE_GRAPH_SHA256), ('hash_sha1', _RE_GRAPH_SHA1), ('hash_md5', _RE_GRAPH_MD5), ('ip', _RE_GRAPH_IPV4), ('domain', _RE_GRAPH_DOMAIN)):
    ...
    async def cleanup(self) -> None:
        ...
        logger.debug('EvidenceNetworkAnalyzer: cleaned up (%d calls served)', self._call_count)

    def is_implemented(self) -> bool:
        """Return True — implementation is live (post T1)."""
        return not self._NOT_IMPLEMENTED

    def _coerce_entities(self, entities: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
